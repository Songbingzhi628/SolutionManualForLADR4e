...
}Because in general, $q\circ (p_1+\lambda p_2)(x)=q\left(p_1(x)+\lambda p_2(x)\right)\neq (q\circ p_1)(x)+\lambda (q\circ p_2)(x).$\par\qquad\qquad\;\;\,
An example: let $q$ be defined by $q(x)=x^2,$ then $q\circ\left(1+(-1)\right)=0\neq q(1)+q(-1)=2.\hfill\square$\par
\ProblemEnding\par
...
\ProblemB{
	\TextB{Suppose $p$ is a poly of $n$ variables in $\Fbb$. Prove that $\Mt\left(p(T_1,\dots,T_n)\right)=p\left(\Mt(T_1),\dots,\Mt(T_n)\right).$}
...
Hence by mathematical induction, we conclude that $\displaystyle\sum\limits_{k_1,\dots,k_n}\alpha_{k_1,\dots,k_n}\Mt\left(\prod\limits_{i=1}^n T_i^{k_i}\right)=\sum\limits_{k_1,\dots,k_n}\alpha_{k_1,\dots,k_n}\prod\limits_{i=1}^n \Mt(T_i^{k_i}).$
\ProblemEnding\par
...
\ProblemN{13}{
	...
	\TextNL{Explain why $AB + AC$ and $DF + EF$ both make sense and prove the distr.}
}Using [3.36], [3.43].\par\quad
For left distr. Suppose $A\in\Fbb^{m,n}$ and $B,C\in\Fbb^{n,p}.$\par\quad
Because $[A(B+C)]_{j,k}=\sum\limits_{r=1}^n A_{j,r}(B+C)_{r,k}=\sum\limits_{r=1}^n (A_{j,r}B_{r,k}+A_{j,r}C_{r,k}).$\par\quad
Hence we conclude that $A(B+C)=AB+AC.$ Similarly, $(D+E)F=DF+EF.\hfill\square$\par\vspace{6pt}\quad
...
Similar arguments will prove the right distr.\vspace{10pt}\par\quad
\Or $A(B+C)=\Mt(T(S+R))\overset{[3.9]}{=}\Mt(TS+TR)=AB+AC.$\par\qquad\;\,
$(D+E)F=\Mt((T+S)R)\overset{[3.9]}{=}\Mt(TR+SR)=DF+EF.\hfill\square$\par
\ProblemEnding\par
...
\Or Suppose $A\in\Fbb^{m,n},B\in\Fbb^{n,p},C\in\Fbb^{p,q}$.\par\quad
Let $(e_1,\dots,e_m)$ be the standard basis of $\Fbb^m$, $(f_1,\dots,f_n)$ be the standard basis of $\Fbb^n.$\par\quad
Suppose $T\in\Lm(\Fbb^n,\Fbb^m)$ such that $Tf_k=\sum\limits_{j=1}^m A_{j,k}e_j$ for each $k=1,\dots,n$. Then $\Mt(T)=A.$\par\quad
Similarly, define $S,R$ such that $\Mt(S)=B,\Mt(R)=C.$\vspace{4pt}\par\quad
Hence \vspace{-8pt}\AlignEq{}{(TS)R=T(SR)&\Rightarrow \Mt((TS)R)=\Mt(T(SR))\hspace{200pt}\\&\Rightarrow\left[\Mt(T)\Mt(S)\right]\Mt(R)=\Mt(T)\left[\Mt(S)\Mt(R)\right]\\&\Rightarrow (AB)C=A(BC).}
\vspace{4pt}\par\quad
\Or $(AB)C=\Mt(T(SR))\overset{[3.9]}{=}\Mt(TSR)\overset{[3.9]}{=}\Mt((TS)R)=A(BC).\hfill\square$\vspace{8pt}\par
\ProblemEnding\par
