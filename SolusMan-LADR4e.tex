...
\newcommand{\ExampleX}[1]{
	{\tgbfx E\footnotesize{XAMPLE:}} #1\vspace{-16pt}\par
	\Solution
}
...
[{\tgsl Existns}] \dbsp Let $x=\displaystyle\frac{1}{3}(w-v)$.\par\quad
[{\tgsl Uniqnes}] \dbsp Suppose $v+3x_1=w,$(I)$\,\,v+3x_2=w\,$(II). Then (I) $-$ (II) $: 3(x_1-x_2)=0\Rightarrow x_1=x_2.\hfill\square$\par\quad
...
\TextN{\large\tgnr(I) $t + \infty = \infty + t = \infty + \infty = \infty,$}
\TextN{\EndI\large\tgnr(II) $t + (-\infty) = (-\infty) + t = (-\infty) + (-\infty) = -∞,$}
\TextN{\EndII\large\tgnr(III) $\infty + (-\infty) = (-\infty) + \infty = 0$.}
...
$\MathRightBrace{r}{
	...
	2[\TXT{\tgbf{I}}]-[\TXT{\tgbf{II}}]:\qquad\qquad\hspace{-1pt} \cos(x)=\cos(x+p)}\Rightarrow$ Let $x=0,$\;\,$ p=\displaystyle\frac{m\pi}{\sqrt{2}}=2k\pi.$\; Contradicts.$\hfill\square$\vspace{10pt}\par
\SepLine\par\vspace{10pt}
...
(b){$\MathRightBrace{r}{f_e\in V_E\Longleftrightarrow f_e(x)=f_e(-x)\Longleftarrow$  let $f_e(x)=\displaystyle\frac{g(x)+g(-x)}{2}\vspace{4pt}\\ f_o\in V_O\Longleftrightarrow f_o(x)=-f_o(-x)\Longleftarrow$ let $f_o(x)=\displaystyle\frac{g(x)-g(-x)}{2}}\Rightarrow \forall g\in\Rbb^\Rbb,g(x)=f_e(x)+f_o(x).\hfill\square$}\vspace{10pt}\par
\SepLine\par
\ChEnd
...
\vspace{-10pt}
\AlignEq{}{\qquad\qquad\qquad Q\Rightarrow P:\;&a_1 v_1+a_2 v_2+a_3 v_3+a_4 v_4=0\\\Rightarrow\;&a_1(v_1-v_2)+(a_1+a_2)(v_2-v_3)+(a_1+a_2+a_3)(v_3-v_4)+(a_1+\dots+a_4)v_4=0\\\Rightarrow\;&a_1=a_1+a_2=a_1+a_2+a_3=a_1+\dots+a_4=0.}\PfEnd[-20pt]
\SepLine\par
%
%\ProblemN{7}{
...
\AlignEq{}{\qquad\quad Q\Rightarrow P:\;\;&a_1 v_1+\dots+a_m v_m=0=b_1 w_1+\dots+b_m w_m=0, \TXT{\;where\;} 0=b_1=a_1,\;0=b_k=\sum\limits_{j=1}^k (-1)^{k-j}a_j
...
Now $w=\lambda^{-1}(a_1 v_1+\dots+a_{j-1}v_{j-1}-v_j)\Rightarrow w\in\Spn(v_1,\dots,v_m).\hfill\square$\vspace{-6pt}\vspace{6pt}\par
\SepLine\par
...
}\hspace{-7pt}$\begin{array}{l}{}^\neg Q\Rightarrow{}^\neg P:$ Suppose $w\in\Spn(v_1,\dots,v_m)$. Then $(v_1,\dots,v_m,w)$ is linely depe.$\\{}^\neg P\Rightarrow{}^\neg Q:$ Suppose $(v_1,\dots,v_m,w)$ is linely dep. Then by [2.21] $w\in\Spn(v_1,\dots,v_m).\end{array}\hfill\square$\vspace{8pt}\par
\SepLine\par
...
$Q\Rightarrow P:\;\forall v\in V,\,\exists\,!\,b_i\in\Fbb,\;v=b_1w_1+\dots+b_mw_m\Rightarrow v=a_1v_1+\dots+a_mv_m,\,\exists\,!\,a_k=\sum\limits_{j=1}^k b_j.\hfill\square$\vspace{3pt}\par
\SepLine\par
...
{\Large$\Rightarrow{\large\Spn}(x^m,\dots,x,1)={\large\Spn}\underbrace{(p_m,\dots,p_1,p_0)}_\TXT{Basis}.$\hfill$\square$
...
\Comment \,\,\,{\tgsl\Large$\dim(V_1+\dots+V_m)=\dim V_1+\dots+\dim V_m\Longleftrightarrow V_1+\dots+V_m$ is a direct sum.}\par\qquad\qquad\qquad
...
{\tgbfx Step k.} Suppose $(v_1,\dots,v_{k-1})$ is linely inde in $V,$ each of which belongs to $V\backslash U.$\TextB{}\quad\qquad\;
...
$T(0,1,0,\dots,0,0)=(A_{1,2},\dots,A_{m,2}),$\qquad Then by [3.5], we are done.\PfEnd\qquad\qquad\quad\quad\,\,
$\vdots$\par\quad\quad\,\,\,
...
$}$ \Or Define $T(x,y)=\sqrt[3]{(x^3+y^3)}.\hfill\square$\vspace{8pt}\par
\SepLine\par
...
\Onumber{3} \TextN{ Suppose $(v_1,\dots,v_m)$ in V. Define $T\in\Lm(\Fbb^m, V)$ by $T(z_1,\dots,z_m)=z_1 v_1+\dots+z_m v_m.$}
	...
	(b) \Largesl{The inje of $T$ corresponds to $(v_1,\dots,v_m)$ being linely inde.}\par
\SepLine\par
...
$a_1 Tv_1+\dots+a_n Tv_n=0=T(\sum\limits_{i=1}^n a_i v_i)\Longleftrightarrow \sum\limits_{i=1}^n a_i v_i=0\Longleftrightarrow a_1=\dots=a_n=0.\hfill\square$\vspace{4pt}\par
\SepLine\par
...
(b) $\forall\,w\in\range T,\,\exists\,v\in V,w=Tv.\,(\,\exists\,\,a_i\in\Fbb,v=a_1 v_1+\dots+a_n v_n)\Rightarrow w=a_1 Tv_1+\dots+a_n Tv_n.\hfill\square$\par
\SepLine\par
...
Getting $V\subseteq\Spn(v_1,\dots,v_n,u_1,\dots,u_m)$. Thus $V$ is finite-dim.\PfEnd
\SepLine\par
...
(\TXT a)\,\,T(\sum\limits_{i=1}^n a_i v_i)=0\Rightarrow \sum\limits_{i=1}^n a_i Tv_i=0\Rightarrow a_1=\dots=a_n=0\Rightarrow\mathcal{K}_R\cap\null T=\{0\}.\\
(\TXT b)\,\,\forall v\in V, Tv=\sum\limits_{i=1}^n a_i Tv_i\Rightarrow Tv-\sum\limits_{i=1}^n a_i Tv_i=T(v-\sum\limits_{i=1}^n a_i v_i)=0\\
...
\centerline{\quad where $n=\dim\range T$ if finite-dim, otherwise $n\in\Nbp.$}\par\quad\Hb
Then $\mathcal{K}_R\oplus\null T=V.$ Define $S\in\Lm(W,V)$ by $S(Tv_i)=v_i.$ Then $TS=I.\hfill\square$\vspace{4pt}\par
\SepLine\par
\pagebreak
...
又 $\dim\null ST=5\leq\dim\null S+\dim\null T\leq 4.$ Contradicts.\PfEnd[0pt]\quad
...
\AlignEq{}{\TXT{And\,}ST=0&\Rightarrow \dim\range TS\leq\dim\range T\leq\dim\null S\leq n-\dim\range TS\qquad\qquad\qquad\qquad\quad\hspace{2pt}\\&\Rightarrow 2\dim\range TS\leq n\Rightarrow\dim\range TS\leq\displaystyle\frac{n}{2}\\&\Rightarrow\dim\range TS\leq\left\lfloor\displaystyle\frac{n}{2}\right\rfloor \TXT{\,(\,because\,}\dim\range TS\TXT{\,is\,an\,integer\,)}.}\PfEnd
\SepLine\par
...
Hence $\forall v\in V,\,\,(\,\exists\,!\,a_i\in\Fbb,u\in\null S\,),\,Tv=a_1 Tv_1+\dots+a_n Tv_n=E(a_1 Sv_1+\dots+a_n Sv_n)\Rightarrow T=ES.$\PfEnd[-6pt]\quad
...
$\varphi_1(v)=a_v\varphi_1(u),\,\,\,\varphi_2(v)=a_v\varphi_2(u)\Rightarrow a_v=\displaystyle\frac{\varphi_1(v)}{\varphi_1(u)}=\frac{\varphi_2(v)}{\varphi_2(u)}\Rightarrow\frac{\varphi_1(u)}{\varphi_2(u)}=\frac{\varphi_1(v)}{\varphi_2(v)}=c\in\Fbb.\hfill\square$\vspace{2pt}\vspace{4pt}\par
\SepLine\par
...
{Hence $\null\varphi$ is a nonzero two-sided ideal of $\Lm(V).\hfill\square$}\par
\SepLine\par
...
$}\Rightarrow\range T=Y.\hfill\square$\vspace{10pt}\par
\SepLine\par

\ProblemB{
	\TextB{Suppose $V$ is finite-dim and $T\in\Lm(V,W)$. Let $(Tv_1,\dots,Tv_n)$ be a basis of $\range T$.}
	\TextB{Extend $(v_1,\dots,v_n)$ to a basis of $V$ as $(v_1,\dots,v_n,u_1,\dots,u_m)$.}
...
&\therefore(AC)_{j,\cdot}=A_{j,\cdot}C_{\cdot,\cdot}=A_{j,\cdot}C.}\vspace{-25pt}\par$\hfill\square$\vspace{-4pt}\par
\SepLine\par
...
$\therefore\,aC=a_{1,\cdot}C_{\cdot,\cdot}=\sum\limits_{r=1}^n a_{1,r}C_{r,\cdot}=a_1 C_{1,\cdot}+\dots+a_n C_{n,\cdot}$\quad \Or By $(aC)_{1,\cdot}=a_{1,\cdot}C$. Using (b) above.$\hfill\square$\vspace{2pt}\par
\SepLine\par
...
\vspace{-6pt}\par$\hfill\square$\vspace{-4pt}\par
\SepLine\par
...
Then $\displaystyle\Mt\left(p(T_1,\dots,T_n)\right)=\Mt\left(\sum\limits_{k_1,\dots,k_n}\alpha_{k_1,\dots,k_n}\prod\limits_{i=1}^n T_i^{k_i}\right)=\sum\limits_{k_1,\dots,k_n}\alpha_{k_1,\dots,k_n}\prod\limits_{i=1}^n \Mt(T_i^{k_i})=p\left(\Mt(T_1),\dots,\Mt(T_n)\right).\hfill\square$\vspace{10pt}\par
...
\TXT{\Or}\,\,\,\,(AAA)_{j,k} &=\sum\limits_{r=1}^n(AA)_{j,r}A_{r,k} =\sum\limits_{r=1}^n(\sum\limits_{p=1}^n A_{j,p}A_{p,r})A_{r,k}\\&=\sum\limits_{r=1}^n\left[A_{j,1}(A_{1,r}A_{r,k})+\dots+A_{j,n}(A_{n,r}A_{r,k})\right]\\&=A_{j,1}\sum\limits_{r=1}^n A_{1,r}A_{r,k}+\dots+A_{j,n}\sum\limits_{r=1}^n A_{n,r}A_{r,k}=\sum\limits_{p=1}^n \sum\limits_{r=1}^n A_{j,p}A_{p,r}A_{r,k}.\hfill\square}\par
\SepLine\par
...
& \ddots \\
0 & & \lambda
\end{pmatrix}$}$\,=\,\Mt\left(\lambda I,(v_1,\dots,v_n)\right)\Rightarrow S=\Mt^{-1}\left(\Mt(\lambda I)\right)=\lambda I.\hfill\square$\vspace{6pt}\par
\SepLine\par
...
\ProblemNnosolus{1}{
	\TextN{A function $T:V\rightarrow W$ is linear $\Longleftrightarrow$ $T$ is a subspace of $V\times W.$\vspace{-10pt}}
}\SepLine\vspace{6pt}\par
...
Thus $\pm(v-x)\in U\cap W \Rightarrow\MathLeftBrace{l}{u_1=(x-v)+w_1\in W\Rightarrow U\subseteq W\\w_2=(v-x)+u_2\in U\Rightarrow W\subseteq U}\Rightarrow U=W.\hfill\square$\vspace{5pt}\par
\SepLine\par
...
\ProblemBnosolus{
	...
	\TextB{[Do it in your mind.]\vspace{-10pt}}
}\SepLine\par
...
Thus $A\apostrophe$ is a subsp of $V$. Hence $a+A\apostrophe=\left\{(x-a)+a:x\in A\right\}=A$ is a translate.$\hfill\square$\vspace{8pt}\par\quad
\Or 
\SepLine\par
...
\ProblemNnosolus{13}{
	...
	\Ha\TextNL{{\large\tgnr For any $(x,y,z)\in\Rbb^3$, $\left(T\apostrophe(\varphi_1)\right)(x,y,z)=4x+5y+6z, \left(T\apostrophe(\varphi_2)\right)(x,y,z)=7x+8y+9z$.}}
	(b) \TextNL{\large Write $T\apostrophe(\varphi_1)$ and $T\apostrophe(\varphi_2)$ as linear combinations of $\psi_1,\psi_2,\psi_3$.}
	\Hb\TextNL{{\large\tgnr$T\apostrophe(\varphi_1)=4\psi_1+5\psi_2+6\psi_3,\,\,T\apostrophe(\varphi_2)=7\psi_1+8\psi_2+9\psi_3.\hfill\square$}}
}\SepLine\par
