...
Because $\dim\null TE_2\geqslant\dim U\Rightarrow\dim\range TE_2\leqslant\dim V-\dim U.$\par\quad
...
\Comment \,\,\,Conversely, by reversing the roles of $U$ and $W,$ we conclude that it is true as well.\vspace{6pt}\par\quad
...
By (3.F.9), $T\apostrophe\Par{\varphi_k}=\sum_{j=1}^n A_{j,k}\varphi_j.$ \Corollary \,\,\,$\Mt{T\apostrophe}=A=\Mt{T}{^t}.$\vspace{3pt}\par\quad\Hb
...
Let $U=\Span{v,u_1,\dots,u_{k-1}}\Rightarrow U$ is not an invar subsp of $V$ under $T.$\PfEnd\vspace{4pt}\quad
\Or Suppose $0\neq v=v_1\in V$ and extend to $B_V=\Par{v_1,\dots,v_n}.$\par\quad
Suppose $Tv_1=c_1 v_1+\dots+c_n v_n,\exists\,!\,c_i\in\Fbb.$ Consider a $k$\hspace{1pt}-\hspace{1pt}$\dim$ subsp $U=\Span{v_1,v_{\alpha_1},\dots,v_{\alpha_{k-1}}}.$\par\quad
Where $\alpha_j\in\Bra{2,\dots,n}$ for each $j$, and $\alpha_1,\dots,\alpha_{k-1}$ are distinct. Because every subsp such $U$ is invar.\par\quad
Thus $Tv_1=c_1 v_1+\dots+c_n v_n\in U\Longrightarrow c_2=\dots=c_n=0.$\vspace{-10pt}\par\quad
For if not, suppose $c_i\neq 0,$ let $W=\Span{v_1,v_{\beta_1},\dots,v_{\beta_{k-1}}},$ where each $\beta_j\in\Bra{\overbrace{2,\dots,i-1,i+1,\dots,n}^{\text{of length }\SmallPar{n-2}\;\geqslant\;k\,-\,1}}.$\par\quad
Hence $Tv_1=c_1 v_1.$ Because $v_1=v\in V$ is arbitrary. We conclude that $T=\lambda I$ for some $\lambda\in\Fbb.$\PfEnd\vspace{4pt}\quad
...
}Let $\lambda_1,\dots,\lambda_m$ be the distinct eigvals of $T$ with correspd eigves $v_1,\dots,v_m.$\par
\Blind{\Solution} \BigPar{ Because $\range T$ is finite-dim. The correspd eigvals are finite. }\par
\Blind{\Solution} Then $\Par{v_1,\dots,v_m}$ linely inde $\Longrightarrow\Par{\lambda_1 v_1,\dots,\lambda_m v_m}$ linely inde, if each $\lambda_k\neq 0.$\par
\Blind{\Solution} Otherwise, $\exists\,!\,\lambda_k=0.$ Now $\Par{\lambda_1v_1,\dots,\lambda_{k-1}v_{k-1},\lambda_{k+1}v_{k+1},\dots,\lambda_mv_m}$ is linely inde.\par
\Blind{\Solution} Hence, by [2.23], $m-1\leqslant\dim\range T.$\PfEnd
\SepLine
