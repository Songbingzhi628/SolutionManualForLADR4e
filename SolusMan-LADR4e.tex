...
\Comment \,\,\,There is no guarantee that $W$ is a vecsp. \BigPar{ although the minimal vecsp $\zeroSubs\subseteq W$ }\par
\Blind{\Comment\,\,\,}Is it guaranteed that $V$ is a vecsp? In other words,\par
\Blind{\Comment\,\,\,}if $V$ is closed under add and scalar multi, then why, or why not, is $V$ a vecsp?\par
\SepLine
...
{\Large\vspace{-30pt} Define $E_{i,j}\in\Lm{V}$ by $E_{i,j}\Par{v_x}=\delta_{ix} w_i$.}\par\quad
...
For $u=\Par{x_1,\cdots,x_p,\cdots}\in\Fbb^{\infty}$, {\large denote} $x_p$ {\large by} $u[p]$. {\large For each} $r\in\Nbp$.\par\quad
{\large Define} $e_r[p]=\MathLeftBrace{l}{1\,,\Par{p-1}\equiv 0\,\BigPar{\TXT{mod}\,r}\\0\,,\TXT{otherwise}}$, {\large simply} $e_r=\Par{1,\underbrace{\;0,\;\,\cdots,\;\,0\,}_{\SmallPar{p-1}\,\,times},1,\underbrace{\;0,\;\,\cdots,\;\,0\,}_{\SmallPar{p-1}\,\,times},1,\cdots}\in\Fbb^{\infty}$.\par\quad
...
Then suppose $u=\Par{x_1,\cdots,x_t,0,\cdots}\Rightarrow u[t+i]=0,\,\forall i\in\Nbp$,\par\quad
then let $j=s\cdot m!+1\geqslant t\,\,\Par{\exists\,\,s\in\Nbp}$ {\large so that} $e_1[j]=\cdots=e_m[j]=1,\,\,u[j+i]=0.$\par\vspace{6pt}\quad
Now we have: $u[j+i]=\XPar{\sum_{r=1}^m a_r e_r}[j+i]=\sum_{r=1}^m a_r e_r\Sbra{s\cdot m!+1+i}=a_{i_1}+\dots+a_{i_{\tau\SmallPar{i}}}=0$,\par\vspace{6pt}\quad
$\Rightarrow\XPar{\sum_{r=1}^m a_r e_r}\Sbra{j+i}=a_{i_1}+\dots+a_{i_{\tau\SmallPar{i}}}=0$.\,\,\,$\Par{\Delta}$\par\quad
...
{\large\XPar{\;Note that by definition, $e_r[s\cdot m!+1+i]=1\Longleftrightarrow s\cdot m!+i\equiv i\equiv 0\,\Par{\,\TXT{mod}\,r\,}\Longleftrightarrow r\mid i.$\;}}\par\vspace{6pt}\quad
...
{\large Again by $\Par{\Delta}$,} $\XPar{\sum_{r=1}^m a_r e_r}[j+i\apostrophe]=a_{i\apostrophe_1}+\dots+a_{i\apostrophe_{\tau\SmallPar{i\apostrophe}}}=a_{i_1}+\dots+a_{i_{\tau\SmallPar{i}-1}}=0.$\par\vspace{6pt}\quad
...
又 $e_i\not\in U \Rightarrow \Par{e_1+U,e_2+U,\cdots}$ is linely inde in $\Fbb^{\infty}/U.$ {\large By [2.B.14]}. \PfEnd\large
\SepLine\pagebreak
...
%$v+U\in\Span{w_1+U,\dots,w_m+U}\Rightarrow V/U\subseteq\Span{w_1+U,\dots,w_m+U}$.\par\quad
%Now suppose $a_1\Par{w_1+U}+\dots+a_m\Par{w_m+U}=0+U\Rightarrow\sum_{i=1}^m a_i w_i\in U$ while $U\cap W=\zeroSubs$.\par\quad
%Then $\sum_{i=1}^m a_i w_i=0\Rightarrow a_1=\dots=a_m=0.$\PfEnd
\SepLine
...
$\Rightarrow\Span{v_1,\dots,v_m}\oplus U=V.$ By (2.B.8).\PfEnd
\SepLine
...
to a basis $\Par{w_1+U,\dots,w_m+U}$ of $V/U$ and let $W=\Span{w_1,\dots,w_m}.$\PfEnd
\SepLine
...
\ProblemBnosolus{
	...
	\Blind{\Corollary\,\,\,}\TextB{Then $\Par{\sum_{\alpha_i\in\Gamma}V_{\alpha_i}}${$^{0}$}$\,=\bigcap_{\alpha_i\in\Gamma}\Par{V^0_{\alpha_i}}$;\quad And $\Par{\bigcap_{\alpha_i\in\Gamma}V_{\alpha_i}}${$^{0}$}$\,=\sum_{\alpha_i\in\Gamma}\Par{V^0_{\alpha_i}}$.}
}\SepLine
