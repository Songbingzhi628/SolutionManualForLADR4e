...
\ProblemN{24}{
	\TextNL{Let $V_E=\left\{\,f\in\Rbb^{\Rbb}:f(x)=f(-x)\right\},V_O=\left\{\,f\in\Rbb^{\Rbb}:-f(x)=f(-x)\right\}.$}
	\TextNL{Show that $V_E\oplus V_O=\Rbb^\Rbb$.}
}\par\quad
(a) { $V_E\cap V_O=\left\{\,f\in\Rbb^{\Rbb}:f(x)=f(-x)=-f(-x)\right\}=\{0\}.$}\par\vspace{6pt}\quad
...
\ProblemB{
	\TextB{Suppose $(v_1,\dots,v_m)$ is a list of vecs in $V$. For $k\in\{1,\dots,m\}$, let $w_k=v_1+\dots+v_k$.}
...
\ProblemN{13}{
	...
	\TextNL{Prove that there exists a list $w_1,\dots,w_m\in W$}
	\TextNL{such that $\not\exists\,T\in\Lm(V, W),Tv_k=w_k,\forall k = 1,\dots,m$.}
}\par\quad
...
{\Large\vspace{6pt}\dbsp Suppose for any list $(w_1,\dots,w_m)\in W,\,\exists\,T\in\Lm(V,W),Tv_k=w_k$  for each $w_k.$}\par\qquad\quad
{\Large\vspace{6pt}\dbsp (We need to) Prove that $(v_1,\dots,v_n)$ is linely inde.}\par\quad
...
{\Large\vspace{6pt}Then $\sum_{k=1}^m\left|a_k\right|^2=0\Rightarrow a_k=0$ for each $k.$ This contradicts the linely dep of $(v_1,\dots,v_n).\hfill\square$}\par
\ProblemEnding\par
\ChapterEnding
\pagebreak
...
}Using \TIPS\;in $2.C$.\vspace{-8pt}\par
...
&\Longleftrightarrow(A_{1,\cdot},\dots,A_{n,\cdot})\text{ is a basis of }\Fbb^{1,n}.}\vspace{-25pt}\par$\hfill\square$\par\quad
Now we show that $(\Delta)$ properly.\par\quad
$(a)\,\Rightarrow(b):\;$\par\quad\Ha
...
By (2.C.???), $\exists\,B_W=(w_1,\dots,w_m)$ such that $w=w_1+\dots+w_m.$\par\quad\Hb
By (2.C.???), $\exists$ a basis $(u_1,\dots,u_n)$ of $V$ such that $u_k\not\in\null T.$\par\quad\Hb
...
Suppose the poly $p$ is defined by $p(x_1,\dots,x_n)=\displaystyle\sum\limits_{k_1,\dots,k_n}\alpha_{k_1,\dots,k_n}\prod\limits_{i=1}^n x_i^{k_i}.$\par\quad
Then $\displaystyle\Mt\left(p(T_1,\dots,T_n)\right)=\sum\limits_{k_1,\dots,k_n}\alpha_{k_1,\dots,k_n}\prod\limits_{i=1}^n T_i^{k_i}=\sum\limits_{k_1,\dots,k_n}\alpha_{k_1,\dots,k_n}\Mt\left(\prod\limits_{i=1}^n T_i^{k_i}\right).$\par\quad
Note that $\Mt(T^x S^y)=\Mt(T)^x\Mt(S)^y$. Hence $\displaystyle\Mt\left(p(T_1,\dots,T_n)\right)=\sum\limits_{k_1,\dots,k_n}\alpha_{k_1,\dots,k_n}\Mt(T)^{k_1}\Mt\left(\prod\limits_{i=2}^n T_i^{k_i}\right).$\par\quad
And for any $k\geq 1,$ $\displaystyle\Mt\left(p(T_1,\dots,T_n)\right)=\sum\limits_{k_1,\dots,k_n}\alpha_{k_1,\dots,k_n}\left(\prod\limits_{i=1}^k\Mt(T)^{k_i}\right)\Mt\left(\prod\limits_{i=k+1}^n T_i^{k_i}\right).$\par\quad
...
Suppose $A, B, C, D, E,F$ are matrices such that $A(B + C)$ and $(D + E)F$ make sense, we prove the distr.\par\quad
$A(B+C)=\Mt(T(S+R))\overset{[3.9]}{=}\Mt(TS+TR)=AB+AC.$\par\quad
$(D+E)F=\Mt((T+S)R)\overset{[3.9]}{=}\Mt(TR+SR)=DF+EF.\hfill\square$\vspace{8pt}\par\quad
\Or Using [3.36], [3.43]. For left distr. Suppose $A\in\Fbb^{m,n}$ and $B,C\in\Fbb^{n,p}.$\par\quad
Because $[A(B+C)]_{j,k}=\sum\limits_{r=1}^n A_{j,r}(B+C)_{r,k}=\sum\limits_{r=1}^n (A_{j,r}B_{r,k}+A_{j,r}C_{r,k}).$\par\quad
Hence we conclude that $A(B+C)=AB+AC.$ \; Similarly, $(D+E)F=DF+EF.\hfill\square$\par\vspace{8pt}\quad
\Or Let $(e_1,\dots,e_m)$ be the standard basis of $\Fbb^m$, $(f_1,\dots,f_n)$ be the standard basis of $\Fbb^n.$\par\quad
Suppose $T\in\Lm(\Fbb^n,\Fbb^m)$ such that $Tf_k=\sum\limits_{j=1}^m A_{j,k}e_j$ for each $k=1,\dots,n$. Then $\Mt(T)=A.$\par\quad
...
$\Rightarrow A(B+C)=AB+AC.\end{array}\right.$\vspace{4pt}\par\quad
Similar arguments will prove the right distr.$\hfill\square$\par
\ProblemEnding\par
...
$(AB)C=\Mt(T(SR))\overset{[3.9]}{=}\Mt(TSR)\overset{[3.9]}{=}\Mt((TS)R)=A(BC).\hfill\square$\vspace{8pt}\par\quad
\Or Because $[(AB)C]_{j,k}=(AB)_{j,\cdot}C_{\cdot,k}=\sum\limits_{s=1}^n(A_{j,s}B_{s,\cdot})C_{\cdot,k}=\sum\limits_{s=1}^n A_{j,s}(B_{s,\cdot}C_{\cdot,k})=\sum\limits_{s=1}^n A_{j,s}(BC)_{s,k}=A(BC)_{j,k}$\par\quad
Hence we conclude that $(AB)C=A(BC).\hfill\square$\par\vspace{8pt}\quad
\Or Similar to Problem (13), define $T,S,R$ such that $\Mt(T)=A,\Mt(S)=B,\Mt(R)=C.$\par\quad
Hence \vspace{-8pt}\AlignEq{}{(TS)R=T(SR)\Rightarrow& \Mt((TS)R)=\Mt(T(SR))\hspace{200pt}\\\Rightarrow&\left[\Mt(T)\Mt(S)\right]\Mt(R)=\Mt(T)\left[\Mt(S)\Mt(R)\right]\\\Rightarrow& (AB)C=A(BC).}\vspace{-25pt}\par$\hfill\square$\par
\ProblemEnding\par
\pagebreak
...
\ProblemB{
	\TextB{Prove that the commutativity does not hold in $\Fbb^{m,n}.$}
...
\ProblemB{
	\TextB{Suppose $T\in\Lm(V),v_1,\dots,v_m\in V$ such that $V=\Spn(Tv_1,\dots,Tv_m)$.}
...
\ProblemB{
	\vspace{2pt}\TextB{Suppose $T\in\Lm(V, W)$ is inv. Show that $T^{-1}$ is inv and $(T^{-1})^{-1}=T$.\vspace{8pt}}
}$\MathRightBrace{l}{
...
\Example\,\,\,$V=\Rbb^\infty,S(a_1,a_2,\dots)=(a_2,\dots);\;T(a_1,\dots)=(0,a_1,\dots);\;U=I\Rightarrow STU=I$ but $T^{-1}$ is not inv.\par
\ProblemEnding\par
...
\ProblemB{
	\TextB{Suppose $V,W$ are finite-dim, $U$ is a subsp of $V$. Let $\mathcal{E} = \left\{T\in\Lm(V, W): U\subseteq\null T\right\}$.}
...
\ProblemBc{
	\TextB{Suppose $V$ is finite-dim and $S\in\Lm(V)$. Define $A\in \Lm\left(\Lm(V)\right)$ by $\mathcal{A}(T) = ST,\forall T\in\Lm(V)$.}
...
\ChapterDecl{Ch5BI}{5.B: I}{
	\quad{\LARGE[}
	{\Large\timessl See {\timesbf 5.B: II} below.}
	{\LARGE ]}
}\large\vspace{5pt}
