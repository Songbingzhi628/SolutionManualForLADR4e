...
{\huge\timesbf 5.B: I}\quad {\LARGE[}{\Large\timessl See {\timesbf 5.B: II} below.}{\LARGE ]} % 1h+(1h+1h/8h)
$\\\\ $
...
{\small $\bullet$} {\timessl\Large 
Suppose $V$ is a complex vector space and $T\in\Lm(V)$ has no eigvals.}\par\,\,
{\timessl\Large Prove that every subspace of \,$V$ invariant under $T$ is either $\{0\}$ or infinite-dim.
}\par
{\timesbf S\footnotesize{OLUTION:}}\par\quad
...
{\small $\bullet$} (a) {\timessl\Large Give an example of $S, T\in\Lm(${\timesbf F}$^2)$ such that}\par\qquad
...
{\small $\bullet$} {\timessl\Large 
Suppose $a_0 ,\dots, a_{n-1}\in\Fbfc.$ Let $T$ be the operator on $\Fbfc^n$ whose matrix (with respect to the standard basis) is
}\par\,\,
{\timessl\Large
}\par
{\timesbf S\footnotesize{OLUTION:}}\par\quad
