...
\ProblemBnoor[]{\Tips}[]{
	\TextB{About the Field $\Fbb$: Many choices. \Example \,\,\,$\Fbb=\Zbb_m=\Bra{K_0,K_1,\dots,K_{m-1}},\forall m-1\in\Nbp.$}
}\SepLine
\ChEnd
\pagebreak
...
{\Large\vspace{6pt}Because $U_2,U_3$ are subsps and hence have at least one element \Par{ zero }.}\par\quad\Hb\HII
{\Large\vspace{6pt}If $U_2=U_3,$ then $\mathcal{U}=U_1\cup U_2$ and by Problem (12) we are done.}\par\quad\Hb\HII
{\Large\vspace{6pt}\Par{ Note that at least one of $U_2,U_3$ is not $\zeroSubs,$ for if not, $U_1\supseteq U_2\cup U_3.$ }}\par\quad\Hb\HII
...
\ProblemB{
	\Tips \,\,\,\TextB{Suppose $V_1\subseteq V_2$ in Exercise {\tgnr(23)}. Prove or give a counterexample: $V_1=V_2.$}
}\par\quad
...
\ChDecl{Ch3A}{3$\cdot$A}{}\orMode{\hLk{3A3}{3}\;\;\hLk{3A4}{4}\;\;\hLk{3A5}{5}\;\;\hLk{3A7}{7}\;\;\hLk{3A8}{8}\;\;\hLk{3A10}{10}\;\;\hLk{3A11}{11}\;\;\hLk{3A12}{12}\;\;\hLk{3A13}{13}\;\;|\;\;\hLk{3A4e10}{4E: 10,}\;\;\hLk{3A4e11}{11,}\;\;\hLk{3A4e16}{16}}{[1]: 5; [2]: 3, 4, 7, 8, 9, (4E 10), 10; [3]: 11, 12, 13, (4E 16); [4]: (4E 11).}
...
\ProblemB{
	\hypertarget{3A4e10}{}\TextB{Prove that if $q\in\PoR{}$ and $T:\PoR{}\rightarrow\PoR{}$ is defined by $Tp=q\circ p,$ then $T$ is not linear.\vspace{4pt}}
}\par\quad
...
\ProblemBnoor{\hypertarget{3A4e16}{4E 3.A.16}}{
	\TextB{}
...
\colorbox{yellow}{TODO: More Exercises and Notes.}
...
\BulletPoint \,\hspace{1pt}\NoteForSmall{[3.88, 3.90, 3.91]} Suppose $W\in\mathcal{S}_V U$ Then $V/U$ and $W$ are iso.\TextB{}
...
$[P]\Rightarrow[Q]:\;$ Notice that $\varphi$ is inje and by (3.B.9).\vspace{2pt}\par\quad
...
\Blind{$[P]\Rightarrow[Q]:\;$} Then $\vartheta\Par{1}=0=a_1 v_1+\dots+a_m v_m\Rightarrow a_1=\dots=a_m=0.$\vspace{4pt}\par\quad
$[Q]\Rightarrow[P]:\;$ Suppose $v\in\Span{v_1,\dots,v_m}.$ Let $v=0=a_1 v_1+\dots+a_m v_m.$\par\quad
\Blind{$[Q]\Rightarrow[P]:\;$} Then $\varphi\Par{v}=0=a_1\varphi\Par{v_1}+\dots+a_m\varphi\Par{v_m}\Rightarrow a_1=\dots=a_m=0.$\PfEnd
\SepLine\pagebreak
...
%for if not, $\exists\,j,k\in\Nbp$ such that $A_{j,k}\neq 0$, then $T\Par{A}_{k,j}\neq 0$, contradicts.\PfEnd\vspace{-4pt}
%\SepLine

\ProblemN{\hypertarget{3F32}{32}}{
	\TextNL{Suppose $T\in\Lm{V}$, and $B_V=\Par{u_1,\dots,u_m},B{_V'}=\Par{v_1,\dots,v_m}.$}
	\TextNL{Prove that $T$ is inv $\Longleftrightarrow$ the rows of $\Mt[\BigPar]{T,B_V,B{_V'}}$ form a basis of $\Fbb^{1,n}$.}
}Note that $T$ is inveritlbe $\Longleftrightarrow$ $T\apostrophe$ is inv. And $\Mt{T\apostrophe}=\Mt{T}{^t}=A^t$, denote it by $B$.\par\quad
Let $\Par{\varphi_1,\dots,\varphi_m}$ be the dual basis of $B{_V'};$\; Let$\Par{\psi_1,\dots,\psi_m}$ be the dual basis of $B_V.$\par\quad
...
\NOTICE that $T\apostrophe\Par{\varphi_i}=B_{1,i}\psi_1+\dots+B_{m,i}\psi_m.$\par\quad\Ha
...
\Blind{\Or }\Or Let $B_V=\Par{\underbrace{u_1, \dots,u_m}_{B_U}, v_1, \dots, v_n}$ with $n\geqslant 1$. Let $B_{V\apostrophe}=\Par{\psi_1,\dots,\psi_m,\varphi_1,\dots,\varphi_n}.$ Let $\varphi=\varphi_i$.\vspace{2pt}\par\quad
\Blind{\Or }\Blind{\Or }\Or Define $\varphi\in V\apostrophe$ by $\varphi\Par{u_1}=\dots=\varphi\Par{u_m}=0$ and $\varphi\Par{v_1}=\dots=\varphi\Par{v_n}=1.$\PfEnd\vspace{4pt}
\Comment \,\,\,Another proof of [3.108]: $T$ is surj $\Longleftrightarrow T\apostrophe$ is inje.\par
...
\ProblemN{\hypertarget{3F9}{9}}{
	\TextN{Suppose $B_V=\Par{v_1,\dots,v_n},\,B_{V\apostrophe}=\Par{\varphi_1,\cdots,\varphi_n}$.}
	\TextN{Prove that $\forall \psi \in V\apostrophe,\psi=\psi\Par{v_1}\varphi_1+\dots+\psi\Par{v_n}\varphi_n.$}
	\Corollary \,\,\,For $B_V'=\Par{u_1,\dots,u_n},B_{V\apostrophe}'=\Par{\rho_1,\dots,\rho_n},\forall\psi\in V\apostrophe,\psi=\psi\Par{u_1}\rho_1+\dots+\psi\Par{u_n}\rho_n.$\TextB{}
}\par\quad
...
Define $i\in\Lm{V,U}$ by $i\Par{v}=u_v.$ So that $\Gamma\Par{\varphi\circ i}=\Par{a_1,\dots,a_m}.$\PfEnd\vspace{8pt}\quad
...
By Problem (30), $\dim U_k=1.$ Thus $\exists\,u_k\in V,U_k=\Span{u_k}\neq 0.$ 又 $\Par{\null\varphi_1}\cap\dots\cap\Par{\null\varphi_n}=\zeroSubs.$\vspace{2pt}\par\quad
If $\varphi_k\Par{u_k}=0,u_k\in\null\varphi_k$ while $u_k=U=\bigcap_{j\in\Gamma_k}\null\varphi_j.$ Then $u_k\in\bigcap_{j\leqslant n}\null\varphi_j=\zeroSubs.$ Contradicts.\vspace{2pt}\par\quad
...
Similarly, $B_V=\Par{v_1,\dots,v_n}.$ And because $\varphi_j\Par{v_k}=\delta_{j,k}.$\PfEnd
\SepLine
...
}Let $B_U=\Par{u_1,\dots,u_m},\,B_V=\Par{u_1,\dots,u_m,\dots,u_n},\,B_{V\apostrophe}=\Par{\varphi_1,\dots,\varphi_m,\dots,\varphi_n}$.\par\quad
...
Then $\varphi\in\Span{\varphi_{m+1},\dots,\varphi_n}$. Thus $\Span{\varphi_{m+1},\dots,\varphi_n}\supseteq U^0.$ 又 Each $\varphi_i\in U^0.$\PfEnd
\SepLine

\ProblemN{\hypertarget{3F37}{37}}{
	\TextNL{Suppose $U$ is a subsp of $V$ and $\pi$ is the quotient map. Thus $\pi\apostrophe\in\Lm[\BigPar]{\Par{V/U}\apostrophe,V\apostrophe}$.\vspace{2pt}}
	(a) \TextNL{Show that $\pi\apostrophe$ is inje{\tgnr\large\envFontDefault:\;\;Because $\pi$ is surj. Use [3.108].}\vspace{2pt}}
	(b) \TextNL{Show that $\range \pi\apostrophe=U^0${\tgnr\large\envFontDefault:\;\;By {\tgnr[3.109](b)}, $\range\pi\apostrophe=\BigPar{\null\pi}{^0}=U^0.$}\vspace{2pt}}
	(c) \TextNL{Conclude that $\pi\apostrophe$ is an iso from $\Par{V/U}\apostrophe$ onto $U^0${\tgnr\large\envFontDefault:\;\;Immediately.}}
...
\ProblemB{
	\TextB{Suppose $U$ is a subsp of $V.$ Prove that $\Par{V/U}\apostrophe$ and $U^0$ are iso. \hfill[{\large\tgsc Another proof of \tgbfx[3.106]}]}
}\par\quad
...
Suppose $\varPhi\in\BigPar{\null T}{^0}.$ Because by (3.B.12), $T\mid_U:U\rightarrow\range T$ is an iso.\par\quad
\BigPar{ And $V=U\oplus\null T;\;\;\forall v\in V,\exists\,!\,u_v\in U,w_v\in\null T,v=u_v+w_v.$ }\vspace{2pt}\par\quad
...
Where $T^{-1}\mid_{\range T}:\range T\rightarrow U;\;\;T:V\rightarrow\range T.$
Note that $T^{-1}\mid_{\range T}\circ T\mid_V=i.$\vspace{2pt}\par\quad
Where $i\in\Lm{V,U}$ is defined by $i\Par{v}=u_v.$ By \TIPS in (3.B), $\varPhi=\varPhi\circ i.$ Thus $T\apostrophe\Par{\psi}=\psi\circ T=\varPhi\circ{i}=\varPhi.$\PfEnd
\SepLine

\ProblemB[]{
	\hypertarget{3F4e17}{}\TextB{Suppose $T\in\Lm{V,W}.$ {\large Using [3.108], [3.110].}}
	...
	}$}$\Longleftrightarrow T\apostrophe$ is inv.}
}\SepLine
...
}$V,W$ are finite-dim $\Rightarrow\dim\Lm{V,W}=\dim\Lm{W\apostrophe,V\apostrophe}$. And by [3.101], $\Gamma$ is linear.\par
\Blind{\Solution} 又 Suppose $\Gamma\Par{T}=T\apostrophe=0$. By Problem (15), $T=0$. Thus $\Gamma$ is inje $\Rightarrow\Gamma$ is inv.\PfEnd
\Comment \,\,\,Is the result still true if one of $V,W$ is infinite-dim?\vspace{2pt}\par
\ProblemB{
	\TextB{Suppose $W$ is finite-dim. Prove that $\Gamma$ surj.}
}\par\quad
%Let $B_W=\Par{w_1,\dots,w_m},B_{W\apostrophe}=\Par{\psi_1,\dots,\psi_m}.$\par\quad
%Suppose $S\in\Lm{W\apostrophe,V\apostrophe}.$ Let $B_{\range S}=\Par{x_1,\dots,x_n},B_W\apostrophe=\Par{x_1,\dots,x_m},B_{W\apostrophe}\apostrophe=\Par{\Chi_1,\dots,\Chi_n,\dots,\Chi_m}$.\par\quad
%Note that by Problem (9), $\forall\psi\in W\apostrophe,\psi=\psi\Par{w_1}\psi_1+\dots+\psi\Par{w_m}\psi_m=\psi\Par{x_1}\Chi_1+\dots+\psi\Par{x_m}\Chi_m.$\par\quad
%Suppose $x_i=Sv_i$ for each $i.$ Then let $B_U=\Bra{v_1,\dots,v_n},B_V=\Bra{v_1,\dots,v_p},B_{V\apostrophe}=\Bra{\varphi_1,\dots,\varphi_p}.$\par\quad
%$\forall v\in V,\exists\,!\,a_i\in\Fbb,v=a_1v_1+\dots+a_pv_p.$\par\quad
%Now $\BigPar{S\Par{\psi}}\Par{v}=\varphi_1\Par{v}x_1+\dots+\varphi_n\Par{v}x_n.$\par\quad

\ProblemB{
	\TextB{Suppose $V$ is finite-dim. Prove that $\Gamma$ surj.}
}\par\quad
%Suppose $S\in\Lm{W\apostrophe,V\apostrophe}.$ Let $B_{\range S}=\Par{\varphi_1,\dots,\varphi_m},B_{V\apostrophe}=\Par{\varphi_1,\dots,\varphi_n}.$\par\quad
\par\quad
\SepLine
...
\AlignEq{}{\forall u,v\in V,\lambda\in\Fbb,\,&T\Par{u+\lambda v}=\textstyle\sum_{i=1}^m\varphi_i\Par{u+\lambda v}w_i\\[-3pt]=\,&T{u}+\lambda T{v}=\XPar{\textstyle\sum_{i=1}^m\varphi_i\Par{u}w_i}+\lambda\XPar{\textstyle\sum_{i=1}^m\varphi_i\Par{v}w_i}=\textstyle\sum_{i=1}^m\BigPar{\varphi_i\Par{u}+\lambda\varphi_i\Par{v}}w_i.\hspace{6pt}}\PfEnd\vspace{3pt}\quad
\Or For each $w_{i},\exists\,v_{i} \in V,\,Tv_{i}=w_{i},$
then $\Par{v_1,\dots, v_m}$ is linely inde.\par\quad
Now we have $Tv=a_1Tv_1+\dots+a_mTv_m,\,\forall v \in V,$ $\exists\,!\,a_{i}\in\Fbb.$ Let $B_{\envFontSmall[\scriptsize]\Par{\range T}\apostrophe}=\Par{\psi_1, \dots, \psi_m}$.\par\quad
...
Thus for each $i\in\Bra{1,\dots,m},\varphi_i=\psi_i\circ T=T\apostrophe\Par{\psi_i}\in V\apostrophe.$\PfEnd
\SepLine
...
\ProblemN{\hypertarget{3F7}{7}}{
	...
	\TextN{{\large\envFontDefault Here $p{^{\envFontSmall[\scriptsize]\Par[0pt]{k}}}$ denotes the $k^{th}$ derivative of $p$, with the understanding that the $0^{th}$ derivative of $p$ is $p$.}}
}\par\quad
...
\Blind{\Or \;}Thus $\varphi_k$ and $\envFontSmall[\scriptsize]\frac{p^{\Par[0pt]{k}}\Par{0}}{k!}$ act exactly the same on the same basis $\Par{1,\dots,x^m}.$\par\quad
\Blind{\Or \;}Hence $\envFontSmall[\scriptsize]\frac{p^{\Par[0pt]{k}}\Par{0}}{k!}$ is just another definition of $\varphi_k$.\PfEnd\vspace{4pt}
...
\Blind{\Example \,\,\,}{\tgnr\large\envFontDefault Let $\varphi_k=\displaystyle\frac{p^{\envFontSmall[\scriptsize]\Par{k}}\Par{5}}{k!}$ for each $k=0,1,\dots,m$. Then $\Par{\varphi_0,\varphi_1,\dots,\varphi_m}$ is the dual basis of $B$.}\par
\SepLine
\vspace{10pt}
...
(a) $\MathRightBrace{r}{U\cap W\subseteq U\\U\cap W\subseteq W}\Rightarrow\hspace{-6pt}\MathRightBrace{r}{\Par{U\cap W}{^0}\supseteq U^0 \\ \Par{U\cap W}{^0}\supseteq W^0}\Rightarrow\Par{U\cap W}{^0}\supseteq U^0+W^0\;\BigPar{\,\supseteq U^0\cap W^0=\Par{U+W}{^0}\,}.$\par\vspace{6pt}\quad\Ha
\Or Suppose $\varphi\in\Par{U\cap W}{^0}.$ Let $X,Y$ be such that $V=U\oplus X=W\oplus Y.$\par\quad\Ha
Define $\psi\in U^0,\beta\in W^0$ by $\psi\Par{u+x}=\frac{1}{2}\varphi\Par{x},\beta\Par{w+y}=\frac{1}{2}\varphi\Par{y}.$\par\quad\Ha
$\forall v=u+x=w+y\in V,\varphi\Par{v}=\varphi\Par{x}=\varphi\Par{y}.$ Now $\varphi\Par{v}=\frac{1}{2}\varphi\Par{x}+\frac{1}{2}\varphi\Par{y}=\psi\Par{v}+\beta\Par{v}.$\par\quad\Ha
Hence $\varphi\in U^0+W^0.$\vspace{4pt}\par\quad
(b) Suppose $\varphi\in U^0,\psi\in W^0.$ Then $\forall v\in U\cap W$, $\Par{\varphi+\psi}\Par{v}=\varphi\Par{v}+\psi\Par{v}=0.$\PfEnd
\SepLine
...
\ProblemBnoor[]{\Corollary}[]{
	...
	(b) \TextB{Suppose $\Bra{V_{\alpha_i}}{_{\alpha_i\in\Gamma}}$ is a collection of subsps of \,$V$. Then $\XPar{\sum_{\alpha_i\in\Gamma}V_{\alpha_i}}{^0}=\bigcap_{\alpha_i\in\Gamma}\BigPar{V^0_{\alpha_i}}.$}
}\SepLine
...
\Corollary \,\,\,$U^0=\Bra{v\in V:\varphi\Par{v}=0,\forall\varphi\in U^0}{^0}.$\par
\Comment \,\,\,$\Bra{v\in V:\varphi\Par{v}=0,\forall\varphi\in U^0}=\BigPar{\Par{\null\varphi_1}\cap\cdots\cap\Par{\null\varphi_m}\cap\cdots},$ where $\varphi_k\in U^0$,\par
...
\Or Suppose $\varphi\in\Omega.$ Then $\forall v\in U,\varphi\Par{v}=0\Rightarrow\varphi\in U^0.$ Hence $\Omega\subseteq U^0.$\par\quad
And, [This may be much trickier.]\colorbox{yellow}{TODO}\par\quad
{\normalsize\tgsl In other words, suppose $\psi\in V\apostrophe$ sends all the vecs that, all $\psi\in\Omega$ send to zero in common, to zero, prove that $\psi\in\Omega$.}\PfEnd\vspace{6pt}\quad
\Or [{\tgsl Only in Finite-dim}] Let $\Par{\varphi_1,\dots,\varphi_m}$ be a basis of $\Omega.$ Then by def, $U\subseteq\Par{\null\varphi_1}\cap\cdots\cap\Par{\null\varphi_m}.$\par\quad
Suppose $\varphi\in\Omega.$ Then $\exists\,!\,a_i\in\Fbb,\varphi=a_1\varphi_1+\dots+a_m\varphi_m.$\par\quad
And $\forall v\in\Par{\null\varphi_1}\cap\cdots\cap\Par{\null\varphi_m},\varphi\Par{v}=a_1\varphi_1\Par{v}+\dots+a_m\varphi_m\Par{v}=0.$\par\quad
Hence $v\in U\Rightarrow\Par{\null\varphi_1}\cap\cdots\cap\Par{\null\varphi_m}\subseteq U.$ Now $U=\Par{\null\varphi_1}\cap\cdots\cap\Par{\null\varphi_m}.$\par\quad
Thus by the \COROLLARY\;in Problem (23), $U^0=\BigPar{\Par{\null\varphi_1}\cap\cdots\cap\Par{\null\varphi_m}}{^0}=\Span{\varphi_1,\dots,\varphi_m}=\Omega.$\par
\Corollary \,\,\,For every subsp $\Omega$ of $V\apostrophe,\;\exists\,!\,$subsp $U$ of $V$ such that $\Omega=U^0.$\par
\SepLine
...
\ProblemBnoor{\hypertarget{3F4e23}{4E 3.F.23}}{
	...
	(a) \TextB{$\Span{\varphi_1,\dots,\varphi_m}$\vspace{3pt}}
	(b) \TextB{$\BigPar{\Par{\null \varphi_1}\cap\dots\cap\Par{\null \varphi_m}}{^0}$\vspace{3pt}}
	(c) \TextB{$\Bra{\varphi\in V\apostrophe:\Par{\null\varphi_1}\cap\dots\cap\Par{\null\varphi_m}\subseteq\null\varphi}$\vspace{2pt}}
}By Problem (17), (b) and (c) are equi.\vspace{4pt}\par\quad
By Problem (26) and the \COROLLARY\;in Problem (23),\par\quad\,
$\MathRightBrace{r}{
...
And because $\forall c\in\Fbb,v\in\null\varphi,c\varphi\Par{v}=0\Rightarrow\Span{\varphi}\subseteq\Par{\null\varphi}{^0}.$ Similarly.\PfEnd\vspace{6pt}
\Corollary \,\,\,\ProblemN[]{\hypertarget{3F30}{30}}{
	\TextNL{Suppose $V$ is finite-dim and $\varphi_1,\dots,\varphi_m$ is a linely inde list in $V\apostrophe.$}
	\Blind{\Corollary \,\,\,}\TextNL{Then $\Dim\BigPar{\Par{\null \varphi_1}\cap\dots\cap\Par{\null \varphi_m}}=\Par{\dim V}-m$.}
}
\SepLine

%\ProblemN{\hypertarget{3F27}{27}}{
%	\TextNL{Suppose $T\in\Lm[\BigPar]{\PoR{}}$ and $\null T\apostrophe=\Span{\varphi}$, where $\varphi\in\BigPar{\PoR{}}^{\apostrophe}$}
%	\TextNL{defined by $\varphi\Par{p}=p\Par{8}$. Prove that $\range T=\Bra{p\in \PoR{}:p\Par{8}=0}.$}
%}\par\quad
%Because $\Par{\null\varphi}{^0}=\Span{\varphi}=\null T\apostrophe=\Par{\range T}{^0}$.\par\quad
%By the \COROLLARY\;in Problem (20, 21), $\range T=\null\varphi=\Bra{p\in\PoR{5}:p\Par{8}=0}.$\PfEnd
%\SepLine

\ProblemB{
	\TextB{Suppose $T\in\Lm{V,W}$.}
	(a) \OR\ProblemN[]{\hypertarget{3F28}{28}}{
		\TextB{Suppose $\varphi\in W\apostrophe.$ Prove that $\null T\apostrophe=\Span{\varphi}\Longleftrightarrow\range T=\null\varphi.$}}\TextB{}
	(b) \OR\ProblemN[]{\hypertarget{3F29}{29}}{
		\TextB{Suppose $\varphi\in V\apostrophe.$ Prove that $\range T\apostrophe=\Span{\varphi}\Longleftrightarrow\null T=\null\varphi.$}}
	\TextB{\vspace{-4pt}}
...
{Define $\def\envFont{\small}\theta\in\Lm[\XPar]{\envFontDefault\BigPar{\PoR{}}\apostrophe,\Rbb^{\infty}\def\envFont{\small}}$ by $\theta\Par{\varphi}=\def\envFont{\small}\XPar{\envFontDefault\varphi\BigPar{1},\varphi\BigPar{x},\cdots,\varphi\BigPar{x^n},\cdots\def\envFont{\small}}.$}\par\quad
{Inje: $\theta\Par{\varphi}=0\Rightarrow \forall x^k$ in the basis $\BigPar{1,x,\dots,x^n}$ of $\PoR{n}\,\BigPar{\,\forall n\,},\,\varphi\Par{x^k}=0\Rightarrow\varphi=0.$}\par\quad
{Surj: $\forall \BigPar{a_k}{_{k=1}^\infty}\in\Fbb^\infty,$ let $\psi$ be such that $\forall k,\psi\BigPar{x^k}=a_k$ and thus $\theta\Par{\psi}=\BigPar{a_k}{_{k=1}^\infty}.$}\PfEnd
\SepLine

\ProblemB{
	\TextB{Suppose $V=U\oplus W$. Define $i:V\rightarrow U$ by $i\Par{u+w}=u.$ Thus $i\apostrophe\in\Lm{U\apostrophe,V\apostrophe}.$\vspace{2pt}}
	(a) \TextB{Show that $\null i\apostrophe=U_U^0=\zeroSubs${\tgnr\large\envFontDefault:\;\;$\null i\apostrophe=\Par{\range i}{^0_U}=U_U^0=\zeroSubs.$}\vspace{2pt}}
	(b) \TextB{Prove that $\range i\apostrophe=W_V^0${\tgnr\large\envFontDefault:\;\;$\range i\apostrophe=\BigPar{\null i}{^0_V}=W_V^0.$}\vspace{2pt}}
	(c) \TextB{Prove that $\tilde{i\apostrophe}$ is an iso from $\envFontSmall U\apostrophe/\zeroSubs$ onto $W^0${\tgnr\large\envFontDefault:\;\;By (a), (b) and [3.91](d).}}
}(a) $i\apostrophe\Par{\psi}=\psi\circ i=0\Longleftrightarrow U\subseteq\null\psi.$\par
\Blind{\Solution} (b) Note that $W=\null\Par{i}\subseteq\null\Par{\psi\circ i}.$ Then $\psi\circ i\in W^0\Rightarrow\range i\apostrophe\in W^0.$\par\Hb
\Blind{\Solution} Suppose $\varphi\in W^0.$ Because $\null i=W\subseteq\null\varphi.$ By \TIPS in (3.B), $\varphi=\varphi\circ i=i\apostrophe\Par{\varphi}.$\PfEnd
\SepLine
...
\ProblemN{\hypertarget{3F36}{36}}{
	...
	(a) \TextNL{Show that null $i\apostrophe=U^0${\tgnr\large\envFontDefault:\;\;$\null i\apostrophe=\Par{\range i}{^0}=U^0\Leftarrow\range i=U$.}\vspace{2pt}}
	(b) \TextNL{Prove that $\range i\apostrophe=U\apostrophe${\tgnr\large\envFontDefault:\;\;$\range i\apostrophe=\BigPar{\null i}_{U}{^0}=\BigPar{\zeroSubs}{_U^0}=U\apostrophe$.}\vspace{2pt}}
	(c) \TextNL{Prove that $\tilde{i\apostrophe}$ is an iso from $V\apostrophe/U^0$ onto $U\apostrophe${\tgnr\large\envFontDefault:\;\;By (a), (b) and [3.91](d).}\vspace{2pt}}
}(a) $\forall\varphi\in V\apostrophe,i\apostrophe\Par{\varphi}=\varphi\circ i=\varphi\mid_U.$ Thus $i\apostrophe\Par{\varphi}=0\Longleftrightarrow\forall u\in U,\varphi\Par{u}=0\Longleftrightarrow\varphi\in U^0.$\par
\Blind{\Solution} (b) Suppose $\psi\in U\apostrophe.$ By (3.A.11), $\exists\,\varphi\in V\apostrophe,\varphi\mid_U=\psi.$ Then $i\apostrophe\Par{\varphi}=\psi.$\PfEnd
\SepLine
...
\ProblemN{\hypertarget{3F34}{34}}{
	...
	(c) \TextNL{Show that if $V$ is finite-dim, then $\Lambda$ is an iso from $V$ onto $V\apostrophe\apostrophe$.\vspace{2pt}}
...
Hence $T\apostrophe\apostrophe\Par{\Lambda v}=\BigPar{\Lambda\Par{Tv}}\Rightarrow T\apostrophe\apostrophe\circ\Lambda=\Lambda\circ T$.\par\quad
