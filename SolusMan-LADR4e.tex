...
\setmathfont[range={\mathcal,\Longleftrightarrow,\Longrightarrow,\Longleftarrow,\Rightarrow,\Leftarrow,\Leftrightarrow,\mapsto,\rightarrow,\leftarrow,\leftrightarrow}]{LatinModernMath-Regular}
...
\def\envFont{\large}
\newcommand{\TextE}[1]{\dbsp\def\envFont{\Large}\def\envFontB{\large}\Largesl{#1}\par\IndentE}
\newcommand{\TextN}[1]{\dbsp\def\envFont{\Large}\def\envFontB{\large}\Largesl{#1}\par\IndentN}
\newcommand{\TextB}[1]{\dbsp\def\envFont{\Large}\def\envFontB{\large}\Largesl{#1}\par\IndentB}
\newcommand{\TextNL}[1]{\dbsp\def\envFont{\Large}\def\envFontB{\large}\Largesl{#1}\par\IndentNL}
%}
...
\def\envFontB{\small}
\newcommand{\Par}[2][-5pt]{\hspace{-1pt}{\envFontB\hText[#1]{\left(}{\right)}  {${\envFont$#2$}$}}}
\newcommand{\Mid}[2][-5pt]{\hspace{-1pt}{\envFontB\hText[#1]{\left|}{\right|}  {${\envFont$#2$}$}}}
\newcommand{\Bra}[2][-5pt]{\hspace{-1pt}{\envFontB\hText[#1]{\left\{}{\right\}}{${\envFont$#2$}$}}}
\def\zeroSubs{\Bra{0}}

\newcommand{\ExampleX}[1]{
	\Example #1\vspace{-16pt}\par
	\Solution\def\envFont{\large}\def\envFontB{\small}
}
\newcommand{\ExampleXnosolus}[1]{
	\Example #1\def\envFont{\large}\def\envFontB{\small}\vspace{-16pt}\par
}
\newcommand{\ProblemN}[2]{
	\Onumber{#1} #2\vspace{-16pt}\par
	\def\envFont{\large}\def\envFontB{\small}\Solution
}
\newcommand{\ProblemNnosolus}[2]{
	\Onumber{#1} #2\def\envFont{\large}\def\envFontB{\small}\vspace{-16pt}\par
}
...
\newcommand{\ProblemBnosolus}[1]{
	\BulletPoint #1\def\envFont{\large}\def\envFontB{\small}\vspace{-16pt}\par
}
...
\Or Because $[\TXT{\tgbf{I}}]: \cos x+\sin\sqrt{2}x=\cos\Par{x+p}+\sin(\sqrt{2}x+\sqrt{2}p).$ By differentiating twice,\par\qquad\qquad\qquad\hspace{0pt}
...
(a) {$V_E\cap V_O=\Bra{\,f\in\Rbb^{\Rbb}:f(x)=f(-x)=-f(-x)}=\{0\}.$}\par\vspace{12pt}\quad
...
\ProblemB{
	...
	(b) \TextB{Show that [P] $\Par{v_1,\dots,v_m}$ is linely inde $\Longleftrightarrow$ $(w_1,\dots,w_m)$ is linely inde [Q].}
}\par\quad
...
$(w_1,\dots,w_m)$ linely dep $\Rightarrow\Par{v_1,\dots,v_m}$ linely dep. Conversely it is true as well.\PfEnd
\SepLine
...
\ProblemN{\hypertarget{2A14}{14}}{
	\TextNL{Prove that [P] $V$ is infinite-dim $\Longleftrightarrow[Q]\left|\hspace{-6pt}\begin{array}{l}$ there is a sequence $(v_1,v_2,\dots)$ in $V$ such that$\\[-8pt]$ $\Par{v_1,\dots,v_m}$ is linely inde for each $m\in\Nbp.\end{array}\right.$}
}\par\quad
...
Let $(v_1 , v_2 , \dots)$ be a sequence in $V$, then $\Par{v_1,v_2,\dots,v_{m+1}}$ must be linely dep.\par\quad
...
\ProblemN{\hypertarget{2A17}{17}}{
	\TextNL{Suppose $p_0,p_1,\dots,p_m\in\PoF{m}$ such that $p_k(2)=0$ for each $k\in\!\Bra{0,\dots,m}$.}
	\TextNL{Prove that $\Par{p_0,p_1,\dots,p_m}$ is not linely inde in $\PoF{m}$.}
}\par\quad
Suppose $\Par{p_0,p_1,\dots,p_m}$ is linely inde. Define $p\in\PoF{m}$ by $p(z)=z\,\,\forall z\in\Fbb.$\par\quad
But $\forall a_i\in\Fbb,z\neq a_0 p_0(z)+\dots+a_m p_m(z)$, for if not, let $z=2$, contradicts.
Thus $z\not\in\Span{p_0,p_1,\dots,p_m}$.\par\quad
Then $\Span{p_0,p_1,\dots,p_m}\subsetneq\PoF{m}$ while the list $\Par{p_0,p_1,\dots,p_m}$ has length $(m+1)$.\par\quad
Hence $\Par{p_0,p_1,\dots,p_m}$ is linely depe in $\PoF{m}$.\par\quad
For if not, because $\Par{1,z,\dots,z^m}$ of length $(m+1)$ spans $\PoF{m}$,\par\qquad\qquad\quad\,\,
thus by [2.23] trivially, $\Par{p_0,p_1,\dots,p_m}$ spans $\PoF{m}$. Contradicts.\PfEnd\vspace{10pt}\par\quad
\Or Note that $\PoF{m}=\Span{\overbrace{1,z,\dots,z^m}_{\text{of length }m+1}}$ and then $\Par{p_0,p_1,\dots,p_m,z}$ of length $(m+2)$ is linely dep.\vspace{3pt}\par\quad
...
%}\par\quad
%\NOTICE that if $\Par{v_1,\dots,v_m}$ is linely inde and $a_1 v_1+\dots+a_m v_m=b_1 v_1+\dots+b_m v_m,$ then $a_k=b_k$ for all $k.$\par\quad
...
\ProblemB{
	\hypertarget{2B4e9}{}\TextB{Suppose $\Par{v_1,\dots,v_m}$ is a list of vecs in $V$. For $k\in\left\{1,_{}\dots,m\right\}$, let $w_k=v_1+\dots+v_k.$}
	\TextB{Show that [P] $\Par{v_1,\dots,v_m}$ is a basis of $V\Longleftrightarrow$ [Q] $(w_1,\dots,w_m)$ is a basis of $V.$}
...
$\Par{v_1,\dots,v_m}$ linely inde $\Rightarrow$ $\Par{v_1,v_2-v_1,\dots,v_m-v_1}$ linely inde $\Rightarrow$ $\underbrace{\Par{v_2-v_1,_{}\dots,v_m-v_1}}_{\text{of length }(m-1)}$ linely inde.\vspace{-8pt}\par\quad
...
{\Large\vspace{4pt}$\Rightarrow x^{i+1}=\displaystyle\frac{1}{a_{i+1}}\Par{p_{i+1}(x)-r_{i+1}(x)}\in\Span{1,x,\dots,x^i,p_{i+1}}=\Span{p_0,p_1,\dots,p_i,p_{i+1}}$.}\par\vspace{2pt}\quad\Hi
...
{\Large$\Rightarrow\Span{x^m,\dots,x,1}=\Spn\Par{\overbrace{p_m,\dots,p_1,p_0}_\text{Basis}}.$}\large\PfEnd\vspace{10pt}\quad
\Or For any $m,k\in\Nbp$ such that $k\leq m.$ Define $p_{k,m}$ by $p_{k,m}(x)=x^k(1-x)^{m-k}.$\par\quad
Define the statement $S(m)$ by $S(m):\underbrace{\Par{p_{0,m},\dots,p_{m,m}}}_{\dim\PoF{m}=m+1}$ is linely inde ( and therefore is a basis ).\vspace{4pt}\par\quad
...
Let $\Par{v_1,\dots,v_m}$ be a basis of $U$, extend to a basis of $V$ as $(v_1,\dots,v_m,u_1,\dots,v_{n-m})$.\par\quad
...
$\Longleftrightarrow\left(\mathcal{E}_1\cap\dots\cap\mathcal{E}_{k-1}\right)\cap\mathcal{E}_k=\varnothing$ for each $i$ 又 $\dim\Span{\mathcal{E}_1\cup\dots\cup\mathcal{E}_m}=\card\Par{\mathcal{E}_1\cup\dots\cup\mathcal{E}_m}$\par\Blind{\Comment\,\,}
...
Because $\Mid{X+Y}=\Mid{X}+\Mid{Y}-\Mid{X\cap Y};\,\,\,(X\cup Y)\cap Z=(X\cap Z)\cup(Y\cap Z)$.\par\quad
Now $\Mid{(A\cup B)\cup C}=\Mid{A\cup B}+\Mid{C}-\Mid{(A\cup B)\cap C}.$\par\quad
And $\Mid{(A\cup B)\cap C}=\Mid{(A\cap C)\cup(B\cap C)}=\Mid{A\cap C}+\Mid{B\cap C}-\Mid{A\cap B\cap C}.$\par\quad
Hence $\Mid{(A\cup B)\cup C}=\Mid{A}+\Mid{B}+\Mid{C}+\Mid{A\cap B\cap C}-\Mid{A\cap B}-\Mid{A\cap C}-\Mid{B\cap C}.$\par\vspace{12pt}\quad
...
Suppose $(v_1,\dots,n)$ is a basis of $V.$ Define $V_i$ by $V_i=\Span{v_i}$ for each $i\in\!\Bra{1,\dots,n}.$\par\quad
...
$T\Par{0,1,0,\dots,0,0}=\Par{A_{1,2},\dots,A_{m,2}},$\quad Then by [3.5], we are done.\PfEnd\Blind{Let $T(1,0,0,\dots$}
$\vdots$\par\quad\Blind{Let}
$T\Par{0,0,0,\dots,0,1}=\Par{A_{1,n},\dots,A_{m,n}}.$\par
\SepLine

\ProblemN{4}{
	\TextN{Suppose $T\in\Lm{V,W},$ and $v_1,\dots,v_m\in V$ such that $\left(Tv_1,\dots,Tv_m\right)$ is linely inde in $W$.} \TextN{Prove that $\Par{v_1,\dots,v_m}$ is linely inde.}
...
$}$ \Or Define $T(x,y)=\sqrt[3]{\Par{x^3+y^3}}.$\PfEnd
\SepLine

\ProblemN{9}{
	\TextN{Give a function $\varphi:\Cbb\rightarrow\Cbb$ such that $\forall w,z\in\Cbb,\varphi\Par{w + z} = \varphi(w) + \varphi(z)$}
...
Because in general, $q\circ \Par{p_1+\lambda p_2}(x)=q\Par{p_1(x)+\lambda p_2(x)}\neq \Par{q\circ p_1}(x)+\lambda \Par{q\circ p_2}(x).$\par
...
Where we let $B_U=\Par{u_1,\dots,u_n},B_V=(u_1,\dots,u_n,\dots,u_m).$\PfEnd
\SepLine
...
Let $\Par{v_1,\dots,v_n}$ be a basis of $V$. Let $(w_1,\dots,w_m)$ be linely inde in $W$ for any $m\in\Nbp$.\par\quad
Define $T_{x,y}\in\Lm{V,W}$ by $T_{x,y}(v_z)=\delta_{zy} w_y$, $\forall x\in\!\Bra{1,\dots,n},y\in\!\Bra{1,\dots,m}$, where $\delta_{zy}=\MathLeftBrace{l}{
0,\quad z\neq y,\\
1,\quad z=y.}$\par\quad
...
Now we prove the assumption by contradiction. Suppose $\Par{v,Tv}$ is linely inde for every $v\in V\backslash\{0\}$.\par\quad
...
\ProblemN{\hypertarget{3B7}{7}}{
	...
	\TextN{Show that $U=\Bra{\,T\in\Lm{V,W}:\null T\neq\{0\}}$ is not a subsp of $\Lm{V,W}$.}
}\par\quad
Let $\Par{v_1,\dots,v_n}$ be a basis of $V$, $(w_1,\dots,w_m)$ be linely inde in $W$.\par\quad
...
Let $\Par{v_1,\dots,v_n}$ be linely inde in $V$, $(w_1,\dots,w_m)$ be a basis of $W$.\par\quad
...
(b) Suppose $\dim V\leq\dim W.$ Let $B_V=\Par{v_1,\dots,v_n},B_W=(w_1,\dots,w_m).$\par\quad\Hb
...
(b) Suppose $\dim V\geq\dim W.$ Let $B_V=\Par{v_1,\dots,v_n},B_W=(w_1,\dots,w_m).$\par\quad\Hb
...
Let $B_U=\Par{u_1,\dots,u_m},B_V=(u_1,\dots,u_m,v_1,\dots,v_n),B_W=(w_1,\dots,w_p)$.\par\quad
...
$\forall v\in V,\,\exists\,!\,w\in\null T,u\in U, v=w+u.$ Then $Tv=T(w+u)=Tu\in\Bra{Tu:u\in U}.$\PfEnd
\SepLine
...
Suppose $\dim X + \dim Y = \dim V$. Let $B_X=\Par{u_1,\dots,u_n},B_Y=(w_1,\dots,w_m),B_V=(u_1,\dots,u_n,v_1,\dots,v_m).$\vspace{3pt}\par\quad
...
Define $S\in\Lm{W,V}$ by $S(Tv_i)=v_i$ and $Su=0,$ where $i\in\!\Bra{1,\dots,n},u\in U.$ Thus $ST=I.$\PfEnd
\SepLine
...
$\range ST=\Bra{Sv:v\in\range T}=\Span{Su_1,\dots,Su_{\dim\range T}},$ where \,$B_{\range T}=\left(u_1,\dots,u_{\dim\range T}\right).$\par\quad
...
(a) $\forall\,v=cu\in\null\varphi\cap\Bra{au:a\in\Fbb},\varphi(v)=0=c\varphi(u)\Rightarrow c=0.$ Hence $\null\varphi\cap\Bra{au:a\in\Fbb}=\{0\}.$\par\vspace{6pt}\quad
(b) $\forall\,v\in V,v=\displaystyle\left(v-\frac{\varphi(v)}{\varphi(u)}u\right)+\frac{\varphi(v)}{\varphi(u)}u.$
$\left|\left.\begin{array}{l}\displaystyle v-\frac{\varphi(v)}{\varphi(u)}u\in\null\varphi\\[16pt]\displaystyle\frac{\varphi(v)}{\varphi(u)}u\in\Bra{au:a\in\Fbb}\end{array}\right\}\right.\Rightarrow V=\null\varphi\oplus\Bra{au:a\in\Fbb}.$\PfEnd\vspace{12pt}\large
...
{Suppose $\varphi\neq 0\Rightarrow\exists\,i,j\in\!\Bra{1,\dots,n},\,\varphi(R_{i,j})\neq 0$.}\par\quad
...
Let $\Par{v_1,\dots,v_p}$ be a basis of $\null T,$ extend to a basis $\Par{v_1,\dots,v_n}$ of $V.$\par\quad
Let $\Par{w_1,\dots,w_m}$ be basis of $W$. Denote $\Mt{T,\Par{v_1,\dots,v_n},\Par{w_1,\dots,w_m}}$ by $A.$\vspace{4pt}\par\quad
...
Let $R=\Par{Tv_1,\dots,Tv_n}$ be a basis of $\range T$, extend to  $B_W=(Tv_1,\dots,Tv_n,w_1,\dots,w_p)$.\par\quad
...
And $Tv_1=T\left(\displaystyle\frac{u_k}{A_{1,k}}\right)=\displaystyle\frac{1}{A_{1,k}}\Par{A_{1,k}w_1+\dots+A_{n,k}w_n}=1w_1+\dots+\displaystyle\frac{A_{n,k}}{A_{1,k}}w_n.$\par\quad
\vspace{15pt}\AlignEq{}{\forall j\in\underset{\qquad\quad\overset{\uwave{\quad\qquad\qquad\qquad}}{\quad\;\;i\,\in\,\{\,k+1,\dots,n\,\}\quad}}{\Bra{2,\dots,k,{k+2,\dots,n+1}}},\;Tv_j=T\left(u_{j-1}-A_{1,j-1}v_1\right)=Tu_{j-1}-T\left(\displaystyle\frac{A_{1,j-1}u_k}{A_{1,k}}\right)\hspace{93pt}\\=A_{1,j-1}w_1+\dots+A_{n,j-1}w_n-A_{1,j-1}\left(1w_1+\dots+\displaystyle\frac{A_{n,k}}{A_{1,k}}w_n\right)=0w_1+\dots+\left(A_{n,j-1}-\displaystyle\frac{A_{1,j-1}A_{n,k}}{A_{1,k}}\right)w_n.}\PfEnd\vspace{4pt}\par
\SepLine
...
(a) Suppose $B_V=\Par{v_1,\dots,v_n},B_W=(w_1,\dots,w_m)$ are the bases such that all entries of $A$ equal $1$.\vspace{6pt}\par\quad\Ha
...
$\forall k\in\!\Bra{1,\dots,n},Tu_k\in\range T=\Span{w}\Rightarrow Tu_k=\lambda_k w,\,\exists\,\lambda_k\in\Fbb\backslash\{0\}.$\par\quad\Hb
...
$\because\Par{Ac}_{j,1}=\sum_{r=1}^n A_{j,r}c_{r,1}=\left(\sum_{r=1}^n\Par{A_{\cdot,r}c_{r,1}}\right){_{j,1}}=\Par{c_1 A_{\cdot,1}+\dots+c_n A_{\cdot,n}}_{j,1}$\vspace{3pt}\par\quad
...
$\because\Par{aC}_{1,k}=\sum_{r=1}^n a_{1,r}C_{r,k}=\left(\sum_{r=1}^n a_{1,r}\Par{C_{r,\cdot}}\right){_{1,k}}=\Par{a_1 C_{1,\cdot}+\dots+a_n C_{n,\cdot}}_{1,k}$\vspace{3pt}\par\quad
...
\TXT{\Or}\,\,\,\,\Par{AAA}_{j,k} &=\textstyle\sum_{r=1}^n\Par{AA}_{j,r}A_{r,k} =\textstyle\sum_{r=1}^n\Par{\sum_{p=1}^n A_{j,p}A_{p,r}}A_{r,k}\\&=\textstyle\sum_{r=1}^n\left[A_{j,1}\Par{A_{1,r}A_{r,k}}+\dots+A_{j,n}\Par{A_{n,r}A_{r,k}}\right]\\&=\textstyle A_{j,1}\sum_{r=1}^n A_{1,r}A_{r,k}+\dots+A_{j,n}\textstyle\sum_{r=1}^n A_{n,r}A_{r,k}=\textstyle\sum_{p=1}^n \sum_{r=1}^n A_{j,p}A_{p,r}A_{r,k}.}\PfEnd
\SepLine
...
\Or Reduce $\Par{Tv_1,\dots,Tv_m}$ to a basis of $V$ as $(Tv_{\alpha_1},\dots,Tv_{\alpha_k})$, where $k=\dim V$ and $\alpha_i\in\!\Bra{1,\dots,k}$.\par\quad
...
Define $T\in\Lm{V}$ by $T(u_i)=Su_i;\,\,\,Tv_j=w_j,$ for each $i\in\!\Bra{1,\dots,m},j\in\!\Bra{1,\dots,n}.$\PfEnd
\SepLine
...
Define $E\in\Lm{W}$ by $E(Tv_i)=Sv_i,\,\,\,E(w_j)=x_j$, for each $i\in\!\Bra{1,\dots,m},j\in\!\Bra{1,\dots,n}.$ Where:\par\vspace{6pt}\normalsize\quad
$\MathLeftrightMid{l}{$
...
Let $B_{\range T}=B_W=(w_1,\dots,w_m)\Rightarrow\forall w_i,\,\exists\,!\,v_i\in V,Tv_i=w_i.$ Let $B_{\mathcal{K}}=\Par{v_1,\dots,v_m}$.\par\quad
...
\BulletPoint \,\hspace{1pt}\NoteFor{[3.60]}\;\;Suppose $\Par{v_1,\dots,v_n}$ is a basis of $V$ and $\Par{w_1,\dots,w_m}$ is a basis of $W$.\TextB{\vspace{10pt}}
...
{\Large Denote $\Mt{E_{i,j}}$ by $\mathcal{E}^{(j,i)}.$ And $\Par{\mathcal{E}^{(j,i)}}_{l,k}=\MathLeftBrace{l}{0,\quad i\neq k\,\vee\,j\neq l\\1,\quad i=k\,\wedge\,j=l}$}\TextB{\vspace{6pt}}
{\Large Because $\Lm{V,W}$ and $\Fbb^{m,n}$ are iso. And $T=\mathcal{M}^{-1}\Mt{T};\;\;E_{i,j}=\mathcal{M}^{-1}\mathcal{E}^{(j,i)}$.}\TextB{}
...
\Or Extend $\Par{u_1,\dots,u_m}$ a basis of $U$ to $(u_1,\dots,u_m,v_1,\dots,v_n)$ a basis of $V$. Let $p=\dim W$.\par\quad\Hb
...
\ProblemBc{
	\hypertarget{3D4e17}{}\TextB{Suppose $V$ is finite-dim and $S\in\Lm{V}$. Define $A\in \Lm{\Lm{V}}$ by $\mathcal{A}(T) = ST,\forall T\in\Lm{V}$.}
...
\BulletPoint \,\hspace{1pt}\Comment\,\,\, {\Large\vspace{6pt}Define $\mathcal{B}\in\Lm{\Lm{V}}$ by $\mathcal{B}(T)=TS,\forall T\in\Lm{V}.$ Similarly to Problem ($\circ$),}\par\quad
...
Let $S=E_{1,1}+\dots+E_{m,m}\Rightarrow \Mt{S,\Par{v_1,\dots,v_n}}=\Mt{I,\Par{w_1,\dots,w_n},\Par{v_1,\dots,v_n}}$.\par\quad
...
$a_{i,j}=\Mt{I,\Par{w_1,\dots,w_n},\Par{v_1,\dots,v_n}}_{i,j}\Longleftrightarrow w_i=Iw_i=a_{1,i} v_1 +\dots+a_{n,i} v_n;$\par\qquad
For each $j$, for all $i.$ Thus $a_{i,i}=a_{k,k}=\lambda, \forall\, k\neq i.$\par\quad
Hence $w_i=\lambda v_i\Rightarrow\Mt{S}\,=\,${\small$\begin{pmatrix}
\lambda & & 0\\
& \ddots \\
0 & & \lambda
\end{pmatrix}$}$\,=\,\Mt{\lambda I,\Par{v_1,\dots,v_n}}\Rightarrow S=\mathcal{M}^{-1}\Par{\Mt{\lambda I}}\lambda I.$\PfEnd
\SepLine
...
Let $V=\Fbb^\infty=U_1,U_2=\Bra{(x,0,\cdots)\in\Fbb^\infty:x\in\Fbb}.$\par\quad
$\MathRightBrace{l}{$
...
Note that $T\Par{u_1,\dots,u_m}=T(u_1,0,\dots,0)+\dots+T(0,\dots,u_m)$.\par\quad
$\MathRightBrace{l}{$
...
Note that $Tv=\Par{w_1,\dots,w_m}$. Define $T_i\in\Lm{V,W_i}$ by $T_i(v)=w_i$.\par\quad
$\MathRightBrace{l}{$
...
Define $T:\Par{v_1,\dots,v_m}\rightarrow\varphi$, where $\varphi:(a_1,\dots,a_m)\mapsto v$ is defined by $\varphi(a_1,\dots,a_m)=a_1 v_1+\dots+a_m v_m.$\par\quad
(a) Suppose $T\Par{v_1,\dots,v_m}=0.$ Then $\forall\,(a_1,\dots,a_n)\in\Fbb^m$, $\varphi(a_1,\dots,a_m)=a_1 v_1+\dots+a_m v_m=0$\par\quad\Ha
...
\ProblemBnosolus{
	\TextB{Let $U=\Bra{(x,y,z)\in\Rbb^3:2x+3y+5z=0}.$ Suppose $A\subseteq\Rbb^3$.}
	\TextB{Then $A$ is a translate of $U\Longleftrightarrow\exists\,c\in\Rbb,A=\Bra{(x,y,z)\in\Rbb^3:2x+3y+5z=c}.$\vspace{-6pt}}
}\SepLine
...
Suppose $\lambda v+(1-\lambda)w\in A,\forall v,w\in A,\lambda\in\Fbb.$ Suppose $a\in A$ and let $A\apostrophe=\Bra{x-a:x\in A}$.\vspace{6pt}\par\quad
...
Thus $A\apostrophe$ is a subsp of $V$. Hence $a+A\apostrophe=\Bra{(x-a)+a:x\in A}{}=A$ is a translate.\PfEnd\vspace{6pt}\quad
...
\ProblemN{13}{
	\TextNL{Suppose $(v_1+U,\dots,v_m+U)$ is a basis of \,$V/U$ and $\Par{u_1,\dots,u_n}$ is a basis of $U$.}
...
\ProblemNnosolus{20}{
	...
	(c) \TextNL{Prove that $\range\Gamma\,(=\range \pi \apostrophe\,)=\Bra{T\in\Lm{V,W}:U\subseteq\null T}$: {\normalsize\tgnr By Problem (18).\PfEnd}}
}\SepLine
\ChEnd
...
\TextB{${v_1,\dots,v_m}$ is linely inde $\Longleftrightarrow\Par{\varphi(v_1),\dots,\varphi(v_m)}$ is linely inde.}}\par\quad
(a) Suppose $\Par{v_1,\dots,v_m}$ is linely inde and $\vartheta\in\Span{\varphi(v_1),\dots,\varphi(v_m)}$.\par\quad\Ha
...
\Or Because $\varphi$ is inje. Suppose $a_1\varphi(v_1)+\dots+a_m\varphi(v_m)=0=\varphi\Par{a_1 v_1+\dots+a_m v_m}.$\par\quad\Ha
...
\ProblemN{9}{
	\TextN{Suppose $\Par{v_1,\dots,v_n}$ is a basis of \,$V$ and $(\varphi_1,\cdots,\varphi_n)$ is the correspd dual basis of $V\apostrophe$.}
...
\Comment\,\,\, For other basis $\Par{u_1,\dots,u_n}$ and the dual basis $(\rho_1,\dots,\rho_n),\,\,\psi=\psi(u_1)\rho_1+\dots+\psi(u_n)\rho_n.$\par
\SepLine

\ProblemN{35}{
	\TextNL{Prove that $\left(\PoR{}\right)\apostrophe$ and $\Rbb^{\infty}$ are iso.}
}\par\quad
{\Large\vspace{6pt}Define $\theta\in\Lm{\left(\PoR{}\right)\apostrophe,\Rbb^{\infty}}$ by $\theta(\varphi)=\left(\varphi(1),\varphi(x),\dots,\varphi(x^n),\dots\right).$}\par\quad
...
Let $\Par{u_1,\dots,u_m}$ be a basis of $U$, extend to a basis of $V$ as $(u_1,\dots,u_m,\dots,u_n)$,\par\quad
...
\ProblemN{25}{
	\TextNL{Suppose $U$ is a subsp of $V$. Explain why $U=\Bra{v\in V:\varphi(v)=0,\forall\varphi\in U^0}.$}
...
\ProblemN{26}{
	\TextNL{Suppose $V$ is finite-dim, $\Omega$ is a subsp of $V\apostrophe$. Prove that $\Omega=\Bra{v\in V:\varphi(v)=0,\forall\varphi\in\Omega}{^0}.$}
}Using the corollary in Problem (20, 21).\par\quad
Suppose $U=\Bra{v\in V:\,\forall\varphi\in\Omega,\varphi(v)=0}$.\par\quad
\dbsp Getting $U^0=\Bra{v\in V:\,\forall\varphi\in\Omega,\varphi(v)=0}{^0}$. We need to show that $\Omega=U^0$.\par\hspace{0pt}
(a) $\forall\varphi\in\Omega,v\in U,\varphi(v)=0\Rightarrow\varphi\in U^0\Rightarrow\Omega\subseteq U^0.$\par\quad
(b) $v\in U\Leftrightarrow\MathLeftBrace{l}{ \forall\varphi\in\Omega,\varphi(v)=0\\ \forall\psi\in U^0,\psi(v)=0}$ Thus $\Omega\supseteq U^0.$\PfEnd
\SepLine

\ProblemN{27}{
	\TextNL{Suppose $T\in\Lm{\PoR{5}}$ and $\null T\apostrophe=\Span{\varphi}$, where $\varphi\in\Lfm{\left(\PoR{5}\right)\!}$}
	\TextNL{defined by $\varphi(p)=p(8)$. Prove that $\range T=\Bra{p\in \PoR{5}:p(8)=0}.$}
}\par\quad
By Problem (26), $\Span{\varphi}=\Bra{p\in\PoR{5}:\psi(p)=0,\forall \psi\in\Span{\varphi}}{^0},$\par\quad
Hence $\Span{\varphi}=\Bra{p\in\PoR{5}:\varphi(p)=p(8)=0}{^0}$, 又 $\Span{\varphi}=\null T\apostrophe=\Par{\range T}{^0}$.\par\quad By the corollary in Problem (20, 21), $\range T=\left\{p\in\PoR{5}:p(8)=0\right\}.$\PfEnd
\SepLine
...
$\MathRightBrace{r}{$
$\left((\null \varphi_1)\cap\dots\cap(\null \varphi_m)\right)^0=(\null \varphi_1)^0+\dots+(\null \varphi_m)^0$.$ \\ $
又 $\Span{\varphi_i}=\Bra{v\in V:\forall\psi\in\Span{\varphi_i},\psi(v)=0}{^0}=\Par{\null \varphi_i}{^0}.}\Rightarrow(a)=(b).$\PfEnd\vspace{10pt}
...
$}$\par\quad
(b) $\MathLeftBrace{l}{$
Suppose $\Gamma$ is surj. Then let $\Gamma(\varphi_i)=e_i$ for each $i$, where $\Par{e_1,\dots,e_m}$ is the standard basis of $\Fbb^m$.$ \\ $
...
\qquad Thus for each $\Par{a_1,\dots,a_m}\in\Fbb^m,\varphi=a_1\varphi_1+\dots+a_m\varphi_m$ so that $\Gamma(\varphi)=\Par{a_1,\dots,a_m}.$
$}$\PfEnd[-10pt]
\SepLine
...
$}$\par\quad
(d) $\MathLeftBrace{l}{$
Suppose $\Par{\varphi_1,\dots,\varphi_m}$ is linely inde. Then by Problem (31), $\Par{v_1,\dots,v_m}$ is linely inde.$\\ $\qquad Thus for any $(a_1,\dots,a_m)\in\Fbb$, by letting $v=\sum_{i=1}^m a_i v_i$, then $\varphi_i(v)=a_i\Rightarrow\Gamma(v)=(a_1,\dots,a_m).\\ $
Suppose $\Gamma$ is surj. Let $e_1,\dots,e_m$ be a basis of $\Fbb^m$.$\\ $\qquad For every $e_i,\,\,\exists\,v_i\in V$ such that $\Gamma(v_i)=\left(\varphi_1(v_i),\dots,\varphi_m(v)\right)=e_i$,$\\ $\qquad fix $v_i$ ($\Rightarrow \Par{v_1,\dots,v_m}$ is linely inde). Thus $\varphi_i(v_i)=1,\varphi_i(v_j)=0.\\ $\qquad Hence $\Par{\varphi_1,\dots,\varphi_m}$ is the dual basis of the basis $v_1,\dots,\varphi_m$ of $\Span{v_1,\dots,v_m}.}$\PfEnd
\SepLine
...
Let $\Par{\varphi_1,\dots,\varphi_m}$ be the dual basis of ${v_1,\dots,v_m},\,\,\,(\psi_1,\dots,\psi_m)$ be the dual basis of $\Par{u_1,\dots,u_m}$.\par\quad
...
\ProblemN{37}{
	\TextNL{Suppose $U$ is a subsp of $V$ and $\pi$ is the quotient map. Thus $\pi\apostrophe\in\Lm{(V/U)\apostrophe,V\apostrophe}$.}
...
Then $\Mid{p(z)}\rightarrow\infty$ as $\left|z\right|\rightarrow\infty$ ( because $\displaystyle\frac{\Mid{p(z)}}{\left|z_m\right|}\rightarrow\left|c_m\right|$ as $\left|z\right|\rightarrow\infty$ ).\TextB{}
...
The function $z\rightarrow\Mid{q(z)}$ has a global minimum value of $1$ at $z = 0$.\TextB{}
...
$t\in (0, 1)$, then $\Mid{q(t\beta)}\leq\left|1 + a_k t^k\beta^k\right|+t^{k+1}c = 1 - t^k (1 - tc)$.\TextB{}
Now letting $t=1/(2c)$, we get $\Mid{q(t\beta)}<1$. Contradicts. Hence $p(\zeta) = 0$, as desired.\PfEnd
\SepLine
...
Suppose $\Par{v_1,\dots,v_m}$ is linely inde, extend it to a basis of $V$ as $(v_1,\dots,v_m,\dots,v_n)$.\par\quad
...
Then $\Par{v_1,\dots,v_n}$ is a basis of $V.$ Because $(ST)v_j=\alpha_j\lambda_j v_j=(TS)v_j$ for each $j.$ Hence $ST=TS.$\PfEnd
\SepLine


\ProblemB{
	\TextB{Suppose $V$ is finite-dim and $T\in\Lm{V}$.}
	\TextB{Define $\mathcal{A}\in\Lm{\Lm{V}}$ by $\mathcal{A}(S) = TS$ for each $S\in\Lm{V}$.}
...
so that $\range S_k=\Span{v_k}$ for each $k\in\!\Bra{1,\dots,m}$, then $\mathcal{A}(S_k)=TS_k=\lambda_k S_k.$\par\quad\Ha
...
\ProblemB{
	...
	\TextB{Prove that if $\lambda$ is an eigval of $T$, then $\left|\lambda\right|\leq n \max\left\{\Mid{A_{j,k}}:1\leq j, k\leq n\right\}.$}
}\par\quad
First we show that $\left|\lambda\right|=n\max\left\{\Mid{A_{j,k}}:1\leq j, k\leq n\right\}$ for some cases.\par\quad
Consider $A=\,${\small$\begin{pmatrix}
k & \cdots & k\\
\vdots & \ddots & \vdots\\
k & \cdots & k
\end{pmatrix}.$} Then $nk$ is an eigval of $T$ with an eigvec $v_1+\dots+v_n.$\par\quad
Now we show that if $\left|\lambda\right|\neq n\max\left\{\Mid{A_{j,k}}:1\leq j, k\leq n\right\},$ then $\left|\lambda\right|<n\max\left\{\Mid{A_{j,k}}:1\leq j, k\leq n\right\}.$\par\quad
\par
\SepLine
...
Thus $\Par{v,Tv}$ is linely inde. Extend to a basis of $V$ as $(v,Tv,u_1,\dots,u_n).$\par\quad
...
Let $\Par{u_1,\dots,u_m}$ be a basis of $U$, extend to a basis of $V$ as $(u_0,u_1,\dots,u_m).$\par\quad\Hb
...
By Problem (4E 5.B.11), $\Mt{T,\Par{(1,0),(0,1)}}=\,${\small$\begin{pmatrix}0 & -1\\ 1 & 0\end{pmatrix}$}$\Rightarrow$ the mini poly of $T$ is $z^2+1.$\par\quad
