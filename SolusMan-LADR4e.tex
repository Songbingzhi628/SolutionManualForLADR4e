...
\def\envFontSmall{\def\envFontA{\Large}\def\envFont{\normalsize}\def\envFontB{\small}}
...
\newcommand{\BigPar}[1]{\text{\forbra(\hspace{1pt}}#1\text{\hspace{1pt}\forbra)}}
\newcommand{\XPar}[1]{\text{\forbrax(\hspace{1pt}}#1\text{\hspace{1pt}\forbrax)}}
\newcommand{\Sbra}[1]{\text{\forbra[}#1\text{\forbra]}}
\newcommand{\XSbra}[1]{\text{\forbrax[}#1\text{\forbrax]}}
...
\Or Because $[\TXT{\tgbf{I}}]: \cos x+\sin\sqrt{2}x=\cos\Par{x+p}+\sin\BigPar{\sqrt{2}x+\sqrt{2}p}.$ By differentiating twice,\par\qquad\qquad\qquad\hspace{0pt}
$[\TXT{\tgbf{II}}]: \cos x+2\sin\sqrt{2}x=\cos\Par{x+p}+2\sin\BigPar{\sqrt{2}x+\sqrt{2}p}.$\par\vspace{4pt}\quad
$\MathRightBrace{r}{
	[\TXT{\tgbf{II}}]-[\TXT{\tgbf{I}}]\,:\;\;\sin\sqrt{2}x=\sin\BigPar{\sqrt{2}x+\sqrt{2}p}\vspace{2pt}\\ 
...
{\Large\vspace{6pt}Note that $W\cap U_1=\emptyset,$ for if $v+\lambda u\in U_1$ then $v+\lambda u-\lambda u=v\in U_1$.}\par\quad\Hb\HII
...
(b) \hspace{-10pt}{$\MathRightBrace{r}{f_e\in V_E\Longleftrightarrow f_e\Par{x}=f_e\Par{-x}\Longleftarrow$  let $f_e\Par{x}=\displaystyle\frac{g\Par{x}+g\Par{-x}}{2}\vspace{4pt}\\ f_o\in V_O\Longleftrightarrow f_o\Par{x}=-f_o\Par{-x}\Longleftarrow$ let $f_o\Par{x}=\displaystyle\frac{g\Par{x}-g\Par{-x}}{2}}\Rightarrow \forall g\in\Rbb^\Rbb,g\Par{x}=f_e\Par{x}+f_o\Par{x}.$}\PfEnd
\SepLine
\ChEnd
...
Now we can let $b_i=\sum_{r=1}^i a_r$ if we are to prove $Q$ with $P$ already assumed;\par\quad\Blind{Now we c\hspace{-1pt}}
or let $a_i=b_i-b_{i-1}$ with $b_{0}=0,$ if we are to prove $P$ with $Q$ already assumed.\PfEnd
\SepLine
...
$\Par{\overbrace{\underbrace{v_1,v_2,v_3}_\TXT{Basis of U},v_4,v_5,v_6}^\TXT{Basis of V}},$ define $\hMath[0pt]{l}{\left|}{\right\}}{$
...
$\Longleftrightarrow\Par{\mathcal{E}_1\cap\dots\cap\mathcal{E}_{k-1}}\cap\mathcal{E}_k=\emptyset$ for each $i$ 又 $\dim\Span{\mathcal{E}_1\cup\dots\cup\mathcal{E}_m}=\card\Par{\mathcal{E}_1\cup\dots\cup\mathcal{E}_m}$\par\Blind{\Comment\,\,}
...
\AlignEq{\hspace{20pt}}{\Dim\Par{V_1+V_2+V_3}=\dim &\,V_1+\dim V_2+\dim V_3\\&-\displaystyle\frac{\Dim\Par{V_1\cap V_2}+\Dim\Par{V_1\cap V_3}+\Dim\Par{V_2\cap V_3}}{3}\\&-\displaystyle\frac{\dim\BigPar{\Par{V_1+V_2}\cap V_3}+\dim\BigPar{\Par{V_1+V_3}\cap V_2}+\dim\BigPar{\Par{V_2+V_3}\cap V_1}}{3}.}\par\quad
...
\ProblemN{10}{
	...
	\TextNL{Define $T: V\rightarrow W$ by $Tv=${\large\envFontDefault$\MathLeftBrace{l}{Sv$, if $v\in U,\\ 0$,\,\,\, if $v\in V\backslash U$.$}$} Prove that $T$ is not a linear map on $V$.}
}\par\quad
...
\Or \envFontLarge{\Large\vspace{6pt}\,\dbsp We prove the contrapositive:}\par\quad\Blind{\Or}
{\Large\vspace{6pt}\dbsp Suppose $\forall w_1,\dots,w_m\in W,\,\exists\,T\in\Lm{V,W},Tv_k=w_k$  for each $w_k.$}\par\quad\Blind{\Or}
{\Large\vspace{12pt}\dbsp \BigPar{We need to} Prove that $\Par{v_1,\dots,v_n}$ is linely inde.}\par\quad
...
\ProblemBor{3.D.16}{
	\TextB{Suppose $V$ is finite-dim and $T\in\Lm{V}$ such that $\forall S\in\Lm{V},ST=TS$.}
...
{\Large\vspace{6pt}$\Par{R_{k,y}S}\Par{v_i}=a_k v_y\Rightarrow\BigPar{\Par{R_{k,y}S}\circ R_{x,i}}\Par{v_x}=a_k v_y,\,\,\,\BigPar{\Par{R_{k,y}S}\circ R_{x,i}}\Par{v_z}=0\,\Par{\,z\neq x\,}$.}\par\quad
...
\Comment \,\,\,If $\dim V=0,$ then $V=\zeroSubs=\Span{\,}$. $\forall\,\,T\in\Lm{V,W}, T$ is inje. Hence $U=\emptyset$.\par\Blind{\Comment\,\,}
...
\Comment \,\,\,If $\dim W=0,$ then $W=\zeroSubs=\Span{\,}$. $\forall\,\,T\in\Lm{V,W}, T$ is surj. Hence $U=\emptyset.$\par\Blind{\Comment\,\,}
...
}By [2.34] ( note that $V$ can be infinite-dim ), $\exists$ subsp $U$ of $V$ such that $V=U\oplus\null T$.\par\Blind{\Solution\!}
...
Define $T\in\Lm{V,W}$ by $T\Par{v_i}=w_i,T\Par{u_j}=0.$\vspace{4pt}\par\quad
...
\Or {\normalsize\envFontSmall Let $\Par{R_{1,\cdot},\dots,R_{r,\cdot}}$ be a basis of $S_r,$ forming $R\in\Fbb^{c,n}$.}\par\quad
...
\Or {\normalsize\envFontSmall For any $k,\;A_{j,\cdot}=C_{j,1}R_{1,\cdot}+\dots+C_{j,c}R_{c,\cdot}=\Par{CR}_{j,\cdot},\,\exists\,!\,C_{j,1},\dots,C_{j,c}\in\Fbb,$ forming $C\in\Fbb^{m,c}.$}\par\quad
...
Then $\Mt[\BigPar]{ p\Par{T_1,\dots,T_n}}={\Mt[\XPar]{\sum_{k_1,\dots,k_n}\!\!\alpha_{k_1,\dots,k_n}\prod_{i=1}^n T_i^{k_i}}}$\vspace{4pt}\par\Blind{Then $\Mt[\BigPar]{ p\Par{T_1,\dots,T_n}}=$}
...
{Define $E_{i,j}\in\Lm{V,W}$ by $E_{i,j}\Par{v_x}=\delta_{ix} w_j;\quad\delta_{ix}=\MathLeftBrace{l}{0,\quad i\neq x\\1,\quad i=x}\quad$} \Corollary\,\,\, \envFontLarge{\Large$E_{l,k}E_{i,j}=\delta_{jl}E_{i,k}.$}\TextB{}
{\Large Denote \envFontLarge$\Mt{E_{i,j}}$ by \envFontSmall$\mathcal{E}^{\Par{j,i}}.$ And \envFontDefault$\BigPar{\mathcal{E}^{\envFontSmall\Par{j,i}}}_{l,k}=\MathLeftBrace{l}{0,\quad i\neq k\,\vee\,j\neq l\\1,\quad i=k\,\wedge\,j=l}$}\TextB{\vspace{6pt}}
\envFontLarge{\Large Because $\Lm{V,W}$ and $\Fbb^{m,n}$ are iso. And $T=\Mneg\Mt{T};\;\;E_{i,j}=\Mneg\mathcal{E}^{\envFontSmall\Par{j,i}}$.}\TextB{}
...
$\therefore\,\Lm{V,W}=\Spn${\normalsize$\underbrace{\begin{pmatrix} \underset{,}{E_{1,1}}, & \cdots & ,\underset{,}{E_{n,1}},\\ \vdots & \ddots & \vdots\\ \overset{,}{E_{1,m}}, & \cdots & ,\overset{,}{E_{n,m}}\end{pmatrix}}_{B}$}$;\quad\Fbb^{m,n}=\Spn${\normalsize\envFontSmall$\underbrace{\begin{pmatrix} \underset{,}{\mathcal{E}^{\Par{1,1}}}, & \cdots & ,\underset{,}{\mathcal{E}^{\Par{1,n}}},\\ \vdots & \ddots & \vdots\\ \overset{,}{\mathcal{E}^{\Par{m,1}}}, & \cdots & ,\overset{,}{\mathcal{E}^{\Par{m,n}}}\end{pmatrix}}_{B_{\mathcal{M}}}$}.\TextB{\vspace{12pt}}
...
{\vspace{4pt}Let {\Large$S=E_{1,1}+\dots+E_{m,m}\Rightarrow \Mt[\BigPar]{S,\Par{v_1,\dots,v_n}}=\Mt[\BigPar]{I,\Par{w_1,\dots,w_n},\Par{v_1,\dots,v_n}}$}.}\par\quad
...
Suppose $A=a+U.$ Then $\forall a+u_1,a+u_2\in A,\lambda\in\Fbb,$\par\qquad
...
(I) $\lambda\Par{x-a}=[\lambda x+\Par{1-\lambda}a]-a\in A\apostrophe.$\par\quad\EndI
(II) $\lambda\Par{x-a}+\Par{1-\lambda}\Par{y-a}=\frac{1}{2}\Par{x-a}+\frac{1}{2}\Par{y-a}=\frac{1}{2}x+\Par{1-\frac{1}{2}}y-a\in A\apostrophe.$\par\quad\HII
...
(I) $\Par{x-\frac{1}{2}a} + \Par{y-\frac{1}{2}a}=x+y-a\in A\Rightarrow x+y-2a=\Par{x-a}+\Par{y-a}\in A\apostrophe.$\par\quad\EndI
...
(ii) $2\leqslant k\leqslant m,$ we assume that $v=\lambda_1 v_1+\dots+\lambda_k v_k\in A\subseteq B.$\,\, ($\forall\lambda_i$ such that $\sum\limits_{i=1}^k\lambda_i=1$)\par\quad\Hb\Hii
...
Thus define $\varphi\in\Lm[\BigPar]{V,U\times\Par{V/U}}$ by $\varphi\Par{v}=\Par{u,\sum\limits_{i=1}^n a_i v_i+U}$\par\qquad\qquad
and $\psi\in\Lm[\BigPar]{U\times\Par{V/U},V}$ by $\psi\Par{u,w+U}=u+w;w=\sum\limits_{i=1}^n b_i v_i+U$.\par\quad
...
{\large Define} $e_r[p]=\MathLeftBrace{l}{1\,,\Par{p-1}\equiv 0\,\Par{\TXT{mod}\,r}\\0\,,\TXT{otherwise}}$, {\large simply} $e_r=\Par{1,\underbrace{0,\dots,0}_{\SmallPar{p-1}\,\,times},1,\underbrace{0,\dots,0}_{\SmallPar{p-1}\,\,times},1,\dots}\in\Fbb^{\infty}$.\par\quad
...
then let $j=s\cdot m!+1\geqslant t\,\,\Par{\exists\,\,s\in\Nbp}$ {\large so that} $e_1[j]=\cdots=e_m[j]=1,\,\,u[j+i]=0.$\par\vspace{6pt}\quad
Now we have: $u[j+i]=\Par{\sum_{r=1}^m a_r e_r}[j+i]=\sum_{r=1}^m a_r e_r[s\cdot m!+1+i]=a_{i_1}+\dots+a_{i_{\tau\SmallPar{i}}}=0$,\par\vspace{6pt}\quad
$\Rightarrow\Par{\sum_{r=1}^m a_r e_r}[j+i]=a_{i_1}+\dots+a_{i_{\tau\SmallPar{i}}}=0$.\,\,\,$\Par{\Delta}$\par\quad
{\large where} $i_1,\dots,i_{\tau\SmallPar{i}}$ {\large are distinct ordered factors of} $i$ \BigPar{ $1=i_1\leqslant\dots\leqslant i_{\tau\SmallPar{i}}=i$ }.\par\quad
{\large$\BigPar{\begin{array}{l}$
...
{\large Let} $i\apostrophe=i_{\tau\SmallPar{i}-1}$. {\large Notice that} $i\apostrophe_l=i_l,\forall l\in\Bra{1,\dots,\tau\SmallPar{i\apostrophe}};\,\,\,$ {\large and} $\tau\SmallPar{i\apostrophe}=\tau\SmallPar{i}-1$.\par\vspace{6pt}\quad
{\large Again by $\Par{\Delta}$,} $\Par{\sum_{r=1}^m a_r e_r}[j+i\apostrophe]=a_{i\apostrophe_1}+\dots+a_{i\apostrophe_{\tau\SmallPar{i\apostrophe}}}=a_{i_1}+\dots+a_{i_{\tau\SmallPar{i}-1}}=0.$\par\vspace{6pt}\quad
{\large Thus} $a_{i_\tau\SmallPar{i}}=a_i=0$ {\large for any} $i\in\Bra{1,\dots,m}$.\par\quad
...
%\Largesl{
%Define $\xi\in\Lm[\BigPar]{V/U,\mathcal{S}_V U}$ by $\xi(v+U)=a_1 v_1+\dots+a_n v_n=(\xi\circ\pi)(v)\in\mathcal{S}_V U$.}\par\quad
%$\MathRightBrace{l}{\xi\circ(\pi\mid_{\mathcal{S}_V U})=I \\ \pi\circ\xi=I}\Rightarrow$ $V/U$ and $\mathcal{S}_V U$ are iso.\par
\SepLine
...
Then $\Par{v_1-v_2}\in U\Rightarrow S\BigPar{\Par{v_1-v_2}+U}=T\Par{v_1-v_2}=0\Rightarrow Tv_1=Tv_2.$ Checked.\par\quad
...
\ProblemNnosolus{20}{
	...
	\Hb\TextNL{{\large\envFontDefault $\Gamma\Par{S}=0=S\circ\pi\Longleftrightarrow\forall v\in V,S\Par{\pi\Par{v}}=0\Longleftrightarrow\forall v+U\in V/U,S\Par{v+U}=0\Longleftrightarrow S=0.$}}
...
{\Large\vspace{6pt}Define $\theta\in\Lm[\BigPar]{\Par{\PoR{}}\apostrophe,\Rbb^{\infty}}$ by $\theta\Par{\varphi}=\Par{\varphi\Par{1},\varphi\Par{x},\dots,\varphi\Par{x^n},\dots}.$}\par\quad
...
\ProblemN{7}{
	\TextN{Show that the dual basis of $\Par{1, x,\dots,x_m}$ of $\PoR{m}$ is $\Par{\varphi_0,\varphi_1,\dots,\varphi_m}$, where {\large\envFontDefault $\varphi_k=\displaystyle\frac{p^{\Par{k}}\Par{0}}{k!}$.}\vspace{3pt}}
	\TextN{{\large\envFontDefault Here $p^{\Par{k}}$ denotes the $k^{th}$ derivative of $p$, with the understanding that the $0^{th}$ derivative of $p$ is $p$.}}
}\par\quad
...
\ProblemNnosolus{8}{
	...
	\vspace{4pt}(a) \TextN{\tgnr{\large\envFontDefault By [2.C.10], $B$} $=\Par{1,x-5,\dots,\Par{x-5}^m}$ is a basis of $\PoR{m}$.}
...
\ProblemNnosolus{13}{
	...
	\Ha\TextNL{{\large\tgnr\envFontDefault For any $\Par{x,y,z}\in\Rbb^3$, $\Par{T\apostrophe\Par{\varphi_1}}\Par{x,y,z}=4x+5y+6z, \Par{T\apostrophe\Par{\varphi_2}}\Par{x,y,z}=7x+8y+9z$.}\vspace{8pt}}
...
\ProblemNnosolus{14}{
	...
	\Ha\TextNL{\large\envFontDefault$\Par{T\apostrophe\Par{\varphi}}\Par{p}=[x^2p\Par{x}+p\apostrophe\apostrophe\Par{x}]\apostrophe\Par{4}=[2xp\Par{x}+x^2p\apostrophe\Par{x}+p\apostrophe\apostrophe\apostrophe\Par{x}]\Par{4}=8p\Par{4}+16p\apostrophe\Par{4}+p\apostrophe\apostrophe\apostrophe\Par{4}$.\vspace{8pt}}
	...
	\Hb\TextNL{\large\envFontDefault $\Par{T\apostrophe\Par{\varphi}}\Par{x^3}=\int_0^1\Par{x^5+6x}\d x=\int_0^1\Par{\frac{1}{6}x^6+3x^2}\apostrophe\d x=\frac{6}{19}.$\PfEnd}
}\SepLine
...
Suppose $\deg p\geqslant \deg s$. Then $\Par{\underbrace{1, z,\dots, z^{\deg s-1}}_{\TXT{of\,length}\,\deg s},\underbrace{s,zs,\cdots,z^{\deg p-\deg s}s}_{\TXT{of\,length}\,\Par{\deg p-\deg s+1}}}$ is a basis of $\PoF{\deg p}.$\TextB{}
...
\ProblemB{
	...
	\TextB{Prove that if $\lambda$ is an eigval of $T$, then $\left|\lambda\right|\leq n \max\Bra{\Mid{A_{j,k}}:1\leqslant j, k\leqslant n}.$}
}\par\quad
...
\TextB{Prove that $\exists\,!\,p_k\in M_T^0,\deg p_k=\min${\large\envFontDefault$\Bra{\deg p_j}{_{j\in\Gamma}}$}$\leq\dim V$.}
...
{\Large The mini poly has the form $\Par{z-\alpha_1}^{\delta_1}\cdots\Par{z-\alpha_n}^{\delta_n},$ where $0\leqslant \delta_1+\dots+\delta_n\leqslant \dim V.$}\par
\SepLine
