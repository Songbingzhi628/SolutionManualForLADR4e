...
}(a) {$V_E\cap V_O=\Bra{\,f\in\Rbb^{\Rbb}:f\Par{x}=f\Par{-x}=-f\Par{-x}}=\zeroSubs.$}\par\vspace{12pt}
\Blind{\Solution}(b) $\MathRightBrace{l}{$
...
\ChDecl{Ch2A}{2$\cdot$A}{}\orMode{\hLk{2A1}{1}\;\;\hLk{2A2}{2}\;\;\hLk{2A6}{6}\;\;\hLk{2A10}{10}\;\;\hLk{2A11}{11}\;\;\hLk{2A14}{14}\;\;\hLk{2A16}{16}\;\;\hLk{2A17}{17}\;\;|\;\;\hLk{2A4e314}{4E: 3,14}}{[1]: 2; [2]: 1, 6, (4E 3, 14), 10; [3]: 11, 14, 16, 17.}
...
$[P]\Rightarrow[Q]:\;$ Notice that $\varphi$ is inje and by (3.B.9).\vspace{4pt}\par\quad
...
\Blind{$[P]\Rightarrow[Q]:\;$} Then $\vartheta\Par{1}=0=a_1 v_1+\dots+a_m v_m\Rightarrow a_1=\dots=a_m=0.$\vspace{6pt}\par\quad
...
\ProblemB{
	\TextB{Suppose $U$ is a subsp of $V.$ Prove that $\Par{V/U}\apostrophe$ and $U^0$ are iso. \hfill[{\large Another proof of [3.106]}]}
}\par\quad
...
Where $T^{-1}\mid_{\range T}:\range T\rightarrow U;\;\;T:V\rightarrow\range T.$\vspace{2pt}\par\quad
Note that $T^{-1}\mid_{\range T}\circ T\mid_V=i$ defined by $i\Par{v}=u_v.$\vspace{2pt}\par\quad
By \TIPS\;in (3.B), $\varPhi=\varPhi\circ i.$ Thus $T\apostrophe\Par{\psi}=\psi\circ T=\varPhi\circ{i}=\varPhi.$\PfEnd
\SepLine
...
\ProblemB[]{
	...
	\TextB{Now $T$ is inv $\Longleftrightarrow${$\envFontDefault\large\MathLeftrightMid{c}{
		...
		\range T=W\Longleftrightarrow\BigPar{\range T}{^0}=\zeroSubs=\null T\apostrophe
...
Suppose $T\apostrophe=0.$ Then $\null T\apostrophe=W\apostrophe=\BigPar{\range T}{^0},$  by [3.107](a).\par\quad
[{\tgsl$W$ can be infinite-dim}] By Problem (25),\par\qquad
$\range T=\Bra{w\in W:\varphi\Par{w}=0,\forall\varphi\in\BigPar{\range T}{^0}}=\Bra{w\in W:\varphi\Par{w}=0,\forall\varphi\in W\apostrophe}.$\par\quad
...
\Or [{\tgsl Only if $W$ is finite-dim}] By [3.106], $\dim \range T=\dim W-\dim\BigPar{\range T}{^0}=0.$\PfEnd
\SepLine\pagebreak
...
\Blind{\Comment \,\,\,}(b) Suppose $T$ is surj. Then $\BigPar{\range T}{^0}=W_W^0=\zeroSubs=\null T\apostrophe.$\PfEnd
\SepLine

\ProblemB[]{
	\TextB{Suppose $V$ is a vecsp and $U$ is a subsp of \,$V.$\vspace{4pt}}
	\ProblemN[]{}{}
	...
	\ProblemN[]{\hypertarget{3F18}{18}}{
		\TextNL{$U^0=V\apostrophe\Longleftrightarrow\forall\varphi\in V\apostrophe, U\subseteq\null \varphi\,\Longleftrightarrow U=\zeroSubs$. [{\envFontDefault\large Which means $\zeroSubs{^0_V}=V\apostrophe.$}]\vspace{3pt}}
	...
	\ProblemN[]{\hypertarget{3F19}{19}}{
		\TextNL{\tgnr $U^0_V=\zeroSubs=V^0_V\Longleftrightarrow U=V$. {\large\tgnr By the inverse and contrapositive of Problem (4). \Or By [3.106].}}}
	\TextB{}
}\SepLine
...
$\psi\Par{v}=\psi\XPar{\sum_{i=1}^n a_i v_i}=\sum_{i=1}^n a_i\psi\Par{v_i}=\sum_{i=1}^n\psi\Par{v_{i}}\varphi_i\Par{v}=\Sbra{\psi\Par{v_1}\varphi_1+\dots+\psi\Par{v_n}\varphi_n}\Par{v}.$\par\quad
...
Suppose $V=\null\varphi_i\oplus\Bra{au_i:a\in\Fbb}$ for each $\varphi_i.$ Because $\Par{\varphi_1,\dots,\varphi_m}$ is linely inde.\par\quad
$\forall j,k\in\Nbp$ such that $j\neq k,\null\varphi_j\neq\null\varphi_k,$ for if not $\varphi_j=c\varphi_k$ \BigPar{ Each $\varphi_i\neq 0.$ }.\par\quad
Thus $\Par{u_1,\dots,u_m}$ is linely inde, for if not,\par\qquad
then $\Bra{au_j:a\in\Fbb}=\Bra{bu_k:b\in\Fbb}\Rightarrow\null\varphi_j=\null \varphi_k$ while $j\neq k.$ Contradicts.\par\quad
又 $\dim V\apostrophe=m=\dim V$. Then $B_V=\Par{u_1,\dots,u_m}$.\PfEnd\vspace{8pt}\quad
\Or 
\SepLine
...
\Or Using (3.B.24). Note that $\null\varphi\subseteq\null\beta\Longleftrightarrow\exists\,E\in\Lm{\Fbb},\beta=E\circ\varphi.$ \Par{ $E$ is inv. $\null\varphi=\null\beta$ }\par\quad
...
$\varphi_k\BigPar{x^j}=\envFontSmall[\footnotesize]\frac{\Par{x^j}^{\envFontSmall[\tiny]\Par{k}}\Par{0}}{k!}=\frac{0}{k!}=0;\;\varphi_k\BigPar{x^k}=\frac{\Par{x^k}^{\envFontSmall[\scriptsize]\Par{k}}\Par{0}}{k!}=1.$\par\quad
\Blind{\Or \;}Thus $\varphi_k$ and $\envFontSmall[\scriptsize]\frac{p^{\Par{k}}\Par{0}}{k!}$ act exactly the same on the same basis $\Par{1,\dots,x^m}.$\par\quad
\Blind{\Or \;}Hence $\envFontSmall[\scriptsize]\frac{p^{\Par{k}}\Par{0}}{k!}$ is just another definition of $\varphi_k$.\PfEnd\vspace{4pt}
...
}Using \COROLLARY in Problem (20, 21).\par\quad
Suppose $U=\Bra{v\in V:\,\forall\varphi\in\Omega,\varphi\Par{v}=0}$.\par\quad
\dbsp Getting $U^0=\Bra{v\in V:\,\forall\varphi\in\Omega,\varphi\Par{v}=0}{^0}$. We need to show that $\Omega=U^0$.\par\quad
(a) $\forall\varphi\in\Omega,v\in U,\varphi\Par{v}=0\Rightarrow\varphi\in U^0\Rightarrow\Omega\subseteq U^0.$\par\quad
(b) $v\in U\Longleftrightarrow\MathLeftBrace{l}{ \forall\varphi\in\Omega,\varphi\Par{v}=0\\ \forall\psi\in U^0,\psi\Par{v}=0}$ Thus $\Omega\supseteq U^0.$\PfEnd
\SepLine

\ProblemN{\hypertarget{3F27}{27}}{
	\TextNL{Suppose $T\in\Lm[\BigPar]{\PoR{5}}$ and $\null T\apostrophe=\Span{\varphi}$, where $\varphi\in\BigPar{\PoR{5}}^{\apostrophe}$}
...
(a) Suppose $\Gamma$ is inje. Then $\Gamma\Par{\varphi}=0\Rightarrow\varphi=0 \Longleftrightarrow\null \varphi=V=\Span{v_1,\dots,v_m}$.\par\quad\Ha
Suppose $\Span{v_1,\dots,v_m}=V$. Then $\Gamma\Par{\varphi}=0\Rightarrow\varphi\Par{v_i}=0$ for each $i$, $\null\varphi=\Span{v_1,\dots,v_m}=V\Rightarrow\varphi=0.$\par\quad
...
Then $\Par{\varphi_1,\dots,\varphi_m}$ is linely inde, suppose $a_1 v_1+\dots+a_m v_m=0$, then for each $i$, we have $\varphi_{i}\Par{a_1 v_1+\dots+a_m v_m}=a_i= 0$. Thus $\Par{v_1,\dots,v_n}$ is linely inde.\par\quad\Hb
Suppose $\Par{v_1,\dots,v_m}$ is linely inde. Let $\Par{\varphi_1,\dots,\varphi_m}$ be the dual basis of $\Span{v_1,\dots,v_m}$.\par\quad\Hb
Thus for each $\Par{a_1,\dots,a_m}\in\Fbb^m,\varphi=a_1\varphi_1+\dots+a_m\varphi_m$ so that $\Gamma\Par{\varphi}=\Par{a_1,\dots,a_m}.$\PfEnd
\SepLine
...
\ProblemN{\hypertarget{3F36}{36}}{
	...
	(a) \TextNL{Show that null $i\apostrophe=U^0${\tgnr\large\envFontDefault:\;\;$\null i\apostrophe=\BigPar{\range i}{^0}=U^0\Leftarrow\range i=U$.}\vspace{2pt}}
...
\ProblemB{
	...
	(a) \TextB{Show that $\null i\apostrophe=U_U^0=\zeroSubs${\tgnr\large\envFontDefault:\;\;$\null i\apostrophe=\BigPar{\range i}{^0_U}=U_U^0=\zeroSubs.$}\vspace{2pt}}
