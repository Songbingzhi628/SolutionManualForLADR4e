...
\ProblemN[]{20}{
	\TextNL{Define $\Gamma:\Lm{V\mySlash U,W}\rightarrow\Lm{V,W}$ by $\Gamma\Par{S}=S\circ\pi.$ Prove that:\vspace{4pt}}
	(a) \TextNL{$\Gamma$ is linear: {\large\tgnr By [3.9] distr and [3.6].}\vspace{4pt}}
	(b) \TextNL{$\Gamma$ is inje: {\large\envFontDefault $\Gamma\Par{S}=0=S\circ\pi\Longleftrightarrow\forall v\in V,S\Par{\pi\Par{v}}=0\Longleftrightarrow\forall v+U\in V\mySlash U,S\Par{v+U}=0\Longleftrightarrow S=0.$}\vspace{4pt}}
...
\ProblemBnoor[]{\NoteForSmall{[3.79, 3.83]}}[]{
	{If $U=\zeroSubs,$ then $v+U=v+\zeroSubs=\Bra{v},V\mySlash U=V\mySlash \zeroSubs=\Bra{\Bra{v}:v\in V}.$}\TextB{}
}\SepLine
...
{\vspace{2pt}Now $\pi\circ\tilde{T}=I_{V\mySlash U},\;\tilde{T}\circ\pi=I_{W}=T\mid_W$ Hence $\tilde{T}$ is an iso of $V\mySlash U$ onto $W.$}\TextB{}
{\vspace{2pt}\Comment \,\,\,Note that $v=u_v+w_v=\Par{u_v-u'}+\Par{w_v'+u'},$ where $w_v'\not\in W\Longleftrightarrow u'\neq 0.$}\TextB{}
{\vspace{2pt}Define $S\in\Lm{V\mySlash U,V}$ by $S\Par{v+U}=v.$ Let $E=S\circ\pi.$ Then $\range E\oplus U=V,$ and $\null E=U.$}\TextB{}
Thus $\forall w\in\range S,S\Par{w+U}=w.$ Which is a big difference from $T$ defined above.
\SepLine
...
\Or [{\tgsl $V\mySlash U,U$ and $V$ can be infinite-dim}]\vspace{2pt}\par\quad
...
\ProblemN{\hypertarget{3F37}{37}}{
	...
	(c) \TextNL{Conclude that $\pi\apostrophe$ is an iso from $\Par{V\mySlash U}\apostrophe$ onto $U^0${\tgnr\large\envFontDefault:\;\;Immediately.}}
...
\ProblemB{
	...
	(c) \TextB{Prove that $\tilde{\iota\apostrophe}$ is an iso from $\envFontSmall U\apostrophe\mySlash \zeroSubs$ onto $W^0${\tgnr\large\envFontDefault:\;\;By (a), (b) and [3.91](d).}}
}\par\quad
...
\ProblemN{\hypertarget{3F36}{36}}{
	...
	(c) \TextNL{Prove that $\tilde{i\apostrophe}$ is an iso from $V\apostrophe\mySlash U^0$ onto $U\apostrophe${\tgnr\large\envFontDefault:\;\;By (a), (b) and [3.91](d).}\vspace{2pt}}
}\par\quad
...
\ProblemB{
	...
	\TextB{Prove that there exists an invar subsp $W$ such that $V=U\oplus W.$}
}\par\quad\PfEnd
\SepLine\pagebreak
...
Let $U$ be invar under $T$ and $V=\Span{v}\oplus U$ \Sbra{ by (4E 5.A.39) }.\par\quad\Ha
...
Thus $1$ is an eigval of $\Phi^{-1}T\apostrophe\Phi=S,$ so of $T\apostrophe,$ \Sbra{ by Problem (15) }, so of $T,$ \Sbra{ by (4E 5.A.15) }.\PfEnd
\SepLine\pagebreak
...
\ProblemN{\hypertarget{5A23}{23}}{
	\TextNL{Suppose $S, T\in\Lm{V}$. Prove that $ST$ and $TS$\, have the same eigvals.\vspace{4pt}}
...
\Blind{{\tgbfx Step 1.}} Then extend $w_1=\alpha_{1,1}$ to a basis of $\Span{v_2,\dots,v_n}$ as $\Par{\alpha_{1,1},\dots,\alpha_{1,n-1}}.$\par\quad\Ha
...
\Blind{{\tgbfx Step k.}} then extend $w_k=\alpha_{k,1}$ to a basis of $\Span{\alpha_{k-1,2},\dots,\alpha_{k-1,n-k+1}}$ as $\Par{\alpha_{k,1},\dots,\alpha_{k,n-k}}.$\par\quad\Ha
...
\Comment \,\,\,Both $\Span{v_2,\dots,v_n}$ and $U\oplus\Span{w_{m-1}}$ are in $\mathcal{S}_V \Span{v_1}.$\par\vspace{6pt}\quad\Ha
...
then $\exists\,u\in U\backslash\zeroSubs,Tu=0,$ we are done.\PfEnd\vspace{4pt}\quad
...
\ProblemN{\hypertarget{5A33}{33}}{
	\TextNL{Suppose $T\in\Lm{V}.$ Prove that $T\mySlash \Par{\range T}=0.$}
...
\ProblemB{
	...
	\TextB{The quotient operator $T\mySlash U\in\Lm{V\mySlash U}$ is defined by}
	\TextB{\centerline{$\Par{T\mySlash U}\Par{v + U} = Tv + U$ for each $v\in V$.}}
	(a) \TextB{Show that the definition of $T\mySlash U$ makes sense}
	\Ha\TextB{( which requires using the condition that $U$ is invar under $T$ )}
	\Ha\TextB{and show that $T\mySlash U$ is an operator on $V\mySlash U$.}
...
\ProblemN{\hypertarget{5A36}{36}}{
	\TextNL{Prove or give a counterexample: The result in Exercise 35 is still true if \,$V$ is infinite-dim.}
}A counterexample:\par\quad
