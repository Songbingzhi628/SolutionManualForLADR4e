...
\newcommand{\ProblemNnoor}[5][\Solution]{
...
{\vspace{2pt}Now $\pi\circ\tilde{T}=I_{V/U},\;\tilde{T}\circ\pi=I_{W}=T\mid_W$ Hence $\tilde{T}$ is an iso.}\par
\SepLine
...
\Blind{Thus de}and $\psi\in\Lm[\BigPar]{U\times\Par{V/U},V}$ by $\psi\Par{u,v+U}=v+u,$ where $\exists\,!\,a_i\in\Fbb,v=\sum_{i=1}^n a_i v_i+U.$\PfEnd\vspace{8pt}\quad
\Or [{\tgsl $V/U,U$ and $V$ can be infinite-dim}]\vspace{2pt}\par\quad
...
\ProblemBnoor[]{\Corollary}[]{
	...
	\Hc\TextB{\envFontDefault\large\tgnr Where $U_V^\apostrophe=\Bra{\varphi\in V\apostrophe:\varphi=\varphi\circ i^-}.$ And $i^-\in\Lm{V,U}$ is defined by $i^-\Par{u_v+w_v}=u_v.$}
}\SepLine[2pt]
...
\ProblemN{\hypertarget{3F36}{36}}{
	...
	(a) \TextNL{Show that $\null i\apostrophe=U^0${\tgnr\large\envFontDefault:\;\;$\null i\apostrophe=\Par{\range i}{^0}=U^0\Leftarrow\range i=U$.}\vspace{2pt}}
	(b) \TextNL{Prove that $\range i\apostrophe=U\apostrophe${\tgnr\large\envFontDefault:\;\;$\range i\apostrophe=\Par{\null i}{_U^0}={\zeroSubs}{_U^0}=U\apostrophe$.}\vspace{2pt}}
...
Because $V,W$ are finite-dim. $\dim\Lm{V,W}=\dim\Lm{W\apostrophe,V\apostrophe}$. Now $\Gamma$ inje $\Rightarrow$ inv.\PfEnd\vspace{6pt}
...
\Blind{\Solution} Surj: For any $B\in\Fbb^{n,m}$, let $A=B^t$ so that $T\Par{A}=B.$\PfEnd
\SepLine
...
Note that $T$ is inveritlbe $\Longleftrightarrow$ $T\apostrophe$ is inv. And Let $B=A^t=\Mt{T\apostrophe,B_{\beta}\apostrophe,B_{\alpha}\apostrophe}.$\par\quad
...
Thus the cols of $B$, namely the rows of $A$, are linely inde.\par\quad
(b) Suppose the rows of $A$ are linely inde, so are the cols of $B$.\colorbox{yellow}{TODO}\par\quad\Hb
