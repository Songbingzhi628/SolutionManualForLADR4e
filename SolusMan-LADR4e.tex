...
Now we can let $b_i=\sum\limits_{r=1}^i a_r$ if we are to prove $Q$ with $P$ already assumed;\par\qquad\qquad\quad\hspace{3pt}
...
}\vspace{6pt}\AlignEq{}{\qquad\qquad\qquad P\Rightarrow Q:\;&a_1(v_1-v_2)+a_2(v_2-v_3)+a_3(v_3-v_4)+a_4 v_4=0\hspace{154pt}\\\Rightarrow&a_1 v_1+(a_2-a_1)v_2+(a_3-a_2)v_3+(a_4-a_3)v_4=0\\\Rightarrow&a_1=a_2-a_1=a_3-a_2=a_4-a_3=0}\par\quad
\vspace{-10pt}
...
(a) $a_k=\sum\limits_{j=1}^k b_j\Leftarrow\;\;a_1 v_1+\dots+a_m v_m=b_1 w_1+\dots+b_m w_m\;\;\Rightarrow b_1=a_1,\;b_k=a_k-\sum\limits_{j=1}^{k-1}b_j=\sum\limits_{j=1}^k (-1)^{k-j}a_j.$\vspace{10pt}\par\quad
...
}\par\vspace{-6pt}\hfill$\square$\vspace{-10pt}\par\quad\Hb
...
\ProblemN{10}{
	...
	(b) \TextNL{Show that [P] $(v_1,\dots,v_m,w)$ is linely inde $\Longleftrightarrow w\not\in\Spn(v_1,\dots,v_m)$ [Q].}
}\par\quad
...
(b) ${}^\neg Q\Rightarrow{}^\neg P:$ Suppose $w\in\Spn(v_1,\dots,v_m)$. Then $(v_1,\dots,v_m,w)$ is linely depe.\par\quad\Hb
$Q\Rightarrow P:$ Suppose $w\not\in\Spn(v_1,\dots,v_m)$. Then by [2.23], $(v_1,\dots,v_m,w)$ is linely inde. $\hfill\square$\par
\ProblemEnding\par

\ProblemN{14}{
	\TextNL{Prove that [P] $V$ is infinite-dim $\Longleftrightarrow[Q]\left|\hspace{-8pt}\begin{array}{l}$ there is a sequence $(v_1,v_2,\dots)$ in $V\\[-8pt]$ such that $(v_1,\dots,v_m)$ is linely inde for each $m\in\Nbp.\end{array}\right.$}
}\par\quad
