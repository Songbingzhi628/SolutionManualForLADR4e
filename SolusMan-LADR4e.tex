...
\newcommand{\Mid}[2][-3pt]{{\text{\envFontA|}}#2{\text{\envFontA|}}}%{\hText[-2pt]{}{}{$\hspace{-1pt}{\envFontB$\hText[#1]{\left|}{\right|}{${\envFont$#2$}$}$}$}}
...
\ChDecl{Ch4}{4}{}\orMode{\hLk{4O2}{2}\;\;\hLk{4O3}{3}\;\;\hLk{4O4}{4}\;\;\hLk{4O5}{5}\;\;\hLk{4O6}{6}\;\;\hLk{4O7}{7}\;\;\hLk{4O8}{8}\;\;\hLk{4O9}{9}\;\;\hLk{4O10}{10}\;\;\hLk{4O11}{11}\;\;|\;\;\hLk{4O4e}{4E: 2,}\;\;\hLk{4O4e3}{3,}\;\;\hLk{4O4e13}{13}}{[1]: ; [2]: ; [3]: ; [4]: ; [5]: .}
...
\ProblemBnoor{\hypertarget{4O4e2}{4E 4 2}}{
	\TextB{Prove that if $w,z\in\Cbb,$ then $\Mid{|w|-|z|}\leqslant|w-z|$.}
}\par\quad
\vspace{-6pt}\AlignEq{}{
	\Mid{w-z}{^2}&=\Par{w-z}\Par{\overline{w}-\overline{z}}\hspace{380pt}\\
	&=|w|^2+|z|^2-\Par{w\overline{z}+\overline{w}z}\\
	&=|w|^2+|z|^2-\BigPar{\overline{\overline{w}z}+\overline{w}z}\\
	&=|w|^2+|z|^2-2Re\Par{\overline{w}z}\\
	&\geqslant|w|^2+|z|^2-2\Mid{\overline{w}z}\\
	&=|w|^2+|z|^2-2|w||z|=\Mid{|w|-|z|}{^2}.}\par
...
}Let $p\Par{z}=\Par{z-\lambda_1}^{{\envFontSmall[\scriptsize]n-\Par{m-1}}}\Par{z-\lambda_2}\cdots\Par{z-\lambda_m}.$\PfEnd\vspace{-3pt}
\SepLine

\ProblemBnoor[]{\Tips \,\,\,}[]{
	\TextB{\tgsl\large\envFontDefault Suppose $z_1,\dots,z_{m+1}$ are distinct in $\Fbb,$ and $p\in\PoF{m}$ is such that $p\Par{z_1}=\cdots=p\Par{z_{m+1}}=0.$}
	\Blind{\Tips \,\,\,} \TextB{\tgsl\large\envFontDefault Then $p=0,$ for if not $p$ has $\Par{m+1}$ distinct zeros while $\deg p\leqslant m,$ contradicts [4.12].}
	\TextB{The contrapositive of [4.12]. \OR $p$ has $m$ distinct zeros $\Rightarrow\deg p\geqslant m.$\vspace{-4pt}}
}\SepLine
...
Let $q_{j}=p_{j-1}$ for each $j\in\Bra{1,\dots,m+1}.$ Then by (2.C.10), $\Par{q_1,\dots,q_m}$ is a basis of $\PoF{m}.$ Let $\Par{e_1,\dots,e_{m+1}}$ be the standard basis of $\Fbb^{m+1}.$\par\quad
...
\end{pmatrix}$}, where $A_{j,\kappa}=\prod_{i=1}^{\kappa-1}\Par{z_j-z_i}$ for $j\geqslant k\geqslant 1.$\par
...
Let $\lambda_1,\dots,\lambda_M$ and $\mu_1,\dots,\mu_N$ be the distinct zeros of $p$ and $q$ respectively. \NOTICE that $M\leqslant m,N\leqslant n.$\par\quad
Note that $p,q$ are nonconst $\Rightarrow M,N\geqslant 1.$ Now $p\Par{\mu_i},q\Par{\lambda_j}\neq 0$ for each $\mu_i$ and $\lambda_j.$\par\quad
Suppose $T\Par{r,s}=rp+sq=0.$ Then for each $\mu_i,r\Par{\mu_i}p\Par{\mu_i}+s\Par{\mu_i}q\Par{\mu_i}=r\Par{\mu_i}p\Par{\mu_i}=0\Rightarrow r\Par{\mu_i}=0.$\par\quad
Now $r$ has at least $N$ distinct zeros $\Bra{\mu_1,\dots,\mu_N}$ while $\deg r\leqslant n-1.$\par\quad
Similarly, $s$ has at least $M$ distinct zeros $\Bra{\lambda_1,\dots,\lambda_M}$ while $\deg s\leqslant m-1.$\par\quad
If $m=M$ and $n=N.$ Then by \TIPS, $r,s=0.$\par\quad
If $m\neq M$ or $n\neq 0.$ Because $rp+sq=0.$ Let $K\geqslant\max\Bra{m-M,n-N}.$\par\quad
Choose distinct $\gamma_0,\gamma_1,\dots,\gamma_K\in\Cbb$ such that each $\gamma_k\not\in\Bra{\lambda_1,\dots,\lambda_M,\mu_1,\dots,\mu_N}$\vspace{3pt}\par\quad
$\Rightarrow\Par{rp}\Par{\gamma_k}+\Par{sq}\Par{\gamma_k}=0\Rightarrow\displaystyle\frac{r\Par{\gamma_k}}{q\Par{\gamma_k}}=-\frac{s\Par{\gamma_k}}{p\Par{\gamma_k}}=0\Rightarrow r\Par{\gamma_k}=s\Par{\gamma_k}=0.$ By \TIPS, $r=s=0.$\vspace{6pt}\par\quad
