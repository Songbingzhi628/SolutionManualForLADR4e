...
\ProblemBnoor{({\normalsize 4E 3.B.21})\TextB{}}{
	\hypertarget{3B4e21}{}\TextB{Suppose $V$ is finite-dim, $T\in\Lm(V, W)$, $U$ is a subsp of \,$W$. Let $\mathcal{K}_U=\left\{\,v\in V: Tv\in U\right\}.$}
...
Suppose $v_1,\dots,v_m\in V$ such that $\forall i,Tv_i=w_i.$ Then $(v_1,\dots,v_m)$ is linely inde, let $B_V=(v_1,\dots,v_m,u_1,\dots,u_n)$.\par\quad
...
$\because(Ac)_{j,1}=\sum_{r=1}^n A_{j,r}c_{r,1}=[\sum_{r=1}^n(A_{\cdot,r}c_{r,1})]_{j,1}=(c_1 A_{\cdot,1}+\dots+c_n A_{\cdot,n})_{j,1}$\par\quad
...
$\therefore\,aC=a_{1,\cdot}C_{\cdot,\cdot}=\sum\limits_{r=1}^n a_{1,r}C_{r,\cdot}=a_1 C_{1,\cdot}+\dots+a_n C_{n,\cdot}$\quad \Or By $(aC)_{1,\cdot}=a_{1,\cdot}C$. Using (b) above.\PfEnd
\SepLine
