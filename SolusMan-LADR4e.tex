...
And $\dim(V_2\cap V_3)=\dim V_2+\dim V_3-\dim(V_2+V_3).$\TextB{}
...
Let $u$ be a nonzero vec in $V\Rightarrow V=\Spn(u)$.\par\quad
...
( in other words, for any two distinct nonzero vecs $v$ and $w$ in V, we have $\lambda_v\neq\lambda_w$ ), we discuss in two cases:\par\quad
$\MathRightBrace{l}{$
...
Suppose $(v,Tv)$ is linely inde for every nonzero vec $v\in V$.\par\quad
...
\ProblemN{13}{
	\TextNL{Suppose $(v_1,\dots,v_m)$ is a linely depe list of vecs in $V$.}
...
\ProblemB{
	...
	\TextB{Prove that the rank of $A$ is $1\Longleftrightarrow\exists\,(c_1,\dots,c_m)\in\Fbb^m$ and $(d_1,\dots,d_n)\in\Fbb^n$}
