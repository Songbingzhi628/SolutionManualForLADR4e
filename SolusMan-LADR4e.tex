...
\TextB{Note that if $\range T$ is finite-dim, then $\mathcal{K}_{\range T}=\mathcal{K}_R$ for any basis $R$ of $\range T.$}\tinypar
...
\par\tinypar\quad
(c) Fix a $k\in\{1,\dots,m\}.$ Given $\lambda_i\in\Fbb\,(\,i\in\{1,\dots,m\}\backslash\{k\}\,).$\par\quad\Habc
Let $\lambda_k=1-\lambda_1-\dots-\lambda_{k-1}-\lambda_{k+1}-\dots-\lambda_m$\par\quad\Habc
Then $\lambda_1 v_1+\dots+\underset{\bullet\,\,\bullet\,\,\bullet\,\,}{\lambda_k v_k}+\dots+\lambda_m v_m=v_k+\sum\limits_{i=1}^m\lambda_i(v_i-v_k)$.\par\quad\Habc
...
Let $(u_1, \dots, u_m)$ be a basis of $U$, extend to $(u_1, \dots,u_m, u_{m+1}, \dots, u_{m+n})$ a basis of $V$.\par\quad Choose a $k \in \{1,\dots,n\}$. Define $\varphi \in V\apostrophe$ by $\varphi(u_{i})=\MathLeftBrace{l}{ 1,\quad\textup{if}\,\,\,i=m+k.\\ 0,\quad\textup{otherwise}.}$ \par\quad
...
\Comment\,\,\, Reversing the roles of $S$ and $T$, letting $\lambda=0$, we conclude that\par\qquad\qquad\qquad
$\null S$ and $\range S$ is invar under $T,$ which will be shown in Problem (2) and (3) below.\par
\ProblemEnding\par

\ProblemBnosolus{
	\TextB{Suppose $S,T\in\Lm(V)$ are such that $ST=TS.$}
	\Onumber{2}\, \TextB{Show that $W=\null S$ is invar under $T$.\quad\large$\forall u\in W,Su=0\Rightarrow TSu=0=STu\Rightarrow Tu\in W.\hfill\square$}
	\Onumber{3}\, \TextB{Show that $U=\range S$ is invar under $T$.\,\,\,\large$\forall w\in U,\,\exists\,v\in V,Sv=w, STv=TSv=Tw\in U.\hfill\square$}
}
\ProblemEnding\par
...
(a) Suppose $\lambda$ is an eigval of $T$ with an eigvec $v.$\par\quad\Habc
Then $T^{-1}Tv=\lambda T^{-1}v=v\Rightarrow T^{-1}v=\frac{1}{\lambda}v.$ Hence $\frac{1}{\lambda}$ is an eigval of $T^{-1}.$\par\quad
(b) Suppose $\frac{1}{\lambda}$ is an eigval of $T^{-1}$ with an eigvec $v.$\par\quad\Habc
Then $TT^{-1}v=v=\frac{1}{\lambda}Tv\Rightarrow Tv=\lambda v.$ Hence $\lambda$ is an eigval of $T.$\par\quad\Habc
...
with correspd eigvals $\lambda_1,\dots,\lambda_m$ respectively ( possibly with repetitions ).\par\qquad\,
...
{\timessl\Large If \,$V$ is finite-dim and $U$ is a subsp of \,$V$ that is invar under every operator on $V$,}\par\,\,\,
...
(a) Suppose $v\in V$ is an eigvec correspd to the eigval $\lambda.$\par\qquad\,
Then $Tv=\lambda v\Rightarrow T_{\Cbb}(v+\i 0)=Tv+\i T0=\lambda v.$\par\qquad\,
Thus $\lambda$ is an eigval of $T.$\par\qquad\,
Suppose $v+\i u\in V_{\Cbb}$ is an eigvec correspd to the eigval $\lambda.$\par\qquad\,
...
{\timessl\Large Show that $\lambda$ is an eigval of $T$ $\Longleftrightarrow$ $\lambda$ is an eigval of the dual operator $T\apostrophe\in\Lm(V\apostrophe)$. 
}\par
...
Give an example of an operator on $\Rbb^4$ that has no $($ real $)$ eigvals.}\par
...
Show that the forward shift operator $T\in\Lm(\Fbb^\infty)$}\par\quad\,
...
Define the backward shift operator $S\in\Lm(\Fbb^\infty)$ by $S(z_1,z_2,z_3,\dots) = (z_2,z_3,\dots)$.}\par\quad\,
...
{\timessl\Large Prove that $u$ and $w$ are eigvecs of $T$ correspd to the same eigval.}\par
...
{\timessl\Large Prove that $T$ is a scalar multi of the identity operator.}\par
...
Suppose $T\in\Lm(\Rbb^2)$ is the operator of counterclockwise rotation by the angel $\theta$, where $x\in\Rbb^+$.}\par\,\,
...
(a) %Let $V=\Fbb^{\infty},$ $S\in\Lm(\Fbb^\infty)$ is the forward shift operator, $T\in\Lm(\Fbb^\infty)$ is the backward shift operator.\par\qquad\,
...
Give an example of an operator on $\Rbb^2$
}\par\quad\,
...
Suppose $a_0 ,\dots, a_{n-1}\in\Fbb.$ Let $T$ be the operator on $\Fbb^n$ such that}\par\tinypar\,\,
...
{\timessl\small Hence a formula or an algorithm that could produce exact eigvals for each operator on each $\Fbb^n$ could then produce exact zeros for}{\footnotesize\par}\,\,
...
\ProblemN{14}{
	({\normalsize O{\small R} 4E 5.C.4})\TextNL{ Give an operator $T$ such that with resp to some basis,}
...
\ProblemN{15}{
	({\normalsize O{\small R} 4E 5.C.5})\TextNL{ Give an operator $T$ such that with resp to some basis,}
...
\ProblemN{20}{
	({\normalsize O{\small R} 4E 5.C.6})\TextNL{}
...
\ProblemBnoor{({\normalsize 4E 5.C.8})}{
	...
	\Habc\TextB{with resp to some basis of \,$V$, then $-1 + \i$ or $-1 - \i$ appears on the diag of $A$.}
}\par
\ProblemEnding\par
