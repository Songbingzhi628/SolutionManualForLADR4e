...
\newcommand{\Exercise}[1]{{\timesbfx\large E{\footnotesize XERCISE} \Onumber{#1}\hspace{2pt}:}}
...
( See \NOTEFOR [3.60])\par\quad\Habc
...
(a) Suppose $v_1,\dots,v_m$ are all linely inde eigvecs of $T$\par\quad\Habc
with correspd eigvals $\lambda_1,\dots,\lambda_m$ respectively ( possibly with repetitions ).\par\quad\Habc
Extend to a basis of $V$ as $(v_1,\dots,v_m,\dots,v_n).$\par\quad\Habc
Then for each $k\in\{1,\dots,m\},\Spn(v_k)\subseteq\null(T-\lambda_k I).$\par\quad\Habc
Define $S_k\in\Lm(V)$ by $S_k(v_j)=v_k$ for each $j\in\{1,\dots,n\}$,\par\quad\Habc\qquad\,
so that $\range S_k=\Spn(v_k)$ for each $k\in\{1,\dots,m\}$, then $\mathcal{A}(S_k)=TS_k=\lambda_k S_k.$\par\quad\Habc
Thus the eigvals of $T$ are eigvals of $\mathcal{A}.$\par\quad
(b) Suppose $\lambda_1,\dots,\lambda_m$ are all eigvals of $\mathcal{A}$ with eigvecs $S_1,\dots,S_m$ respectively.\par\quad\Habc
Then for each $k\in\{1,\dots,m\},$ $\exists\,v\in V,0\neq u=S_k(v)\in V\Rightarrow Tu=(TS_k) v=(\lambda_k S_k) v=\lambda_k u.$\par\quad\Habc
Thus the eigvals of $\mathcal{A}$ are eigvals of $T.\hfill\square$\par\tinypar\quad
\Or \par\quad
(a) Suppose $\lambda$ is an eigval of $T$ with an eigvec $v.$\par\quad\Habc
Let $v_1=v$ and extend to a basis $(v_1,\dots,v_m)$ of $V.$\par\quad\Habc
Define $S\in\Lm(V)$ by $Sv_1=v_1,\,\,Sv_k=0$ for $k\geq 2.$\par\quad\Habc
Then $(T-\lambda I)Sv_1=0=(T-\lambda I)Sv_k=0.$\par\quad\Habc
...
(a) Suppose $v\in V$ is an eigvec correspd to the eigval $\lambda.$\par\quad\Habc
Then $Tv=\lambda v\Rightarrow T_{\Cbb}(v+\i 0)=Tv+\i T0=\lambda v.$\par\quad\Habc
Thus $\lambda$ is an eigval of $T.$\par\quad\Habc
Suppose $v+\i u\in V_{\Cbb}$ is an eigvec correspd to the eigval $\lambda.$\par\quad\Habc
Then $T_{\Cbb}(v+\i u)=\lambda v+\i \lambda u\Rightarrow Tv=\lambda v,Tu=\lambda u.$ ( Note that $v$ or $u$ might be zero ).\par\quad\Habc
...
Suppose $\lambda$ is an eigval of $T_{\Cbb}$ with an eigvec $v+\i u.$\par\quad\Habc
Let $(v_1,\dots,v_n)$ be a basis of $V.$ Write $v=\sum\limits_{i=1}^n a_i v_i,u=\sum\limits_{i=1}^n b_i v_i,$ where $a_i,b_i\in\Rbb.$\par\quad\Habc
Then $T_{\Cbb}(v+\i u)=Tv+\i Tu=\lambda v+\i\lambda u=\lambda\sum\limits_{i=1}^n(a_i+\i b_i)v_i.$ Conjugating two sides, we have:\par\quad\Habc
$\overline{T_{\Cbb}(v+\i u)}=\overline{Tv+\i Tu}=\overline{Tv}-\i\overline{Tu}=Tv-\i T u=T_{\Cbb}(\overline{v+\i u})=\overline{\lambda\sum\limits_{i=1}^n(a_i+\i b_i)v_i}=\overline\lambda\sum\limits_{i=1}^n(a_i-\i b_i)v_i.$\par\quad\Habc
...
(a) Suppose $\lambda$ is an eigval of $T$ with an eigvec $v$.\par\par\quad\Habc
Then $(T-\lambda I_V)$ is not inv. 又 $V$ is finite-dim.\par\quad\Habc
Thus by [3.108, 110], [3.101] and Problem (12) in (3.F), $(T-\lambda I_V)\apostrophe=T\apostrophe-\lambda I_{V\apostrophe}$ is not inv.\par\quad\Habc
Hence $\lambda$ is an eigval of $T\apostrophe.$\par\quad
(b) Suppose $\lambda$ is an eigval $T\apostrophe$ with an eigvec $\psi.$ Then $T\apostrophe(\psi)=\psi\circ T=\lambda\psi.$\par\quad\Habc
又 $\psi\neq 0\Rightarrow\exists\,v\in V$ such that $\psi(v)\neq 0.$ Note that $\psi(Tv)=\lambda\psi(v).$\par\quad\Habc
...
(a) Suppose $v=(x_1,x_2,x_3,\dots,x_n)$ is an eigvec of $T$ with an eigval $\lambda.$\par\quad\Habc
Then $Tv=\lambda v=(x_1,2 x_2,3 x_3,\dots,n x_n)=(\lambda x_1,\lambda x_2,\lambda x_3,\dots,\lambda x_n).$\par\quad\Habc
Hence $1,\dots,n$ are eigvals of $T.$\par\quad\Habc
...
\ProblemN{24}{
	...
	\Habc\TextNL{Prove that $1$ is an eigval of $T$.}
...
x_1\\ \vdots\\ x_n
\end{pmatrix}.$}\par\quad\Habc
Then {\small$Tx=Ax=\begin{pmatrix} \sum\limits_{c=1}^n A_{1,c}x_c\\ \vdots\\ \sum\limits_{c=1}^n A_{n,c}x_c\end{pmatrix}=\lambda \begin{pmatrix} x_1\\ \vdots\\ x_n\end{pmatrix}.$} While $\sum\limits_{r=1}^n A_{R,c}=1$ for each $R=1,\dots,n.$\par\tinypar\quad\Habc
...
x_1\\ \vdots\\ x_n
\end{pmatrix}.$}\par\quad\Habc
Then {\small$Tx=Ax=\begin{pmatrix} \sum\limits_{r=1}^n A_{1,r}x_r\\ \vdots\\ \sum\limits_{r=1}^n A_{n,r}x_r\end{pmatrix}=\lambda \begin{pmatrix} x_1\\ \vdots\\ x_n\end{pmatrix}.$} While $\sum\limits_{r=1}^n A_{r,C}=1$ for each $C=1,\dots,n.$\par\quad\Habc
...
\end{pmatrix}.$}\par\quad\Habc
Hence $\lambda=1,$ for all $x$ such that $\sum\limits_{c=1}^n x_{c,1}\neq 0.\hfill\square$\par\tinypar\quad\Habc
\Or Prove that $(T-I)$ is not inv, so that we can conclude $\lambda=1$ is an eigval.\par\quad\Habc
Because $(T-I)x=\left(A-\Mt(I)\right)x=\,${\small$\begin{pmatrix} \sum\limits_{r=1}^n A_{1,r}x_r-x_1\\ \vdots\\ \sum\limits_{r=1}^n A_{n,r}x_r-x_n\end{pmatrix}=\begin{pmatrix} y_1\\ \vdots\\ y_n\end{pmatrix}.$}\par\quad\Habc
Then $y_1+\dots+y_n=\sum\limits_{r=1}^n\sum\limits_{c=1}^n (A_{r,c}x_c-x_r)=\sum\limits_{c=1}^n x_c\sum\limits_{r=1}^n A_{r,c}-\sum\limits_{r=1}^n x_r=0.$\par\tinypar\quad\Habc
...
\ProblemB{
	...
	\Habc\TextB{Prove that $1$ is an eigval of $T$.}
...
x_1 & \cdots & x_n
\end{pmatrix}.$}\par\quad\Habc
Then {\small$Tx=xA=\begin{pmatrix} \sum\limits_{r=1}^n x_r A_{r,1} &\cdots & \sum\limits_{r=1}^n x_r A_{r,n}\end{pmatrix}=\lambda \begin{pmatrix} x_1 & \cdots & x_n\end{pmatrix}.$} While $\sum\limits_{r=1}^n A_{r,C}=1$ for each $C=1,\dots,n.$\par\quad\Habc
Thus if we let $x_1=\dots=x_n,$ then $\lambda=1,$ hence is an eigval of $T.$\par\quad
(b) Suppose $\lambda$ is an eigval of $T$ with an eigvec $x=\,${\small$\begin{pmatrix}
x_1 & \cdots & x_n
\end{pmatrix}.$}\par\quad\Habc
Then {\small$Tx=xA=\begin{pmatrix} \sum\limits_{c=1}^n x_c A_{c,1} &\cdots & \sum\limits_{c=1}^n x_c A_{c,n}\end{pmatrix}=\lambda \begin{pmatrix} x_1 & \cdots & x_n\end{pmatrix}.$} While $\sum\limits_{c=1}^n A_{R,c}=1$ for each $R=1,\dots,n.$\par\quad\Habc
...
x_1\,\,\,\,+&\cdots&+\,\,\,\,x_n\end{pmatrix}.$}\par\quad\Habc
Hence $\lambda=1,$ for all $x$ such that $\sum\limits_{r=1}^n x_{1,r}\neq 0.\hfill\square$\par\tinypar\quad\Habc
\Or Prove that $(T-I)$ is not inv, so that we can conclude $\lambda=1$ is an eigval.\par\quad\Habc
Because $(T-I)x=x\left(A-\Mt(I)\right)=\,${\small$=\begin{pmatrix} \sum\limits_{c=1}^n x_c A_{c,1}-x_1 &\cdots & \sum\limits_{c=1}^n x_c A_{c,n}-x_n\end{pmatrix}= \begin{pmatrix} y_1 & \cdots & y_n\end{pmatrix}.$}\par\quad\Habc
Then $y_1+\dots+y_n=\sum\limits_{c=1}^n\sum\limits_{r=1}^n(x_r A_{r,c}-x_c)=\sum\limits_{r=1}^n x_r\sum\limits_{c=1}^n A_{r,c}-\sum\limits_{c=1}^n x_c=0.$\par\tinypar\quad\Habc
Thus $\range(T-I)\subseteq\{\,${\small$\begin{pmatrix}
y_1 & \cdots & y_n
...
(a) Suppose $\lambda$ is an eigval of $T$ with an eigvec $v.$\par\quad\Habc
$(\,$ If $\dim V=1,$ then $U=\{0\}$ and we are done. $\,)$\par\quad\Habc
Extend $v_1=v$ to a basis of $V$ as $(v_1,v_2\dots,v_n).$\par\quad\Habc
{\timesbf Step 1} $\,$If $\,\exists\,w_1\in\Spn(v_2,\dots,v_n)$ such that $0\neq Tw_1\in\Spn(v_1)$,\par\qquad\Habc\qquad
then extend $w_1=\alpha_{1,1}$ to a basis of $\Spn(v_2,\dots,v_n)$ as $(\alpha_{1,1},\dots,\alpha_{1,n-1}).$\par\qquad\qquad\quad\, Otherwise, we stop at step $1.$\par\quad\Habc
...
%Otherwise, we stop at step $2.$\par\quad\Habc
$\quad\vdots$\par\quad\Habc
...
Otherwise, we stop at step $k.$\par\quad\Habc
$\quad\vdots$\par\quad\Habc
Finally, we stop at step $m,$ thus we get $(v_1,w_1,\dots,w_{m-1})$ and $(\alpha_{m-1,2},\dots,\alpha_{m-1,n-m+1}),$\par\quad\Habc
$\range T|_{\Spn(w_1,\dots,w_{m-1})}=\Spn(v_1,w_1,\dots,w_{m-2})\Rightarrow\dim\null T|_{\Spn(w_1,\dots,w_{m-1})}=0,$\par\quad\Habc
$\Spn(\underbrace{v_1,w_1,\dots,w_{m-1}}_{\textup{length} \dim m})$ and $\Spn(\underbrace{\alpha_{m-1,2},\dots,\alpha_{m-1,n-m+1}}_{\textup{length} \dim (n-m)})$ are invar under $T.$\par\quad\Habc
Let $U=\Spn(\alpha_{m-1,2},\dots,\alpha_{m-1,n-m+1})\oplus\Spn(v_1,w_1,\dots,w_{m-2})$ and we are done.$\hfill\square$\par\quad\Habc
\Comment\,\,\, Both $\Spn(v_2,\dots,v_n)$ \,{\small and}\, $\Spn(\alpha_{m-1,2},\dots,\alpha_{m-1,n-m+1})\oplus\Spn(w_1,\dots,w_{m-1})$ are in $\mathcal{S}_V \Spn(v_1).$\par\tinypar\quad
(b) Suppose $U$ is an invar subpsace of $V$ under $T$ with $\dim U=m=\dim V-1.$\par\quad\Habc
( If $m=0,$ then $\dim V=1$ and we are done. )\par\quad\Habc
Let $(u_1,\dots,u_m)$ be a basis of $U$, extend to a basis of $V$ as $(u_0,u_1,\dots,u_m).$\par\quad\Habc
We discuss in cases:\par\quad\Habc
For $Tu_0\in U,$ then $\range T=U$ so that $T$ is not surj $\Longleftrightarrow \null T\neq\{0\}\Longleftrightarrow$ $0$ is an eigval of $T.$\par\quad\Habc
For $Tu_0\not\in U,$ then $Tu_0=a_0 u_0+a_1 u_1+\dots+a_mu_m.$\par\quad\Habc
(1) If $Tu_0\in\Spn(u_0),$ then we are done.\par\quad\Habc
(2) Otherwise, if $\range T|_U=U,$ then $Tu_0=a_0 u_0$ and we are done;\par\qquad\qquad\qquad\quad\Habc
...
\rightline{Otherwise $\exists\,u\in U\backslash\{0\},Tu=0$ and we are done. )\qquad\qquad}\par\qquad\qquad\qquad\quad\Habc
...
\ProblemB{
	...
	\Habc\TextB{( which requires using the condition that $U$ is invar under $T$ )}
	\Habc\TextB{and show that $T/U$ is an operator on $V/U$.}
	(b) \TextB{{\normalsize (O{\footnotesize R} Problem 35)} Show that each eigval of $T/U$ is an eigval of $T$.}
}\par\quad
(a) Suppose $v+U=w+U\,(\,\Longleftrightarrow v-w\in U\,).$\par\quad\Habc
Then because $U$ is invar under $T,$ $T(v-w)\in U\Longleftrightarrow Tv+U=Tw+U.$\par\quad\Habc
Hence the definition of $T/U$ makes sense.\par\quad\Habc
Now we show that $T/U$ is linear.\par\quad\Habc
...
(b) Suppose $\lambda$ is an eigval of $T/U$ with an eigvec $v+U.$\par\quad\Habc
Then $(T/U)(v+U)=\lambda(v+U)=Tv+U=\lambda v+U\Rightarrow (T-\lambda I)v\in U.$\par\quad\Habc
If $(T-\lambda I)v=0\Rightarrow Tv=\lambda v,$ then we are done.\par\quad\Habc
Otherwise, then $(T|_U-\lambda I):U\rightarrow U$ is inv,\par\qquad\qquad\quad\Habc\,\,\,
hence $\,\exists\,!\,w\in U,(T|_U-\lambda I)(w)=(T-\lambda I) v\Rightarrow T(v-w)=\lambda (v-w).$\par\quad\Habc
Note that $v-w\neq 0\,($ for if not, $v\in U\Rightarrow v+U=0+U$ is not an eigvec $).\hfill\square$\par\quad\Habc
\par
\ProblemEnding\par
...
(a) Suppose $T^m$ is inje. Then $Tv=0\Rightarrow T^{m-1} Tv=T^m v=0\Rightarrow v=0.\hfill\square$\par\quad\Habc
Suppose $T$ is inje.\par\quad\Habc
Then $T^m v=T(T^{m-1}v)=0$\par\qquad\qquad\quad\Habc\quad\,
...
(b) Suppose $T^m$ is surj. $\forall u\in V,\,\exists\,v\in V,T^m v=u=Tw,$ let $w=T^{m-1}v.\hfill\square$\par\quad\Habc
Suppose $T$ is surj.\par\quad\Habc
Then $\forall u\in V,\,\exists\,v\in V, T\underline{(v)}=u$\par\qquad\quad\Habc\,\,\,
$\Rightarrow\exists\,v_2\in V,Tv_2=\underline{v}, T^2\underline{(v_2)}=u$
\par\qquad\qquad\qquad\qquad\qquad\qquad\quad\,
$\quad\Habc\,\vdots$\par\qquad\qquad\quad\Habc\,\,
...
$\Rightarrow\exists\,v_{m-1}\in V,Tv_{m-1}=\underline{v_{m-2}}, T^{m-1}\underline{(v_{m-1})}=u$\par\qquad\qquad\qquad\qquad\qquad\qquad\quad\Habc\,
...
(a) Suppose $u\in\null p(T)$. Then $p(T)u = 0$.\par\quad\Habc
Thus $p(T)(Tu) = (p(T) T)u \left(Tp(T)\right)u=T(p(T)u) = 0$. Hence $Tu\in\null p(T).\hfill\square$\par\quad
(b) Suppose $u\in\range p(T)$. Then $\,\exists\,v\in V$ such that $u = p(T)v$.\par\quad\Habc
...
(c) At least one of $T_1,\dots,T_m$ is not inje $\not\Rightarrow$ $(T_1\circ\cdots\circ T_m)$ is not inje.\TextB{}\Habc
\Example\,\, On infinite-dim only. Let $V=\Fbb^\infty.$\TextB{}\Habc
...
$(\,$mini poly of $T|_U\,) \times (\,$mini poly of $T/U\,)$ is a poly multi of $\,p\,$.}
	...
	\Habc\TextB{the union of the set of eigvals of $T|_U$ and the set of eigvals of $T/U$.}
}\par\quad
...
(c) Suppose $r$ is the mini poly of $T|_U,s$ is the mini poly of $T/U.$\par\quad\Habc
Because $\forall v\in V,s(T/U)(v+U)=s(T)v+U=0.$
So that $\forall v\in V$ but $v\not\in U,s(T)v\in U.$\par\quad\Habc
又 $\forall u\in U,r(T|_U)u=r(T)u=0.$\par\quad\Habc
Thus $\forall v\in V$ but $v\not\in U,(rs)(T)v=r(s(T)v)=0.$\par\quad\Habc
And $\forall u\in U,(rs)(T)u=r(s(T)u)=0$ ( because $s(T)u=s(T|_U)u\in U$ ).\par\quad\Habc
...
(a) Suppose $j\in\Nbp$ is the smallest such that $T^j v\in\Spn(v,Tv,\dots,T^{j-1}v)=U_0.$ Then $j\leq m.$\par\quad\Habc
Write $T^j v=c_0 v+c_1 Tv+\dots+c_{j-1}T^{j-1}v.$ And because $T(T^k v)=T^{k+1}\in U_0.$ $U_0$ is invar under $T.$\par\quad\Habc
By Problem (6), $\forall k\in\Nbb,\,\,\,T^{j+k}v=T^k(T^j v)\in U_0.$\par\quad\Habc
Thus $U_0=\Spn(v,Tv,\dots,T^{j-1} v,\dots,T^n v)$ for all $n\geq j-1.$ Let $n=m-1$ and we are done.\par\quad
(b) Let $U=\Spn(v,Tv,\dots,T^{m-1}v).$\par\quad\Habc
...
(b) $\forall S=a_0 I+a_1 T+\dots+a_{m-1}T^{m-1}\in\mathcal{E},$ define $p\in\PoF{m-1}$ by\par\quad\Habc
...
Then $(T^2-9I)v=(T-3I)(T+3I)v=0$ for some $v.$ By T{\small IPS.}\par\Habc\quad
...
$\exists\,v\in V,(T^2+I)v\neq 0$ while $(T^2-I)\left((T^2+I)v\right)=0$ but $(T^2-I)$ is inje. Contradicts.$\square$\par\tinypar\quad
...
Suppose the mini poly $p$ is defined by $p(z)=z^2+bz+c$ for all $z\in\Rbb.$\par\smallpar\quad
Because\vspace{-30pt}\par\quad
\hspace{45pt}\includegraphics[scale=0.15]{./diagram.png}\par\vspace{-60pt}\quad
\hspace{165pt}{\normalsize$\MathLeftMid{l}{Tv=\displaystyle\frac{\left|\overset{\rightarrow}{v}\right|}{2L}(T^2 v+v)\Rightarrow T=\frac{\left|\overset{\rightarrow}{v}\right|}{2L}(T^2+I)\\\vspace{8pt}\displaystyle L=\left|\overset{\rightarrow}{v}\right|\cos\theta\Rightarrow\frac{\left|\overset{\rightarrow}{v}\right|}{2L}=\frac{1}{2\cos\theta}}$}\par\tinypar\quad
...
(a) Suppose the basis is $(v,w)$. Because $\MathLeftBrace{l}{\displaystyle Tv=av+bw\Rightarrow (T-aI)v=bw,$ then apply $(T-dI)$ to both sides. $\\ Tw=cv+dw\Rightarrow (T-dI)w=cv,$ then apply $(T-aI)$ to both sides. $}$\par\tinypar\quad\Habc
Hence $(T-aI)(T-dI)=bc I\Rightarrow T^2 - (a + d)T + (ad - bc)I = 0.$\par\quad
(b) If $b=c=0$ and $a=d.$ Then $\Mt(T)=${\small$\begin{pmatrix}a & 0\\ 0 & a\end{pmatrix}$}$=a\Mt(I)$. Thus $T=aI.$ Hence the mini poly is $z-a.$\par\quad\Habc
Otherwise, by (a), $z^2-(a+d)z+(ad-bc)$ is a poly multi of the mini poly.\par\quad\Habc
Now we prove that $T\not\in\Spn(I),$ so that then the mini poly of $T$ has exactly degree $2.$\par\quad\Habc
$($ At least one of the assumption of (I),(II) below is true. $)$\par\quad\Habc
...
\ProblemBnoor{({\normalsize 4E 5.B.7})\TextB{}}{
	...
	\Habc\TextB{the mini poly of
$ST$ does not equal the mini poly of $TS$.}
	(b) \TextB{Suppose $V$ is finite-dim and $S, T\in\Lm(V)$. Prove that if $S$ or $T$ is inv,}
	\Habc\TextB{then the mini poly of $ST$ equals the mini poly of $TS$.}
}\par\quad
(a) %Let $V=\Fbb^{\infty},$ $S\in\Lm(\Fbb^\infty)$ is the forward shift operator, $T\in\Lm(\Fbb^\infty)$ is the backward shift operator.\par\quad\Habc
%Then $ST(x_1,x_2,x_3,\dots)=(0,x_2,x_3,\dots)\Rightarrow 0,1$ are all the eigvals of $ST,$ $(ST)^2-(ST)=0.$\par\quad\Habc
%$TS(x_1,x_2,\dots)=(x_1,x_2,\dots)\Rightarrow 1$ is the only eigval of $TS,$ $TS=I.$\par\quad\Habc
Define $S$ by $S(x,y)=(x,x).$ Define $T$ by $T(x,y)=(0,y).$\par\quad\Habc
Then $ST(x,y)=0,\,\,TS(x,y)=(0,x)$ for all $(x,y)\in\Fbb^2.$\par\quad\Habc
Thus $ST=0\neq TS$ and $(TS)^2=0.$\par\quad\Habc
Hence the mini poly of $ST$ does not equal to the mini poly of $TS.$\par\quad
(b) Denote the mini poly of $ST$ by $p,$ and the mini poly $TS$ by $q.$\par\quad\Habc
Suppose $S$ is inv.\par\,\,\Habc
$\MathRightBrace{l}{$
$p(ST)=0=S p(TS)S^{-1}\Rightarrow p(TS)=0,p$ is a poly multi of $q.\\ $
$q(TS)=0=S^{-1} q(ST)S\Rightarrow q(ST)=0,q$ is a poly multi of $p.$
$}\Rightarrow p=q.$\par\tinypar\quad\Habc
...
(a) Suppose $\alpha$ is an eigval of $p(T)\Leftrightarrow (p(T)-\alpha I)$ is not inje.\par\quad\Habc
Write $p(z)-\alpha=c(z-\lambda_1)\cdots(z-\lambda_m)\Rightarrow p(T)-\alpha I=c(T-\lambda_1 I)\cdots(T-\lambda_m I).$\par\quad\Habc
By T{\small IPS,} $\,\exists\,(T-\lambda_j I)$ not inje. Thus $p(\lambda_j)-\alpha=0.$\par\quad
(b) Suppose $\alpha=p(\lambda)$ and $\lambda$ is an eigval of $T$ with an eigvec $v.$ Then $p(T)v=p(\lambda)v=\alpha v.\hfill\square$\par\vspace{3pt}\quad\Habc
\Or Define $q$ by $q(z)=p(z)-\alpha.$ $\lambda$ is a zero of $q.$\par\quad\Habc
Because $q(T)v=(p(T)-\alpha I)v=q(\lambda)v=(p(\lambda)-\alpha)v=0.$\par\quad\Habc
...
\ProblemN{18} ({\normalsize \OR 4E 5.B.15})\par\quad\, {\timessl\Large 
Suppose $V$ is a finite-dim complex vecsp with $\dim V > 0$ and $T\in\Lm(V)$.}\par\quad\,
...
{\small $\bullet$} {\normalsize \OR (4E 5.B.16), \OR (8.C.18})\par\,\, {\timessl\Large 
