...
\def\NEWTHEOREM{{\timesnr\large N{\footnotesize EW} T{\footnotesize HEOREM}}}
\def\Notice{{\timesnr\large N{\footnotesize OTICE}}}
\def\COROLLARY{{\timesnr\large C{\footnotesize OROLLARY}}}
\def\Tips{{\timesbfx\large T{\footnotesize IPS:}}}
\def\TIPS{{\timesnr\large T{\footnotesize IPS}}}
...
}Suppose $a\neq 0,\,\exists\,a^{-1}\in\Fbb,a^{-1}a=1,$ hence $v=1\cdot v=(a^{-1}a)v=a^{-1}(av)=a^{-1}\cdot 0=0.\hfill\square$\par
\ProblemEnding\par
...
[{\timessl Uniqnes}] \dbsp Suppose $v+3x_1=w,$(I)$\,\,v+3x_2=w\,$(II). Then (I) $-$ (II) $: 3(x_1-x_2)=0\Rightarrow x_1=x_2.\hfill\square$\par\quad
\Or \quad$v + 3x = w \quad\Leftrightarrow\quad 3x = w − v\quad \Leftrightarrow\quad x = \frac{1}{3} (w − v).\hfill\square$
\ProblemEnding\par

\ProblemN{5}{
	\TextN{Show that in the def of a vecsp, the add inv condition can be replaced by [1.29].}
	\TextN{\normalsize Suppose $V$ satisfies all conds in the def, except we've replaced the add inv cond with [1.29]. Prove that the add inv is true.}
}
...
\ChapterDecl{Ch1C}{1$\cdot$C}{}\orMode{\qquad\hLk{1C7}{7}\;\;\hLk{1C8}{8}\;\;\hLk{1C9}{9}\;\;\hLk{1C11}{11}\;\;\hLk{1C12}{12}\;\;\hLk{1C13}{13}\;\;\hLk{1C15}{15}\;\;\hLk{1C16}{16}\;\;\hLk{1C17}{17}\;\;\hLk{1C18}{18}\;\;\hLk{1C21}{21}\;\;\hLk{1C22}{22}\;\;\hLk{1C23}{23}\;\;\hLk{1C24}{24}}{\nLineDB\qquad[1]: 7, 8, 9, 15, 16, 17, 18, 11; [2]: 12, 13; [3]:21, 23, 22, 24.}


\ProblemN{\hypertarget{1C7}{7}}{
	\TextN{Give a nontrivial example of $U\subseteq\Rbb^2,$}
	\TextN{$U$ is closed under taking add invs and under add, but $U$ is not a subsp of $\Rbb^2.$}
}Let $U=\Zbb^2,(\Zbb^*)^2$, $(\Qbb^*)^2,\Qbb^2\backslash\left\{0\right\}$, or $\Rbb^2\backslash\left\{0\right\}$.\vspace{6pt}\par
\ProblemN{\hypertarget{1C8}{8}}{
	\TextN{Give a nontrivial example of $U\subseteq\Rbb^2,$}
	\TextN{$U$ is closed under scalar multi, but $U$ is not a subsp of $\Rbb^2$.}
...
Hence $2k=\displaystyle\frac{m}{\sqrt{2}}\Rightarrow \sqrt{2}=\displaystyle\frac{m}{2k}\in\Qbb$. Contradiction!$\hfill\square$\vspace{10pt}\par\quad
\Or Because $[\textup{\timesbf{I}}]: \cos(x)+\sin(\sqrt{2}x)=\cos(x+p)+\sin(\sqrt{2}x+\sqrt{2}p).$ By differentiating twice,\par\qquad\qquad\qquad\hspace{0pt}
$[\textup{\timesbf{II}}]: \cos(x)+2\sin(\sqrt{2}x)=\cos(x+p)+2\sin(\sqrt{2}x+\sqrt{2}p).$\par\vspace{2pt}\quad
$\MathRightBrace{r}{
	[\textup{\timesbf{II}}]-[\textup{\timesbf{I}}]:\sin(\sqrt{2}x)=\sin(\sqrt{2}x+\sqrt{2}p)\vspace{2pt}\\\, 
	2[\textup{\timesbf{I}}]-[\textup{\timesbf{II}}]:\qquad\quad\,\, \cos(x)=\cos(x+p)}\Rightarrow p=\displaystyle\frac{m\pi}{\sqrt{2}}=2k\pi,$ if $x=0.$ Contradicts.$\hfill\square$\vspace{10pt}\par
\ProblemEnding\par\vspace{10pt}

\BulletPoint \,\hspace{1pt}{\timessl\Large Suppose $U,W,V_1,V_2,V_3$ are subsps of $V.$}\par
\Onumber{\hypertarget{1C15}{15}}\,\, \centerline{\large\timesnr$U+U\ni u+w\in U.$\qquad\qquad\!}$\hfill\square$\par
\Onumber{\hypertarget{1C16}{16}}\,\, \centerline{\large\timesnr$U+W\ni u+w=w+u\in W+U.$}$\hfill\square$\par
\Onumber{\hypertarget{1C17}{17}}\,\, \centerline{\large\timesnr$(V_1+V_2)+V_3\ni(v_1+v_2)+v_3=v_1+(v_2+v_3)\in V_1+(V_2+V_3).$}$\hfill\square$\par
\ProblemEnding\par
...
{\Large\vspace{6pt}Then $u\in U_i$ while $u\not\in U_2\cup U_3.$ Contradicts.\large$\hfill\square$}\par
\ProblemEnding\vspace{20pt}\par
...
And $T\ni(x,x,y,z)\Rightarrow(0,0,y-x,y-x)+(x,x,x,-y+x+z)\in U+W$. Hence $T\subseteq U+W.\hfill\square$\par
\ProblemEnding\par
\pagebreak
...
And $\Fbb^5\ni(x,y,z,w,u)\Rightarrow(x,y,x+y,x-y,2x)+(0,0,z-x-y,w-x-y,u-2x)\in U+W.\hfill\square$\par
\ProblemEnding\par
...
(b){$\MathRightBrace{r}{f_e\in V_E\Longleftrightarrow f_e(x)=f_e(-x)\Longleftarrow$  let $f_e(x)=\displaystyle\frac{g(x)+g(-x)}{2}\vspace{4pt}\\ f_o\in V_O\Longleftrightarrow f_o(x)=-f_o(-x)\Longleftarrow$ let $f_o(x)=\displaystyle\frac{g(x)-g(-x)}{2}}\Rightarrow \forall g\in\Rbb^\Rbb,g(x)=f_e(x)+f_o(x).\hfill\square$}\vspace{6pt}\par
\ProblemEnding\par
\ChapterEnding

\ChapterDecl{Ch2A}{2$\cdot$A}{}\orMode{\qquad\hLk{2A1}{1}\;\;\hLk{2A2}{2}\;\;\hLk{2A6}{6}\;\;\hLk{2A10}{10}\;\;\hLk{2A11}{11}\;\;\hLk{2A14}{14}\;\;\hLk{2A16}{16}\;\;\hLk{2A17}{17}\;\;\hLk{2A4e314}{4E 3,14}}{\qquad[1]: 2; [2]: 1, 6, (4E 3, 14), 10; [3]: 11, 14, 16, 17.}
...
\Comment\,\,\, (1) with (3) and (2) with (4) will do as well.$\hfill\square$\par\vspace{6pt}\quad
...
\Comment\,\,\, (1) with (3) and (2) with (4) will do as well.$\hfill\square$\par
\ProblemEnding\par
\vspace{20pt}
\clearpage
...
or let $a_i=b_i-b_{i-1}$ with $b_{-1}=0,$ if we are to prove $P$ with $Q$ already assumed.$\hfill\square$\par
\ProblemEnding\par
...
\vspace{-10pt}
\AlignEq{}{\qquad\qquad\qquad Q\Rightarrow P:\;&a_1 v_1+a_2 v_2+a_3 v_3+a_4 v_4=0\\\Rightarrow\;&a_1(v_1-v_2)+(a_1+a_2)(v_2-v_3)+(a_1+a_2+a_3)(v_3-v_4)+(a_1+\dots+a_4)v_4=0\\\Rightarrow\;&a_1=a_1+a_2=a_1+a_2+a_3=a_1+\dots+a_4=0.}\par
\vspace{-25pt}$\hfill\square$\par
\ProblemEnding\par
%
%\ProblemN{7}{
...
(a) {\timessl\normalsize let} $a_k=\sum\limits_{j=1}^k b_j\Leftarrow\;\,a_1 v_1+\dots+a_m v_m=b_1 w_1+\dots+b_m w_m\;\,\Rightarrow$ {\timessl\normalsize let} $b_1=a_1,\;b_k=a_k-\sum\limits_{j=1}^{k-1}b_j=\sum\limits_{j=1}^k (-1)^{k-j}a_j.$\vspace{10pt}\par\quad
(b) \vspace{-13.5pt}\AlignEq{}{\qquad
	P\Rightarrow Q:\;\;&b_1 w_1+\dots+b_m w_m=0=a_1 v_1+\dots+a_m v_m,\textup{\;where\;}0=a_k=\sum\limits_{j=1}^k b_k\hspace{106pt}
...
\AlignEq{}{\qquad\quad Q\Rightarrow P:\;\;&a_1 v_1+\dots+a_m v_m=0=b_1 w_1+\dots+b_m w_m=0, \textup{\;where\;} 0=b_1=a_1,\;0=b_k=\sum\limits_{j=1}^k (-1)^{k-j}a_j
%\\[-15pt]\Rightarrow\;&b_1=\cdots=b_m=0\Rightarrow a_k=\sum\limits_{j=1}^k b_j=0 {\;for\;each\;} k.
}\par\quad\Hb
...
$(w_1,\dots,w_m)$ linely dep $\Rightarrow(v_1,\dots,v_m)$ linely dep. Conversely it is true as well.$\hfill\square$\par
\ProblemEnding\par
...
\Or By contrapositive, $w\not\in\Spn(v_1,\dots,v_m),$ similarly.\hfill$\square$\par\quad
%Suppose $a_1(v_1+w)+\dots+a_m(v_m+w)=0\Rightarrow a_1v_1+\dots+a_mv_m=-(a_1+\dots+a_m)w.$\par\quad
%Then $a_1+\dots+a_m=0,$ otherwise $w\in\Spn(v_1,\dots,w_m)$. Thus $a_1=\dots=a_m=0.\hfill\square$\par\quad
...
Now $w=\lambda^{-1}(a_1 v_1+\dots+a_{j-1}v_{j-1}-v_j)\Rightarrow w\in\Spn(v_1,\dots,v_m).\hfill\square$\vspace{-6pt}\par
\ProblemEnding\par
...
}\hspace{-7pt}$\begin{array}{l}{}^\neg Q\Rightarrow{}^\neg P:$ Suppose $w\in\Spn(v_1,\dots,v_m)$. Then $(v_1,\dots,v_m,w)$ is linely depe.$\\{}^\neg P\Rightarrow{}^\neg Q:$ Suppose $(v_1,\dots,v_m,w)$ is linely dep. Then by [2.21] $w\in\Spn(v_1,\dots,v_m).\end{array}\hfill\square$\par
\ProblemEnding\par
...
Hence no list spans $V$. Thus $V$ is infinite-dim.$\hfill\square$\par
\ProblemEnding\par

%\ProblemN{15}{
%	\TextNL{Prove that $\Fbb^\infty$ is infinite-dim.}
%}\par\quad
%Let $e_i=(0,\dots,0,1,0,\dots)\in\Fbb^\infty$ for every $m\in\Nbp$, where $‘1’$ is on the $i^\textup{th}$ entry of $e_i$.\par\quad
%Suppose $\Fbb^\infty$ is finite-dim and $\Spn(e_1,\dots,e_m)=V$. But $\exists\,e_{m+1}\not\in\Spn(e_1,\dots,e_m)$. Contradicts. $\hfill\square$\par
%\ProblemEnding\par
...
Thus $(1,x,\dots,x^m)$ is linely inde in $\Rbb^{[0,1]}.$ Similar to [2.16], $U$ is infinite-dim.\par\vspace{10pt}\quad
...
\ChapterDecl{Ch2B}{2$\cdot$B}{}\orMode{\qquad\hLk{2B1}{1}\;\;\hLk{2B7}{7}\;\;\hLk{2B8}{8}\;\;\hLk{2B4e5}{4E 5,}\;\;\hLk{2B4e9}{\!9}}{\qquad[1]: 7, 1, (4E 9,5), 8.}
...
By {\timessc some theories or facts} given in the course of Elementary Abstract Algebra, we fail.$\hfill\square$\par
\ProblemEnding\par
%
%\ProblemN{6}{
...
%Then $v=b_1 v_1+(b_1+b_2)v_2+(b_2+b_3)v_3+(b_3+b_4)v_4\Rightarrow\exists\,!\,b_1=a_1,\;b_k=a_k-b_{k-1}\in\Fbb.\hfill\square$\par
%\ProblemEnding\par
...
$Q\Rightarrow P:\;\forall v\in V,\,\exists\,!\,b_i\in\Fbb,\;v=b_1w_1+\dots+b_mw_m\Rightarrow v=a_1v_1+\dots+a_mv_m,\,\exists\,!\,a_k=\sum\limits_{j=1}^k b_j.\hfill\square$\par
\ProblemEnding\par
...
\ChapterDecl{Ch2C}{2$\cdot$C}{}\orMode{\qquad\hLk{2C1}{1}\;\;\hLk{2C7}{7}\;\;\hLk{2C9}{9}\;\;\hLk{2C10}{10}\;\;\hLk{2C14}{14,\,16}\;\;\hLk{2C15}{15}\;\;\hLk{2C17}{17}\;\;\hLk{2C4e10}{4E 10,}\;\;\hLk{2C4e14}{14,}\;\;\hLk{2C4e15}{15,}\;\;\hLk{2C4e16}{16}}{\nLineDB\qquad[1]: 1, 9, 10; [2]: (4E 10); [3]: 7, (4E 14, 15, 16); [4]: 14, 17; [5]: 15.}
...
%\TextN{\timesnr Let $(u_1,\dots,u_m)$ be a basis of $U$. Then $m=\dim U=\dim V.$ 又 $u_i\in V$.}
...
%If $n=2$, then $U=\Spn(v,w)=\left\{\lambda v+\mu w:\lambda,\mu\in\Rbb\right\},\,\exists\,$ linely inde $v,w\in\Rbb^3.\hfill\square$\par
%\ProblemEnding\par
...
$(v_1,\dots,v_m)$ linely inde $\Rightarrow$ $(v_1,v_2-v_1,\dots,v_m-v_1)$ linely inde $\Rightarrow$ $(\underbrace{v_2-v_1,\dots,v_m-v_1}_{\textup{of\;length\;}(m-1)})$ linely inde.\vspace{-8pt}\par\quad
又 $w\not\in\Spn(v_1,\dots,v_m)\Rightarrow (v_1+w,\dots,v_m+w)$ is linely inde.\par\quad
Hence $m\geq\dim\Spn(v_1+w,\dots,v_m+w)\geq m-1.\hfill\square$\par
\ProblemEnding\par
...
{\Large Now $L=a_{k-1}p_{k-1}(x)+\dots+a_0p_0(x).\hfill\square$}\par
\ProblemEnding\par

\ProblemBnoor{(\hypertarget{2C4e10}{\normalsize 4E 2.C.10})}{
...
\ProblemBnoor{\hypertarget{2C4e14}{({\normalsize 4E 2.C.14})}}{
	\TextB{Suppose V is a 10-dim vecsp and $V_1,V_2,V_3$ are subsps of \,$V$ with}
	\TextB{$\dim V_1 = \dim V_2 = \dim V_3 = 7$. Prove that $V_1\cap V_2\cap V_3\neq\{0\}$.}
}%{\normalsize$\MathRightBrace{l}{\dim V_1+\dim V_2 > \dim V\geq\dim V-\dim(V_1\cap V_2)\Rightarrow V_1\cap V_2\neq\{0\}\\\dim V_2+\dim V_3 > \dim V\geq\dim V-\dim(V_1\cap V_2)\Rightarrow V_2\cap V_3\neq\{0\}\\\dim V_1+\dim V_3 > \dim V\geq\dim V-\dim(V_1\cap V_2)\Rightarrow V_1\cap V_3\neq\{0\}}\Rightarrow V_1\cap V_2\cap V_3\neq\{0\}.\hfill\square$}\par\vspace{6pt}
By \TIPS, $\dim(V_1\cap V_2\cap V_3)\geq\dim V_1+\dim V_2+\dim V_3-2\dim V>0.\hfill\square$\par
\ProblemEnding\par

\ProblemBnoor{\hypertarget{2C4e15}{({\normalsize 4E 2.C.15})}}{
...
}%{\normalsize $\MathRightBrace{l}{\dim V_1+\dim V_2 > 2\dim V-\dim V_3\geq\dim V\Rightarrow V_1\cap V_2\neq\{0\}\\\dim V_2+\dim V_3 > 2\dim V-\dim V_1\geq\dim V\Rightarrow V_2\cap V_3\neq\{0\}\\\dim V_1+\dim V_3 > 2\dim V-\dim V_2\geq\dim V\Rightarrow V_1\cap V_3\neq\{0\}}\Rightarrow V_1\cap V_2\cap V_3\neq\{0\}.\hfill\square$}\par\vspace{6pt}
...
Notice that $(v_1,\dots,v_n)$ is a spanning list of $\Spn(v_1,\dots,v_n)$ of length $n=\dim\Spn(v_1,\dots,v_n).\hfill\square$\par
\ProblemEnding\par
...
Then $\forall v\in V,\exists\,!\,a_i\in\Fbb,v=a_1 v_1+\dots+a_n v_n\Rightarrow\exists\,!\,u_i\in V_i,v=u_1+\dots+u_n.$ Thus $V=V_1\oplus\dots\oplus V_n.\hfill\square$\par
\ProblemBnosolus{
	\Corollary\TextB{}
...
Suppose $w=x_1+\dots+x_n,$ where each $x_i=c_i w_i\in W_i.$ Then $(x_1,\dots,x_m)$ is also a basis of $W.\hfill\square$\par
\ProblemEnding\par
...
{\timesbfx Step 1.} $\exists\,v_1\in V\backslash U\Rightarrow v_1\neq 0.$ If $\Spn(v_1)=V$ then we stop.\TextB{}
{\timesbfx Step k.} Suppose $(v_1,\dots,v_{k-1})$ is linely inde in $V,$ each of which belongs to $V\backslash U.$\TextB{}\quad\qquad\;
Note that $\Spn(v_1,\dots,v_{k-1})\neq V.$ And if $\Spn(v_1,\dots,v_{k-1})\cup U=V,$ then by (1.C.12),\TextB{}\quad\qquad\;
( because $\Spn(v_1,\dots,v_{k-1})\not\subseteq U,$ ) $U\subseteq \Spn(v_1,\dots,v_{k-1})\Rightarrow\Spn(v_1,\dots,v_{k-1})=V.$\TextB{}\quad\qquad\;
Hence because $\Spn(v_1,\dots,v_{k-1})\neq V,$ it must be case that $\Spn(v_1,\dots,v_{k-1})\cup U\neq V.$\TextB{}\quad\qquad\;
Thus $\exists\,v_k\in V\backslash U$ such that $v_k\not\in\Spn(v_1,\dots,v_{k-1}).$\TextB{}\quad\qquad\;
...
\ProblemN{4}{
	\TextN{Suppose $T\in\Lm(V,W)$ and $(v_1,\dots,v_m)$ is a list of vecs in $V$ such that}
...
Define $\varphi(u\,+\i v)=u=\mathfrak{Re}\,(u\,+\i v)$\quad\Or Define $\varphi(u\,+\i v)=v=\mathfrak{Im}\,(u\,+\i v).\hfill\square$\par
\ProblemEnding\par
...
Then $T(a_1 v_1+\dots+a_m v_m)=0=a_1 w_1+\dots+a_m w_m=a_j w_j$ while $a_j\neq 0$ and $w_j\neq 0.$ Contradicts.$\hfill\square$\par\vspace{12pt}\quad
\Or {\Large\vspace{6pt}\,\dbsp We prove the contrapositive:}\par\qquad\quad
{\Large\vspace{6pt}\dbsp Suppose $\forall w_1,\dots,w_m\in W,\,\exists\,T\in\Lm(V,W),Tv_k=w_k$  for each $w_k.$}\par\qquad\quad
...
{\Large Then $\sum_{k=1}^m\left|a_k\right|^2=0\Rightarrow a_k=0$ for each $k.$  $(v_1,\dots,v_n).\hfill\square$}\par
\ProblemEnding\par
...
\Or \;{\Large\vspace{6pt}Let $(v_1,\dots,v_m)$ be a basis of $V.$}\par\qquad\quad
{\Large\vspace{6pt}Define $\varphi\in\Lm(V,\Fbb)$ by $\varphi(v_1)=\cdots=\varphi(v_m)=1.$ Let $\lambda=\varphi(Tv_1)\in\Fbb.$}\par\qquad\quad
\dbsp{\Large\vspace{6pt}For any $v\in V,$ define $S_v\in\Lm(V)$ by $S_v u=\varphi(u)v.$}\par\qquad\quad
{\Large\vspace{6pt}Then $Tv=T(\varphi(v_1)v)=T(S_v v_1)=S_v(Tv_1)=\varphi(Tv_1)v=\lambda v.\hfill\square$}\par
\ProblemEnding\par
...
{\Large\vspace{6pt}Getting $(\displaystyle\frac{1}{a_k}T_{1,1}+\dots+\frac{1}{a_k}T_{n,n})v_j=v_j\Rightarrow${$\sum\limits_{r=1}^n \displaystyle\frac{1}{a_k}T_{r,r}=I.$}}\par\quad
{\Large\vspace{6pt}又 By assumption, $T_{x,y}\in\mathcal{E}\Rightarrow I\in\mathcal{E}.$}\par\quad
{\Large\vspace{6pt}Hence for any $T\in\Lm(V),I\circ T=T\circ I=T\in\mathcal{E}\Rightarrow \mathcal{E}=\Lm(V).\hfill\square$}\par\large
\ProblemEnding\par

\ChapterEnding
\pagebreak
\ChapterDecl{Ch3B}{3$\cdot$B}
...
Define $T_2\in\Lm(V,W)$ as $T_2:\,\,\,\,v_1\mapsto w_1,$ \quad\,$v_2\mapsto 0,$\,\,\,\qquad$v_i\mapsto w_i$,\qquad $i=3,\dots,n.}$ Thus $T_1+T_2\not\in U.\hfill\square\\[10pt]$
\Comment \,\,\,If $\dim V=0,$ then $V=\{0\}=\Spn(\,)$. $\forall\,\,T\in\Lm(V,W), T$ is inje. Hence $U=\varnothing$.\par\qquad\qquad\quad\,If $\dim V=1,$ then $V=\Spn(v_0).$ Thus $U=\Spn(T_0),$ where $T_0 v_0=0.$\par
\ProblemEnding\par
...
\Comment \,\,\,If $\dim W=0,$ then $W=\{0\}=\Spn(\,)$. $\forall\,\,T\in\Lm(V,W), T$ is surj. Hence $U=\varnothing.$\par\qquad\qquad\quad\,If $\dim W=1,$ then $W=\Spn(v_0).$ Thus $U=\Spn(T_0),$ where $T_0 v_0=0.$\par
\ProblemEnding\par
\pagebreak
...
(b) $\forall\,w\in\range T,\,\exists\,v\in V,w=Tv.\,(\,\exists\,\,a_i\in\Fbb,v=a_1 v_1+\dots+a_n v_n)\Rightarrow w=a_1 Tv_1+\dots+a_n Tv_n.\hfill\square$\par\quad
\ProblemEnding\par
...
\ProblemN{12}{
	\TextNL{Suppose that $V$ is finite-dim and that $T\in\Lm(V, W)$.}
	\TextNL{Prove that $\exists$ a subsp $U$ of \,$V$ such that $U\cap\null T=\{0\},\;\range T=\left\{Tu:u\in U\right\}$.}
}\par\quad
...
Let $(u_1,\dots,u_m)$ be a basis of $U$, extend to a basis of $V$ as $(u_1,\dots,u_m,v_1,\dots,v_n).$\par\quad
Let $(w_1,\dots,w_p)$ be a basis of $W$. Note that $\dim W=p\geq n=\dim V-\dim U$.\par\quad
Define $T\in\Lm(V,W)$ by $T(a_1 v_1+\dots+a_n v_n+b_1 u_1+\dots+b_m u_m)=a_1 w_1+\dots+a_n w_n.\hfill\square$\par
\ProblemEnding\par

\BulletPoint \,\hspace{1pt}\Tips \,\,\,Suppose $T\in\Lm(V,W)$ and $R=(Tv_1,\dots,Tv_n)$ is linely inde in $\range T$.\TextB{}\hspace{30pt}
\large ( Let $\dim\range T=n$, if $\range T$ is finite, otherwise let $n\in\Nbp.$ )\TextB{}\hspace{30pt}
{By (3.A.4), $L=(v_1,\dots,v_n)$ is linely inde in $V$.\vspace{4pt}}\par
...
Note that if $\range T$ is finite-dim, then $\mathcal{K}_{\range T}=\mathcal{K}_R$ for any basis $R$ of $\range T.$\par\vspace{6pt}
...
\BulletPoint \,\hspace{1pt}\Comment\,\,\,$\null T\in\mathcal{S}_V \mathcal{K}_R$.\vspace{-10pt}\par
\ProblemEnding\par
...
\centerline{\quad where $n=\dim\range T$ if finite-dim, otherwise $n\in\Nbp.$}\par\quad\Hb
Then $\mathcal{K}_R\oplus\null T=V.$ Define $S\in\Lm(W,V)$ by $S(Tv_i)=v_i.$ Then $TS=I.\hfill\square\\[-6pt]$
\ProblemEnding\par
\pagebreak
...
$}\Rightarrow\square$\par\vspace{10pt}\quad
...
\ProblemB{
	\dbsp(a) \TextB{Suppose $\dim V = 5,S,T\in\Lm(V)$ are such that $ST = 0$. Prove that $\dim\range TS\leq 2$.}
...
\vspace{-5pt}
\AlignEq{}{T&:\quad v_1\mapsto 0,\quad\,\, v_2\mapsto 0,\,\,\quad v_i\mapsto v_i\,\,\,;\\S&:\quad v_1\mapsto v_4,\quad v_2\mapsto v_5,\quad v_i\mapsto 0\,\,\,\,\,;\qquad i=3,4,5.}\vspace{-6pt}\par
\ProblemEnding\par
\pagebreak
\ProblemB{
	\TextB{Suppose $\dim V=n$ and $S,T\in\Lm(V)$ are such that $ST=0$.}
	\TextB{Prove that $\dim\range TS\leq\left\lfloor\displaystyle\frac{n}{2}\right\rfloor.$}
}\par\quad
...
Assume that $\dim\range TS\geq \left\lfloor\displaystyle\frac{n}{2}\right\rfloor+1$.\par\quad
\AlignEq{}{\textup{Then\,}&\min\left\{n-\dim\null T,n-\dim\null S\right\}\geq \left\lfloor\displaystyle\frac{n}{2}\right\rfloor+1\qquad\qquad\qquad\qquad\qquad\qquad\qquad\qquad\qquad\qquad\hspace{1.8pt}\\\Rightarrow&\max\left\{\dim\null T,\dim\null S\right\}\leq n-\left\lfloor\displaystyle\frac{n}{2}\right\rfloor-1.}\par\quad
又 $\dim\null ST=n\leq\dim\null S+\dim\null T\leq 2(n-\left\lfloor\displaystyle\frac{n}{2}\right\rfloor-1)$\par\quad
$\Rightarrow \left\lfloor\displaystyle\frac{n}{2}\right\rfloor+1\leq\displaystyle\frac{n}{2}$. Contradicts. Thus $\dim\range TS\leq \left\lfloor\displaystyle\frac{n}{2}\right\rfloor.\hfill\square$\par\vspace{6pt}\quad
\Or $\dim\null S=n-\dim\range S\leq n-\dim\range TS.$\par
\AlignEq{}{\textup{And\,}ST=0&\Rightarrow \dim\range TS\leq\dim\range T\leq\dim\null S\leq n-\dim\range TS\qquad\qquad\qquad\qquad\quad\hspace{2pt}\\&\Rightarrow 2\dim\range TS\leq n\Rightarrow\dim\range TS\leq\displaystyle\frac{n}{2}\\&\Rightarrow\dim\range TS\leq\left\lfloor\displaystyle\frac{n}{2}\right\rfloor \textup{\,(\,because\,}\dim\range TS\textup{\,is\,an\,integer\,)}.\hfill\square}\par
\ProblemEnding\par
...
Suppose $\null S\subseteq\null T$. Let $R=(Sv_1,\dots,Sv_n)$ be a basis of $\range S$\par\quad
Then $(v_1,\dots,v_n)$ is linely inde. Let $\mathcal{K}_R=\Spn(v_1,\dots,v_n)\Rightarrow V=\mathcal{K}_R\oplus\null S$.\par\quad
...
\ProblemN{27}{
	\TextNL{Suppose $p\in\PoR{}$. Prove that $\,\exists\,q\in\PoR{}$ such that $5q\apostrophe\apostrophe + 3q\apostrophe = p$.\vspace{6pt}}
}\par\quad
...
$\varphi_1(v)=a_v\varphi_1(u),\,\,\,\varphi_2(v)=a_v\varphi_2(u)\Rightarrow a_v=\displaystyle\frac{\varphi_1(v)}{\varphi_1(u)}=\frac{\varphi_2(v)}{\varphi_2(u)}\Rightarrow\frac{\varphi_1(u)}{\varphi_2(u)}=\frac{\varphi_1(v)}{\varphi_2(v)}=c\in\Fbb.\hfill\square$\par\large
\ProblemEnding\par
...
While $w_2=Tv_2=\lambda Sv_2=2\lambda w_2\Rightarrow \lambda=\frac{1}{2}$. Contradicts. $\hfill\square$\vspace{-3pt}\par
\ProblemEnding\par
...
{\Large\vspace{4pt}Because $R_{i,j}=R_{x,j}\circ R_{i,x},\,\,\forall x=1,\dots,n$}\par\qquad\,
...
{\Large\vspace{4pt}$\Rightarrow\varphi(R_{l,k})=0$ or $\varphi(R_{i,j})=0.$ Contradicts.$\hfill\square$}\par\vspace{10pt}\quad
\Or {\Large\vspace{4pt}Note that by (4E 3.A.16), $\,\exists\,S,T\in\Lm(V),ST-TS\neq 0.$}\par\quad
{\Large\vspace{4pt}Then $\varphi(ST-TS)=\varphi(S)\varphi(T)-\varphi(T)\varphi(S)=0\Rightarrow ST-TS\in\null\varphi\neq\{0\}.$}\par\quad
{\Large\vspace{4pt}Thus $\forall\,E\in\null\varphi,T\in\Lm(V),\varphi(ET)=\varphi(TE)=0\Rightarrow ET,TE\in\null\varphi.$}\par\quad
{\Large\vspace{4pt}Hence $\null\varphi$ is a nonzero two-sided ideal of $\Lm(V).\hfill\square$}\vspace{-5pt}\par
\ProblemEnding\par
...
Extend $(u_1,\dots,u_n)$ to a basis of $V$ as $(u_1,\dots,u_n,v_1,\dots,v_m).$\vspace{3pt}\par\quad
...
$}\Rightarrow\range T=Y.\hfill\square$\vspace{6pt}\par
\ProblemEnding\par
...
$\Rightarrow w\in\Spn(Tv_1,\dots,Tv_n)\Rightarrow\range T\subseteq\Spn(Tv_1,\dots,Tv_n).\hfill\square$\par
\Comment\,\,\, If $T$ is inje, then $(Tv_1,\dots,Tv_n)$ is a basis of $\range T$.\par
\ProblemEnding\par
\ChapterEnding
\vspace{18pt}

\ChapterDecl{Ch3C}{3$\cdot$C}{}\orMode{\qquad\hLk{3C1}{1}\;\;\hLk{3C3}{3}\;\;\hLk{3C4}{4}\;\;\hLk{3C5}{5}\;\;\hLk{3C6}{6}\;\;\hLk{3C9}{9}\;\;\hLk{3C10}{10}\;\;\hLk{3C11}{11}\;\;\hLk{3C12}{12}\;\;\hLk{3C13}{13}\;\;\hLk{3C14}{14}\;\;\hLk{3C15}{15}\;\;\hLk{3C4e16}{16}\;\;\hLk{3C4e17}{17}}{\nLineDB\qquad[2]: 10, 9, 11; [3]: (4E 17); [4]: (4E 16), 1; [5]: 3, 4, 5, 6; [6]: 13, 14, 15, 12.}
\vspace{10pt}
...
\BulletPoint \,\NoteForSmall{[3.49]} \AlignEq{}{&\because[(AC)_{\cdot,k}]_{j,1}=(AC)_{j,k}=\sum\limits_{r=1}^n A_{j,r}C_{r,k}=\sum\limits_{r=1}^n A_{j,r}(C_{\cdot,k})_{r,1}=(AC_{\cdot,k})_{j,1}\\&\therefore(AC)_{\cdot,k}=A_{\cdot,\cdot}C_{\cdot,k}=AC_{\cdot,k}}\vspace{-10pt}\par
\ProblemEnding\par

\BulletPoint \,\Exercise{\hypertarget{3C10}{10}} \AlignEq{}{&\because[(AC)_{j,\cdot}]_{1,k}=(AC)_{j,k}=\sum\limits_{r=1}^n A_{j,r}C_{r,k}=\sum\limits_{r=1}^n (A_{j,\cdot})_{1,r}C_{r,k}=(A_{j,\cdot}C)_{1,k}\\
&\therefore(AC)_{j,\cdot}=A_{j,\cdot}C_{\cdot,\cdot}=A_{j,\cdot}C.}\vspace{-10pt}\par
\ProblemEnding\par

\BulletPoint
	({\normalsize 4E 3.51})\;\TextB{Suppose $C\in\Fbb^{m,c},R\in\Fbb^{c,p}$.}
	(a) \TextB{\large\timesnr For $k=1,\dots,p,$\quad $(CR)_{\cdot,k}=CR_{\cdot,k}=C_{\cdot,\cdot}R_{\cdot,k}=\sum\limits_{r=1}^c C_{\cdot,r}R_{r,k}=R_{1,k}C_{\cdot,1}+\dots+R_{c,k}C_{\cdot,c}$\vspace{3pt}}
	\Ha\TextB{\large Which means that each cols $CR$ is a linear combination of the cols of $C.$\vspace{6pt}}
	(b) \TextB{\large\timesnr For $j=1,\dots,m,$\quad $(CR)_{j,\cdot}=C_{j,\cdot}R=C_{j,\cdot}R_{\cdot,\cdot}=\sum\limits_{r=1}^c C_{j,r}R_{r,\cdot}=C_{j,1}R_{1,\cdot}+\dots+C_{j,c}R_{c,\cdot}$\vspace{3pt}}
	\Hb\TextB{\large Which means that each rows $CR$ is a linear combination of the rows of $R.$}\vspace{-25pt}\par
%	\Example\TextB{}
...
\ProblemEnding\par
\BulletPoint \,\Exercise{\hypertarget{3C11}{11}}\qquad\,\,\,\, $a\in\Fbb^{1,n},C\in\Fbb^{n,p}\Rightarrow aC\in\Fbb^{1,p}$\par\quad
...
Let $(C_{\cdot,1},\dots,C_{\cdot,c})$ be a basis of $S_c$, forming $C\in\Fbb^{m,c}$.\par\qquad
\Or Let $(R_{1,\cdot},\dots,R_{r,\cdot})$ be a basis of $S_r,$ forming $R\in\Fbb^{c,n}$.\par\quad
Then for any $k,\;A_{\cdot,k}=R_{1,k}C_{\cdot,1}+\dots+R_{c,k}C_{\cdot,c}=(CR)_{\cdot,k},\,\exists\,!\,R_{1,k},\dots,R_{c,k}\in\Fbb,$ forming $R\in\Fbb^{c,n}.$\par\qquad
\Or For any $k,\;A_{j,\cdot}=C_{j,1}R_{1,\cdot}+\dots+C_{j,c}R_{c,\cdot}=(CR)_{j,\cdot},\,\exists\,!\,C_{j,1},\dots,C_{j,c}\in\Fbb,$ forming $C\in\Fbb^{m,c}.$\par\quad
Now we have $A=CR.\hfill\square$\vspace{6pt}\par
\Example\par\quad
...
\ProblemBor{\hypertarget{3C4e17}{\normalsize 4E 3.C.17, 3.F.32}}{
...
&\Longleftrightarrow(A_{1,\cdot},\dots,A_{n,\cdot})\text{ is a basis of }\Fbb^{1,n}.}\vspace{-25pt}\par$\hfill\square$\vspace{18pt}\par\quad
Now we show that $(\Delta)$ properly.\vspace{5pt}\par\quad
...
Since there are $n-p=q$ choices of such $k,$ $A$ has at least $q=\dim\range T$ nonzero entries.$\hfill\square$\vspace{8pt}\par\quad
...
$\Rightarrow\dim\range T=\dim\Spn(Tv_{p+1},\dots,Tv_n)\leq\dim\range T-1.$ Contradicts.$\hfill\square$\par
\ProblemEnding\par
\pagebreak
...
Let $\mathcal{K}_R=\Spn(v_1,\dots,v_n)$. And let $(u_1,\dots,u_m)$ be a basis of $\null T$. Then $B_V=(v_1,\dots,v_n,u_1,\dots,u_m).\hfill\square$\par
\ProblemEnding\par
...
\vspace{15pt}\AlignEq{}{\forall j\in\{2,\dots,k,\underbrace{k+2,\dots,n+1}_{i\,\in\,\{k+1,\dots,n\}}\},\;Tv_j=T\left(u_{j-1}-A_{1,j-1}v_1\right)=Tu_{j-1}-T\left(\displaystyle\frac{A_{1,j-1}u_k}{A_{1,k}}\right)\hspace{97pt}\\=A_{1,j-1}w_1+\dots+A_{n,j-1}w_n-A_{1,j-1}\left(1w_1+\dots+\displaystyle\frac{A_{n,k}}{A_{1,k}}w_n\right)=0w_1+\dots+\left(A_{n,j-1}-\displaystyle\frac{A_{1,j-1}A_{n,k}}{A_{1,k}}\right)w_n.}
%\vspace{10pt}\par\quad\AlignEq{}{\forall i\in\{k+1,\dots,n\},\;Tv_i=T\left(u_{i}-A_{1,i}v_1\right)=Tu_{i}-T\left(\displaystyle\frac{A_{1,i}u_k}{A_{1,k}}\right)\hspace{199pt}\\=A_{1,i}w_1+\dots+A_{n,i}w_n-A_{1,i}\left(1w_1+\dots+\displaystyle\frac{A_{n,k}}{A_{1,k}}w_n\right)=0w_1+\dots+\left(A_{n,i}-\displaystyle\frac{A_{1,i}A_{n,k}}{A_{1,k}}\right)w_n.}
\vspace{-6pt}\par$\hfill\square$\vspace{-10pt}\par
\ProblemEnding\par
...
Let $v_1=u_1,v_i=u_1+u_i.$ Extend to a basis of $V$ and we have $B_V.\hfill\square$\vspace{10pt}\par\quad\Hb
...
Let $v_k=\lambda_k^{-1}u_k\neq 0\Rightarrow B_V=(v_1,\dots,v_n).$ Hence for each $v_k,Tv_k=w=w_1+\dots+w_m.\hfill\square$\par
\ProblemEnding\par
...
Then $\displaystyle\Mt\left(p(T_1,\dots,T_n)\right)=\Mt\left(\sum\limits_{k_1,\dots,k_n}\alpha_{k_1,\dots,k_n}\prod\limits_{i=1}^n T_i^{k_i}\right)=\sum\limits_{k_1,\dots,k_n}\alpha_{k_1,\dots,k_n}\prod\limits_{i=1}^n \Mt(T_i^{k_i})=p\left(\Mt(T_1),\dots,\Mt(T_n)\right).\hfill\square$\par
%Hence $\displaystyle\Mt\left(p(T_1,\dots,T_n)\right)=\sum\limits_{k_1,\dots,k_n}\alpha_{k_1,\dots,k_n}\left(\Mt(T)^{k_1}\right)\Mt\left(\prod\limits_{i=2}^n T_i^{k_i}\right).$\par\quad
%And for any $n>k\geq 1,$ $\displaystyle\Mt\left(p(T_1,\dots,T_n)\right)=\sum\limits_{k_1,\dots,k_n}\alpha_{k_1,\dots,k_n}\left(\prod\limits_{i=1}^k\Mt(T)^{k_i}\right)\Mt\left(\prod\limits_{i=k+1}^n T_i^{k_i}\right).$\par\quad
%Hence by induction, we conclude that $\displaystyle\sum\limits_{k_1,\dots,k_n}\alpha_{k_1,\dots,k_n}\Mt\left(\prod\limits_{i=1}^n T_i^{k_i}\right)=\sum\limits_{k_1,\dots,k_n}\alpha_{k_1,\dots,k_n}\prod\limits_{i=1}^n \Mt(T_i^{k_i}).\hfill\square$\vspace{6pt}\par
\ProblemEnding\par
...
Note that $[A(B+C)]_{j,k}=\sum\limits_{r=1}^n A_{j,r}(B+C)_{r,k}=\sum\limits_{r=1}^n (A_{j,r}B_{r,k}+A_{j,r}C_{r,k})=(AB+AC)_{j,k}.\hfill\square$\vspace{8pt}\par\quad
Define $T,S,R$ such that $\Mt(T)=A,\Mt(S)=B,\Mt(R)=C.$\par\quad
$A(B+C)=\Mt(T(S+R))\overset{[3.9]}{=}\Mt(TS+TR)=AB+AC.$\par\quad
\Or $T(S+R)=TS+TR\Rightarrow\Mt\left(T(S+R)\right)=\Mt(TS+TR)\Rightarrow A(B+C)=AB+AC.\hfill\square$\par
\ProblemEnding\par
...
Suppose $A\in\Fbb^{m,n}$ and $B,C\in\Fbb^{n,p}.$\par\quad
Note that $[(AB)C]_{j,k}=(AB)_{j,\cdot}C_{\cdot,k}=\sum\limits_{s=1}^n(A_{j,s}B_{s,\cdot})C_{\cdot,k}=\sum\limits_{s=1}^n A_{j,s}(B_{s,\cdot}C_{\cdot,k})=\sum\limits_{s=1}^n A_{j,s}(BC)_{s,k}=[A(BC)]_{j,k}.\hfill\square$\vspace{8pt}\par\quad
Define $T,S,R$ such that $\Mt(T)=A,\Mt(S)=B,\Mt(R)=C.$\par\quad
$(AB)C=\Mt(T(SR))\overset{[3.9]}{=}\Mt(TSR)\overset{[3.9]}{=}\Mt((TS)R)=A(BC).$\par\quad
\Or $(TS)R=T(SR)\Rightarrow\Mt((TS)R)=\Mt(T(SR))\Rightarrow (AB)C=A(BC).\hfill\square$\par
\ProblemEnding\par
...
\textup{\Or}\,\,\,\,(AAA)_{j,k} &=\sum\limits_{r=1}^n(AA)_{j,r}A_{r,k} =\sum\limits_{r=1}^n(\sum\limits_{p=1}^n A_{j,p}A_{p,r})A_{r,k}\\&=\sum\limits_{r=1}^n\left[A_{j,1}(A_{1,r}A_{r,k})+\dots+A_{j,n}(A_{n,r}A_{r,k})\right]\\&=A_{j,1}\sum\limits_{r=1}^n A_{1,r}A_{r,k}+\dots+A_{j,n}\sum\limits_{r=1}^n A_{n,r}A_{r,k}=\sum\limits_{p=1}^n \sum\limits_{r=1}^n A_{j,p}A_{p,r}A_{r,k}.\hfill\square}\par
\ProblemEnding\par
...
Hence $ST=TS.$ Which in general is not true. ( See 3.D )$\hfill\square$\par
\ProblemEnding\par
\pagebreak

\ProblemBor{10.A.3, 4E 3.D.19}{
	\TextB{Suppose $V$ is finite-dim and $T\in\Lm(V)$.}
...
{\normalsize$\MathRightBrace{l}{$
		...
		$}\Rightarrow T=\lambda I,\,\exists\,\lambda\in\Fbb.\hfill\square$}\par\vspace{20pt}\quad
...
Thus $A_{k,k}=A_{j,j}.\hfill\square$\vspace{10pt}\par\quad
\Or ??? ( There must be another solution using theorems and facts given in 3.D )\par
\ProblemEnding\par
\ChapterEnding
\pagebreak

\ChapterDecl{Ch3D}{3$\cdot$D}{}
\orMode{\qquad\hLk{3D1}{1}\;\;\hLk{3D2}{2}\;\;\hLk{3D3}{3}\;\;\hLk{3D4}{4}\;\;\hLk{3D5}{5}\;\;\hLk{3D6}{6}\;\;\hLk{3D8}{8}\;\;\hLk{3D9}{9}\;\;\hLk{3D10}{10}\;\;\hLk{3D11}{11}\;\;\hLk{3D12}{12}\;\;\hLk{3D13}{13}\;\;\hLk{3D15}{15}\;\;\hLk{3D16}{16}\;\;\hLk{3D17}{17}\;\;\hLk{3D18}{18}\;\;\hLk{3D19}{19}\;\;\hLk{3D4e1}{4E 1,}\;\;\hLk{3D4e3}{3,}\;\;\hLk{3D4e10}{10,}\;\;\hLk{3D4e15}{15,}\;\;\hLk{3D4e17}{17,}\;\;\hLk{3D4e20}{20,}\;\;\hLk{3D4e22}{22,}\;\;\hLk{3D4e23}{23,}\;\;\hLk{3D4e24}{24}}
{$\\[-30pt]$\normalsize\vspace{-4pt}\nLineDB\qquad\qquad[1]: (4E 3, 15, 22, 1), 1, 2, 3; [2]: 4, 5, 6, 8; [3]: 9, 10, 11, 12, 13, 15, (4E 24);{\large\par\qquad\quad\;\;\hspace{1pt}}[4]: (4E 10); [5]: (4E 17); [6]: 17, (4E 23); [7]: 16, 18, (4E 20), 19.\vspace{4pt}}
...
$\Rightarrow\Mt^{-1}\Mt(TS)=\Mt^{-1}\Mt(ST)=I$\large$\,=\,TS=ST\Rightarrow S=T^{-1}.\hfill\square$\par
\ProblemEnding\par

\ProblemB{
	\hypertarget{3D4e1}{}\vspace{2pt}\TextB{Suppose $T\in\Lm(V, W)$ is inv. Show that $T^{-1}$ is inv and $(T^{-1})^{-1}=T$.\vspace{8pt}}
}$\MathRightBrace{l}{
	TT^{-1}=(T^{-1})^{-1}T^{-1}=I\in\Lm(V)\\
	T^{-1}T=T^{-1}(T^{-1})^{-1}=I\in\Lm(W)
}\Rightarrow T=(T^{-1})^{-1}$, by the uniqnes of inverse$.\hfill\square$\vspace{5pt}\par
\ProblemEnding\par
...
}\Rightarrow (ST)^{-1} = T^{-1}S^{-1}$, by the uniqnes of inverse$.\hfill\square$\vspace{5pt}\par
\ProblemEnding\par
...
$S(a_1 v_1+\dots+a_n v_n)=a_1 v_1,T(a_1 v_1+\dots+a_n v_n)=a_2 v_1+\dots+a_n v_n$. Hence $S+T=I$ is inv.$\hfill\square$\par
...
Define $T\in\Lm(V)$ by $T(u_i)=Su_i;\,\,\,Tv_j=w_j,$ for each $i\in\{1,\dots,m\},j\in\{1,\dots,n\}.\hfill\square$\par
\ProblemEnding\par
...
\ProblemN{\hypertarget{3D8}{8}}{
	\TextN{Suppose $V$ is finite-dim and $T: V\rightarrow W$ is a {\timessc surj} linear map of $V$ onto $W$.}
...
Then $\dim\mathcal{K}=\dim W.$ Thus $T|_{\mathcal{K}}$ is an iso of ${\mathcal{K}}$ onto $W.\hfill\square$\par\vspace{6pt}\quad
...
$\forall v\in V,v=(ST)Rv=S(TRv)\in\range S}\Rightarrow T$ is inje, $S$ is surj. While $V$ is finite-dim.$\hfill\square$\vspace{6pt}\par\quad
\Or Because by Problem (23) in 3.B, $\dim V=\dim\range ST\leq\min\left\{\range T,\range S\right\}$.$\hfill\square$\par
\ProblemEnding\par
...
$\forall v\in V,v=(RST)Xv\in\range R\Rightarrow R$ is surj.$\\\end{array}\right\}\Rightarrow S=R^{-1}(RST)T^{-1}$ is inv.\hfill$\square$\vspace{6pt}\par\quad
\Or $(RST)^{-1}=\left((RS)T\right)^{-1}=T^{-1}(RS)^{-1}=T^{-1}S^{-1}R^{-1}.\hfill\square$\par
\ProblemEnding\par
...
$\forall k=1,\dots,n,$ suppose $T(E_{k})=A_{1,k}R_1+\dots+A_{m,k}R_m,\exists\,A_{j,k}\in\Fbb$, forming $A\,=\,${\normalsize$\begin{pmatrix} A_{1,1} & \cdots & A_{1,n}\\ \vdots & \ddots & \vdots\\ A_{m,1} & \cdots & A_{m,n}\end{pmatrix}.\hfill\square$}\vspace{-6pt}\par\quad
\Or Let $A=\Mt(T,B_1,B_2)$. Note that $\Mt(x,B_1)=x,\Mt(y,B_2)=y.$\par\quad
Hence $Tx=\Mt(Tx,B_2)=\Mt(T,B_1,B_2)\Mt(x,B_1)=Ax,$ by [3.65].$\hfill\square$\par
\ProblemEnding\par
...
Thus $AB=I\Leftrightarrow A(Bx)=x\Longleftrightarrow T(Sx)=x\Leftrightarrow TS=I\Longleftrightarrow ST=I\Longleftrightarrow \Mt(S)\Mt(T)=BA=I.\hfill\square$\par
\ProblemEnding\vspace{10pt}\par
...
\ProblemB{
	...
	\TextB{{\normalsize Hint: Define $\Phi:\Lm(V, W)\rightarrow \Lm(U, W)$ by $\Phi(T) = T|_U$ . What is $\null\Phi$? What is $\range\Phi$?}}
}\par\quad
...
$\forall \,T\in\mathcal{E},k\in\{1,\dots,m\},TE_{k,k}=0\Rightarrow{\normalsize\underbrace{\Spn\begin{Bmatrix} \underset{,}{E_{1,1}}, & \cdots & ,\underset{,}{E_{m,1}},\\ \vdots & \ddots & \,\vdots\\ \overset{,}{E_{1,p}}, & \cdots & ,\overset{,}{E_{m,p}}\end{Bmatrix}}_{\textup{Denote\,it\,by\,R}}}\,\cap\,\mathcal{E}=\{0\}$.\vspace{-12pt}\par\quad\Hb
...
{\Large\vspace{6pt}Thus $\range\mathcal{A}=\left\{R\in\Lm(V):\range R\subseteq\range S\right\}=\Lm(V,\range S).$\hfill$\square$}\par\vspace{6pt}\quad
...
Hence (a) $\dim\null\mathcal{B}=n\times(n-m);$\par\hspace{266pt} (b) $\dim\range\mathcal{B}=n\times m.\hfill\square$\par
\ProblemEnding\par
...
Again, {\Large$E_{y,x\apostrophe},E_{y\apostrophe,x}\in\mathcal{E}$} for all $x\apostrophe,y\apostrophe,x,y=1,\dots,n.$ Thus $\mathcal{E}=\Lm(V).\hfill\square$\par
\ProblemEnding\par
...
(b) $\forall T\in\Lm(\Fbb,V),$ let $v= T(1)\Rightarrow T(\lambda)=\lambda v=\Psi_v(\lambda),\forall\lambda\in\Fbb\Rightarrow T=\Psi\left( T(1)\right).$ Hence $\Psi$ is surj.\hfill$\square$\vspace{6pt}\par\quad
...
(b) For any $v\in V,$ define $T\in\Lm(\Fbb,V)$ by $T(\lambda)=\lambda v.$ Then $\Phi(T)=T(1)=v.$ Thus $\Phi$ is surj.$\hfill\square$\par
...
Hence $\exists\,p\in\PoR{m}, q(x)=T_m(p)=(x^2 + x)p\apostrophe\apostrophe(x) + 2xp\apostrophe(x) + p(3)$ for all $x\in\Rbb.\hfill\square$\par
\ProblemEnding\par
...
While $\deg s=\deg Ts=\deg Tr<\deg r$. Contradicts. Hence $\forall p\in\Po_{n+1}(\Rbb),\deg Tp=\deg p.\hfill\square$\par
\ProblemEnding\par
\ChapterEnding

\ChapterDecl{Ch3E}{3$\cdot$E}
...
\ProblemNnosolus{20}{
	...
	\dbsp(a) \TextNL{Prove that $\Gamma$ is linear: {\large\timesnr By [3.9] distr and [3.6].}}
...
\ProblemN{7}{
	\TextN{Suppose $m\in\Nbp$. Show that the dual basis of the basis $(1, x,\dots,x_m)$ of $\PoR{m}$}
...
\ProblemNnosolus{8}{
	...
	\vspace{4pt}(a) \TextN{\timesnr{\large By [2.C.10], $B$} $=(1,x-5,\dots,(x-5)^m)$ is a basis of $\PoR{m}$.}
	(b) \TextN{\timesnr$\varphi_k=\displaystyle\frac{p^{(k)}(5)}{k!}$ for each $k=0,1,\dots,m$. Then $(\varphi_0,\varphi_1,\dots,\varphi_m)$ is the dual basis of {\large$B$}.}
}\par
\ProblemEnding\par
...
}$T=0\Longleftrightarrow T\apostrophe(\varphi)=\varphi\circ T=0$ for all $\varphi\in V\apostrophe\Longleftrightarrow T\apostrophe=0.\hfill\square \\ $
\ProblemEnding\par
...
\BulletPoint \,\hspace{1pt}{\timessl\Large Suppose $V$ is a vecsp and $U\subseteq V.$}\par
\ProblemNnosolus{17}{
	\TextNL{\timesnr $U^0=\left\{\varphi\in V\apostrophe:U\subseteq null\varphi\right\}$. Noticing $\varphi\in V\apostrophe$, $U\subseteq\null \varphi\Longleftrightarrow\forall u\in U,\varphi(u)=0$. }
}\vspace{5pt}\par
\ProblemNnosolus{18}{
...
\ProblemNnosolus{19}{
	\TextNL{\timesnr $U=V\Longleftrightarrow U^0_V=\{0\}=V^0_V$. By the inverse and contrapositive of Problem (4).}
}\par
...
\BulletPoint \,\hspace{1pt}\Corollary\;\;\TextB{Suppose $\left\{V_{\alpha_i}\right\}_{\alpha_i\in\Gamma}$ is a collection of subsps of\, $V$.}
...
{\timesbfx Step 1.} $\,$If $\,\exists\,w_1\in\Spn(v_2,\dots,v_n)$ such that $0\neq Tw_1\in\Spn(v_1)$,\par\qquad\Ha\qquad
...
{\timesbfx Step k.} $\,$If $\,\exists\,w_k\in\Spn(\alpha_{k-1,2},\dots,\alpha_{k-1,n-k+1})$ such that $0\neq Tw_k\in\Spn(v_1,w_1,\dots,w_{k-1}),$\par\qquad\qquad\quad\,
...
\ChapterDecl{Ch5BI}{5.B: I}{
	\quad{\LARGE[}
	{\Large\timessl See \hLk{Ch5BII}{5.B: II} below.}
	{\LARGE ]}
}\large\vspace{5pt}
...
Suppose $T$ is surj. Then $\forall u\in V,\,\exists\,v_1,\dots,v_m\in V, T(v_1)=T^2v_2=\cdots=T^m v_m=u.\hfill\square$\par
\ProblemEnding\par
...
\ChapterEnding
\ChapterDecl{Ch5E}{5.E* (4E)} % 0.5h/4h
