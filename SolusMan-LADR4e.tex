...
%{\timessl\Large for all $a,b\in$ {\timesbf R} and all $u,v\in V$.
	%}\par\,\,\,
%{\timessl\Large $V_\Cbb$ is a complex vector space, with the definitions above.
	%}\par\quad
%{\normalsize\timessl
...
$}\Rightarrow U\cup W=U$ or $W.$ Contradicts!\par\tinypar\quad\Habc
...
\par\quad\Habc\HII
$\exists\,u\in U_1\wedge u\not\in U_2\cup U_3;\,\,\,v\in U_2\cup U_3\wedge v\not\in U_1.$ Let $W=\left\{v+\lambda u:\lambda\in\Fbb\right\}\subseteq\mathcal{U}.$\par\quad\Habc\HII
...
(a) By def, $U+W=\left\{(x_1+x_2,x_1+x_2,y_1+x_2,y_1+y_2)\in\Fbb^4:(x_1,x_1,y_1,y_1)\in U,(x_2,x_2,x_2,y_2)\in W\right\}$.\par\quad\Habc
$\Rightarrow\forall v\in U+W,\,\exists\,t\in T,v=t\Rightarrow U+W\subseteq T$.\par\quad
(b) $\forall x,y,z\in\Fbb,$ let $u=(0,0,y-x,y-x)\in U,w=(x,x,x,-y+x+z)\in W$\par\quad\Habc
...
(b) $\forall x,y,z,w,u\in\Fbb,$ let $u=(x,y,x+y,x-y,2x)\in U,w=(0,0,z-x-y,w-x-y,u-2x)\in W$\par\quad\Habc
...
(a) { $V_E\cap V_O=\left\{f:f(x)=f(-x)=-f(-x)\right\}=\{0\}.$}\par\tinypar\quad
(b){$\MathRightBrace{r}{$
...
(a) Let $\Spn(v_1,\dots,v_m)=U$. Assume that $\forall v\in U,\,\,\exists\,\,a_i\in\Fbb,$\par\quad\Habc
$v=a_1 v_1+\dots+a_m v_m=b_1 w_1+\dots+b_m w_m=\sum\limits_{j=1}^m(\sum\limits_{i=j}^m b_i)v_j$\par\quad\Habc
$\Rightarrow b_1=a_1,\,\,\,b_i=a_i-\sum\limits_{r=1}^{i-1} b_r.$ Thus $\,\,\exists\,\,b_i\in\Fbb$ such that $v=b_1 w_1+\dots+b_m w_m$.\par\quad\Habc
...
\ProblemBor{9.4}{
	...
	\TextB{then $(v_1,\dots,v_n)$ is also a basis of the complexification $V_\Cbb$ (as a complex vector space).}\hspace{0.5pt}
...
Hence $\left|(A\cup B)\cup C\right|=\left| A\right|+\left| B\right|+\left| C\right|+\left| A\cap B\cap C\right|-\left| A\cap B\right|-\left| A\cap C\right|-\left| B\cap C\right|.$\par\footpar\quad
Because $(V_1+V_2)+V_3=V_1+(V_2+V_3)=(V_1+V_3)+V_2$.\par\quad
\AlignEq{}{\dim(V_1+V_2+V_3)&=\dim(V_1+V_2)+\dim(V_3)-\dim\left((V_1+V_2)\cap V_3\right)\quad(1)\qquad\qquad\qquad\qquad\quad\,\,\\&=\dim(V_2+V_3)+\dim(V_1)-\dim\left((V_2+V_3)\cap V_1\right)\quad(2)\\&=\dim(V_1+V_3)+\dim(V_2)-\dim\left((V_1+V_3)\cap V_2\right)\quad(3)}\par\quad
...
\ProblemN{9}{
	...
	\hspace{0.5pt}\TextN{\large(Here $\Cbb$ is thought of as a complex vector space.)}
}\par\quad
Suppose $V_\Cbb$ is the complexification of a vector space $V$. Suppose $\varphi:V_\Cbb\rightarrow V_\Cbb.$\par\quad
...
(a) $\range T=\left\{Tv:v\in V\right\}=\left\{Tv:v\in\Spn(v_1,\dots,v_n)\right\}\Rightarrow Tv_1,\dots,Tv_n\in\range T\Rightarrow$ By [2.7].\par\quad\Habc
...
\AlignEq{}{\textup{Then\,}&\min\left\{n-\dim\null T,n-\dim\null S\right\}\geq \left[\displaystyle\frac{n}{2}\right]+1\qquad\qquad\qquad\qquad\qquad\qquad\qquad\qquad\qquad\qquad\hspace{1.5pt}\\\Rightarrow&\max\left\{\dim\null T,\dim\null S\right\}\leq n-\left[\displaystyle\frac{n}{2}\right]-1.}\par\quad
...
$\end{array}\right\}\right.\Rightarrow V=\null\varphi\oplus\left\{au:a\in\Fbb\right\}.\hfill\square$\par\tinypar\large\quad
...
%}\par\quad
%Let $(v_1,\dots,v_5)$ be a basis of $\Rbb^5$, $(w_1,w_2)$ be a basis of $\Rbb^2$. Define $T,S\in\Lm(V,W)$ by\par\tinypar\quad
%$\MathRightBrace{l}{$
...
$}\Rightarrow\square$\par\tinypar\quad
...
(a) $\,\forall u_1+\i v_1,u_2+\i v_2\in V_{\Cbb}, \lambda\in\Fbb,$\par\vspace{-6pt}
\AlignEq{}{&T\left((u_1+\i v_1)+\lambda(u_2+\i v_2)\right)=T\left((u_1+\lambda u_2)+\i(v_1+\lambda v_2)\right)=T(u_1+\lambda u_2)+\i T(v_1+\lambda v_2)\\= \,& Tu_1+\i Tv_1+\lambda Tu_2+\i\lambda Tv_2=T(u_1+\i v_1)+\lambda T(u_2+\i v_2).}\par\quad
...
\BulletPoint \,\NoteFor{[3.52]}\quad $A\in\Fbb^{m,n},c\in\Fbb^{n,1}\Rightarrow Ac\in\Fbb^{m,1}$\par\quad
...
\BulletPoint \,{\Large C{\normalsize OLUMN}-R{\normalsize OW} F{\normalsize ACTORIZATION}}\qquad (CR Factorization)\vspace{3pt}\TextB{}
...
(a) Suppose $A=\begin{pmatrix}c_1 \\ \vdots \\ c_m \end{pmatrix}\begin{pmatrix} d_1 & \cdots & d_n\end{pmatrix}=\begin{pmatrix} c_1 d_1 & \cdots & c_1 d_n\\ \vdots & \ddots & \vdots \\ c_m d_1 & \cdots & c_m d_n \end{pmatrix}.\,\,\,\,(\,\,\,\exists\,c_j,d_k\in\Fbb,\forall j,k\,)$\par\,\par\quad\Habc
...
(a) Left distr:\quad Suppose $A\in\Fbb^{m,n}$ and $B,C\in\Fbb^{n,p}.$\par\quad\Habc
...
\Or Let $(e_1,\dots,e_M)$ be the standard basis of $\Fbb^M$, where $M=\max\{m,n,p\}.$\par\quad\Habc
Suppose $T\in\Lm(\Fbb^n,\Fbb^m)$ such that $Te_k=\sum\limits_{j=1}^m A_{j,k}e_j$ for each $k=1,\dots,n$. Then $\Mt(T)=A.$\par\quad\Habc
...
$\forall v\in V,v=(ST)Rv=S(TRv)\in\range S}\Rightarrow T$ is inje, $S$ is surj.\par\tinypar\quad
...
Suppose $ST=I$. $\MathRightBrace{l}{Tv=0\Rightarrow v=STv=0\\ v\in V\Rightarrow v=S(Tv)\in\range S}\Rightarrow T$ is inje, $S$ is surj.\par\tinypar\quad
...
$(RST)^{-1}=\left((RS)T\right)^{-1}=T^{-1}(RS)^{-1}=T^{-1}S^{-1}R^{-1}.$\par\tinypar\quad
...
\ProblemN{15}{
	\TextNL{Prove that every linear map from $\Fbb^{n,1}$ to $\Fbb^{m,1}$ is given by a mtx multi.}
...
Suppose $T=\lambda I$ for some $\lambda\in\Fbb.$ Then $T$ has the same mtx with resp to every basis of $V$.\par\quad
...
$}\Rightarrow\square$}\par\smallpar\smallpar\quad
\Or Conversely, denote $\Mt\left(T,(u_1,\dots,u_m)\right)$ by $A,$ where the basis $(u_1,\dots,u_m)$ is arbitrarily chosen.\par\quad
...
\ProblemN{18}{
	\TextNL{Show that $V$ and $\Lm(\Fbb, V)$ are iso vector spaces.}
}\par\quad
...
(ii) \dbsp Suppose $\deg f=\deg Tf$ for all $f\in\PoR{n}.$ Then suppose $\deg g=n+1,g\in\Po_{n+1}(\Rbb)$.\par\quad\Habc\Hii
\dbsp Assume that $\deg Tg<\deg g$ ( $\Rightarrow\deg Tg\leq n,Tg\in\Po_{n}(\Rbb)$ ).\par\quad\Habc\Hii
\dbsp Then by (a), \,$\exists\,f\in\Po_{n}(\Rbb),T(f)=(Tg).$ 又 $T$ is inje $\Rightarrow f=g$.\par\quad\Habc\Hii
\dbsp While $\deg f=\deg Tf=\deg Tg<\deg g$. Contradicts the assumption.\par\quad\Habc\Hii
\dbsp Hence $\deg Tp=\deg p$ for all $p\in\Po_{n+1}(\Rbb)$.\par\quad\Habc
...
$\forall v\in V,\,\,\exists\,a_i\in\Fbb, v=a_1 Tv_1+\dots+a_n Tv_n\Rightarrow T^{-1}v=a_1 v_1+\dots+a_n v_n\Rightarrow \range T^{-1}\subseteq\Spn(v_1,\dots,v_n).\hfill\square$\par\tinypar\quad
...
Thus we let $(Sv_1,\dots,Sv_m,x_1,\dots,x_n)$ be a basis of $W.}\begin{array}{l}$\,$\therefore$ $E$ is inv $\\ $ and $S=ET$.$\end{array}$\par\tinypar\large\quad
...
$}$\,$\therefore$ $E$ is inv and $S=TE$.\par\tinypar\large\quad
...
$}$\,$\therefore$ $E_1,E_2$ are inv and $S=E_2 TE_1.$\par\tinypar\large\quad
...
(a) Suppose $T(v_1,\dots,v_m)=0.$ Then $\forall\,(a_1,\dots,a_n)\in\Fbb^m$, $\varphi(a_1,\dots,a_m)=a_1 v_1+\dots+a_m v_m=0$\par\quad\Habc
$\Rightarrow (v_1,\dots,v_m)=0\Rightarrow T$ is inje.\par\quad
(b) Suppose $\psi\in\Lm(\Fbb^m,V).$ Let $(e_1,\dots,e_m)$ be the standard basis of $\Fbb^m$. Then $\forall\,(b_1,\dots,b_n)\in\Fbb^m$,\par\quad\Habc
$\left[T\left(\psi(e_1),\dots,\psi(e_m)\right)\right](b_1,\dots,b_m)=b_1\psi(e_1)+\dots+b_m\psi(e_m)=\psi(b_1 e_1+\dots+b_m e_m)=\psi(b_1,\dots,b_m)$.\par\quad\Habc
...
(b) Let $v=\lambda_1 v_+\dots+\lambda_m v_m\in A$. To show that $v\in B$, use induction on $m$ by $k$.\par\quad\Habc
(i) $k=1,v=\lambda_1 v_1\Rightarrow \lambda_1=1.$ 又 $v_1\in B.$ Hence $v\in B$.\par\quad\Habc\Hi
$k=2,v=\lambda_1 v_1+\lambda_2 v_2\Rightarrow\lambda_2=1-\lambda_1.$ 又 $v_1,v_2\in B.$ By problem (8), $v\in B$.\par\quad\Habc\Endi
(ii) $2\leq k\leq m,$ we assume that $v=\lambda_1 v_1+\dots+\lambda_k v_k\in A\subseteq B.$\,\, ($\forall\lambda_i$ such that $\sum\limits_{i=1}^k\lambda_i=1$)\par\quad\Habc\Hii
For $u=\mu_1 v_1+\dots+\mu_k v_k+\mu_{k+1} v_{k+1}\in A.\,\,\forall i=1,\dots,k,\,\,\exists\,\mu_i\neq 1,$ fix one such $i$ by $\iota$.\par\quad\Habc\Hii
Then $\sum\limits_{i=1}^{k+1}\mu_i-\mu_\iota=1-\mu_\iota\Rightarrow(\sum\limits_{i=1}^{k+1}\displaystyle\frac{\mu_i}{1-\mu_\iota})-\displaystyle\frac{\mu_\iota}{1-\mu_\iota}=1.$\par\quad\Habc\Hii
Let $w=\mathop{\underbrace{\displaystyle\frac{\mu_1}{1-\mu_\iota}v_1+\dots+\displaystyle\frac{\mu_{\iota-1}}{1-\mu_\iota}v_{\iota-1}+\displaystyle\frac{\mu_{\iota+1}}{1-\mu_\iota}v_{\iota+1}+\dots+\displaystyle\frac{\mu_{k+1}}{1-\mu_\iota}v_{k+1}}}\limits_{k\,terms}.$\par\quad\Habc\Hii
Let $\lambda_i=\displaystyle\frac{\mu_i}{1-\mu_\iota}$ for $i=1,\dots,\iota-1;$ \,$\lambda_j=\displaystyle\frac{\mu_{j+1}}{1-\mu_\iota}$ for $j=\iota,\dots,k$. Then,\par\quad\Habc\Hii
...
\par\tinypar\quad
(c) $\forall k\in\{1,\dots,m\},$ choose $\lambda_i\in\Fbb\,(\,\forall i\in\{1,\dots,m\}\backslash\{k\}\,)$ arbitrarily.\par\quad\Habc
Let $\lambda_k=1-\lambda_1-\dots-\lambda_{k-1}-\lambda_{k+1}-\dots-\lambda_m$\par\quad\Habc
$\Rightarrow \lambda_1 v_1+\dots+\lambda_m v_m$\par\quad\Habc
$=\lambda_1 v_1+\dots+\lambda_{k-1}v_{k-1}+(1-\lambda_1-\dots-\lambda_{k-1}-\lambda_{k+1}-\dots-\lambda_m)v_k+\lambda_{k+1}v_{k+1}+\dots+\lambda_m v_m$\par\quad\Habc
$=v_k+\lambda_1(v_1-v_k)+\dots+\lambda_{k-1}(v_{k-1}-v_k)+\lambda_{k+1}(v_{k+1}-v_k)+\dots+\lambda_m(v_m-v_k)$.\par\quad\Habc
...
\BulletPoint \,\hspace{1pt}\NoteFor{[3.88, 3.90, 3.91]}\TextB{}
...
(a) $\forall v\in V,\,\,\exists\,\,u\in U,w\in W$ such that $v=u+w\Rightarrow v+U=w+U$\par\quad\Habc
Then $V/U\subseteq\Spn(w_1+U,\dots,w_n+U)\Rightarrow V/U=\Spn(w_1+U,\dots,w_n+U)$.\par\quad\Habc
Hence $\dim V/U=\dim\Spn(w_1+U,\dots,w_n+U)\leq\dim W.$\par\quad
(b) Let $W\in\mathcal{S}_V U.$ In other words,\par\quad\Habc
...
(b) Suppose $\,\exists\,\,S\in\Lm(V/U,W),\,\,T=S\circ\pi.$\par\quad\Habc
...
\ProblemNnosolus{20}{
	...
	\vspace{3pt}\dbsp\Habc\hspace{0.1pt}\TextNL{{\large $\Gamma(S)=0=S\circ\pi\Longleftrightarrow\forall v\in V,S\left(\pi(v)\right)=0\Longleftrightarrow\forall v+U\in V/U,S(v+U)=0\Longleftrightarrow S=0.$}}
...
(a) Suppose $(v_1,\dots,v_m)$ is linely inde and $\vartheta\in\Spn\left(\varphi(v_1),\dots,\varphi(v_m)\right)$.\par\quad\Habc
Let $\vartheta=0=a_1\varphi(v_1)+\dots+a_m\varphi(v_m).$ Then $\vartheta(1)=0=a_1 v_1+\dots+a_m v_m\Rightarrow a_1=\dots=a_m=0.$\par\quad\Habc
\Or Because $\varphi$ is inje. Suppose $a_1\varphi(v_1)+\dots+a_m\varphi(v_m)=0=\varphi(a_1 v_1+\dots+a_m v_m).$\par\quad\Habc
Then $a_1 v_1+\dots+a_m v_m=0\Rightarrow a_1=\dots=a_m=0.$\par\quad\Habc
Thus $\left(\varphi(v_1),\dots,\varphi(v_m)\right)$ is linely inde.\par\quad
(b) Suppose $\left(\varphi(v_1),\dots,\varphi(v_m)\right)$ is linely inde and $v\in\Spn(v_1,\dots,v_m).$\par\quad\Habc
Let $v=0=a_1 v_1+\dots+a_m v_m.$ Then $\varphi(v)=a_1\varphi(v_1)+\dots+a_m\varphi(v_m)=0 \Rightarrow a_1=\dots=a_m=0.$\par\quad\Habc
Thus $v_1,\dots,v_m$ is linely inde. $\,\hfill\square$\par
\ProblemEnding\par

\ProblemN{1}{
	\TextN{Explain why each linear functional is surj or is the zero map.}
}$\MathRightBrace{r}{$ For any $\varphi \in V\apostrophe$ and $\varphi \neq 0,\,\exists\,v \in V, \,$ such that $\varphi(v) \neq 0.$ (a)$ \\ \dim\range\varphi=\dim\Fbb=1.$ (b)$} \Rightarrow \square$\par
\ProblemEnding\tinypar
...
Let $(u_1, \dots, u_m)$ be a basis of $U$, extend to $(u_1, \dots,u_m, u_{m+1}, \dots, u_{m+n})$ a basis of $V$.\par\quad Choose $k \in \{1,\dots,n\}$ arbitrarily. Define $\varphi \in V\apostrophe$ by $\varphi(u_{i})=\MathLeftBrace{l}{ 1,\quad\textup{if}\,\,\,i=m+k.\\ 0,\quad\textup{otherwise}.}$ \par\quad
...
\ProblemN{9}{
	\TextN{Suppose $(v_1,\dots,v_n)$ is a basis of \,$V$ and $(\varphi_1,\cdots,\varphi_n)$ is the corresponding dual basis of $V\apostrophe$.}
...
\ProblemN{12}{
	\TextNL{Show that the dual map of the identity optor on $V$ is the identity optor on $V\apostrophe$.}
...
\ProblemNnosolus{13}{
	...
	\Habc\TextNL{{\large\timesnr For any $(x,y,z)\in\Rbb^3$, $\left(T\apostrophe(\varphi_1)\right)(x,y,z)=4x+5y+6z, \left(T\apostrophe(\varphi_2)\right)(x,y,z)=7x+8y+9z$.}}
	(b) \TextNL{\large Write $T\apostrophe(\varphi_1)$ and $T\apostrophe(\varphi_2)$ as linear combinations of $\psi_1,\psi_2,\psi_3$.}
	\Habc\TextNL{{\large\timesnr$T\apostrophe(\varphi_1)=4\psi_1+5\psi_2+6\psi_3,\,\,T\apostrophe(\varphi_2)=7\psi_1+8\psi_2+9\psi_3.\hfill\square$}}
}\ProblemEnding\tinypar
...
\ProblemNnosolus{14}{
	...
	\Habc\TextNL{\large$\left(T\apostrophe(\varphi)\right)(p)=[x^2p(x)+p\apostrophe\apostrophe(x)]\apostrophe(4)=[2xp(x)+x^2p\apostrophe(x)+p\apostrophe\apostrophe\apostrophe(x)](4)=8p(4)+16p\apostrophe(4)+p\apostrophe\apostrophe\apostrophe(4)$.}
	...
	\Habc\TextNL{\large $\left(T\apostrophe(\varphi)\right)(x^3)=\int_0^1(x^5+6x)\d x=\int_0^1(\frac{1}{6}x^6+3x^2)\apostrophe\d x=\frac{6}{19}.\hfill\square$}
}\ProblemEnding\tinypar
...
(a) $\MathRightBrace{l}{U\subseteq U+W\\W\subseteq U+W}\Rightarrow\hspace{-6pt}\MathRightBrace{l}{(U+W)^0\subseteq U^0 \\ (U+W)^0\subseteq W^0}\Rightarrow(U+W)^0\subseteq U^0\cap W^0.$\par\tinypar\quad
...
(a) $\MathRightBrace{r}{U\cap W\subseteq U\\U\cap W\subseteq W}\Rightarrow\hspace{-6pt}\MathRightBrace{r}{(U\cap W)^0\supseteq U^0 \\ (U\cap W)^0\supseteq W^0}\Rightarrow(U\cap W)^0\supseteq U^0+W^0\supseteq U^0\cap W^0.$\par\tinypar\quad
...
(a) Suppose $\varphi\in\Spn(\varphi_{m+1},\dots,\varphi_n)$, then $\exists\,a_i\in\Fbb,\varphi=a_{m+1}\varphi_{m+1}+\dots+a_n\varphi_n.$\par\quad\quad\quad\,
For all $u\in U$, $\varphi(u)=0$. Thus $\varphi\in U^0,$ getting $\Spn(\varphi_{m+1},\dots,\varphi_n)\subseteq U^0.$\par\qquad
(b) Suppose $\varphi\in U^0$, then $\exists\,a_i\in\Fbb,\varphi=a_1\varphi_1+\dots+a_m\varphi_m+\dots+a_n\varphi_n.$\par\quad\quad\quad\,
...
(b) If $T(A)=0$ for some $A\in\Fbb^{n,m}$, then $A=0$. Thus $T$ is inje,\par\quad\Habc
...
(a) Suppose $T$ is inv, so is $T\apostrophe$. Because $T\apostrophe(\varphi_1),\dots,T\apostrophe(\varphi_m)$ is linely inde.\par\quad\Habc
...
(b) Suppose the rows of $A$ are linely inde, so are the cols of $B$.\par\quad\Habc
Then $\left(T\apostrophe(\varphi_1),\dots,T\apostrophe(\varphi_m)\right)$ is a basis of $\range T\apostrophe$, namely $V\apostrophe$. Thus $T\apostrophe$ is surj.\par\quad\Habc
...
(a) $\forall\varphi\in V\apostrophe,\,\,\forall v,w\in V,a\in\Fbb,\,\left(\Lambda(v+a w)\right)(\varphi)=\varphi(v+aw)=\varphi(v)+a\varphi(w)=(\Lambda v)(\varphi)+a(\Lambda w)(\varphi)$.\par\quad\Habc
Thus $\Lambda(v+aw)=\Lambda v+a\Lambda w$. Hence $\Lambda$ is linear.\par\quad
(b) $\left(T\apostrophe\apostrophe(\Lambda v)\right)(\varphi)=\left((\Lambda v)\circ(T\apostrophe)\right)(\varphi)=(\Lambda v)\left(T\apostrophe(\varphi)\right)=\left(T\apostrophe(\varphi)\right)(v)=(\varphi\circ T)(v)=\varphi(Tv)=\left(\Lambda(Tv)\right)(\varphi)$.\par\quad\Habc
...
(c) Suppose $\Lambda v=0$. Then $\forall\varphi\in V\apostrophe,(\Lambda v)(\varphi)=\varphi(v)=0\Rightarrow v=0$. Thus $\Lambda$ is inje.\par\quad\Habc
...
(a) Suppose $p$ has $m$ distinct zeros. By [4.14] and $\deg p=m$, let $p(z)=c(z-\lambda_1)\cdots(z-\lambda_m),\,\exists\,!\,c,\lambda_i\in\Cbb.$\par\tinypar\quad\Habc
For each $j\in\{1,\dots,m\}$, let $\displaystyle\frac{p(z)}{(z-\lambda_j)}=q_j\in\PoC{m-1},$ then $p(z)=(z-\lambda_j)q_j(z)$ and $q_j(\lambda_j)\neq 0$.\par\tinypar\quad\Habc
...
(b) To prove the implication on the other direction, we prove the contrapositive:\par\quad\Habc
Suppose $p$ has less than $m$ distinct roots.\par\quad\Habc
We must show that $p$ and its derivative $p\apostrophe$ have at least one zero in common.\par\quad\Habc	
Let $\lambda$ be a zero of $p,$ then write $p(z)=(z-\lambda)^n q(z),\,\exists\,!\,n\in\Nbp,q\in\PoC{m-n}.$\par\quad\Habc
...
Using the notation and proof of [4.17]. $\deg p=2M+m$ is odd $\Rightarrow m$ is odd. Hence $\lambda_1$ exists.$\hfill\square$\par\tinypar\quad
...
N{\small OTE:} Define $P:\rightarrow\PoF{}$ by $(Pq)(x)=p\left(q(x)\right)=(p\circ q)(x)\,(\,\neq p(x)q(x)\,).$ $P$ is not linear.\par\tinypar\quad
(a) By [4.8], $\forall f\in\PoF{},\,\exists\,!\,q,r\in\PoF{},f=(p)q+(r);\,\deg r<\deg p.$\par\qquad\,
Hence $\forall f\in\PoF{},\,\exists\,!\,pq\in U,r\in\PoF{\deg p-1},f=(pq)+(r); r\not\in U.$\par\quad\Habc
Thus $\PoF{}=U\oplus\PoF{\deg p-1}.$ Therefore $\PoF{}/U$ and $\PoF{\deg p-1}$ are iso.\par\tinypar\quad\Habc
\Or $\forall f\in\PoF{},\,\exists\,!\,q,r\in\PoF{},f=(p)q+(r);\,\deg r<\deg p.$\par\quad\Habc
Define $R:\PoF{}\rightarrow\PoF{\deg p-1}$ by $(Rf)(z)=r(z)$ for each $z\in\Fbb.$\par\quad\Habc
$\forall f,g\in\PoF{},\lambda\in\Fbb,R(f+\lambda g)(z)=R(f)+\lambda R(g).$\par\quad\Habc
...
\ProblemB{
	...
	\vspace{3pt}\hspace{-0.5pt}\Habc\TextB{Show that the linear map T is inje.}
...
(a) $T$ is linear because $\forall\,r_1,r_2\in\PoC{n-1},s_1,s_2\in\PoC{m-1},\lambda\in\Fbb,$\par\quad\Habc
$T\left((r_1,s_1)+\lambda(r_2,s_2)\right)=T(r_1+\lambda r_2,s_1+\lambda s_2)=(r_1+\lambda r_2)p+(s_1+\lambda s_2)q=T(r_1,s_1)+\lambda T(r_2,s_2).$\par\tinypar\quad\Habc
Suppose $T(r,s)=rp+sq=0.$ Notice that $p,q$ have no zeros in common.\par\quad\Habc
Then $r=s=0,$ for if not, write $\displaystyle\frac{q(z)}{r(z)}=\frac{p(z)}{s(z)},$ while for any zero $\lambda$ of $q$, $\displaystyle\frac{q(\lambda)}{r(z)}=0\neq\frac{p(\lambda)}{s(z)}.\hfill\square$\par\tinypar\quad
(b) $\dim(\PoC{n-1}\times\PoC{m-1})=\dim\PoC{n-1}+\dim\PoC{m-1}=n+m=\dim\PoC{m+n-1}.$\par\quad\Habc
又 $T$ is inje. Hence $\dim\range T=\dim(\PoC{n-1}\times\PoC{m-1})-\dim\null T=\dim\PoC{m+n-1}.$\par\quad\Habc
...
$\\ $
\BulletPoint \,\hspace{1pt}\NoteFor{[5.6]}{\large\par}\,\, More generally, suppose we do not know whether $V$ is finite-dim. Then $(a)\Longleftrightarrow(b).${\large\par}\,\,
...
(a) {\timessl\Large Suppose $\lambda\in\Fbb$ with $\lambda\neq 0$.}\par\qquad\,\,\,
...
{\small $\bullet$} {\timessl\Large 
Suppose $V$ is finite-dim and $T\in\Lm(V)$.}\par\,\,
{\timessl\Large Define $\mathcal{A}\in\Lm\left(\Lm(V)\right)$ by $\mathcal{A}(S) = TS$ for each $S\in\Lm(V)$.}\par\,\,
...
{\small $\bullet$ } {\timessl\Large 
Suppose $\Fbb=\Rbb,$ $T\in\Lm(V)$. }\par\,\,
(a) ({\normalsize O{\small R (9.11)}}) {\timessl\Large $\lambda\in\Rbb.$ Prove that $\lambda$ is an eigval of $T$ $\Longleftrightarrow$ $\lambda$ is an eigval of $T_{\Cbb}.$
}\par\,\,
(b) ({\normalsize O{\small R Problem (16)}}) {\timessl\Large $\lambda\in\Cbb.$ Prove that $\lambda$ is an eigval of $T_{\Cbb}$ $\Longleftrightarrow$ $\overline \lambda$ is an eigval of $T_{\Cbb}.$
}\par
...
Let $(v_1,\dots,v_n)$ be a basis of $V.$ Write $v=\sum\limits_{i=1}^n a_i v_i,u=\sum\limits_{i=1}^n b_i v_i,$ where $a_i,b_i\in\Rbb.$\par\qquad\,
...
{\small $\bullet$} {\timessl\Large
Suppose $V$ is finite-dim, $T\in\Lm(V)$, and $\lambda\in\Fbb.$
}\par\,\,
...
Suppose $T\in\Lm(\Rbb^2)$ is defined by $T(x, y) = (-3y, x)$. Find the eigvals of $T$.
}\par
...
Define $T\in\Lm(\Fbb^2)$ by $T(w, z) = (z, w)$. Find all eigvals and eigvecs of $T$.
}\par
...
\ProblemN{9} 
{\timessl\Large Define $T\in\Lm(\Fbb^3)$ by $T(z_1,z_2,z_3)=(2z_2, 0,5z_3)$.}\par\,\,\,
...
Define $T\in\Lm(\Fbb^n)$ by $T(x_1,x_2,x_3,\dots,x_n)=(x_1,2 x_2,3 x_3,\dots,n x_n)$
}\par\quad\,
...
Define $T\in\Lm(\PoR{4})$ by $(Tp)(x) = xp\apostrophe(x)$ for all $x\in\Rbb.$}\par\quad\,
...
Suppose $T\in\Lm(\Rbb^3)$ and $-4,5,\sqrt{7}$ are eigvals of $T$.}\par\quad\,
{\timessl\Large Prove that $\,\exists\,x\in\Rbb^3$ such that $Tx - 9x = (-4, 5, \sqrt{7})$.}\par
...
Suppose $V$ is finite-dim, $T\in\Lm(V),$ and $\lambda\in\Fbb.$}\par\quad\,
{\timessl\Large Prove that $\,\exists\,\alpha\in\Fbb,\left|\alpha-\lambda\right|<\frac{1}{1000}$ and $(T-\alpha I)$ is inv.}\par

Let $\alpha_k\in\Fbb$ be such that $\left|\alpha_k-\lambda\right|=\displaystyle\frac{1}{1000+k}$ for each $k=1,\dots,\dim V+1.$\par\tinypar\quad
...
{\small $\bullet$} {\timessl\Large 
Suppose $V$ is finite-dim, $T\in\Lm(V)$, and $\lambda\in\Fbb.$}\par\,\,
{\timessl\Large Prove that $\,\exists\,\delta > 0$ such that $(T-\alpha I)$ is inv for all $\alpha\in\Fbb$ such that $0 < \left|\alpha-\lambda\right| < \delta$.
}\par
...
Give an example of an optor on $\Rbb^4$ that has no $($ real $)$ eigvals.}\par
{\timesbfx S\footnotesize{OLUTION:}}\par\small\quad
Define $T\in\Lm(\Rbb^4)$ by $\Mt\left(T,(e_1,e_2,e_3,e_4)\right)=${\scriptsize$\begin{pmatrix}
 1 & 1 & 1 & 1\\
 -1 & 1 & -1 & -1\\
 3 & 8 & 11 & 5\\
 3 & -8 & -11 & 5
\end{pmatrix}.$} Where $(e_1,e_2,e_3,e_4)$ is the standard basis of $\Rbb^4.$\par\quad
Suppose $\lambda$ is an eigval of $T$ with an eigvec $(x,y,z,w).$\par\quad
Then $T(x,y,z,w)=\lambda(x,y,z,w)\Rightarrow${\scriptsize$\MathLeftBrace{l}{
(1-\lambda)x+y+z+w=0\\
-x+(1-\lambda)y-z-w=0\\
...
Suppose $\lambda$ is an eigval of $T$ with an eigvec $(x,y,z,w).$\par\quad
Then $T(x,y,z,w)=(\lambda x,\lambda y,\lambda z,\lambda w)=(-y,x,-w,z)${\scriptsize$\Rightarrow\MathLeftBrace{l}{
-y=\lambda x\\
x=\lambda y\\
-w=\lambda z\\
...
Suppose $(v_1,\dots,v_n)$ is a basis of \,$V$ and $T\in\Lm(V),\Mt\left(T,(v_1,\dots,v_n)\right)=A.$}\par\,\,
{\timessl\Large Prove that if $\lambda$ is an eigval of $T$, then $\left|\lambda\right|\leq n \max\left\{\left|A_{j,k}\right|:1\leq j, k\leq n\right\}.$
}\par
...
Show that the forward shift optor $T\in\Lm(\Fbb^\infty)$}\par\quad\,
...
\ProblemN{19} {\timessl\Large 
Suppose $n\in\Nbp$. Define $T\in\Lm(\Fbb^n)$ by}\par
...
Define the backward shift optor $S\in\Lm(\Fbb^\infty)$ by $S(z_1,z_2,z_3,\dots) = (z_2,z_3,\dots)$.}\par\quad\,
(a) {\timessl\Large Show that every element of $\Fbb$ is an eigval of $S$.}\par\quad\,
...
Suppose $A\in\Fbb^{n,n}.$ Define $T\in\Lm(\Fbb^n)$ by $Tx = Ax$,}\par\quad\,
{\timessl\Large where elements of $\Fbb^n$ are thought of as $n$-by-$1$ col vectors.}\par\quad\,
...
Because $(T-I)x=\left(A-\Mt(I)\right)x=\,${\small$\begin{pmatrix} \sum\limits_{r=1}^n A_{1,r}x_r-x_1\\ \vdots\\ \sum\limits_{r=1}^n A_{n,r}x_r-x_n\end{pmatrix}=\begin{pmatrix} y_1\\ \vdots\\ y_n\end{pmatrix}.$}\par\qquad\,
...
{\small $\bullet$} {\timessl\Large Suppose $A\in\Fbb^{n,n}.$ Define $T\in\Lm(\Fbb^n)$ by $Tx = xA$,}\par\quad\,
{\timessl\Large where elements of $\Fbb^n$ are thought of as $1$-by-$n$ row vectors.}\par\quad\,
...
\Or Prove that $(T-I)$ is not inv, so that we can conclude $\lambda=1$ is an eigval.\par\qquad\,
Because $(T-I)x=x\left(A-\Mt(I)\right)=\,${\small$=\begin{pmatrix} \sum\limits_{c=1}^n x_c A_{c,1}-x_1 &\cdots & \sum\limits_{c=1}^n x_c A_{c,n}-x_n\end{pmatrix}= \begin{pmatrix} y_1 & \cdots & y_n\end{pmatrix}.$}\par\qquad\,
...
Because $\forall v\in V,\exists\,!\,\lambda_v\in\Fbb,Tv=\lambda_v v.$\par\quad
...
If $T\neq\lambda I,\forall\lambda\in\Fbb,$ then $\,\exists$ a subsp $U$ of $V$ such that $\dim U=k,$ and $U$ is invar under $T$.\par\quad
...
{\timessl\Large Prove that $\left(\cos(\lambda_1 x),\dots,\cos(\lambda_n x)\right)$ is linely inde in $\Rbb^{\Rbb}.$
}\par
...
$\forall v+U,w+U\in V/U,\lambda\in\Fbb,(T/U)\left((v+U)+\lambda(w+U)\right)$\par\,\,
...
Because $\exists\,\lambda \in \Cbb$ such that $p(\lambda) = 0\Rightarrow\exists\,q \in\PoC{},p(z) = (z-\lambda)q(z),\forall z \in\Cbb.$\par\quad
...
{\timesbfx S\footnotesize{OLUTION:}} Suppose $U$ is a finite-dim nonzero invar subsp on $\Cbb.$ Then by [5.21], $T|_U$ has an eigval.$\hfill\square$
\par
\ProblemEnding\tinypar
...
E{\small XAMPLE}: On infinite-dim only. Let $V=\Fbb^\infty.${\small\par}\qquad
...
Thus $\,\exists\,c_j\in\Fbb,c_0 u + c_1 Tu +\dots + c_{m-1} T^{m-1} u + T^m u = 0$.\par\qquad
Define $q$ by
$q(z) = c_0 + c_1 z + \dots + c_{m-1} z^{m-1} + z^m$.\par\qquad Then $0=T^k(q(T)u)=q(T)(T^k u),\forall k\in\{1,\dots,m-1\}\subseteq\Nbb.$\par\qquad
...
Hence $\forall v\in V,\left((sq)(T)\right)(v) = s(T)(q(T)v) = 0.$\par\qquad
...
Hence contradicts the minimality of $\deg p$. Thus $p-q=0$ and we are done.$\hfill\square$\par
\ProblemEnding\tinypar
...
{\small $\bullet$} ({\normalsize 4E 5.B.27})\par\,\, {\timessl\Large Suppose $\Fbb=\Rbb,$ $V$ is finite-dim, and $T\in \Lm(V)$.}\par\,\,
{\timessl\Large Prove that the mini poly $p$ of $T_{\Cbb}$ equals the mini poly $q$ of $T$.}\par
{\timesbfx S\footnotesize{OLUTION:}}\par\,\,\,
$\MathRightBrace{l}{\forall u+\i 0\in V_{\Cbb},p(T_{\Cbb})(u)=p(T)u=0\Rightarrow\forall u\in V,p(T)u=0\Rightarrow p$ is a poly multi of $q.\\ q(T)=0\Rightarrow \forall u+\i v\in V_{\Cbb},q(T_{\Cbb})(u+\i v)=q(T)u+\i q(T)v=0\Rightarrow q$ is a poly multi of $p.}\Rightarrow\square$
\par
\ProblemEnding\par
...
{\timesbfx S\footnotesize{OLUTION:}}\par\,\,\,
$\forall \varphi\in V\apostrophe,p(T\apostrophe)\varphi=\varphi\circ \left(p(T)\right)=0\Rightarrow\forall\varphi\in V\apostrophe,p(T)\in\null\varphi\Rightarrow p(T)=0\\ \MathRightBrace{l}{\qquad\qquad\qquad\qquad\qquad\qquad\qquad\,\,\,\,\Rightarrow p(T)=0\Rightarrow p$ is a poly multi of $q.\\ q(T)=0\Rightarrow \forall \varphi\in V\apostrophe,\varphi\circ\left(q(T)\right)=q(T\apostrophe)\varphi=0\Rightarrow q$ is a poly multi of $p.}\Rightarrow\square$
\par
\ProblemEnding\par
...
By Problem (6), $\forall k\in\Nbb,\,\,\,T^{j+k}v=T^k(T^j v)\in U_0.$\par\qquad\,
...
\ProblemN{19} {\timessl\Large Suppose $V$ is finite-dim and $T\in \Lm(V)$. Let $\mathcal{E} =\left\{q(T): q \in \PoF{}\right\}$.
}\par\quad\,
{\timessl\Large
...
又 $\Spn(v,Tv,\dots,T^{m-1}v)$ is invar under $T.$\par\qquad Hence $\,\exists\,!\,k_j\in\Fbb,T^{n+1}=T(T^n)=k_0+k_1 z+\dots+k_{m-1}z^{m-1}.$\par\qquad
...
Define $q$ by $\displaystyle q(z)=z^m+\frac{a_1}{a_0} z^{m-1}+\dots+\frac{a_{m-1}}{a_0}z+\frac{1}{a_0}$ for all $z\in\Fbb.$\par\tinypar\quad
...
Hence $T^2+I=0\in\Lm(V),$ for if not, $\,\exists\,v\in V,(T^2+I)v\neq 0$ while $(T^2-I)\left((T^2+I)v\right)=0$. Contradicts.$\square$\par\tinypar\quad
...
Give an example of $T\in\Lm(\Rbb^2)$ such that $T^4=-I$.}\par
...
Define $T\in\Lm(\Fbb^n )$ by $T(x_1 , x_2 , x_3 , \dots , x_n ) = (x_1 , 2x_2 , 3x_3 , \dots , nx_n )$. Find
the mini poly.
}\par
...
{\small $\bullet$} {\timessl\Large 
Suppose $n\in\Nbp$. Define $T\in\Lm(\Fbb^n)$ by $T(x_1,\dots,x_n)=(x_1+\dots+x_n,\dots,x_1+\dots+x_n).$}\par\,\,
...
Suppose $T\in\Lm(\Rbb^2)$ is the optor of counterclockwise rotation by the angel $\theta$, where $x\in\Rbb^+$.}\par\,\,
...
Then $\Spn(v,Tv)=\Rbb^2.$\par\quad
Suppose the mini poly $p$ is defined by $p(z)=z^2+bz+c$ for all $z\in\Rbb.$\par\tinypar\quad
Because \includegraphics[scale=0.25]{./diagram.png} $\MathLeftBrace{l}{Tv=\displaystyle\frac{\left|\overset{\rightarrow}{v}\right|}{2L}(T^2 v+v)\Rightarrow T=\frac{\left|\overset{\rightarrow}{v}\right|}{2L}(T^2+I)\\\\ \displaystyle L=\left|\overset{\rightarrow}{v}\right|\cos\theta\Rightarrow\frac{\left|\overset{\rightarrow}{v}\right|}{2L}=\frac{1}{2\cos\theta}}$\par\tinypar\quad
...
\centerline{\timessl$\MathLeftBrace{l}{$
$z-a\qquad\qquad\qquad\qquad\qquad$ if $b=c=0$ and $a=d$,$\\ $
$z^2-(a+d)z+(ad-bc)\quad$ \,\,otherwise.
$}$}
\par

(a) Suppose the basis is $(v,w)$. Because $\MathLeftBrace{l}{\displaystyle Tv=av+bw\Rightarrow (T-aI)v=bw,$ then apply $(T-dI)$ to both sides. $\\ Tw=cv+dw\Rightarrow (T-dI)w=cv,$ then apply $(T-aI)$ to both sides. $}$\par\tinypar\qquad\,
...
(a) {\timessl\Large Give an example of $S, T\in\Lm(\Fbb^2)$ such that}\par\qquad
...
(a) %Let $V=\Fbb^{\infty},$ $S\in\Lm(\Fbb^\infty)$ is the forward shift optor, $T\in\Lm(\Fbb^\infty)$ is the backward shift optor.\par\qquad\,
...
Then $ST(x,y)=0,\,\,TS(x,y)=(0,x)$ for all $(x,y)\in\Fbb^2.$\par\qquad\,
...
Give an example of an optor on $\Rbb^2$
}\par\quad\,
{\timessl\Large that shows the result above does not hold if $\Cbb$ is replaced with $\Rbb$.
}\par
...
Suppose $V$ is finite-dim, $T\in \Lm(V)$, and $p$ is the mini poly of $T$. Suppose $\lambda \in\Fbb.$}\par\,\,
...
Suppose $V$ is finite-dim, $T\in \Lm(V)$, and $p$ is the mini poly of $T$. Suppose $\lambda \in\Fbb\backslash\{0\}$.}\par\,\,
...
{\timessl\Large Define $f:\Cbb\rightarrow\Rbb$ by $f(\lambda) = \dim \range(T-\lambda I)$.
...
Note that $\forall s\in\Nbp,\Mt(T^s)=\Mt(T)^s=A^s\in\Qbb^{n,n}$ and $T^s v_k=A^s_{1,k} v_1+\dots+A^s_{n,k}v_n$ for all $k\in\{1,\dots,n\}.$\par\tinypar\quad
...
Suppose $a_0 ,\dots, a_{n-1}\in\Fbb.$ Let $T$ be the optor on $\Fbb^n$ such that}\par\tinypar\,\,
...
{\timessl\small Hence a formula or an algorithm that could produce exact eigvals for each optor on each $\Fbb^n$ could then produce exact zeros for}{\footnotesize\par}\,\,
...
{\timessl\Large Suppose $\Fbb=\Rbb,$ $V$ is finite-dim, $T\in\Lm(V)$ and $b, c\in\Rbb$ with $b^2 < 4c$.}\par\,\,
...
(a) Suppose $\Fbb=\Cbb.$ Then by [5.21], we are done.\par\,\,
(b) {\timessl\Large Suppose $\Fbb=\Rbb,$ $V$ is finite-dim, and $\dim V=n\neq 0$ is an odd number.\par\qquad
...
If $p$ is a poly multi of $(x - \lambda)$ for some $\lambda\in\Rbb,$ then by [8.49] $\lambda$ is an eigval of $T$ and we are done.\par\qquad\,
Now suppose $b, c\in\Rbb$ such that $b^2 < 4c$ and $p$ is a poly multi of $x^2 + bx + c$ (see [4.17]).\par\qquad\,
Then $\,\exists\,q\in\PoR{}$ such that $p(x) = q(x)(x^2 + bx + c)$ for all $x\in\Rbb.$\par\qquad\,
Now $0 = p(T) = \left(q(T)\right)(T^2 + bT + cI),$ which means that $q(T)|_{\range(T^2+bT+cI)}=0.$\par\qquad\,
...
{\timessl\Large 
Suppose $\Fbb=\Rbb,T\in\Lm(V)$ has no eigvals.}\par\,\,
...
{\timessl\small
Exercise $($4E 5.C.6$)$ will give an improvement of this result when $\Fbb=\Cbb.$
}\par
...
Suppose $\Fbb=\Cbb,$ $V$ is finite-dim, and $T\in\Lm(V)$.
}\par\quad\,
...
(a) {\timessl\Large
Prove that if $\Fbb=\Rbb,$ then $\nexists$ a basis of \,$V$ with resp to which $T$ has an upper-trig mtx.
}\par\,\,
(b) {\timessl\Large
Prove that if $\Fbb=\Cbb$ and $A$ is an upper-trig mtx that equals the mtx of $T$}\par\quad\,\,\,
...
Suppose $B\in\Fbb^{n,n}$ with complex entries.}\par\,\,
{\timessl\Large Prove that $\,\exists$ inv $A\in\Fbb^{n,n}$ with complex entries}\par\,\,
...
Suppose $\Fbb=\Cbb$ and $V$ is finite-dim.}\par\,\,
...
\ProblemN{1}{
	\TextN{Give an example of two commuting optors $S, T\in\Fbb^4$ such that there is a subsp of $\Fbb^4$}
	\TextN{that is invar under $S$ but not under $T$ and there is a subsp of $\Fbb^4$}
...
\ProblemN{8}{
	...
	\TextN{and $D_x , D_y$ are the commuting partial differentiation optors on $\Po_3(\Rbb^2)$ from that example.}
	\TextN{Find a basis of $\Po_3(\Rbb^2)$ with resp to which $D_x$ and $D_y$ each have an upper-trig mtx.}
}\par
\ProblemEnding\par
