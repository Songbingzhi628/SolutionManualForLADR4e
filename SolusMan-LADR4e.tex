...
\def\ChapterEnding{\rightline{\timesbfx\Large{E{\small NDED}}}\par\tinypar}
...
\newcommand{\ProblemBor}[2]{
	\BulletPoint\,\hspace{0.5pt}\dbsp{O{\small R}} (#1) #2\vspace{-16pt}\par
	\Solution
}
...
%{\footnotesize $\bullet$} {\timessl\Large Complex scalar multi on $V_{\mathbb{C}}$ is defined by $\\ $\centerline{$(a+b \i)(u+\i v)=(au-bv)+\i(av+bu)$}}\par\quad\,\,
...
%(b) {\timessl\Large The set of continuous real-valued functions on the interval $[0,1]$ is a subsp of $\Rbfc^{[0,1]}$.}\par\qquad\,\,
...
%(c) {\timessl\Large The set of differentiable real-valued functions on {\timesbf R} is a subsp of $\,\mathbb{R}^{\mathbb{R}}$.}\par\qquad\,\,
...
%{\timessl\Large is a subsp of $\Rbfc^{(0,3)}$ if and only if $b=0$.}\par\qquad\,\,
...
%$}$\large$\Rightarrow U$ is a subsp of $\Rbfc^{(0,3)}.$\par\qquad
%Suppose $U$ is a subsp of $\Rbfc^{(0,3)}.$ Suppose $f=0\Rightarrow f\in U$. Then $f\apostrophe(2)=0=b.\hfill\square$\par\tinypar\,\,\,
%(e) {\timessl\Large The set of all sequences with limit 0, of complex numbers, is a subsp of $\Cbfc^\infty$.}\par\qquad\,\,
%Denote the set by $A$.\normalsize\par\qquad
%$\MathRightBrace{l}{$
%(一) $(0,0,\dots)\in A\\ $
...
%	\TextN{such that $\int_0^1 f=b$ is a subsp of $\Rbfc^{[0,1]}$ $\Longleftrightarrow$ $b=0$}
...
%
%\ProblemN{5} {\timessl\Large Is  {\timesbf R}$^2$ a subsp of the complex vecsp $\Cbfc^2$?}
%\,\,\,{\timesbf A\footnotesize{NSWER:}}\,\,\,No. $\Rbfc^2$ is not closed on scalar multi on {\timesbf C}.\par
%\ProblemEnding\tinypar
%
%\ProblemN{6}
%(a) {\timessl\Large Is $\{(a,b,c)\in\Rbfc^3:a^3=b^3 \}$ a subsp of $\Rbfc^3$?
%}\,\,
%{\timesbf A\footnotesize{NSWER:}}\,\,\,True. ( note that $a^3=b^3\Leftrightarrow a=b$ ).\par\,\,\,\,
%(b) {\timessl\Large Is $\{(a,b,c)\in\Cbfc^3:a^3=b^3 \}$ a subsp of $\Cbfc^3$?
%}\,\,
...
\ProblemN{7}{
	...
	\TextN{taking add invs and under add, then $U$ is a subsp of $\Rbfc^2.$}
...
\ProblemN{8}{
	...
	\TextN{but $U$ is not a subsp of $\Rbfc^2$.}
...
\ProblemN{9}{
	...
	\TextN{Is the set of periodic functions from {\timesbf R} to {\timesbf R} a subsp of \,$\mathbb{R}^{\mathbb{R}}$ ? Explain.}
...
\ProblemNL{11}{\TextNL{Prove that the intersection of every collection of subsps of $V$ is a subsp of $V$.
}}\par\quad
Suppose $\{U_\alpha\}_{\alpha\in\Gamma}$ is a collection of subsps of $V$; here $\Gamma$ is an arbitrary index set.\par\quad
We need to prove that $\bigcap_{\alpha\in\Gamma}U_\alpha,$ which equals the set of vectors that are in $U_\alpha$ for each $\alpha\in\Gamma$,\par\qquad\qquad\qquad\qquad\qquad\,\,\, is a subsp of $V$.\par\tinypar\,
$\begin{array}{l}$
...
(a) \dbsp Suppose that one of the subsps contains the other two.\par\quad\Habc
\dbsp Then $\mathcal{U}=U_1,U_2$ or $U_3$ is a subsp of $V.$\par\quad
(b) Suppose that $U_1\cup U_2\cup U_3$ is a subsp of $V$.\par\quad\Habc
By distinct we notice that $A\cup B\cup C=(A\cup B)\cup(B\cup C)=(A\cup C)\cup(B\cup C)=(A\cup B)\cup(A\cup C).$\par\quad\Habc
Also note that, if $U\cup W=V$ is a vecsp, then in general $U$ and $W$ are not subsps of $V.$\par\quad\Habc
Hence this literal trick is invalid.\par\quad\Habc
(I) \dbsp ss\par\quad\Habc%\EndI
%(II)\par\quad\Habc
...
\par\quad\Habc\EndI
(II) \dbsp Assume that no $U_j$ is contained in the union of the other two,\par\quad\Habc\HII\hspace{46pt} and no $U_j$ contains the union of the other two.\par\quad\Habc\HII
\dbsp Say $U_1\not\subseteq U_2\cup U_3$ and $U_1\not\supseteq U_2\cup U_3.$
\par\quad\Habc\HII
$\exists\,u\in U_1\wedge u\not\in U_2\cup U_3;\,\,\,v\in U_2\cup U_3\wedge v\not\in U_1.$ Let $W=\{v+\lambda u:\lambda\in\Fbfc\}\subseteq\mathcal{U}.$\par\quad\Habc\HII
Note that $W\cap U_1=\varnothing,$ for if $v+\lambda u\in U_1$ then $v+\lambda u-\lambda u=v\in U_1$ while $v\not\in U_1.$\par\quad\Habc\HII
又 $W\subseteq U_1\cup U_2\cup U_3.$ Thus $W\subseteq U_2\cup U_3.$\par\quad\Habc\HII
$\forall v+\lambda u\in W,\,\exists\,i\in\{2,3\},v+\lambda u\in U_i.$\par\quad\Habc\HII
Because $U_2,U_3$ are subsps and hence have at least one element.\par\quad\Habc\HII
If $U_2=U_3,$ then $\mathcal{U}=U_1\cup U_2$ and by Problem (12) we are done.\par\quad\Habc\HII
Otherwise, $\,\exists\,\lambda,\mu\in\Fbf\,$with $\lambda\neq\mu$ such that $v+\lambda u,v+\mu u\in U_i$ for some $i\in\{2,3\}.$\par\quad\Habc\HII
...
\ProblemNL{21}{
	...
	\TextNL{Find a subsp $W$ of $\Fbfc^5$ such that $\Fbfc^5=U\oplus W$.}
}\par\quad
...
\ProblemNL{22}{
	\TextNL{Suppose $U=\{(x,y,x+y,x-y,2x)\in\Fbfc^5:x,y\in\Fbfc\}$.}
	\TextNL{Find three subsps $W_1,W_2,W_3$ of $\Fbfc^5$, none of which equals $\{0\}$,}
...
{\small
$\MathRightMid{l}{$Because if $a_0,\dots,a_m\in\Rbf$ are such that $a_0+a_1 x+\dots+a_m x^m=0,\,\,\forall x\in[0,1],\\ $ then the poly has infinitely many roots and hence $a_0=\dots=a_m=0$. $}$\quad Similar to [2.16], $U$ is infinite-dim.}\par\quad
...
$“\complement_V U \cap \{0\}”$ is supposed to be a subsp $“W”$ such that $V=U\oplus W$.\par\quad
...
\ProblemBor{9.4}{
	\TextB{ Suppose $V$ is a real vector space.}
...
\ProblemN{2}{
	\TextN{Show that the subsps of $\Rbfc^2$ are precisely $\{0\}$, all lines in $\Rbfc^2$}
	\TextN{containing the origin, and $\Rbfc^2$.}
}Suppose $U$ is a subsp of $\Rbfc^2$. Let $\dim U=n.$\par\quad
...
\ProblemN{3}{
	\TextN{Show that the subsps of $\Rbfc^3$ are precisely $\{0\}$, all lines in $\Rbfc^3$}
	\TextN{containing the origin, all planes in $\Rbfc^3$ containing the origin, and $\Rbfc^3$.}
}Suppose $U$ is a subsp of $\Rbfc^3$. Let $\dim U=n.$\par\quad
If $n=0$, then $U=\{0\}.$\par\quad
...
(ii) \dbsp{\Large$k\geq 2.$ {\large Suppose for} $p_{m-k}(x),\,\,\exists\,\,!\,\,a_i\in\Fbfc,$ $x^{m-k}=p_{m-k}(x)+a_m x^m+\dots+a_{m-k+1}x^{m-k+1}.$\par}\quad\Hii
...
{\Large {\large Thus for each} $x^i,\,\,\exists\,\,!\,\,b_i\in\Fbfc,x^i=b_m p_m(x)+\dots+b_{m-i}p_{m-i}(x)$\par}\quad\Hii
...
\ProblemN{17}{
	\TextNL{Suppose $V_1,V_2,V_3$ are subsps of a finite-dim vector space, then}
...
\ProblemN{7}{
	...
	\TextN{Show that $U=\{\,T\in\Lm(V, W):T$ is not inje $\}$ is not a subsp of $\Lm(V, W)$.}
}\par\quad
...
\ProblemN{8}{
	...
	\TextN{Show that $U=\{\,T\in\Lm(V, W):T$ is not surj $\}$ is not a subsp of $\Lm(V, W)$.}
}\par\quad
...
(b) Suppose $\underbrace{\dim U}_m\geq\underbrace{\dim V}_{m\,+\,n}-\underbrace{\dim W}_{p}\,(\,\Rightarrow\dim W=p\geq n=\dim V-\dim U\,)$.\par\quad\Habc
...
\ProblemB{
	...
	\TextB{Prove that $\mathcal{K}_U=\{\,v\in V: Tv\in U\,\}$ is a subsp of \,$V$}
	\TextB{and $\dim\mathcal{K}_U=\dim\null T +\dim(U\cap\range T)$.}
}\par\quad
For any $u,w\in\mathcal{K}_U$ and $\lambda\in\Fbf,T(u+\lambda w)=Tu+\lambda Tw\in U\Rightarrow \mathcal{K}_U$ is a subsp of $V$.\par\quad
...
\Example\,\,\, Let $(v_1,\dots,v_5)$ be a basis of $\Fbfc^5.$ Define $S,T\in\Lm(\Fraw^5)$ by:\par
\vspace{-5pt}
...
$\end{array}\right\}\right.\Rightarrow V=\null\varphi\oplus\{au:a\in\Fbf\}.\hfill\square$\par\tinypar\large\quad
...
%}\par\quad
%Let $(v_1,\dots,v_5)$ be a basis of $\Rbfc^5$, $(w_1,w_2)$ be a basis of $\Rbfc^2$. Define $T,S\in\Lm(V,W)$ by\par\tinypar\quad
%$\left.\begin{array}{l}$
%$Tv_1=w_1,\quad Tv_2=w_2,\quad\,\, Tv_3=Tv_4=Tv_5=0\\ $
%$Sv_1=w_1,\quad Sv_2=2w_2,\quad Sv_3=Sv_4=Sv_5=0$
%$\end{array}\right\}\Rightarrow\null T=\null S.$\par\tinypar\quad
...
\BulletPoint \,{\Large C{\normalsize OLUMN}-R{\normalsize OW} F{\normalsize ACTORIZATION}}\qquad (CR Factorization)\vspace{3pt}\par\,\,
{\Large\timessl Suppose $A\in\Fbfc^{m,n},A\neq 0.$ Let $S_c=\Spn(A_{\cdot,1},\dots,A_{\cdot,n})\subseteq\Fbfc^{m,1},\dim S_c=c.$}\vspace{5pt}\TextB{}
{\Large\timessl \rightline{And $S_r=\Spn(A_{1,\cdot},\dots,A_{n,\cdot})\subseteq\Fbfc^{1,n},\dim S_r=r.$\qquad\qquad\qquad\quad}}\vspace{5pt}\TextB{}
{\Large\timessl Prove that $A=CR.\,\,\,\exists\,\,C\in\Fbfc^{m,c},R\in\Fbfc^{c,n}.$}\vspace{2pt}\par
...
\ProblemB{
	...
	(a) \TextB{$T$ is inje.}
	(b) \TextB{The cols of $\Mt(T)$ are linely inde in $\Fbfc^{n,1}$.}
	(c) \TextB{The cols of $\Mt(T)$ span $\Fbfc^{n,1}$.}
...
\ProblemB{
	\TextB{Suppose $A$ is an m-by-n mtx with $A\neq 0$.}
...
\ProblemN{1}{
	...
	\TextN{the mtx of $T$ has at least $\dim\range T$ nonzero entries.}
}\par\quad
...
\ProblemN{13}{
	\TextNL{Prove that the distr property holds for mtx add and mtx multi.}
	\TextNL{In other words, suppose $A, B, C, D, E$ and $F$ are mtcs}
...
(a) Left distr:\quad Suppose $A\in\Fbfc^{m,n}$ and $B,C\in\Fbfc^{n,p}.$\par\quad\Habc
...
\ProblemN{14}{
	\TextNL{Prove that mtx multi is associ. In other words,}
...
Hence we conclude that $(AB)C=A(BC).$\par\tinypar\quad
...
\ProblemN{15}{
	\TextNL{Suppose $A$ is an n-by-n mtx and $1\leq j, k\leq n$.}
...
Suppose $ST$ is inv. Let $R=(ST)^{-1}\Rightarrow R(ST)=(ST)R=I$.\par\tinypar\quad
$\left.\begin{array}{l}$
...
\Or {\normalsize By Problem (9), $V$ is finite-dim and $ST=I$ is inv $\Rightarrow$ $S,T$ are inv.}\par\tinypar\quad
$S((TS)v)=ST(Sv)=Sv\Rightarrow (TS)v=v\Rightarrow S$ is inv.\par\quad
\Or {\normalsize$ST=I\Rightarrow S=T^{-1}\Rightarrow S^{-1}=T.$ 又 $S=S\Rightarrow TS=S^{-1}S=I$.}\par\tinypar\quad
...
Let $V=\Rbfc^\infty,S(a_1,a_2,\dots)=(a_2,\dots),T(a_1,\dots)=(0,a_1,\dots),U=I.$ Then $STU=I$ but $T^{-1}$ is not inv.
\par
\ProblemEnding\tinypar
...
$(RST)^{-1}=((RS)T)^{-1}=T^{-1}(RS)^{-1}=T^{-1}S^{-1}R^{-1}.$\par\tinypar\quad
...
\ProblemN{15}{
	\TextNL{Prove that every linear map from $\Fbfc^{n,1}$ to $\Fbfc^{m,1}$ is given by a mtx multi.}
	\TextNL{In other words, prove that if $T\in\Lm(\Fbfc^{n,1},\Fbfc^{m,1})$, then $\,\exists\,A\in\Fbfc^{m,n},Tx = Ax,\,\forall x\in\Fbfc^{n,1}$.}
}\par\quad
...
\ProblemBor{10.A.2}{
	\TextNL{Suppose $A,B\in\Fbfc^{n,n}$. Prove that $AB = I\Longleftrightarrow BA=I$.}
...
\BulletPoint \,\NoteFor{[3.60]} Suppose $(v_1,\dots,v_n)$ is a basis of $V$ and $(w_1,\dots,w_m)$ is a basis of $W$.\TextB{}
Define $E_{i,j}\in\Lm(V,W)$ by $E_{i,j}(v_x)=\delta_{ix} w_j;\quad\delta_{ix}=\left\{\begin{array}{l}0,\quad i\neq x\\1,\quad i=x\end{array}\right.\qquad$ \Corollary\,\,\, $E_{l,k}E_{i,j}=\delta_{jl}E_{i,k}.$\TextB{}
Denote $\Mt(E_{i,j})$ by $\mathcal{E}^{(j,i)}.$ $(\mathcal{E}^{(j,i)})_{l,k}=\left\{\begin{array}{l}0,\quad i\neq k\,\,\,\textup{or}\quad j\neq l\\1,\quad i=k\,\,\,\textup{and}\,\,j=l\end{array}\right.$\TextB{\tinypar}
Because $\Lm(V,W)$ and $\Fbfc^{m,n}$ are iso. And $T=\Mt^{-1}\Mt(T),E_{i,j}=\Mt^{-1}\mathcal{E}^{(j,i)}$.\TextB{}
...
\end{pmatrix}$}.\tinypar\TextB{}
$\therefore\,\Lm(V,W)=\Spn${\small$\underbrace{\begin{pmatrix} \underset{,}{E_{1,1}}, & \cdots & ,\underset{,}{E_{n,1}},\\ \vdots & \ddots & \vdots\\ \overset{,}{E_{1,m}}, & \cdots & ,\overset{,}{E_{n,m}}\end{pmatrix}}_{B}$}$;\quad\Fbfc^{m,n}=\Spn${\small$\underbrace{\begin{pmatrix} \underset{,}{\mathcal{E}^{(1,1)}}, & \cdots & ,\underset{,}{\mathcal{E}^{(1,n)}},\\ \vdots & \ddots & \vdots\\ \overset{,}{\mathcal{E}^{(m,1)}}, & \cdots & ,\overset{,}{\mathcal{E}^{(m,n)}}\end{pmatrix}}_{B_m}$}.\TextB{}
...
\ProblemBc{
	...
	\TextB{Define $A\in \Lm(\Lm(V))$ by $\mathcal{A}(T) = ST$ for $T\in\Lm(V)$.}
...
(b) For all $R\in\Lm(V),$ $\range R\subseteq\range S \Longleftrightarrow\exists\,T\in\Lm(V),R=ST.$ ( By Problem (25) in 3.B )\par\quad\Habc
Thus $\range\mathcal{A}=\Lm(V,\range S).$\hfill$\square$\par\tinypar\quad
...
Thus $S=E_{1,1}+\dots+E_{m,m};\quad\Mt(S,(v_1,\dots,v_n),(w_1,\dots,w_n))\,=\,${\footnotesize$\begin{pmatrix} 1 & 0 & \cdots & 0 & 0 & \cdots & 0\\ 0 & 1 & \cdots & 0 & 0 & \cdots & 0\\ \vdots & \vdots & \ddots & \vdots & \vdots & \ddots & \vdots\\ 0 & 0 & \cdots & 1 & 0 & \cdots & 0\\ 0 & 0 & \cdots & 0 & 0 & \cdots & 0\\ \vdots & \vdots & \ddots & \vdots & \vdots & \ddots & \vdots\\ 0 & 0 & \cdots & 0 & 0 & \cdots & 0\end{pmatrix}.$}\par\quad\,
Define $R_{i,j}\in\Lm(V)$ by $R_{i,j}(w_x)=\delta_{ix} v_i$.\par\quad\,
Let $E_{j,k}R_{i,j}=Q_{i,k},\quad R_{j,k}E_{i,j}=G_{i,k}$\par\quad\,
Because $\forall T\in\Lm(V),\,\,\,\,\,\exists\,!\,A_{i,j}\in\Fbf(\,\forall i,j=1,\dots,n\,),\,\,\,\,T\,=\,\,${\footnotesize$\begin{pmatrix}
...
\Comment\,\,\, Define $\mathcal{B}\in\Lm(\Lm(V))$ by $\mathcal{B}(T)=TS$ for $T\in\Lm(V)$.\par\,\,\,
...
\ProblemBor{10.A.1}{
	...
	\TextB{Prove that $\Mt(T, (v_1,\dots,v_n))$ is inv $\Longleftrightarrow T$ is inv.}
}\par\quad
...
\ProblemBor{10.A.4}{
	...
	\TextB{Prove that $\Mt(T, (\alpha_1,\dots,\alpha_n))=\Mt(I,(\beta_1 ,\dots,\beta_n),(\alpha_1,\dots,\alpha_n))$.}
}\par\quad
For ease of notation, let $\Mt(T,\alpha\rightarrow\beta)=\Mt(T,(\alpha_1,\dots,\alpha_n),(\beta_1,\dots,\beta_n)),\,\,\,\Mt(T,\alpha\rightarrow\alpha)=\Mt(T,(\alpha_1,\dots,\alpha_n)).$\par\quad
...
\Or Note that $\Mt(T,\alpha\rightarrow\beta)$ is the identity mtx.\par\tinypar\quad
...
\ProblemN{16}{
	...
	\TextNL{Prove that $\exists\,\lambda\in\Fbfc,S=\lambda I\Longleftrightarrow$ $ST=TS$ for every $T\in\Lm(V)$.}
...
\ProblemBor{10.A.3}{
	\TextB{Suppose $V$ is finite-dim and $T\in\Lm(V)$.}
	\TextB{Prove that $T$ has the same mtx with resp to every basis of $V$}
	\TextB{if and only if $T$ is a scalar multi of the identity optor.}
}[ Compare with the first solution of Problem (16) in (3.A) ]\par\quad
Suppose $T=\lambda I$ for some $\lambda\in\Fbfc.$ Then $T$ has the same mtx with resp to every basis of $V$.\par\quad
...
to show that for any two nonzero distinct vectors $v,w\in V,\lambda_v=\lambda_w$. Thus $T=\lambda I,\,\exists\,\lambda\in\Fbfc.$\par\qquad{\normalsize
$\left.\begin{array}{l}$
...
$\end{array}\right\}\Rightarrow\square$}\par\smallpar\smallpar\quad
{\timesbfx\Or} Conversely, denote $\Mt(T,(u_1,\dots,u_m))$ by $A,$ where the basis $(u_1,\dots,u_m)$ is arbitrarily chosen.\par\quad
...
\ProblemN{18}{
	\TextNL{Show that $V$ and $\Lm(\Fraw, V)$ are iso vector spaces.}
}\par\quad
Define $\Psi\in\Lm${\Large(}$V,\Lm(\Fraw, V)${\Large)} by $\Psi(v)=\Psi_v;$ where $\Psi_v\in\Lm(\Fraw, V)$ and $\Psi_v(\lambda)=\lambda v.$\par\quad
(a) $\Psi(v)=\Psi_v=0\Rightarrow \forall \lambda\in\Fbfc,\Psi_v(\lambda)=\lambda v=0\Rightarrow v=0.$ Hence $\Psi$ is inje.\par\quad
(b) $\forall T\in\Lm(\Fraw,V),$ let $v= T(1)\Rightarrow T(\lambda)=\lambda v=\Psi_v(\lambda),\forall\lambda\in\Fbfc\Rightarrow T=\Psi( T(1)).$ Hence $\Psi$ is surj.\hfill$\square$\par\quad
\Or Define $\Phi\in\Lm(\Lm(\Fraw,V), V)$ by $\Phi(T)=T(1).$\par\quad
(a) Suppose $\Phi(T)=0=T(1)=\lambda T(1)=T(\lambda),\forall\lambda\in\Fbfc\Rightarrow T=0.$ Thus $\Phi$ is inje.\par\quad
(b) For any $v\in V,$ define $T\in\Lm(\Fraw,V)$ by $T(\lambda)=\lambda v.$ Then $\Phi(T)=T(1)=v.$ Thus $\Phi$ is surj.$\hfill\square$\par\quad
\Comment\,\,\, $\Phi=\Psi^{-1}.$\par
\ProblemEnding\par

\ProblemB{
	\TextB{Suppose $q\in\PoR{}$. Prove that $\,\exists\,p\in\PoR{},q(x) = (x^2 + x)p\apostrophe\apostrophe(x) + 2xp\apostrophe(x) + p(3),\forall x\in\Rbfc.$}
}\par\quad
...
As can be easily checked, $T_n$ is an optor.\par\quad
...
(b) Using mathematical induction.\par\quad\Habc
(i) \dbsp $\deg p=0\Rightarrow p=C\Rightarrow\deg Tp=\deg p=0.$\par\quad\Habc\Hi
\dbsp $\deg p=-\infty\Rightarrow p=0\Rightarrow\deg Tp=\deg p=-\infty.$\par\quad\Habc\Endi
(ii) \dbsp Suppose $\deg f=\deg Tf$ for all $f\in\PoR{n}.$ Then suppose $\deg g=n+1,g\in\Po_{n+1}(\Rraw)$.\par\quad\Habc\Hii
\dbsp Assume that $\deg Tg<\deg g$ ( $\Rightarrow\deg Tg\leq n,Tg\in\Po_{n}(\Rraw)$ ).\par\quad\Habc\Hii
\dbsp Then by (a), \,$\exists\,f\in\Po_{n}(\Rraw),T(f)=(Tg).$ 又 $T$ is inje $\Rightarrow f=g$.\par\quad\Habc\Hii
\dbsp While $\deg f=\deg Tf=\deg Tg<\deg g$. Contradicts the assumption.\par\quad\Habc\Hii
\dbsp Hence $\deg Tp=\deg p$ for all $p\in\Po_{n+1}(\Rraw)$.\par\quad\Habc
...
$\forall v\in V,\,\,\exists\,a_i\in\Fbfc, v=a_1 Tv_1+\dots+a_n Tv_n\Rightarrow T^{-1}v=a_1 v_1+\dots+a_n v_n\Rightarrow \range T^{-1}\subseteq\Spn(v_1,\dots,v_n).\hfill\square$\par\tinypar\quad
...
\ProblemN{2}{
	...
	\TextN{Prove that the set of non-inv optors on $V$ is not a subsp of $\Lm(V)$.}
...
(b) Suppose $(u_1,\dots,u_m)$ be a basis of $U$ and $S$ is inje $\Rightarrow(Su_1,\dots,Su_m)$ is linely inde in $V$.\par\quad\Habc
Extend these to bases of $V$ as $(u_1,\dots,u_m,v_1,\dots,v_n)$ and $(Su_1,\dots,Su_m,w_1,\dots,w_n)$.\par\quad\Habc
...
Thus we let $(Sv_1,\dots,Sv_m,x_1,\dots,x_n)$ be a basis of $W.\end{array}\right|\begin{array}{l}$\,$\therefore$ $E$ is inv $\\ $ and $S=ET$.$\end{array}$\par\tinypar\large\quad
...
$\end{array}\right|$\,$\therefore$ $E$ is inv and $S=TE$.\par\tinypar\large\quad
...
$\end{array}\right|$\,$\therefore$ $E_1,E_2$ are inv and $S=E_2 TE_1.$\par\tinypar\large\quad
...
Thus $T|_{\mathcal{K}}$ maps a basis of $\mathcal{K}$ to a basis of $\range T=W.$ Denote $\mathcal{K}$ by $U$. $\hfill\square$\par\tinypar\quad
...
\ProblemB{
	...
	\TextB{Let $\mathcal{E} = \{\,T\in\Lm(V, W): U\subseteq\null T\,\}$.}
...
(b) Define $\Phi$ as in the hint.\par\quad\Habc
Because $T\in\null\Phi\Longleftrightarrow\Phi(T)=0\Longleftrightarrow\forall u\in U,Tu=0\Longleftrightarrow T\in\mathcal{E}.$\par\quad\Habc
Hence $\null\Phi=\mathcal{E}$.\par\quad\Habc
Because $S\in\Lm(U,W)\Rightarrow\exists\,T\in\Lm(V,W),\Phi(T)=S,$ by (3.B.11) $\Rightarrow S\in\range T.$\par\quad\Habc
Hence $\range\Phi=\Lm(U,W)$.\par\quad\Habc
Thus $\dim\null\Phi=\dim\mathcal{E}=\dim\Lm(V,W)-\dim\range\Phi=(\dim V-\dim U)\dim W.\hfill\square$%\par\,\par
\par\tinypar\quad\Habc
\Or Extend $(u_1,\dots,u_m)$ a basis of $U$ to $(u_1,\dots,u_m,v_1,\dots,v_n)$ a basis of $V$. Let $p=\dim W$.\par\quad\Habc
( See N{\small OTE} F{\small OR} [3.60])\par\quad\Habc
$\forall \,T\in\mathcal{E},k\in\{1,\dots,m\},TE_{k,k}=0\Rightarrow\underbrace{\Spn\begin{Bmatrix} \underset{,}{E_{1,1}}, & \cdots & ,\underset{,}{E_{m,1}},\\ \vdots & \ddots & \,\vdots\\ \overset{,}{E_{1,p}}, & \cdots & ,\overset{,}{E_{m,p}}\end{Bmatrix}}_{\textup{Denote\,it\,by\,R}}\,\cap\,\mathcal{E}=\{0\}$.\par\quad\Habc
又 $W=\Spn${\small$\begin{Bmatrix} \underset{,}{E_{m+1,1}}, & \cdots & ,\underset{,}{E_{n,1}},\\ \vdots & \ddots & \,\vdots\\ \overset{,}{E_{m+1,p}}, & \cdots & ,\overset{,}{E_{n,p}}\end{Bmatrix}$}$\,\subseteq\mathcal{E}$. Where $\Lm(V,W)=R\oplus W\Rightarrow\Lm(V,W)=R+\mathcal{E}.$\par\tinypar\quad\Habc
...
$\end{array}\right\}\Rightarrow S=T^{-1}.\hfill\square$\par\vspace{5pt}
\ProblemEnding\par
...
$\end{array}\right\}\Rightarrow\psi=\varphi^{-1}.\hfill\square$\par\vspace{5pt}
\ProblemEnding\par
...
\ProblemN{6}{
	\TextN{For $m\in\Nbp,$ define $V^m$ by $\mathop{\underbrace{V\times\cdots\times V}}\limits_{m\,\,times}$. Prove that $V^m$ and $\Lm(\Fraw^m,V)$ are iso.}
}\par\quad
...
(a) Suppose $T(v_1,\dots,v_m)=0.$ Then $\forall\,(a_1,\dots,a_n)\in\Fbfc^m$, $\varphi(a_1,\dots,a_m)=a_1 v_1+\dots+a_m v_m=0$\par\quad\Habc
$\Rightarrow (v_1,\dots,v_m)=0\Rightarrow T$ is inje.\par\quad
(b) Suppose $\psi\in\Lm(\Fraw^m,V).$ Let $(e_1,\dots,e_m)$ be the standard basis of $\Fbfc^m$. Then $\forall\,(b_1,\dots,b_n)\in\Fbfc^m$,\par\quad\Habc
$(\,T(\psi(e_1),\dots,\psi(e_m))\,\,)(b_1,\dots,b_m)=b_1\psi(e_1)+\dots+b_m\psi(e_m)=\psi(b_1 e_1+\dots+b_m e_m)=\psi(b_1,\dots,b_m)$.\par\quad\Habc
Thus $T(\psi(e_1),\dots,\psi(e_m))=\psi$. Hence $T$ is surj.$\hfill\square$\par
\ProblemEnding\par

\ProblemN{7}{
	\TextN{Suppose $v,x\in V$ (chosen arbitrarily) and $U$ and $W$ are subsps of\, $V$.}
...
\BulletPoint\,\hspace{1pt}\TextB{Let $U=\{(x,y,z)\in\Rbfc^3:2x+3y+5z=0\}.$ Suppose $A\subseteq\Rbfc^3$.}
\TextB{Prove that $A$ is a translate of $U\Longleftrightarrow\exists\,c\in\Rbfc,A=\{(x,y,z)\in\Rbfc^3:2x+3y+5z=c\}.$}
{\timessl\Large[{\timessl\Large Do it in your mind.}]}\par\ProblemEnding\footpar

\ProblemB{
	\TextB{Suppose $T\in\Lm(V,W)$ and $c\in W$. Prove that $U=\{x\in V:Tx=c\}$ is either $\varnothing$}
...
又 $\tilde{T}(z+\null T)=\tilde{T}(u+\null T)\Rightarrow z+\null T=u+\null T\Rightarrow z-u\in\null T$, contradicts.$\hfill\square$\par\tinypar
...
\ProblemN{8}{
	...
	\TextN{Prove that $A$ is a translate of some subsp of \,$V$ $\Longleftrightarrow\lambda v+(1-\lambda)w\in A\,\,,\forall v,w\in A,\lambda\in\Fbfc.$}
}\par\quad
Suppose $A=a+U$, where $U$ is a subsp of $V$. $\,\,\forall a+u_1,a+u_2\in A,\lambda\in\Fbfc,$\par\quad
$\lambda(a+u_1)+(1-\lambda)(a+u_2)=a+[\lambda(u_1-u_2)+u_2]\in A$.\par\quad
Suppose $\lambda v+(1-\lambda)w\in A,\forall v,w\in A,\lambda\in\Fbfc.$ Suppose $a\in A$ and let $A\apostrophe=\{x-a:x\in A\}$.\par\quad
Then $\,\,\forall x-a,y-a\in A\apostrophe,\lambda\in\Fbfc,$\par\quad
...
Thus $A\apostrophe$ is a subsp of $V$. Hence $a+A\apostrophe=\{(x-a)+a:x\in A\}=A$ is a translate.$\hfill\square$\par
\ProblemEnding\tinypar
...
$\forall\lambda\in\Fbfc,\lambda(v+u_1)+(1-\lambda)(w+u_2)\in A_1$ and $A_2$. Thus $A_1\cap A_2$ is a translate of some subsp of $V.\hfill\square$\par
\ProblemEnding\tinypar

\ProblemN{10} {\timessl\Large 
Prove that the intersection of any collection of translates of subsps of $V$}\par\quad\,
{\timessl\Large is either a translate of some subsp or $\varnothing$.}\par
Suppose $\{A_{\alpha}\}_{\alpha\in\Gamma}$ is a collection of translates of subsps of $V$, where $\Gamma$ is an arbitrary index set.\par\quad
Suppose $x,y\in\bigcap_{\alpha\in\Gamma}A_{\alpha}\neq\varnothing$, then by Problem (18), $\forall\lambda\in\Fbfc,\lambda x+(1-\lambda)y\in A_{\alpha}$ for every $\alpha\in\Gamma$.\par\quad
Thus $\bigcap_{\alpha\in\Gamma}A_\alpha$ is a translate of some subsp of $V$.\,$\hfill\square$\par
\ProblemEnding\par

\ProblemN{11} {\timessl\Large 
Suppose $A=\{\lambda_1 v_1+\dots+\lambda_m v_m:\sum\limits_{i=1}^m \lambda_i=1\}$, where each $v_i\in V,\lambda_i\in\Fbfc.$}\par\quad\,
(a) {\timessl\Large Prove that $A$ is a translate of some subsp of $V$:\,\,\,{\large By Problem (8),}}\par\qquad\quad
$\forall\,\sum\limits_{i=1}^m a_i v_i,\sum\limits_{i=1}^m b_i v_i\in A,\lambda\in\Fbfc,\quad\lambda\sum\limits_{i=1}^m a_i v_i+(1-\lambda)\sum\limits_{i=1}^m b_i v_i=(\lambda\sum\limits_{i=1}^m a_i+(1-\lambda)\sum\limits_{i=1}^m b_i)v_i\in A.\hfill\square$\par\quad\,
(b) {\timessl\Large
Prove that if $B$ is a translate of some subsp of $V$ and $\{v_1,\dots,v_m\}\subseteq B$, then $A\subseteq B$.
}\par\quad\,
(c) {\timessl\Large
Prove that $A$ is a translate of some subsp of $V$ and $\dim V<m.$
}\par
...
\par\quad
(c) $\forall k=1,\dots,m,\,\,\,\forall\lambda_1,\dots,\lambda_{k-1},\lambda_{k+1},\dots,\lambda_m,$ let $\lambda_k=1-\lambda_1-\dots-\lambda_{k-1}-\lambda_{k+1}-\dots-\lambda_m$\par\qquad $\Rightarrow \lambda_1 v_1+\dots+\lambda_m v_m$\par\qquad $=\lambda_1 v_1+\dots+\lambda_{k-1}v_{k-1}+(1-\lambda_1-\dots-\lambda_{k-1}-\lambda_{k+1}-\dots-\lambda_m)v_k+\lambda_{k+1}v_{k+1}+\dots+\lambda_m v_m$\par\qquad $=v_k+\lambda_1(v_1-v_k)+\dots+\lambda_{k-1}(v_{k-1}-v_k)+\lambda_{k+1}(v_{k+1}-v_k)+\dots+\lambda_m(v_m-v_k)$.\par\qquad\,\,Thus $A=v_k+\Spn(v_1-v_k,\dots,v_{k-1}-v_k,v_{k+1}-v_k,\dots,v_m-v_k).\hfill\square$\par
\ProblemEnding\par

\ProblemN{12} {\timessl\Large 
Suppose $U$ is a subsp of \,$V$ such that $V/U$ is finite-dim.}\par\quad\,
{\timessl\Large Prove that is $V$ is iso to $U\times (V/U)$.
}\par
Let $(v_1+U,\dots,v_n+U)$ be a basis of $V/U$. Note that\par\quad
$\forall v\in V,\,\,\exists\,\,!\,\,a_1,\dots,a_n\in\Fbfc,v+U=\sum\limits_{i=1}^n a_i(v_i+U)=(\sum\limits_{i=1}^n a_i v_i)+U$\par\quad
...
$\exists\,\,!\,\,u\in U,w\in W,v=u+w$ 又 $\exists\,\,!\,\,c_i\in\Fbf\,$such that $w=\sum\limits_{i=1}^m c_i w_i\Rightarrow v=u+\sum\limits_{i=1}^m c_i w_i$.\par\quad
...
(a) {\timessl\Large Show that $U$ is a subsp of $\Fbfc^{\infty}$. [Do it in your mind]}\par\quad\,
...
Note that by definition, $e_r[s\cdot m!+1+i]=1\Longleftrightarrow s\cdot m!+i\equiv i\equiv 0\,(\,\textup{mod}\,r\,)\Longleftrightarrow r\mid i.\end{array}\right)$}\par\tinypar\quad
...
Suppose $\varphi\in\Lm(V,\Fraw)\backslash\{0\}$. Prove that $\dim V/(\null\varphi)=1$.
}\par
...
%Then for any $v\in V,v+U=(a_1 v_1+\dots+a_n v_n)+U,\,\,\exists\,!\,\,n\in\Nbp,(a_1,\dots,a_n)\in\Fbfc^n$.\par
%{\Large\timessl
...
Suppose $\dim V/U=1$. Prove that $\,\,\exists\,\,\varphi\in\Lm(V,\Fraw)$ such that null $\varphi=U$
.}\par
...
Suppose $V/U$ is finite-dim. \,$W$ is a subsp of \,$V$.}\par\quad\,
...
Suppose $T\in\Lm(V,W)$ and $U$ is a subsp of \,$V$. Let $\pi$ denote the quotient map.}\par\quad\,
...
(a) {\timessl\Large Prove that $\Gamma$ is linear:} By [3.9] distr properties and [3.6].\hfill$\square$\par\quad\,
...
{\small $\bullet$} By (18) in (3.D) {\timessl\Large
 we know that $\varphi:V\rightarrow\Lm(\Fraw,V)$ is an iso. Now we prove that}\par\quad {\timessl\Large $(v_1,\dots,v_m)$ is linely inde $\Longleftrightarrow(\varphi(v_1),\dots,\varphi(v_m))$ is linely inde.
}\par
...
\ProblemN{1}{
	\TextN{\timessl\Large Explain why each linear functional is surj or is the zero map.}
}$\left.\begin{array}{r}\,$ For any $\varphi \in V${\timessc\apostrophe} and $\varphi \neq 0,\,\exists\,v \in V, \,$ such that $\varphi(v) \neq 0.$ (a)$ \\ \dim$ range $\varphi=\dim\Fbf=1.$ (b)$\end{array}\right\} \Rightarrow \square$\par
\ProblemEnding\tinypar

\ProblemN{4} {\timessl\Large
Suppose $V$ is finite-dim and $U$ is a subsp of $V$ such that $U \neq V$.}\par\,\,\,
{\timessl\Large  Prove that $\exists\,\varphi \in V${\timessc\apostrophe} and $\varphi \neq 0$ such that $\varphi(u)=0$ for every $u \in U$.}\par

Let $(u_1, \dots, u_m)$ be a basis of $U$, extend to $(u_1, \dots,u_m, u_{m+1}, \dots, u_{m+n})$ a basis of $V$.\par\quad Choose $k \in \{1,\dots,n\}$ arbitrarily. Define $\varphi \in V${\timessc\apostrophe} 
...
\ProblemB{
	...
	\TextB{thus defining functions $\varphi_1, \dots , \varphi_m$ from $V$ to {\timesbf F}. Show that each $\varphi_{i} \in V\apostrophe.$}
}\par\quad
For each $w_{i},\exists\,v_{i} \in V,\,Tv_{i}=w_{i},$
getting a linely inde list ($v_1,\dots, v_m$).\par\quad
Now we have $Tv=a_1Tv_1+\dots+a_mTv_m,\,\forall v \in V,$ $\exists\,!\,a_{i}\in\Fbfc.$\par\quad
Let $(\psi_1, \dots, \psi_m)$ be the dual basis of range$T$. Then $(T\apostrophe(\psi_{i}))(v)=\psi_{i}\circ T(v)\,=\,a_{i}.$\par\quad
Thus letting $\varphi_{i}=\psi_{i}\circ T.\hfill\square$

\ProblemEnding\tinypar

\ProblemB{
	\TextB{Suppose $\varphi,\beta\in V\apostrophe$. Prove that null$\varphi\subseteq\null\beta$ if and only if $\beta=c\varphi$. $\exists\,c\in\Fbfc.$}
}Using (3.B.29, 30)\par\quad
(a) Suppose null$\varphi \subseteq\null\beta$. Choose a $u \not\in\null\beta$. $V=\null\beta\oplus\{au:a \in\Fbfc\}.$\par\quad\Habc
If $\null\varphi=\null \beta$, then let $c=\frac{\beta(u)}{\varphi(u)}$, we are done.\par\quad\Habc
Otherwise, suppose $u\apostrophe \in\null \beta,$ but $u\apostrophe \not\in\null \varphi,$ then $V=\null \varphi \oplus\{ bu\apostrophe : b \in\Fbfc\}.$\par\quad\Habc
$\forall v \in V, v=w+au=w\apostrophe+bu\apostrophe,\,\,\exists\,!\,w, w\apostrophe\in\null\varphi,a, b\in\Fbfc.$\par\quad\Habc
Thus $\beta(v)=a\beta(u),\,\,\varphi(v) = b\varphi(u\apostrophe).$ Let $c=\frac{a\beta(u)}{b\varphi(u\apostrophe)}.$ We are done\par\quad
(b) Suppose $\beta=c\varphi$ for some $c\in\Fbfc.$\par\quad\Habc
If $c=0$, then null$\beta=V\supseteq\null\varphi$, we are done.\par\quad\Habc
...
\ProblemN{7} {\timessl \Large Suppose $m$ is a positive integer. Show that the dual basis of the basis $(1, x,\dots,x_m)$ of \,\,$\PoR{m}$
}\par\,\,\,
{\timessl \Large is $\varphi_0,\varphi_1,\dots,\varphi_m$, where {\large $\varphi_k=\displaystyle\frac{p^{(k)}(0)}{k!}$.}{\scriptsize Here $p^{(k)}$ denotes the $k^{th}$ derivative of $p$, with the understanding that the $0^{th}$ derivative of $p$ is $p$.}
}\par
{\timesbfx S\footnotesize{OLUTION:}} $\,\,$\par\quad
...
Thus $\varphi_k=\psi_k$, where $\psi_1,\dots,\psi_m$ is the dual basis of $(1, x,\dots,x_m)$ of $\,\,\PoR{m}$. \qquad\qquad\qquad\qquad\qquad\qquad $\square$
 \ProblemEnding\tinypar

\ProblemN{8} {\timessl\Large Suppose $m$ is a positive integer.}\par\,\,\,
{\timessl \Large \timesnr{(a)} {\small By [2.C.10], $B$} $=(1,x-5,\dots,(x-5)^m)$ is a basis of $\PoR{m}$.}\par\,\,\,
{\timessl \Large \timesnr{(b)} Let $\varphi_k=\frac{p^{(k)}(5)}{k!}$ for each $k=0,1,\dots,m$. Then $(\varphi_0,\varphi_1,\dots,\varphi_m)$ is the dual basis of $B$.}\par
 \ProblemEnding\tinypar

\ProblemN{9} {\timessl\Large
Suppose $(v_1,\dots,v_n)$ is a basis of \,$V$ and $(\varphi_1,\cdots,\varphi_n)$ is the corresponding dual basis of $V\apostrophe$.}\par\quad
{\timessl \Large
...
{\Large\timesbf 12} {\timessl\Large Show that the dual map of the identity optor on $V$ is the identity optor on $V\apostrophe$.
}\par
...
\ProblemN{13} {\timessl\Large
Define $T:\Rraw^3\rightarrow$$\Rbfc^2$ by $T(x,y,z)=(4x+5y+6z,7x+8y+9z)$.}\par\quad\,
...
(a) {\timessl\Large Describe the linear functionals $T\apostrophe(\varphi_1),T\apostrophe(\varphi_2)\in\Lm(\Rraw^3,$ {\timesbf R}$)$}\par\qquad\,\,
...
\ProblemN{19} {\timessl\Large
$U$ is a subsp of $V$. Prove that $U=V\Longleftrightarrow U^0_V=\{0\}=V^0_V$.}\par
...
{\timessl Suppose} {\Large $\{V_{\alpha_i}\}_{\alpha_i\in\Gamma}$} {\timessl is a collection of subsps of} {\Large $V$}.\par\quad
...
\ProblemN{24} {\timessl\Large Suppose $V$ is finite-dim and $U$ is a subsp of $V$.}\par\quad\,
...
(b) Suppose $\varphi\in U^0$, then $\exists\,a_i\in${\timesbf F}, $\varphi=a_1\varphi_1+\dots+a_m\varphi_m+\dots+a_n\varphi_n.$\par\quad\quad\quad\,
...
Suppose $U$ is a subsp of $V$. Explain why $\,U=\{v\in V:\varphi(v)=0$\,\,for every $\varphi\in U^0\}.$
}\par\quad\,
{\timesbf S{\small OLUTION:}}\,\,Note that $U=\{v\in V:v\in U\}$ is a subsp of $V$ and $\varphi(v)=0$ for every $\varphi\in U^0 \Longleftrightarrow v\in U.\hfill\square$
\ProblemEnding\par

\ProblemN{26} {\timessl\Large 
Suppose $V$ is finite-dim and $\Omega$ is a subsp of $V\apostrophe$.
}\par\quad\,
...
Suppose $T\in\Lm(\PoR{5}$ and null$T\apostrophe=\Spn(\varphi)$, where $\varphi$ is the linear
functional on $\PoR{5}$
}\par\quad\,
{\timessl\Large defined by $\varphi(p)=p(8)$. Prove that range$T=\{p\in \PoR{5}:p(8)=0\}.$
}\par
{\timesbf S{\small OLUTION:}}\,\,\,By Problem (26), span$(\varphi)=\{p\in\PoR{5}:\psi(p)=0,\forall \psi\in\Spn(\varphi)\}^0,$\par\qquad\qquad\qquad\qquad\quad\,\,
Hence span$(\varphi)=\{p\in\PoR{5}:\varphi(p)=p(8)=0\}^0$, 又\,\,span$(\varphi)$ = null$T\apostrophe=(\range T)^0$.\par\quad By the corollary in Problem (20, 21), range$T=\{p\in\PoR{5}:p(8)=0\}.\,\hfill\square$\par
\ProblemEnding\par
...
{\timesbf S{\small OLUTION:}}\,\,\,\,By Problem (17),\,\,(b) and (c) are equi. By Problem (26) and the corollary in Problem (23),\par\quad\,
$\left.\begin{array}{r}$
$((\null \varphi_1)\cap\dots\cap(\null \varphi_m))^0=(\null \varphi_1)^0+\dots+(\null \varphi_m)^0$.$ \\ $
...
$ \\ $
Suppose $(v_1,\dots,v_m)$ is linely inde. Let $(\varphi_1,\dots,\varphi_m)$ be the dual basis of span$(v_1,\dots,v_m)$.
$ \\ $\qquad
Thus for each $(a_1,\dots,a_m)\in\Fbfc^m$, we have $\varphi=a_1\varphi_1+\dots+a_m\varphi_m$ so that $\Gamma(\varphi)=(a_1,\dots,a_m). \,\hfill\square$
$\end{array}\right.\\ $\par
 \ProblemEnding\par
{\small $\bullet$} {\timessl\Large Define \,$\Gamma: V\rightarrow\Fbfc^m\,$ by $\,\Gamma(v)=(\varphi_1(v),\dots,\varphi_m(v)),$ where $\varphi_1,\dots,\varphi_m\in V\apostrophe$.}\par\quad 
(c) {\timessl\Large
 Show that span$(\varphi_1,\dots,\varphi_m)=V\apostrophe\,\Longleftrightarrow \Gamma$ is inje.
}\par\quad
(d) {\timessl\Large
 Show that $(\varphi_1,\dots,\varphi_m)$ is linely inde $\Longleftrightarrow \Gamma$ is surj.
}\par
{\timesbfx S\footnotesize{OLUTION:}} \par\quad
...
(b) Suppose the rows of $A$ are linely inde, so are the cols of $B$.\par\qquad\,\,\,
...
Suppose $U$ is a subsp of $V$. Define $i:U\rightarrow V$  by $i(u)=u$. Thus $i\apostrophe\in\Lm(V\apostrophe,U\apostrophe).$}\par\quad\,
...
\ProblemEnding\par
\ProblemN{37} {\timessl\Large Suppose $U$ is a subsp of $V$ and $\pi$ is the quotient map. Thus} {\Large$\pi\apostrophe$} {\timessl\Large $\in\Lm((V/U)\apostrophe,V\apostrophe)$.}\par\quad\,
...
Suppose $\deg p\geq \deg s$. Then $(\underbrace{1, z,\dots, z^{\deg s-1}}_{\textup{of\,length}\,\deg s},\underbrace{s,zs,\cdots,z^{\deg p-\deg s}s}_{\textup{of\,length}\,(\deg p-\deg s+1)})$ is a basis of $\PoF{\deg p}.$\par\quad
...
{\small $\bullet$ } \ProblemN{N{\normalsize OTE} F{\normalsize OR} [4.11]:} {\timessl each zero of a poly corresponds to a degree-one factor;}\,\,\,{\timessl\Large Another Proof:}\par\quad
...
{\timessl Every nonconstant poly with complex coefficients has a zero in {\timesbf C}.} {\timessl\Large Another Proof:}\par\quad
...
Define $T:\PoF{m}\rightarrow\Fbfc^{m+1}$ by $Tq=(q(z_1),\dots,q(z_m),q(z_{m+1})).$ As can be easily checked, $T$ is linear.\par\quad
...
$\,\Longleftrightarrow$ $q\in\PoF{m}$ is the zero poly, for if not,\par \rightline{$q$ has at least $m+1$ distinct roots, while $\deg q=m.$ Contradicts (by [4.12]). Hence $T$ is inje.}\par\quad
...
Let $\lambda$ be a zero of $p,$ then write $p(z)=(z-\lambda)^n q(z),\,\exists\,!\,n\in\Nbp,q\in\PoC{m-n}.$\par\qquad\,
...
Prove that every poly of odd degree with real coefficients has a real zero.
}\par
...
{\timessl\Large Prove that $q$ is a poly with real coefficients.
}\par
...
$U$ is a subsp of $\PoF{}$ because $\forall f,g\in\PoF{},\lambda\in\Fbfc,pf+\lambda pg=p(f+\lambda g)\in U.$\par\quad
...
(b) $\dim(\PoC{n-1}\times\PoC{m-1})=\dim\PoC{n-1}+\dim\PoC{m-1}=n+m=\dim\PoC{m+n-1}.$\par\qquad\,
又 $T$ is inje. Hence $\dim\range T=\dim(\PoC{n-1}\times\PoC{m-1})-\dim\null T=\dim\PoC{m+n-1}.$\par\qquad\,
...
Suppose (a) $\lambda$ is an eigval of $T$ with an eigvec $v.$ Then $(T-\lambda I)v=0.${\large\par}\,\,
...
{\small $\bullet$} \ProblemN{N{\normalsize OTE} F{\normalsize OR} [5.10]:} {\normalsize linely inde eigvecs} {\Large\timessl Suppose $T\in\Lm(V).$ Prove that }\par\,\,
{\Large\timessl every list of eigvecs of $T$ corresponding to distinct eigvals is linely inde.}\par
{\timesbfx S\footnotesize{OLUTION:}} \,\,O{\small R} Another Proof.\par\quad
Suppose the desired result is false. Then ( $m\neq 1$ because eigvecs are nonzero )\par\quad
$\,\exists$ smallest $\Nbp\ni m>1$ such that $\,\exists\,(v_1,\dots,v_m)$ of eigvecs of $T$ linely depe\par
\rightline{corresponding to distinct eigvals $\lambda_1,\dots,\lambda_m$ of $T.$\qquad\qquad\qquad\qquad}\par\quad
...
Because the eigvals $\lambda_1,\dots,\lambda_m$ are distinct, and $a_j\neq 0$ for all $a_j$.\par\quad
Thus $(v_1 ,\dots, v_{m-1})$ of length $(m-1)$ is linely depe corresponding to distinct eigvals.\par\quad
...
{\timessl\Large $\Longleftrightarrow\exists\,T\in\Lm(V),$ $v_1,\dots,v_m$ are eigvecs of $T$ corresponding to distinct eigvals.}\par
...
Suppose $T\in\Lm(V)$ and $U$ is a subsp of \,$V$.}\par\,\,\,
(a) {\timessl\Large Prove that if $U\subseteq\null T$, then $U$ is invar under $T$.} \quad{$\forall u\in U\subseteq\null T,Tu=0\in U.\hfill\square$}\par\,\,\,
(b) {\timessl\Large Prove that if $\range T\subseteq U$, then $U$ is invar under $T$.
...
(a) {\timessl\Large Prove that $\null(T-\lambda I)$ is invar under $S,$ where $\lambda$ is chosen arbitrarily.
}\par\,\,
(b) {\timessl\Large Prove that $\range(T-\lambda I)$ is invar under $S,$ where $\lambda$ is chosen arbitrarily.}\par
...
Hence $Sv\in\range(T-\lambda I)$ and therefore $\range(T-\lambda I)$ is invar under $S.\hfill\square$\par\quad
C{\small OMMENT:}\, Reversing the roles of $S$ and $T$, letting $\lambda=0$, we can conclude that\par\qquad\qquad\qquad
$\null S$ and $\range S$ is invar under $T,$ which is what we will prove in Problem (2) and (3) below.\par
\ProblemEnding\par

{\small $\bullet$} {\timessl\Large 
Suppose $S,T\in\Lm(V)$ are such that $ST=TS.$}\par
\ProblemN{2} {\timessl\Large Show that $W=\null S$ is invar under $T$.}\quad{$\forall u\in W,Su=0\Rightarrow TSu=0=STu\Rightarrow Tu\in W.\square$}\par
\ProblemN{3} {\timessl\Large Show that $U=\range S$ is invar under $T$.}\,\,{$\forall w\in U,\,\exists\,v\in V,Sv=w, STv=TSv=Tw\in U.\square$}\par
\ProblemEnding\par

\ProblemN{15} {\timessl\Large 
Suppose $T\in\Lm(V)$. Suppose $S\in\Lm(V)$ is inv.}\par\quad\,
(a) {\timessl\Large Prove that $T$ and $S^{-1}TS$ have the same eigvals.}\par\quad\,
(b) {\timessl\Large What is the relationship between the eigvecs of $T$ and the eigvecs of $S^{-1}TS$?
}\par

Suppose $\lambda$ is an eigval of $T$ with an eigvec $v$.\par\quad Then $S^{-1}TS(S^{-1}v)=S^{-1}Tv=S^{-1}(\lambda v)=\lambda S^{-1}v.$\par\quad
Thus $\lambda$ is also an eigval of $S^{-1}TS$ with an eigvec $S^{-1}v$.\par\tinypar\quad
Suppose $\lambda$ is an eigval of $S^{-1}TS$ with an eigvec $v.$\par\quad Then $S(S^{-1}TS)v=TSv=\lambda Sv.$\par\quad
Thus $\lambda$ is also an eigval of $T$ with an eigvec $Sv.\hfill\square$\par\tinypar\quad
...
{\timessl\Large Prove that $\lambda$ is an eigval of $T$ $\Longleftrightarrow$ $\frac{1}{\lambda}$ is an eigval of $T^{-1}$.}\par\quad\,
(b) {\timessl\Large Prove that $T$ and $T^{-1}$ have the same eigvecs.
}\par

(a) Suppose $\lambda$ is an eigval of $T$ with an eigvec $v.$\par\qquad\,
Then $T^{-1}Tv=\lambda T^{-1}v=v\Rightarrow T^{-1}v=\frac{1}{\lambda}v.$ Hence $\frac{1}{\lambda}$ is an eigval of $T^{-1}.$\par\quad
(b) Suppose $\frac{1}{\lambda}$ is an eigval of $T^{-1}$ with an eigvec $v.$\par\qquad\,
Then $TT^{-1}v=v=\frac{1}{\lambda}Tv\Rightarrow Tv=\lambda v.$ Hence $\lambda$ is an eigval of $T.$\par\qquad\,
\Or Note that $(T^{-1})^{-1}=T$ and $\frac{1}{\frac{1}{\lambda}}=\lambda.\hfill\square$\par
\ProblemEnding\tinypar

\ProblemN{23} {\timessl\Large 
Suppose $V$ is finite-dim, $S, T\in\Lm(V)$. Prove that $ST$ and $TS$\, have the same eigvals.}\par

Suppose $\lambda$ is an eigval of $ST$ with an eigvec $v.$ Then $T(STv)=\lambda Tv=TS(Tv).$\par\quad
If $Tv\neq 0,$ then $\lambda$ is an eigval of $TS.$\par\quad
Otherwise, $\lambda=0,\,(\,v\neq 0,\lambda v=0=STv\,),$ then $T$ is not inv\par\qquad\qquad
 $\Rightarrow TS$ is not inv $\Rightarrow (TS-0I)$ is not inv $\Rightarrow \lambda=0$ is an eigval of $TS.$\par\quad
Reversing the roles of $T$ and $S$, we conclude that $ST$ and $TS$ have the same eigvals.$\hfill\square$\par
\ProblemEnding\par

{\small $\bullet$} ({\normalsize 2E Ch5.20})\par\,\,
{\timessl\Large 
Suppose $T\in\Lm(V)$ has $\dim V$ distinct eigvals and $S\in\Lm(V)$ has the same eigvecs}\par\,\,
{\timessl\Large $($ but might not with the same eigvals $)$. Prove that $ST=TS.$}\par\,\,
...
{\timessl\Large Prove that the set of eigvals of $T$ equals the set of eigvals of $\mathcal{A}$.
}\par

(a) Suppose $v_1,\dots,v_m$ are all linely inde eigvecs of $T$\par\qquad\,
with corresponding eigvals $\lambda_1,\dots,\lambda_m$ respectively ( possibly with repetitions ).\par\qquad\,
...
(b) Suppose $\lambda_1,\dots,\lambda_m$ are all eigvals of $\mathcal{A}$ with eigvecs $S_1,\dots,S_m$ respectively.\par\qquad\,
Then for each $k\in\{1,\dots,m\},$ $\exists\,v\in V,0\neq u=S_k(v)\in V\Rightarrow Tu=(TS_k) v=(\lambda_k S_k) v=\lambda_k u.$\par\qquad\,
Thus the eigvals of $\mathcal{A}$ are eigvals of $T.\hfill\square$\par\tinypar\quad
\Or \par\quad
(a) Suppose $\lambda$ is an eigval of $T$ with an eigvec $v.$\par\qquad\,
...
Suppose $T\in\Lm(V)$ and $V_1,\dots,V_m$ are subsps of \,$V$ invar under $T$.}\par\,\,\,
{\timessl\Large Prove that $V_1+\dots+V_m$ is invar under $T$.
}\par
...
{\timessl\Large of subsps of \,$V$ invar under $T$ is invar under $T$.
}\par

Suppose $\{V_\alpha\}_{\alpha\in\Gamma}$ is a collection of subsps of $V$ invar under $T$; here $\Gamma$ is an arbitrary index set.\par\quad
We need to prove that $\bigcap_{\alpha\in\Gamma}V_\alpha,$ which equals the set of vectors\par\qquad\qquad\qquad\qquad that are in $V_\alpha$ for each $\alpha\in\Gamma$, is invar under $T$.\par\quad
...
{\timessl\Large If \,$V$ is finite-dim and $U$ is a subsp of \,$V$ that is invar under every optor on $V$,}\par\,\,\,
...
Suppose $U\neq\{0\}$ and $U\neq V,$ then $\,\exists\,T\in\Lm(V)$ such that $U$ is not invar under $T$.\par\quad
...
(a) ({\normalsize O{\small R (9.11)}}) {\timessl\Large $\lambda\in\Rbfc.$ Prove that $\lambda$ is an eigval of $T$ $\Longleftrightarrow$ $\lambda$ is an eigval of $T_{\Cbb}.$
}\par\,\,
(b) ({\normalsize O{\small R Problem (16)}}) {\timessl\Large $\lambda\in\Cbfc.$ Prove that $\lambda$ is an eigval of $T_{\Cbb}$ $\Longleftrightarrow$ $\overline \lambda$ is an eigval of $T_{\Cbb}.$
}\par

(a) Suppose $v\in V$ is an eigvec corresponding to the eigval $\lambda.$\par\qquad\,
Then $Tv=\lambda v\Rightarrow T_{\Cbb}(v+\i 0)=Tv+\i T0=\lambda v.$\par\qquad\,
Thus $\lambda$ is an eigval of $T.$\par\qquad\,
Suppose $v+\i u\in V_{\Cbb}$ is an eigvec corresponding to the eigval $\lambda.$\par\qquad\,
...
Suppose $\lambda$ is an eigval of $T_{\Cbb}$ with an eigvec $v+\i u.$\par\qquad\,
...
{\timessl\Large Show that $\lambda$ is an eigval of $T$ $\Longleftrightarrow$ $\lambda$ is an eigval of the dual optor $T\apostrophe\in\Lm(V\apostrophe)$. 
}\par
...
(b) Suppose $\lambda$ is an eigval $T\apostrophe$ with an eigvec $\psi.$ Then $T\apostrophe(\psi)=\psi\circ T=\lambda\psi.$\par\qquad\,
...
\ProblemEnding\par
\ProblemN{7} {\timessl\Large 
Suppose $T\in\Lm(\Rraw^2)$ is defined by $T(x, y) = (-3y, x)$. Find the eigvals of $T$.
}\par
...
Hence the set of solution for this equation is $\varnothing,$ and therefore $T$ has no eigvals in $\Rbfc.\hfill\square$\par
\ProblemEnding\par

\ProblemN{8} {\timessl\Large 
Define $T\in\Lm(\Fraw^2)$ by $T(w, z) = (z, w)$. Find all eigvals and eigvecs of $T$.
}\par
...
Thus the set of all eigvecs is $\{(z,-z),(z,z):z\in\Fbfc\,\wedge\,z\neq 0\}.\hfill\square$
\par
\ProblemEnding\par

{\small $\bullet$} {\timessl\Large 
Suppose $P\in\Lm(V)$ is such that $P^2 = P$.}\par\,\,
{\timessl\Large Prove that if $\lambda$ is an eigval of $P$, then $\lambda = 0$ or $\lambda = 1$.
}\par
...
{\timessl\Large Prove that $3$ or $-3$ is an eigval of $T$.
}\par
{\small C{\scriptsize OMMENT}: $Tu=3w,Tw=3u\Rightarrow T(Tu)=9u\Rightarrow T^2$ has an eigval $9.$}\par\quad
$Tu=3w,Tw=3u\Rightarrow T(u+w)=3(u+w),T(u-w)=3(w-u)=-3(u-w).\hfill\square$\par
\ProblemEnding\par

\ProblemN{9} 
{\timessl\Large Define $T\in\Lm(\Fraw^3)$ by $T(z_1,z_2,z_3)=(2z_2, 0,5z_3)$.}\par\,\,\,
{\timessl\Large Find all eigvals and eigvecs of $T$.
}\par

Suppose $\lambda$ is an eigval of $T$ with an eigvec $(z_1,z_2,z_3)\in\Fbfc^3.$\par\quad
...
The set of all eigvecs is $\{(0,0,z),(z,0,0):z\in\Fbfc\,\wedge\,z\neq 0\}.\hfill\square$\par
\ProblemEnding\par


\ProblemN{10} {\timessl\Large 
Define $T\in\Lm(\Fraw^n)$ by $T(x_1,x_2,x_3,\dots,x_n)=(x_1,2 x_2,3 x_3,\dots,n x_n)$
}\par\quad\,
(a) {\timessl\Large Find all eigvals and eigvecs of $T.$}\par\quad\,
(b) {\timessl\Large Find all invar subsps of \,$V$ under $T.$}\par

(a) Suppose $v=(x_1,x_2,x_3,\dots,x_n)$ is an eigvec of $T$ with an eigval $\lambda.$\par\qquad\,
Then $Tv=\lambda v=(x_1,2 x_2,3 x_3,\dots,n x_n)=(\lambda x_1,\lambda x_2,\lambda x_3,\dots,\lambda x_n).$\par\qquad\,
Hence $1,\dots,n$ are eigvals of $T.$\par\qquad\,
...
Therefore $\deg \lambda p=-\infty=\deg p\Rightarrow p$ is a nonzero constant poly.\par\quad
Hence the set of all eigvecs is $\{C:C\in\Rbf\wedge\,C\neq 0\}=\Po_0(\Rraw)\backslash\{0\}.\hfill\square$\par
\ProblemEnding\par

\ProblemN{12} {\timessl\Large 
Define $T\in\Lm(\Po_4(\Rraw))$ by $(Tp)(x) = xp\apostrophe(x)$ for all $x\in\Rbfc.$}\par\quad\,
{\timessl\Large Find all eigvals and eigvecs of $T$.}\par
...
The set of all eigvecs of $T$ is $\{cx^\lambda:\lambda=0,1,\dots,n,\,\,c\in\Fbf\wedge\,c\neq 0\}.\hfill\square$\par
\ProblemEnding\par

\ProblemN{30} {\timessl\Large 
Suppose $T\in\Lm(\Rraw^3)$ and $-4,5,\sqrt{7}$ are eigvals of $T$.}\par\quad\,
...
\ProblemN{14} {\timessl\Large 
Suppose $V = U\oplus W$, where $U$ and $W$ are nonzero subsps of \,$V$.}\par\quad\,
{\timessl\Large Define $P\in\Lm(V)$ by $P(u + w) = u$ \,for each $u\in U$ and each $w\in W$.}\par\quad\,
{\timessl\Large
 Find all eigvals and eigvecs of $P$.
}\par
...
If $T$ has no eigvals, then $(T-\alpha I)$ is inje for all $\alpha\in\Fbf\,$and we are done.\par\quad
...
\ProblemN{17} {\timessl\Large 
Give an example of an optor on $\Rbfc^4$ that has no $($ real $)$ eigvals.}\par
{\timesbfx S\footnotesize{OLUTION:}}\par\small\quad
Define $T\in\Lm(\Rraw^4)$ by $\Mt(T,(e_1,e_2,e_3,e_4))=${\scriptsize$\begin{pmatrix}
 1 & 1 & 1 & 1\\
 -1 & 1 & -1 & -1\\
 3 & 8 & 11 & 5\\
 3 & -8 & -11 & 5
...
\Or Define $T\in\Lm(\Rraw^4)$ by $T(x_1,x_2,x_3,x_4)=(-x_2,x_1,-x_4,x_3).$\par\quad
Suppose $\lambda$ is an eigval of $T$ with an eigvec $(x,y,z,w).$\par\quad
...
Thus $T$ has no eigvals.$\hfill\square$\par
\ProblemEnding\par

{\small $\bullet$} TODO {\timessl\Large 
Suppose $(v_1,\dots,v_n)$ is a basis of \,$V$ and $T\in\Lm(V),\Mt(T,(v_1,\dots,v_n))=A.$}\par\,\,
{\timessl\Large Prove that if $\lambda$ is an eigval of $T$, then $|\lambda|\leq n \max\{|A_{j,k}|:1\leq j, k\leq n\}.$
}\par
...
\ProblemEnding\par
\ProblemN{18} {\timessl\Large 
Show that the forward shift optor $T\in\Lm(\Fraw^\infty)$}\par\quad\,
{\timessl\Large defined by $T(z_1,z_2,\dots) = (0, z_1,z_2,\dots)$ has no eigvals.
}\par
...
\ProblemN{19} {\timessl\Large 
Suppose $n\in\Nbp$. Define $T\in\Lm(\Fraw^n)$ by}\par
\centerline{\timessl\Large $T(x_1,\dots,x_n)=(x_1+\dots+x_n,\dots,x_1+\dots+x_n).$}\par\quad\,
{\timessl\Large In other words, the entries of $\Mt(T)$ with resp to the standard basis are all \,$1$'s.}\par\quad\,
{\timessl\Large Find all eigvals and eigvecs of $T.$}\par
...
And the set of all eigvecs of $T$ is $\{(x_1,\dots,x_n)\in\Fbfc^n:x_1+\dots+x_n=0\vee x_1=\dots=x_n\}.\hfill\square$\par
\ProblemEnding\par

\ProblemN{20} {\timessl\Large 
Define the backward shift optor $S\in\Lm(\Fraw^\infty)$ by $S(z_1,z_2,z_3,\dots) = (z_2,z_3,\dots)$.}\par\quad\,
(a) {\timessl\Large Show that every element of $\Fbf\,$is an eigval of $S$.}\par\quad\,
(b) {\timessl\Large Find all eigvecs of $S$.}\par
...
Hence (a) each element of $\lambda\in\Fbf\,$is an eigval of $T.$\par\quad
And (b) the set of all eigvecs of $T$ is $\{(z_1,\lambda z_1,\lambda^2 z_1,\dots,\lambda^k z_1,\dots)\in\Fbfc^\infty:\lambda\in\Fbfc,z_1\neq 0\}\hfill\square$
\par
\ProblemEnding\par

\ProblemN{24} {\timessl\Large 
Suppose $A\in\Fbfc^{n,n}.$ Define $T\in\Lm(\Fraw^n)$ by $Tx = Ax$,}\par\quad\,
{\timessl\Large where elements of $\Fbfc^n$ are thought of as $n$-by-$1$ col vectors.}\par\quad\,
(a) {\timessl\Large Suppose the sum of the entries in each row of $A$ equals $1$.}\par\qquad\,\,\,
{\timessl\Large Prove that $1$ is an eigval of $T$.}\par\quad\,
(b) {\timessl\Large Suppose the sum of the entries in each col of $A$ equals $1$.}\par\qquad\,\,\,
{\timessl\Large Prove that $1$ is an eigval of $T$.}\par
...
Thus if we let $x_1=\dots=x_n,$ then $\lambda=1,$ and hence is an eigval of $T.$\par\tinypar\tinypar\tinypar\quad
...
\Or Prove that $(T-I)$ is not inv, so that we can conclude $\lambda=1$ is an eigval.\par\qquad\,
...
{\small $\bullet$} {\timessl\Large Suppose $A\in\Fbfc^{n,n}.$ Define $T\in\Lm(\Fraw^n)$ by $Tx = xA$,}\par\quad\,
{\timessl\Large where elements of $\Fbfc^n$ are thought of as $1$-by-$n$ row vectors.}\par\quad\,
(a) {\timessl\Large Suppose the sum of the entries in each col of $A$ equals $1$.}\par\qquad\,\,\,
{\timessl\Large Prove that $1$ is an eigval of $T$.}\par\quad\,
...
\ProblemN{25} {\timessl\Large 
Suppose $T\in\Lm(V)$ and $u, w$ are eigvecs of $T$}\par\qquad\qquad\qquad\qquad\qquad\,\,\,\,
{\timessl\Large such that $u + w$ is also an eigvec of $T$.}\par\quad\,
{\timessl\Large Prove that $u$ and $w$ are eigvecs of $T$ corresponding to the same eigval.}\par

Suppose $\lambda_1,\lambda_2,\lambda_0$ are eigvals of $T$ corresponding to $u,w,u+w$ respectively.\par\quad
...
\ProblemN{26} {\timessl\Large 
Suppose $T\in\Lm(V)$ is such that every nonzero vector in $V$ is an eigvec of $T$.}\par\quad\,
{\timessl\Large Prove that $T$ is a scalar multi of the identity optor.}\par
...
{\timessl\Large Suppose $T\in\Lm(V)$ is such that every subsp of \,$V$ of dim $k$ is invar under $T$.}\par\qquad\quad\,
{\timessl\Large Prove that $T$ is a scalar multi of the identity optor.}\par

We prove the contrapositive:\par\qquad\,\,
If $T\neq\lambda I,\forall\lambda\in\Fbfc,$ then $\,\exists$ a subsp $U$ of $V$ such that $\dim U=k,$ and $U$ is invar under $T$.\par\quad
...
{\timessl\Large $T$ has an eigval $\Longleftrightarrow\exists\,$ a subsp $U$ of \,$V$}\par\qquad\qquad\qquad\qquad\qquad\qquad\qquad\,
{\timessl\Large such that $\dim U=\dim V - 1,$ $U$ is invar under $T$.
}\par
...
$\Spn(\underbrace{v_1,w_1,\dots,w_{m-1}}_{\textup{length} \dim m})$ and $\Spn(\underbrace{\alpha_{m-1,2},\dots,\alpha_{m-1,n-m+1}}_{\textup{length} \dim (n-m)})$ are invar under $T.$\par\qquad\,
...
(b) Suppose $U$ is an invar subpsace of $V$ under $T$ with $\dim U=m=\dim V-1.$\par\qquad\,
...
For $Tu_0\in U,$ then $\range T=U$ so that $T$ is not surj $\Longleftrightarrow \null T\neq\{0\}\Longleftrightarrow$ $0$ is an eigval of $T.$\par\qquad\,
For $Tu_0\not\in U,$ then $Tu_0=a_0 u_0+a_1 u_1+\dots+a_mu_m.$\par\qquad\,
...
{\timessl\Large Prove that $T$ has at most $1 + \dim\range T$ distinct eigvals.
}\par

Let $\lambda_1,\dots,\lambda_m$ be the distinct eigvals of $T$ and let $v_1,\dots,v_m$ be the corresponding eigvecs.\par\quad
...
H{\small INT}: {\timessl\normalsize Let $V=\Spn(e^{\lambda_1}x,\dots,e^{\lambda_n}x)$, and define an optor $D\in\Lm(V)$ by $Df = f\apostrophe$.}\par\qquad\quad\,\,\,\,
{\timessl\normalsize Find eigvals and eigvecs of $D$.}\par
...
with the corresponding eigvecs \,\,$\cos(\lambda_1 x),\dots,\cos(\lambda_n x)$\,\, respectively.\par\quad
And then $(\cos(\lambda_1 x),\dots,\cos(\lambda_n x))$ is linely inde in $\Rbb^{\Rbb}.\hfill\square$\par
\ProblemEnding\par

{\small $\bullet$} {\timessl\Large 
Suppose $V$ is finite-dim, $T\in\Lm(V)$, and $U$ is a subsp of \,$V$ invar under $T$.}\par\,\,
{\timessl\Large The quotient optor $T/U\in\Lm(V/U)$ is defined by}\par \centerline{\timessl\Large $(T/U)(v + U) = Tv + U$ for each $v\in V$.}\par\,\,
(a) {\timessl\Large Show that the definition of $T/U$ makes sense}\par\quad\,\,\,\,
$(${\timessl\Large which requires using the condition that $U$ is invar under $T$}$)$\par\quad\,\,\,\,
{\timessl\Large and show that $T/U$ is an optor on $V/U$.}\par\,\,
(b) {\normalsize (O{\footnotesize R} Problem 35)} {\timessl\Large Show that each eigval of $T/U$ is an eigval of $T$.
}\par

(a) Suppose $v+U=w+U\,(\,\Longleftrightarrow v-w\in U\,).$\par\qquad\,
Then because $U$ is invar under $T,$ $T(v-w)\in U\Longleftrightarrow Tv+U=Tw+U.$\par\qquad\,
...
(b) Suppose $\lambda$ is an eigval of $T/U$ with an eigvec $v+U.$\par\qquad\,
...
Note that $v-w\neq 0\,($ for if not, $v\in U\Rightarrow v+U=0+U$ is not an eigvec $).\hfill\square$\par\qquad\,
$ \\\\\\\\ $
\ProblemEnding\par
...
$\lceil$注：[8.40]\qquad\, O{\small R} (4E 5.22)\qquad ——\,mini poly;\par\qquad\qquad\qquad\quad\,\,\,\,
[8.44,8.45] O{\small R} (4E 5.25,5.26) ——\,how to find the mini poly;\par\qquad\qquad\qquad\quad\,\,\,\,
[8.49]\qquad\, O{\small R} (4E 5.27)\qquad ——\,eigvals are the zeros of the mini poly;\par\qquad\qquad\qquad\quad\,\,\,\,
[8.46]\qquad\, O{\small R} (4E 5.29)\qquad ——\,$q(T) = 0$ $\Leftrightarrow$ $q$ is a poly multi of the mini poly.$\rfloor$
}\par\large
$ \\ $
...
{\timessl\Large Prove that $\null p(T)$ and $\range p(T)$ are invar under $T$.}\par
...
{\small $\bullet$} \ProblemN{N{\normalsize OTE} F{\normalsize OR} [5.21]:} {\timessl\normalsize Every optor on a finite-dim nonzero complex vector space has an eigval.}\par\quad
...
{\timessl\normalsize an optor on a complex vector space with no eigvals}\par\quad
...
{\timessl\Large Prove that every subsp of \,$V$ invar under $T$ is either $\{0\}$ or infinite-dim.}\par
{\timesbfx S\footnotesize{OLUTION:}} Suppose $U$ is a finite-dim nonzero invar subsp on $\Cbfc.$ Then by [5.21], $T|_U$ has an eigval.$\hfill\square$
\par
\ProblemEnding\tinypar
...
E{\small XAMPLE}: On infinite-dim only. Let $V=\Fraw^\infty.${\small\par}\qquad
...
{\small $\bullet$} \ProblemN{N{\normalsize OTE} F{\normalsize OR} [8.40]:} {\timessl\normalsize def for mini poly }\par\,\,
...
{\timessl Assume that $\dim V > 0$ and that the desired result is true for all optors on all vecsps of smaller dim.}\par\qquad
...
{\small $\bullet$} ({\normalsize 4E 5.31, 4E 5.B.25 and 26}) {\timessl\normalsize \quad mini poly of restriction optor and mini poly of quotient optor}\par\,\,
{\timessl\Large 
Suppose $V$ is finite-dim, $T\in \Lm(V)$, and $U$ is an invar subsp of \,$V$ under $T$.}\par\,\,
{\timessl\Large Let $p$ be the mini poly of $T.$}\par\,\,
(a) {\timessl\Large Prove that $p$ is a poly multi of the mini poly of $T|_U$.}\par\,\,
(b) {\timessl\Large Prove that $p$ is a poly multi of the mini poly of $T/U$.}\par\,\,
...
$\left.\begin{array}{l}\forall u+\i 0\in V_{\Cbb},p(T_{\Cbb})(u)=p(T)u=0\Rightarrow\forall u\in V,p(T)u=0\Rightarrow p$ is a poly multi of $q.\\ q(T)=0\Rightarrow \forall u+\i v\in V_{\Cbb},q(T_{\Cbb})(u+\i v)=q(T)u+\i q(T)v=0\Rightarrow q$ is a poly multi of $p.\end{array}\right\}\Rightarrow\square$
\par
\ProblemEnding\par
...
{\timesbfx S\footnotesize{OLUTION:}}\par\,\,\,
$\forall \varphi\in V\apostrophe,p(T\apostrophe)\varphi=\varphi\circ (p(T))=0\Rightarrow\forall\varphi\in V\apostrophe,p(T)\in\null\varphi\Rightarrow p(T)=0\\ \left.\begin{array}{l}\qquad\qquad\qquad\qquad\qquad\qquad\qquad\,\,\,\,\Rightarrow p(T)=0\Rightarrow p$ is a poly multi of $q.\\ q(T)=0\Rightarrow \forall \varphi\in V\apostrophe,\varphi\circ(q(T))=q(T\apostrophe)\varphi=0\Rightarrow q$ is a poly multi of $p.\end{array}\right\}\Rightarrow\square$
\par
\ProblemEnding\par
...
\ProblemN{6} {\timessl\Large 
Suppose $T\in\Lm(V)$ and $U$ is a subsp of $V$ invar under $T$.}\par\,\,\,
{\timessl\Large Prove that $U$ is invar under $p(T)$ for every poly $p\in\PoF{}$.
}\par
...
Write $T^j v=c_0 v+c_1 Tv+\dots+c_{j-1}T^{j-1}v.$ And because $T(T^k v)=T^{k+1}\in U_0.$ $U_0$ is invar under $T.$\par\qquad\,
...
{\timessl\Large
Prove that $\dim \mathcal{E}$ equals the degree of the mini poly of $T$.
}\par
...
又 $\Spn(v,Tv,\dots,T^{m-1}v)$ is invar under $T.$\par\qquad Hence $\,\exists\,!\,k_j\in\Fbfc,T^{n+1}=T(T^n)=k_0+k_1 z+\dots+k_{m-1}z^{m-1}.$\par\qquad
...
%{\Large The mini poly and the {\timessc characteristic poly} are definitely not the same} { $($ Compare with $[$8.34$]$ and see $[$8.51$]$ $)$.}{\par}
\ProblemEnding\par
...
Also, the mini poly of $T$ is a poly multi of, but not equal to, $(z-\alpha_1)\cdots(z-\alpha_n)$.\par\,\,
If we define $q$ by $q(z)=(z-\alpha_1)^{\dim V-(n-1)}\cdots(z-\alpha_n)^{\dim V-(n-1)},$\par\qquad\quad\,\,\,
then $q$ is a poly multi of the char poly $($ see $[$8.34$]$ and $[$8.26$]$ $)$\par\,\,
...
Give an example of $T\in\Lm(\Rraw^2)$ such that $T^4=-I$.}\par
...
Define $T\in\Lm(\Fraw^n )$ by $T(x_1 , x_2 , x_3 , \dots , x_n ) = (x_1 , 2x_2 , 3x_3 , \dots , nx_n )$. Find
the mini poly.
}\par

$T(x_1,\dots,0)=$
By (5.A.9) and [8.49], $1,2,\dots,n$ are zeros of the mini poly of $T.$\par\quad
$($ 又 Each eigvals of $T$ corresponds to exact one-dim subsp of $\Fbfc^n.$ $)$\par\quad
...
{\small $\bullet$} {\timessl\Large 
Suppose $n\in\Nbp$. Define $T\in\Lm(\Fraw^n)$ by $T(x_1,\dots,x_n)=(x_1+\dots+x_n,\dots,x_1+\dots+x_n).$}\par\,\,
...
Suppose $T\in\Lm(\Rraw^2)$ is the optor of counterclockwise rotation by the angel $\theta$, where $x\in\Rbfc^+$.}\par\,\,
{\timessl\Large Find the mini poly of $T$.}\par\,\,
...
{\small $\bullet$} ({\normalsize 4E 5.B.11})\par\,\, {\timessl\Large Suppose $V$ is a two-dim vecsp, $T\in\Lm(V)$, and the mtx of $T$}\par\,\,
{\timessl\Large with resp to some basis of \,$V$ is {\large$\begin{pmatrix} a & c\\ b & d\end{pmatrix}.$
}}\par\,\,
...
Otherwise, by (a), $z^2-(a+d)z+(ad-bc)$ is a poly multi of the mini poly.\par\qquad\,
...
(i) $m=0,1.$ $TS^0=I=S^{-1}(ST)^0S;\,\,\,TS=S^{-1}(ST)S.$\par\,\,\,
...
(a) {\timessl\Large Give an example of $S, T\in\Lm(\Fraw^2)$ such that}\par\qquad
...
(a) %Let $V=\Fbfc^{\infty},$ $S\in\Lm(\Fraw^\infty)$ is the forward shift optor, $T\in\Lm(\Fraw^\infty)$ is the backward shift optor.\par\qquad\,
...
{\timessl\Large Show that the mini poly of $(T - \lambda I)$ is the poly $q$ defined by $q(z) = p(z + \lambda).$
}\par
...
{\timessl\Large Show that the mini poly of $\lambda T$ is the poly q defined by $q(z) = \lambda^{\deg p} p(\frac{z}{\lambda})$.
}\par
...
Suppose $T\in\Lm(V)$ is such that with resp to some basis of \,$V$,}\par\,\,
{\timessl\Large all entries of the mtx of $T$ are rational numbers.}\par\,\,
...
Suppose $a_0 ,\dots, a_{n-1}\in\Fbfc.$ Let $T$ be the optor on $\Fbfc^n$ such that}\par\tinypar\,\,
{\timessl\Large $\Mt(T)=\,${\normalsize$\begin{pmatrix}
	...
	1 & 0 &        &  &   & -a_1     \\
	  ...
	  &   & \ddots &  & 0 & -a_{n-2} \\
	0 &   &        &  & 1 & -a_{n-1}
\end{pmatrix} $}, with resp to the standard basis $(e_1,\dots,e_n)$.
}\par\tinypar\,\,
{\timessl\Large Show that the mini poly of $T$ is $p$ defined by $p(z)=a_0 + a_1 z + \dots + a_{n-1} z^{n-1} + z^n$.
}\par\,\,
{\timessl\small
$\Mt(T)$ is called the {\timessc companion mtx} of the poly above. This exercise shows that every monic poly is the mini poly of some optor.}{\par}\,\,
{\timessl\small Hence a formula or an algorithm that could produce exact eigvals for each optor on each $\Fbfc^n$ could then produce exact zeros for}{\footnotesize\par}\,\,
...
{\timessl\small approximations for the eigvals of an optor.}{\footnotesize\par}
...
(ii) $\,$Suppose $n\geq 3.$ Assume that every optor, on odd-dim vecsps of dim less than $n,$ has an eigval.\par\qquad\,
If $p$ is a poly multi of $(x - \lambda)$ for some $\lambda\in\Rbfc,$ then by [8.49] $\lambda$ is an eigval of $T$ and we are done.\par\qquad\,
Now suppose $b, c\in\Rbf\,$such that $b^2 < 4c$ and $p$ is a poly multi of $x^2 + bx + c$ (see [4.17]).\par\qquad\,
...
By [5.18], $\range(T^2 + bT + cI)$ is an invar subsp of $V$ under $T$ that has odd dim less than $n.$\par\qquad\,
...
{\timessl\Large Prove that every invar subsp of \,$V$ under $T$ is even-dim.
}\par
...
Show that every optor on a finite-dim vecsp of dim $\geq 2$}\par\,\,
{\timessl\Large has an invar subsp of dim $2$.
}\par\,\,
{\timessl\small
...
Prove or give a counterexample:}\par\,\,
{\timessl\Large If $T\in \Lm(V)$ and $T^2$ has an upper-trig mtx with resp to some basis of \,$V$,}\par\,\,
{\timessl\Large then $T$ has an upper-trig mtx with resp to some basis of \,$V$.
}\par
...
Suppose $A$ and $B$ are upper-trig mtcs of the same size,}\par\,\,
{\timessl\Large with $\alpha_1 , \dots , \alpha_n$ on the diag of $A$ and $\beta_1 , \dots , \beta_n$ on the diag of $B$.
}\par\,\,
(a) {\timessl\Large Show that $A + B$ is an upper-trig mtx with $\alpha_1 + \beta_1 , \dots , \alpha_n + \beta_n$ on the diag.
}\par\,\,
(b) {\timessl\Large Show that $AB$ is an upper-trig mtx with $\alpha_1 \beta_1 , \dots , \alpha_n \beta_n$ on the diag.
}\par
...
Suppose $T\in \Lm(V)$ is inv and $(v_1 , \dots , v_n)$ is a basis of \,$V$ with resp}\par\,\,
{\timessl\Large to which the mtx of $T$ is upper trig, with $\lambda_1 , \dots , \lambda_n$ on the diag.}\par\,\,
{\timessl\Large
Show that the mtx of $T^{-1}$ is also upper trig with resp to the same basis,}\par\,\,
{\timessl\Large with
...
(b) {\timessl\Large
Prove that the mini poly of $T$ is a poly multi of $p_v$.
}\par
...
Give an optor $T$ such that with resp to some basis, $\Mt(T)_{k,k}=0$ for each $k$,}\par\quad\,
...
Give an optor $T$ such that with resp to some basis, $\Mt(T)_{k,k}\neq 0$ for each $k$,}\par\quad\,
...
{\timessl\Large Prove that if $k\in\{1,\dots,\dim V\},$ then \,$V$ has a $k$ dim subsp invar under $T$.
}\par
...
(a) {\timessl\Large
Prove that if $\Fbf=\Rbfc,$ then $\nexists$ a basis of \,$V$ with resp to which $T$ has an upper-trig mtx.
}\par\,\,
(b) {\timessl\Large
Prove that if $\Fbf=\Cbf\,$and $A$ is an upper-trig mtx that equals the mtx of $T$}\par\quad\,\,\,
{\timessl\Large with resp to some basis of \,$V$, then $-1 + \i$ or $-1 - \i$
appears on the diag of $A$.
}\par
...
{\timessl\Large such that $A^{-1} BA$ is an upper-trig mtx.
}\par
...
{\timessl\Large Show that the following are equi.
}\par\,\,
(a) {\timessl\Large
The mtx of $T$ with resp to $(v_1 , \dots , v_n)$ is lower trig.
}\par\,\,
(b) {\timessl\Large
$\Spn(v_k , \dots , v_n )$ is invar under $T$ for each $k = 1, \dots , n$.
}\par\,\,
(c) {\timessl\Large
$Tv_k \in \Spn(v_k , \dots , v_n )$ for each $k = 1, \dots , n$.
}\par\,\,
{\timessl\small
A square mtx is called lower trig if all entries above the diag are $0$.}\par
...
{\timessl\Large Prove that if $T\in \Lm(V)$, then $T$ has a lower-trig mtx with resp to some basis.
}\par
...
Suppose $V$ is finite-dim, $T\in \Lm(V)$ has an upper-trig mtx with resp to some basis,}\par\,\,
{\timessl\Large and $U$ is a subsp of \,$V$ that is invar under $T$.
}\par\,\,
(a) {\timessl\Large
Prove that $T|_U$ has an upper-trig mtx with resp to some basis of $U$.}\par\,\,
(b) {\timessl\Large
Prove that $T/U$ has an upper-trig mtx with resp to some basis of \,$V/U$.
}\par
...
Suppose $V$ is finite-dim and $T\in \Lm(V)$.}\par\,\,
{\timessl\Large Suppose $\,\exists\,U$ of \,$V$ that is invar under $T$ such that}\par\,\,
{\timessl\Large $T|_U$ has an upper-trig mtx with resp to some basis of $U$}\par\,\,
{\timessl\Large and also $T/U$ has an upper-trig mtx with resp to some basis of \,$V/U$.}\par\,\,
{\timessl\Large Prove that $T$ has an upper-trig mtx with resp to some basis of \,$V$.
}\par
...
Suppose $V$ is finite-dim and $T\in \Lm(V)$.}\par\,\,
{\timessl\Large Prove that $T$ has an upper-trig mtx with resp to some basis of \,$V$}\par\qquad\quad
{\timessl\Large $\Longleftrightarrow$ $T\apostrophe$ has an upper-trig mtx with resp to some basis of \,$V\apostrophe$.
}\par
...
\ProblemN{1}{
	\TextN{Give an example of two commuting optors $S, T\in\Fbfc^4$ such that there is a subsp of $\Fbfc^4$}
	\TextN{that is invar under $S$ but not under $T$ and there is a subsp of $\Fbfc^4$}
	\TextN{that is invar under $T$ but not under $S$.}
}
\par
\ProblemEnding\par
...
\ProblemN{2}{
	...
	\TextN{every element of $\mathcal{E}$ has a diag mtx $\Longleftrightarrow$ every pair of elements of $\mathcal{E}$ commutes.}
...
\ProblemN{3}{
	...
	\TextN{See N{\normalsize OTE} F{\normalsize OR} [5.17] for the special case $S = T$.}
}\par
\ProblemEnding\par

\ProblemN{4}{
	\TextN{Prove or give a counterexample:}
	\TextN{If $A$ is a diag mtx and $B$ is an upper-trig mtx}
	\TextN{of the same size as $A$, then $A$ and $B$ commute.}
}\par
\ProblemEnding\par

\ProblemN{5}{
	\TextN{Prove that a pair of optors on a finite-dim vecsp commute $\Longleftrightarrow$ their dual optors commute.}
}\par
\ProblemEnding\par

\ProblemN{6}{
	\TextN{Suppose $V$ is a finite-dim complex vecsp and $S, T\in\Lm(V)$ commute.}
	\TextN{Prove that $\,\exists\,\alpha,\lambda\in\Cbf\,$such that $\range(S -\alpha I) + \range(T -\lambda I)\neq V$.}
}\par
\ProblemEnding\par

\ProblemN{7}{
	\TextN{Suppose $V$ is a complex vecsp, $S\in\Lm(V)$ is diagable, and $T$ commutes with $S$.}
	\TextN{Prove that $\,\exists$ basis $B$ of $V$ such that $S$ has a diag mtx with resp to $B$}
	\TextN{and T has an upper-trig mtx with resp to $B$.}
}\par
\ProblemEnding\par

\ProblemN{8}{
	\TextN{Suppose $m = 3$ in Example [5.72]}
	\TextN{and $D_x , D_y$ are the commuting partial differentiation optors on $\Po_3(\Rraw^2)$ from that example.}
	\TextN{Find a basis of $\Po_3(\Rraw^2)$ with resp to which $D_x$ and $D_y$ each have an upper-trig mtx.}
}\par
\ProblemEnding\par
...
\ProblemN{9}{
	...
	(b) \TextN{Prove that $\,\exists$ a basis of \,$V$ with resp to which every element of $\mathcal{E}$ has an upper-trig mtx.}
}\par
\ProblemEnding\par

\ProblemN{10}{
	\TextNL{Give an example of two commuting optors $S, T$ on a finite-dim real vecsp such that}
