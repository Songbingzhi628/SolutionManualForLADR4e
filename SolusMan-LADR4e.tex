...
\ProblemN{\hypertarget{3F35}{35}}{
	\TextNL{Let $\Fbb^\infty=\Bra{\Par{a_1,\cdots,a_n,\!\underbrace{\,0,\cdots\,}_\text{all zero}\!}\in\Fbb^\infty:\exists\,!\,n\in\Nbp}.$ Prove that $\BigPar{\PoF{}}\apostrophe$ and $\Fbb^{\infty}$ are iso.}
}\par\quad
...
{Surj: $\forall \BigPar{a_k}{_{k=1}^\infty}\in\Fbb^\infty,$ let $\psi$ be such that $\forall k,\psi\BigPar{z^k}=a_k$ and thus $\theta\Par{\psi}=\BigPar{a_k}{_{k=1}^\infty}.$}\PfEnd\vspace{6pt}\quad
\Or Using Problem (7). \par\quad
\par\quad
\par\quad
\par\quad
\par\quad
...
\ProblemN{\hypertarget{3F7}{7}}{
	\TextN{Show that the dual basis of $\Par{1, x,\dots,x_m}$ of $\PoR{m}$ is $\Par{\varphi_0,\varphi_1,\dots,\varphi_m}$, where {\large\envFontDefault $\varphi_k\Par{p}=\displaystyle\frac{p^{\envFontSmall[\scriptsize]\Par{k}}\Par{0}}{k!}$.}\vspace{2pt}}
	\TextN{{\large\envFontDefault Here $p{^{\envFontSmall[\scriptsize]\Par[0pt]{k}}}$ denotes the $k^{th}$ derivative of $p$, with the understanding that the $0^{th}$ derivative of $p$ is $p$.}\vspace{2pt}}
}\par\quad
...
\ChDecl{Ch4}{4}{}\orMode{\hLk{4O2}{2}\;\;\hLk{4O3}{3}\;\;\hLk{4O4}{4}\;\;\hLk{4O5}{5}\;\;\hLk{4O6}{6}\;\;\hLk{4O7}{7}\;\;\hLk{4O8}{8}\;\;\hLk{4O9}{9}\;\;\hLk{4O10}{10}\;\;\hLk{4O11}{11}\;\;|\;\;\hLk{4O4e}{4E: 2,}\;\;\hLk{4O4e3}{3,}\;\;\hLk{4O4e13}{13}}{[2]: (4E 2), (4E 3), 4, 5, 2; [3]: 3, 6, 7, 9; [4]: 8, 11; [5]: 10, (4E 13).}
...
If $p=0$ then we are done. Now suppose $p$ has exactly $n$ distinct zeros $\lambda_1,\dots,\lambda_n.$\TextB{}
Then $\exists\,!\,\alpha_i\geqslant 1,q\in\PoF{},$ and $q\neq 0,$ such that $p\Par{z}=\Sbra{\Par{z-\lambda_1}{^{\alpha_1}}\cdots\Par{z-\lambda_n}{^{\alpha_n}}}q\Par{z}.$\PfEnd\vspace{2pt}
...
\BulletPoint \,\hspace{1pt}\NoteFor{[4.7]} {\tgsl the uniqness of coeffs of polys} \hfill[{\tgsc Another proof}]\TextB{\vspace{2pt}}
...
\BulletPoint \,\hspace{1pt}\NoteFor{[4.8]} {\tgsl division algorithm for polys} \hfill[{\tgsc Another proof}]\TextB{\vspace{4pt}}
...
\BulletPoint \,\hspace{1pt}\NoteFor{[4.11]}\;\;{\tgsl each zero of a poly corresponds to a degree-one factor;}\hfill[{\tgsc Another proof}]\TextB{\vspace{2pt}}
...
\BulletPoint \,\hspace{1pt}\NoteFor{[4.13]}\;\;{\tgsl Every nonconst poly with complex coefficients has a zero in $\Cbb$.} \vspace{2pt}\hfill[{\tgsc Another proof}]\TextB{}
...
Inje: $Tq=0\Longleftrightarrow q\Par{z_1}=\dots=q\Par{z_m}=q\Par{z_{m+1}}=0\Longleftrightarrow q=0.$\vspace{1pt}\par\quad
...
Now suppose $M\geqslant 1.$ We show that $s=0.$ Showing $r=0$ is almost the same by supposing $N\geqslant 1.$\vspace{2pt}\par\quad
...
Note that $I_{A-1,\alpha}\subseteq \cdots\subseteq I_{0,\alpha}=\Bra{1,\dots,M}.$ Because $rp+sq=0\Rightarrow\Par{rp+sq}{^{\envFontSmall[\scriptsize]\Par[0pt]{k}}}=0$ for all $k\in\Nbp.$\vspace{2pt}\par\quad
We use induction by $D$ to show that $s{^{\envFontSmall[\scriptsize]\Par[0pt]{D}}}\Par{\lambda_{\gamma_{D,j}}}=0$ for each $D\in\Bra{1,\dots,A-1}.$\vspace{2pt}\par\quad
(i) $D=0.$ $\Par{rp+sq}\Par{\lambda_{\gamma_{0,j}}}=\Par{rp}\Par{\lambda_{\gamma_{0,j}}}=0\Rightarrow r\Par{\lambda_{\gamma_{0,j}}}=0.$\vspace{2pt}\par\quad\Hi
$D=1.$ $\Par{rp+sq}\apostrophe\Par{\lambda_{\gamma_{1,j}}}=\Par{r\apostrophe p+rp\apostrophe}\Par{\lambda_{\gamma_{1,j}}}+\Par{s\apostrophe q+sq\apostrophe}\Par{\lambda_{\gamma_{1,j}}}=\Par{s\apostrophe q}\Par{\lambda_{\gamma_{1,j}}}=0\Rightarrow s\apostrophe\Par{\lambda_{\gamma_{1,j}}}=0.$\vspace{2pt}\par\quad\Endi
(ii) $2\leqslant D\leqslant A-1.$ Assume that $\;s{^{\envFontSmall[\scriptsize]\Par[0pt]{d}}}\Par{\lambda_{\gamma_{d,j}}}=0$ for each $d\in\Bra{1,\dots,D-1}$ and $j.$\vspace{2pt}\par\quad\Hii
Note that $\Par{rp+sq}{^{\envFontSmall[\scriptsize]\Par[0pt]{D-1}}}\Par{\lambda_{\gamma_{D-1,j}}}=\XPar{s{^{\envFontSmall[\scriptsize]\Par[0pt]{D-1}}}q}\Par{\lambda_{\gamma_{D-1,j}}}=0.$\vspace{2pt}\par\quad\Hii $\Par{rp+sq}{^{\envFontSmall[\scriptsize]\Par[0pt]{D}}}\Par{\lambda_{\gamma_{D,j}}}=\XPar{\Par{rp+sq}{^{\envFontSmall[\scriptsize]\Par[0pt]{D-1}}}}\apostrophe\Par{\lambda_{\gamma_{D,j}}}=0.$\colorbox{yellow}{TODO}\vspace{2pt}\par\quad\Hii
\vspace{2pt}\par\quad\Hii
\vspace{2pt}\par\quad\Hii
Thus $s{^{\envFontSmall[\scriptsize]\Par[0pt]{d}}}\Par{\lambda_{\gamma_{d,j}}}=0$ for each $d\in\Bra{1,\dots,D}$ and $j.$ Hence holds for all $D\in\Bra{2,\dots,A-1}.$\vspace{6pt}\par\quad
\NOTICE that $\forall k=\Bra{0,\dots,A-2},s{^{\envFontSmall[\scriptsize]\Par[0pt]{k}}}$ and $s{^{\envFontSmall[\scriptsize]\Par[0pt]{k+1}}}$ have zeros $\Bra{\lambda_{\gamma_{k+1,1}},\dots,\lambda_{\gamma_{k+1,J}}}$ in common.\vspace{2pt}\par\quad
Now $\forall D\in\Bra{1,A-1},s=s{^{\envFontSmall[\scriptsize]\Par[0pt]{0}}},\dots,s{^{\envFontSmall[\scriptsize]\Par[0pt]{D}}}$ have zeros $\Bra{\lambda_{\gamma_{D,1}},\dots,\lambda_{\gamma_{D,J}}}$ in common.\vspace{2pt}\par\quad
Thus $\forall D\in\Bra{0,A-1},\;s\Par{z}$ is divisible by $\XPar{\Par{z-\lambda_{\gamma_{D,1}}}{^{\alpha_{\gamma_{D,1}}}}\cdots\Par{z-\lambda_{\gamma_{D,J}}}{^{\alpha_{\gamma_{D,J}}}}}$ $s\Par{z}.$\vspace{2pt}\par\quad
Hence we write $s\Par{z}=\Par{z-\lambda_1}{^{\alpha_1}}\cdots\Par{z-\lambda_M}{^{\alpha_M}}$ while $\deg s\leqslant m-1<m=\alpha_1+\dots+\alpha_M.$\vspace{2pt}\par\quad
