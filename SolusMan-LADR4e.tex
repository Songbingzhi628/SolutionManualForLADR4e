...
\newcommand{\hTextpar}					[2][-4pt]{\hMath[#1]{c}{\left(}{\right)}{#2}}
\newcommand{\hTextbra}					[2][-4pt]{\hMath[#1]{c}{\left\{}{\right\}}{#2}}
\newcommand{\hTextmid}					[2][-4pt]{\hMath[#1]{c}{\left|}{\right|}{#2}}
\newcommand{\hTextrmLarge}				[3][\Large]{{\large#2{${#1$#3$}$}}}
\newcommand{\hTextrmNorm}				[3][\normalsize]{{\small#2{${#1$#3$}$}}}
\newcommand\zeroSubsSmall				[1][\large]{\hTextrmNorm[#1]{\hTextbra}{0}}
\newcommand\zeroSubsLarge				[1][\large]{\hTextrmLarge[#1]{\hTextbra}{0}}
...
\newcommand{\List}[2][\large]{\hTextrmNorm[#1]{\hTextpar[-5pt]}{#2}}
\newcommand{\Span}[2][\large]{{\textup{\tgnr span}}\,\hTextrmNorm[#1]{\hTextpar[-5pt]}{#2}}
\newcommand{\Lm}[2][\large]{\mathcal{L}\hTextrmNorm[#1]{\hTextpar[-5pt]}{#2}}
\newcommand{\Lfm}[2][\large]{\mathcal{L}\hTextrmNorm[#1]{\hTextpar[-5pt]}{#2\apostrophe}}
\newcommand{\Mt}[2][\large]{\mathcal{M}\hTextrmNorm[#1]{\hTextpar[-5pt]}{#2}}
\def\Po{\mathcal{P}}
...
(a) For any subsp $U$ of $V$. $\Omega\subseteq U+\Omega=U\Rightarrow\Omega\subseteq U$. Let $U=\{0\},$ then $\Omega=\{0\}.$\par\quad
...
}Suppose $\hTextbra{U_{\alpha}}{_{\alpha\in\Gamma}}$ is a collection of subsps of $V$; here $\Gamma$ is an arbitrary index set.\vspace{4pt}\par\quad
...
{\Large\vspace{6pt}$\exists\,u\in U_1\wedge u\not\in U_2\cup U_3;\;v\in U_2\cup U_3\wedge v\not\in U_1.$ Let $W=\hTextrmLarge{\hTextbra}{v+\lambda u:\lambda\in\Fbb}\,\subseteq\mathcal{U}.$}\par\quad\Hb\HII
...
\ProblemN{\hypertarget{1C24}{24}}{
	\TextNL{Let $V_E=\hTextrmLarge{\hTextbra}{\,f\in\Rbb^{\Rbb}:\text{f is even}},V_O=\hTextrmLarge{\hTextbra}{\,f\in\Rbb^{\Rbb}:\text{f is odd}}$ Show that $V_E\oplus V_O=\Rbb^\Rbb$.}
}\par\quad
(a) { $V_E\cap V_O=\hTextbra{\,f\in\Rbb^{\Rbb}:f(x)=f(-x)=-f(-x)}{}=\{0\}.$}\par\vspace{12pt}\quad
...
\ProblemB{
	\hypertarget{2A4e314}{}\TextB{Suppose $\List{v_1,\dots,v_m}$ is a list of vecs in $V$. For each $k$, let $w_k=v_1+\dots+v_k$.}
	(a) \TextB{Show that $\Span[\Large]{v_1,\dots,v_m}=\Span[\Large]{w_1,\dots,w_m}$.}
	(b) \TextB{Show that [P] $\List{v_1,\dots,v_m}$ is linely inde $\Longleftrightarrow$ $(w_1,\dots,w_m)$ is linely inde [Q].}
}\par\quad
...
$(w_1,\dots,w_m)$ linely dep $\Rightarrow\List{v_1,\dots,v_m}$ linely dep. Conversely it is true as well.\PfEnd
\SepLine

\ProblemN{\hypertarget{2A10}{10}}{
	\TextNL{Suppose $\List{v_1,\dots,v_m}$ is linely inde in $V$ and $w\in V$.}
	\TextNL{Prove that if $\hTextrmLarge{\hTextpar}{v_1+w,\dots,v_m+w}$ is linely depe, then $w\in\Span[\Large]{v_1,\dots,v_m}$.}
}\par\quad
Suppose $a_1\hTextpar{v_1+w}+\dots+a_m\hTextpar{v_m+w}=0,\,\,\exists\,\,a_i\neq 0\Rightarrow a_1 v_1+\dots+a_m v_m=0=-(a_1+\dots+a_m)w.$\par\quad
...
\Or $\exists\,j\in\!\hTextbra{1,\dots,m},v_j+w\in\Span{v_1+w,\dots,v_{j-1}+w}.$ If $j=1$ then $v_1+w=0$ and we are done.\par\quad
If $j\geq 2,$ then $\exists\,a_i\in\Fbb,v_j+w=a_1\hTextpar{v_1+w}+\dots+a_{j-1}\hTextpar{v_{j-1}+w}\Longleftrightarrow v_j+\lambda w=a_1 v_1+\dots+a_{j-1}v_{j-1}.$\par\quad
...
Now $w=\lambda^{-1}\hTextpar{a_1 v_1+\dots+a_{j-1}v_{j-1}-v_j}\Rightarrow w\in\Span{v_1,\dots,v_m}.$\PfEnd
\SepLine

\ProblemN{\hypertarget{2A11}{11}}{
	\TextNL{Suppose $\List{v_1,\dots,v_m}$ is linely inde in $V$ and $w\in V$.}
	\TextNL{Show that [P] $\List{v_1,\dots,v_m,w}$ is linely inde $\Longleftrightarrow w\not\in\Span{v_1,\dots,v_m}$ [Q].\vspace{8pt}}
}\hspace{-7pt}$\begin{array}{l}{}^\neg Q\Rightarrow{}^\neg P:$ Suppose $w\in\Span{v_1,\dots,v_m}$. Then $\hTextpar{v_1,\dots,v_m,w}$ is linely depe.$\\{}^\neg P\Rightarrow{}^\neg Q:$ Suppose $\hTextpar{v_1,\dots,v_m,w}$ is linely dep. Then by [2.21] $w\in\Span{v_1,\dots,v_m}.\end{array}$\PfEnd
\SepLine

\ProblemN{\hypertarget{2A14}{14}}{
	\TextNL{Prove that [P] $V$ is infinite-dim $\Longleftrightarrow[Q]\left|\hspace{-6pt}\begin{array}{l}$ there is a sequence $(v_1,v_2,\dots)$ in $V$ such that$\\[-8pt]$ $\List{v_1,\dots,v_m}$ is linely inde for each $m\in\Nbp.\end{array}\right.$}
}\par\quad
...
{\tgbf Step m}\; Pick a $v_m\not\in\Span{v_1,\dots,v_{m-1}},$ by Problem (10)(b), $(v_1 , ..., v_m)$ is linely inde.\par\quad\hspace{44pt}
...
Let $(v_1 , v_2 , \dots)$ be a sequence in $V$, then $\hTextpar{v_1,v_2,\dots,v_{m+1}}$ must be linely dep.\par\quad
\Or $Q\Rightarrow P:\;$ Suppose there is such a sequence.\par\qquad\hspace{54pt}
Choose an $m$. Suppose a linely inde list $\List{v_1,\dots,v_m}$ spans $V$.\par\qquad\hspace{54pt}
...
\ProblemN{\hypertarget{2A17}{17}}{
	\TextNL{Suppose $p_0,p_1,\dots,p_m\in\PoF{m}$ such that $p_k(2)=0$ for each $k\in\hTextrmLarge{\hTextbra}{0,\dots,m}$.}
	\TextNL{Prove that $\List{p_0,p_1,\dots,p_m}$ is not linely inde in $\PoF{m}$.}
}\par\quad
Suppose $\List{p_0,p_1,\dots,p_m}$ is linely inde. Define $p\in\PoF{m}$ by $p(z)=z\,\,\forall z\in\Fbb.$\par\quad
But $\forall a_i\in\Fbb,z\neq a_0 p_0(z)+\dots+a_m p_m(z)$, for if not, let $z=2$, contradicts.
Thus $z\not\in\Span{p_0,p_1,\dots,p_m}$.\par\quad
Then $\Span{p_0,p_1,\dots,p_m}\subsetneq\PoF{m}$ while the list $\List{p_0,p_1,\dots,p_m}$ has length $(m+1)$.\par\quad
Hence $\List{p_0,p_1,\dots,p_m}$ is linely depe in $\PoF{m}$.\par\quad
For if not, because $\List{1,z,\dots,z^m}$ of length $(m+1)$ spans $\PoF{m}$,\par\qquad\qquad\quad\,\,
thus by [2.23] trivially, $\List{p_0,p_1,\dots,p_m}$ spans $\PoF{m}$. Contradicts.\PfEnd\vspace{10pt}\par\quad
\Or Note that $\PoF{m}=\Span{\overbrace{1,z,\dots,z^m}_{\text{of length }m+1}}$ and then $\List{p_0,p_1,\dots,p_m,z}$ of length $(m+2)$ is linely dep.\vspace{3pt}\par\quad
( See the above ) Now $z\not\in\Span{p_0,p_1,\dots,p_m}$ and hence $(p_0,p_1,\dots,p_m)$ is linely dep.\PfEnd
\SepLine
\ChEnd
...
\ProblemN{\hypertarget{2B7}{7}}{
	\TextN{Prove or give a counterexample: If $\List{v_1,v_2,v_3,v_4}$ is a basis of \,$V$ and $U$ is a subsp of \,$V$}
	\TextN{such that $v_1,v_2\in U$ and $v_3\not\in U$ and $v_4\not\in U$, then $\List{v_1,v_2}$ is a basis of U.}
}A counterexample:\par\quad
Let $V=\Rbb^4$ and $e_j$ be the $j^\TXT{th}$ standard basis.\par\quad
Let $v_1=e_1,v_2=e_2,v_3=e_3+e_4,v_4=e_4.$ Then $\List{v_1,\dots,v_4}$ is a basis of $\Rbb^4.$\par\quad
Let $U=\Span{e_1,e_2,e_3}=\Span{v_1,v_2,v_3-v_4}.$ Then $v_3\not\in U$ and $\List{v_1,v_2}$ is not a basis of $U.$\PfEnd\SepLine
...
(I) Consider $\Fbb=\Rbb$ or $\Cbb$. Let $\List{v_1,\dots,v_m}$ be a basis of $V\neq\{0\}$.\par\quad\HI
...
%}\par\quad
%\NOTICE that if $\List{v_1,\dots,v_m}$ is linely inde and $a_1 v_1+\dots+a_m v_m=b_1 v_1+\dots+b_m v_m,$ then $a_k=b_k$ for all $k.$\par\quad
...
\ProblemB{
	\hypertarget{2B4e9}{}\TextB{Suppose $\List{v_1,\dots,v_m}$ is a list of vecs in $V$. For $k\in\left\{1,_{}\dots,m\right\}$, let $w_k=v_1+\dots+v_k.$}
	\TextB{Show that [P] $\List{v_1,\dots,v_m}$ is a basis of $V\Longleftrightarrow$ [Q] $(w_1,\dots,w_m)$ is a basis of $V.$}
}\NOTICE that $\List{u_1,\dots,u_n}$ is a basis of $U$ $\Longleftrightarrow\forall u\in U,\,\exists\,!\,a_i\in\Fbb,u=a_1 u_1+\dots+a_n u_n.$\par\quad
$P\Rightarrow Q:\forall v\in V,\,\exists\,!\,a_i\in\Fbb,\;v=a_1 v_1+\dots+a_m v_m\Rightarrow v=b_1 w_1+\dots+b_m v_m,\,\exists\,!\,b_k=\sum_{j=1}^k(-1)^{k-j}a_j.$\par\quad
...
}Let $\List{u_1,\dots,u_m}$ and $\List{w_1,\dots,w_n}$ be bases of $U$ and $W$ respectively.\par\quad
...
\ProblemN{\hypertarget{2B8}{8}}{
	...
	\TextN{Suppose $\List{u_1,\dots,u_m}$ is a basis of $U$ and $\List{w_1,\dots,w_n}$ is a basis of $W$.}
...
%\ProblemBor{9.4}{
%	\TextB{Show that if $\List{v_1,\dots,v_n}$ is a basis of \,$V$ (on $\Rbb$),}
%	\TextB{then $\List{v_1,\dots,v_n}$ is also a basis of the complexification $V_\Cbb$ (as a complex vecsp).}\hspace{0.5pt}
...
%$u+\i v=\hTextrmNorm[\large]{\hTextpar[-5pt]}{a_1 v_1+\dots+a_n v_n}+\i(b_1 v_1+\dots+b_n v_n)=(a_1+b_1\i)v_1+\dots+(a_n+b_n\i)v_n$\par
%$\Rightarrow\,u+\i v=c_1 v_1+\dots+c_n v_n,\,\exists\,!\,c_i=a_i+b_i\i\in\Cbb$ \par
%$\Rightarrow$ By the uniqnes of $c_i$ and [2.29], $\List{v_1,\dots,v_n}$ is a basis of $V_\Cbb.$\PfEnd
%\SepLine
...
%\TextN{\tgnr Let $\List{u_1,\dots,u_m}$ be a basis of $U$. Then $m=\dim U=\dim V.$ 又 $u_i\in V$.}
%{\Large Then by [2.39], $\List{u_1,\dots,u_m}$ is also a basis of \,$V$. Thus $V=U$.}\par
\SepLine


\ProblemN{\hypertarget{2C9}{9}}{
		\TextN{Suppose $\List{v_1,\dots,v_m}$ is linely inde in $V$ and $w\in V$.}
		\TextN{Prove that $\dim\Span[\Large]{v_1+w,\dots,v_m+w}\geq m-1$.}
...
$\List{v_1,\dots,v_m}$ linely inde $\Rightarrow$ $\List{v_1,v_2-v_1,\dots,v_m-v_1}$ linely inde $\Rightarrow$ $\underbrace{\List{v_2-v_1,_{}\dots,v_m-v_1}}_{\text{of length }(m-1)}$ linely inde.\vspace{-8pt}\par\quad
又 $w\not\in\Span{v_1,\dots,v_m}\Rightarrow \List(v_1+w,\dots,v_m+w)$ is linely inde.\par\quad
...
\ProblemN{\hypertarget{2C10}{10}}{
	...
	\TextNL{each $p_k$ has degree $k$. Prove that $\List{p_0,p_1,\dots,p_m}$ is a basis of $\PoF{m}$.}
}\par\quad
...
{\Large\vspace{4pt}$\Rightarrow x^{i+1}=\displaystyle\frac{1}{a_{i+1}}\hTextpar{p_{i+1}(x)-r_{i+1}(x)}\in\Span{1,x,\dots,x^i,p_{i+1}}=\Span{p_0,p_1,\dots,p_i,p_{i+1}}$.}\par\vspace{2pt}\quad\Hi
...
{\Large$\Rightarrow\Span[\Large]{x^m,\dots,x,1}=\Spn\List{\overbrace{p_m,\dots,p_1,p_0}_\text{Basis}}.$}\large\PfEnd\vspace{10pt}\quad
\Or For any $m,k\in\Nbp$ such that $k\leq m.$ Define $p_{k,m}$ by $p_{k,m}(x)=x^k(1-x)^{m-k}.$\par\quad
Define the statement $S(m)$ by $S(m):\underbrace{\hTextpar{p_{0,m},\dots,p_{m,m}}}_{\dim\PoF{m}=m+1}$ is linely inde ( and therefore is a basis ).\vspace{4pt}\par\quad
...
(c) Let $W=\Span{z,z^2}=\hTextbra{az+bz^2:a,b\in\Fbb}$, so that $\PoF{4}=U\oplus W.$\PfEnd
\SepLine
...
Let $\List{v_1,\dots,v_m}$ be a basis of $U$, extend to a basis of $V$ as $(v_1,\dots,v_m,u_1,\dots,v_{n-m})$.\par\quad
...
\ProblemN{\hypertarget{2C14}{14}}{
	...
	\TextNL{Prove that $V_1+\dots+V_m$ is finite-dim and $\dim\!\left(V_1+\dots+V_m\right)\leq\dim V_1+\dots+\dim V_m$.}
}\par\quad
...
Then $\dim\!\left(V_1+\dots+V_m\right)=\dim\Span{\mathcal{E}_1\cup\cdots\cup \mathcal{E}_m}$.\par\quad
又 $\dim\Span{\mathcal{E}_1\cup\cdots\cup\mathcal{E}_m}\leq\card\List{\mathcal{E}_1\cup\cdots\cup \mathcal{E}_m}\leq\card \mathcal{E}_1+\cdots+\card \mathcal{E}_m$.\par\quad
Thus $\dim\!\left(V_1+\dots+V_m\right)\leq \dim V_1+\dots+\dim V_m.$\PfEnd\vspace{10pt}
\Comment \,\,\,{\tgsl\Large$\dim\!\left(V_1+\dots+V_m\right)=\dim V_1+\dots+\dim V_m\Longleftrightarrow V_1+\dots+V_m$ is a direct sum.}\par\Blind{\Comment\,\,}
For each $i$, $(V_1+\dots+V_i)\cap V_{i+1}=\{0\}\Longleftrightarrow V_1+\dots+V_m$ is a direct sum\par\Blind{\Comment\,\,}
$\Longleftrightarrow\left(\mathcal{E}_1\cap\dots\cap\mathcal{E}_{k-1}\right)\cap\mathcal{E}_k=\varnothing$ for each $i$ 又 $\dim\Span{\mathcal{E}_1\cup\dots\cup\mathcal{E}_m}=\card\List{\mathcal{E}_1\cup\dots\cup\mathcal{E}_m}$\par\Blind{\Comment\,\,}
...
$\Longleftrightarrow\dim\!\left(V_1+\dots+V_m\right)=\dim V_1+\dots+\dim V_m.$\PfEnd
\SepLine
...
Because $\hTextmid{X+Y}=\hTextmid{X}+\hTextmid{Y}-\hTextmid{X\cap Y};\,\,\,(X\cup Y)\cap Z=(X\cap Z)\cup(Y\cap Z)$.\par\quad
Now $\hTextmid{(A\cup B)\cup C}=\hTextmid{A\cup B}+\hTextmid{C}-\hTextmid{(A\cup B)\cap C}.$\par\quad
And $\hTextmid{(A\cup B)\cap C}=\hTextmid{(A\cap C)\cup(B\cap C)}=\hTextmid{A\cap C}+\hTextmid{B\cap C}-\hTextmid{A\cap B\cap C}.$\par\quad
Hence $\hTextmid{(A\cup B)\cup C}=\hTextmid{A}+\hTextmid{B}+\hTextmid{C}+\hTextmid{A\cap B\cap C}-\hTextmid{A\cap B}-\hTextmid{A\cap C}-\hTextmid{B\cap C}.$\par\vspace{12pt}\quad
...
\TextB{Suppose $v_1,\dots,v_n\in V,\dim\Span[\Large]{v_1,\dots,v_n}=n.$ Then $\List{v_1,\dots,v_n}$ is a basis of $\Span[\Large]{v_1,\dots,v_n}.$}
Notice that $\List{v_1,\dots,v_n}$ is a spanning list of $\Span{v_1,\dots,v_n}$ of length $n=\dim\Span{v_1,\dots,v_n}.$\PfEnd
\SepLine
...
Suppose $(v_1,\dots,n)$ is a basis of $V.$ Define $V_i$ by $V_i=\Span{v_i}$ for each $i\in\!\hTextbra{1,\dots,n}.$\par\quad
...
\ProblemBnosolus{
	...
	\qquad\qquad\qquad\quad\hspace{1pt}\TextB{Prove that $\exists\,B_V=\List{v_1,\dots,v_n}$ such that each $v_k\not\in U.$}
}\TextB{\vspace{0pt}}
...
By (2.A.11), $\List{v_1,\dots,v_k}$ is linely inde in $V$. If $\Span{v_1,\dots,v_k}=V,$ then we stop.\TextB{}
...
Let $\List{u_1,\dots,u_m}$ be a basis of $U,$ extend to a basis $\List{u_1,\dots,u_n}$ of $V.$\TextB{}
Then let $B_V=\List{u_1-u_k,\dots,u_m-u_k,u_{m+1},\dots,u_k,\dots,u_n}.$\PfEnd
\SepLine
\ChEnd
...
\ProblemN{3}{
	\TextN{Suppose $T\in\Lm[\Large]{\Fbb^n,\Fbb^m}$. Prove that $\,\exists\,\,A_{j,k}\in\Fbb\,$such that for any $\hTextrmLarge{\hTextpar}{x_1,\dots,x_n}\in\Fbb^n$}\hspace{-40pt}
	\TextN{\centerline{ $T\hTextpar{x_1,\dots,x_n}=\MathLeftrightPare{l}{
	A_{1,1}x_1+\dots+A_{1,n}x_n,\\
	\Blind{A}\!\vdots\Blind{_{1}x_1+}\,\ddots\Blind{+A}\vdots\\
	A_{m,1}x_1+\dots+A_{m,n}x_n}$}}
}\par\quad
Let $T\hTextpar{1,0,0,\dots,0,0}=\hTextpar{A_{1,1},\dots,A_{m,1}},$\quad Note that $\hTextpar{1,0,\dots,0,0},\cdots,\hTextpar{0,0,\dots,0,1}$ is a basis of $\Fbb^n$.\par\quad\Blind{Let}
$T\hTextpar{0,1,0,\dots,0,0}=\hTextpar{A_{1,2},\dots,A_{m,2}},$\quad Then by [3.5], we are done.\PfEnd\Blind{Let $T(1,0,0,\dots$}
$\vdots$\par\quad\Blind{Let}
$T\hTextpar{0,0,0,\dots,0,1}=\hTextpar{A_{1,n},\dots,A_{m,n}}.$\par
\SepLine

\ProblemN{4}{
	\TextN{Suppose $T\in\Lm[\Large]{V,W},$ and $v_1,\dots,v_m\in V$ such that $\left(Tv_1,\dots,Tv_m\right)$ is linely inde in $W$.} \TextN{Prove that $\List{v_1,\dots,v_m}$ is linely inde.}
}Suppose $a_1 v_1+\dots+a_m v_m=0$. Then $a_1 Tv_1+\dots+a_m Tv_m=0$. Thus $a_1=\dots=a_m=0.$\PfEnd
\SepLine

\ProblemNnosolus{5}{
	\TextN{Because $\Lm[\Large]{V,W}$ is a subsp of $W^V,$ $\Lm[\Large]{V,W}$ is a vecsp.}
}\par
...
\ProblemN{7}{
	...
	\TextN{More precisely, prove that if $\dim V = 1$ and $T\in\Lm[\Large]{V}$, then $\exists\,\lambda\in\Fbb,Tv = \lambda v,\forall v\in V$.}
}\par\quad
...
Define $T(x,y)=\MathLeftBrace{l}{x+y,$\,if $(x,y)\in\Span{3,1},\\ 0,$\qquad otherwise.
$}$ \Or Define $T(x,y)=\sqrt[3]{\hTextpar{x^3+y^3}}.$\PfEnd
\SepLine

\ProblemN{9}{
	\TextN{Give a function $\varphi:\Cbb\rightarrow\Cbb$ such that $\forall w,z\in\Cbb,\varphi\hTextrmLarge{\hTextpar}{w + z} = \varphi(w) + \varphi(z)$}
...
Define $\varphi\hTextpar{u+\i v}=u=\mathfrak{Re}\hTextpar{u+\i v}$\quad\Or Define $\varphi\hTextpar{u+\i v}=v=\mathfrak{Im}\hTextpar{u+\i v}.$\PfEnd
\SepLine

\ProblemB{
	\TextB{Prove that if $q\in\PoR{}$ and $T:\PoR{}\rightarrow\PoR{}$ is defined by $Tp=q\circ p,$ then $T$ is not linear.}
}\par\quad
Because in general, $q\circ \hTextpar{p_1+\lambda p_2}(x)=q\hTextpar{p_1(x)+\lambda p_2(x)}\neq \hTextpar{q\circ p_1}(x)+\lambda \hTextpar{q\circ p_2}(x).$\par
\Example\,\,\, Let $q$ be defined by $q(x)=x^2,$ then $q\circ\left(1+(-1)\right)=0\neq q(1)+q(-1)=2.$\PfEnd
\SepLine

\ProblemN{10}{
	\TextNL{Suppose $U$ is a subsp of \,$V$ with $U\neq V$. Suppose $S\in\Lm[\Large]{U,W}$ with $S\neq 0$}
...
\ProblemN{11}{
	\TextNL{Suppose $U$ is a subsp of finite-dim \,$V$. Suppose $S\in\Lm[\Large]{U,W}$.}
	\TextNL{Prove that $\exists\,T\in\Lm[\Large]{V,W},Tu = Su,\forall u\in U$.}
	\TextNL{\large In other words, every linear map on a subsp of \,$V$ can be extended to a linear map on the entire $V$.}
}Define $T\in\Lm{V,W}$ by $T\hTextpar{a_1 u_1+\dots+a_n u_n+a_{n+1}u_{n+1}+\dots+a_m u_m}=a_1 Su_1+\dots+a_n Su_n.$\par\qquad\qquad\quad
Where we let $B_U=\List{u_1,\dots,u_n},B_V=(u_1,\dots,u_n,\dots,u_m).$\PfEnd
\SepLine

\ProblemN{12}{
	\TextNL{Suppose $V$ is finite-dim with $\dim V>0,$ and $W$ is infinite-dim.}
	\TextNL{Prove that $\Lm[\Large]{V,W}$ is infinite-dim.}
}\par\quad
Let $\List{v_1,\dots,v_n}$ be a basis of $V$. Let $(w_1,\dots,w_m)$ be linely inde in $W$ for any $m\in\Nbp$.\par\quad
Define $T_{x,y}\in\Lm{V,W}$ by $T_{x,y}(v_z)=\delta_{zy} w_y$, $\forall x\in\!\hTextbra{1,\dots,n},y\in\!\hTextbra{1,\dots,m}$, where $\delta_{zy}=\MathLeftBrace{l}{
0,\quad z\neq y,\\
1,\quad z=y.}$\par\quad
...
\ProblemN{13}{
	\TextNL{Suppose $\List{v_1,\dots,v_m}$ is linely depe in $V$ and $W\neq\{0\}$.}
	\TextNL{Prove that $\exists\,w_1,\dots,w_m\in W,\not\exists\,T\in\Lm[\Large]{V,W}$ such that $Tv_k=w_k,\forall k = 1,\dots,m$.}
}\par\quad
We prove by contradiction. By linear dependence lemma, $\,\,\exists\,\,j\in\!\hTextbra{1,\dots,m},v_j\in\Span{v_1,\dots,v_{j-1}}$.\par\quad
Fix $j$. Let $w_j\neq 0$, while $w_1=\dots=w_{j-1}=w_{j+1}=w_m=0.$\par\quad
Define $T$ by $Tv_k=w_k$ for all $k$. Suppose $a_1 v_1+\dots+a_m v_m=0$ ( where $a_j\neq 0$ ).\par\quad
Then $T\hTextrmNorm[\large]{\hTextpar[-5pt]}{a_1 v_1+\dots+a_m v_m}=0=a_1 w_1+\dots+a_m w_m=a_j w_j$ while $a_j\neq 0$ and $w_j\neq 0.$ Contradicts.\PfEnd\vspace{12pt}\quad
\Or {\Large\vspace{6pt}\,\dbsp We prove the contrapositive:}\par\quad\Blind{\Or}
{\Large\vspace{6pt}\dbsp Suppose $\forall w_1,\dots,w_m\in W,\,\exists\,T\in\Lm[\Large]{V,W},Tv_k=w_k$  for each $w_k.$}\par\quad\Blind{\Or}
{\Large\vspace{12pt}\dbsp (We need to) Prove that $\List{v_1,\dots,v_n}$ is linely inde.}\par\quad
{\Large\vspace{6pt}Suppose $\,\exists\,a_i\in\Fbb,a_1 v_1+\dots+a_n v_n=0.$ Choose a nonzero $w\in W.$}\par\quad
{\Large\vspace{10pt}By assumption, for the list $\hTextpar{\overline{a_1}w,\dots,\overline{a_m}w},\,\exists\,T\in\Lm[\Large]{V,W},Tv_k=\overline{a_k}w$ for each $v_k.$}\par\quad
...
{\Large Then $\sum_{k=1}^m\left|a_k\right|^2=0\Rightarrow a_k=0$ for each $k.$  $\List{v_1,\dots,v_n}.$}\PfEnd
\SepLine

\ProblemBor{3.D.16}{
	\TextB{Suppose $V$ is finite-dim and $T\in\Lm[\Large]{V}$.}
	\TextB{Suppose $ST=TS$ for every $S\in\Lm[\Large]{V}$. Prove that $T$ is a scalar multi of the identity.}
}\par\quad
If $V=\{0\},$ then we are done. Now suppose $V\neq\{0\}.$\par\quad
Assume that $\List{v,Tv}$ is linely depe for every $v\in V$, then by (2.A.2.(b)), $Tv=\lambda_v v$ for some $\lambda_v\in\Fbb.$\par\quad
...
Now we prove the assumption by contradiction. Suppose $\List{v,Tv}$ is linely inde for every $v\in V\backslash\{0\}$.\par\quad
...
\Or \;{\Large\vspace{6pt}Let $\List{v_1,\dots,v_m}$ be a basis of $V.$}\par\quad\Blind{\Or }
{\Large\vspace{6pt}Define $\varphi\in\Lm[\Large]{V,\Fbb}$ by $\varphi(v_1)=\cdots=\varphi(v_m)=1.$ Let $\lambda=\varphi(Tv_1)\in\Fbb.$}\par\quad\Blind{\Or }
\dbsp{\Large\vspace{6pt}For any $v\in V,$ define $S_v\in\Lm[\Large]{V}$ by $S_v u=\varphi(u)v.$}\par\quad\Blind{\Or }
{\Large\vspace{6pt}Then $Tv=T\left(\varphi(v_1)v\right)=T(S_v v_1)=S_v(Tv_1)=\varphi(Tv_1)v=\lambda v.$}\PfEnd
\SepLine

\ProblemB{
	\TextB{{\large({\normalsize 4E 3.A.16})}}
	\TextB{Suppose $V$ is finite-dim. Show that the only two-sided ideals of $\Lm[\Large]{V}$ are $\{0\}$ and $\Lm[\Large]{V}$.}
	\TextB{{\large A subsp $\mathcal{E}$ of $\Lm[\Large]{V}$ is called a two-sided ideal of $\Lm[\Large]{V}$ if $TE\in\mathcal{E},ET\in\mathcal{E},\,\,\forall E\in\mathcal{E},T\in\Lm[\Large]{V}$.}}
}\par\quad
{\Large\vspace{6pt}Let $\List{v_1,\dots,v_n}$ be a basis of $V$. If $\mathcal{E}=0$, then we are done.}\par\quad
{\Large\vspace{6pt}Suppose $\mathcal{E}\neq 0$ and $\mathcal{E}$ is a two-sided ideal of $\Lm[\Large]{V}$. Let $S\in\mathcal{E}\backslash\{0\}$.}\par\quad
{\Large\vspace{6pt}Suppose $Sv_i\neq 0$ and $Sv_i=a_1 v_1+\dots+a_n v_n$, where $a_k\neq 0$.}\par\quad
{\Large\vspace{6pt}Define $R_{x,y}\in\Lm[\Large]{V}$ by $R_{x,y}(v_x)=v_y, R_{x,y}(v_z)=0\,(\,z\neq x\,)$. Then for any $x,y\in\Nbp,$}\par\quad
...
{\Large\vspace{6pt}Hence for any $T\in\Lm[\Large]{V},I\circ T=T\circ I=T\in\mathcal{E}\Rightarrow \mathcal{E}=\Lm[\Large]{V}.$}\large\PfEnd
\SepLine
...
\ProblemB{
	\hypertarget{3B4e33}{}\TextB{Suppose that $V$ and $W$ are real vecsps and $T\in\Lm[\Large]{V,W}$.}
	\TextB{Define $T_{\Cbb}:V_{\Cbb}\rightarrow W_{\Cbb}$ by $T_{\Cbb}\hTextrmLarge{\hTextpar}{u+\i v}=Tu+\i Tv$ for all $u,v\in V$.}
...
%\ProblemN{2}{
%	\TextN{Suppose $S,T\in\Lm[\Large]{V}$ are such that $\range S\subseteq\null T$. Prove that $(ST)^2=0$.}
%}$TS=0\Rightarrow STST=(ST)^2=0.$\PfEnd
%\SepLine

\ProblemNnosolus{\hypertarget{3B3}{3}}{
	\TextN{Suppose $\List{v_1,\dots,v_m}$ in V. Define $T\in\Lm[\Large]{\Fbb^m, V}$ by $T\List{z_1,\dots,z_m}=z_1 v_1+\dots+z_m v_m.$\vspace{3pt}}
	(a) \TextN{\Large The surj of $T$ corresponds to $\List{v_1,\dots,v_m}$ spanning $V$.\vspace{3pt}}
	(b) \TextN{The inje of $T$ corresponds to $\List{v_1,\dots,v_m}$ being linely inde.}
}\SepLine

%\ProblemN{4}{
%	\TextN{Show that $U=\left\{T\in\Lm[\Large]{\Rbb^5,\Rbb^4}:\dim\null T > 2\left\}$ is not a subsp of $\Lm[\Large]{\Rbb^5,\Rbb^4}$.}
%}\par\quad
...
%
%\ProblemN{5}{
%	\TextN{Give an example of $T\in\Lm[\Large]{\Rbb^4}$ such that $\range T=\null T.$}
...
\ProblemN{\hypertarget{3B7}{7}}{
	...
	\TextN{Show that $U=\hTextbra{\,T\in\Lm[\Large]{V,W}:\null T\neq\{0\}}$ is not a subsp of $\Lm[\Large]{V,W}$.}
}\par\quad
Let $\List{v_1,\dots,v_n}$ be a basis of $V$, $(w_1,\dots,w_m)$ be linely inde in $W$.\par\quad
( Let $\dim W=m$, if $W$ is finite, otherwise, let $m\in\hTextbra{n,n+1,\dots}$; $2\leq n\leq m$ ).\par\,
$\MathRightMid{l}{$
...
\Comment \,\,\,If $\dim V=0,$ then $V=\{0\}=\Span{\,}$. $\forall\,\,T\in\Lm{V,W}, T$ is inje. Hence $U=\varnothing$.\par\Blind{\Comment\,\,}
...
\ProblemN{\hypertarget{3B8}{8}}{
	...
	\TextN{Show that $U=\hTextbra{\,T\in\Lm[\Large]{V,W}:\range T\neq W}$ is not a subsp of $\Lm[\Large]{V,W}$.}
}\par\quad
Let $\List{v_1,\dots,v_n}$ be linely inde in $V$, $(w_1,\dots,w_m)$ be a basis of $W$.\par\quad
( Let $n=\dim V$, if $V$ is finite, otherwise we choose $n\in\hTextbra{m,m+1,\dots}$; $2\leq m\leq n$ ).\par\quad
...
\Comment \,\,\,If $\dim W=0,$ then $W=\{0\}=\Span{\,}$. $\forall\,\,T\in\Lm{V,W}, T$ is surj. Hence $U=\varnothing.$\par\Blind{\Comment\,\,}
...
\ProblemN{\hypertarget{3B9}{9}}{
	\TextN{Suppose $\List{v_1,\dots,v_n}$ is linely inde. Prove that $\forall$ inje $T, \hTextrmLarge{\hTextpar[-5pt]}{Tv_1,\dots,Tv_n}$ is linely inde.}
...
\ProblemN{\hypertarget{3B10}{10}} {
	\TextNL{Suppose $\Span[\Large]{v_1,\dots,v_n}=V$. Show that $\Span[\Large]{Tv_1,\dots,Tv_n}=\range T$.}
}\par\quad
(a) $\range T=\hTextbra{Tv:v\in V}=\hTextbra{Tv:v\in\Span{v_1,\dots,v_n}}\Rightarrow Tv_1,\dots,Tv_n\in\range T\Rightarrow$ By [2.7].\par\quad\Ha
\Or $\Span{Tv_1,\dots,Tv_n}\ni a_1 Tv_1+\dots+a_n Tv_n=T\hTextrmNorm[\large]{\hTextpar[-5pt]}{a_1 v_1+\dots+a_n v_n}\in\range T.$\par\quad
...
\ProblemN{\hypertarget{3B16}{16}}{
	\TextNL{Suppose $\exists\,T\in\Lm[\Large]{V}$ such that $\null T,\range T$ are
finite-dim. Prove that $V$ is finite-dim.}
}\par\quad
Let $B_{\range T}=\hTextrmNorm[\large]{\hTextpar[-5pt]}{Tv_1,\dots,Tv_n},B_{\null T}=\List{u_1,\dots,u_m}$.\par\quad
...
\ProblemN{\hypertarget{3B17}{17}}{
	\TextNL{Suppose $V,W$ are finite-dim. Prove that $\exists$ inje $T\in\Lm[\Large]{V,W}\Longleftrightarrow\dim V\leq\dim W$.}
}\par\quad
(a) Suppose $\exists$ inje $T$. Then $\dim V=\dim\range T\leq\dim W$.\par\quad
(b) Suppose $\dim V\leq\dim W.$ Let $B_V=\List{v_1,\dots,v_n},B_W=(w_1,\dots,w_m).$\par\quad\Hb
Define $T\in\Lm{V,W}$ by $Tv_i=w_i,\,\,\,i=1,\dots,n\,(\,=\dim V\,).$\PfEnd
\SepLine

\ProblemN{\hypertarget{3B18}{18}}{
	\TextNL{Suppose $V,W$ are finite-dim. Prove that $\exists$ surj $T\in\Lm[\Large]{V,W}\Longleftrightarrow\dim V\geq\dim W$.}
}\par\quad
(a) Suppose $\exists$ surj $T$. Then $\dim V=\dim W+\dim\null T\Rightarrow\dim W\leq\dim V$.\par\quad
(b) Suppose $\dim V\geq\dim W.$ Let $B_V=\List{v_1,\dots,v_n},B_W=(w_1,\dots,w_m).$\par\quad\Hb
...
\ProblemN{\hypertarget{3B19}{19}}{
	...
	\TextNL{Prove that if $\underbrace{\dim U}_m\geq\underbrace{\dim V}_{m\,+\,n}-\underbrace{\dim W}_{p},$ then $\exists\,\,T\in\Lm[\Large]{V,W},\,\,\null T = U$.}
}\par\quad
Let $B_U=\List{u_1,\dots,u_m},B_V=(u_1,\dots,u_m,v_1,\dots,v_n),B_W=(w_1,\dots,w_p)$.\par\quad
Define $T\in\Lm{V,W}$ by $T(a_1 v_1+\dots+a_n v_n+b_1 u_1+\dots+b_m u_m)=a_1 w_1+\dots+a_n w_n.$\PfEnd
\SepLine

\ProblemBnoor{({\normalsize 4E 3.B.21})\TextB{}}{
	\hypertarget{3B4e21}{}\TextB{Suppose $V$ is finite-dim, $T\in\Lm[\Large]{V,W}$, $U$ is a subsp of \,$W$. Let $\mathcal{K}_U=\hTextbra{v\in V: Tv\in U}$.}
	\TextB{Prove that $\mathcal{K}_U$ is a subsp of \,$V$ and $\dim\mathcal{K}_U=\dim\null T +\dim\left(U\cap\range T\right)$.}
}\par\quad
...
\ProblemN{\hypertarget{3B12}{12}}{
	\TextNL{Prove that $\forall T\in\Lm[\Large]{V,W},\,\exists$ a subsp $U$ of $V,U\,\cap\,\null T=\zeroSubsLarge,\,\range T=\hTextbra{Tu:u\in U}.$}
}\par\quad
By [2.34] ( note that $V$ can be infinite-dim ), $\exists$ a subsp $U$ of $V,V=U\oplus\null T$.\par\quad
$\forall v\in V,\,\exists\,!\,w\in\null T,u\in U, v=w+u.$ Then $Tv=T(w+u)=Tu\in\hTextbra{Tu:u\in U}.$\PfEnd
\SepLine

\BulletPoint \,\hspace{1pt}\NewNotation\TextB{}
Suppose $T\in\Lm{V,W}$ and $R=\hTextrmNorm[\large]{\hTextpar[-5pt]}{Tv_1,\dots,Tv_n}$ is linely inde in $\range T$.\TextB{}
Where $n=\dim\range T$ if finite-dim, otherwise $n\in\Nbp.$ By (3.A.4), $L=\List{v_1,\dots,v_n}$ is linely inde in $V$.\TextB{}
...
\ProblemBnosolus{
	...
	that $\forall Tv\in\range T,\,\exists\,!\,n\in\Nbp,Tv\in\Span{Tv_1,\dots,Tv_n},$ where $(Tv_k)_{k=1}^\infty$ is linely inde.\TextB{\vspace{2pt}}
	So that $\range T\subseteq\Spn\uwave{\List{Tv_1,\cdots,Tv_n,\cdots}}.$ This would be invalid, as we have shown before.\TextB{\vspace{2pt}}
}
...
\ProblemB{
	\TextB{Suppose $V$ is finite-dim, $T\in\Lm[\Large]{V,W},B_{\range T}=\List[\Large]{Tv_1,\dots,Tv_n},B_V=\List[\Large]{v_1,\dots,v_n,u_1,\dots,u_m}$.}
	\TextB{Prove or give a counterexample: $\List{u_1,\dots,u_m}$ is a basis of $\null T$.}
}A counterexample:\par\quad
...
Always notice that $\mathcal{S}_V\Span{v_1,\dots,v_n}=\hTextbra{U_1,\cdots,\null T,\cdots,U_n,\cdots}$\par
\SepLine

%\ProblemB{
%	\TextB{Suppose $V$ is finite-dim, $T\in\Lm[\Large]{V,W},\;B_{\null T}=\List{u_1,\dots,u_m},\;B_V=(u_1,\dots,u_m,v_1,\dots,v_n)$.}
%	\TextB{Prove or give a counterexample: $\Span{Tv_1,\dots,Tv_n}=\range T$.}
%}\par\quad
%$\forall w\in\range T,\,\exists\,v\in V,\,(\,\exists\,!\,a_i,b_i\in\Fbb\,),Tv=T\hTextrmNorm[\large]{\hTextpar[-5pt]}{a_1 v_1+\dots+a_n v_n}=w\in\Span{Tv_1,\dots,Tv_n}\supseteq\range T.$\par
%\Comment\,\,\, If $T$ is inje, then $\hTextrmNorm[\large]{\hTextpar[-5pt]}{Tv_1,\dots,Tv_n}$ is a basis of $\range T$.\PfEnd
%\SepLine

\ProblemB{
	\hypertarget{3B4e31}{}\TextB{Suppose $V$ is finite-dim, $X$ is a subsp of $V$, and $Y$ is a finite-dim subsp of $W$.}
	\TextB{Prove that if $\dim X + \dim Y = \dim V$, then $\,\exists\,T\in\Lm[\Large]{V,W},\null T = X,\range T = Y$.}
}\par\quad
Suppose $\dim X + \dim Y = \dim V$. Let $B_X=\List{u_1,\dots,u_n},B_Y=(w_1,\dots,w_m),B_V=(u_1,\dots,u_n,v_1,\dots,v_m).$\vspace{3pt}\par\quad
...
$\MathRightBrace{l}{$
	...
	$w\in Y\Rightarrow w=a_1 Tv_1+\dots+a_m Tv_m=T\hTextrmNorm[\large]{\hTextpar[-5pt]}{a_1 v_1+\dots+a_m v_m}\Rightarrow w\in\range T$
	$}\Rightarrow\range T=Y.$\PfEnd
\SepLine

\ProblemBor{5.B.4}{
	\hypertarget{3B4e27}{}\TextB{Suppose $P\in\Lm[\Large]{V}$ and $P^2 = P$. Prove that $V=\null P\oplus\range P$.}
}\par\quad
...
\ProblemN{\hypertarget{3B20}{20}}{
	\TextNL{Suppose $W$ is finite-dim. Prove that $T\in\Lm[\Large]{V,W}$ is inje $\Longleftrightarrow\exists\,\,S\in\Lm[\Large]{W,V},\,\,ST=I_V$.}
}\par\quad
(a) Suppose $\exists\,\,S\in\Lm{W,V},\,\,ST=I$. Then if $Tv=0\Rightarrow ST(v)=0=v.$\par\quad
(b) Suppose $T$ is inje. Let $R=B_{\range T}=\hTextrmNorm[\large]{\hTextpar[-5pt]}{Tv_1,\dots,Tv_n}$.\par\quad\Hb
Then $\mathcal{K}_R\oplus\null T=V$. And let $U\oplus\range T=W$.\par\quad\Hb
Define $S\in\Lm{W,V}$ by $S(Tv_i)=v_i$ and $Su=0,$ where $i\in\!\hTextbra{1,\dots,n},u\in U.$ Thus $ST=I.$\PfEnd
\SepLine

\ProblemN{\hypertarget{3B21}{21}}{
	\TextNL{Suppose $W$ is finite-dim. Prove that $T\in\Lm[\Large]{V,W}$ is surj $\Longleftrightarrow\exists\,\,S\in\Lm[\Large]{W,V},\,\,TS=I_W$.}
}\par\quad
(a) Suppose $\exists\,\,S\in\Lm{W,V},\,\,TS=I$. Then $\forall\,w\in W,TS(w)=w\in\range T\Rightarrow \range T=W.$\par\quad
(b) Suppose $T$ is surj. Let $R=B_{\range T}=B_{W}=\hTextrmNorm[\large]{\hTextpar[-5pt]}{Tv_1,\dots,Tv_n}$\par\quad\Hb
Then $\mathcal{K}_R\oplus\null T=V.$ Define $S\in\Lm{W,V}$ by $S(Tv_i)=v_i.$ Then $TS=I.$\PfEnd
\SepLine

\ProblemN{\hypertarget{3B24}{24}}{
	\TextNL{Suppose that $W$ is finite-dim and $S, T\in\Lm[\Large]{V,W}$.}
	\TextNL{Prove that $\null S\subseteq\null T\Longleftrightarrow\exists\,\,E\in\Lm[\Large]{W}$ such that $T=ES$.}
}\par\quad
...
Now \;$v-\hTextrmNorm[\large]{\hTextpar[-5pt]}{a_1 v_1+\dots+a_n v_n}\in\null S\Rightarrow v-\hTextrmNorm[\large]{\hTextpar[-5pt]}{a_1 v_1+\dots+a_n v_n}\in\null T\Rightarrow T\left(v-\hTextrmNorm[\large]{\hTextpar[-5pt]}{a_1 v_1+\dots+a_n v_n}\right)=0.$\par\quad
...
\ProblemN{\hypertarget{3B25}{25}}{
	\TextNL{Suppose that $V$ is finite-dim and $S, T\in\Lm[\Large]{V,W}$.}
	\TextNL{Prove that $\range S\subseteq\range T\Longleftrightarrow\exists\,\,E\in\Lm[\Large]{V}$ such that $S = TE$.}
}\par\quad
Suppose $\,\,\exists\,\,E\in\Lm{V}$ such that $S = TE$. Then $\range S=\range TE\subseteq\range T.$\par\quad
Suppose $\range S\subseteq\range T$. Let $\List{v_1,\dots,v_m}$ be a basis of $V$.\par\quad
...
\ProblemN{\hypertarget{3B22}{22}}{
	\TextNL{Suppose $U$ and $V$ are finite-dim vecsps and $S\in\Lm[\Large]{V,W},T\in\Lm[\Large]{U,V}$.}
...
Thus $\null ST=\mathcal{K}_{\null S\cap\range T}=\hTextbra{u\in U:Tu\in\null S}.$ By Problem (4E 3B.21),\par\quad
$\dim\null ST=\dim\null T+\dim\left(\null S\cap\range T\right)\leq\dim\null T+\dim\null S.$\vspace{6pt}\PfEnd
...
\ProblemN{\hypertarget{3B23}{23}}{
	\TextNL{Suppose $U$ and $V$ are finite-dim vecsps and $S\in\Lm[\Large]{V,W}$ and $T\in\Lm[\Large]{U,V}$.}
	\TextNL{Prove that $\dim\range ST\leq\min\hTextrmLarge{\hTextbra}{\dim \range S, \dim\range T}$.}
}\par\quad
$\range ST=\hTextbra{Sv:v\in\range T}=\Span{Su_1,\dots,Su_{\dim\range T}},$ where \,$B_{\range T}=\left(u_1,\dots,u_{\dim\range T}\right).$\par\quad
...
\ProblemB{
	\hypertarget{3B4e24}(a) \TextB{Suppose $\dim V = 5,S,T\in\Lm[\Large]{V}$ are such that $ST = 0$. Prove that $\dim\range TS\leq 2$.}
	(b) \TextB{Let $\dim V=n$ in {\tgnr\large(a)}. Prove that $\dim\range TS\leq\left\lfloor\displaystyle\frac{n}{2}\right\rfloor.$}
	(c) \TextB{Give an example of $S, T\in\Lm[\Large]{\Fbb^5}$ with $ST = 0$ and $\dim\range TS = 2$.}
}\par\quad
...
(c) Let $\List{v_1,\dots,v_5})$ be a basis of $\Fbb^5.$ Define $S,T\in\Lm{\Fbb^5}$ by:\par\quad\Hc
\hspace{60pt}$T:\quad v_1\mapsto 0,\quad\,\, v_2\mapsto 0,\,\,\quad v_i\mapsto v_i\,\,\,;$\par\quad\Hc
\hspace{60pt}$S:\quad v_1\mapsto v_4,\quad v_2\mapsto v_5,\quad v_i\mapsto 0\,\,\,\,;\qquad i=3,4,5.$\PfEnd[-10pt]\vspace{30pt}\quad
...
(ii) {Suppose we have defined $p_0,\dots,p_n$ such that $Dp_k=x^k$ for each $k\in\hTextbra{0,\dots,n}.$}\par\quad\Hii
{Because $\deg D(x^{n+2})=n+1,$}\par\quad\Hii
{Let {\;$D(x^{n+2})=a_{n+1}x^{n+1}+a_n x^n+\dots+a_1 x+a_0,$} where $a_{n+1}\neq 0.$}\par\quad\Hii
{Then {\;$a_{n+1}^{-1}D(x^{n+2})=x^{n+1}+a_{n+1}^{-1}(a_n Dp_n+\dots+a_1 Dp_1 +a_0 Dp_0)$}}\par\quad\Hii
{$\Rightarrow x^{n+1}=D\left(a_{n+1}^{-1}(x^{n+2}-a_n p_n-\dots-a_1 p_1-a_0 p_0)\right).$}\par\quad\Hii
...
\ProblemN{\hypertarget{3B28}{28}}{
	\TextNL{Suppose $T\in\Lm[\Large]{V,W},B_{\range T}=(w_1,\dots,w_m)$.}
	\TextNL{Prove that $\exists\,\varphi_1,\dots,\varphi_m\in\Lm[\Large]{V,\Fbb}$ such that $\forall v\in V,Tv=\varphi_1(v)w_1+\dots+\varphi_m(v)w_m$.}
}\par\quad
Suppose $v_1,\dots,v_m\in V$ such that $\forall i,Tv_i=w_i.$ Then $\List{v_1,\dots,v_m}$ is linely inde, let $B_V=\List{v_1,\dots,v_m,u_1,\dots,u_n}$.\par\quad
...
\ProblemN{\hypertarget{3B29}{29}}{
	\TextNL{Suppose $\varphi\in\Lm[\Large]{V,\Fbb}.$ Suppose $u\in V\backslash\null\varphi.$ Prove that $V = \null\varphi\oplus\hTextbra{au :a\in\Fbb}.$}
}If $\varphi=0$ then we are done. Suppose $\varphi\neq 0.$\par\quad
(a) $\forall\,v=cu\in\null\varphi\cap\hTextbra{au:a\in\Fbb},\varphi(v)=0=c\varphi(u)\Rightarrow c=0.$ Hence $\null\varphi\cap\hTextbra{au:a\in\Fbb}=\{0\}.$\par\vspace{6pt}\quad
(b) $\forall\,v\in V,v=\displaystyle\left(v-\frac{\varphi(v)}{\varphi(u)}u\right)+\frac{\varphi(v)}{\varphi(u)}u.$
$\left|\left.\begin{array}{l}\displaystyle v-\frac{\varphi(v)}{\varphi(u)}u\in\null\varphi\\[16pt]\displaystyle\frac{\varphi(v)}{\varphi(u)}u\in\hTextbra{au:a\in\Fbb}\end{array}\right\}\right.\Rightarrow V=\null\varphi\oplus\hTextbra{au:a\in\Fbb}.$\PfEnd\vspace{12pt}\large
\Comment \,\,\,$\varphi\neq 0\Rightarrow\varphi(v_i)=a_i\neq 0$ for each $v_i$, for some linely inde list $\List{v_1,\dots,v_k}.$\par\Blind{\Comment \,\,}
Fix one $v_k$. Then $\displaystyle\varphi\left(v_k-\frac{a_k}{a_j}v_j\right)=0$ for each $j=1,\dots,k-1,k+1,\dots,n$.\par\Blind{\Comment \,\,}
...
\ProblemN{\hypertarget{3B31}{31}}{
	\TextNL{Prove that $\exists\,T_1,T_2\in\Lm[\Large]{\Rbb^5,\Rbb^2},\null T_1=\null T_2$ and $T_1\neq cT_2,\forall c\in\Fbb$.}
}\par\quad
Let $\List{v_1,\dots,v_5}$ be a basis of $\Rbb^5$, $\List{w_1,w_2}$ be a basis of $\Rbb^2$. Define $T,S\in\Lm{V,W}$ by\par\vspace{6pt}\,
$\MathRightBrace{l}{$
...
\ProblemB{
	...
	\TextB{Show that if $\varphi:\Lm[\Large]{V}\rightarrow\Fbb$ is linear and $\forall S,T\in\Lm[\Large]{V},\varphi(ST) = \varphi(S)\cdot\varphi(T)$, then $\varphi = 0$.}
}Using notations in (4E 3.A.16).\par\quad
{Suppose $\varphi\neq 0\Rightarrow\exists\,i,j\in\!\hTextbra{1,\dots,n},\,\varphi(R_{i,j})\neq 0$.}\par\quad
...
{Again, because {\Large\vspace{4pt}$R_{i,x}=R_{y,x}\circ R_{i,y},\,\,\forall y=1,\dots,n.$} \;Thus $\varphi(R_{y,x})\neq 0,\;\forall x,y=1,\dots,n.$}\par\quad
{Let $l\neq i,k\neq j$ and then {\Large\vspace{4pt}$\varphi(R_{l,k}\circ R_{i,j})=\varphi(0)=0=\varphi(R_{l,k})\cdot\varphi(R_{i,j})$}}\par\quad
...
\ProblemBor{\hypertarget{3C4e17}4E 3.C.17, 3.F.32}{
	\TextB{Suppose $T\in\Lm[\Large]{V}$ and $\List{u_1,\dots,u_n},\List{v_1,\dots,v_n}$ are bases of \,$V$.}
	\TextB{Prove that the following are equi.\;\;{\normalsize Here $A=\Mt{T}$ means $\Mt[\normalsize]{T, \List{u_1,\dots,u_n},\List{v_1,\dots,v_n}}$.}}
	(a) \TextB{$T$ is inje.}
...
&\hspace{-18.5pt}\Delta\MathLeftBrace{l}{\Longleftrightarrow\List{Tu_1,\dots,Tu_n}\text{ is a basis of }V; 又 \dim\range T=\dim\Span{\Mt{Tu_1},\dots,\Mt{Tu_n}}=n\\
\Longleftrightarrow\List{\Mt{Tu_1},\dots,\Mt{Tu_n}}\text{ is a basis of }\Fbb^{n,1},\text{ as well as }\List{A_{\cdot,1},\dots,A_{\cdot,n}}}\\
...
&\Longleftrightarrow\hTextpar{A_{1,\cdot},\dots,A_{n,\cdot}}\text{ is a basis of }\Fbb^{1,n}.}\vspace{-25pt}\par\PfEnd\vspace{18pt}\par\quad
...
Then \vspace{-7.2pt}\AlignEq{}{Tu&=b_1Tu_1+\dots+b_nTu_n\hspace{280pt}\\&=b_1\hTextpar{A_{1,1}v_1+\dots+A_{n,1}v_n}+\dots+b_n\hTextpar{A_{1,n}v_1+\dots+A_{n,n}v_n}\\&=\hTextpar{b_1A_{1,1}+\dots+b_nA_{1,n}}v_1+\dots+\hTextpar{b_1A_{n,1}+\dots+b_nA_{n,n}}v_n\\&=0v_1+\dots+0v_n=0\\&\Rightarrow b_1=\dots=b_n=0.}\par\quad\Ha
Thus by (2.39), $(b)$ holds.\vspace{10pt}\par\quad
$(b)\,\Rightarrow(a):\;$\par\quad\Hb
Suppose $\;u=b_1u_1+\dots+b_nu_n=\in\null T.$\par\quad\Hb
Then $\;Tu=0=\hTextpar{b_1A_{1,1}+\dots+b_nA_{1,n}}v_1+\dots+\hTextpar{b_1A_{n,1}+\dots+b_nA_{n,n}}v_n.$\par\quad\Hb
...
\ProblemN{\hypertarget{3C1}{1}}{
	\TextN{Suppose $T\in\Lm[\Large]{V,W}$. Show that with resp to each choice of bases of \,$V$ and $W$,}
	\TextN{the matrix of $T$ has at least $\dim\range T$ nonzero entries.}
}\par\quad
Let $\List{v_1,\dots,v_p}$ be a basis of $\null T,$ extend to a basis $\List{v_1,\dots,v_n}$ of $V.$\par\quad
Let $\List{w_1,\dots,w_m}$ be basis of $W$. Denote $\Mt{T,\List{v_1,\dots,v_n},\List{w_1,\dots,w_m}}$ by $A.$\vspace{4pt}\par\quad
...
Suppose $A$ has at most $\hTextpar{\dim\range T-1}$ nonzero entries.\par\quad
Then by Pigeon Hole Principle, at least one of $A_{\cdot,p+1},\dots,A_{\cdot,n}$ equals $0$.\par\quad
Thus there are at most $\hTextpar{\dim\range T-1}$ nonzero vecs in $Tv_{p+1},\dots,Tv_n$.\par\quad
Hence $\range T=\hspace{-12pt}\underbrace{\Span{\;Tv_{p+1},\;\dots,\;Tv_n\;}}_{\text{at most }(\dim\range T-1)\text{ nonzero vecs}}$\par\vspace{6pt}\quad
$\Rightarrow\dim\range T=\dim\Span{Tv_{p+1},\dots,Tv_n}\leq\dim\range T-1.$ Contradicts.\PfEnd
\SepLine
\pagebreak

\ProblemN{\hypertarget{3C3}{3}}{
	\TextN{Suppose $V$ and $W$ are finite-dim and $T\in\Lm[\Large]{V,W}$. Prove that $\exists\,B_V,B_W$ such that}
	\TextN{[ letting $A=\Mt{T,B_V,B_W}$ ] $A_{k,k}=1,A_{i,j}=0$, where $1\leq k\leq \dim\range T, i\neq j$.}
}\par\quad
Let $R=\hTextrmNorm[\large]{\hTextpar[-5pt]}{Tv_1,\dots,Tv_n}$ be a basis of $\range T$, extend to  $B_W=(Tv_1,\dots,Tv_n,w_1,\dots,w_p)$.\par\quad
Let $\mathcal{K}_R=\Span{v_1,\dots,v_n}.$ Let $\List{u_1,\dots,u_m}$ be a basis of $\null T$. Then $B_V=\List{v_1,\dots,v_n,u_1,\dots,u_m}.$\PfEnd
\SepLine

\ProblemN{\hypertarget{3C4}{4}}{
	\TextN{Suppose $B_V=\List{v_1,\dots,v_m}$ and $W$ is finite-dim. Suppose $T\in\Lm[\Large]{V,W}$.\vspace{2pt}}
	\TextN{Prove that $\exists\,B_W=\List{w_1,\dots,w_n},\;\;\Mt{T,B_V,B_W}^t_{\cdot,1}=\begin{pmatrix}1&0&\cdots&0\end{pmatrix}$ or $\begin{pmatrix}0&\cdots&0\end{pmatrix}$.\vspace{4pt}}
}If $Tv_1=0$, then we are done. If not then extend $\List{Tv_1}.$\PfEnd
\SepLine

\ProblemN{\hypertarget{3C5}{5}}{
	\TextN{Suppose $B_W=\List{w_1,\dots,w_n}$ and $V$ is finite-dim. Suppose $T\in\Lm[\Large]{V,W}$.\vspace{2pt}}
	\TextN{Prove that $\exists\,B_V=\List{v_1,\dots,v_m},\;\;\Mt{T,B_V,B_W}_{1,\cdot}=\begin{pmatrix}0&\cdots&0\end{pmatrix}$ or $\begin{pmatrix}1&0&\cdots&0\end{pmatrix}$.}
}\par\quad
Let $\List{u_1,\dots,u_n}$ be a basis of $V$. Denote $\Mt{T,\List{u_1,\dots,u_n},B_W}$ by $A.$\par\quad
If $A_{1,\cdot}=0,$ then let $B_V=\List{u_1,\dots,u_n},$ we are done.\par\quad
...
Now because each $u_k\in\Span{v_1,\dots,v_n}\Rightarrow V=\Span{v_1,\dots,v_n},B_V=\List{v_1,\dots,v_n}.$\vspace{6pt}\par\quad
And $Tv_1=T\left(\displaystyle\frac{u_k}{A_{1,k}}\right)=\displaystyle\frac{1}{A_{1,k}}\hTextpar{A_{1,k}w_1+\dots+A_{n,k}w_n}=1w_1+\dots+\displaystyle\frac{A_{n,k}}{A_{1,k}}w_n.$\par\quad
\vspace{15pt}\AlignEq{}{\forall j\in\underset{\qquad\quad\overset{\uwave{\quad\qquad\qquad\qquad}}{\quad\;\;i\,\in\,\{\,k+1,\dots,n\,\}\quad}}{\hTextbra{2,\dots,k,{k+2,\dots,n+1}}},\;Tv_j=T\left(u_{j-1}-A_{1,j-1}v_1\right)=Tu_{j-1}-T\left(\displaystyle\frac{A_{1,j-1}u_k}{A_{1,k}}\right)\hspace{93pt}\\=A_{1,j-1}w_1+\dots+A_{n,j-1}w_n-A_{1,j-1}\left(1w_1+\dots+\displaystyle\frac{A_{n,k}}{A_{1,k}}w_n\right)=0w_1+\dots+\left(A_{n,j-1}-\displaystyle\frac{A_{1,j-1}A_{n,k}}{A_{1,k}}\right)w_n.}\PfEnd\vspace{4pt}\par
\SepLine

\ProblemN{\hypertarget{3C6}{6}}{
	\TextN{Suppose $V$ and $W$ are finite-dim and $T\in\Lm[\Large]{V,W}$.}
	\TextN{Prove that $\dim\range T = 1\Longleftrightarrow\exists\,B_V,B_W,$ all entries of $A=\Mt{T,B_V,B_W}$ equal $1$.}
}\par\quad
(a) Suppose $B_V=\List{v_1,\dots,v_n},B_W=(w_1,\dots,w_m)$ are the bases such that all entries of $A$ equal $1$.\vspace{6pt}\par\quad\Ha
...
By (2.C [\NEWTHEOREM]), $\exists$ a basis $\List{u_1,\dots,u_n}$ of $V$ such that each $u_k\not\in\null T.$\par\quad\Hb
$\forall k\in\!\hTextbra{1,\dots,n},Tu_k\in\range T=\Span{w}\Rightarrow Tu_k=\lambda_k w,\,\exists\,\lambda_k\in\Fbb\backslash\{0\}.$\par\quad\Hb
Let $v_k=\lambda_k^{-1}u_k\neq 0\Rightarrow B_V=\List{v_1,\dots,v_n}.$ Hence for each $v_k,Tv_k=w=w_1+\dots+w_m.$\PfEnd
\SepLine
...
\BulletPoint \,\NoteForSmall{[3.49]} \vspace{-11pt}\AlignEq{}{&\because[(AC)_{\cdot,k}]_{j,1}=(AC)_{j,k}=\sum\limits_{r=1}^n A_{j,r}C_{r,k}=\sum\limits_{r=1}^n A_{j,r}(C_{\cdot,k})_{r,1}=(AC_{\cdot,k})_{j,1}\hspace{-10pt}\\&\therefore(AC)_{\cdot,k}=A_{\cdot,\cdot}C_{\cdot,k}=AC_{\cdot,k}}\PfEnd[-25pt]
\SepLine
...
\BulletPoint \,\Exercise{\hypertarget{3C10}{10}} \vspace{-9pt}\AlignEq{}{&\because[(AC)_{j,\cdot}]_{1,k}=(AC)_{j,k}=\sum\limits_{r=1}^n A_{j,r}C_{r,k}=\sum\limits_{r=1}^n (A_{j,\cdot})_{1,r}C_{r,k}=(A_{j,\cdot}C)_{1,k}\\
	&\therefore(AC)_{j,\cdot}=A_{j,\cdot}C_{\cdot,\cdot}=A_{j,\cdot}C.}\PfEnd[-26pt]
\SepLine
...
$\because\hTextpar{Ac}_{j,1}=\sum_{r=1}^n A_{j,r}c_{r,1}=\left(\sum_{r=1}^n\hTextpar{A_{\cdot,r}c_{r,1}}\right){_{j,1}}=\hTextpar{c_1 A_{\cdot,1}+\dots+c_n A_{\cdot,n}}_{j,1}$\vspace{3pt}\par\quad
$\therefore\,Ac=A_{\cdot,\cdot}c_{\cdot,1}=\sum_{r=1}^n A_{\cdot,r}c_{r,1}=c_1 A_{\cdot,1}+\dots+c_n A_{\cdot,n}$\quad \Or By $(Ac)_{\cdot,1}=Ac_{\cdot,1}$ Using (a) above.\PfEnd
\SepLine

\BulletPoint \,\Exercise{\hypertarget{3C11}{11}}\quad$a\in\Fbb^{1,n},C\in\Fbb^{n,p}\Rightarrow aC\in\Fbb^{1,p}$\vspace{2pt}\par\quad
$\because\hTextpar{aC}_{1,k}=\sum_{r=1}^n a_{1,r}C_{r,k}=\left(\sum_{r=1}^n a_{1,r}\hTextpar{C_{r,\cdot}}\right){_{1,k}}=\hTextpar{a_1 C_{1,\cdot}+\dots+a_n C_{n,\cdot}}_{1,k}$\vspace{3pt}\par\quad
$\therefore\,aC=a_{1,\cdot}C_{\cdot,\cdot}=\sum_{r=1}^n a_{1,r}C_{r,\cdot}=a_1 C_{1,\cdot}+\dots+a_n C_{n,\cdot}$\quad \Or By $(aC)_{1,\cdot}=a_{1,\cdot}C$. Using (b) above.\PfEnd
\SepLine

\ProblemB{
	\TextB{Suppose $p$ is a poly of $n$ variables in $\Fbb$. Prove that $\Mt[\Large]{p\List[\Large]{T_1,\dots,T_n}}=p\List[\Large]{\Mt[\Large]{T_1},\dots,\Mt[\Large]{T_n}}.$}
	\TextB{\large Where the linear maps $T_1,\dots,T_n$ are such that $p\hTextrmLarge{\hTextpar}{T_1,\dots,T_n}$ makes sense. See [5.B.16,17,20].}
}\par\quad
Suppose the poly $p$ is defined by $p\List{x_1,\dots,x_n}=\sum_{k_1,\dots,k_n}\!\!\alpha_{k_1,\dots,k_n}\prod_{i=1}^n x_i^{k_i}.$\par\quad
Note that $\Mt{T^x S^y}=\Mt{T}^x\Mt{S}^y;\;\Mt{T^x+S^y}=\Mt{T}^x+\Mt{S}^y$.\par\quad
Then $\underbrace{\Mt{\sum_{k_1,\dots,k_n}\!\!\alpha_{k_1,\dots,k_n}\prod_{i=1}^n T_i^{k_i}}}_{\Mt{p\List(T_1,\dots,T_n}}=\underbrace{\sum_{k_1,\dots,k_n}\!\!\alpha_{k_1,\dots,k_n}\prod_{i=1}^n \Mt{T_i^{k_i}}}_{p\List{\Mt{T_1},\dots,\Mt{T_n}}}.$\PfEnd
\SepLine

\ProblemN{\hypertarget{3C13}{13}}{
	\TextNL{Prove that the distr holds for matrix add and matrix multi.}
	\TextNL{\large Suppose $A, B, C$ are matrices such that $A(B + C)$ make sense, we prove the left distr.}
}\par\quad
...
Or $T(S+R)=TS+TR\Rightarrow\Mt{T(S+R)}=\Mt{TS+TR}\Rightarrow A(B+C)=AB+AC.$\PfEnd
\SepLine

\ProblemN{\hypertarget{3C14}{14}}{
	\TextNL{Prove that matrix multi is associ.}
	\TextNL{\large Suppose $A,B,C$ are matrices such that $(AB)C$ makes sense, we prove that $(AB)C=A(BC).$}
}\par\quad
Suppose $A\in\Fbb^{m,n}$ and $B,C\in\Fbb^{n,p}.$\par\quad
...
\ProblemN{\hypertarget{3C15}{15}}{
	\TextNL{Suppose $A\in\Fbb^{n,n},j,k\in\hTextrmLarge{\hTextbra}{1,\dots,n}$. Show that $\hTextrmLarge{\hTextpar}{A^3}_{j,k}=\sum_{p=1}^n \sum_{r=1}^n A_{j,p}A_{p,r}A_{r,k}$.\vspace{6pt}}
}\vspace{-4pt}\AlignEq{}{\hTextpar{AAA}_{j,k} &= \hTextpar{AA}_{j,\cdot}A_{\cdot,k}=\textstyle\sum_{p=1}^n \hTextpar{A_{j,p}A_{p,\cdot}}A_{\cdot,k}=\textstyle\sum_{p=1}^n \sum_{r=1}^n A_{j,p}A_{p,r}A_{r,k}.\hspace{60pt}\\
\TXT{\Or}\,\,\,\,\hTextpar{AAA}_{j,k} &=\textstyle\sum_{r=1}^n\hTextpar{AA}_{j,r}A_{r,k} =\textstyle\sum_{r=1}^n\hTextpar{\sum_{p=1}^n A_{j,p}A_{p,r}}A_{r,k}\\&=\textstyle\sum_{r=1}^n\left[A_{j,1}\hTextpar{A_{1,r}A_{r,k}}+\dots+A_{j,n}\hTextpar{A_{n,r}A_{r,k}}\right]\\&=\textstyle A_{j,1}\sum_{r=1}^n A_{1,r}A_{r,k}+\dots+A_{j,n}\textstyle\sum_{r=1}^n A_{n,r}A_{r,k}=\textstyle\sum_{p=1}^n \sum_{r=1}^n A_{j,p}A_{p,r}A_{r,k}.}\PfEnd
\SepLine
...
Suppose $T\neq 0,$ and $v\in V\backslash\zeroSubsSmall[\large].$ Assume that $\hTextpar{v,Tv}$ is linely inde.\vspace{3pt}\par\quad
Extend $\hTextpar{v,Tv}$ to $B_V=\hTextpar{v,Tv,u_3,\dots,u_n}.$ Let $B=\Mt
\!\hTextpar{T,B_V}.$\vspace{3pt}\par\quad
$\Rightarrow Tv=B_{1,1} v+B_{2,1}\hTextpar[-5pt]{Tv}+B_{3,1}u_3+\dots+B_{n,1}u_n\Rightarrow B_{2,1}=1,B_{i,1}=0,\,\forall i\neq 2.$\vspace{3pt}\par\quad
By assumption, $A=\Mt{T,B_V'}=B,\forall B_V'=\List{v,w_2,\dots,w_{n}}$. Then $A_{2,1}=1,A_{i,1}=0,\forall i\neq 2.$\vspace{2pt}\par\quad
$\Rightarrow Tv=w_2,$ which is not true if we let $w_2=u_3,\;w_3=Tv,\;w_j=u_j,\forall j\in\!\hTextbra{4,\dots,n}.$ Contradicts.\vspace{2pt}\par\quad
Hence $\List{v,Tv}$ is linely depe $\Rightarrow \forall v\in V,\,\exists\,\lambda_v\in\Fbb,Tv=\lambda_v v.$\par\quad
Now we show that $\lambda_v$ is independent of $v$, that is, to show that for all $v\neq w\in V\backslash\zeroSubsSmall[\large],\lambda_v=\lambda_w$.\par\;
{$\MathRightBrace{l}{
...
\Or Conversely, denote $\Mt{T,B_V}$ by $A,$ where $B_V=\hTextpar{u_1,\dots,u_m}$ is arbitrary.\vspace{3pt}\par\quad
Fix one $B_V=\hTextpar{v_1,\dots,v_m}$ and then $\hTextpar{v_1,\dots,\frac{1}{2}v_k,\dots,v_m}$ is also a basis for any given $k\in\!\hTextbra{1,\dots,m}.$\vspace{3pt}\par\quad
Fix one $k.$ Now we have $T\!\left(\frac{1}{2}v_k\right)=A_{1,k}v_1+\dots+A_{k,k}(\frac{1}{2}v_k)+\dots+A_{m,k}v_m$\vspace{3pt}\par\quad
...
Then $A_{j,k}=2A_{j,k}\Rightarrow A_{j,k}=0$ for all $j\neq k.$ Thus $Tv_k=A_{k,k}v_k,\forall k\in\!\hTextbra{1,\dots,m}.$\vspace{3pt}\par\quad
Now we show that $A_{k,k}=A_{j,j}$ for all $j\neq k.$ Choose $j,k$ such that $j\neq k.$\vspace{3pt}\par\quad
Consider the basis $B_V'=(v\apostrophe_1,\dots,v\apostrophe_j,\dots,v\apostrophe_k,\dots,v\apostrophe_m),$\vspace{3pt}\par\qquad\qquad\quad\hspace{1pt}
where $v\apostrophe_j=v_k,\,\,v_k\apostrophe=v_j$ and $v\apostrophe_i=v_i$ for all $i\in\!\hTextbra{1,\dots,m}\hText{\left\backslash}{\right.}{\!\!}\hTextrmNorm[\large]{\hTextbra[-4pt]}{j,k}.$\vspace{3pt}\par\quad
Remember that $\Mt{T,B_V'}=\Mt{T,B_V}=A.$\vspace{3pt}\par\quad
Hence $T(v\apostrophe_k)=A_{1,k}v\apostrophe_1+\dots+A_{k,k}v\apostrophe_k+\dots+A_{m,k}v\apostrophe_m=A_{k,k}v\apostrophe_k=A_{k,k}v_j,$ while $T(v\apostrophe_k)=T(v_j)=A_{j,j}v_j.$\vspace{3pt}\par\quad
...
\ProblemBnosolus{
	\hypertarget{3D4e3}{}\TextB{Suppose $V$ is finite-dim and $T\in\Lm[\Large]{V}$.}
}$\MathRightBrace{l}{$
	$\hTextpar{Tv_1,\dots,Tv_n}$ is a basis of \,$V$ for some basis $\List{v_1,\dots,v_n}$ of \,$V$ $\Longleftrightarrow$ $T$ is surj$\\ $ $\hTextpar{Tv_1,\dots,Tv_n}$ is a basis of \,$V$ for every basis $\List{v_1,\dots,v_n}$ of \,$V$ $\Longleftrightarrow$ $T$ is inje
	$}\Longleftrightarrow$ $T$ is inv.\vspace{6pt}\par
\SepLine

\ProblemB{
	\hypertarget{3D4e15}{}\TextB{Suppose $T\in\Lm[\Large]{V},v_1,\dots,v_m\in V$ such that $V=\Span{Tv_1,\dots,Tv_m}$.}
...
\Or Reduce $\List{Tv_1,\dots,Tv_m}$ to a basis of $V$ as $(Tv_{\alpha_1},\dots,Tv_{\alpha_k})$, where $k=\dim V$ and $\alpha_i\in\!\hTextbra{1,\dots,k}$.\par\quad
Then $\hTextrmNorm[\large]{\hTextpar[-5pt]}{v_{\alpha_1},\dots,v_{\alpha_k}}$ is linely inde of length $k$, hence is a basis of $V$, contained in the list $\List{v_1,\dots,v_m}.$\PfEnd
\SepLine

\ProblemBor{\hypertarget{3D4e22}{10.A.1}}{
	\TextB{Suppose $T\in\Lm[\Large]{V},B_V=\List[\Large]{v_1,\dots,v_n}.$ Prove that $\Mt[\Large]{T, B_V}$ is inv $\Longleftrightarrow T$ is inv.}
...
\ProblemB{
	\hypertarget{3D4e1}{}\vspace{2pt}\TextB{Suppose $T\in\Lm[\Large]{V,W}$ is inv. Show that $T^{-1}$ is inv and $(T^{-1})^{-1}=T$.\vspace{4pt}}
}$\MathRightBrace{l}{
...
\ProblemN{\hypertarget{3D1}{1}}{
	\TextN{Suppose $T\in\Lm[\Large]{U,V},S\in\Lm[\Large]{V,W}$ are inv. Prove that $ST$ is inv and $(ST)^{-1} = T^{-1}S^{-1}$.\vspace{8pt}}
}$\MathRightBrace{l}{
...
\ProblemN{\hypertarget{3D2}{2}}{
	...
	\TextN{Prove that the set of non-inv operators on $V$ is not a subsp of $\Lm[\Large]{V}$.}
}Denote the set by $U.$\par\quad
Suppose $\dim V=n>1.$ Let $\List{v_1,\dots,v_n}$ be a basis of $V$. Define $S,T\in\Lm{V}$ by\par\qquad
$S\hTextrmNorm[\large]{\hTextpar[-5pt]}{a_1 v_1+\dots+a_n v_n}=a_1 v_1,T\hTextrmNorm[\large]{\hTextpar[-5pt]}{a_1 v_1+\dots+a_n v_n}=a_2 v_1+\dots+a_n v_n$. Hence $S+T=I$ is inv.\PfEnd
\Comment\,\,\, If $\dim V=1,$ then $U=\{0\}$ is a subsp of $\Lm{V}.$\par
\par
\SepLine

\ProblemN{\hypertarget{3D3}{3}}{
	\TextN{Suppose $V$ is finite-dim, $U$ is a subsp of \,$V$, and $S\in\Lm[\Large]{U,V}$.}
	\TextN{Prove that $\exists$ inv $T\in\Lm[\Large]{V},Tu = Su,\forall u\in U$ $\Longleftrightarrow$ $S$ is inje.{\large $[$ Compare this with (3.A.11). $]$}}
}\par\quad
(a) $Tu = Su$ for every $u\in U$ $\Rightarrow$ $u=T^{-1}Su\Rightarrow S$ is inje. \Or $\null S=\null T\cap U=\{0\}\cap U=\{0\}.$\par\quad
(b) Suppose $\List{u_1,\dots,u_m}$ be a basis of $U$ and $S$ is inje $\Rightarrow\List{Su_1,\dots,Su_m}$ is linely inde in $V$.\par\quad\Hb
Extend these to bases of $V$ as $(u_1,\dots,u_m,v_1,\dots,v_n)$ and $(Su_1,\dots,Su_m,w_1,\dots,w_n)$.\par\quad\Hb
Define $T\in\Lm{V}$ by $T(u_i)=Su_i;\,\,\,Tv_j=w_j,$ for each $i\in\!\hTextbra{1,\dots,m},j\in\!\hTextbra{1,\dots,n}.$\PfEnd
\SepLine

\ProblemN{\hypertarget{3D4}{4}}{
	\TextN{Suppose that $W$ is finite-dim and $S, T\in\Lm[\Large]{V,W}$.}
	\TextN{Prove that $\null S = \null T${\normalsize $(\,= U\,)$} $\Longleftrightarrow$ $S = ET,\,\,\exists$ inv $E\in\Lm[\Large]{W}$.}
}\par\quad
Define $E\in\Lm{W}$ by $E(Tv_i)=Sv_i,\,\,\,E(w_j)=x_j$, for each $i\in\!\hTextbra{1,\dots,m},j\in\!\hTextbra{1,\dots,n}.$ Where:\par\vspace{6pt}\normalsize\quad
$\MathLeftrightMid{l}{$
	Let $B_{\range T}=\Lm{Tv_1,\dots,Tv_m}$, extend to $B_W=(Tv_1,\dots,Tv_m,w_1,\dots,w_n).\\ $
	...
	Hence $B_{\range S}=\List{Sv_1,\dots,Sv_m}.$ Thus we let $B_W'=\List{Sv_1,\dots,Sv_m,x_1,\dots,x_n}.}\begin{array}{l}$\,$\therefore$ $E$ is inv and $S=ET$.$\end{array}$\par\vspace{6pt}\large\quad
...
\ProblemN{\hypertarget{3D5}{5}}{
	\TextN{Suppose that $V$ is finite-dim and $S, T\in\Lm[\Large]{V,W}$.}
	\TextN{Prove that $\range S = \range T${\normalsize $(\,= R\,)$} $\Longleftrightarrow$ $S = TE,\,\,\exists$ inv $E\in\Lm[\Large]{V}$.}
}\par\quad
Define $E\in\Lm{V}$ as \;$E:\,\,\, v_i\mapsto r_i\,\,;\quad u_j\mapsto s_j;$\quad for each $i\in\!\hTextbra{1,\dots,m},j\in\!\hTextbra{1,\dots,n}.$ Where:\par\vspace{6pt}\normalsize\quad
$\MathLeftrightMid{l}{$
	Let $B_R=\Lm{Tv_1,\dots,Tv_m};\;B_R'=(Sr_1,\dots,Sr_m)$ such that $\forall\,i,Tv_i=Sr_i.\\ $
	Let $B_{\null T}=\List{u_1,\dots,u_n};\;B_{\null S}=(s_1,\dots,s_n).\\ $
	Thus $B_V=\List{v_1,\dots,v_m,u_1,\dots,u_n};\;B_V'=\List{r_1,\dots,r_m,s_1,\dots,s_n}.}$\,$\therefore$ $E$ is inv and $S=TE$.\par\vspace{6pt}\large\quad
...
\ProblemN{\hypertarget{3D6}{6}}{
	\TextN{Suppose $V$ and $W$ are finite-dim and $S, T\in\Lm[\Large]{V,W}$.}
	\TextN{Prove that  $S=E_2 T E_1,\,\exists$ inv $ E_1\in\Lm[\Large]{V}, E_2\in\Lm[\Large]{W}$ $\Longleftrightarrow$ $\dim\null S = \dim\null T=n$.}
}\par\quad
$\TXT{Define}\;\,E_1:\,\,\,\,\, v_i\mapsto r_i\,\,;\quad u_j\mapsto s_j;$\quad for each $i\in\!\hTextbra{1,\dots,m},j\in\!\hTextbra{1,\dots,n}.$\par\quad
$\TXT{Define}\;\,E_2:Tv_i\mapsto Sr_i\,\,;\,\,x_j\mapsto y_j;$\quad for each $i\in\!\hTextbra{1,\dots,m},j\in\!\hTextbra{1,\dots,n}.$
Where:\par\vspace{6pt}\normalsize\quad
$\MathLeftrightMid{l}{$
	Let $B_{\range T}=\Lm{Tv_1,\dots,Tv_m};\;B_{\range S}=(Sr_1,\dots,Sr_m).\\ $
	Extend to $B_W=(Tv_1,\dots,Tv_m,x_1,\dots,x_p);\;B_W'=(Sr_1,\dots,Sr_m,y_1,\dots,y_p).\\ $
	Let $B_{\null T}=\List{u_1,\dots,u_n};\;B_{\null S}=(s_1,\dots,s_n).\\ $
	Thus $B_V=\List{v_1,\dots,v_m,u_1,\dots,u_n};\;B_V'=\List{r_1,\dots,r_m,s_1,\dots,s_n}.}$\,$\therefore$ $E_1,E_2$ are inv and $S=E_2 TE_1.$\par\vspace{6pt}\large\quad
...
Let $B_{\range T}=B_W=(w_1,\dots,w_m)\Rightarrow\forall w_i,\,\exists\,!\,v_i\in V,Tv_i=w_i.$ Let $B_{\mathcal{K}}=\List{v_1,\dots,v_m}$.\par\quad
...
$U\cap \null T=\{0\}=\null T|_U,\range T=\hTextbra{Tu:u\in U}=\range T|_U.$\PfEnd
\SepLine
\pagebreak

\ProblemN{\hypertarget{3D9}{9}}{
	\TextN{Suppose $V$ is finite-dim and $S, T\in\Lm[\Large]{V}$. Prove that
...
\Or Because by Problem (23) in 3.B, $\dim V=\dim\range ST\leq\min\hTextbra{\range T,\range S}$.\PfEnd
\SepLine

\ProblemN{\hypertarget{3D10}{10}}{
	\TextN{Suppose $V$ is finite-dim and $S,T\in\Lm[\Large]{V}.$ Prove that $ST=I\Longleftrightarrow TS=I.$}
}\par\quad
...
\ProblemN{\hypertarget{3D11}{11}}{
	\TextNL{Suppose $V$ is finite-dim, $S, T, U\in\Lm[\Large]{V}$ and $STU = I$. Show that $T$ is inv and $T^{-1} = US$.}
...
\ProblemN{\hypertarget{3D13}{13}}{
	\TextNL{Suppose $V$ is finite-dim, $R, S, T\in\Lm[\Large]{V}$ are such that $RST$ is surj. Prove that $S$ is inje.}
...
\ProblemN{\hypertarget{3D15}{15}}{
	...
	\TextNL{In other words, prove that if $T\in\Lm[\Large]{\Fbb^{n,1},\Fbb^{m,1}}$, then $\,\exists\,A\in\Fbb^{m,n},Tx = Ax,\,\forall x\in\Fbb^{n,1}$.}
}\par\quad
...
\BulletPoint \,\hspace{1pt}\NoteFor{[3.60]}\;\;Suppose $\List{v_1,\dots,v_n}$ is a basis of $V$ and $\List{w_1,\dots,w_m}$ is a basis of $W$.\TextB{\vspace{10pt}}
...
{\Large Denote $\Mt[\Large]{E_{i,j}}$ by $\mathcal{E}^{(j,i)}.$ And $\List[\Large]{\mathcal{E}^{(j,i)}}_{l,k}=\MathLeftBrace{l}{0,\quad i\neq k\,\vee\,j\neq l\\1,\quad i=k\,\wedge\,j=l}$}\TextB{\vspace{6pt}}
{\Large Because $\Lm[\Large]{V,W}$ and $\Fbb^{m,n}$ are iso. And $T=\mathcal{M}^{-1}\Mt{T};\;\;E_{i,j}=\mathcal{M}^{-1}\mathcal{E}^{(j,i)}$.}\TextB{}
Hence $\forall T\in\Lm{V,W},\,\,\exists\,!\,A_{i,j}\in\Fbb\hTextpar{\forall i\in\!\hTextbra{1,\dots,m},j\in\!\hTextbra{1,\dots,n}},\Mt{T}=A\,=\,${\normalsize$\begin{pmatrix} A_{1,1} & \cdots & A_{1,n}\\ \vdots & \ddots & \vdots\\ A_{m,1} & \cdots & A_{m,n}\end{pmatrix}$}.\par\quad
...
$\therefore\,\Lm{V,W}=\Spn${\normalsize$\underbrace{\begin{pmatrix} \underset{,}{E_{1,1}}, & \cdots & ,\underset{,}{E_{n,1}},\\ \vdots & \ddots & \vdots\\ \overset{,}{E_{1,m}}, & \cdots & ,\overset{,}{E_{n,m}}\end{pmatrix}}_{B}$}$;\quad\Fbb^{m,n}=\Spn${\normalsize$\underbrace{\begin{pmatrix} \underset{,}{\mathcal{E}^{(1,1)}}, & \cdots & ,\underset{,}{\mathcal{E}^{(1,n)}},\\ \vdots & \ddots & \vdots\\ \overset{,}{\mathcal{E}^{(m,1)}}, & \cdots & ,\overset{,}{\mathcal{E}^{(m,n)}}\end{pmatrix}}_{B_{\mathcal{M}}}$}.\TextB{\vspace{12pt}}
Hence by [2.42] and [3.61], we conclude that $B$ is a basis of $\Lm{V,W}$ and that $B_{\mathcal{M}}$ is a basis of $\Fbb^{m,n}.$\par
\SepLine

\ProblemB{
	\hypertarget{3D4e10}{}\TextB{Suppose $V,W$ are finite-dim, $U$ is a subsp of $V$. Let $\mathcal{E} = \hTextbra{T\in\Lm[\Large]{V,W}: U\subseteq\null T}$.}
	(a) \TextB{Show that $\mathcal{E}$ is a subsp of $\Lm[\Large]{V,W}$.}
...
\ProblemBc{
	\hypertarget{3D4e17}{}\TextB{Suppose $V$ is finite-dim and $S\in\Lm[\Large]{V}$. Define $A\in \Lm[\Large]{\Lm[\Large]{V}}$ by $\mathcal{A}(T) = ST,\forall T\in\Lm[\Large]{V}$.}
	(a) \TextB{Show that $\dim\null \mathcal{A} = (\dim V)(\dim \null S)$.}
	(b) \TextB{Show that $\dim\range \mathcal{A} = (\dim V)(\dim \range S)$.}
}\par\quad
(a) {\Large\vspace{3pt}For all $T\in\Lm[\Large]{V},$ $ST=0\Longleftrightarrow \range T\subseteq\null S.$}\par\quad\Ha
{\Large\vspace{6pt}Thus $\null\mathcal{A}=\hTextbra{T\in\Lm[\Large]{V}:\range T\subseteq\null S}=\Lm[\Large]{V,\null S}.$}\par\quad
(b) {\Large\vspace{3pt}For all $R\in\Lm[\Large]{V},$ $\range R\subseteq\range S \Longleftrightarrow\exists\,T\in\Lm[\Large]{V},R=ST,$ by (3.B 25).}\par\quad\Hb
{\Large\vspace{6pt}Thus $\range\mathcal{A}=\hTextbra{R\in\Lm[\Large]{V}:\range R\subseteq\range S}=\Lm[\Large]{V,\range S}.$}\PfEnd\quad
\Or Using \NOTEFOR\;[3.60].\par\quad
Let $\List{w_1,\dots,w_m}$ be a basis of $\range S$, extend it to a basis of $V$ as $\List{w_1,\dots,w_m,\dots,w_n}.$\par\quad
Let $v_i\in V$ such that $Sv_i=w_i$ for $m=1,\dots,m$. Extend $\List{v_1,\dots,v_m}$ to a basis of $V$ as $\List{v_1,\dots,v_m,\dots,v_n}.$\par\quad
{\Large Define $E_{i,j}\in\Lm[\Large]{V}$ by $E_{i,j}(v_x)=\delta_{ix} w_i$.}\par\quad
{\Large\vspace{6pt}Thus $S=E_{1,1}+\dots+E_{m,m};\quad\Mt{S,\List{v_1,\dots,v_n},\List{w_1,\dots,w_n}}\,=\,${\normalsize$\begin{pmatrix} \mathbb{1} & 0 & \cdots & 0 & 0 & \cdots & 0\\ 0 & \mathbb{1} & \cdots & 0 & 0 & \cdots & 0\\ \vdots & \vdots & \ddots & \vdots & \vdots & \ddots & \vdots\\ 0 & 0 & \cdots & \mathbb{1} & 0 & \cdots & 0\\ 0 & 0 & \cdots & 0 & 0 & \cdots & 0\\ \vdots & \vdots & \ddots & \vdots & \vdots & \ddots & \vdots\\ 0 & 0 & \cdots & 0 & 0 & \cdots & 0\end{pmatrix}.$}}\vspace{-30pt}\par\quad
{\Large\vspace{6pt}Define $R_{i,j}\in\Lm[\Large]{V}$ by $R_{i,j}(w_x)=\delta_{ix} v_i$.}\par\quad
{\Large\vspace{6pt}Let $E_{j,k}R_{i,j}=Q_{i,k},\quad R_{j,k}E_{i,j}=G_{i,k}.$}\par\quad
{\Large\AlignEq{}{\text{Because }\forall T\in\Lm[\Large]{V},\,\exists\,!\,A_{i,j}\in&\Fbb,\hspace{313pt}\\[-40pt]&T\,=\,{\normalsize\begin{pmatrix}
...
\BulletPoint \,\hspace{1pt}\Comment\,\,\, {\Large\vspace{6pt}Define $\mathcal{B}\in\Lm[\Large]{\Lm[\Large]{V}}$ by $\mathcal{B}(T)=TS,\forall T\in\Lm[\Large]{V}.$ Similarly to Problem ($\circ$),}\par\quad
(a) {\Large\vspace{3pt}For all $T\in\Lm[\Large]{V},TS=0\Longleftrightarrow\range S\subseteq\null T.$}\par\quad\Ha
{\Large\vspace{6pt}Thus $\null\mathcal{B}=\hTextbra{T\in\Lm[\Large]{V}:\range S\subseteq\null T}.$}\par\quad
(b) {\Large\vspace{3pt}For all $R\in\Lm[\Large]{V},\null S\subseteq\null R\Longleftrightarrow\exists\,T\in\Lm[\Large]{V},R=TS,$ by (3.B.24).}\par\quad\Hb
{\Large\vspace{6pt}Thus $\range\mathcal{B}=\hTextbra{R\in\Lm[\Large]{V}:\null S\subseteq\null R}.$}\par\quad
...
\ProblemN{\hypertarget{3D17}{17}}{
	\TextNL{Suppose $V$ is finite-dim. Show that the only two-sided ideals of $\Lm[\Large]{V}$ are $\{0\}$ and $\Lm[\Large]{V}$.}
	\TextNL{{\large A subsp $\mathcal{E}$ of $\Lm[\Large]{V}$ is called a two-sided ideal of $\Lm[\Large]{V}$ if $TE\in\mathcal{E},ET\in\mathcal{E},\,\,\forall E\in\mathcal{E},T\in\Lm[\Large]{V}$.\vspace{-2pt}}}
}
Using \NOTEFOR\;[3.60]. Let $\List{v_1,\dots,v_n}$ be a basis of $V$. If $\mathcal{E}=0$, then we are done.\par\quad Suppose $\mathcal{E}\neq 0$ and $\mathcal{E}$ is a two-sided ideal of $\Lm{V}$.\vspace{3pt}\par\quad
...
\ProblemBor{\hypertarget{3D4e23}{10.A.4}}{
	...
	\TextB{Let $T\in\Lm[\Large]{V}$ be such that $T\alpha_k = \beta_k,\forall k.$ Prove that $\Mt{T,\alpha\rightarrow\alpha}=\Mt{I,\beta\rightarrow\alpha}$}
	\TextB{\large For ease of notation, let $\Mt{T,\alpha\rightarrow\beta}=\Mt{T,(\alpha_1,\dots,\alpha_n),(\beta_1,\dots,\beta_n)},\,\,\,\Mt{T,\alpha\rightarrow\alpha}=\Mt{T,(\alpha_1,\dots,\alpha_n)}.$}
}\par\quad
Denote $\Mt{T,\alpha\rightarrow\alpha}$ by $A$ and $\Mt{I,\beta\rightarrow\alpha}$ by $B.$\par\quad
$\forall k\in\!\hTextbra{1,\dots,n},Iu_k=u_k=B_{1,k}\alpha_1+\dots+B_{n,k}\alpha_n=Tv_k=A_{1,k}\alpha_1+\dots+A_{n,k}\alpha_n\Rightarrow A=B.$\PfEnd\quad
...
$u_k=Iu_k=B_{1,k}\alpha_1+\dots+B_{n,k}\alpha_n,\,\,\forall \,\,k\in\!\hTextbra{1,\dots,n}.$\par\quad
...
\ProblemN{\hypertarget{3D16}{16}}{
	\TextNL{Suppose $V$ is finite-dim and $S\in\Lm[\Large]{V}$.}
	\TextNL{Prove that $\exists\,\lambda\in\Fbb,S=\lambda I\Longleftrightarrow$ $ST=TS$ for every $T\in\Lm[\Large]{V}$.}
...
Let $S=E_{1,1}+\dots+E_{m,m}\Rightarrow \Mt{S,\List{v_1,\dots,v_n}}=\Mt{I,\List{w_1,\dots,w_n},\List{v_1,\dots,v_n}}$.\par\quad
...
$a_{i,j}=\Mt{I,\List{w_1,\dots,w_n},\List{v_1,\dots,v_n}}_{i,j}\Longleftrightarrow w_i=Iw_i=a_{1,i} v_1 +\dots+a_{n,i} v_n;$\par\qquad
For each $j$, for all $i.$ Thus $a_{i,i}=a_{k,k}=\lambda, \forall\, k\neq i.$\par\quad
Hence $w_i=\lambda v_i\Rightarrow\Mt{S}\,=\,${\small$\begin{pmatrix}
\lambda & & 0\\
& \ddots \\
0 & & \lambda
\end{pmatrix}$}$\,=\,\Mt{\lambda I,\List{v_1,\dots,v_n}}\Rightarrow S=\mathcal{M}^{-1}\List{\Mt{\lambda I}}\lambda I.$\PfEnd
\SepLine

\ProblemN{\hypertarget{3D18}{18}}{
	\TextNL{Show that $V$ and $\Lm[\Large]{\Fbb, V}$ are iso vecsps.}
}\par\quad
Define $\Psi\in\Lm{V,\Lm{\Fbb, V}}$ by $\Psi(v)=\Psi_v;$ where $\Psi_v\in\Lm{\Fbb, V}$ and $\Psi_v(\lambda)=\lambda v.$\par\quad
...
Define $T_n:\PoR{n}\rightarrow\PoR{n}$ by $T_n(p)=(x^2 + x)p\apostrophe\apostrophe(x) + 2xp\apostrophe(x) + p(3).$ Then $T_n\in\Lm{\PoR{n}}$.\par\quad
...
\ProblemN{\hypertarget{3D19}{19}}{
	\TextNL{Suppose $T\in\Lm[\Large]{\PoR{}}$ is inje. $\deg Tp\leq\deg p$ for every nonzero $p\in\PoR{}$.}
...
For any $k\in\!\hTextbra{1,\dots,m},$ define $p_k:V_1\times\dots\times V_m\rightarrow V_k$ by $p_k\List{v_1,\dots,v_m}=v_k.$\par\quad
...
For if not, $U_1\times U_2$ is finite-dim and $\dim(U_1\times U_2)=\dim(U_1+U_2)=\dim U_1+\dim U_2.$}\par\quad
{\tgsl And $V$ must be infinite-dim. For if not, both $U_1$ and $U_2$ are finite-dim subsps.}\par\quad
Let $V=\Fbb^\infty=U_1,U_2=\hTextbra{(x,0,\cdots)\in\Fbb^\infty:x\in\Fbb}.$\par\quad
$\MathRightBrace{l}{$
...
\ProblemN{4}{
	\TextN{Prove that $\Lm[\Large]{V_1\times\cdots\times V_m,W}$ and $\Lm[\Large]{V_1,W}\times\cdots\times\Lm[\Large]{V_m,W}$ are iso.}
}Using the notation in Problem (2).\par\quad
Note that $T\List{u_1,\dots,u_m}=T(u_1,0,\dots,0)+\dots+T(0,\dots,u_m)$.\par\quad
$\MathRightBrace{l}{$
...
\ProblemN{5}{
	\TextN{Prove that $\Lm[\Large]{V,W_1\times\cdots\times W_m}$ and $\Lm[\Large]{V,W_1}\times\cdots\times\Lm[\Large]{V,W_m}$ are iso.}
}Using the notation in Problem (2).\par\quad
Note that $Tv=\List{w_1,\dots,w_m}$. Define $T_i\in\Lm{V,W_i}$ by $T_i(v)=w_i$.\par\quad
$\MathRightBrace{l}{$
...
\ProblemN{6}{
	\TextN{For $m\in\Nbp,$ define $V^m$ by $\mathop{\underbrace{V\times\cdots\times V}}\limits_{m\,\,times}$. Prove that $V^m$ and $\Lm[\Large]{\Fbb^m,V}$ are iso.}
}\par\quad
Define $T:\List{v_1,\dots,v_m}\rightarrow\varphi$, where $\varphi:(a_1,\dots,a_m)\mapsto v$ is defined by $\varphi(a_1,\dots,a_m)=a_1 v_1+\dots+a_m v_m.$\par\quad
(a) Suppose $T\List{v_1,\dots,v_m}=0.$ Then $\forall\,(a_1,\dots,a_n)\in\Fbb^m$, $\varphi(a_1,\dots,a_m)=a_1 v_1+\dots+a_m v_m=0$\par\quad\Ha
$\Rightarrow \List{v_1,\dots,v_m}=0\Rightarrow T$ is inje.\par\quad
...
\ProblemBnosolus{
	\TextB{Let $U=\hTextbra{(x,y,z)\in\Rbb^3:2x+3y+5z=0}.$ Suppose $A\subseteq\Rbb^3$.}
	\TextB{Then $A$ is a translate of $U\Longleftrightarrow\exists\,c\in\Rbb,A=\hTextbra{(x,y,z)\in\Rbb^3:2x+3y+5z=c}.$\vspace{-6pt}}
}\SepLine

\ProblemB{
	\TextB{Suppose $T\in\Lm[\Large]{V,W}$ and $c\in W$. Prove that $U=\left\{x\in V:Tx=c\right\}$ is either $\varnothing$}
...
Suppose $A=a+U.$ Then $\forall a+u_1,a+u_2\in A,\lambda\in\Fbb,\lambda(a+u_1)+(1-\lambda)(a+u_2)=a+[\lambda(u_1-u_2)+u_2]\in A$.\vspace{8pt}\par\quad
Suppose $\lambda v+(1-\lambda)w\in A,\forall v,w\in A,\lambda\in\Fbb.$ Suppose $a\in A$ and let $A\apostrophe=\hTextbra{x-a:x\in A}$.\vspace{6pt}\par\quad
Then $0\in A\apostrophe$ and $\,\,\forall x-a,y-a\in A\apostrophe,\;(\,\forall x,y\in A\,),\;\lambda\in\Fbb,$\par\quad
...
Thus $A\apostrophe$ is a subsp of $V$. Hence $a+A\apostrophe=\hTextbra{(x-a)+a:x\in A}{}=A$ is a translate.\PfEnd\vspace{6pt}\quad
\Or Suppose $x-a,y-a\in A\apostrophe,\lambda\in\Fbb.$\par\quad
Note that $x,a\in A\Rightarrow \lambda x+(1-\lambda)a=2x-a\in A.$ Similarly $2y-a\in A.$\par\quad
(I) $(x-\frac{1}{2}a) + (y-\frac{1}{2}a)=x+y-a\in A\Rightarrow x+y-2a=(x-a)+(y-a)\in A\apostrophe.$\par\quad
(II) $\lambda(x-a)=\left(\lambda x+(1-\lambda)a\right)-a\in A\apostrophe.$\par\quad
Thus $-x+A$ is a subsp of $V$. Hence $A=x+(-x+A)$ is a translate of the subsp $-x + A$.\PfEnd
\SepLine
...
$\forall\lambda\in\Fbb,\lambda(v+u_1)+(1-\lambda)(w+u_2)\in A_1$ and $A_2$. Thus $A_1\cap A_2$ is a translate of some subsp of $V.$\PfEnd\vspace{6pt}\quad
\Or Let $A_1=v+U_1,A_2=w+U_2.$ Suppose $x\in (v+U_1)\cap (w+U_2)\neq\varnothing.$\par\quad
Then $\exists\,u_1\in U_1,x=v+u_1\Rightarrow x-v\in U_1,\;\;\exists\,u_2\in U_2,x=w+u_2\Rightarrow x-w\in U_2$.\par\quad
Note that by [3.85], $A_1=v+U_1=x+U_1,A_2=w+U_2=x+U_2.$ We show that $A_1\cap A_2=x+(U_1\cap U_2).$\par\quad
(a) $y\in A_1\cap A_2\Rightarrow\exists\,u_1\in U_1,u_2\in U_2,y=x+u_1=x+u_2\Rightarrow u_1=u_2\in U_1\cap U_2\Rightarrow y\in x+(U_1\cap U_2).$\par\quad
(b) $y=x+u\in x+(U_1\cap U_2)=(x+U_1)\cap (x+U_2)\Rightarrow y\in A_1\cap A_2.$\PfEnd
\SepLine
...
Suppose $\hTextbra{A_{\alpha}}{_{\alpha\in\Gamma}}$ is a collection of translates of subsps of $V$, where $\Gamma$ is an arbitrary index set.\vspace{8pt}\par\quad
Suppose $x,y\in\bigcap_{\alpha\in\Gamma}A_{\alpha}\neq\varnothing$, then by Problem (8), $\forall\lambda\in\Fbb,\lambda x+(1-\lambda)y\in A_{\alpha}$ for every $\alpha\in\Gamma$.\par\quad
Thus $\bigcap_{\alpha\in\Gamma}A_\alpha$ is a translate of some subsp of $V$.\PfEnd\vspace{6pt}\quad
\Or Let $A_\alpha=w_\alpha+V_\alpha$ for each $\alpha\in\Gamma$. Suppose $x\in\bigcap_{\alpha\in\Gamma}(w_\alpha+V_\alpha)\neq\varnothing.$\par\quad
...
(b) $y=x+v\in x+\bigcap_{\alpha\in\Gamma}V_\alpha=\bigcap_{\alpha\in\Gamma}(x+V_\alpha)\Rightarrow y\in\bigcap_{\alpha\in\Gamma}A_\alpha.$ Hence $\bigcap_{\alpha\in\Gamma}A_\alpha=x+\bigcap_{\alpha\in\Gamma}V_\alpha.$\PfEnd
\SepLine

\ProblemN{11}{
	\TextNL{Suppose $A=\hTextbra{\lambda_1 v_1+\dots+\lambda_m v_m:\sum\limits_{i=1}^m \lambda_i=1}$, where each $v_i\in V,\lambda_i\in\Fbb.$}
...
(c) Fix a $k\in\!\hTextbra{1,\dots,m}.$ Given $\lambda_i\in\Fbb\hTextpar{i\in\!\hTextbra{1,\dots,m}\backslash\!\hTextbra{k}}.$\par\quad\Hc
...
\ProblemB{
	\TextB{Suppose $V=U\oplus W$, $\List[\Large]{w_1,\dots,w_m}$ is a basis of $W$.}
	\TextB{Prove that $\List[\Large]{w_1+U,\dots,w_m+U}$ is a basis of $V/U$.}
}\par\quad
...
\ProblemN{13}{
	\TextNL{Suppose $(v_1+U,\dots,v_m+U)$ is a basis of \,$V/U$ and $\List{u_1,\dots,u_n}$ is a basis of $U$.}
...
\ProblemN{15}{
	\TextNL{Suppose $\varphi\in\Lm[\Large]{V,\Fbb}\backslash\{0\}$. Prove that $\dim V/(\null\varphi)=1$.}
...
%Then for any $v\in V,v+U=\hTextrmNorm[\large]{\hTextpar[-5pt]}{a_1 v_1+\dots+a_n v_n}+U,\,\,\exists\,!\,\,n\in\Nbp,(a_1,\dots,a_n)\in\Fbb^n$.\par
%\Largesl{
...
\ProblemN{16}{
	\TextNL{Suppose $\dim V/U=1$. Prove that $\,\,\exists\,\,\varphi\in\Lm[\Large]{V,\Fbb}$ such that null $\varphi=U.$}
}\par\quad
...
}Let $\List{w_1,\dots,w_n}$ be a basis of \,$W$\par\quad
...
reduce $\List{w_1+U,\dots,w_n+U}$ to a basis of $V/U$ as $\List{w_1+U,\dots,w_m+U}$ and let $W=\Span{w_1,\dots,w_m}.$\PfEnd
\SepLine

\ProblemN{18}{
	\TextNL{Suppose $T\in\Lm[\Large]{V,W}$ and $U$ is a subsp of \,$V$. Let $\pi$ denote the quotient map.}
	\TextNL{Prove that $\,\exists\,\,S\in\Lm[\Large]{V/U,W}$ such that
...
\ProblemNnosolus{20}{
	\TextNL{Define $\Gamma:\Lm[\Large]{V/U,W}\rightarrow\Lm[\Large]{V,W}$ by $\Gamma(S)=S\circ\pi$ $\left(\,= \pi\apostrophe (S)\right)$.}
	(a) \TextNL{Prove that $\Gamma$ is linear: {\large\tgnr By [3.9] distr and [3.6].}}
	(b) \TextNL{Prove that $\Gamma$ is inje:}
	\Hb\TextNL{{\large $\Gamma(S)=0=S\circ\pi\Longleftrightarrow\forall v\in V,S\left(\pi(v)\right)=0\Longleftrightarrow\forall v+U\in V/U,S(v+U)=0\Longleftrightarrow S=0.$}}
	(c) \TextNL{Prove that $\range\Gamma\,(=\range \pi \apostrophe\,)=\hTextbra{T\in\Lm[\Large]{V,W}:U\subseteq\null T}$: {\normalsize\tgnr By Problem (18).\PfEnd}}
}\SepLine
\ChEnd
...
\TextB{$\varphi:V\rightarrow\Lm[\Large]{\Fbb,V}$ is an iso. Now we prove that}
\TextB{${v_1,\dots,v_m}$ is linely inde $\Longleftrightarrow\List{\varphi(v_1),\dots,\varphi(v_m)}$ is linely inde.}}\par\quad
(a) Suppose $\List{v_1,\dots,v_m}$ is linely inde and $\vartheta\in\Span{\varphi(v_1),\dots,\varphi(v_m)}$.\par\quad\Ha
...
\Or Because $\varphi$ is inje. Suppose $a_1\varphi(v_1)+\dots+a_m\varphi(v_m)=0=\varphi\hTextrmNorm[\large]{\hTextpar[-5pt]}{a_1 v_1+\dots+a_m v_m}.$\par\quad\Ha
...
(b) Suppose $\left(\varphi(v_1),\dots,\varphi(v_m)\right)$ is linely inde and $v\in\Span{v_1,\dots,v_m}.$\par\quad\Hb
...
\ProblemB{
	\TextB{Suppose $T \in \Lm[\Large]{V,W}$ and $(w_1 , \dots , w_m)$ is a basis of $\range T$.}
...
\ProblemN{9}{
	\TextN{Suppose $\List{v_1,\dots,v_n}$ is a basis of \,$V$ and $(\varphi_1,\cdots,\varphi_n)$ is the correspd dual basis of $V\apostrophe$.}
...
\Comment\,\,\, For other basis $\List{u_1,\dots,u_n}$ and the dual basis $(\rho_1,\dots,\rho_n),\,\,\psi=\psi(u_1)\rho_1+\dots+\psi(u_n)\rho_n.$\par
\SepLine

\ProblemN{35}{
	\TextNL{Prove that $\left(\PoR{}\right)\apostrophe$ and $\Rbb^{\infty}$ are iso.}
}\par\quad
{\Large\vspace{6pt}Define $\theta\in\Lm[\Large]{\left(\PoR{}\right)\apostrophe,\Rbb^{\infty}}$ by $\theta(\varphi)=\left(\varphi(1),\varphi(x),\dots,\varphi(x^n),\dots\right).$}\par\quad
...
\ProblemNnosolus{13}{
	...
	(a) \TextNL{Describe the linear functionals $T\apostrophe(\varphi_1),T\apostrophe(\varphi_2)\in\Lm[\Large]{\Rbb^3,\Rbb}$\vspace{3pt}}
...
\ProblemBnosolus{
	\TextB{Suppose $W$ is finite-dim, $T \in \Lm[\Large]{V,W}$.}
...
\ProblemBnosolus{
	\TextB{Suppose $V,W$ are finite-dim, $T\in\Lm[\Large]{V,W}$.}
	\TextB{Then by [3.108] and [3.110], $T$ is inv $\Longleftrightarrow$ $T\apostrophe$ is inv.}
}\SepLine
 
\ProblemN{16}{
	\TextNL{Suppose $V$ and $W$ are finite-dim. Define $\Gamma$ by $\Gamma(T)=T\apostrophe$ for any $T\in\Lm[\Large]{V,W}$.}
	\TextNL{Prove that $\Gamma$ is an iso of $\Lm[\Large]{V,W}$ onto $\Lm[\Large]{W\apostrophe,V\apostrophe}$.}
}\par\quad
...
\ProblemBnosolus{
	\Corollary \,\,\,\TextB{\vspace{4pt}Suppose $\hTextbra{V_{\alpha_i}}{_{\alpha_i\in\Gamma}}$ is a collection of subsps of\, $V$.}\Blind{\Corollary\,\,\,}
	\TextB{Then $\left(\sum_{\alpha_i\in\Gamma}V_{\alpha_i}\right)${$^{0}$}$\,=\bigcap_{\alpha_i\in\Gamma}\left(V^0_{\alpha_i}\right)$;\quad And $\left(\bigcap_{\alpha_i\in\Gamma}V_{\alpha_i}\right)${$^{0}$}$\,=\sum_{\alpha_i\in\Gamma}\left(V^0_{\alpha_i}\right)$.}
}\SepLine
...
Let $\List{u_1,\dots,u_m}$ be a basis of $U$, extend to a basis of $V$ as $(u_1,\dots,u_m,\dots,u_n)$,\par\quad
...
For all $u\in U$, $\varphi(u)=0$. Thus $\varphi\in U^0,$ getting $\Span{\varphi_{m+1},\dots,\varphi_n}\subseteq U^0.$\par\quad
...
\ProblemN{27}{
	\TextNL{Suppose $T\in\Lm[\Large]{\PoR{5}}$ and $\null T\apostrophe=\Span[\Large]{\varphi}$, where $\varphi\in\Lfm[\Large]{\left(\PoR{5}\right)\!}$}
	\TextNL{defined by $\varphi(p)=p(8)$. Prove that $\range T=\hTextbra{p\in \PoR{5}:p(8)=0}.$}
}\par\quad
By Problem (26), $\Span{\varphi}=\hTextbra{p\in\PoR{5}:\psi(p)=0,\forall \psi\in\Span{\varphi}}{^0},$\par\quad
Hence $\Span{\varphi}=\hTextbra{p\in\PoR{5}:\varphi(p)=p(8)=0}{^0}$, 又 $\Span{\varphi}=\null T\apostrophe=\hTextrmNorm[\large]{\hTextpar[-5pt]}{\range T}{^0}$.\par\quad By the corollary in Problem (20, 21), $\range T=\left\{p\in\PoR{5}:p(8)=0\right\}.$\PfEnd
\SepLine

\ProblemN{28, 29}{
	\TextNL{Suppose $V,W$ are finite-dim, $T\in\Lm[\Large]{V,W}$.}
...
Because $\Span{\varphi}=\left\{v\in V:\forall\psi\in\Span{\varphi},\psi(v)=0\right\}{^0}=\left\{v\in V:\varphi(v)=0\right\}{^0}=(\null\varphi)^0$.\par\quad
(a) $(\range T)^0=\null T\apostrophe=\Span{\varphi}=(\null \varphi)^0$ $\Longleftrightarrow\range T=\null \varphi.$\par\quad
(b) $(\null T)^0=\range T\apostrophe=\Span{\varphi}=(\null \varphi)^0$ $\Longleftrightarrow\null T=\null\varphi.$\PfEnd
\SepLine
...
Thus $\List{u_1,\dots,u_m}$ is linely inde, for if not, then $\exists\,i,j$ such that $\null\varphi_i=\null \varphi_j$, contradicts.\par\quad
又 $\dim V\apostrophe=m=\dim V$. Then $\List{u_1,\dots,u_m}$ is a basis of $V$ whose dual basis is $(\varphi_1,\dots,\varphi_n).$\PfEnd
\SepLine
...
$\MathRightBrace{r}{$
$\left((\null \varphi_1)\cap\dots\cap(\null \varphi_m)\right)^0=(\null \varphi_1)^0+\dots+(\null \varphi_m)^0$.$ \\ $
又 $\Span{\varphi_i}=\hTextbra{v\in V:\forall\psi\in\Span{\varphi_i},\psi(v)=0}{^0}=\hTextrmNorm[\large]{\hTextpar[-5pt]}{\null \varphi_i}{^0}.}\Rightarrow(a)=(b).$\PfEnd\vspace{10pt}
...
\ProblemN{6}{
	...
	(b) \TextN{Show that $\List{v_1,\dots,v_m}$ is linely inde $\Longleftrightarrow \Gamma$ is surj.}
}\par\quad
(a) $\MathLeftBrace{l}{$
...
$}$\par\quad
(b) $\MathLeftBrace{l}{$
Suppose $\Gamma$ is surj. Then let $\Gamma(\varphi_i)=e_i$ for each $i$, where $\List{e_1,\dots,e_m}$ is the standard basis of $\Fbb^m$.$ \\ $
\qquad Then $\List{\varphi_1,\dots,\varphi_m}$ is linely inde, suppose $a_1 v_1+\dots+a_m v_m=0$,$ \\ $
\qquad then for each $i$, we have $\varphi_{i}\List{a_1 v_1+\dots+a_m v_m}=a_i= 0$. Thus $\List{v_1,\dots,v_n}$ is linely inde.$ \\ $
Suppose $\List{v_1,\dots,v_m}$ is linely inde. Let $\List{\varphi_1,\dots,\varphi_m}$ be the dual basis of $\Span{v_1,\dots,v_m}$.$ \\ $
\qquad Thus for each $\List{a_1,\dots,a_m}\in\Fbb^m,\varphi=a_1\varphi_1+\dots+a_m\varphi_m$ so that $\Gamma(\varphi)=\List{a_1,\dots,a_m}.$
$}$\PfEnd[-10pt]
\SepLine
...
\ProblemB{
	...
	(d) \TextB{Show that $\List{\varphi_1,\dots,\varphi_m}$ is linely inde $\Longleftrightarrow \Gamma$ is surj.}
}\par\quad
(c) $\MathLeftBrace{l}{$Suppose $\Gamma$ is inje. Then $\Gamma(v)=0\Leftrightarrow\forall i,\varphi_i(v)=0\Leftrightarrow v\in(\null \varphi_1)\cap\dots\cap(\null\varphi_m)\Leftrightarrow v=0.\\ $\qquad Getting $(\null\varphi_1)\cap\dots\cap(\null \varphi_m)=\{0\}$. By Problem ($\bullet$) above, $\Span{\varphi_1,\dots,\varphi_m}=V\apostrophe$
$\\ $Suppose $\Span{\varphi_1,\dots,\varphi_m}=V\apostrophe$. Again by Problem ($\bullet$), $(\null\varphi_1)\cap\dots\cap(\null\varphi_m)=\{0\}.\\ $\qquad Thus $\Gamma(v)=0\Rightarrow\forall i,\varphi_i(v)=0\Rightarrow v=0$.
$}$\par\quad
(d) $\MathLeftBrace{l}{$
Suppose $\List{\varphi_1,\dots,\varphi_m}$ is linely inde. Then by Problem (31), $\List{v_1,\dots,v_m}$ is linely inde.$\\ $\qquad Thus for any $(a_1,\dots,a_m)\in\Fbb$, by letting $v=\sum_{i=1}^m a_i v_i$, then $\varphi_i(v)=a_i\Rightarrow\Gamma(v)=(a_1,\dots,a_m).\\ $
Suppose $\Gamma$ is surj. Let $e_1,\dots,e_m$ be a basis of $\Fbb^m$.$\\ $\qquad For every $e_i,\,\,\exists\,v_i\in V$ such that $\Gamma(v_i)=\left(\varphi_1(v_i),\dots,\varphi_m(v)\right)=e_i$,$\\ $\qquad fix $v_i$ ($\Rightarrow \List{v_1,\dots,v_m}$ is linely inde). Thus $\varphi_i(v_i)=1,\varphi_i(v_j)=0.\\ $\qquad Hence $\List{\varphi_1,\dots,\varphi_m}$ is the dual basis of the basis $v_1,\dots,\varphi_m$ of $\Span{v_1,\dots,v_m}.}$\PfEnd
\SepLine
...
\ProblemN{32}{
	\TextNL{Suppose $T\in\Lm[\Large]{V}$, and $\List{u_1,\dots,u_m},\List{v_1,\dots,v_m}$ are bases of \,$V$. Prove that}
	\TextNL{T is inv $\Longleftrightarrow$ the rows of $\Mt[\Large]{T,\List{u_1,\dots,u_m},\List[\Large]{v_1,\dots,v_m}}$ form a basis of $\Fbb^{1,n}$.}
}Note that $T$ is inveritlbe $\Longleftrightarrow$ $T\apostrophe$ is inv. And $\Mt{T\apostrophe}=\Mt{T}^t=A^t$, denote it by $B$.\par\quad
Let $\List{\varphi_1,\dots,\varphi_m}$ be the dual basis of ${v_1,\dots,v_m},\,\,\,(\psi_1,\dots,\psi_m)$ be the dual basis of $\List{u_1,\dots,u_m}$.\par\quad
...
of $V\apostrophe$.}
	\TextNL{In other words, $V\apostrophe\apostrophe=\Lm[\Large]{V\apostrophe,\Fbb}$. Define $\Lambda:V\rightarrow V\apostrophe\apostrophe$ by $(\Lambda v)(\varphi) = \varphi(v)$.}
	(a) \TextNL{Show that $\Lambda$ is a linear map from $V$ to $V\apostrophe\apostrophe$.}
	(b) \TextNL{Show that if $T\in\Lm[\Large]{V}$, then $T\apostrophe\apostrophe\circ\Lambda=\Lambda\circ T$, where $T\apostrophe\apostrophe=(T\apostrophe)\apostrophe$.}
...
\ProblemN{36}{
	\TextNL{Suppose $U$ is a subsp of $V$. Define $i:U\rightarrow V$  by $i(u)=u$. Thus $i\apostrophe\in\Lm[\Large]{V\apostrophe,U\apostrophe}.$}
...
\ProblemN{37}{
	\TextNL{Suppose $U$ is a subsp of $V$ and $\pi$ is the quotient map. Thus $\pi\apostrophe\in\Lm[\Large]{(V/U)\apostrophe,V\apostrophe}$.}
...
Then $\hTextmid{p(z)}\rightarrow\infty$ as $\left|z\right|\rightarrow\infty$ ( because $\displaystyle\frac{\hTextmid{p(z)}}{\left|z_m\right|}\rightarrow\left|c_m\right|$ as $\left|z\right|\rightarrow\infty$ ).\TextB{}
...
The function $z\rightarrow\hTextmid{q(z)}$ has a global minimum value of $1$ at $z = 0$.\TextB{}
...
$t\in (0, 1)$, then $\hTextmid{q(t\beta)}\leq\left|1 + a_k t^k\beta^k\right|+t^{k+1}c = 1 - t^k (1 - tc)$.\TextB{}
Now letting $t=1/(2c)$, we get $\hTextmid{q(t\beta)}<1$. Contradicts. Hence $p(\zeta) = 0$, as desired.\PfEnd
\SepLine
...
%\TextB{Suppose $T\in\Lm[\Large]{V}.$ Prove that every list of eigvecs of $T$ correspd to distinct eigvals is linely inde.}
%\Solution \Or Another Proof.\TextB{}
%Suppose the desired result is false. Then ( $m\neq 1$ because eigvecs are nonzero )\TextB{}
%$\,\exists$ smallest $m>1$ so that $\,\exists\,\List{v_1,\dots,v_m}$ linely depe eigvecs of $T$ correspd to distinct eigvals $\lambda_1,\dots,\lambda_m$ of $T.$\TextB{}
...
\ProblemN{31}{
	\TextNL{Suppose $V$ is finite-dim and $v_1,\dots,v_m \in V$. Prove that $\List{v_1,\dots,v_m}$ is linely inde}
	\TextNL{$\Longleftrightarrow\exists\,T\in\Lm[\Large]{V},$ $v_1,\dots,v_m$ are eigvecs of $T$ correspd to distinct eigvals.}
}\par\quad
Suppose $\List{v_1,\dots,v_m}$ is linely inde, extend it to a basis of $V$ as $(v_1,\dots,v_m,\dots,v_n)$.\par\quad
Define $T\in\Lm{V}$ by $Tv_k=kv_k$ for each $k\in\{1,\dots,m,\dots,n\}.$ Conversely by [5.10].\PfEnd
\SepLine

\ProblemNnosolus{1}{
	\TextN{Suppose $T\in\Lm[\Large]{V}$ and $U$ is a subsp of \,$V$.}
...
\ProblemB{
	\TextB{Suppose $S,T\in\Lm[\Large]{V}$ are such that $ST=TS.$ }
...
\ProblemN{15}{
	\TextNL{Suppose $T\in\Lm[\Large]{V}$. Suppose $S\in\Lm[\Large]{V}$ is inv.}
...
\ProblemN{21}{
	\TextNL{Suppose $T\in\Lm[\Large]{V}$ is inv.}
...
\ProblemN{23}{
	\TextNL{Suppose $S, T\in\Lm[\Large]{V}$. Prove that $ST$ and $TS$\, have the same eigvals.}
}\par\quad
...
\ProblemBnoor{({\normalsize 2E Ch5.20})\TextB{}}{
	\TextB{Suppose $T\in\Lm[\Large]{V}$ has $\dim V$ distinct eigvals and $S\in\Lm[\Large]{V}$ has the same eigvecs}
...
Then $\List{v_1,\dots,v_n}$ is a basis of $V.$ Because $(ST)v_j=\alpha_j\lambda_j v_j=(TS)v_j$ for each $j.$ Hence $ST=TS.$\PfEnd
\SepLine


\ProblemB{
	\TextB{Suppose $V$ is finite-dim and $T\in\Lm[\Large]{V}$.}
	\TextB{Define $\mathcal{A}\in\Lm[\Large]{\Lm[\Large]{V}}$ by $\mathcal{A}(S) = TS$ for each $S\in\Lm[\Large]{V}$.}
...
Then for each $k\in\{1,\dots,m\},\Span{v_k}\subseteq\null(T-\lambda_k I).$\par\quad\Ha
Define $S_k\in\Lm{V}$ by $S_k(v_j)=v_k$ for each $j\in\{1,\dots,n\}$,\par\quad\Ha\qquad\,
so that $\range S_k=\Span{v_k}$ for each $k\in\!\hTextbra{1,\dots,m}$, then $\mathcal{A}(S_k)=TS_k=\lambda_k S_k.$\par\quad\Ha
...
\Or \par\quad
(a) Suppose $\lambda$ is an eigval of $T$ with an eigvec $v.$\par\quad\Ha
Let $v_1=v$ and extend to a basis $\List{v_1,\dots,v_m}$ of $V.$\par\quad\Ha
...
\Comment\,\,\, {\normalsize Define $\mathcal{B}\in\Lm[\normalsize]{\Lm[\normalsize]{V}}$ by $\mathcal{B}(S)=ST,$ $\forall S\in\Lm{V}.$
Then the eigvals of $\mathcal{B}$ are not the eigvals of $T.$}\par
\SepLine

\ProblemN{4}{
	\TextN{Suppose $T\in\Lm[\Large]{V}$ and $V_1,\dots,V_m$ are invar subsps of \,$V$ under $T$.}
...
%
%\ProblemN{5}{
%	\TextN{Suppose $T\in\Lm[\Large]{V}$. Prove that the intersection of every collection}
%	\TextN{of subsps of \,$V$ invar under $T$ is invar under $T$.}
%}\par\quad
%Suppose $\hTextbra{V_{\alpha}}{_{\alpha\in\Gamma}}$ is a collection of subsps of $V$ invar under $T$; here $\Gamma$ is an arbitrary index set.\par\quad
...
Let $\List{u_1,\dots,u_m}$ be a basis of $U$ and $\List{w_1,\dots,w_n}$ be a basis of $W.$\par\quad
...
\ProblemB{
	\TextB{Suppose $\Fbb=\Rbb,$ $T\in\Lm[\Large]{V}$.}
...
Let $\List{v_1,\dots,v_n}$ be a basis of $V.$ Write $v=\sum_{i=1}^n a_i v_i,u=\sum_{i=1}^n b_i v_i,$ where $a_i,b_i\in\Rbb.$\par\quad\Hb
...
\ProblemB{
	\TextB{Suppose $V$ is finite-dim, $T\in\Lm[\Large]{V}$, and $\lambda\in\Fbb.$}
	\TextB{Show that $\lambda$ is an eigval of $T$ $\Longleftrightarrow$ $\lambda$ is an eigval of the dual operator $T\apostrophe\in\Lm[\Large]{V\apostrophe}$.}
}\par\quad
...
\ProblemN{7}{
	\TextN{Suppose $T\in\Lm[\Large]{\Rbb^2}$ is defined by $T(x, y) = (-3y, x)$. Find the eigvals of $T$.}
}\par\quad
...
\ProblemN{8}{
	\TextN{Define $T\in\Lm[\Large]{\Fbb^2}$ by $T(w, z) = (z, w)$. Find all eigvals and eigvecs of $T$.}
}\par\quad
...
\ProblemB{
	\TextB{Suppose $P\in\Lm[\Large]{V}$ is such that $P^2 = P$.}
...
\large
\ProblemN{22}{
	\TextNL{Suppose $T\in\Lm[\Large]{V}$ and $\,\exists$ nonzero vecs $u,w$ in $V$ such that $Tu = 3w$ \,{\normalsize and}\, $Tw = 3u$.}
...
\ProblemN{9}{
	\TextN{Define $T\in\Lm[\Large]{\Fbb^3}$ by $T(z_1,z_2,z_3)=(2z_2, 0,5z_3)$.}
...
\ProblemN{10}{
	\TextNL{Define $T\in\Lm[\Large]{\Fbb^n}$ by $T(x_1,x_2,x_3,\dots,x_n)=(x_1,2 x_2,3 x_3,\dots,n x_n)$}
...
\ProblemN{12}{
	\TextNL{Define $T\in\Lm[\Large]{\PoR{4}}$ by $(Tp)(x) = xp\apostrophe(x)$ for all $x\in\Rbb.$}
...
\ProblemN{30}{
	\TextNL{Suppose $T\in\Lm[\Large]{\Rbb^3}$ and $-4,5,\sqrt{7}$ are eigvals of $T$.}
...
\ProblemN{14}{
	...
	\TextNL{Define $P\in\Lm[\Large]{V}$ by $P(u + w) = u$ \,for each $u\in U$ and each $w\in W$.}
...
\ProblemN{13}{
	\TextNL{Suppose $V$ is finite-dim, $T\in\Lm[\Large]{V},$ and $\lambda\in\Fbb.$}
...
Define $T\in\Lm{\Rbb^4}$ by $\Mt{T,\List{e_1,e_2,e_3,e_4}}=${\footnotesize$\begin{pmatrix}
 1 & 1 & 1 & 1\\
 -1 & 1 & -1 & -1\\
 3 & 8 & 11 & 5\\
 3 & -8 & -11 & 5
...
\ProblemB{
	\TextB{Suppose $\List{v_1,\dots,v_n}$ is a basis of \,$V$ and $T\in\Lm[\Large]{V},\Mt{T,\List{v_1,\dots,v_n}}=A.$}
	\TextB{Prove that if $\lambda$ is an eigval of $T$, then $\left|\lambda\right|\leq n \max\left\{\hTextmid{A_{j,k}}:1\leq j, k\leq n\right\}.$}
}\par\quad
First we show that $\left|\lambda\right|=n\max\left\{\hTextmid{A_{j,k}}:1\leq j, k\leq n\right\}$ for some cases.\par\quad
Consider $A=\,${\small$\begin{pmatrix}
k & \cdots & k\\
\vdots & \ddots & \vdots\\
k & \cdots & k
\end{pmatrix}.$} Then $nk$ is an eigval of $T$ with an eigvec $v_1+\dots+v_n.$\par\quad
Now we show that if $\left|\lambda\right|\neq n\max\left\{\hTextmid{A_{j,k}}:1\leq j, k\leq n\right\},$ then $\left|\lambda\right|<n\max\left\{\hTextmid{A_{j,k}}:1\leq j, k\leq n\right\}.$\par\quad
\par
\SepLine

\ProblemN{18}{
	\TextNL{Show that the forward shift operator $T\in\Lm[\Large]{\Fbb^\infty}$}
...
\ProblemN{19}{
	\TextNL{Suppose $n\in\Nbp$. Define $T\in\Lm[\Large]{\Fbb^n}$ by}
...
\ProblemN{20}{
	\TextNL{Define the backward shift operator $S\in\Lm[\Large]{\Fbb^\infty}$ by $S(z_1,z_2,z_3,\dots) = (z_2,z_3,\dots)$.}
...
\ProblemN{24}{
	\TextNL{Suppose $A\in\Fbb^{n,n}.$ Define $T\in\Lm[\Large]{\Fbb^n}$ by $Tx = Ax$,}
...
\ProblemB{
	\TextB{Suppose $A\in\Fbb^{n,n}.$ Define $T\in\Lm[\Large]{\Fbb^n}$ by $Tx = xA$,}
...
\ProblemN{25}{
	\TextNL{Suppose $T\in\Lm[\Large]{V}$ and $u, w$ are eigvecs of $T$ such that $u + w$ is also an eigvec of $T$.}
...
\ProblemN{26}{
	\TextNL{Suppose $T\in\Lm[\Large]{V}$ is such that every nonzero vec in $V$ is an eigvec of $T$.}
...
\ProblemN{27, 28}{
	...
	\TextNL{Suppose $T\in\Lm[\Large]{V}$ is such that every subsp of \,$V$ of dim $k$ is invar under $T$.}\qquad
	\TextNL{Prove that $T$ is a scalar multi of the identity operator.}
...
Thus $\List{v,Tv}$ is linely inde. Extend to a basis of $V$ as $(v,Tv,u_1,\dots,u_n).$\par\quad
Let $U=\Span{v,u_1,\dots,u_{k-1}}\Rightarrow U$ is not an invar subsp of $V$ under $T.$\par\quad
\Or Suppose $0\neq v=v_1\in V$ and extend to a basis of $V$ as $\List{v_1,\dots,v_n}.$\par\quad
...
\ProblemB{
	\TextB{Suppose $V$ is finite-dim and $T\in\Lm[\Large]{V}$. Prove that}
...
then extend $w_1=\alpha_{1,1}$ to a basis of $\Span{v_2,\dots,v_n}$ as $\List{\alpha_{1,1},\dots,\alpha_{1,n-1}}.$\par\qquad\qquad\quad\, Otherwise, we stop at step $1.$\par\quad\Ha
%{\tgbf Step 2} $\,$If $\,\exists\,w_2\in\Span{\alpha_{1,2},\dots,\alpha_{1,n-1}}$ such that $0\neq Tw_2\in\Span{v_1,w_1},$\par\qquad\qquad\quad\,
%then extend $w_2=\alpha_{2,1}$ to a basis of $\Span{\alpha_{1,2},\dots,\alpha_{1,n-1}}$ as $\List{\alpha_{2,1},\dots,\alpha_{2,n-2}}.$\par\qquad\qquad\quad\,
...
then extend $w_k=\alpha_{k,1}$ to a basis of $\Span{\alpha_{k-1,2},\dots,\alpha_{k-1,n-k+1}}$ as $\List{\alpha_{k,1},\dots,\alpha_{k,n-k}}.$\par\qquad\qquad\quad\,
Otherwise, we stop at step $k.$\par\quad\Ha
$\quad\vdots$\par\quad\Ha
Finally, we stop at step $m,$ thus we get $\List{v_1,w_1,\dots,w_{m-1}}$ and $\List{\alpha_{m-1,2},\dots,\alpha_{m-1,n-m+1}},$\par\quad\Ha
$\range T|_{\Span{w_1,\dots,w_{m-1}}}=\Span{v_1,w_1,\dots,w_{m-2}}\Rightarrow\dim\null T|_{\Span{w_1,\dots,w_{m-1}}}=0,$\par\quad\Ha
$\underbrace{\Span{v_1,w_1,\dots,w_{m-1}}}_{\dim m}$ and $\underbrace{\Span{\alpha_{m-1,2},\dots,\alpha_{m-1,n-m+1}}}_{\dim (n-m)}$ are invar under $T.$\par\quad\Ha
...
Let $\List{u_1,\dots,u_m}$ be a basis of $U$, extend to a basis of $V$ as $(u_0,u_1,\dots,u_m).$\par\quad\Hb
...
\ProblemN{29}{
	\TextNL{Suppose $T\in\Lm[\Large]{V}$ and $\range T$ is finite-dim.}
...
( Because $\range T$ is finite-dim. Let $\List{v_1,\dots,v_n}$ be a list of all the linely inde eigvecs of $T$,\par\quad\,\,\,
...
\ProblemN{32}{
	...
	\TextNL{{\tgbf\large H{\small INT}:} Let $V=\Span{e^{\lambda_1}x,\dots,e^{\lambda_n}x}$, and define an operator $D\in\Lm[\Large]{V}$ by $Df = f\apostrophe$.}
...
Let $V=\Span{\cos(\lambda_1 x),\dots,\cos(\lambda_n x)}.$ Define $D\in\Lm{V}$ by $Df=f\apostrophe.$\par\quad
...
\ProblemB{
	\TextB{Suppose $V$ is finite-dim, $T\in\Lm[\Large]{V}$, and $U$ is a subsp of \,$V$ invar under $T$.}
	\TextB{The quotient operator $T/U\in\Lm[\Large]{V/U}$ is defined by}
...
\ProblemN{33}{
	\TextNL{Suppose $T\in\Lm[\Large]{V}.$ Prove that $T/(\range T)=0.$}
}\par\quad
...
\ProblemN{34}{
	\TextNL{Suppose $T\in\Lm[\Large]{V}.$ Prove that $T/(\null T)$ is inje $\Longleftrightarrow$ $(\null T)\cap(\range T)=\{0\}.$}
}\par\quad
...
$\\ $
\ProblemB{
	\TextB{Suppose $T\in\Lm[\Large]{V}$ and $\,m\,$ is a positive integer.}
...
\TextB{Suppose $T\in\Lm[\Large]{V},p\in\PoF{}$. Prove that $\null p(T)$ and $\range p(T)$ are invar under $T$.}
...
\ProblemN{13}{
	\TextNL{Suppose $V$ is a complex vecsp and $T\in\Lm[\Large]{V}$ has no eigvals.}
...
\ProblemN{16}{
	\TextNL{Suppose $0\neq v\in V.$ Define $S\in\Lm[\Large]{\PoC{\dim V},V}$ by $S(p)=p(T)v.$ Prove $[$ 5.21 $].$}
}\par\quad
...
\ProblemN{17}{
	\TextNL{Suppose $0\neq v\in V.$ Define $S\in\Lm[\Large]{\PoC{(\dim V)^2},\Lm[\Large]{V}}$ by $S(p)=p(T).$ Prove [5.21].}
}\par\quad
...
\ProblemBnoor{({\normalsize 4E 5.31, 4E 5.B.25 and 26})}{
	...
	\TextB{Suppose $V$ is finite-dim, $T\in \Lm[\Large]{V}$, and $U$ is an invar subsp of \,$V$ under $T$.}
...
\ProblemBnoor{({\normalsize 4E 5.B.27})}{
	\TextB{Suppose $\Fbb=\Rbb,$ $V$ is finite-dim, and $T\in \Lm[\Large]{V}$.}
...
\ProblemBnoor{({\normalsize 4E 5.B.28})}{
	\TextB{Suppose $V$ is finite-dim and $T\in \Lm[\Large]{V}$.}
	\TextB{Prove that the mini poly $\,p\,$ of $T\apostrophe \in\Lm[\Large]{V\apostrophe}$ equals the mini poly $\,q\,$ of $T$.}
}\par\quad
...
\ProblemBnoor{(4E 5.32)}{
	\TextB{Suppose $T\in\Lm[\Large]{V}$ and $\,p\,$ is the mini poly.}
...
\ProblemBnoor{({\normalsize 4E 5.B.22})\TextB{}}{
	\TextB{Suppose $V$ is finite-dim, $T\in \Lm[\Large]{V}$. Prove that $T$ is inv $\Longleftrightarrow I\in\Span{T, T^2, \dots , T^{\dim V}}.$}
...
\ProblemN{6}{
	\TextN{Suppose $T\in\Lm[\Large]{V}$ and $U$ is a subsp of $V$ invar under $T$.}
...
\ProblemBnoor{({\normalsize 4E 5.B.10, 5.B.23})}{
	\TextB{Suppose $V$ is finite-dim, $T\in \Lm[\Large]{V}$ and $\,p\,$ is the mini poly with degree $\,m.$ Suppose $v\in V.$}
...
$1$ is the smallest positive integer such that $T^1 v\in\Span{v,\dots,T^0 v}.$ Define $q$ by $q(z)=z\Rightarrow q(T)v=0.$\par\quad
...
\ProblemN{19}{
	\TextNL{Suppose $V$ is finite-dim and $T\in \Lm[\Large]{V}$. Let $\mathcal{E} =\left\{q(T): q \in \PoF{}\right\}$.}
...
For any $k\in\Nbp,\,\,T^{m+k}=T^k(T^m)\in\Span{I,T,\dots,T^{m-1}}=U.$\par\quad
...
Note that by the minimality of $m,$ $\List{I,T,\dots,T^{m-1}}$ is linely inde.\par\quad
Thus $\dim U=m=\dim\Span{I,T,\dots,T^{(\dim V)^2-1}}=\dim\Span{I,T,\dots,T^n}$ for all $m<n\in\Nbp.$\par\vspace{6pt}\quad
...
\ProblemBnoor{({\normalsize 4E 5.B.13})}{
	\TextB{Suppose $T\in \Lm[\Large]{V}$ and $q \in \PoF{}$ is defined by}
...
\ProblemBnoor{({\normalsize 4E 5.B.14})}{
	\TextB{Suppose $V$ is finite-dim, $T\in \Lm[\Large]{V}$ has mini poly $\,p\,$}
...
\TextB{Suppose $V$ is a finite-dim complex vecsp and $T\in\Lm[\Large]{V}.$}
...
\ProblemN{10}{
	\TextNL{Suppose $T\in\Lm[\Large]{V},\lambda$ is an eigval of $T$ with an eigvec $v.$}
...
\ProblemN{1}{
	\TextN{Suppose $T\in\Lm[\Large]{V}$ and $\,\exists\,n\in\Nbp$ such that $T^n=0.$}
...
\ProblemN{2}{
	\TextN{Suppose $T\in\Lm[\Large]{V}$ and $(T - 2I)(T - 3I)(T - 4I) = 0$.}
...
\ProblemN{7}{
	...
	\TextN{Suppose $T\in\Lm[\Large]{V}$. Prove that $9$ is an eigval of $T^2$ $\Longleftrightarrow$ $3$ or $-3$ is an eigval of $T$.}
}\par\quad
...
\ProblemN{3}{
	\TextN{Suppose $T\in\Lm[\Large]{V},T^2=I$ and $-1$ is not an eigval of $T.$ Prove that $T=I.$}
}\par\quad
...
\ProblemBnoor{({\normalsize 4E 5.A.32})}{
	\TextB{Suppose $T\in\Lm[\Large]{V}$ has no eigvals and $T^4 = I$. Prove that $T^2=-I$.}
}\par\quad
...
\ProblemN{8}{
	...
	\TextN{Give an example of $T\in\Lm[\Large]{\Rbb^2}$ such that $T^4=-I$.}
}\par\quad
...
\ProblemBnoor{({\normalsize 4E 5.B.12} See also at 5.A.9)\TextB{}}{
	\TextB{Define $T\in\Lm[\Large]{\Fbb^n}$ by $T(x_1 , x_2 , x_3 , \dots , x_n ) = (x_1 , 2x_2 , 3x_3 , \dots , nx_n )$. Find
the mini poly.}
}\par\quad
$T(x_1,\dots,0)=$
...
\ProblemB{
	\TextB{Suppose $n\in\Nbp$. Define $T\in\Lm[\Large]{\Fbb^n}$ by $T(x_1,\dots,x_n)=(x_1+\dots+x_n,\dots,x_1+\dots+x_n).$}
...
\ProblemBnoor{({\normalsize 4E 5.B.8})\TextB{}}{
	\TextB{Suppose $T\in\Lm[\Large]{\Rbb^2}$ is the operator of counterclockwise rotation by the angel $\theta$, where $\theta\in\Rbb^+$.}
...
Now suppose $\List{v,Tv}$ is linely inde. Then $\Span{v,Tv}=\Rbb^2.$\par\quad
...
\ProblemBnoor{({\normalsize 4E 5.B.11})}{
	\TextB{Suppose $V$ is a two-dim vecsp, $T\in\Lm[\Large]{V}$, and the matrix of $T$}
...
\ProblemN{5}{
	\TextN{Suppose $S,T\in\Lm[\Large]{V},S$ is inv, and $p\in\PoF{}$. Prove that $p(TS) = S^{-1} p(ST)S.$}
}\par\quad
...
\ProblemBnoor{({\normalsize 4E 5.B.7})\TextB{}}{
	(a) \TextB{Give an example of $S, T\in\Lm[\Large]{\Fbb^2}$ such that}
	\Ha\TextB{the mini poly of
$ST$ does not equal the mini poly of $TS$.}
	(b) \TextB{Suppose $V$ is finite-dim and $S, T\in\Lm[\Large]{V}$. Prove that if $S$ or $T$ is inv,}
...
\ProblemN{11}{
	\TextNL{Suppose $\Fbb = \Cbb,$ $T\in\Lm[\Large]{V}, p\in\PoC{}$, and $\alpha\in\Cbb.$}
...
By Problem (4E 5.B.11), $\Mt{T,\List{(1,0),(0,1)}}=\,${\small$\begin{pmatrix}0 & -1\\ 1 & 0\end{pmatrix}$}$\Rightarrow$ the mini poly of $T$ is $z^2+1.$\par\quad
...
\ProblemBnoor{({\normalsize4E 5.B.17})}{
	\TextB{Suppose $V$ is finite-dim, $T\in \Lm[\Large]{V},\lambda\in\Fbb$, and $\,p\,$ is the mini poly of $T$.}
...
$n$ is the smallest such that $(T-\lambda I)^n\in\Span{I,(T-\lambda I),\dots,(T-\lambda I)^{n-1}};$\par\quad
$m$ is the smallest such that $T^m\in\Span{I,T,\dots,T^{m-1}}.$\par\quad
又 $T^k\in\Span{I,T,\dots,T^{k-1}}\Longleftrightarrow (T-\lambda)^k\in\Span{I,(T-\lambda I),\dots,(T-\lambda I)^{k-1}}.$\par\quad
Thus $n=m.$ 又 $q$ is monic. By the uniqnes of mini poly.\PfEnd
\SepLine

\ProblemBnoor{({\normalsize4E 5.B.18})}{
	\TextB{Suppose $V$ is finite-dim, $T\in \Lm[\Large]{V},\lambda\in\Fbb\backslash\{0\}$, and $\,p\,$ is the mini poly of $T$.}
...
By definition of mini poly,\par\quad
$n$ is the smallest such that $(\lambda T)^n\in\Span{\lambda I,\lambda T,\dots,(\lambda T)^{n-1}};$\par\quad
$m$ is the smallest such that $T^m\in\Span{I,T,\dots,T^{m-1}}.$\par\quad
又 $(\lambda T)^k\in\Span{\lambda I,\lambda T,\dots,(\lambda T)^{k-1}}\Longleftrightarrow T^k\in\Span{I,T\dots,T^{k-1}}.$\par\quad
Thus $n=m.$ 又 $q$ is monic. By the uniqnes of mini poly.\PfEnd
\SepLine

\ProblemN{18}{
	({\normalsize \OR 4E 5.B.15}) \TextNL{Suppose $V$ is a finite-dim complex vecsp with $\dim V > 0$ and $T\in\Lm[\Large]{V}$.}
...
\ProblemBnoor{({\normalsize 4E 5.B.9})}{
	\TextB{Suppose $T\in\Lm[\Large]{V}$ is such that with resp to some basis of \,$V$,}
...
Let $\List{v_1,\dots,v_n}$ denote the basis such that $\Mt{T,\List{v_1,\dots,v_n}}_{j,k}=A_{j,k}\in\Qbb$ for all $j,k=1,\dots,n$.\par\quad
Denote $\Mt{v_j,\List{v_1,\dots,v_n}}$ by $x_j$ for each $v_j.$\par\quad
...
\ProblemB{
	...
	\TextB{Suppose $\Fbb=\Rbb,$ $V$ is finite-dim, $T\in\Lm[\Large]{V}$ and $b, c\in\Rbb$ with $b^2 < 4c$.}
...
\ProblemB{
	...
	\Hb\TextB{Let $T\in\Lm[\Large]{V}$ and the mini poly is $\,p\,$. Prove that $T$ has an eigval.}
}\par\quad
...
\ProblemBnoor{({\normalsize 2E Ch5.24})}{
	\TextB{Suppose $\Fbb=\Rbb,T\in\Lm[\Large]{V}$ has no eigvals.} \TextB{Prove that every invar subsp of \,$V$ under $T$ is even-dim.}
}\par\quad
...
\ProblemBnoor{({\normalsize4E 5.C.1})}{
	...
	\TextB{If $T\in \Lm[\Large]{V}$ and $T^2$ has an upper-trig matrix, then $T$ has an upper-trig matrix.}
}\par
\SepLine
...
\ProblemBnoor{({\normalsize4E 5.C.3})\TextB{}}{
	\TextB{Suppose $T\in \Lm[\Large]{V}$ is inv and $B=\List{v_1 , \dots , v_n}$ is a basis of \,$V$ such that}
...
\ProblemN{9}{
	...
	\TextN{Suppose $V$ is finite-dim, $T\in \Lm[\Large]{V}$, and $v \in V$.}
...
\ProblemN{20}{
	...
	\TextNL{Suppose $\Fbb=\Cbb,$ $V$ is finite-dim, and $T\in\Lm[\Large]{V}$.}
	\TextNL{Prove that if $k\in\{1,\dots,\dim V\},$ then \,$V$ has a $k$ dim subsp invar under $T$.}
}\par
\SepLine

\ProblemBnoor{({\normalsize 4E 5.C.8})}{
	\TextB{Suppose $V$ is finite-dim, $T\in \Lm[\Large]{V}$, and $\,\exists\,v \in V\backslash\{0\}$ such that $T^2 v + 2Tv = -2v$.}
...
\ProblemBnoor{({\normalsize 4E 5.C.10})}{
	\TextB{Suppose $T\in \Lm[\Large]{V}$ and $\List{v_1,\dots,v_n}$ is a basis of \,$V$.}
	\TextB{Show that the following are equi.}
	(a) \TextB{The matrix of $T$ with resp to $\List{v_1,\dots,v_n}$ is lower trig.}
...
\ProblemBnoor{({\normalsize 4E 5.C.11})}{
	...
	\TextB{Prove that if $T\in \Lm[\Large]{V}$, then $T$ has a lower-trig matrix with resp to some basis.}
}\par
\SepLine

\ProblemBnoor{({\normalsize 4E 5.C.12})\TextB{}}{
	\TextB{Suppose $V$ is finite-dim, $T\in \Lm[\Large]{V}$ has an upper-trig matrix with resp to some basis,}
...
\ProblemBnoor{({\normalsize 4E 5.C.13})}{
	\TextB{Suppose $V$ is finite-dim, $T\in \Lm[\Large]{V}$. Suppose $U$ is an invar subsp of \,$V$ under $T$}
...
\ProblemN{2}{
	\TextN{Suppose $\mathcal{E}$ is a subset of $\Lm[\Large]{V}$ and every element of $\mathcal{E}$ is diagable.}
...
\ProblemN{3}{
	\TextN{Suppose $S, T\in\Lm[\Large]{V}$ are such that $ST = TS$. Suppose $p\in\PoF{}$.}
...
\ProblemN{6}{
	\TextN{Suppose $V$ is a finite-dim complex vecsp and $S, T\in\Lm[\Large]{V}$ commute.}
	\TextN{Prove that $\,\exists\,\alpha,\lambda\in\Cbb$ such that $\range(S -\alpha I) + \range(T -\lambda I)\neq V$.}
}\par
\SepLine

\ProblemN{7}{
	\TextN{Suppose $V$ is a complex vecsp, $S\in\Lm[\Large]{V}$ is diagable, and $T$ commutes with $S$.}
...
\ProblemN{9}{
	...
	\TextN{Suppose that $\mathcal{E}\subseteq\Lm[\Large]{V}$ is such that $S$ and $T$ commute for all $S, T\in\mathcal{E}$.}
