...
}Using [1.31]. $0v=0\Longleftrightarrow\Par{1+\Par{-1}}v=1\cdot v+\Par{-1}v=v+\Par{-v}=0.$\PfEnd
...
\quad\TextA{\FontNorm\EndI\tgnr(II) $t + \Par{-\infty} = \Par{-\infty} + t = \Par{-\infty} + \Par{-\infty} = -∞,$}
...
By applying Exe (12) we conclude that one $U_j$ contains the other two. Thus done.\vspace{6pt}\par\quad\Hb\EndI
(II) \envFontLarge{Asum no $U_j$ is contained in the union of the other two,}\par\quad\Hb\HII
...
{\Large\vspace{6pt}Note that $W\cap U_1=\emptySet,$ for if any $v+\lambda u\in W\cap U_1$ then $v+\lambda u-\lambda u=v\in U_1$.}\par\quad\Hb\HII
{\Large\vspace{6pt}Now $W\subseteq U_1\cup U_2\cup U_3\Rightarrow W\subseteq U_2\cup U_3.$ $\forall v+\lambda u\in W,v+\lambda u\in U_i,i=2,3.$}\par\quad\Hb\HII
...
\ProblemN{\Anchor{2A17}{17}}{
	\TextA{Supp $p_0,p_1,\dots,p_m\in\PoF{m}$ suth each $p_k\Par{2}=0.$}
	\TextA{Prove $\Par{p_0,p_1,\dots,p_m}$ is not liney indep in $\PoF{m}$.}
}\par\quad
...
\ProblemN{\Anchor{2B8}{8}}{
	\TextA{Supp $V=U\oplus W,\;B_U=\Par{u_1,\dots,u_m},\;B_W=\Par{w_1,\dots,w_n}.$}
	\TextA{Prove $B_V=\BigPar{u_1,\dots,u_m,w_1,\dots,w_n}$.}
...
\Blind{(b)} $Q\Rightarrow P:$ \;$\forall v\in V,\exists\,\!\,a_i+\i b_i\in\Cbb,\;v+\i 0=\BigBigPar{{\sum_{i=1}^na_iv_i}}+\i\,\BigBigPar{{\sum_{i=1}^nb_iv_i}}\envFontSmall\Rightarrow v\in\Span{v_1,\dots,v_m}.$\par\quad
...
\ProblemN{\Anchor{2C9}{9}}{
	\TextA{Supp $\Par{v_1,\dots,v_m}$ is liney indep in $V,w\in V.$ Prove $\dim\Span[\BigPar]{v_1+w,\dots,v_m+w}\geq m-1$.}
...
\Anchor{2C4e14}\Anchor{2C4e15}\ProblemB[]{
	...
	(a) \TextB{$\dim V=10,\;\dim V_{\!1} = \dim V_{\!2} = \dim V_{\!3} = 7$. Prove $V_{\!1}\cap V_{\!2}\cap V_{\!3}\neq\zeroSubs$.}
	\Blind{(a)} By {\TIPS}, $\Dim\Par{V_{\!1}\cap V_{\!2}\cap V_{\!3}}\geq\dim V_{\!1}+\dim V_{\!2}+\dim V_{\!3}\uline{{}-2\dim V}>0.$\TextB{\vspace{4pt}}
	(b) \TextB{$\dim V_{\!1}+\dim V_{\!2}+\dim V_{\!3} > 2\dim V$. Prove $V_{\!1}\cap V_{\!2}\cap V_{\!3}\neq\zeroSubs$.}
...
\Anchor{4O4e3}\ProblemBnoor{{4E 4.3}}{
	\TextB{Supp $\Fbb=\Cbb,\,\varphi\in\Lm{V,\Fbb},\,\sigma=\REAL\circ\varphi.$ \,Show all $\varphi\Par{v}=\sigma\Par{v}-\i\sigma\Par{\i v}.$\vspace{1pt}}
...
\Anchor{3AT2}\ProblemBX[]{\TipsN{2}}{
	...
	\ACoro $\Bra{T\in\Lm{V,W}:\range T\subseteq U}=\Bra{T\in\Lm{V,U}}=\Lm{V,U}.$\TextA{\vspace{-2pt}}
}\SepLine
...
\ProblemN{\Anchor{3A4}{4}}{
	\TextA{Supp $T\in\Lm{V,W},$ and $v_1,\dots,v_m\in V$ suth $\Par{Tv_1,\dots,Tv_m}$ is liney indep in $W$.} \TextA{Prove $\Par{v_1,\dots,v_m}$ is liney indep.}
}Supp $a_1 v_1+\dots+a_m v_m=0$. Then $a_1 Tv_1+\dots+a_m Tv_m=0$. Thus $a_1=\dots=a_m=0.$\PfEnd
\SepLine

\ProblemN{\Anchor{3A7}{7}}{
	\TextA{Show every liney map from a $1$-dim vecsp to itself is a multi by some scalar.}
	\TextA{More precisely, prove if $\dim V = 1$ and $T\in\Lm{V}$, then $\exists\,\lambda\in\Fbb,Tv = \lambda v,\forall v\in V$.}
...
For all $x,y\in\Bra{1,\dots,n},$ {$\Par{R_{k,y}S}{v_i}=a_k v_y\Rightarrow\BigPar{\Par{R_{k,y}S}\circ R_{x,i}}{v_z}=\delta_{z,x}\Par{a_k v_y}$}.\par\quad
...
{Hence $\null\varphi$ is a non0 two-sided ideal of $\Lm{V}.$}\PfEnd
\SepLine

\Anchor{3A4e11}\ProblemB{
...
\Blind{\Or \;}{\vspace{6pt}Note that {\Large$S_k\BigBigPar{{\sum_{i=1}^n a_i v_i}}=a_k v_k$}. Then {\Large$S_kv=v\Longleftrightarrow\exists\,!\,a_k\in\Fbb,v=a_kv_k$}.}\par\quad
...
\Anchor{3A'2}\ProblemB{
	...
	\TextB{\FontNorm And by [3.2], the add and homo imply that $V$ is closd add and scalar multi.\;\BigPar{ $W^V$ might not be a vecsp. }}
}\par\quad
...
\Anchor{3B3}\ProblemN[]{3}{
	...
	(a) \TextA{The surj of $T$ corres to $\Par{v_1,\dots,v_m}$ spanning $V$.\hfill\FontNorm$\range T=\Span{v_1,\dots,v_m}=V.$}
	(b) \TextA{The inje of $T$ corres to $\Par{v_1,\dots,v_m}$ being liney indep.\hfill\FontNorm $\Par{v_1,\dots,v_m}$ liney indep $\Longleftrightarrow T$ inje.}
}
...
\ProblemN{\Anchor{3B9}{9}}{
	\TextA{Supp $\Par{v_1,\dots,v_n}$ is liney indep. Prove $\forall$ inje $T,\Par{Tv_1,\dots,Tv_n}$ is liney indep.}
...
\Anchor{3B4e27}\ProblemBnoor{{4E 27}}{
...
\Anchor{3B4e24}\ProblemB{
	...
	(a) \TextB{Supp $\dim V=n,$ \,$ST=0$ where $S,T\in\Lm{V}.$ Prove $\dim\range TS\leqslant\Big\lfloor${\LARGE$\frac{\,n\,}{2}$}$\Big\rfloor.$}
...
\ProblemN{\Anchor{3B28}28}{
	...
	(a) \TextA{Prove $\exists\,\varphi_1,\dots,\varphi_m\in\Lm{V,\Fbb}$ suth $\forall v\in V,Tv=\varphi_1\Par{v}w_1+\dots+\varphi_m\Par{v}w_m$.\vspace{2pt}}
	(b) {\FontSmall\Sbra{4E 3.F.5}} \TextA{$\forall v\in V,\exists\,!\,\varphi_i\Par{v}\in\Fbb,Tv=\varphi_1\Par{v}w_1+\dots+\varphi_m\Par{v}w_m.$\vspace{1pt}}\Anchor{3F4e5}
	\Blind{(b) {\FontSmall\Sbra{4E 3.F.5}}} \TextA{Thus defining each $\varphi_i:V\rightarrow\Fbb.$ \;Show each $\varphi_i\in\Lm{V,\Fbb}.$}
...
\Anchor{3CT1}\ProblemBX{\TipsN{1}}{
	...
	(a) \TextA{Show [P] $L$ is liney indep $\Longleftrightarrow L_{\mathcal{M}}$ is liney indep. [Q]\vspace{2pt}}
	(b) \TextA{Show [P] $\spn L=W\Longleftrightarrow\spn L_{\mathcal{M}}=\Fbb^{m,1}.$ [Q]\hfill\FontNorm\Sbra{ Let $A=\Mt{T,B_V,B_W}.$\hspace{1pt}}\vspace{3pt}}
...
(a) By Exe (9), if all $V_k$ finide and iso to each other, then $S_m\circ\cdots\circ S_1$ inje $\Rightarrow$ inv, so are all $S_k.$\TextB{}
(b) By the ctrapos of (3.B.11), $S_m\circ\cdots\circ S_1$ not inje $\Rightarrow\exists\,S_k$ not inje. Convly not true unless $k=1.$\TextB{}
...
\ProblemN{\Anchor{3E11}{11}}{
	...
	(a) \TextA{Prove $A$ is a trslate of some subsp of $V$}
	(b) \TextA{Prove if $B$ is a trslate of some subsp of $V$ and $\Bra{v_1,\dots,v_m}\subseteq B$, then $A\subseteq B$.}
	(c) \TextA{Prove $A$ is a trslate of some subsp of $V$ of dim $< m.$\vspace{2pt}}
...
\Anchor{3ET4}\ProblemB{
	...
	(a) \TextB{Prove \FontNorm $\LmQxx{T}$ is well-defined $\Longleftrightarrow\BigPar{\range T\mmid_{U\,\cap\,V}}\XSlash\Par{X\cap W}=\zeroSubs${${}\Longleftrightarrow\range T\mmid_{U\,\cap\,V}$ is a subsp of $X\cap W$}.\vspace{2pt}}
	...
	(c) \TextB{Prove $\LmQxx{T}$ is surj $\Longleftrightarrow W=\range T+X\cap W.$\vspace{4pt}}
...
\ProblemN{\Anchor{3F37}{37}}{
	...
	(a) \TextA{Show $\pi\apostrophe$ is inje\hspace{1pt}$:${\tgnr\FontNorm\;\;Becs $\pi$ is surj. Use [3.108].}\vspace{2pt}}
	(b) \TextA{Show $\range \pi\apostrophe=U^0$\hspace{1pt}$:${\tgnr\FontNorm\;\;By {\tgnr[3.109](b)}, $\range\pi\apostrophe=\BigPar{\null\pi}{^0}=U^0.$}\vspace{2pt}}
	(c) \TextA{Conclude that $\pi\apostrophe$ is iso from $\Par{V\XSlash U}\apostrophe$ onto $U^0$\hspace{1pt}$:${\tgnr\FontNorm\;\;Immed.}}
...
\ProblemN{\Anchor{3F34}{34}}{
	\TextA{The double dual space of $V$, denoted by $V\apostrophe\apostrophe$, is defined to be the dual space
		of $V\apostrophe$.}
	...
	(a) \TextA{Show $\Lambda$ is a liney map from $V$ to $V\apostrophe\apostrophe$.\vspace{2pt}}
	(b) \TextA{Show if $T\in\Lm{V}$, then $T\apostrophe\apostrophe\circ\Lambda=\Lambda\circ T$, where $T\apostrophe\apostrophe=\Par{T\apostrophe}\apostrophe$.\vspace{2pt}}
	(c) \TextA{Show if $V$ is finide, then {\tgsc $\Lambda$ is iso from $V$ onto $V$\hspace{1pt}$\apostrophe\apostrophe$}.\vspace{2pt}}
...
\ProblemN{\Anchor{3F36}{36}}{
	...
	(b) \TextA{Prove $\rangep i\apostrophe=U\apostrophe$\hspace{1pt}$:${\tgnr\FontNorm\;\;$\rangep i\apostrophe=\Par{\nullp i}{_U^0}={\zeroSubs}{_U^0}=U\apostrophe$.}\vspace{2pt}}
	(c) \TextA{Prove $\tilde{i\apostrophe}$ is iso from $V\apostrophe\XSlash U^0$ onto $U\apostrophe$\hspace{1pt}$:${\tgnr\FontNorm\;\;Immed.}\vspace{1pt}}
...
$\forall\varPhi\in\BigPar{\null T}{^0},$ let $\psi=\varPhi\circ R,$ then $T\apostrophe\Par{\psi}=\psi\circ T=\varPhi\circ\BigPar{R\circ T\mmid_V}=\varPhi\circ\iota=\varPhi\in\range T\apostrophe.$\PfEnd\vspace{3pt}\Anchor{3F4e17}
...
\ProblemN{\Anchor{3F4e24}\Anchor{3F6}{6}}{
	...
	(a) \TextA{Show $\Span{v_1,\dots,v_m}=V\,\Longleftrightarrow \Gamma$ is inje.}
	(b) \TextA{Show $\Par{v_1,\dots,v_m}$ is liney indep $\Longleftrightarrow \Gamma$ is surj.}
...
\Anchor{3F13}\ProblemN[]{13}{
	...
	(a) \TextA{Describe the liney functionals $T\apostrophe\Par{\varphi_1},T\apostrophe\Par{\varphi_2}.$}
	\Ha\TextA{{\FontNorm\tgnr For any $\Par{x,y,z}\in\Rbb^3$, $\BigPar{T\apostrophe\Par{\varphi_1}}\Par{x,y,z}=4x+5y+6z, \BigPar{T\apostrophe\Par{\varphi_2}}\Par{x,y,z}=7x+8y+9z$.}\vspace{6pt}}
	(b) \TextA{Write $T\apostrophe\Par{\varphi_1}$ and $T\apostrophe\Par{\varphi_2}$ as liney combinations of $\psi_1,\psi_2,\psi_3$.}
	\Hb\TextA{{\FontNorm\tgnr$T\apostrophe\Par{\varphi_1}=4\psi_1+5\psi_2+6\psi_3,\,\,T\apostrophe\Par{\varphi_2}=7\psi_1+8\psi_2+9\psi_3.$}\vspace{6pt}}
...
\Anchor{3E14}\ProblemBnoor{3.E.14}{
	...
	(a) \TextA{Show $U$ is a subsp of $\Fbb^{\infty}$. {\FontNorm\Sbra{Do it in your mind}} \;\; {\tgnr\large(b)} Prove $\Fbb^{\infty}\XSlash[-3pt]U$ is infinide.}
...
\Anchor{3D19}\ProblemBnoor{3.D.19}{
	...
	(a) \TextA{Prove $T$ is surj. \quad {\large\tgnr(b)} Prove for every non0 $p$, $\deg Tp=\deg p$.}
