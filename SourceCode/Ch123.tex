...
\ProblemBnoor[]{\Tips}[]{
	...
	\TextB{\Example \,\,\,$\Fbb=\Zbb_m=\Bra{K_0,K_1,\dots,K_{m-1}},\forall m-1\in\Nbp.$ \FontNorm\tgnr\Sbra{ Using Euler's Theorem. }}
}\SepLine
\ChEnd
...
%\vspace{10pt}
%\Example\,\,\, {Supp $\dim V=6,\dim U=3$.\par\vspace{2pt}\quad
...
%\AComm Conversely, if $\varphi=c\beta,\exists\,c\in\nonzeroFbb.$ \,Then $\null\beta=\null\Par{c\varphi}=\null\varphi.$\parCom
...
\BulletPointX\AComm If $U$ is merely a subset of $V,$ then [3.85, 3.86] do not hold, and $V\XSlash U$ is not a vecsp.\parCom{\IndentB}
...
\uline{If $U$ is a vecsp but not a subsp of $V,$ then everything will be all right.}\parCom{\IndentB}
If $U$ is a vecsp and $U\cap V=\zeroSubs,$ then $v+U=w+U\Rightarrow v=w.$\parCom{\IndentB}
Supp $V\XSlash U$ is a vecsp. Then certainly $U$ is a vecsp.
\SepLine
...
}{\def\mK{\mathcal{K}_{\!u}}Let $\mK=\Bra{v\in V:Tv=w}.$ \Sbra{ Not a vecsp.\hspace{1pt}} \;Supp $u\in\mK.$ Then $u+\null T\subseteq\mK.$\parSol{}
...
\ProblemN{\hypertarget{3E7}{7}}{
	\TextN{Supp $v,x\in V,$ and $U,W$ are subsps of $V.$ Prove $v+U=x+W\Rightarrow U=W$.}
...
\ProblemN{\hypertarget{3E9}{9}}{
	\TextN{Supp $A=v+U$ and $B=x+W$ for some $v,x\in V$ and some subsps $U,W$ of $V$.}
...
\Or Let $A=v+U,\;B=x+W.$ \;Supp $\alpha\in\Par{v+U}\cap\Par{x+W}\neq\emptySet.$\parSol{}
...
\BulletPointX\NoteFor{[3.88, 3.90, 3.91]}\;\;{Supp $W\in\mathcal{S}_{\!V} U.$ Then $V\XSlash U$ is iso to $W.$\quad\FontSmall\Sbra{\,Conversely not true.\,}}\TextB{}
...
{Then $\tilde{T}\in\Lm{V\XSlash \null T,V}$ is defined by $\tilde{T}\Par{v+U}=\tilde{T}\Par{w_v'+U}=Tw_v'=w_v.$ \FontSmall\Sbra{\tgsl See {\TIPSN{1}} below}}\TextB{}
...
\BulletPointX\NewNotation\;$\Pure V\XSlash U=W\Longleftrightarrow V=U\oplus W,\;W=\range S.$ {\FontSmall\tgsl The uniqnes of $\Pure V\XSlash U$ follows from $\range S.$}
\SepLine
...
\BulletPointX\NewNotation\;\;Suppose $T\in\Lm{V,W}.$ Define $T\XSlash{^U_X}\in\Lm[\BigPar]{V\XSlash U,W\XSlash X}$ by ${T\XSlash{^U_X}}\Par{v+U}=Tv+X.$
\SepLine\pagebreak
...
\Example \,\,\,\hypertarget{3F8}{}{\tgnr\FontNorm Supp $m\in\Nbp$. By [2.C.10], $B$ $=\BigPar{1,x-5,\dots,\Par{x-5}{^m}}$ is a bss of $\PoR{m}$.}\par
\Blind{\Example \,\,\,}{\tgnr\FontNorm Let $\varphi_k=\Frac{p^{\SmallPar{k}}\Par{5}}{k!}$ for each $k=0,1,\dots,m$. Then $\Par{\varphi_0,\varphi_1,\dots,\varphi_m}$ is the dual bss of $B$.}\par
\SepLine
}
