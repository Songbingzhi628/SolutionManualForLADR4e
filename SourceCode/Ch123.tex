...
\Or \NOTICE that $V=U\oplus\null T.$ By the {\COROLLARY} in Problem (12), $\range T=\range T\mmid_U.$\parSol{}
...
\ProblemB{
	...
	\hypertarget{3B4e24}{}(a) \TextB{Suppose $\dim V=n,$ \,$ST=0$ where $S,T\in\Lm{V}.$ Prove that $\dim\range TS\leqslant\left\lfloor\Frac{\;n\;}{2}\right\rfloor.$}
	(b) \TextB{Give an example of such $S, T$ with $n=5$ and $\dim\range TS = 2$.\vspace{2pt}}
}Note that $\dim\range TS\leqslant\min\!\envFontLarge\Bra{\!\dim\range T,\dim\range S}.$ We prove by contradiction.\par\vspace{2pt}\quad
...
Assume that $\dim\range TS\geqslant \Big\lfloor${\Large$\frac{\,n\,}{2}$}$\Big\rfloor+1.$ \;Then \;$\min\!\envFontLarge\Bra{n-\dim\null T,n-\dim\null S}\envFontDefault\geqslant \Big\lfloor${\Large$\frac{\,n\,}{2}$}$\Big\rfloor+1$\vspace{2pt}\par\quad
又 $\dim\null ST=n\leqslant\dim\null S+\dim\null T\;\Big|\hspace{-0.7pt}\Rightarrow\max\!\envFontLarge\Bra{\!\dim\null T,\dim\null S}\envFontDefault\leqslant n-\Big\lfloor${\Large$\frac{\,n\,}{2}$}$\Big\rfloor-1.$\par\vspace{2pt}\quad
Thus \;$n\leqslant 2\envFontLarge\BigPar{\envFontDefault n-\Big\lfloor${\Large$\frac{\,n\,}{2}$}$\Big\rfloor-1\envFontLarge}\envFontDefault\Rightarrow \Big\lfloor${\Large$\frac{\,n\,}{2}$}$\Big\rfloor+1\leqslant{}${\Large$\frac{\,n\,}{2}$}. \;Contradicts.\PfEnd\vspace{6pt}\quad
...
$\hspace{-2pt}\hText{$\Example \,\,\,Let $B_V=\Par{v_1,\dots,v_5}.$ \;Define$\\[-2pt]\;}\hText{T: \; v_1\mapsto 0, \;\;\, v_2\mapsto 0, \;\;\; v_i\mapsto v_i\;;\\[-2pt]\hspace{1pt}S: \; v_1\mapsto v_4, \; v_2\mapsto v_5, \;\; v_i\mapsto 0\;;\;\; i=3,4,5.}$\vspace{6pt}
\SepLine
...
(a) Suppose $U\subseteq W.$ Then $\forall\varphi\in W^0,u\in U\subseteq W,\varphi\Par{u}=0\Rightarrow\varphi\in U^0.$ Thus $W^0\subseteq U^0.$\par\quad
...
\Or \Sbra[3pt]{{\tgsl Req $U$ Subsp}} \;By Problem (25). Let $U=\Bra{v\in V:\varphi\Par{v}=0,\forall\varphi\in U^0}.$\par\quad\Hb
...
$\hText{$
	...
	\Ha$\varphi\Par{u}=\varphi\Par{w}=0\Rightarrow\varphi\in U^0\cap W^0.}\;\;\hMath{l}{\left|\;}{\;\right\}}{U\subseteq U+W\Rightarrow\BigPar{U+W}{^0}\subseteq U^0\\W\subseteq U+W\Rightarrow\BigPar{U+W}{^0}\subseteq W^0}$\vspace{4pt}\par\quad
...
\ProblemN{\hypertarget{3F23}{23}}{
	\TextNL{Suppose $U$ and $W$ are subsets of $V$. Prove that $\BigPar{U\cap W}{^0}=U^0+W^0$.}
}\vspace{2pt}\par\quad
$\hText{$
...
$\Blind{\Dim\BigPar{U^0+W^0}}=2\dim V-\dim U-\dim W-\BigPar{\!\dim V-\Dim\Par{U+W}}=\dim V-\Dim\Par{U\cap W}.$\par\vspace{6pt}\quad\Hb
\Or \Sbra[3pt]{{\tgsl Req $U,W$ Subsps}} \;Let $I=U\cap W.$ Using \TIPSN{3}.\par\vspace{-18pt}\quad\Hb
\!$\hText{$Now $S_V I=S_V U\oplus S_U I=S_V W\oplus S_W I.\\$Suppose $\varphi\in\Par{U\cap W}{^0}=I^0.$ Then $\varphi\in U^0$ or $W^0.\\\;}\qquad\hText{$\includegraphics[width=80pt]{diagram3F-1}$\\\;\\\;}\hText{$Replace $S_V$ with $\complement_V\\$and $\oplus$ with $\cup.$ Still true$\\$if merely subsets.$\\\;\\\;}$\PfEnd[-50pt]\vspace{-4pt}
\SepLine

\ProblemBnoor[]{\Corollary}[]{
	\;\;$\BigPar{U\cap W}{^0}=U^0+W^0\supseteq U^0\cap W^0=\BigPar{U+W}{^0}.$\TextB{\vspace{1pt}}
	(a) Suppose $\Bra{V_{\!\alpha_i}}{_{\alpha_i\in\Gamma}}$ is a collection of \uline{subsets} of $V$. Then ${\envFontLarge\BigPar{{\envFontDefault\bigcap_{\alpha_i\in\Gamma}V_{\!\alpha_i}}}}{^0}=\sum_{\alpha_i\in\Gamma}\BigPar{V^0_{\!\alpha_i}}.$\TextB{\vspace{2pt}}
	(b) Suppose $\Bra{V_{\!\alpha_i}}{_{\alpha_i\in\Gamma}}$ is a collection of \uline{subsps} of $V$. Then ${\envFontLarge\BigPar{{\envFontDefault\sum_{\alpha_i\in\Gamma}V_{\!\alpha_i}}}}{^0}=\bigcap_{\alpha_i\in\Gamma}\BigPar{V^0_{\!\alpha_i}}.$\TextB{\vspace{-2pt}}
}\SepLine
...
\ProblemBnoor{\hypertarget{3F4e23}{4E 23}}{
	...
	(a) \TextB{$\Span{\varphi_1,\dots,\varphi_m};$ \;\; {\tgnr\large(b)} $\BigPar{\Par{\null \varphi_1}\cap\dots\cap\Par{\null \varphi_m}}{^0}.$\vspace{2pt}}
	\Comment \,\,\,$\Span{\varphi_1,\dots,\varphi_m}=\Bra{\varphi\in V\apostrophe:\Par{\null\varphi_1}\cap\dots\cap\Par{\null\varphi_m}\subseteq\null\varphi}.$\TextB{}
}\par\quad
For each $\varphi_k=0,$ $\Span{\varphi_k}=\zeroSubs=\BigPar{\null\varphi_k}{^0}.$\par\quad
For each $\varphi_k\neq 0.$ Using (3.B.29) and {\TIPSN{1}}. $\exists\,v_k\in V,\varphi\Par{v_k}\neq 0.$ Now $\null\varphi_k\oplus\Span{v_k}=V.$\par\quad
Then $\BigPar{\null\varphi_k}{^0}=\BigPar{\Span{v_k}}{_V\upapostrophe}=\Bra{\varphi\in V\apostrophe:\varphi=\varphi\circ\iota}=\Span{\varphi_k},$ where $\iota:cv_k+u_0\rightarrow cv_k.$\par\quad
Thus $\Span{\varphi_1,\dots,\varphi_m}=\Span{\varphi_1}+\dots+\Span{\varphi_m}$\par\quad
\Blind{Thus $\Span{\varphi_1,\dots,\varphi_m}$}${}=\BigPar{\null\varphi_1}{^0}+\dots+\BigPar{\null\varphi_m}{^0}=\BigPar{\Par{\null \varphi_1}\cap\dots\cap\Par{\null \varphi_m}}{^0}.$\PfEnd\vspace{4pt}\quad
By Problem (26) \Sbra[3pt]{{\tgsl May req Finite-dim}}.\vspace{2pt}\par\quad\,
$\MathRightBrace{r}{
...
\Or By Problem (4E 6), for each $\varphi_i$, we have\par\quad
...
Using (3.B.29). Let $\varphi_i\Par{u_i}=1.$ Then each \uline{$\null\varphi_i\oplus\Span{u_i}=V.$}\par\quad
Now $a_1u_1+\dots+a_nu_n=0\Rightarrow$ \uline{Each $a_i=\varphi_i\Par{a_1u_1+\dots+a_nu_n}=0.$}\par\quad
Thus $B_V=\Par{u_1,\dots,u_n}$ with $\varphi_i\Par{u_x}=\delta_{i,x}.$\PfEnd\vspace{6pt}\quad
\Or Using Problem (30). Define each $\Gamma_{\!k}=\Bra{1,\dots,n}\Backslash[\Big]\Bra{k}$ and $U_k=\bigcap_{j\in\Gamma_{\!k}}\null\varphi_j.$ Then $\dim U_k=1.$\par\vspace{2pt}\quad
\Sbra{Cannot use (4E 2.C.16) here.} Thus each $U_k=\Span{u_k},\exists\,u_k\in V\nonzero.$\vspace{2pt}\par\quad
又 Each $U_k\cap\null\varphi_k=\Par{\null\varphi_1}\cap\dots\cap\Par{\null\varphi_n}=\zeroSubs.$\par\quad
Then if $\varphi_k\Par{u_k}=0\Rightarrow u_k\in\null\varphi_k$ while $u_k\in U_k\Rightarrow u_k=0,$ contradicts. Thus $\varphi_k\Par{u_k}\neq 0.$\par\quad
\Or Because each $\dim\null\varphi_k=n-1.$ Each $\null\varphi_k\oplus U_k=V.$ Thus $\varphi_k\Par{u_k}\neq 0.$\par\quad
Let $v_k=\BigPar{\varphi_k\Par{u_k}}{^{-1}}u_k\Rightarrow\varphi_k\Par{v_k}=1.$ Similarly.\PfEnd
\SepLine
...
By \Sbra{1.C \TIPSN{2}}, $\Span{\varphi_{m+1},\dots,\varphi_n}=\Span{v_1,\dots,v_m}{^0}.$\par\quad\HIII
