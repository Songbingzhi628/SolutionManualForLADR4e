...
\Or By (a), let $W=\Span{v_1,\dots,v_m}=\Span{w_1,\dots,w_m}.$ Supp $\Par{w_1,\dots,w_m}$ is liney dep.\par\quad\Hb
...
}${}^\neg Q\Rightarrow{}^\neg P:$ Supp $w\in\Span{v_1,\dots,v_m}$. Then $\Par{v_1,\dots,v_m,w}$ is linely depe.\parSol{}
${}^\neg P\Rightarrow{}^\neg Q:$ Supp $\Par{v_1,\dots,v_m,w}$ is liney dep. Then by [2.21](a), $w\in\Span{v_1,\dots,v_m}.$\PfEnd
\SepLine
...
\AComm \Or Using (4E 3.C 17).\vspace{-2pt}
\SepLine\pagebreak
...
Define $E\in\Lm{\range S,W}$ by $E\Par{Sv_i}=Tv_i.$ \;Extend to $E\in\Lm{W}.$\par\quad
...
\ProblemN{\hypertarget{3B25}{25}}{
	\TextNL{Supp $S, T\in\Lm{V,W},$ and $\range T\subseteq\range S.$ Prove $\exists\,E\in\Lm{V},T = SE$.}
}\par\quad
...
\Or $\forall v\in V,\exists\,!\,a_i\in\Fbb,Tv=a_1Tv_1+\dots+a_mTv_m.$ Let $B_{\SmallPar[1pt]{\range T}\apostrophe}=\Par{\psi_1, \dots, \psi_m}$.\parSol{\Hb}
Then $\Sbra{T\apostrophe\Par{\psi_{i}}}\Par{v}=\BigPar{\psi_{i}\circ T}\Par{v}=a_i.$ \,Thus each $\varphi_i=\psi_i\circ T=T\apostrophe\Par{\psi_i}\in V\apostrophe.$\PfEnd
\SepLine\pagebreak
...
\BulletPointX\NoteForSmall{Trspose}\;\;{\FontSmall\Sbra{\hypertarget{3F33}{3.F.33}}}\;Define $\fT:A\rightarrow A^t.$ By [3.111], $\fT$ is liney. Becs $\Par{A^t}{^t}=A.$ \TextB{}
...
\ProblemB{
	...
	\IndentTipsN{1}(b) \TextB{Show [P] $\Spn L=W\Longleftrightarrow\Spn M=\Fbb^{m,1}.$ [Q]\hfill\FontNorm\Sbra{ Let $A=\Mt{T,B_V,B_W}.$\hspace{1pt}}}
}\par\quad
...
\Note \,\,\,Let $L=\BigPar{Tv_1,\dots,Tv_n},\;M=\BigPar{A_{\cdot,1},\cdots,A_{\cdot,n}}.$\parNot
Then (a*) By \Sbra{3.B.9, \TIPSN{4}}, $T$ is inje $\Longleftrightarrow L$ is liney indep, so is $M$.\parNot
And (b*) $T$ is surj $\Longleftrightarrow\Spn L=W\Longleftrightarrow\Spn M=\Fbb^{m,1}.$\parNot
...
\Blind{$T$ surj }$\xLongleftrightarrow{\text{(a)}}{\envFontLarge\BigPar{{\envFontDefault\Par{A^t}{_{\cdot,1}},\cdots,\Par{A^t}{_{\cdot,m}}}}}$ liney indep in $\Fbb^{n,1},$ so is $\BigPar{A_{1,\cdot},\cdots,A_{m,\cdot}}$ in $\Fbb^{1,n}.$\vspace{6pt}\parNot
...
$\forall w\in W,\exists\,!\,\Par{u,x_1}\in U\times X_1,w=u+x_1.$ While $\exists\,!\,\Par{w\apostrophe,x_2}\in U\times X,u=w\apostrophe+x_2.$\parSol{}
Now $x_1=-x_2,w=w\apostrophe.$ Thus $\pi:U\rightarrow W$ defined by $\pi\Par{u}=w,$ \,is surj.\PfEnd
\SepLine

\ProblemB{
	\TextB{Supp $V=U\oplus X=W\oplus Y,$ and $X,Y$ are iso. Give a counterexa: $U,W$ are iso.}
}Let $V=\Fbb^\infty,U=\zeroSubs,X=\Fbb^\infty,Y=\Bra{\Par{0,x_1,x_2,\cdots}\in\Fbb^\infty},W=\Bra{\Par{x_1,0,\cdots}\in\Fbb^\infty}.$\parSol{}
Define $S\in\Lm{X,Y}$ by $S\Par{x_1,x_2,\cdots}=\Par{0,x_1,x_2,\cdots}.$ Becs $S$ is inje and surj.\PfEnd
\SepLine\pagebreak

\ProblemN{\hypertarget{3D3}{3}}{
	\TextN{Supp $V$ and $W$ are iso, $U$ is a subsp of $V$, and $S\in\Lm{U,W}.$}
...
%\Blind{$[P]\Rightarrow[Q]:\;$} \Or Supp $\vartheta\in\Span[\BigPar]{\Psi\Par{v_1},\dots,\Psi\Par{v_m}}$. Let $\vartheta=0=a_1\Psi\Par{v_1}+\dots+a_m\Psi\Par{v_m}.$\par\quad
%\Blind{$[P]\Rightarrow[Q]:\;$} Then $\vartheta\Par{1}=0=a_1 v_1+\dots+a_m v_m\Rightarrow a_1=\dots=a_m=0.$\vspace{2pt}\par\quad
%$[Q]\Rightarrow[P]:\;$ Supp $v\in\Span{v_1,\dots,v_m}.$ Let $v=0=a_1 v_1+\dots+a_m v_m.$\par\quad
...
%TODO: About the maps T: x\mapsto Ax, and T: x\mapsto xA.
...
\ProblemBnoor{4E 22, \OR \hypertarget{3D4e22}{10.A.1}}{
	\TextB{Supp $T\in\Lm{V}.$ Prove $\Mt{T,B_V}$ is inv $\Longleftrightarrow T$ itself is inv.}
...
\BulletPointX\NoteForSmall{[3.79, 3.83]}\;\;If $v\in U,$ then $v+U=0+U=\Bra{u:u\in U}=U.$ Now $U=0\in V\XSlash U.$\TextB{}
\Blind{\NoteForSmall{[3.79, 3.83]}\;\;}If $U=\zeroSubs,$ then $v+U=v+\zeroSubs=\Bra{v},\;V\XSlash U=V\XSlash\zeroSubs=\Bra{\Bra[\envFontB]{v}:v\in V}.$\TextB{}
\Blind{\NoteForSmall{[3.79, 3.83]}\;\;}If $V\subseteq U,$ then $u+V=0+V=V,\;U\XSlash V=\Bra{u+V:u\in U}=\zeroSubs=\Bra{V}.$\TextB{}
\Blind{\NoteForSmall{[3.79, 3.83]}\;\;}If $U=\emptySet,$ then $v+U=v+\emptySet=\emptySet,\;V\XSlash U=V\XSlash\emptySet=\Bra{\emptySet}.$\TextB{\vspace{2pt}}
If $U$ is merely a subset of $V,$ then [3.85, 3.86] do not hold, and $V\XSlash U$ is not a vecsp.\TextB{}
...
If $U$ and $V$ are subsps of some vecsp, and $U\cap V=\zeroSubs,$ then $v_1+U=v_2+U\in V\XSlash U\Rightarrow v_1=v_2\in V.$\TextB{}
Supp $V\XSlash U$ is a vecsp. Then certainly $U$ is a vecsp.\TextB{\vspace{2pt}}
\AComm Supp $U,V$ are subsps and $U$ is not a subsp of $V.$ Then $V\XSlash U=\Par{V+U}\XSlash U.$\TextB{}
Supp $v+U\in V\XSlash U.$ Then $v\in V$, or possibly $v\in V+U$ as well. To avoid this ambiguity,\TextB{}
you have to specify the precond, what subsp that $v$ belongs to.
\SepLine
...
(a) Supp $A=a+U.$ Then $\lambda\Par{a+u_1}+\Par{1-\lambda}\Par{a+u_2}=a+\BigPar{\lambda\Par{u_1-u_2}+u_2}\in A.$\vspace{2pt}\par\quad
(b) Supp $\lambda v+\Par{1-\lambda}w\in A,\forall v,w\in A,\lambda\in\Fbb.$ \;Supp \uline{$a\in A$} and let $A\apostrophe=\Bra{x-a:x\in A}.$\par\Hb\quad
Then $0\in A\apostrophe$ and $\forall\Par{v-a},\Par{w-a}\in A\apostrophe,\lambda\in\Fbb,$\par\vspace{0pt}\Hb\quad
...
Then $v-\alpha\in U\Rightarrow v+U=\alpha+U=A,$ and similr $v+W=\beta+W=B.$\parSol{}
We show $A\cap B=v+\Par{U\cap W}.$ Note that $v+\Par{U\cap W}\subseteq A\cap B.$\parSol{}
And $\forall w=v+u=v+w\in A\cap B\Rightarrow u=w\in U\cap W\Rightarrow w\in v+\Par{U\cap W}.$\PfEnd
\SepLine
...
\ProblemN{\hypertarget{3E18}{18}}{
	\TextNL{Supp $T\in\Lm{V,W}$ and $U$ is a subsp of $V.$ Let $\pi:V\rightarrow V\XSlash U$ be the quot map.}
	\TextNL{Prove $\exists\,S\in\Lm{V\XSlash U,W},T=S\circ\pi\Longleftrightarrow U\subseteq\null T$.}
}(a) Supp $\exists\,S\in\Lm{V\XSlash U,W},T=S\circ\pi.$ Then $U=\null\pi\subseteq\null\Par{S\circ\pi}=\null T.$\parSol{}
...
\hypertarget{3E20}{}\ACoro Define $\Gamma:S\mapsto S\circ\pi.$ Then  $\Gamma$ is inje, $\range\Gamma=\Bra{T\in\Lm{V,W}:U\subseteq\null T}.$\PfEnd\vspace{2pt}
\AComm The map $S:V\XSlash U\rightarrow V$ defined by $S\Par{v+U}=v$ is not well-defined, unless $U=\zeroSubs\subseteq I_V.$
\SepLine

\BulletPointX\NoteFor{[3.88, 3.90, 3.91]}\;\;{Supp $W\in\mathcal{S}_{\!V} U.$ Then $V\XSlash U$ is iso to $W.$\quad\FontSmall\Sbra{\,Convly not true.\,}}\TextB{}
...
Thus define \;$\varphi\in\Lm[\BigPar]{V,U\times\Par{V\XSlash U}}$\quad\;\hspace{1.4pt}and \;$\psi\in\Lm[\BigPar]{U\times\Par{V\XSlash U},V}$\par\quad
\Blind{Thus defi}by \;$\varphi\Par{v}=\BigPar{u,\;\sum_{i=1}^nv_i+U},$ and \;$\psi\Par{u,\;v+U}=\sum_{i=1}^nv_i+u.$ \quad Then $\psi=\varphi^{-1}.$\PfEnd\vspace{8pt}\quad
...
\!\!\!$\MathRightBrace{l}{\Par{\psi\circ\varphi}\Par{u_v+w_v}=\psi\Par{u_v,w_v+U}=u_v+w_v\\\Par{\varphi\circ\psi}\Par{u,v+U}=\varphi\Par{u+w_v}=\Par{u,w_v+U}}\Rightarrow\psi=\varphi^{-1}.$ \; \OR Becs $\psi$ or $\varphi$ is inje and surj.\PfEnd
...
\ProblemBnoor{\hypertarget{3E4e14}{4E 14}}{
	\TextB{Supp $V=U\oplus W,\;B_W=\Par{w_1,\dots,w_m}.$ Prove $B_{V\XSlash U}=\BigPar{w_1+U,\dots,w_m+U}.$\vspace{2pt}}
...
\ProblemB{
	\TipsN{1}\,\,\,\TextB{Supp $U,W$ are subsps of $V.$ Let $I=U\cap W.$}
	\IndentTipsN{1}\TextB{Prove $V=U+W\Longleftrightarrow V\XSlash I=U\XSlash I\oplus W\XSlash I.$}
}(a) Supp $U+W.$ Then $\forall x\in V\XSlash I,\exists\,v\in V,\Par{u_v,w_v}\in U\times W,x=v+I=\Par{u_v+w_v}+I.$\parSol{\Ha}
...
\uline{$\Rightarrow\exists\,w'\in I,u=w+w'\in U\cap W$}${}\Rightarrow x=u+I=0+I.$ Thus $\Par{U\XSlash I}\,${\Large$\cap$}$\,\Par{W\XSlash I}=\zeroSubs.$\parSol{\vspace{3pt}}
...
\newcommandx{\LmQxx}[3][2=U,3=X]{#1\XSlash{^{#2}_{\!#3}}}

\ProblemB{
	Supp $T\in\Lm{V,W},$ and $U,V$ are subsps of some vecsp, and $X,W$ are subsps of some vecsp.\TextB{}
	\TextB{Define $\LmQxx{T}:V\XSlash U\rightarrow W\XSlash X$ by $\LmQxx{T}\Par{v+U}=Tv+X.$\vspace{2pt}}
	(a) \TextB{Prove $\LmQxx{T}$ is well-defined $\Longleftrightarrow\range T\mmid_U$ is a subsp of $X.$\vspace{2pt}}
	Now supp $\LmQxx{T}$ is well-defined.\;Define $\pi_U\in\Lm{V,V\XSlash U}$ by $\pi:v\mapsto v+U.$ Similr for $\pi_X.$\TextB{}
	Then $\LmQxx{T}\circ\pi_U=\pi_X\circ T.$ \;Define $\xi_V\in\Lm[\BigPar]{V\XSlash\zeroSubs,V}$ by $\xi_V:v+\zeroSubs\mapsto v.$ Similr for $\xi_W.$\TextB{\vspace{3pt}}
	...
	(d) \TextB{Show $\null\LmQxx{T}[0][\!X]=\BigPar{\range\xi_V\text{\large\Big|}{_{\null\LmQxx{T}[0][\!X]}}}\XSlash U.$ \; {\FontSmall Where $\LmQxx{T}[0][\!X]\BigPar{v+\zeroSubs[\def\envFont{\small}]}=Tv+X.$}\vspace{2pt}}
	(e) \TextB{$\LmQxx{T}$ is inje $\Longleftrightarrow\range\xi_V\text{\large\Big|}{_{\null\LmQxx{T}[0][\!X]}}\subseteq U.$}
}\par\quad
(a) If $v+U=w+U\Longleftrightarrow v-w\in U\Rightarrow Tv-Tw\in X\Longleftrightarrow Tv+X=Tw+X.$\par\quad\Ha
Then $\forall u\in U,Tu\in X\Rightarrow\range T\mmid_U\subseteq X.$ \,Convly true as well.\vspace{2pt}\par\quad
(c) Supp $\LmQxx{T}$ is surj. $\forall w\in W,w+X\in W\XSlash X$\par\quad\Hc
$\Rightarrow\exists\,Tv+X=w+X\Rightarrow w-Tv\in X\Rightarrow w\in\range T+X.$ \,Hence $W\subseteq\range T+X.$\vspace{2pt}\par\quad\Hc
Convly, $W\subseteq\range T+X\Rightarrow\Par{\range T}\XSlash X=\Par{\range T+X}\XSlash X\supseteq W\XSlash X.$\vspace{4pt}\par\quad
(d) $v+U\in\null\LmQxx{T}\Longleftrightarrow Tv\in X\Longleftrightarrow v+\zeroSubs\in\null\LmQxx{T}[0][\!X]\Longleftrightarrow v+U\in\BigPar{\range\xi_V\text{\large\Big|}{_{\null\LmQxx{T}}}}\XSlash U.$\PfEnd\vspace{4pt}
\BulletPointX\AComm Supp $T\in\Lm{V}.$ Define $T\XSlash U\in\Lm{V\XSlash U}$ by $T\XSlash U=\LmQxx{T}[U][U].$ Then\TextB{}
(a) $T\XSlash U$ is well-defined $\Longleftrightarrow \range T\mmid_U$ is a subsp of $U\Longleftrightarrow U$ is invard $T.$\TextB{}
...
(d) $\null T\XSlash U=\BigPar{\range\xi\text{\large\Big|}{_{\null\LmQxx{T}[0][\!U]}}}\XSlash U.$\;\;(e) $T\XSlash U$ inje $\Longleftrightarrow\range\xi\text{\large\Big|}{_{\null\LmQxx{T}[0][\!U]}}\subseteq U.$
\SepLine\pagebreak
...
\ProblemB[]{
	\TextB{Supp $V$ is a vecsp and $U$ is a subsp of $V.$\vspace{2pt}}
	\ProblemN[]{}{}
	\ProblemN[]{\hypertarget{3F18}{18}}{
		\TextNL{$U^0=V\apostrophe\Longleftrightarrow\forall\varphi\in V\apostrophe, U\subseteq\null \varphi\,\Longleftrightarrow U=\zeroSubs$. \Sbra[3pt]{{\FontNorm Which means $\zeroSubs{^0_V}=V\apostrophe.$}}\vspace{4pt}}
%		\TextNL{\FontNorm\tgnr\Or $U^0=V\apostrophe\Longleftrightarrow\dim U^0=\dim V\Longleftrightarrow\dim U=0.$}\vspace{5pt}
	}
...
\ProblemB{
	...
	\IndentTipsN{1}(b) \TextB{Supp $U\oplus W=V.$ Prove $U^0=\Bra{\varphi\in V\apostrophe:\varphi=\varphi\circ\iota},$}
...
\ProblemN{\hypertarget{3F31}{31}}{
	\TextNL{Supp $U$ is a subsp of $V.$ \;Let $B_{U_V\upapostrophe}=\Par{\varphi_1,\dots,\varphi_n}.$ Show corres $B_U$ exis.}
...
\AExa Cannot extend $B_U$ freely. Let $B_V=\Par{e_1,e_2-e_1}.$ Let corres  $B_{V\apostrophe}=\Par{\varphi_1,\varphi_2}.$\parExa
Let $U_V\upapostrophe=\Span{\varphi_1}.$ Then extend to $B_U=\Par{e_1}$ to $B_V'=\Par{e_1,e_2}.$ Corres $B_{V\apostrophe}'\neq B_{V\apostrophe}.$
\SepLine
}
...
\AComm If $\Omega$ is infinide. Then $\null_I=\bigcap_{\varphi\,\in\,\Omega}\null\varphi=\null_C.$
\SepLine
...
}{\normalsize\hfill Here (a) is {\FontSmall\Sbra{\hypertarget{3F4e23}{4E 23}}}, (b) is Exe \hypertarget{3F26}{(26)}.}\par\quad
...
\Or Simlr to (II), let $\Omega=\Span{\varphi_{m+1},\dots,\varphi_n},$ immed.\PfEnd
\SepLine
