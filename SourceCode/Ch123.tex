...
and $1+1=0.$ Then the list $\Par{1}$ is the unique basis. Now the vecsp $\Bra{0,1}$ will do.\parSol{}
...
(b) Define the range-restricted map $R$ of $T$ by $R=T\mmid_{\mathcal{K}_{\!Y}}.$\par\quad{\Hb}
Obviously $R:\mathcal{K}_{\!Y}\rightarrow Y$ is linear. Now $\range R=Y\cap \range T.$\par\quad{\Hb}
...
\Blind{\Or}$Ac=\Mt{Tv,B_W}=\Mt[\BigPar]{T\Par{c_1v_1+\dots+c_nv_n},B_W}=c_1{\small\begin{pmatrix}A_{1,1}\\[-2pt]\vdots\\[-6pt]A_{m,1}\end{pmatrix}}+\dots+c_n{\small\begin{pmatrix}A_{1,n}\\[-2pt]\vdots\\[-6pt]A_{m,n}\end{pmatrix}}.$\PfEnd
\SepLine
