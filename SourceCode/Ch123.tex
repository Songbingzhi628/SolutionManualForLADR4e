...
\ProblemB[]{
	\TipsN{5}\,\,\,Suppose $S\in\Lm{U,V}$ is surj. Define $\mathcal{B}\in\Lm[\BigPar]{\Lm{V,W},\Lm{U,W}}$ by $\mathcal{B}\Par{T}=TS.$\TextB{}
	\IndentTipsN{5}Then $\mathcal{B}$ is inje. Because $\mathcal{B}\Par{T}=TS=0\Longleftrightarrow T\mmid_V=T\mmid_{\range S}=0.$\TextB{\vspace{-4pt}}
...
\ProblemBnoor{\hypertarget{3D4e19}{}10.A.3, \OR 4E 3.D.19}{
	\TextB{Suppose $V$ is finite-dim and $T\in\Lm{V}$.\large\envFontDefault\hfill\Sbra{\Sbra{See also in (3.A).}}}
...
Suppose $x,y\in\bigcap_{\alpha\in\Gamma}A_{\alpha}\neq\emptyset.$ Then by Problem (8), $\forall\lambda\in\Fbb,\lambda x+\Par{1-\lambda}y\in A_{\alpha}$ for each $\alpha\in\Gamma.$\PfEnd\vspace{6pt}\quad
\Or Let each $A_\alpha=w_\alpha+V_\alpha.$ Suppose $x\in\bigcap_{\alpha\in\Gamma}\Par{w_\alpha+V_\alpha}\neq\emptyset.$\par\quad
Then for each $A_\alpha,\exists\,v_\alpha\in V_\alpha,x=w_\alpha+v_\alpha\Rightarrow x-w_\alpha\in V_\alpha\Rightarrow A_\alpha=w_\alpha+V_\alpha=x+V_\alpha.$\par\vspace{2pt}\quad
(a) $y\in\bigcap_{\alpha\in\Gamma}A_\alpha\Rightarrow\forall\alpha\in\Gamma,\exists\,v_\alpha,y=x+v_\alpha\Rightarrow\forall\alpha,\beta\in\Gamma,v_\alpha=v_\beta\Rightarrow y\in x+\bigcap_{\alpha\in\Gamma}V_\alpha.$\par\vspace{2pt}\quad
...
\ProblemBnoor{\hypertarget{3E4e8}{4E 8}}{
...
}{\def\mK{\mathcal{K}_w}Let $\mK=\Bra{u\in V:Tu=w}.$ Suppose $u\in\mK.$\parSol{}
	...
	\Comment \,\,\,$\mK$ is not a vecsp.}
\SepLine

\ProblemN{\hypertarget{3E18}{18}}{
	\TextNL{Suppose $T\in\Lm{V,W}$ and $U$ is a subsp of \,$V.$}
	\TextNL{Prove that $\exists\,S\in\Lm{V\XSlash U,W},T=S\circ\pi\Longleftrightarrow U\subseteq\null T$.}
}Note that $\pi:V\rightarrow V\XSlash U$ and that $\null\pi=U$ because $u\in U\Longleftrightarrow\pi\Par{u}=u+U=0.$\par\quad
(a) Suppose $\null\pi\subseteq\null T.$ By (3.B.24), we are done.\par\quad\Ha
\Or Define $S\in\Lm{V\XSlash U,W}$ by $S\Par{v+U}=Tv.$ Then $S\circ\pi=T.$ Now we show that $S$ is {\tgsl well-defined}.\par\vspace{-2pt}\quad\Ha
$v_1+U=v_2+U\Longleftrightarrow\Par{v_1-v_2}\in U\Longleftrightarrow S\BigPar{\Par{v_1-v_2}+U}=T\Par{v_1-v_2}=0\Longleftrightarrow Tv_1=Tv_2.$\par\vspace{2pt}\quad
(b) Suppose $\exists\,S\in\Lm{V\XSlash U,W},T=S\circ\pi.$ Then $U=\null\pi\subseteq\null\Par{S\circ\pi}=\null T.$\PfEnd\vspace{2pt}\quad
\hypertarget{3E20}{}\Corollary \,\,\,Define $\Gamma:S\mapsto S\circ\pi.$ Then $\Gamma$ is inje, $\range\Gamma=\Bra{T\in\Lm{V,W}:U\subseteq\null T}.$
\SepLine

\BulletPointX\NoteForSmall{[3.79, 3.83]}\;If $U=\zeroSubs,$ then $v+U=v+\zeroSubs=\Bra{v},\;V\XSlash U=V\XSlash\zeroSubs=\Bra{\Bra{v}:v\in V}.$
\SepLine
