...
%\AComm \Or Using (4E 3.C 17). Let $\mathcal{V}=\Par{v_1,\dots,v_m},\mathcal{W}=\Par{w_1,\dots,w_m}.$\parCom
%Let $T=I\mmid_{\Spn\mathcal{W}}\in\Lm{\Spn\mathcal{W},\Spn\mathcal{V}}$ and $A=\Mt{T,\mathcal{W},\mathcal{V}}.$\par\quad
%$P\Rightarrow Q:\;\Par{A_{\cdot,1},\dots,A_{\cdot,m}}$ is liney indep and spans $\Fbb^{m,1}\Longleftrightarrow\Par{Iw_1,\dots,Iw_m}$ is liney indep and spans $V.$\par\quad
%$Q\Rightarrow P:$ \;Simlr.
\SepLine
...
we can extend inje $T\BigPar{S\mmid_{U}}{^{-1}}\in\Lm[\BigPar]{\range S,W}$ to inv $E\in\Lm{W}.$\vspace{6pt}\par\quad
...
\ANote $\null S=\null T\Rightarrow \range S,\range T$ iso. While $\range S=\range T\notRightarrow\null S,\null T$ iso.\parNot\quad
Counterexa: Forward shift optor on $\Fbb^{\infty}$ and backward shift optor on $\Bra{\Par{0,x_1,x_2,\cdots}\in\Fbb^{\infty}}.$\par\vspace{8pt}\quad
\Sbra[3pt]{{\tgsl Req $\range T$ Finide}} \;Let $B_{\range T}=\Par{Tv_1,\dots,Tv_n}.$ Then \uline{$V=\Span{v_1,\dots,v_n}\oplus\null T.$}\par\quad
...
\BulletPointX\NoteForSmall{[3.30, 32]}\;\;{\tgsl matrix of equations}\TextB{}
Let the lists $L_{\alpha}=\Par{\alpha_1,\dots,\alpha_n},L_{\beta}=\Par{\beta_1,\dots,\beta_m}$ be in a vecsp $V.$\TextB{}
Supp each $\alpha_k=A_{1,k}\beta_1+\dots+A_{m,k}\beta_m.$ Forming $A\in\Fbb^{m,n},$ which is the matrix of equations.\TextB{}
...
\end{pmatrix}}={\normalsize\begin{pmatrix}\alpha_1 &\hspace{-6pt} \cdots &\hspace{-6pt} \alpha_n\end{pmatrix}}.$\TextB{\vspace{4pt}}
(a) Supp $m=n.$ If $\Par{A_{\cdot,1},\dots,A_{\cdot,n}}$ is liney indep. Then $L_{\alpha}$ liney indep $\Longleftrightarrow L_{\beta}$ liney indep.\TextB{}
(b) Supp $m\geqslant n.$ If $L_{\beta}$ liney indep. Then $\Par{A_{\cdot,1},\dots,A_{\cdot,n}}$ liney indep $\Longleftrightarrow L_{\alpha}$ liney indep.\TextB{}
(c) Supp $m<n.$ Then $L_{\alpha}$ is liney dep.
\SepLine
...
\ProblemB{
	...
	\IndentTipsN{1}(b) \TextB{Show [P] $\Spn L=W\Longleftrightarrow\Spn L_{\mathcal{M}}=\Fbb^{m,1}.$ [Q]\hfill\FontNorm\Sbra{ Let $A=\Mt{T,B_V,B_W}.$\hspace{1pt}}}
}\par\quad
(a) Note that $\mathcal{M}\!:\,Tv_k\rightarrow A_{\cdot,k}$ is iso of $W$ onto $\Fbb^{m,1}.$ \quad (b) Reduce $L$ to $B_W',$ and $L_{\mathcal{M}}$ to $B_{\Fbb^{m,1}}.$ Simlr.\PfEnd\vspace{4pt}\quad
...
}$\Par{U\cup W}{^0}=\BigPar{\varphi\in V\apostrophe:\varphi\Par{u}=\varphi\Par{w}=\varphi\Par{u+w}=0,\forall u\in U,w\in W}=\Par{U+W}{^0}.$\PfEnd
\SepLine\pagebreak
...
$=2\dim V-\dim U-\dim W-\BigPar{\!\dim V-\Dim\Par{U+W}}=\dim V-\Dim\Par{U\cap W}.$\PfEnd\vspace{4pt}
