...
\ProblemN{1}{
	\vspace{2pt}\TextN{Provet $\forall v\in V,-\Par{-v} = v$.}
}$-\Par{-v}=\Par{-1}\BigPar{\Par{-1}v}=\BigPar{\Par{-1}\Par{-1}}v=1\cdot v=v.$\parSol{\vspace{4pt}}
\Or Beca $-\Par{-v}+\Par{-v}=0$ 又 $v+\Par{-v}=0.$ Now by the uniqnes of add inv.\PfEnd
\SepLine

\ProblemN{2}{
	\TextN{Sup $a\in\Fbb,v\in V$, and $av = 0$. Provet $a = 0$ or $v = 0$.}
}Sup $a\neq 0,\exists\,a^{-1}\in\Fbb,a^{-1}a=1,$ hence $v=1\cdot v=\Par{a^{-1}a}v=a^{-1}\Par{av}=a^{-1}\cdot 0=0.$\PfEnd
\SepLine

\ProblemN{3}{
	\TextN{Sup $v, w\in V$. Explain why $\exists\,!\,x\in V,v + 3x = w.$}
...
\ProblemN[]{5}{
	\TextN{Showt in the def of a vecsp, the add inv condition can be replaced by [1.29].}
	\TextN{\large{\tgsc Hint\tgbf:} Sup $V$ satisfies all conds in the def, except we've replaced the add inv cond with [1.29].}
	\TextN{\large\Blind{{\tgsc Hint\tgbf:}} Provet the add inv is true.}
}\Blind{\IndentN} Using [1.31]. $0v = 0$ for all $v\in V$ $\Longleftrightarrow \Par{1+\Par{-1}}v=1\cdot v+\Par{-1}v=v+\Par{-v}=0.$\PfEnd
%\Blind{\IndentN} \Comment \,\,\,Beca $\forall v\in V,v+\Par{-1}v=0v=0\Rightarrow\exists\,w=\Par{-1}v\in V,v+w=0.$\par
...
\BulletPointX\NoteForSmall{[1.45]}\;\;If $\Fbb=\Bra{0,1}.$ Provet if $U+W$ is a direct sum, then $U\cap W=\zeroSubs.$\TextB{}
Beca $\forall v\in U\cap W,\exists\,!\,\Par{u,w}\in U\times W,v=u+w.$\TextB{}
If $U\cap W\neq\zeroSubs,$ then $\Par{u,w}$ can be $\Par{v,0}$ or $\Par{0,v},$ ctradic the uniqnes.\PfEnd\vspace{-2pt}
\SepLine

\BulletPointX\TipsN{1}\,\,\,Sup $U,W\subseteq V.$ And $U,W,V$ are vecsps $\Rightarrow U,W$ are subsps of $V.$\TextB{}
\IndentTipsN{1}Then $U+W$ is also a subsp of $V.$ Beca $\forall u\in U,w\in U,u+w\in V$ since $u,w\in V.$
\SepLine
...
Sup $h\Par{x}=\cos x+\sin\!\sqrt{2}x\in S$, since $\cos x,\sin\!\sqrt{2}x\in S$.\par\quad
Asm $\exists\,p\in\Nbp$ shat $h\Par{x}=h\Par{x+p},\forall x\in\Rbb.$ Let $x=0\Rightarrow h\Par{0}=h\Par{\pm p}=1$.\par\quad
...
\Or Beca $[\text{\tgbf{I}}]: \cos x+\sin\!\sqrt{2}x=\cos\!\Par{x+p}+\sin\!\envFontLarge\BigPar{\!\sqrt{2}x+\sqrt{2}p}.$ By differentiating twice,\par\vspace{2pt}\qquad\qquad\qquad\hspace{0pt}
...
\ProblemN{\hypertarget{1C24}{24}}{
	\TextNL{Let $V_{\!E}=\Bra{\,f\in\Rbb^{\Rbb}:\text{f is even}},V_{\!O}=\Bra{\,f\in\Rbb^{\Rbb}:\text{f is odd}}.$ Showt $V_{\!E}\oplus V_{\!O}=\Rbb^{\Rbb}.$\vspace{4pt}}
...
\BulletPointX\Largesl{Sup $U,W,V_{\!1},V_{\!2},V_{\!3}$ are subsps of $V.$}\par
...
}Sup $\Omega$ is the unique add id.\par\quad
(a) For any subsp $U$ of $V$. $\Omega\subseteq U+\Omega=U\Rightarrow\Omega\subseteq U$. Let $U=\zeroSubs,$ then $\Omega=\zeroSubs.$\par\quad
(b) Now sup $W$ is an add inv of $U\Rightarrow U+W=\Omega$.\par\quad\Hb
Note that $U+W\supseteq U,W\Rightarrow \Omega\supseteq U,W$. Thus $U=W=\Omega=\zeroSubs.$\PfEnd
\SepLine\pagebreak

\ProblemN{\hypertarget{1C11}{11}}{
	\TextNL{Provet the intersec of every collec of subsps of $V$ is a subsp of $V$.}
}Sup $\Bra{U_{\alpha}}{_{\alpha\in\Gamma}}$ is a collec of subsps of $V$; here $\Gamma$ is an index set.\vspace{2pt}\par\quad
We showt $\bigcap_{\alpha\in\Gamma}U_\alpha,$ which equals the set of vecs that are in $U_\alpha$ for each $\alpha\in\Gamma$, is a subsp of $V$.\par\vspace{4pt}\,
$\begin{array}{l}$
...
\ProblemN{\hypertarget{1C12}{12}}{
	\TextNL{Sup $U,W$ are subsps of $V.$ Provet $U\cup W$ is a subsp of $V\Longleftrightarrow U\subseteq W$ or $W\subseteq U$.}
}(a) Sup $U\subseteq W$. Then $U\cup W=W$ is a subsp of $V$.\parSol{}
(b) Sup $U\cup W$ is a subsp of $V$. Asm $U\not\subseteq W,\;U\not\supseteq W$ \BigPar{ $U\cup W\neq U$ and $W$ }.\parSol{\Hb}
...
Ctradic asm.$\\\;}$\PfEnd[-14pt]
\SepLine

\ProblemN{\hypertarget{1C13}{13}}{
	\TextNL{Provet the union of three subsps of $V$ is a subsp of $V$}
	\TextNL{if and only if one of the subsps contains the other two.}
	\TextNL{\large This exercise is not true if we \uline{replace $\Fbb$ with a field containing only two ele.}}
}\par\quad
Sup $U_1,U_2,U_3$ are subsps of $V$. Denote $U_1\cup U_2\cup U_3$ by $\mathcal{U}.$\par\quad
(a) \dbsp Sup that one of the subsps contains the other two.\par\quad\Ha
\dbsp Then $\mathcal{U}=U_1,U_2$ or $U_3$ is a subsp of $V.$\par\quad
(b) Sup that $U_1\cup U_2\cup U_3$ is a subsp of $V$.\par\quad\Hb
...
{\dbsp}By applying Exe (12) we conclude that one $U_j$ contains the other two. Thus we are done.\vspace{6pt}\par\quad\Hb\EndI
(II) \envFontLarge\dbsp{Asm no $U_j$ is contained in the union of the other two,}\par\quad\Hb\HII
\Blind{Asm }{\vspace{6pt}and no $U_j$ contains the union of the other two. Say $U_1\not\subseteq U_2\cup U_3$ and $U_1\not\supseteq U_2\cup U_3.$}\par\quad\Hb\HII
...
{\Large\vspace{6pt}If $U_2\subseteq U_3$ or $U_2\supseteq U_3,$ then $\mathcal{U}=U_1\cup U_i,i=2,3.$} {By Exe (12) we are done.}\par\quad\Hb\HII
{\Large\vspace{6pt}Otherwise, {\large\envFontDefault both $U_2,U_3\neq\zeroSubs.$} Beca \tgsl$W\subseteq U_2\cup U_3$ has at least three ele.}\par\quad\Hb\HII
...
\ProblemB{
	\TextB{Sup $U=\Bra{\Par{x,x,y,y}},W=\Bra{\Par{x,x,x,y}}\subseteq\Fbb^4.$ Provet $U+W=\Bra{\Par{x,x,y,z}}.$\vspace{2pt}}
...
\ProblemN{\hypertarget{1C21}{21}}{
	\TextNL{Sup $U=\Bra{\Par{x,y,x+y,x-y,2x}}$. Find a $W$ shat $\Fbb^5=U\oplus W$.}
...
\ProblemN{\hypertarget{1C23}{23}}{
	\TextNL{Give an exa of vecsps $V_{\!1},V_{\!2},U$ shat $V_{\!1}\oplus U=V_{\!2}\oplus U$, but $V_{\!1}\neq V_{\!2}$.\vspace{3pt}}
}$V=\Fbb^2$,  $U=\Bra{\Par{x,x}}$, $V_{\!1}=\Bra{\Par{x,0}}$, $V_{\!2}=\Bra{\Par{0,x}}$.\par
\SepLine

\ProblemB{
	\TipsN{2}\,\,\,\TextB{Sup $V_{\!1}\subseteq V_{\!2}$ in Exercise {\tgnr(23)}. Provet $V_{\!1}=V_{\!2}.$}
}\par\quad
Beca the subset $V_{\!1}$ of vecsp $V_{\!2}$ is closd add and scalar multi, $V_{\!1}$ is a subspace of $V_{\!2}.$\par\quad
Sup $W$ is shat $V_{\!2}=V_{\!1}\oplus W.$ Now $V_{\!2}\oplus U=\Par{V_{\!1}\oplus W}\oplus U=\Par{V_{\!1}\oplus U}\oplus W=V_{\!1}\oplus U.$\par\quad
If $W\neq\zeroSubs,$ then $V_{\!1}\oplus U\subsetneq\Par{V_{\!1}\oplus U}\oplus W$, ctradic. Hence $W=\zeroSubs,V_{\!1}=V_{\!2}.$\PfEnd
\SepLine

\ProblemB{
	\TextB{Sup $V_{\!1},V_{\!2},U_1,U_2$ are vecsps, $V_{\!1}\oplus U_1=V_{\!2}\oplus U_2,\;V_{\!1}\subseteq V_{\!2},\;U_2\subseteq U_1.$}
...
\ProblemB{
	\TipsN{3}\,\,\,\TextB{Sup the intersec of any two of the vecsps $U,W,X,Y$ is $\zeroSubs.$}
	\IndentTipsN{3}\TextB{Give an exa that $\BigPar{X\oplus U}\,${\LARGE$\cap$}$\,\BigPar{Y\oplus W}\neq\zeroSubs.$}
}\Sbra{ Using notations in Chapter 2. } \;Let $B_X=\Par{e_1},B_U=\Par{e_2-e_1},B_Y=\Par{\,},B_W=\Par{e_2}.$
\SepLine

\ProblemB{
	\TipsN{4}\,\,\,\TextB{Let $V=U+W,\;I=U\cap W,\,U=I\oplus X,\,W=I\oplus Y.$ Provet $V=I\oplus\Par{X\oplus Y}.$}
}We showt $X\cap Y=U\cap Y=W\cap X=\zeroSubs$ by ctradic.\parSol{}
$X\cap Y=\Delta\neq\zeroSubs\Rightarrow I=U\cap W\supseteq\Delta\Rightarrow I\cap X\neq\zeroSubs, I\cap Y\neq\zeroSubs.$\parSol{}
$U\cap Y=\Delta\neq\zeroSubs\Rightarrow I=U\cap W\supseteq\Delta\Rightarrow I\cap Y\neq\zeroSubs.$ Similar for $W\cap X.$\parSol{\vspace{2pt}}
Thus $I+\Par{X+Y}=\Par{I\oplus X}\oplus Y=I\oplus\Par{X\oplus Y}.$\parSol{\vspace{2pt}}
Now we showt $V=I+\Par{X+Y}.$ \;$\forall v\in V,v=u+w,\exists\,\Par{u,w}\in U\times W$\parSol{}
$\Rightarrow\exists\,\Par{i_u,x_u}\in I\times X,\,\Par{i_w,y_w}\in I\times Y,\;v=\Par{i_u+i_w}+x_u+y_w\in I+\Par{X+Y}.$\PfEnd
\SepLine

%\ProblemB{
%	\TipsN{3}\,\,\,\TextB{Let $V=U+W,\,I=U\cap W,\,U=I\oplus X,W=I\oplus Y.$ Provet $V=I\oplus\Par{X\oplus Y}.$}
%}Note that $V=\Par{I\oplus X}+\Par{I\oplus Y},$ and $X\cap Y=\zeroSubs.$\parSol{}
%\SepLine

%\ProblemN{\hypertarget{1C22}{22}}{
%	\TextNL{Sup $U=\Bra{\Par{x,y,x+y,x-y,2x}\in\Fbb^5}$.}
%	\TextNL{Find nonzero subsps $W_1,W_2,W_3$ of $\,\Fbb^5$ shat $\Fbb^5 = U\oplus W_1\oplus W_2\oplus W_3 $.}
%}\par\quad
...
\ProblemN{\hypertarget{2A1}{1}}{
	\TextN{Provet [P] $\Par{v_1,v_2,v_3,v_4}$ spans $V\Longleftrightarrow\BigPar{v_1-v_2,v_2-v_3,v_3-v_4,v_4}$ also spans V [Q].}
}Note that $V=\Span{v_1,\dots,v_n}\Longleftrightarrow\forall v\in V,\exists\,a_1,\dots,a_n\in\Fbb,v=a_1v_1+\dots+a_nv_n.$\par\quad
Asm $\forall v\in V,\exists\,a_1,\dots,a_4,b_1,\dots,b_4\in\Fbb,$ \BigPar{ that is, if $\exists\,a_i,$ then we are to find $b_i,$ vice versa }\par\quad
...
\ProblemB{
	\hypertarget{2A4e314}{}\TextB{Sup $\Par{v_1,\dots,v_m}$ is a list of vecs in $V$. For each $k$, let $w_k=v_1+\dots+v_k$.}
	(a) \TextB{Showt $\Span{v_1,\dots,v_m}=\Span{w_1,\dots,w_m}.$}
	(b) \TextB{Showt [P] $\Par{v_1,\dots,v_m}$ is linely inde $\Longleftrightarrow$ $\Par{w_1,\dots,w_m}$ is linely inde [Q].}
}\par\quad
(a) Asm $a_1v_1+\dots+a_mv_m=b_1w_1+\dots+b_mw_m=b_1v_1+\dots+b_k\Par{v_1+\dots+v_k}+\dots+b_m\Par{v_1+\dots+v_m}.$\par\quad\Ha
Then $a_k=b_k+\dots+b_m;\;\;a_{k+1}=b_{k+1}+\dots+b_m\Rightarrow b_k=a_k-a_{k+1};\;b_m=a_m.$ Similar to Exe (1).\vspace{4pt}\par\quad
...
\Or By (a), let $W=\Span{v_1,\dots,v_m}=\Span{w_1,\dots,w_m}.$ Sup $\Par{w_1,\dots,w_m}$ is linely dep.\par\quad\Hb
By [2.21](b), a list of len $\Par{m-1}$ spans $W.$ 又 By [2.23], $\Par{w_1,\dots,w_m}$ linely inde $\Rightarrow m\leqslant m-1.$\par\quad\Hb
Thus $\Par{w_1,\dots,w_m}$ is linely dep. Now reversing the roles of $v$ and $w.$\PfEnd
\SepLine

\ProblemN{\hypertarget{2A2}{2}}{
	(a) \TextN{[P]\hspace{17pt}{\; A list $\Par{v}$ of len $1$ in $V$ is linely inde $\Longleftrightarrow v\neq 0.$} \hfill[Q]}
	(b) \TextN{[P]{\; A list $\Par{v,w}$ of len $2$ in $V$ is linely inde $\Longleftrightarrow\forall\lambda,\mu\in\Fbb,v\neq \lambda w,w\neq \mu v.$} \hfill[Q]\vspace{4pt}}
}(a) $Q\Rightarrow P:$ $v\neq 0\Rightarrow$ if $av=0$ then $a=0\Rightarrow\Par{v}$ linely inde.\parSol{\Ha}
$P\Rightarrow Q:$ $\Par{v}$ linely inde $\Rightarrow v\neq 0$, for if $v=0,$ then $av=0\notRightarrow a=0.$\parSol{\Ha}
${}^\neg Q\Rightarrow{}^\neg P:$ $v=0\Rightarrow av=0$ while we can let $a\neq 0\Rightarrow\Par{v}$ is linely dep.\parSol{\Ha}
${}^\neg P\Rightarrow{}^\neg Q:$ $\Par{v}$ linely dep $\Rightarrow av=0$ while $a\neq 0\Rightarrow v=0.$\parSol{\vspace{4pt}}
(b) $P\Rightarrow Q:$ $\Par{v,w}$ linely inde $\Rightarrow$  if $av+bw=0,$ then $a=b=0\Rightarrow$ no scalar multi.\parSol{\Hb}
$Q\Rightarrow P:$ no scalar multi $\Rightarrow$ if $av+bw=0,$ then $a=b=0\Rightarrow\Par{v,w}$ linely inde.\parSol{\Hb}
${}^\neg P\Rightarrow{}^\neg Q:$ $\Par{v,w}$ linely dep $\Rightarrow$ if $av+bw=0,$ then $a$ or $b\neq 0\Rightarrow$ scalar multi.\parSol{\Hb}
${}^\neg Q\Rightarrow{}^\neg P:$ scalar multi $\Rightarrow$ if $av+bw=0,$ then $a$ or $b\neq 0\Rightarrow$ linely dep.\PfEnd
\SepLine

\ProblemN{\hypertarget{2A10}{10}}{
	\TextNL{Sup $\Par{v_1,\dots,v_m}$ is linely inde in $V$ and $w\in V$.}
	\TextNL{Provet if $\BigPar{v_1+w,\dots,v_m+w}$ is linely depe, then $w\in\Span{v_1,\dots,v_m}$.}
}\par\quad
...
\Or We prove the ctrapos: Sup $w\not\in\Span{v_1,\dots,v_m}.$ Then $a_1+\dots+a_m=0.$\par\quad
Thus $a_1v_1+\dots+a_mv_m=0\Rightarrow a_1=\dots=a_m=0.$ Hence $\BigPar{v_1+w,\dots,v_m+w}$ is linely inde.\PfEnd\vspace{2pt}\quad
...
\ProblemN{\hypertarget{2A11}{11}}{
	\TextNL{Sup $\Par{v_1,\dots,v_m}$ is linely inde in $V$ and $w\in V$.}
	\TextNL{Showt [P] $\Par{v_1,\dots,v_m,w}$ is linely inde $\Longleftrightarrow w\not\in\Span{v_1,\dots,v_m}$ [Q].\vspace{8pt}}
}\hspace{-7pt}$\begin{array}{l}{}^\neg Q\Rightarrow{}^\neg P:$ Sup $w\in\Span{v_1,\dots,v_m}$. Then $\Par{v_1,\dots,v_m,w}$ is linely depe.$\\{}^\neg P\Rightarrow{}^\neg Q:$ Sup $\Par{v_1,\dots,v_m,w}$ is linely dep. Then by [2.21](a), $w\in\Span{v_1,\dots,v_m}.\end{array}$\PfEnd[-13pt]\vspace{-4pt}
\SepLine

\ProblemN{\hypertarget{2A14}{14}}{
	\TextNL{Provet [P] $V$ is infindim $\Longleftrightarrow[Q]\left|\hspace{-6pt}\begin{array}{l}$ there is a sequence $\Par{v_1,v_2,\dots}$ in $V$ shat$\\[-8pt]$ $\Par{v_1,\dots,v_m}$ is linely inde for each $m\in\Nbp.\end{array}\right.$}
}\par\quad
$P\Rightarrow Q:\;$ Sup $V$ is infindim, so that no list spans $V$.\par\quad
\Blind{$P\Rightarrow Q:\;$} {\tgbf Step 1}\;\; Pick a $v_1\neq 0,\Par{v_1}$ linely inde.\par\quad
\Blind{$P\Rightarrow Q:\;$} {\tgbf Step m}\; Pick a $v_m\not\in\Span{v_1,\dots,v_{m-1}},$ by Exe (11), $\Par{v_1,\dots,v_m}$ is linely inde.\par\quad
\Blind{$P\Rightarrow Q:\;$} This process recursively defines the desired sequence $\Par{v_1,v_2,\dots}.$\vspace{4pt}\par\quad
${}^\neg P\Rightarrow{}^\neg Q:\;$ Sup $V$ is findim and $V=\Span{w_1 , ..., w_m}$.\par\quad
\Blind{${}^\neg P\Rightarrow{}^\neg Q:\;$} Let $\Par{v_1 , v_2 , \dots}$ be a sequence in $V$, then $\Par{v_1,v_2,\dots,v_{m+1}}$ must be linely dep.\vspace{4pt}\par\quad
\Or\; $Q\Rightarrow P:\;$ Sup there is such a sequence.\par\quad
\Blind{\Or\; $Q\Rightarrow P:\;$} Choose an $m$. Sup a linely inde list $\Par{v_1,\dots,v_m}$ spans $V$.\par\quad
...
\ProblemN{\hypertarget{2A16}{16}}{
	\TextNL{Provet the vecsp of all continuous functions in $\Rbb^{[0,1]}$ is infindim.}
}Denote the vecsp by $U$.\par\quad
Choose one $m\in\Nbp.$ Sup $a_0,\dots,a_m\in\Rbb$ are shat $p\Par{x}=a_0+a_1 x+\dots+a_m x^m=0,\,\forall x\in\Sbra{0,1}.$\par\quad
Then $p$ has infinily many roots and hence each $a_k=0,$ otherwise $\deg p\geqslant 0,$ ctradic [4.12].\par\quad
Thus $\Par{1,x,\dots,x^m}$ is linely inde in $\Rbb^{[0,1]}.$ Similar to [2.16], $U$ is infindim.\PfEnd\vspace{8pt}\quad
\Or Note that\; $\Frac{\;1\;}{1}>\Frac{\;1\;}{2}>\dots>\Frac{\;1\;}{m},\,\,\,\forall m\in\Nbp.$ Sup\; $f_m=\MathLeftBrace{l}{
...
\ProblemN{\hypertarget{2A17}{17}}{
	\TextNL{Sup $p_0,p_1,\dots,p_m\in\PoF{m}$ shat $p_k\Par{2}=0$ for each $k\in\Bra{0,\dots,m}$.}
	\TextNL{Provet $\Par{p_0,p_1,\dots,p_m}$ is not linely inde in $\PoF{m}$.}
}\par\quad
Sup $\Par{p_0,p_1,\dots,p_m}$ is linely inde. Define $p\in\PoF{m}$ by $p\Par{z}=z.$\par\quad
...
Hence $\Par{p_0,p_1,\dots,p_m}$ is linely depe. For if not, then beca $\Par{1,z,\dots,z^m}$ of len $\Par{m+1}$ spans $\PoF{m},$\par\quad
by the steps in [2.23] trivially, $\Par{p_0,p_1,\dots,p_m}$ of len $\Par{m+1}$ spans $\PoF{m}.$ Ctradic.\PfEnd\vspace{6pt}\par\quad
\Or Note that $\PoF{m}=\Span{\underbrace{\;1,\;z,\;\dots,\;z^m\;}_{\text{of len }\SmallPar{m+1}}}.$ Then $\Par{p_0,p_1,\dots,p_m,z}$ of len $\Par{m+2}$ is linely dep.\vspace{2pt}\par\quad
...
\ProblemN{\hypertarget{2B7}{7}}{
	\TextN{Prove or give a counterexa\hspace{1pt}$:$ If $\Par{v_1,v_2,v_3,v_4}$ is a basis of $V$ and $U$ is a subsp of $V$}
	\TextN{shat $v_1,v_2\in U$ and $v_3\not\in U$ and $v_4\not\in U$, then $\Par{v_1,v_2}$ is a basis of U.}
}A counterexa\hspace{1pt}: Let $V=\Rbb^4$ and $B_V=\Par{e_1,e_2,e_3,e_4}$ be std basis.\par\quad
...
\BulletPointX\NoteForSmall{\Large $“\complement_V U \cup \zeroSubs”$}\;\;$“\complement_V U \cup \zeroSubs”$ is supd to be a subsp $W$ shat $V=U\oplus W$.\TextB{\vspace{4pt}}
...
\ProblemB[]{
	\Tips \,\,\,\TextB{Sup $V$ is findim with $\dim V=n$ and $U$ is a subsp of $V$ with $U\neq V.$}
	\IndentTips\TextB{Provet $\exists\,B_V=\Par{v_1,\dots,v_n}$ shat each $v_k\not\in U.$}
}\TextB{\vspace{0pt}}
...
with $1+1=0.$ Then the list $\Par{1}$ is the unique basis. Now the vecsp $\Bra{0,1}$ will do.\parSol{}
...
\ProblemBnoor{\hypertarget{2B4e9}{4E 9}}{
	\TextB{Sup $\Par{v_1,\dots,v_m}$ is a list of vecs in $V$. For $k\in\Bra{1,\dots,m}$, let $w_k=v_1+\dots+v_k.$}
	\TextB{Showt [P] $B_V=\Par{v_1,\dots,v_m}\Longleftrightarrow B_W=\Par{w_1,\dots,w_m}.$ [Q]}
...
\ProblemBnoor{\hypertarget{2B4e5}{4E 5}}{
	\TextB{Sup $U,W$ are findim, $V=U+W,\;B_U=\Par{u_1,\dots,u_m},\;B_W=\Par{w_1,\dots,w_n}.$}
	\TextB{Provet $\exists\,B_V$ consisting of vecs in $U\cup W$.\vspace{-4pt}}
...
\ProblemN{\hypertarget{2B8}{8}}{
	\TextN{Sup $V=U\oplus W,\;B_U=\Par{u_1,\dots,u_m},\;B_W=\Par{w_1,\dots,w_n}.$}
	\TextN{Provet $B_V=\BigPar{u_1,\dots,u_m,w_1,\dots,w_n}$.}
...
\ProblemBnoor[]{\hypertarget{2B4e}{9.A.3,4 \OR 4E 11}}{
	\TextB{Sup $V$ is on $\Rbb,$ and $v_1,\dots,v_n\in V.$ Let $B=\Par{v_1,\dots,v_n}.$\vspace{2pt}}
	(a) \TextB{Showt [P] $B$ is linely inde in $V$ $\Longleftrightarrow B$ is linely inde in $V_{\!\Cbb}.$ [Q]}
	(b) \TextB{Showt [P] $B$ spans $V$ $\Longleftrightarrow B$ spans $V_{\!\Cbb}.$ [Q]\vspace{2pt}}
}{\envFontSmall\normalsize\par\quad
...
If we allow using $“$infini list$”$, then we must guarantee that $\Par{v_1,\dots,v_n,\dots}$ is a spanning $“$list$”$\TextB{}
shat $\forall v\in V,\exists$ smallest $n\in\Nbp,\;v=a_1 v_1+\dots+a_n v_n.$ Moreover, given a list $\Par{w_1,\cdots,w_n,\cdots}$ in $W,$\TextB{}
we can provet $\exists\,!\,T\in\Lm{V,W}$ with each $Tv_k=w_k,$ which has less restrictions than [3.5].\TextB{}
...
\ProblemN{\hypertarget{2C15}{15}}{
	\TextNL{Sup $V$ is findim and $\dim V=n\geqslant 1.$}
	\TextNL{Provet $\exists$ one-dim subsps $V_{\!1},\dots,V_{\!n}$ of $V$ shat $V=V_{\!1}\oplus \dots\oplus V_{\!n}.$}
}Sup $B_V=\Par{v_1,\dots,v_n}.$ Define $V_{\!i}$ by $V_{\!i}=\Span{v_i}$ for each $i\in\Bra{1,\dots,n}.$\parSol{}
...
\ProblemBnoor{\NoteForSmall{Problem (15)}}[]{
	\TextB{}
	\TextB{Sup $v\in V\nonzero,$ and $\dim V=n\geqslant 1.$ Provet $\exists\,B_V=\Par{v_1,\dots,v_n},v=v_1+\dots+v_n.$}
}If $n=1$ then let $v_1=v$ and we are done. Sup $n>1.$\parSol{}
...
\Comment \,\,\,Let $B_V=\Par{v_1,\dots,v_n}$ and sup $v=u_1+\dots+u_n,$ where each $u_i=a_i v_i\in V_{\!i}.$\parCom
But $\Par{u_1,\dots,u_n}$ might not be a basis, beca there might be some $u_i=0.$\vspace{-2pt}
\SepLine\pagebreak

\ProblemNnoor[]{\hypertarget{2C1}{1}}{{\COROLLARY} for [2.38,39]}{
	\TextN{Sup $U$ is a subsp of $V$ shat $\dim V=\dim U$. Then $V=U.$}
...
\ProblemN{\hypertarget{2C7}{7}}{
	...
	(c) \TextN{Find a subsp $W$ of $\PoF{4}$ shat $\PoF{4}=U\oplus W$.}
}Using Exe (10).
%Sup $p\Par{z}=az^4+bz^3+cz^2+dz+e$ shat $p\Par{2}=p\Par{5}=p\Par{6}$.\vspace{4pt}\par\quad
...
(b) Extend to a basis of $\PoF{4}$ as $\envFontLarge\BigPar{\envFontDefault 1,z,z^2,\Par{z-2}\Par{z-5}\Par{z-6},z\Par{z-2}\Par{z-5}\Par{z-6}\envFontLarge}.$\par\quad
(c) Let $W=\Span{z,z^2}=\Bra{az+bz^2:a,b\in\Fbb}$, so that $\PoF{4}=U\oplus W.$\PfEnd
\SepLine

\ProblemN{\hypertarget{2C9}{9}}{
	\TextN{Sup $\Par{v_1,\dots,v_m}$ is linely inde in $V$ and $w\in V$. Provet $\dim\Span[\BigPar]{v_1+w,\dots,v_m+w}\geq m-1$.}
...
$\Par{v_1,\dots,v_m}$ linely inde $\Rightarrow$ $\BigPar{v_1,v_2-v_1,\dots,v_m-v_1}$ linely inde $\Rightarrow$ $\underbrace{\BigPar{v_2-v_1,_{}\dots,v_m-v_1}}_{\text{of len }\Par{m-1}}$ linely inde.\vspace{-15pt}\par\quad
又 If $w\not\in\Span{v_1,\dots,v_m}.$ Then $\BigPar{v_1+w,\dots,v_m+w}$ is linely inde.\par\quad
Hence $m\geqslant\dim\Span[\BigPar]{v_1+w,\dots,v_m+w}\geq m-1.$\PfEnd
\SepLine

\ProblemBnoor{\hypertarget{2C4e16}{4E 16}}{
	\TextB{Sup $V$ is findim, $U$ is a subsp of $V$ with $U\neq V$. Let $n=\dim V,m=\dim U$.}
	\TextB{Provet $\exists\,\Par{n-m}$ subsps  $U_1,\dots,U_{n-m}$, each of dim $\Par{n-1}$, shat $\bigcap\limits_{i=1}^{n-m}U_i=U$.}
...
And beca $\forall v\in \bigcap\limits_{i=1}^{n-m}U_i,v=v_0+b_1 u_1+\dots+b_{n-m} u_{n-m}\in U_i\Rightarrow$ each $b_i=0\Rightarrow v\in U.$\vspace{-4pt}\parSol{}
Hence $\bigcap\limits_{i=1}^{n-m}U_i\subseteq U.$\PfEnd
%\vspace{10pt}
%\Example\,\,\, {Sup $\dim V=6,\dim U=3$.\par\vspace{2pt}\quad
...
\BulletPointX\NoteForSmall{Problem 10} \,\,\,For each nonconst $p\in\Span{1,z,\dots,z^m},\;\exists$ smallest $m\in\Nbp,$ which is $\deg p.$\TextB{}
(a) If $p_0,p_1,\dots,p_m$ are shat all $a_{k,k}\neq 0,$ and\TextB{}
...
(b) If $p_0,p_1,\dots,p_m$ are shat all $a_{k,k}\neq 0,$ and\TextB{}
...
\ProblemN{\hypertarget{2C10}{10}}{
	\TextNL{Sup $m\in\Nbp,\;p_0,p_1,\dots,p_m\in\PoFi$ are shat each $p_k$ has deg $k.$}
	\TextNL{Provet $\Par{p_0,p_1,\dots,p_m}$ is a basis of $\PoF{m}$.}
...
(ii) {$1\leqslant k\leqslant m-1.$ \;Asm $\Span[\BigPar]{p_0,p_1,\dots,p_k}=\Span[\BigPar]{1,x,\dots,x^k}.$}\par\quad\Hii
...
{Sup $L=a_m p_m\Par{x}+\dots+a_1 p_1\Par{x}+a_0p_0\Par{x}=0\cdot x^m+\dots+0\cdot x+0\cdot 1=R,\forall x\in\Fbb.$}\par\quad
{We showt $a_m=\dots=a_0=0$ via the following process. So that $\Par{p_0,p_1,\dots,p_m}$ is linely inde.}\vspace{2pt}\par\quad
...
\ProblemBnoor{\Tips}[]{
	\TextB{Sup $m\in\Nbp,\;p_0,p_1,\dots,p_m\in\PoF{m}$ are shat the lowest term of each $p_k$ is of deg $k.$}
	\Blind{\Tips} \TextB{Provet $\Par{p_0,p_1,\dots,p_m}$ is a basis of $\PoF{m}.$}
}{Using mathematical induction on $m.$\par}\quad
...
(ii) {$1\leqslant k\leqslant m-1.$ \;Asm $\Span[\BigPar]{x^m,\dots,x^{m-k}}=\Span[\BigPar]{p_m,\dots,p_{m-k}}.$}\par\quad\Hii
...
To make it clear, we \uline{specify that {\tgsl in} $\PoFi,$ $a_0z^0=a_0,$ where $z^0$ appears just for notational convenience.}\TextB{}
Beca by definition, the term $a_0z^0$ in a poly only represents the const term of the poly, which is $a_0.$\TextB{}
For convenicence, we asm $z^0=1$ in formula deduction and poly def. Absolutely without $0^0.$
\SepLine

\ProblemBnoor{\hypertarget{2C4e10}4E 10}{
	\TextB{Sup $m$ is a positive integer. For $0\leqslant k\leqslant m$, let $p_k\Par{x}=x^k\Par{1-x}{^{m-k}}$.}
	\TextB{Showt $\Par{p_0,\dots,p_m}$ is a basis of $\PoF{m}$.\vspace{0pt}}
...
\Or Similar to the {\TIPS} above. We will recursively provet each $x^{m-k}\in\Span{p_m,\dots,p_{m-k}}.$\par\quad
(i) $k=1.$ \;$p_m\Par{x}=x^m\in\Span{p_m};$ \;\; $p_{m-1}\Par{x}=x^{m-1}-x^m\Rightarrow x^{m-1}\in\Span{p_{m-1},p_m}.$\vspace{2pt}\par\quad\Endi
(ii) $k\in\Bra{1,\dots,m-1}.$ \;Sup for each $k\in\Bra{0,\dots,k},$ we have $x^{m-k}\in\Span[\BigPar]{p_{m-k},\dots,p_m},\exists\,!\,a_m\in\Fbb.$\vspace{2pt}\par\quad\Hii
...
\Or For any $m,k\in\Nbp$ shat $k\leqslant m.$ Define $p_{k,m}$ by $p_{k,m}\Par{x}=x^k\Par{1-x}{^{m-k}}.$\par\quad
Define the stam $S\Par{m}$ by $S\Par{m}:\Par{p_{0,m},\dots,p_{m,m}}$ is linely inde \BigPar{ and therefore is a basis }.\par\quad
We use induction on to showt $S\Par{m}$ holds for all $m\in\Nbp.$\vspace{2pt}\par\quad
...
(ii) $1\leqslant m.$ \;Asm $S\Par{m}$ and $S\Par{m-1}$ holds. Now we showt $S\Par{m+1}$ holds.\vspace{2pt}\par\quad\Hii
Sup $\sum_{k=0}^{m+1}a_kp_{k,m+1}\Par{x}=\sum_{k=0}^{m+1}a_k\Sbra{x^k\Par{1-x}{^{m+1-k}}}=0,\forall x\in\Fbb.$\vspace{6pt}\par\quad\Hii
...
\envFontDefault{\vspace{4pt}Hence \;$\sum_{k=0}^{m-1}a_{k+1}p_{k,m-1}\Par{x}=0,\forall x\in\Fbb\Backslash{\def\envFont{\envFontB}\Bra{\,0,1}}.$ Which has infinily many zeros.}\par\quad\Hii
Moreover, \;$\sum_{k=0}^{m-1}a_{k+1}p_{k,m-1}\Par{x}=0.$ By asm, $a_1=\dots=a_{m-1}=a_{m}=0.$\par\quad\Hii
Thus $\Par{p_{0,m+1},\dots,p_{m+1,m+1}}$ is linely inde and $S\Par{m+1}$ holds.\PfEnd
\SepLine

\ProblemN{\hypertarget{2C14}{14}}{
	\TextNL{Sup $V_{\!1},\dots,V_{\!m}$ are findim. Provet $\dim\!\Par{V_{\!1}+\dots+V_{\!m}}\leqslant\dim V_{\!1}+\dots+\dim V_{\!m}$.}
...
\ProblemN{\hypertarget{2C17}{17}}{
	\TextNL{Sup $V_{\!1},V_{\!2},V_{\!3}$ are subsps of a findim vecsp, then}
...
Beca \;\;$\cMid{X\cup Y}=\cMid{X}+\cMid{Y}-\cMid{X\cap Y};\,\,\,\Par{X\cup Y}\cap Z=\Par{X\cap Z}\cup\Par{Y\cap Z}$.\par\quad
...
and the wrong formual holds. Similar for $Y\subseteq Z,X\subseteq Z,$ and $X,Y\subseteq Z.$\par\vspace{4pt}\quad
However, $\Par{X+Y}\cap Z\supseteq\Par{X\cap Z}+\Par{Y\cap Z}$ holds. Beca $\forall v\in\Par{X\cap Z}+\Par{Y\cap Z},$\par\quad
...
\BulletPointX\Corollary \,\,\,\Largesl{Sup $V_{\!1},V_{\!2},V_{\!3}$ are findim, then} {\small $\Frac{\Par{1}+\Par{2}+\Par{3}}{3}$}:\vspace{-4pt}\par
\AlignEq{\IndentB\,}{\Dim\Par{V_{\!1}+V_{\!2}+V_{\!3}}=\dim &\,V_{\!1}+\dim V_{\!2}+\dim V_{\!3}\\&-\Frac{\Dim\Par{V_{\!1}\cap V_{\!2}}+\Dim\Par{V_{\!1}\cap V_{\!3}}+\Dim\Par{V_{\!2}\cap V_{\!3}}}{3}\\&-\Frac{\Dim\BigPar{\Par{V_{\!1}+V_{\!2}}\cap V_{\!3}}+\Dim\BigPar{\Par{V_{\!1}+V_{\!3}}\cap V_{\!2}}+\Dim\BigPar{\Par{V_{\!2}+V_{\!3}}\cap V_{\!1}}}{3}.}
\SepLine

\BulletPointX\Tips \,\,\,Beca $\dim \Par{V_{\!1}\cap V_{\!2}\cap V_{\!3}}=\dim V_{\!1}+\Dim\Par{V_{\!2}\cap V_{\!3}}-\Dim\BigPar{V_{\!1}+\Par{V_{\!2}\cap V_{\!3}}}.$\TextB{}
\IndentTips{}And $\Dim\Par{V_{\!2}\cap V_{\!3}}=\dim V_{\!2}+\dim V_{\!3}-\Dim\Par{V_{\!2}+V_{\!3}}.$ We have $\Par{1},$ and $\Par{2},\Par{3}$ similarly.\TextB{}
...
\ProblemB[]{
	\TextB{Sup $V_{\!1},V_{\!2},V_{\!3}$ are subsps of $V$ with}
	\hypertarget{2C4e14}{}(a) \TextB{$\dim V=10,\;\dim V_{\!1} = \dim V_{\!2} = \dim V_{\!3} = 7$. Provet $V_{\!1}\cap V_{\!2}\cap V_{\!3}\neq\zeroSubs$.}
	\Blind{(a)} By {\TIPS}, $\Dim\Par{V_{\!1}\cap V_{\!2}\cap V_{\!3}}\geq\dim V_{\!1}+\dim V_{\!2}+\dim V_{\!3}\uline{{}-2\dim V}>0.$\TextB{\vspace{4pt}}
	\hypertarget{2C4e15}{}(b) \TextB{$\dim V_{\!1}+\dim V_{\!2}+\dim V_{\!3} > 2\dim V$. Provet $V_{\!1}\cap V_{\!2}\cap V_{\!3}\neq\zeroSubs$.}
...
\ProblemBnoor{\hypertarget{3B4e33}{9.A.2,6 \OR 4E 3.B.33}}{
	\TextB{Sup that $V,W$ are on $\Rbb,$ and $T\in\Lm{V,W}.$ Showt}
...
\ProblemBnoor{9.A.5}{
	\TextB{Sup $V$ is on $\Rbb,$ and $S,T\in\Lm{V,W}.$ Provet $\Par{S+\lambda T}{_\Cbb}=S_\Cbb+\lambda T_{\!\Cbb}.$}
...
\ProblemB{
	\TextB{Sup $U,V,W$ are on $\Rbb,$ $S\in\Lm{V,W},T\in\Lm{U,V}.$ Provet $\Par{ST}{_\Cbb}=S_\Cbb T_{\!\Cbb}.$}
...
\ProblemBnoor{4E 1.B.7}{
	...
	(b) \TextB{Provet $W^V$ is a vecsp with these definitions.}
}\par\quad
...
\ProblemN[]{\hypertarget{3A5}{5}}{
	\TextN{Beca $\Lm{V,W}=\Bra{T:V\rightarrow W\;{\envFontHuge\envFontA\mmid}\;T\text{ is linear}}$ is a subsp of $W^V,$ $\Lm{V,W}$ is a vecsp.}
}\SepLine

\ProblemN{\hypertarget{3A3}{3}}{
	\TextN{Sup $T\in\Lm{\Fbb^n,\Fbb^m}$. Provet $\exists\,A_{j,k}\in\Fbb\,$shat for any $\Par{x_1,\dots,x_n}\in\Fbb^n,$}\hspace{-40pt}
...
\ProblemN{\hypertarget{3A4}{4}}{
	\TextN{Sup $T\in\Lm{V,W},$ and $v_1,\dots,v_m\in V$ shat $\Par{Tv_1,\dots,Tv_m}$ is linely inde in $W$.} \TextN{Provet $\Par{v_1,\dots,v_m}$ is linely inde.}
}Sup $a_1 v_1+\dots+a_m v_m=0$. Then $a_1 Tv_1+\dots+a_m Tv_m=0$. Thus $a_1=\dots=a_m=0.$\PfEnd
\SepLine

\ProblemN{\hypertarget{3A7}{7}}{
	\TextN{Showt every linear map from a one-dim vecsp to itself is a multi by some scalar.}
	\TextN{More precisely, provet if $\dim V = 1$ and $T\in\Lm{V}$, then $\exists\,\lambda\in\Fbb,Tv = \lambda v,\forall v\in V$.}
}Let $u$ be a nonzero vec in $V\Rightarrow V=\Span{u}$. Beca $Tu\in V\Rightarrow Tu=\lambda u$ for some $\lambda$.\parSol{}
Sup $v\in V\Rightarrow v=au,\,\exists\,!\,a\in\Fbb$. Then $Tv=T\Par{au}=\lambda au=\lambda v.$\PfEnd
\SepLine

\ProblemN{\hypertarget{3A8}{8}}{
	\TextN{Give a map $\varphi:\Rbb^2\rightarrow\Rbb$ shat $\forall a\in\Rbb,v\in\Rbb^2,\varphi\Par{av} = a\varphi\Par{v}$ but $\varphi$ is not linear.\vspace{4pt}}
...
\ProblemN{\hypertarget{3A9}{9}}{
	\TextN{Give a map $\varphi:\Cbb\rightarrow\Cbb$ shat $\forall w,z\in\Cbb,\varphi\Par{w + z} = \varphi\Par{w} + \varphi\Par{z}$ but $\varphi$ is not linear.}
}Define $\varphi\Par{u+\i v}=u=\REAL\Par{u+\i v}$\quad\Or Define $\varphi\Par{u+\i v}=v=\IMAGINARY\Par{u+\i v}.$\PfEnd
\SepLine

\ProblemB{
	\hypertarget{3A4e10}{}\TextB{Provet if $q\in\PoRi$ and $T:\PoRi\rightarrow\PoRi$ is defined by $\underbrace{Tp=q\circ p}_{composition},$ then $T$ is not linear.\vspace{-8pt}}
...
Beca in general, $\Sbra{q\circ \Par{p_1+\lambda p_2}}\Par{x}=q\BigPar{p_1\Par{x}+\lambda p_2\Par{x}}\neq \Par{qp_1}\Par{x}+\lambda \Par{qp_2}\Par{x}.$\par\quad
...
\ProblemN{\hypertarget{3A10}{10}}{
	\TextNL{Sup $U$ is a subsp of $V$ with $U\neq V.$\vspace{-4pt}}
	\TextNL{Sup $S\in\Lm{U,W}$ with $S\neq 0.$ \;Define $T: V\rightarrow W$ by $Tv={}${\large\envFontDefault$\MathLeftBrace{l}{Sv,\;\text{if }v\in U,\\ 0,\Blind{S}\;\text{if }v\in V\Backslash U.}$}\vspace{-4pt}}
	\TextNL{Provet $T$ is not a linear map on $V$.}
}Asm $T$ is a linear map. Sup $v\in V\Backslash U,\,\, u\in U$ shat $Su\neq 0$.\parSol{}
...
\ProblemN{\hypertarget{3A11}{11}}{
	\TextNL{Sup $U$ is a subsp of $V$ and $S\in\Lm{U,W}$.}
	\TextNL{Provet $\exists\,T\in\Lm{V,W},Tu = Su,\forall u\in U$. {\large\tgnr\envFontDefault\BigPar{ \Or $\exists\,T\in\Lm{V,W},T\mmid_U=S.$ }}}
	\TextNL{\large In other words, every linear map on a subsp of $V$ can be {\tgsc extended} to a linear map on the entire $V$.}
}Sup $W$ is shat $V=U\oplus W.$ Then $\forall v\in V,\exists\,!\,u_v\in U,w_v\in W,v=u_v+w_v.$\parSol{}
...
\ProblemN{\hypertarget{3A12}{12}}{
	\TextNL{Sup nonzero $V$ is findim and $W$ is infindim. Provet $\Lm{V,W}$ is infindim.}
}Using (2.A.14).\par\quad
Let $B_V=\Par{v_1,\dots,v_n}$ be a basis of $V.$ Let $\Par{w_1,\dots,w_m}$ be linely inde in $W$ for any $m\in\Nbp.$\par\vspace{-4pt}\quad
...
Linearity checked. Now sup $a_1 T_{x,1}+\dots+a_m T_{x,m}=0$.\par\quad
Then $\BigPar{a_1 T_{x,1}+\dots+a_m T_{x,m}}\Par{v_x}=0=a_1 w_1+\dots+a_m w_m\Rightarrow a_1=\dots=a_m=0.$ 又 $m$ arb.\par\quad
Thus $\Par{T_{x,1},\dots,T_{x,m}}$ is a linely inde list in $\Lm{V,W}$ for any $x$ and len $m$. Hence by (2.A.14).\PfEnd
\SepLine

\ProblemN{\hypertarget{3A13}{13}}{
	\TextNL{Sup $\Par{v_1,\dots,v_m}$ is linely depe in $V$ and $W\neq\zeroSubs$.}
	\TextNL{Provet $\exists\,w_1,\dots,w_m\in W,\not\exists\,T\in\Lm{V,W}$ shat $Tv_k=w_k,\forall k = 1,\dots,m$.}
}\par\quad
We prove by ctradic. By linear dependence lemma, $\exists\,j\in\Bra{1,\dots,m},v_j\in\Span{v_1,\dots,v_{j-1}}$.\par\quad
Sup $a_1 v_1+\dots+a_m v_m=0,$ where $a_j\neq 0.$ \;Now let $w_j\neq 0$, while $w_1=\dots=w_{j-1}=w_{j+1}=w_m=0.$\par\quad
...
\Or We prove the ctrapos\hspace{1pt}: \,Sup $\forall w_1,\dots,w_m\in W,\exists\,T\in\Lm{V,W},Tv_k=w_k$  for each $w_k.$\par\quad
{Now we showt $\Par{v_1,\dots,v_n}$ is linely inde. Sup {$\exists\,a_i\in\Fbb,a_1 v_1+\dots+a_n v_n=0$}.}\vspace{2pt}\par\quad
{Choose one $w\in W\nonzero.$ By asm, for {$\BigPar{\overline{a_1}w,\dots,\overline{a_m}w},\exists\,T\in\Lm{V,W},Tv_k=\overline{a_k}w$} for each $v_k$.}\vspace{2pt}\par\quad
...
{Then {$\sum_{k=1}^m\aMid{a_k}{^2}=0.$ Thus $a_1=\dots=a_m=0.$} Hence $\Par{v_1,\dots,v_n}$ is linely inde.}\PfEnd
\SepLine

\ProblemBnoor{\hypertarget{3A4e17}{4E 17}}{
	\TextB{Sup $V$ is findim. Showt all two-sided ideals of $\Lm{V}$ are $\zeroSubs$ and $\Lm{V}$.}
...
{\vspace{6pt}Then {\Large$\BigPar{R_{1,1}+\dots+R_{n,n}}v_j=v_j\Rightarrow\sum_{r=1}^n R_{r,r}=I$}. Asm {\Large each $R_{x,y}\in\mathcal{E}$}.}\par\quad
{\vspace{6pt}Hence {\Large$\forall T\in\Lm{V},I\circ T=T\circ I=T\in\mathcal{E}\Rightarrow \mathcal{E}=\Lm{V}$}. Now we prove the asm.}\par\quad
...
\ProblemBnoor{\hypertarget{3B4e32}{4E 3.B.32}}{
	\TextB{Sup $V$ is findim with $n=\dim V > 1$.}
	\TextB{Showt if $\varphi:\Lm{V}\rightarrow\Fbb$ is linear and $\forall S,T\in\Lm{V},\varphi\Par{ST} = \varphi\Par{S}\cdot\varphi\Par{T}$, then $\varphi = 0$.}
}Using notations in (4E 3.A.17). Using the result in {\NOTEFOR} [3.60].\parSol{\vspace{2pt}}
Sup $\varphi\neq 0\Rightarrow\exists\,i,j\in\Bra{1,\dots,n},\,\varphi\Par{R_{i,j}}\neq 0$. \envFontLarge Beca {\Large\vspace{4pt}$R_{i,j}=R_{x,j}\circ R_{i,x},\,\,\forall x=1,\dots,n$}\parSol{}
...
{\vspace{4pt}Again, beca {\Large$R_{i,x}=R_{y,x}\circ R_{i,y},\,\,\forall y=1,\dots,n.$} \;Thus {\Large$\varphi\Par{R_{y,x}}\neq 0,\;\forall x,y=1,\dots,n$}.}\parSol{}
...
\ProblemB{
	\hypertarget{3A4e11}{}\TextB{Sup $V$ is findim. $T\in\Lm{V}$ is shat $\forall S\in\Lm{V},ST=TS$.}
	\TextB{Provet $\exists\,\lambda\in\Fbb,T=\lambda I$.}
}If $V=\zeroSubs,$ then we are done. Now sup $V\neq\zeroSubs.$\par\quad
Asm $\forall v\in V,\Par{v,Tv}$ is linely depe, then by (2.A.2.(b)), $\exists\,\lambda_v\in\Fbb,Tv=\lambda_v v.$\par\quad
To provet $\lambda_v$ is independent of $v$, we discuss in two cases:\vspace{4pt}\par\hspace{1pt}
$\MathRightBrace{l}{$
	...
	($=$) Otherwise, sup $w=cv$, $\lambda_w w=Tw=cTv=c\lambda_v v=\lambda_v w\Rightarrow\Par{\lambda_w-\lambda_v}w$
	$}\Rightarrow \lambda_w=\lambda_v.$\vspace{4pt}\par\quad
Now we prove the asm. Asm $\exists\,v\in V,\Par{v,Tv}$ is linely inde. Let $B_V=\Par{v,Tv,u_1,\dots,u_n}$.\par\quad
...
\Blind{\Or \;}{\Large\vspace{6pt}Sup $v\in V.$ Define $S_v\in\Lm{V}$ by $S_v \Par{u}=\varphi\Par{u}v.$}\par\quad
...
\ProblemB{
	\TipsN{3}\,\,\,\TextB{Sup $T\in\Lm{V,W}.$ Provet $Tv\neq 0\Rightarrow v\neq 0.$}
}Asm $v=0.$ Then $Tv=T\Par{0}=T\Par{0\cdot 0}=0\cdot T\Par{0}=0.$\parSol{}
...
\ProblemB{
		...
		\TextB{\envFontDefault\large And by [3.2], the additivity and homogeneity imply that $V$ is closd add and scalar multi.}
...
%\ProblemN{2}{
%	\TextN{Sup $S,T\in\Lm{V}$ are shat $\range S\subseteq\null T$. Provet $(ST)^2=0$.}
%}$TS=0\Rightarrow STST=(ST)^2=0.$\PfEnd
%\SepLine

\ProblemN[]{\hypertarget{3B3}{3}}{
	\TextN{Sup $\Par{v_1,\dots,v_m}$ in V. Define $T\in\Lm{\Fbb^m, V}$ by $T\Par{z_1,\dots,z_m}=z_1 v_1+\dots+z_m v_m.$\vspace{2pt}}
	(a) \TextN{The surj of $T$ correspds to $\Par{v_1,\dots,v_m}$ spanning $V$.\hfill\large\envFontDefault$\range T=\Span{v_1,\dots,v_m}=V.$}
	(b) \TextN{The inje of $T$ correspds to $\Par{v_1,\dots,v_m}$ being linely inde.\hfill\large\envFontDefault $\Par{v_1,\dots,v_m}$ linely inde $\Longleftrightarrow T$ inje.}
}
\Comment \,\,\,Let $\Par{e_1,\dots,e_m}$ be the std basis of $\Fbb^m.$ Then $T{e_k}=v_k.$
\SepLine

\ProblemN{\hypertarget{3B7}{7}}{
	\TextN{Sup $V$ is findim with $2\leqslant\dim V.$ And $\dim V\leqslant\dim W=m$, if $W$ is findim.}
	\TextN{Showt $U=\Bra{\,T\in\Lm{V,W}:\null T\neq\envFontDefault\zeroSubs\envFontLarge}$ is not a subsp of $\Lm{V,W}$.}
...
\ProblemN{\hypertarget{3B8}{8}}{
	\TextN{Sup $W$ is findim with $\dim W\geqslant 2.$ And $n=\dim V\geqslant\dim W$, if $V$ is findim.}
	\TextN{Showt $U=\Bra{\,T\in\Lm{V,W}:\range T\neq W}$ is not a subsp of $\Lm{V,W}$.}
...
\BigPar{ For each $j=2,\dots,m;\,\,i=1,\dots,n-m$, if $V$ is findim, otherwise let $i\in\Nbp.$ } Thus $T_1+T_2\not\in U.$\PfEnd\vspace{2pt}
...
\ProblemN{\hypertarget{3B9}{9}}{
	\TextN{Sup $\Par{v_1,\dots,v_n}$ is linely inde. Provet $\forall$ inje $T,\Par{Tv_1,\dots,Tv_n}$ is linely inde.}
...
\ProblemN{\hypertarget{3B10}{10}} {
	\TextNL{Sup $\Span{v_1,\dots,v_n}=V$. Showt $\Span{Tv_1,\dots,Tv_n}=\range T$.}
...
%\ProblemN{\hypertarget{3B11}{11}}{
	%	\TextNL{Sup $S_1,\dots,S_n$ are linear and inje. $S_1 S_2\cdots S_n$ makes sense. Provet $S_1 S_2\dots S_n$ is inje.}
...
\ProblemN[]{\hypertarget{3B11}{11}}{
	\TextNL{Sup $S_1,\dots,S_n\in\Lm{V}$ and $S=S_1S_2\cdots S_n$ makes sense. \large\tgnr Then using induction\hspace{1pt}$:$\vspace{2pt}}
	...
	(3) $P_{inv}$ and $Q_{inv}$ hold. $Q_{p}$ in (1) and (2) holds $\Longleftrightarrow V$ is findim.\TextB{}
	(4) $P_{inje\;or\;surj}$ holds $\Longleftrightarrow V$ is findim $\Longleftrightarrow Q_{inje\;or\;surj}$ holds.\TextB{\vspace{-4pt}}
}\SepLine

\ProblemB{
	\TextB{Sup $S,T\in\Lm{V}.$ Prove or give a counterexa\hspace{1pt}$:$}
...
\ProblemN{\hypertarget{3B16}{16}}{
	\TextNL{Sup $T\in\Lm{V}$ shat $\null T,\range T$ are findim. Provet $V$ is findim.}
...
\ProblemN{\hypertarget{3B17}{17}}{
	\TextNL{Sup $V,W$ are findim. Provet $\exists$ inje $T\in\Lm{V,W}\Longleftrightarrow\dim V\leqslant\dim W$.}
}(a) Sup $\exists$ inje $T$. Then $\dim V=\dim\range T\leqslant\dim W$.\parSol{}
(b) Sup $\dim V\leqslant\dim W.$ Let $B_V=\Par{v_1,\dots,v_n},B_W=\Par{w_1,\dots,w_m}.$\parSol{\Hb}
Define $T\in\Lm{V,W}$ by $Tv_i=w_i,\,\,\,i=1,\dots,n\,\Par{\,=\dim V\,}.$\PfEnd
\SepLine

\ProblemN{\hypertarget{3B18}{18}}{
	\TextNL{Sup $V,W$ are findim. Provet $\exists$ surj $T\in\Lm{V,W}\Longleftrightarrow\dim V\geqslant\dim W$.}
}(a) Sup $\exists$ surj $T$. Then $\dim V=\dim W+\dim\null T\Rightarrow\dim W\leqslant\dim V$.\parSol{}
(b) Sup $\dim V\geqslant\dim W.$ Let $B_V=\Par{v_1,\dots,v_n},B_W=\Par{w_1,\dots,w_m}.$\parSol{\Hb}
Define $T\in\Lm{V,W}$ by $T\BigPar{a_1 v_1+\dots+a_m v_m+\dots+a_n v_n}=a_1 w_1+\dots+a_m w_m.$\PfEnd
\SepLine

\ProblemN{\hypertarget{3B19}{19}}{
	\TextNL{Sup $V,W$ are findim, $U$ is a subsp of $V$.}
	\TextNL{Provet $\exists\,T\in\Lm{V,W},\,\,\null T = U\Longleftrightarrow\underbrace{\dim U}_m\geqslant\underbrace{\dim V}_{m\,+\,n}-\underbrace{\dim W}_{p}.$}
}\par\quad
(a) Sup $\exists\,T\in\Lm{V,W},\null T=U.$ Then $\dim U+\dim\range T=\dim V\leqslant\dim U+\dim W.$\par\quad
(b) Let $B_U=\Par{u_1,\dots,u_m},B_V=\Par{u_1,\dots,u_m,v_1,\dots,v_n},B_W=\Par{w_1,\dots,w_p}.$ Sup that $p\geqslant n.$\par\quad\Hb
Define $T\in\Lm{V,W}$ by $T\Par{a_1 v_1+\dots+a_n v_n+b_1 u_1+\dots+b_m u_m}=a_1 w_1+\dots+a_n w_n.$\PfEnd
\SepLine\pagebreak

\ProblemB[]{
	\TipsN{1}\,\,\,\TextB{Sup $U$ is a subsp of $V.$ Then $\forall T\in\Lm{V,W},U\cap\null T=\null T\mmid_U.$\vspace{0pt}}
}%Note that $U\cap\null T\subseteq\null T\mmid_U.$ On the other hand, sup $u\in\null T\mmid_U\subseteq U.$\parSol{}
%Then $T\mmid_U\Par{u}=0$ makes sense and equals $Tu.$ Now $Tu=0\Rightarrow u\in\null T.$\PfEnd

\ProblemB{
	\TipsN{2}\,\,\,\TextB{Sup $T\in\Lm{V,W}$ and $T\mmid_U$ is inje. Let $V=M+N,\;U=X+Y.$\vspace{0pt}}
	{\IndentTipsN{2}}\TextB{Then $\range T=\range T\mmid_M+\range T\mmid_N=\range T\mmid_X+\range T\mmid_Y.$\vspace{1pt}}
	{\IndentTipsN{2}}(a) \TextB{Showt if $U=X\oplus Y,$ then $\range T=\range T\mmid_X\oplus\range T\mmid_Y.$\vspace{1pt}}
	{\IndentTipsN{2}}(b) \TextB{Give an exa shat $V=M\oplus N,\;\range T\neq\range T\mmid_M\oplus\range T\mmid_N.$}
}Asm for some $v\in V,$ there exist two disti pairs $\Par{x_1,y_1},\Par{x_2,y_2}$ in $X\times Y$\parSol{}
shat $Tv=Tx_1+Ty_1=Tx_2+Ty_2.$ Beca $\forall v\in X\oplus Y,\exists\,!\,\Par{x,y}\in X\times Y,v=x+y.$\parSol{}
...
\ProblemN{\hypertarget{3B12}{12}}{
	\TextNL{Provet $\forall T\in\Lm{V,W},\exists$ subsp $U$ of $V$ shat}
...
}By [2.34] \BigPar{ note that $V$ can be infindim }, $\exists$ subsp $U$ of $V$ shat $V=U\oplus\null T$.\parSol{}
...
We have shown $Q\Rightarrow P.$ Now we showt $P\Rightarrow Q$ to complete the proof.\parCor
...
Asm $\range T=\range T\mmid_U.$ Then $\range T\mmid_X=\zeroSubs.$ While $T\mmid_X$ is inje. Ctradic.\parCor
...
\Comment \,\,\,If $T\mmid_U:U\rightarrow\range T$ is an iso. Let $R\oplus U=V.$ Then $R$ might not be $\null T.$\parCom
...
\BulletPointX\TipsN{3}\,\,\,{Sup $T\in\Lm{V,W}$ and $U$ is a subsp shat $V=U\oplus\null T.$ Let $\null T=X\oplus Y.$}\TextB{}
...
\uline{Then $T=T\circ i.$} Beca $\forall v\in V,T\Par{v}=T\Par{u_v+x_v+y_v}=T\Par{u_v}=T\Par{u_v+x_v}=T\BigPar{i\Par{v}}=\Par{T\circ i}\Par{v}.$\par
\SepLine

\BulletPointX\TipsN{4}\,\,\,Sup $T\in\Lm{V,W},T\neq 0.$ Let $B_{\range T}=\Par{Tv_1,\dots,Tv_n}.$\TextB{}
By (3.A.4), $R=\Par{v_1,\dots,v_n}$ is linely inde in $V.$ Let $\Spn R=U.$ We will provet $U\oplus\null T=V.$\TextB{\vspace{2pt}}
...
Beca $\range T=\range T\mmid_U=\Span{Tv_1,\dots,Tv_n},$ 又 $T$ is inje.
\SepLine

%\ProblemB{
%	\TextB{Sup $V$ is findim, $T\in\Lm{V,W},B_{\range T}=\BigPar{Tv_1,\dots,Tv_n},B_V=\BigPar{v_1,\dots,v_n,u_1,\dots,u_m}$.\vspace{3pt}}
%	\TextB{Prove or give a counterexa\hspace{1pt}$:$ $\Par{u_1,\dots,u_m}$ is a basis of $\null T$.}
...
\ProblemBnoor{\hypertarget{3B4e27}{4E 27}, \OR 5.B.4}[\Sbra]{
	\TextB{Sup $P\in\Lm{V}$ and $P^2 = P$. Provet $V=\null P\oplus\range P$.\vspace{0pt}}
...
\ProblemB{
	\TextB{Sup $T\in\Lm{V},v\in V,$ and $n\in\Nbp$ shat $T^{n-1}v\neq 0,\;T^nv=0.$\qquad\large\envFontDefault\tgnr\Sbra{See [5.16]}}
	\TextB{Provet $\BigPar{v,Tv,\dots,T^{n-1}v}$ is linely inde.}
}$a_0v+a_1Tv+\dots+a_{n-1}T^{n-1}v=0\Rightarrow a_0T^{n-1}v=0\Rightarrow a_0=0.$ \,Similar for $a_1,\dots,a_{n-1}.$\PfEnd
\SepLine

\ProblemBnoor{\hypertarget{3B4e21}{4E 21}}{
	\TextB{Sup $V$ is findim, $T\in\Lm{V,W},$ $Y$ is a subsp of $W$. Let $\Bra{v\in V: Tv\in Y}.$\vspace{1.5pt}}
	(a) \TextB{Provet $\Bra{v\in V: Tv\in Y}$ is a subsp of $V.$\vspace{1.5pt}}
	(b) \TextB{Provet $\Dim\Bra{v\in V: Tv\in Y}=\dim\null T +\Dim\BigPar{Y\cap\range T}.$}
...
(b) Define the range-restricted map $R$ of $T$ by $R=T\mmid_{\mathcal{K}_{\!Y}}\in\Lm{\mathcal{K}_{\!Y},Y}.$ Now $\range R=Y\cap \range T.$\par\quad\Hb
...
\ProblemBnoor{\hypertarget{3B4e31}{4E 31}}{
	\TextB{Sup $V$ is findim, $X$ is a subsp of $V$, and $Y$ is a findim subsp of $W$.}
	\TextB{Provet if $\dim X + \dim Y = \dim V$, then $\exists\,T\in\Lm{V,W},\null T = X,\range T = Y$.}
...
%\ProblemN{\hypertarget{3B31}{31}}{
	%	\TextNL{Provet $\exists\,T_1,T_2\in\Lm{\Rbb^5,\Rbb^2},\null T_1=\null T_2$ and $T_1\neq cT_2,\forall c\in\Fbb$.}
...
%Sup $T=\lambda S$. Then $w_1=Tv_1=\lambda Sv_1=\lambda w_1\Rightarrow \lambda=1$.\parSol{\quad\qquad\qquad\qquad\,}
%While $w_2=Tv_2=\lambda Sv_2=2\lambda w_2\Rightarrow \lambda=\frac{\;1\;}{2}$. Ctradic.\PfEnd
%\SepLine

%\ProblemB{
%	\TipsN{5}\,\,\,\TextB{Sup $U,V$ are findim, $S\in\Lm{V,W},T\in\Lm{U,V}.$ Showt}
...
\ProblemN{\hypertarget{3B22}{22}}{
	\TextNL{Sup $U,V$ are findim, $S\in\Lm{V,W},T\in\Lm{U,V}$.}
	\TextNL{Provet $\dim \null ST\leqslant\dim\null S+\dim\null T$.\vspace{2pt}}
}We showt $\dim \null ST=\dim \null S\mmid_{\range T} + \dim \null T.$\parSol{}
Beca (a) \uline{$\range T\mmid_{\null ST}=\range T\cap\null S=\null S\mmid_{\range T}\,,$}\parSol{}
...
\ProblemN{\hypertarget{3B23}{23}}{
	\TextNL{Sup $V$ is findim, $S\in\Lm{V,W},T\in\Lm{U,V}$.}
	\TextNL{Provet $\dim\range ST\leqslant\min\!\Bra{\dim \range S, \dim\range T}$.}
...
\ProblemB{
	%(a) \TextB{Sup $\dim V = 5,$ and $ST = 0$ where $S,T\in\Lm{V}$. Provet $\dim\range TS\leqslant 2.$\vspace{2pt}}
	\hypertarget{3B4e24}{}(a) \TextB{Sup $\dim V=n,$ \,$ST=0$ where $S,T\in\Lm{V}.$ Provet $\dim\range TS\leqslant\Big\lfloor${\LARGE$\frac{\,n\,}{2}$}$\Big\rfloor.$}
...
%We showt $\dim\range TS\leqslant 2$ by ctradic. Asm $\dim\range TS\geqslant 3$.\par\quad\Ha
...
Asm $\dim\range TS\geqslant \Big\lfloor${\Large$\frac{\,n\,}{2}$}$\Big\rfloor+1.$ \,Then $\min\!\envFontLarge\Bra{n-\dim\null T,n-\dim\null S}\envFontDefault\geqslant \Big\lfloor${\Large$\frac{\,n\,}{2}$}$\Big\rfloor+1$\vspace{1pt}\par\quad
...
\Or Beca $\dim\range TS\leqslant\Big\lfloor${\Large$\frac{\,n\,}{2}$}$\Big\rfloor,$ and $\Big\lfloor${\Large$\frac{\,n\,}{2}$}$\Big\rfloor+\Big\lceil${\Large$\frac{\,n\,}{2}$}$\Big\rceil=n.$\par\vspace{1pt}\quad
We showt $\dim\null TS\geqslant\Big\lceil${\Large$\frac{\,n\,}{2}$}$\Big\rceil.$ Note that $\dim\null S+\dim\null T\geqslant n.$\par\quad
$\dim\null S+\dim\null T\mmid_{\range S}=\dim\null TS.$ \;If $\dim\null S\geqslant\Big\lceil${\Large$\frac{\,n\,}{2}$}$\Big\rceil.$ Then we are done.\par\vspace{1pt}\quad
...
\ProblemN{\hypertarget{3B26}{26}}{
	\TextNL{Sup $D\in\Lm[\BigPar]{\PoRi}$ and $\forall p,\Deg\BigPar{Dp}=\Par{\!\deg p}-1.$ \;Provet $D\in\PoRi$ is surj.\vspace{2pt}}
}\!\Sbra{ $D$ might not be $D:p\mapsto p\apostrophe.$\hspace{1pt}} \;\NOTICE that the following proof is wrong:\parSol{}
Beca $\Span{Dx,Dx^2,Dx^3,\cdots}\subseteq\range D,$ and $\deg Dx^n=n-1.$\parSol{}
...
Beca $B_{\PoR[\SmallPar]{m}}=\Par{p_1,\dots,p_m,p_{m+1}}.$ And for all $p\in\PoRi,\exists\,!\,m=\deg p\in\Nbp.$\par\quad
...
(i) {Beca $\deg Dx=\Par{\!\deg x}-1=0,Dx=C\in\nonzeroFbb.$ Let $p_0=C^{-1}x\Rightarrow Dp_0=C^{-1}Dx=1.$}\vspace{2pt}\par\quad\Endi
(ii) {Sup we have defined $Dp_0=1,Dp_k=x^k$ for each $k\in\Bra{1,\dots,n}.$ Beca $\deg D\Par{x^{n+2}}=n+1.$}\vspace{2pt}\par\quad\Hii
...
\ProblemN{\hypertarget{3B20}{}\hypertarget{3B21}{20, 21}}{
	(a) \Text{Provet if $ST=I\in\Lm{V},$ then $T$ is inje and $S$ is surj.}
	\Blind{\Onumber{20, 21} }(b) \Text{Sup $T\in\Lm{V,W}.$ Provet if $T$ is inje, then $\exists\,S\in\Lm{W,V},\;ST=I$.}
	\Blind{\Onumber{20, 21} }(c) \Text{Sup $S\in\Lm{W,V}.$ Provet if $S$ is surj, then $\exists\,T\in\Lm{V,W},\;ST=I.$\vspace{2pt}}
}\par\quad
...
Note that $S\mmid_U:U\rightarrow V$ is an iso. Define $T=\BigPar{S\mmid_U}{^{-1}},$ where $\BigPar{S\mmid_U}{^{-1}}:V\rightarrow U.$\par\quad\Hc
...
\BulletPointX\TipsN{5}\,\,\,Sup $S\in\Lm{U,V}$ is surj. Define $\mathcal{B}\in\Lm[\BigPar]{\Lm{V,W},\Lm{U,W}}$ by $\mathcal{B}\Par{T}=TS.$\TextB{}
\IndentTipsN{5}Then $\mathcal{B}$ is inje. Beca {$\mathcal{B}\Par{T}=TS=0\Longleftrightarrow T\mmid_{\range S}=0.$ \;\Or $\range TS=\range T=\zeroSubs.$}\vspace{-3pt}
%	\IndentTipsN{5}(b) Sup $S\in\Lm{V,W}$ is inje. Define $\mathcal{A}\in\Lm[\BigPar]{\Lm{U,V},\Lm{U,W}}$ by $\mathcal{A}\Par{T}=ST.$\TextB{}
%	\IndentTipsN{5}{\Hb}Then $\mathcal{A}$ is surj. Beca $\range\mathcal{F}=\forall R\in\Lm{U,W}.$\TextB{}
\SepLine

\ProblemN{\hypertarget{3B24}{24}}{
	\TextNL{Sup $S, T\in\Lm{V,W},$ and $\null S\subseteq\null T.$ Provet $\exists\,E\in\Lm{W},T=ES$.}
}\par\quad
...
\!\!\!$\hText{$
	Let \uline{$V=U\oplus\null S$}$\\$
	\uline{$\Rightarrow S\mmid_{U}:U\rightarrow\range S$ is an iso.}$\\$
...
$\hText{$\includegraphics[width=88pt]{diagram3B-2}$}\hText{$
	...
	while $E\mmid_{\cdots}:\range S\mmid_{U_\Delta}\rightarrow\range T$ is an iso.$}$\par\vspace{6pt}\quad
...
\Corollary \,\,\,\Sbra[3pt]{{\tgsl Req $W$ Finite-dim}} \;Sup $\null S=\null T.$ We showt $\exists$ inv $E\in\Lm{W},T=ES.$\par\quad
...
\ProblemN{\hypertarget{3B25}{25}}{
	\TextNL{Sup $S, T\in\Lm{V,W},$ and $\range T\subseteq\range S.$ Provet $\exists\,E\in\Lm{V},T = SE$.}
}\par\quad
Let $V=U\oplus\null S\Rightarrow S\mmid_U:U\rightarrow\range S$ is an iso. Beca $\Par{S\mmid_U}{^{-1}}:\underset{\supseteq\,\range T}{\uline{\range S}}\rightarrow U.$\par\vspace{-12pt}\quad
...
If $\Delta\neq\zeroSubs,$ asm $\exists$ inv $E\in\Lm{V}$ re-extended from $E\mmid_{U_2}$ still satisfying $T=SE,$\par\quad
...
\ProblemN{\hypertarget{3B28}{28}}{
	\TextNL{Sup $T\in\Lm{V,W}.$ Let $B_{\range T}=\Par{w_1,\dots,w_m}.$\vspace{4pt}}
	(a) \TextNL{Provet $\exists\,\varphi_1,\dots,\varphi_m\in\Lm{V,\Fbb}$ shat $\forall v\in V,Tv=\varphi_1\Par{v}w_1+\dots+\varphi_m\Par{v}w_m$.\vspace{2pt}}
	...
	\Blind{(b) {\envFontSmall\normalsize\Sbra{4E 3.F.5}}} \TextNL{Thus defining each $\varphi_i:V\rightarrow\Fbb.$ Showt each $\varphi_i\in\Lm{V,\Fbb}.$}
}(a) Using \TIPSN{4}. Let each $w_i=Tv_i.$ \;Then $\Par{v_1,\dots,v_m}$ is linely inde.\parSol{\Ha}
...
Linearity: $\forall v,w\in V\,\Sbra{\exists\,!\,a_i,b_i\in\Fbb},\lambda\in\Fbb,\varphi_i\Par{v+\lambda w}=a_i+\lambda b_i=\varphi\Par{v}+\lambda\varphi\Par{w}.$\PfEnd\parSol{\vspace{4pt}}
...
\Or Using (3.F). Let each $w_i=Tv_i\Rightarrow\Par{v_1,\dots,v_m}$ is linely inde.\parSol{\Hb}
...
\ProblemN{\hypertarget{3B29}{29}}{
	\TextNL{Sup $\varphi\in\Lm{V,\Fbb}.$ Sup $\varphi\Par{u}\neq 0.$ Provet $V = \null\varphi\oplus\Bra{au :a\in\Fbb}.$}\vspace{2pt}
...
\ProblemN{\hypertarget{3B30}{30}}{
	\TextNL{Sup $\varphi,\beta\in\Lm{V,\Fbb}$ and $\null\varphi=\null\beta=\eta.$ Provet $\exists\,c\in\Fbb,\varphi=c\beta.$}
}If $\eta=V$, then $\varphi=\beta=0,$ we are done. Now by Exe (29),\par\quad
...
\Or We discuss in two cases. If $\null \beta=\null \varphi$, or if $\varphi=0,$ then we are done. Otherwise,\par\quad\Ha
...
\BulletPointX\NoteForSmall{Transpose}\;\;{\normalsize\envFontSmall\Sbra{\hypertarget{3F33}{3.F.33}}}\;Define $\fT:A\rightarrow A^t.$ By [3.111], $\fT$ is linear. Beca $\Par{A^t}{^t}=A.$ \TextB{}
$\fT^2=I,\,\fT=\fT^{-1}\Rightarrow \fT$ is an iso of $\Fbb^{m,n}$ onto $\Fbb^{n,m}.$ \;Define $\fC_k:A\rightarrow A_{\cdot,k},\;\fR_j:A\rightarrow A_{j,\cdot},\;\fP_{j,k}:A\rightarrow A_{j,k}.$\TextB{}
...
%	\!\!$\Par{A_{2,\cdot}}{^t}={}{\small\begin{pmatrix}4 &\hspace{-6pt} 5 &\hspace{-6pt} 6\end{pmatrix}}{^t}\neq{}{\small\begin{pmatrix}2 &\hspace{-6pt} 5\end{pmatrix}}{}=\Par{A^t}{_{2,\cdot}}$ \;Similar for all $A_{j,\cdot},A_{\cdot,k},$ and $A_{j,k}.\\[2pt]$
%	\!\!Thus in general, $\fC_k\fT\neq \fT\fC_k,\;\fR_j\fT\neq \fT\fR_j,$ \,and $\fP_{j,k}\fT\neq \fT\fP_{j,k},}$\TextB{\vspace{6pt}}
%But $\Par{A_{2,\cdot}}{^t}=\Par{A^t}{_{\cdot,2}}.$ \;
Now we showt (a) \uline{$\fT\fR_j=\fC_j\fT,$} \,(b) \uline{$\fT\fC_k=\fR_k\fT,$} \,and (c) \uline{$\fT\fP_{j,k}=\fP_{k,j}\fT.$}\TextB{\vspace{2pt}}
...
\!\!$\Longrightarrow$ (b) holds. Similar for (a).$}$\par\vspace{6pt}
\SepLine
...
\ProblemBnoor[]{4E 3.51}[\Sbra]{
	Sup $C\in\Fbb^{m,c},R\in\Fbb^{c,p}$. \hfill\Sbra{ See also {\NOTEFOR} [3.49] and Exe (10). }\TextB{\vspace{2pt}}
...
\ProblemB{
	\hspace{0pt}\textsc{\Large Column-Row Factorization}\;\;\BigPar{CR Factorization}\;\;\vspace{3pt}\TextB{Sup $A\in\Fbb^{m,n},A\neq 0.$}
	\TextB{Prove, with $p$ specified below, that $\exists\,C\in\Fbb^{m,p},R\in\Fbb^{p,n},A=CR.$\vspace{3pt}}
	(a) \TextB{Sup $S_c=\Span[\BigPar]{A_{\cdot,1},\cdots,A_{\cdot,n}}\subseteq\Fbb^{m,1},\dim S_c=c,\text{ the col rank}.$ Let $p=c.$\vspace{3pt}}
	(b) \TextB{Sup $S_r=\Span[\BigPar]{A_{1,\cdot},\cdots,A_{m,\cdot}}\subseteq\Fbb^{1,n},\dim S_r=r,\text{ the row rank}.$ Let $p=r.$\vspace{3pt}}
}Using [4E 3.51]. Notice that $A\neq 0\Rightarrow c,r\geqslant 1.$\vspace{2pt}\par\quad
(a) Reduce to basis $B_C=\BigPar{C_{\cdot,1},\cdots,C_{\cdot,c}},$ forming $C\in\Fbb^{m,c}$. Then $\forall k\in\Bra{1,\dots,n},$\vspace{2pt}\par\quad\Ha
...
(b) Reduce to basis $B_R=\BigPar{R_{1,\cdot},\cdots,R_{r,\cdot}},$ forming $R\in\Fbb^{r,n}$. Then $\forall j\in\Bra{1,\dots,m},$\vspace{2pt}\par\quad\Hb
...
{\small$\begin{pmatrix} 46 &\hspace{-4pt} 33 &\hspace{-4pt} 20 &\hspace{-4pt} 7 \end{pmatrix}$}${}\in\Span{A_{1,\cdot},A_{2,\cdot}},$ and $\Par{A_{1,\cdot},A_{2,\cdot}}$ is linely inde. Thus $B_R=\BigPar{A_{1,\cdot},A_{2,\cdot}}.$\par\vspace{6pt}\quad\EndI
...
%$\exists\,C={}${\normalsize$\begin{pmatrix}c_1 \\[-4pt] \vdots \\[-5pt] c_m \end{pmatrix}$}${}\in\Fbb^{m,1},R=\begin{pmatrix} d_1 & \hspace{-6pt}\cdots & \hspace{-6pt}d_n\end{pmatrix}\in\Fbb^{1,n}$ shat $A=CR.$
...
$P\Rightarrow Q:$\,\;Beca $\dim S_c=\dim S_r=1.$\vspace{4pt}\par\quad
...
%(a) Sup $T$ is inv, so is $T\apostrophe$. Beca $\BigPar{T\apostrophe\Par{\varphi_1},\dots,T\apostrophe\Par{\varphi_m}}$ is linely inde.\par\quad\Ha
...
%(b) Sup the rows of $A$ are linely inde, so are the cols of $A^t$. \NOTICE that $A^t$ has $\dim V\apostrophe$ cols.
...
\ProblemB{
	\TipsN{1}\,\,\,\TextB{Sup $T\in\Lm{V,W},\;B_V=\Par{v_1,\dots,v_n},B_W=\Par{w_1,\dots,w_m}.$\vspace{-1pt}}
	...
	\IndentTipsN{1}(a) \TextB{Showt [P] $L$ is linely inde $\Longleftrightarrow M$ is linely inde. [Q]\vspace{2pt}}
	\IndentTipsN{1}(b) \TextB{Showt [P] $\Spn L=W\Longleftrightarrow\Spn M=\Fbb^{m,1}.$ [Q]\hfill\large\envFontDefault\Sbra{ Let $A=\Mt{T,B_V,B_W}.$\hspace{1pt}}}
}\par\quad
(a) Note that $\mathcal{M}\!:\,Tv_k\rightarrow A_{\cdot,k}$ is an iso of $W$ onto $\Fbb^{m,1}.$ \quad (b) Reduce $L$ to $B_W',\;M$ to $B_{\Fbb^{m,1}}.$ Similarly.\PfEnd\vspace{4pt}\quad
...
(a) $P\Rightarrow Q:$\,\;Sup $\;c_1A_{\cdot,{\alpha_1}}+\dots+c_kA_{\cdot,{\alpha_k}}=0.$ \;Let $v=c_1v_{\alpha_1}+\dots+c_kv_{\alpha_k}.$\par\quad\Ha
...
\Blind{$Q\Rightarrow P:$\,\;}Now $c_1Tv_{\alpha_1}+\dots+c_kTv_{\alpha_k}=0.$ Then each $c_i=0\Rightarrow M$ linely inde.\vspace{4pt}\par\quad\Ha
$Q\Rightarrow P:$\,\;Beca $c_1Tv_{\alpha_1}+\dots+c_kTv_{\alpha_k}=0.$ For each $i\in\Bra{1,\dots,m},\;c_1A_{i,{\alpha_1}}+\dots+c_kA_{i,{\alpha_k}}=0.$\par\quad\Ha
\Blind{$Q\Rightarrow P:$\,\;}Which is equi to $c_1A_{\cdot,{\alpha_1}}+\dots+c_kA_{\cdot,{\alpha_k}}=0.$ \;Thus each $c_i=0\Rightarrow L$ linely inde.\par\vspace{4pt}\quad\Ha
...
$P\Rightarrow Q:$\,\;Sup each $w_i=Iw_i=J_{1,i}Tv_{\alpha_1}+\dots+J_{k,i}Tv_{\alpha_k}.$\par\quad\Hb
...
\Blind{$Q\Rightarrow P:$\,\;}Beca $w=a_1\BigPar{J_{1,1}Tv_{\alpha_1}+\dots+J_{k,1}Tv_{\alpha_k}}+\dots+a_m\BigPar{J_{1,m}Tv_{\alpha_1}+\dots+J_{k,m}Tv_{\alpha_k}}$\par\vspace{2pt}\quad\Hb
\Blind{$Q\Rightarrow P:$\,\;Beca} $\Blind{w}=\BigPar{a_1J_{1,1}+\dots+a_mJ_{1,m}}Tv_{\alpha_1}+\dots+\BigPar{a_1J_{k,1}+\dots+a_mJ_{k,m}}Tv_{\alpha_k}.$\par\vspace{2pt}\quad\Hb
...
Then (a*) By \Sbra{3.B.9, \TIPSN{4}}, $T$ is inje $\Longleftrightarrow L$ is linely inde, so is $M$.\parCor
...
(c) $T$ surj $\Longleftrightarrow T\apostrophe$ inje $\Longleftrightarrow\BigPar{T\apostrophe\Par{\psi_1},\dots,T\apostrophe\Par{\psi_m}}$ linely inde\parCor\Hc
\Blind{$T$ surj }$\xLongleftrightarrow{\text{(a)}}{\envFontLarge\BigPar{{\envFontDefault\Par{A^t}{_{\cdot,1}},\cdots,\Par{A^t}{_{\cdot,m}}}}}$ linely inde in $\Fbb^{n,1},$ so is $\BigPar{A_{1,\cdot},\cdots,A_{m,\cdot}}$ in $\Fbb^{1,n}.$\vspace{6pt}\parCor
...
\ProblemB{
	\TipsN{2}\,\,\,\TextB{Sup $p$ is a poly of $\,n\,$ variables in $\Fbb.$ Provet $\Mt[\BigPar]{p\Par{T_1,\dots,T_n}}=p\BigPar{\Mt{T_1},\dots,\Mt{T_n}}.$}
	\IndentTipsN{2}\TextB{\large\envFontDefault Where the linear maps $T_1,\dots,T_n$ are shat $p\Par{T_1,\dots,T_n}$ makes sense. \tgnr See [5.16,17,20].}
}Sup the poly $p$ is defined by $p\Par{x_1,\dots,x_n}=\sum_{k_1,\dots,k_n}\!\!\alpha_{k_1,\dots,k_n}\prod_{i=1}^n x_i^{k_i}.$\parSol{\vspace{4pt}}
...
\BulletPointX\Corollary \,\,\,Sup $\tau$ is an algebraic property. Then $\tau$ holds for linear maps $\Longleftrightarrow \tau$ holds for matrices.\parCor{\IndentB}
...
\ProblemN{\hypertarget{3C13}{13}}{
	\TextNL{Provet the distr holds for matrix add and matrix multi.}
	\TextNL{\large\envFontDefault Sup $A, B, C$ are matrices shat $A\Par{B + C}$ make sense, we prove the left distr.}
}Sup $A\in\Fbb^{m,n}$ and $B,C\in\Fbb^{n,p}.$\parSol{}
...
\Or Define $T,S,R$ shat $\Mt{T}=A,\Mt{S}=B,\Mt{R}=C.$\parSol{}
...
%\ProblemN{\hypertarget{3C14}{14}}{
	%	\TextNL{Provet matrix multi is associ.}
	%	\TextNL{\large\envFontDefault Sup $A,B,C$ are matrices shat $\Par{AB}C$ makes sense, we provet $\Par{AB}C=A\Par{BC}.$}
	%}Sup $A\in\Fbb^{m,n}$ and $B,C\in\Fbb^{n,p}.$ We showt $LHS=\Sbra{\Par{AB}C}_{j,k}=\Sbra{A\Par{BC}}_{j,k}=RHS.$\parSol{}
...
%\ProblemN{\hypertarget{3C15}{15}}{
	%	\TextNL{Sup $A\in\Fbb^{n,n},j,k\in\Bra{1,\dots,n}$. Showt $\BigPar{A^3}_{j,k}=\sum_{p=1}^n \sum_{r=1}^n A_{j,p}A_{p,r}A_{r,k}$.\vspace{6pt}}
...
%\ProblemB{
	%	\hypertarget{3C12}{}\TextB{Provet the commutativity does not hold in $\Fbb^{m,n}.$}
	%}Sup $\dim V=n,\dim W=m$ and the commutativity holds in $\Fbb^{n,m}.$\parSol{}
%$\forall T\in\Lm{V,W},S\in\Lm{W,V},\Mt{TS}=\Mt{T}\Mt{S}=\Mt{S}\Mt{T}=\Mt{ST}.$\parSol{}
%Hence $ST=TS.$ Which in general does not hold.\PfEnd
%\SepLine

%\ProblemN{12}{
	%	\TextNL{Give an exa of 2-by-2 matrices $A$ and $B$ shat $AB\neq BA$.}\vspace{6pt}
...
\ProblemN{\hypertarget{3C1}{1}}{
	\TextN{Sup $T\in\Lm{V,W}.$ Showt for each pair of $B_V$ and $B_W,$}
...
\Or We prove by ctradic. Sup $A$ has at most $\Par{n-1}$ nonzero ent.\par\quad
...
\ProblemN{\hypertarget{3C6}{6}}{
	\TextN{Sup $V$ and $W$ are findim and $T\in\Lm{V,W}$.}
	\TextN{Provet $\dim\range T = 1\Longleftrightarrow\exists\,B_V,B_W,$ all ent of $A=\Mt[\BigPar]{T,B_V,B_W}$ equal $1$.}
}\par\quad
(a) Sup $B_V=\Par{v_1,\dots,v_n},B_W=\Par{w_1,\dots,w_m}$ are the bases shat all ent of $A$ equal $1$.\par\quad\Ha
Then $Tv_i=w_1+\dots+w_m$ for all $i=1,\dots,n$. Beca $w_1,\dots,w_n$ is linely inde, $w_1+\dots+w_n\neq 0.$\vspace{4pt}\par\quad
(b) Sup $\dim\range T=1$. Then $\dim\null T=\dim V-1$.\par\quad\Hb
...
\Or Sup $B_{\range T}=\Par{w}.$ \,By \Sbra{2.C {\NOTEFOR} (15)}, $\exists\,B_W=\Par{w_1,\dots,w_m},\;w=w_1+\dots+w_m.$\par\quad\Hb
By \Sbra{2.C {\TIPS}}, $\exists$ a basis $\Par{u_1,\dots,u_n}$ of $V$ shat each $u_k\not\in\null T.$\par\quad\Hb
...
\ProblemN{\hypertarget{3C3}{3}}{
	\TextN{Sup $V$ and $W$ are findim and $T\in\Lm{V,W}$. Provet $\exists\,B_V,B_W$ shat}
...
Now $\exists\,B_{V\apostrophe}$ shat $\Mt{T\apostrophe,B_{W\apostrophe}\,,B_{V\apostrophe}}{_{\cdot,1}}={\small\begin{pmatrix}1&0&\cdots&0\end{pmatrix}}{^t}$ or ${\small\begin{pmatrix}0&\cdots&0\end{pmatrix}}{^t}.$\par\quad
Which is equiv to $\exists\,B_V$ \Sbra{Using (3.F.31)} shat $\Mt{T,B_V,B_W}{_{1,\cdot}}={\small\begin{pmatrix}1&0&\cdots&0\end{pmatrix}}$ or ${\small\begin{pmatrix}0&\cdots&0\end{pmatrix}}.$\PfEnd
\SepLine
\ChEnd
...
\ProblemN{\hypertarget{3D2}{2}}{
	\TextN{Sup $V$ is findim and $\dim V > 1$.}
	\TextN{Provet the set $U$ of non-inv operators on $V$ is not a subsp of $\Lm{V}$.}
	\TextN{\large\tgsl The set of inv operators is not either. Although multi id/inv, and commu for vec multi hold.}
...
\ProblemB{
	\Tips \,\,\,\TextB{Sup $U\oplus X=W\oplus Y,$ and $X,Y$ are iso. Provet $U,W$ are iso.}
...
Now sup $u_1,u_2\in U,$ then each $u_i=w_i+\xi\Par{x_i},\exists\,!\,w_i\in W,x_i\in X.$\parSol{}
...
\ProblemN{\hypertarget{3D3}{3}}{
	\TextN{Sup $V$ and $W$ are iso, $U$ is a subsp of $V$, and $S\in\Lm{U,W}.$}
	\TextN{Provet $\exists$ inv $T\in\Lm{V,W},Tu = Su,\forall u\in U$ $\Longleftrightarrow$ $S$ is inje. \hfill{\envFontDefault\large\Sbra{ See also (3.A.11). }}}\vspace{2pt}
...
(b) Let $X\oplus U=V.$ Beca $S:U\rightarrow V$ is inje. By (3.B.12), $S:U\rightarrow\range S$ is an iso.\parSol{\Hb}
...
\ProblemN{\hypertarget{3D8}{8}}{
	\TextN{Sup $T\in\Lm{V,W}$ is {\tgsc surj}. Provet $\exists$ subsp $U$ of $V,$ \;$T\mmid_U:U\rightarrow W$ is an iso.}
}By (3.B.12). Note that $\range T=W.$ \; \Or \Sbra[3pt]{{\tgsl Req $\range T$ Finite-dim}} \;By \Sbra{3.B \TIPSN{4}}.\PfEnd
%\vspace{4pt}\Comment \,\,\,See (3.B.12), (4E 3.B.21), (3.B \TIPS).\vspace{-2pt}
\SepLine

\ProblemN{\hypertarget{3D18}{18}}{
	\TextNL{Showt $V$ and $\Lm{\Fbb, V}$ are iso vecsps.}
}\par\quad
...
(a) Sup $\Phi\Par{T}=0=T\Par{1}=\lambda T\Par{1}=T\Par{\lambda},\forall\lambda\in\Fbb\Rightarrow T=0.$ Thus $\Phi$ is inje.\par\quad
...
%\ProblemB{
	%	\TextB{Provet [P] $\Par{v_1,\dots,v_m}$ is linely inde $\Longleftrightarrow\BigPar{\Psi\Par{v_1},\dots,\Psi\Par{v_m}}$ is linely inde. [Q]}
	%}\par\quad
%$[P]\Rightarrow[Q]:\;$ Notice that $\Psi$ is inje and by (3.B.9).\par\quad
%\Blind{$[P]\Rightarrow[Q]:\;$} \Or Sup $\vartheta\in\Span[\BigPar]{\Psi\Par{v_1},\dots,\Psi\Par{v_m}}$. Let $\vartheta=0=a_1\Psi\Par{v_1}+\dots+a_m\Psi\Par{v_m}.$\par\quad
%\Blind{$[P]\Rightarrow[Q]:\;$} Then $\vartheta\Par{1}=0=a_1 v_1+\dots+a_m v_m\Rightarrow a_1=\dots=a_m=0.$\vspace{2pt}\par\quad
%$[Q]\Rightarrow[P]:\;$ Sup $v\in\Span{v_1,\dots,v_m}.$ Let $v=0=a_1 v_1+\dots+a_m v_m.$\par\quad
%\Blind{$[Q]\Rightarrow[P]:\;$} Then $\Psi\Par{v}=0=a_1\Psi\Par{v_1}+\dots+a_m\Psi\Par{v_m}\Rightarrow a_1=\dots=a_m=0.$\PfEnd\vspace{-3pt}
\SepLine

\ProblemB[]{
	\TextB{Sup $S,T\in\Lm{V,W}.$ \envFontDefault\large\hfill \Sbra{ For Exe \hypertarget{3D4}{(4)} and \hypertarget{3D5}{(5)}, see the {\COROLLARY} in (3.B.24, 25). }}
}
%\ProblemN[]{\hypertarget{3D4}{4}}{
%	\TextN{Sup $\null S = \null T${\normalsize\envFontSmall\Par{$\,= U\,$}}. Then $S = ET,\,\exists$ inv $E\in\Lm{W}$.}
%}
%\ProblemN{\hypertarget{3D5}{5}}{
%	\TextN{Sup $V$ is findim. If $\range S=\range T.$ Then $S=TE,\,\exists$ inv $E\in\Lm{V}$}
...
%$\MathLeftrightMid{l}{$
%	Let $B_R=\Par{Tv_1,\dots,Tv_m};\;B_R'=\Par{Sr_1,\dots,Sr_m}$ shat $\forall\,i,Tv_i=Sr_i.\\ $
...
\ProblemN{\hypertarget{3D6}{6}}{
	\TextN{Sup $V$ and $W$ are findim. $\dim\null S=\dim\null T=n.$}
	\TextN{Provet $S=E_2 T E_1,\exists$ inv $ E_1\in\Lm{V}, E_2\in\Lm{W}.$}
...
\ProblemB{
	(a) \TextB{Sup $T=ES$ and $E\in\Lm{W}$ is inv. Provet $\null S=\null T.$\vspace{2pt}}
	(b) \TextB{Sup $T=SE$ and $E\in\Lm{V}$ is inv. Provet $\range S=\range T.$\vspace{2pt}}
	(c) \TextB{Sup $T=E_2SE_1$ and $E_1\in\Lm{V},E_2\in\Lm{W}$ are inv.}
	\Hc\TextB{Provet $\dim\null S=\dim\null T.$}
...
\BulletPointX\NoteForSmall{[3.69]}\;\;Sup $V,W$ are findim and iso, $T\in\Lm{V,W}.$ Then $T$ inv $\Longleftrightarrow$ inje $\Longleftrightarrow$ surj.
\SepLine
...
\ProblemNnoor{\hypertarget{3D9}{9}}{\OR \hypertarget{3D1}{1}}{
	\TextN{Sup $U,V,W$ are iso and findim, $S\in\Lm{V,W},T\in\Lm{U,V}.$}
	\TextN{Provet $ST$ is inv $\Longleftrightarrow S,T$ are inv.}
	\Comment \,\,\,If any two of $U,V,W$ are not iso or findim, then $S,T$ are inv $\Longrightarrow ST$ is inv.\TextN{}
	%\Corollary \,\,\,Sup $T$ is inv. Let $S=T^{-1}\Rightarrow ST=I_U.$ Then $I_U$ inv $\Rightarrow T^{-1}$ inv.
}Sup $S,T$ are inv. Then $\Par{ST}\Par{T^{-1}S^{-1}}=I_W,\Par{T^{-1}S^{-1}}\Par{ST}=I_U.$ Hence $ST$ is inv.\parSol{}
Sup $ST$ is inv. Let $R=\Par{ST}{^{-1}}\Rightarrow R\Par{ST}=I_U,\Par{ST}R=I_W.$\parSol{\vspace{4pt}}
...
\ProblemN{\hypertarget{3D13}{13}}{
	\TextNL{Sup $U,V,W,X$ are iso and findim, $R\in\Lm{W,X}, S\in\Lm{V,W}, T\in\Lm{U,V}.$}
	\TextNL{Sup $RST$ is surj. Provet $S$ is inje.}
}Using Exe (9). Notice that $U,X$ are findim, so that $RST$ is inv.\vspace{4pt}\par\quad
...
%	\TextN{Sup $T\in\Lm{U,V},S\in\Lm{V,W}$ are inv. Provet $ST$ is inv and $\Par{ST}{^{-1}} = T^{-1}S^{-1}$.\vspace{8pt}}
...
\ProblemN{\hypertarget{3D10}{10}}{
	\TextN{Sup $V$ is findim and $S,T\in\Lm{V}.$ Provet $ST=I\Longleftrightarrow TS=I.$}
}(a) Sup $ST=I.$\parSol{\Ha}
By (3.B 20, 21)(a), $ST=I\Rightarrow T$ is inje and $S$ is surj. 又 $V$ is findim. $S,T$ are inv.\parSol{\Ha}
\Or {\normalsize By Exe (9), $V$ is findim and $ST=I$ is inv $\Rightarrow$ $S,T$ are inv.}\parSol{\vspace{4pt}\Ha}
...
\ProblemN{\hypertarget{3D11}{11}}{
	\TextNL{Sup $V$ is findim, $S, T, U\in\Lm{V}$ and $STU = I$. Showt $T$ is inv and $T^{-1} = US$.}
}Using Exe (9) and (10). {\tgsl\normalsize This result can fail without the hypothesis that $V$ is findim.}\parSol{}
...
{\normalsize\envFontSmall$\MathRightMid{l}{\!\!\!$\BulletPointX\hypertarget{3D4e3}{\Par{4E 3}} $T\in\Lm{V}\\$\quad\;$V$ is findim$}\MathRightBrace{l}{$
...
\ProblemBnoor{4E 15}{
	\hypertarget{3D4e15}{}\TextB{Sup $T\in\Lm{V}$ and $V=\Span{Tv_1,\dots,Tv_m}$. Provet $V=\Span{v_1,\dots,v_m}$.}
}Beca $V=\Span{Tv_1,\dots,Tv_m}\Rightarrow T$ is surj, and therefore is inv $\Rightarrow$ $T^{-1}$ is inv.\parSol{}
...
\ProblemN{\hypertarget{3D15}{15}}{
	\TextNL{Provet every linear map from $\Fbb^{n,1}$ to $\Fbb^{m,1}$ is given by a matrix multi.}
	\TextNL{In other words, provet if $T\in\Lm{\Fbb^{n,1},\Fbb^{m,1}}$, then $\exists\,A\in\Fbb^{m,n},Tx = Ax,\,\forall x\in\Fbb^{n,1}$.}
}Let $B_1=\Par{E_1,\dots,E_n},B_2=\Par{R_1,\dots,R_m}$ be the std bases of $\Fbb^{n,1},\Fbb^{m,1}$.\vspace{-12pt}\parSol{}
...
\BulletPointX\NoteForSmall{[3.62]}\;\;$\Mt{v}=\Mt[\BigPar]{I,\Par{v},B_V}.$ Where $I$ is the id operator restricted to $\Span{v}.$\par\vspace{2pt}
...
\ProblemBnoor{4E 23, \OR \hypertarget{3D4e23}{10.A.4}}{
	\TextB{Sup that $\Par{\beta_1,\dots,\beta_n}$ and $\Par{\alpha_1,\dots,\alpha_n}$ are bases of $V$.}
	\TextB{Let $T\in\Lm{V}$ be shat each $T\alpha_k = \beta_k.$ Provet $\Mt{T,\alpha\rightarrow\alpha}=\Mt{I,\beta\rightarrow\alpha}.$}
	\TextB{{\large\envFontDefault For ease of notation, let $\Mt[\BigPar]{T,\alpha\rightarrow\beta}=\Mt[\BigPar]{T,\Par{\alpha_1,\dots,\alpha_n},\Par{\beta_1,\dots,\beta_n}}.$}}
}\par\quad
...
\ProblemBnoor{4E 22, \OR \hypertarget{3D4e22}{10.A.1}}{
	\TextB{Sup $T\in\Lm{V}.$ Provet $\Mt{T,B_V}$ is inv $\Longleftrightarrow T$ itself is inv.}
...
(b) $\Mt{T}\Mt{T}{^{-1}}=\Mt{T}{^{-1}}\Mt{T}=I,\;\exists\,!\,S\in\Lm{V}$ shat $\Mt{T}{^{-1}}=\Mt{S}$\par\quad\Hb
...
\ProblemBnoor{4E 24, \OR \hypertarget{3D4e24}{10.A.2}}{
	\TextNL{Sup $A,B\in\Fbb^{n,n}$. Provet $AB = I\Longleftrightarrow BA=I.$\hfill\small\envFontSmall\Sbra{Using Exe (10, 15).}}
...
\Or Beca $\mathcal{M}\!:\Lm{\Fbb^{n,1},\Fbb^{n,1}}\rightarrow\Fbb^{n,n}$ is an iso. $\Mneg\Par{AB}=TS=ST=\Mneg\Par{BA}=I.$\PfEnd
\SepLine\pagebreak

\BulletPointX\NoteFor{[3.60]}\;\;Sup $B_V=\Par{v_1,\dots,v_n},\;B_W=\Par{w_1,\dots,w_m}$.\TextB{\vspace{2pt}}
...
\ProblemN{\hypertarget{3D17}{17}}{
	\TextNL{Sup $V$ is findim. Showt the only two-sided ideals of $\Lm{V}$ are $\zeroSubs$ and $\Lm{V}$.}
...
Let $B_V=\Par{v_1,\dots,v_n}.$ If $\mathcal{E}=0$, then we are done. Sup $\mathcal{E}\neq 0$ and $\mathcal{E}$ is a two-sided ideal of $\Lm{V}$.\vspace{3pt}\par\quad
Then {\Large\envFontLarge$\forall E_{i,j}\in\mathcal{E},$} by asm, {\Large\envFontLarge\vspace{3pt}$\forall x,y\in\Bra{1,\dots,n},E_{j,x}E_{i,j}=E_{i,x}\in\mathcal{E},E_{i,j}E_{y,i}=E_{y,j}\in\mathcal{E}.$}\par\quad
...
\ProblemBnoor{4E 10}{
	\hypertarget{3D4e10}{}\TextB{Sup $V,W$ are findim, $U$ is a subsp of $V$.\vspace{3pt}}
	\TextB{Let $\mathcal{E} = \Bra{T\in\Lm{V,W}: U\subseteq\null T}=\Bra{T\in\Lm{V,W}:T\mmid_U=0}$.\vspace{2pt}}
	(a) \TextB{Showt $\mathcal{E}$ is a subsp of $\Lm{V,W}$.\vspace{1pt}}
...
\ProblemBnoor{4E 17}{
	\hypertarget{3D4e17}{}\TextB{Sup $V$ is findim and $S\in\Lm{V}$. Define $\mathcal{A}\in \Lm[\BigPar]{\Lm{V}}$ by $\mathcal{A}\Par{T} = ST$.}
	(a) \TextB{Showt $\dim\null \mathcal{A} = \Par{\!\dim V}\Par{\!\dim \null S}$.}
	(b) \TextB{Showt $\dim\range \mathcal{A} = \Par{\!\dim V}\Par{\!\dim \range S}$.}
...
\ProblemB{
	...
	(b) \TextB{Showt $\dim\range\mathcal{B}=\Par{\!\dim V}\Par{\!\dim\range S}.$}
}(a) %\Large\vspace{3pt}
...
\Or Using {\NOTEFOR} [3.60] and notation in Exe (4E 17).\par\vspace{-18pt}\quad
...
\ProblemBnoor{\hypertarget{3D4e20}{4E 20}}{
	\TextB{Sup $q\in\PoRi.$ Provet $\exists\,p\in\PoRi,q\Par{x} = \Par{x^2 + x}p\apostrophe\apostrophe\Par{x} + 2xp\apostrophe\Par{x} + p\Par{3}.$\vspace{1pt}}
...
\ProblemN{\hypertarget{3D19}{19}}{
	\TextNL{Sup $T\in\Lm[\BigPar]{\PoRi}$ is inje. And $\deg Tp\leqslant\deg p$ for every nonzero $p\in\PoRi$.}
	(a) \TextNL{Provet $T$ is surj;\quad {\large\tgnr(b)} Provet for every nonzero $p$, $\deg Tp=\deg p$.}
...
(ii) Asm $\forall s\in\PoR{n},\deg s=\deg Ts.$ We show $\forall p\in\PoR{n+1},\deg Tp=\deg p$ by ctradic.\par\quad\Hii
Sup $\exists\,r\in\PoR{n+1},\;\deg Tr\leqslant n<n+1=\deg r.$ Then by (a), $\exists\,s\in\PoR{n},\;T\Par{s}=\Par{Tr}.$\par\quad\Hii
又 $T$ is inje $\Rightarrow s=r.$ While $\deg s=\deg Ts=\deg Tr<\deg r.$ Ctradic.\PfEnd
\SepLine

\ProblemN{\hypertarget{3D16}{16}}{
	\TextNL{Sup $V$ is findim and $S\in\Lm{V}$ shat $\forall T\in\Lm{V},ST=TS$. Provet $\exists\,\lambda\in\Fbb,S=\lambda I$.}
}If $S=0$, we are done. Now sup $S\neq 0.$\hfill\Sbra{Using notation in Exe (4E 17). See also in (3.A).}\par\quad
...
\ProblemBnoor{\hypertarget{3D4e19}{}10.A.3, \OR 4E 19}{
	\TextB{Sup $V$ is findim and $T\in\Lm{V}$.\large\envFontDefault\hfill\Sbra{See also in (3.A).}}
	\TextB{Provet $\forall B_V\neq B_V',\;\Mt[\BigPar]{T,B_V}=\Mt[\BigPar]{T,B'_V}\Longrightarrow T=\lambda{I},\exists\,\lambda\in\Fbb.$}
}Sup $\forall B_V\neq B_V',\;\Mt{T,B_V}=\Mt{T,B'_V}.$ If $T=0$, then we are done.\vspace{1pt}\parSol{}
Sup $T\neq 0,$ and $v\in V\nonzero.$ Asm $\Par{v,Tv}$ is linely inde.\vspace{1pt}\parSol{}
...
By asm, $A=\Mt{T,B_V'}=B,\forall B_V'=\Par{v,w_2,\dots,w_{n}}$. Then $A_{2,1}=1,A_{i,1}=0,\forall i\neq 2.$\parSol{}
...
Now we showt $\lambda_v$ is independent of $v$, that is, for all disti $v,w\in V\nonzero,\lambda_v=\lambda_w.$\parSol{}
\!\!\!\!$\MathRightBrace{l}{
...
Now we showt $A_{k,k}=A_{j,j}$ for all $j\neq k.$ Choose $j,k$ shat $j\neq k.$\vspace{1pt}\par\quad
...
\ProblemN{\hypertarget{3E2}{2}}{
	\TextN{Sup $V_{\!1}\times\dots\times V_{\!m}$ is findim. Provet each $V_{\!j}$ is findim.}
}\par\quad
For any $k\in\Bra{1,\dots,m},$ define $S_k\in\Lm{V_{\!1}\times\dots\times V_{\!m},V_{\!k}}$ by $S_k\Par{v_1,\dots,v_m}=v_k.$\par\quad
Then $S_k$ is linear map. By [3.22], $\range S_k=V_{\!k}$ is findim.\PfEnd\vspace{6pt}\par\quad
\Or Denote $V_{\!1}\times\cdots\times V_{\!m}$ by $U$. Denote $\zeroSubs\times\cdots\zeroSubs\times V_{\!i}\times\zeroSubs\cdots\times\zeroSubs$ by $U_i$.\par\quad
We showt each $U_i$ is iso to $V_{\!i}.$ Then $U$ is findim $\Longrightarrow$ its subsp $U_i$ is  findim, so is $V_{\!i}.$\par\vspace{2pt}\quad
...
\ProblemN{\hypertarget{3E4}{4}}{
	\TextN{Provet $\Lm{V_{\!1}\times\cdots\times V_{\!m},W}$ and $\Lm{V_{\!1},W}\times\cdots\times\Lm{V_{\!m},W}$ are iso.}
}Using notation in Exe (2): $R_i:u_i\mapsto\Par{0,\dots,u_i,\dots,0};\;S_i:\Par{u_1,\dots,u_m}\mapsto u_i.$\par\quad
...
\ProblemN{\hypertarget{3E5}{5}}{
	\TextN{Provet $\Lm{V,W_1\times\cdots\times W_m}$ and $\Lm{V,W_1}\times\cdots\times\Lm{V,W_m}$ are iso.}
}Using notation in Exe (2): $R_i:u_i\mapsto\Par{0,\dots,u_i,\dots,0};\;S_i:\Par{u_1,\dots,u_m}\mapsto u_i.$\par{\hspace{0pt}}
...
\ProblemN{\hypertarget{3E6}{6}}{
	\TextN{For $m\in\Nbp,$ define $V^m$ by $\mathop{\underbrace{V\times\cdots\times V}}\limits_{m\,\,times}$. Provet $V^m$ and $\Lm{\Fbb^m,V}$ are iso.}
}\par\quad
...
(a) Sup $T\Par{v_1,\dots,v_m}=0.$ Then $\forall\Par{a_1,\dots,a_n}\in\Fbb^m$, $\varphi\Par{a_1,\dots,a_m}=a_1 v_1+\dots+a_m v_m=0$\par\quad\Ha
...
(b) Sup $\psi\in\Lm{\Fbb^m,V}.$ Let $\Par{e_1,\dots,e_m}$ be the std basis of $\Fbb^m$. Then $\forall\Par{b_1,\dots,b_n}\in\Fbb^m$,\vspace{3pt}\par\quad\Hb
...
\ProblemN{\hypertarget{3E3}{3}}{
	\TextN{Give an exa of a vecsp $V$ and its two subsps $U_1,U_2$ shat}
	\TextN{$U_1\times U_2$ and $U_1+U_2$ are iso but $U_1+U_2$ is not a direct sum.\large\envFontDefault\hfill\Sbra{$V$ must be infindim.}}
}{\NOTE} {\tgsl that at least one of $U_1,U_2$ must be infindim.\;\; And at least one must be findim??}\par\quad
...
Beca $\BigPar{\Par{v-w}+u}\in U$ or $u-u\apostrophe\in U$ needs that $U$ is closd add.\parCom{\IndentB}
And beca $\Par{v-\hat{v}}+\Par{w-\hat{w}}\in U$ and $\lambda\Par{v-\hat{v}}\in U$ asm $U$ is a subsp.\vspace{4pt}\parCom{\IndentB}
...
%\ProblemBnoor[]{\hypertarget{3E4e7}{4E 7}}{
	%	Let $U=\Bra{\Par{x,y,z}\in\Rbb^3:2x+3y+5z=0}.$ Sup $A\subseteq\Rbb^3$.\TextB{}
	%	Then $A$ is a tslate of $U\Longleftrightarrow\exists\,c\in\Rbb,A=\Bra{\Par{x,y,z}\in\Rbb^3:2x+3y+5z=c}.$\TextB{\vspace{-6pt}}
	%}\SepLine

\ProblemBnoor{\hypertarget{3E4e8}{4E 8}}{
	\TextB{Sup $T\in\Lm{V,W},w\in\range T.$ Provet $\Bra{v\in V:Tv=w}=u+\null T.$}
}{\def\mK{\mathcal{K}_{\!u}}Let $\mK=\Bra{v\in V:Tv=w}.$ \Sbra{ Not a vecsp.\hspace{1pt}} \;Sup $u\in\mK.$ Then $u+\null T\subseteq\mK.$\parSol{}
...
\ProblemN{\hypertarget{3E7}{7}}{
	\TextN{Sup $v,x\in V,$ and $U,W$ are subsps of $V.$ Provet $v+U=x+W\Rightarrow U=W$.}
...
\ProblemN{\hypertarget{3E8}{8}}{
	\TextN{Sup $A$ is a nonempty subset of $V$.}
	\TextN{Provet $A$ is a tslate of some subsp of $V$ $\Longleftrightarrow\lambda v+\Par{1-\lambda}w\in A\,\,,\forall v,w\in A,\lambda\in\Fbb.$}
}\par\quad
(a) Sup $A=a+U.$ Then $\lambda\Par{a+u_1}+\Par{1-\lambda}\Par{a+u_2}=a+\BigPar{\lambda\Par{u_1-u_2}+u_2}\in A.$\vspace{2pt}\par\quad
(b) Sup $\lambda v+\Par{1-\lambda}w\in A,\forall v,w\in A,\lambda\in\Fbb.$ \;Sup \uline{$a\in A$} and let $A\apostrophe=\Bra{x-a:x\in A}.$\par\Hb\quad
...
(II) Beca $\lambda\Par{v-a}+\Par{1-\lambda}\Par{w-a}=\Sbra{\lambda v+\Par{1-\lambda}w}-a\in A\apostrophe.$\par\Hb\quad\HII
Let $\lambda=\frac{\;1\;}{2}$ here and use (I) above by $\lambda=2,$ we have $\Par{v-a}+\Par{w-a}\in A\apostrophe.$\vspace{4pt}\par\Hb\quad\HII
\Or Note that $v,a\in A\Rightarrow\lambda v+\Par{1-\lambda}a=2v-a\in A.$ Similarly $2w-a\in A.$\par\Hb\quad\HII
...
Thus $A\apostrophe=-a+A$ is a subsp of $V.$ Hence $a+A\apostrophe=a+\Bra{x-a:x\in A}=A$ is a tslate.\PfEnd
\SepLine\pagebreak

\ProblemN{\hypertarget{3E9}{9}}{
	\TextN{Sup $A=v+U$ and $B=x+W$ for some $v,x\in V$ and some subsps $U,W$ of $V$.}
	\TextN{Provet $A\cap B$ is either a tslate of some subsp of $V$ or is $\emptySet$.}
}$\forall v+u,x+w\in A\cap B\neq\emptySet,\lambda\in\Fbb,\lambda\Par{v+u}+\Par{1-\lambda}\Par{x+w}\in A\cap B.$ By Exe (8).\PfEnd\parSol{}
\Or Let $A=v+U,\;B=x+W.$ \;Sup $\alpha\in\Par{v+U}\cap\Par{x+W}\neq\emptySet.$\parSol{}
Then $\alpha-v\in U\Rightarrow \alpha+U=v+U=A,$ and $\alpha-x\in W\Rightarrow \alpha+W=x+W=B.$\parSol{}
We showt $A\cap B=\alpha+\Par{U\cap W}.$ Note that $\alpha+\Par{U\cap W}\subseteq A\cap B.$\parSol{}
...
\ProblemN{\hypertarget{3E10}{10}}{
	\TextNL{Provet the intersec of any collec of tslates of subsps is either a tslate of some subsps or $\emptySet.$}
}Sup $\Bra{A_{\alpha}}{_{\alpha\in\Gamma}}$ is a collec of tslates of subsps of $V$, where $\Gamma$ is an index set.\vspace{2pt}\parSol{}
...
\Or Let each $A_\alpha=w_\alpha+V_{\!\alpha}.$ Sup $x\in\bigcap_{\alpha\in\Gamma}\Par{w_\alpha+V_{\!\alpha}}\neq\emptySet.$\parSol{}
Then $x-w_\alpha\in V_{\!\alpha}\Longrightarrow x+V_{\!\alpha}=w_\alpha+V_{\!\alpha}=A_\alpha,$ for each $\alpha.$\parSol{}
We showt $\bigcap_{\alpha\in\Gamma}A_\alpha=\bigcap_{\alpha\in\Gamma}\Par{x+V_{\!\alpha}}=x+\bigcap_{\alpha\in\Gamma}V_{\!\alpha}.$\parSol{}
...
\ProblemN{\hypertarget{3E11}{11}}{
	\TextNL{Sup $A=\Bra{\lambda_1 v_1+\dots+\lambda_m v_m:\sum_{i=1}^m \lambda_i=1}$, where each $v_i\in V,\lambda_i\in\Fbb.$\vspace{2pt}}
	(a) \TextNL{Provet $A$ is a tslate of some subsp of $V$}
	(b) \TextNL{Provet if $B$ is a tslate of some subsp of $V$ and $\Bra{v_1,\dots,v_m}\subseteq B$, then $A\subseteq B$.}
	(c) \TextNL{Provet $A$ is a tslate of some subsp of $V$ of dim less than $m.$\vspace{2pt}}
...
(b) Sup $B=v+U,$ where $v\in V$ and $U$ is a subsp of $V.$ \uline{Let each $v_k=v+u_k\in B,\exists\,!\,u_k\in U.$}\par\vspace{2pt}\quad\Hb
...
\Or Let $v=\lambda_1 v_1+\dots+\lambda_m v_m\in A$. To showt $v\in B$, use induction on $m$ by $k$.\par\quad\Hb
...
(ii) $2\leqslant k<m.$ Asm $v=\lambda_1 v_1+\dots+\lambda_k v_k\in A\subseteq B.\;\Sbra{\forall\lambda_i$ shat $\sum_{i=1}^k\lambda_i=1}$\par\quad\Hb\Hii
...
(c) If $m=1,$ then let $A=v_1+\zeroSubs$ and we are done. \;Now sup $m\geqslant 2.$ Fix one $k\in\Bra{1,\dots,m}.$\par\quad\Hc
...
\BulletPointX\NoteFor{[3.88, 3.90, 3.91]}\;\;{Sup $W\in\mathcal{S}_{\!V} U.$ Then $V\XSlash U$ is iso to $W.$}\TextB{}
{Beca $\forall v\in V,\exists\,!\,u_v\in U,w_v\in W,v=u_v+w_v.$ \uline{Define $T\in\Lm{V}$ by $T\Par{v}=w_v.$}}\TextB{}
{Hence $\null T=U,\;\range T=W,\;\range T\oplus\null T=V.$}\TextB{}
{Then $\tilde{T}\in\Lm{V\XSlash \null T,V}$ is defined by $\tilde{T}\Par{v+U}=\tilde{T}\Par{w_v'+U}=Tw_v'=w_v.$ \Sbra{See {\TIPSN{1}} below}}\TextB{}
...
\BulletPointX\TipsN{1}\,\,\,{Sup $U$ is a subsp of $V.$ \uline{Define $S\in\Lm{V\XSlash U,V}$ by $S\Par{v+U}=v.$}}\TextB{}
{Then $\range S$ is the {\tgsl purest} in $\mathcal{S}_{\!V} U.$ Now $\null S=\zeroSubs,\;U\oplus\range S=V.$}\TextB{}
{\uline{Let $E=S\circ\pi.$} Beca $S$ is inje and $\pi$ is surj, $\null E=\null\pi=U,\;\range E=\range S.$}\TextB{}
...
{beca $e_2=e_1+\Par{e_2-e_1},$ now still, $\tilde{T}\BigPar{\Par{e_2-e_1}+U}=e_2-e_1=Te_2.$}\TextE{}
...
\ProblemN{\hypertarget{3E12}{12}}{
	\TextNL{Sup $U$ is a subsp of $V.$ Provet is $V$ is iso to $U\times\Par{V\XSlash U}$.}
}\par\quad
\!\Sbra[3pt]{{\tgsl Req $V\XSlash U$ Finite-dim}} \;Let $B_{V\XSlash U}=\BigPar{v_1+U,\dots,v_n+U}.$\par\quad
...
\ProblemBnoor{\hypertarget{3E4e14}{4E 14}}{
	\TextB{Sup $V=U\oplus W,\;B_W=\Par{w_1,\dots,w_m}.$ Provet $B_{V\XSlash U}=\BigPar{w_1+U,\dots,w_m+U}.$\vspace{2pt}}
...
\ProblemN{\hypertarget{3E13}{13}}{
	\TextNL{Provet $B_{V\XSlash U}=\BigPar{v_1+U,\dots,v_m+U},B_U=\Par{u_1,\dots,u_n}\Rightarrow B_V=\BigPar{v_1,\dots,v_m,u_1,\dots,u_n}.$\vspace{2pt}}
...
\Or Note that $B=\Par{v_1,\dots,v_m}$ is linely inde, and $\Sbra{\Span{v_1,\dots,v_m}+U}\subseteq V.$\parSol{}
...
Beca $\Dim\Sbra{\Span{v_1,\dots,v_m}\oplus U}=m+n=\dim V.$ \;Now by (2.B.8).\PfEnd
\SepLine\pagebreak

\BulletPointX\NoteForSmall{Problem (13) and (4E 14)}\;\;Let $U\oplus W=V.$ Define $S\Par{w+U}=w.$ \;\Sbra{ See also \TIPSN{1}. }\TextB{}
...
\ProblemB{
	\TipsN{2}\,\,\,\TextB{Sup $U,W$ are subsps of $V.$ Let $I=U\cap W.$ Provet $V=U+W\Longleftrightarrow V\XSlash I=U\XSlash I\oplus W\XSlash I.$}
}(a) Sup $U+W.$ Then $\forall x\in V\XSlash I,\exists\,v\in V,\Par{u_v,w_v}\in U\times W,x=v+I=\Par{u_v+w_v}+I.$\parSol{\Ha}
Note that $U\XSlash I,W\XSlash I\subseteq V\XSlash I.$ Thus $V\XSlash I=U\XSlash I+W\XSlash I.$\parSol{\Ha}
$\forall x\in\Par{U\XSlash I}\,${\Large$\cap$}$\,\Par{W\XSlash I},\exists\,u+I\in U\XSlash I,w+I\in W\XSlash I,x=u+I=w+I\Rightarrow u-w\in I=U\cap W$\parSol{\Ha}
$\Rightarrow\exists\,w'\in I,u=w+w'\in U\cap W\Rightarrow x=u+I=0+I.$ Thus $\Par{U\XSlash I}\,${\Large$\cap$}$\,\Par{W\XSlash I}=\zeroSubs.$\parSol{\vspace{2pt}}
(b) Sup $V\XSlash I=U\XSlash I\oplus W\XSlash I.$ Then $\forall v\in V,v+I=\Par{u+I}+\Par{w+I}$\parSol{\Hb}
$\Rightarrow v-u-w\in I=U\cap W\Rightarrow\exists\,x\in U\cap W,v=u+w+x\in U+W.$\PfEnd
\SepLine

\ProblemB{
	\TipsN{3}\,\,\,\TextB{Sup $I$ is a subsp of $U.$ Sup $U$ is a subsp of $V.$}
	\IndentTipsN{3}\TextB{Let $V=S_V I\oplus I=S_V U\oplus U.$ \;Let $U=S_U I\oplus I.$ \;Then $V=S_V U\oplus S_U I\oplus I.$\vspace{2pt}}
	\IndentTipsN{3}\TextB{Sup $S_V I=\Pure V\XSlash I,$ similar for $S_V U,S_U I.$ \;Provet $S_V I=S_V U\oplus S_U I.$}
}$\forall v_i\in S_V I,v_i=v_u+u,\exists\,!\,v_u\in S_V U,u\in U\Rightarrow\exists\,!\,u_i\in S_U I,i\in I,v_i=v_u+u_i+i.$\parSol{}
又 $v_i\in\Pure V\XSlash I.$ Hence $i=0,$ and $v_i\in S_V U\oplus S_U I.$ Now beca $S_V U,U\subseteq S_V I.$\PfEnd
\SepLine

\ProblemN{\hypertarget{3E15}{15}}{
	\TextNL{Sup $\varphi\in\Lm[\Par]{V,\Fbb}\nonzero.$ Provet $\dim V\XSlash \Par{\null\varphi}=1$.}
...
\ProblemN{\hypertarget{3E16}{16}}{
	\TextNL{Sup $\dim V\XSlash U=1$. Provet $\exists\,\varphi\in\Lm{V,\Fbb},\null\varphi=U.$}
}Sup $V_0\oplus U=V.$ Then $V_0$ is iso to $V\XSlash U.$ $\dim V_0=1.$\parSol{}
...
\ProblemN{\hypertarget{3E17}{17}}{
	\TextNL{Sup $V\XSlash U$ is findim, $W$ is a subsp of $V$.}
	(a) \TextNL{Showt if $V=U+W$, then $\dim W\geqslant\dim V\XSlash U$.}
	(b) \TextNL{Showt $\exists\,W\in\mathcal{S}_{\!V} U,\dim W=\dim V\XSlash U.$}
...
\ProblemN{\hypertarget{3E18}{18}}{
	\TextNL{Sup $T\in\Lm{V,W}$ and $U$ is a subsp of $V.$ Let $\pi:V\rightarrow V\XSlash U$ be the quotient map.}
	\TextNL{Provet $\exists\,S\in\Lm{V\XSlash U,W},T=S\circ\pi\Longleftrightarrow U\subseteq\null T$.}
}\par\quad
(a) Sup $\exists\,S\in\Lm{V\XSlash U,W},T=S\circ\pi.$ Then $U=\null\pi\subseteq\null\Par{S\circ\pi}=\null T.$\par\quad
(b) Sup $U=\null\pi\subseteq\null T.$ By ({3.B.24}), we are done. \;\Or Define $S:\Par{v+U}\mapsto Tv.$\par\Hb\quad
...
\ProblemN{\hypertarget{3E14}{14}}{
	\TextNL{Sup $U=\Bra{\Par{x_1,x_2,\cdots}\in\Fbb^{\infty}:x_k\neq 0\text{\;for\;only\;finily\;many}\;k}$.}
	(a) \TextNL{Showt $U$ is a subsp of $\Fbb^{\infty}$. {\large\envFontDefault\Sbra{Do it in your mind}} \;\; {\tgnr\large(b)} Provet $\Fbb^{\infty}\XSlash[-3pt]U$ is infindim.}
}For ease of notation, denote the $p^\text{th}$ term of $u=\Par{x_1,\cdots,x_p,\cdots}\in\Fbb^{\infty}$ by $u\Sbra{p}$.\vspace{2pt}\par\quad
For each $r\in\Nbp,$ let $\;e_r\Sbra{k}=\hMath[0pt]{l}{\left\{}{\right|}{1\,,\;\Par{k-1}\equiv 0\,\BigPar{\text{mod}\,r}\\0\,,\;\text{otherwise}}$ \;simply \,$e_r=\BigPar{1,\underbrace{0,\cdots,0}_{\SmallPar{r-1}},1,\underbrace{0,\cdots,0}_{\SmallPar{r-1}},1,\cdots}.$\vspace{6pt}\par\quad
...
Sup $u=\Par{x_1,\cdots,x_L,{0,\cdots}},$ where $L$ is the largest shat $u\Sbra{L}\neq 0.$\vspace{2pt}\par\quad
Let $s\in\Nbp$ be shat $h=s\cdot m!+1> L,$ \,and \,$e_1\Sbra{h}=\cdots=e_m\Sbra{h}=1.$\vspace{2pt}\par\quad
...
Thus $a_{p_{\tau\TinyPar{p}}}=a_p=0$ for all $p\in\Bra{1,\dots,m}\Rightarrow\Par{e_1,\dots,e_m}$ is linely inde in $\Fbb^{\infty}.$\vspace{2pt}\par\quad
So is $\Par{e_1+U,\dots,e_m+U}$ in $\Fbb^{\infty}\XSlash[-2.5pt]U.$ Beca $m$ is arb. By (2.A.14).\PfEnd\vspace{10pt}\quad
...
Sup $L$ is the largest shat $u\Sbra{L}\neq 0.$ And $l$ is shat $2^{ml}> L.$\vspace{6pt}\par\quad
...
\Or \Sbra[3pt]{{\tgsl Req $V$ Finite-dim}} \;By [3.106], $\dim U^0=\dim V-\dim U>0.$\parSol{}
...
\ProblemN{\hypertarget{3F25}{25}}{
	\TextNL{Sup $U$ is a subsp of $V$. Explain why $U=\Bra{v\in V:\varphi\Par{v}=0,\forall\varphi\in U^0}.$}
}Note that $U=\Bra{v\in V:v\in U}$ is a subsp. Now we show $\forall\varphi\in U^0,\varphi\Par{v}=0\Rightarrow v\in U.$\parSol{}
Asm $v\in V\Backslash U.$ Then let $\Span{v}\oplus U\oplus X=V.$ $\exists\,\psi\in V\apostrophe,\null\psi=U\oplus X.$\parSol{}
...
\ProblemN{\hypertarget{3F20}{20}}{
	\TextNL{Sup $U,W$ are nonempty subsets of $V$. Provet $U\subseteq W\Rightarrow W^0\subseteq U^0.$}
...
\ProblemN{\hypertarget{3F21}{21}}{
	\TextNL{Sup $U,W$ are subsps of $V$. Provet $W^0\subseteq U^0\Rightarrow U\subseteq W.$}
...
\ProblemN{\hypertarget{3F22}{22}}{
	\TextNL{Sup $U$ and $W$ are subsps of $V$. Provet $\BigPar{U+W}{^0}=U^0\cap W^0$.}
}\vspace{-14pt}\parSol{}
\!\!\!$\hText{$
...
\ProblemN{\hypertarget{3F23}{23}}{
	\TextNL{Sup $U$ and $W$ are subsps of $V$. Provet $\BigPar{U\cap W}{^0}=U^0+W^0$.}
}\vspace{2pt}\par\quad
\!\!\!$\hText{$
...
(b) \!\Sbra[3pt]{{\tgsl Only in Finite-dim; Req $U,W$ Subsps}} \;Using Exe (22).\par\vspace{2pt}\quad\Hb
...
for all $u\in\Pure V\XSlash\Span{u_v}$ and $i\in\Pure V\XSlash\Span{i_u}.$ \OR Define $\psi\in W^0,\gamma\in U^0,$ similarly.\par\quad
...
%Now we showt $X\cap Y=I.$ So that $\BigPar{U\cap W}{^0}=I^0=\BigPar{X\cap Y}{^0}=X^0+Y^0=U^0+W^0.$\parCom
...
\ProblemB{
	\TipsN{1}\,\,\,(a) \TextB{Provet $V=U\oplus W\Longleftrightarrow V\apostrophe=U^0\oplus W^0.$\vspace{1pt}}
	\IndentTipsN{1}(b) \TextB{Sup $U\oplus W=V.$ Provet $U^0=\Bra{\varphi\in V\apostrophe:\varphi=\varphi\circ\iota},$}
...
\ProblemN{\hypertarget{3F31}{31}}{
	\TextNL{Sup $U$ is a subsp of $V.$ \;Let $B_{U_V\upapostrophe}=\Par{\varphi_1,\dots,\varphi_n}.$ Showt the correspd $B_U$ exists.}
}Let each \uline{$\null\varphi_i\oplus\Span{u_i}=V$} with $\varphi_i\Par{u_i}=1.$\parSol{}
Now $a_1u_1+\dots+a_nu_n=0\Rightarrow$ \uline{Each $a_i=\varphi_i\Par{a_1u_1+\dots+a_nu_n}=0,$ by def of dual basis.}\PfEnd\vspace{2pt}
\Example \,\,\,Cannot extend $B_U$ freely. Let $B_V=\Par{e_1,e_2-e_1}.$ Let the correspd  $B_{V\apostrophe}=\Par{\varphi_1,\varphi_2}.$\par
\Blind{\Example \,\,\,}Let $U_V\upapostrophe=\Span{\varphi_1}.$ Then extend to $B_U=\Par{e_1}$ to $B_V'=\Par{e_1,e_2}.$ The correspd $B_{V\apostrophe}'\neq B_{V\apostrophe}.$
\SepLine
}

%\def\OrbikenII
{
\BulletPointX\TipsN{2}\,\,\,Sup $\varphi_1,\dots,\varphi_m\in V\apostrophe.$ Let $\null_I=\Par{\null\varphi_1}\cap\dots\cap\Par{\null\varphi_m}.$\TextB{}
\IndentTipsN{2}Sup $\Omega$ is a subsp of $V\apostrophe.$ Let $\null_C=\Bra{v\in V:\varphi\Par{v}=0,\forall\varphi\in\Omega}.$\TextB{}
\IndentTipsN{2}If $\Omega=\Span{\varphi_1,\dots,\varphi_m}.$ Then $\null_I=\null_C.$\TextB{}
\IndentTipsN{2}Beca $v\in\null_I\Longleftrightarrow$ each $\varphi_i\Par{v}=0\Longleftrightarrow\forall\varphi\in\Omega,\varphi_i\Par{v}=0\Longleftrightarrow v\in\null_C.$\TextB{}
\Comment \,\,\,If $\Omega$ is infindim. Then $\null_I=\bigcap_{\varphi\,\in\,\Omega}\null\varphi=\null_C.$
\SepLine

\ProblemB{
	\TipsN{3}\,\,\,\TextB{Let $\Omega=\Span{\varphi_1,\dots,\varphi_m}\subseteq V\apostrophe.$ Provet {\tgnr\large(a)} $\Omega=\BigPar{\null_I}{^0};\;$ {\tgnr\large(b)} $\Omega=\BigPar{\null_C}{^0}.$}
...
%For notational convienence of understanding, let $\null_C=\Bra{v\in V:\varphi\Par{v}=0,\forall\varphi\in\Omega}.$\par\quad
%又 $\null_C=\Bra{v\in V:\varphi\Par{v}=0,\forall\varphi\in\BigPar{\null_C}{^0}}.$\par\quad
%$\forall\varphi\in\Omega,\null_C\subseteq\null\varphi\Rightarrow\varphi\in\BigPar{\null_C}{^0}.$ Hence $\Omega\subseteq\BigPar{\null_C}{^0}.$\par\quad
%Asm $\Omega\subsetneq\BigPar{\null_C}{^0}.$ Sup $\psi\in\BigPar{\null_C}{^0}\Backslash\Omega.$\par\quad
%Let $\null\psi\oplus\Span{w}=V.$ Then sup $\varphi\in\Omega\nonzero\subseteq\BigPar{\null_C}{^0}.$\par\quad
%Let $\null\varphi\oplus\Span{v}=V.$ Then let $\Theta\oplus\null_C=\null\psi,\;\Delta\oplus\null_C=\null\varphi.$\par\quad
%Thus $V=\Theta\oplus\null_C\oplus\Span{w}=\Delta\oplus\null_C\oplus\Span{v}.$ Now we showt $\psi\in\Omega.$\par\quad
%\PfEnd\vspace{6pt}

\BulletPointX\NoteForSmall{Problem (26)}\;\;For every subsp $\Omega$ of $V\apostrophe,\;\exists\,!\,$subsp $U$ of $V$ shat $\Omega=U^0.$
\SepLine

\ProblemN{24}{
	\TextNL{Sup $V$ is findim and $U$ is a subsp of $V$.}
...
And let the correspd (I) $B_U=\Par{v_{m+1},\dots,v_n},$ \; (II) $B_W=\Par{v_1,\dots,v_m}.$\parSol{}
...
(I)
%Sup for each $\varphi_k,$ let $\mathcal{K}_k$ be shat $V=\mathcal{K}_k\oplus\null\varphi_k.$ By (3.B.29), this $\mathcal{K}_k$ can be $\Span{v_k}.$\parSol{\HI}
...
\ProblemB{
	\TextB{Sup $T\in\Lm{V,W},\varphi_k\in V\apostrophe,\psi_k\in W\apostrophe.$}
	\ProblemN[]{}{}
	...
	\ProblemN[]{\hypertarget{3F29}{29}}{
		\TextNL{Provet $\range T\apostrophe=\Span{\varphi_1,\dots,\varphi_m}\Longleftrightarrow\null T=\Par{\null\varphi_1}\cap\cdots\cap\Par{\null\varphi_m}.$}}
	\TextB{\vspace{-4pt}}
...
\Hc Sup $T\apostrophe\Par{x\varphi_1+y\varphi_2}=\Par{4x+7y}\varphi_1+\Par{5x+8y}\varphi_2+\Par{6x+9y}\varphi_3=0.$\TextNL{}
...
%	(a) \TextNL{Sup $\varphi\in\PoRi\apostrophe$ is defined by $\varphi\Par{p}=p\apostrophe\Par{4}$. Describe $T\apostrophe\Par{\varphi}\in\PoRi\apostrophe$.\vspace{4pt}}
...
%	(b) \TextNL{Sup $\varphi\in\PoRi\apostrophe$ is defined by $\varphi\Par{p}=\int_0^1 p\Par{x}\d x$. Evaluate $\BigPar{T\apostrophe\Par{\varphi}}\Par{x^3}$.\vspace{4pt}}
...
%We showt ${U^0}=\Span{\varphi_{m+1},\dots,\varphi_n}.$ So that $\dim U^0=n-m=\dim V-\dim U.$\par\quad
...
\ProblemN{\hypertarget{3F37}{37}}{
	\TextNL{Sup $U$ is a subsp of $V$ and $\pi$ is the quotient map. Thus $\pi\apostrophe\in\Lm[\BigPar]{\Par{V\XSlash U}\apostrophe,V\apostrophe}$.\vspace{2pt}}
	(a) \TextNL{Showt $\pi\apostrophe$ is inje\hspace{1pt}$:${\tgnr\large\envFontDefault\;\;Beca $\pi$ is surj. Use [3.108].}\vspace{2pt}}
	(b) \TextNL{Showt $\range \pi\apostrophe=U^0$\hspace{1pt}$:${\tgnr\large\envFontDefault\;\;By {\tgnr[3.109](b)}, $\range\pi\apostrophe=\BigPar{\null\pi}{^0}=U^0.$}\vspace{2pt}}
...
\ProblemB{
	\TextB{Sup $U$ is a subsp of $V.$ Provet $\Par{V\XSlash U}\apostrophe$ is iso to $U^0.$ \hfill\Sbra[3pt]{{\large\tgsc Another proof of \tgbfx[3.106]}}}
}\par\quad
...
We showt $\xi$ is inje and surj.\par\quad
...
{
\ProblemB{
	\TextB{Sup $V=U\oplus W$. Define $\iota:V\rightarrow U$ by $\iota\Par{u+w}=u.$ Thus $\iota\apostrophe\in\Lm{U\apostrophe,V\apostrophe}.$\vspace{2pt}}
	...
	(b) \TextB{Provet $\range\iota\apostrophe=W_V^0$\hspace{1pt}$:${\tgnr\large\envFontDefault\;\;$\range\iota\apostrophe=\BigPar{\null\iota}{^0_V}=W_V^0.$}\vspace{2pt}}
	(c) \TextB{Provet $\tilde{\iota\apostrophe}$ is an iso from $U\apostrophe\XSlash{\envFontDefault\zeroSubs}$ onto $W^0$\hspace{1pt}$:${\tgnr\large\envFontDefault\;\;By (a), (b) and [3.91](d).}}
}\par\quad
...
Sup $\varphi\in W^0.$ Beca $\null\iota=W\subseteq\null\varphi.$ By \Sbra{3.B \TIPSN{3}}, $\varphi=\varphi\circ\iota=\iota\apostrophe\Par{\varphi}.$\PfEnd
\SepLine

\ProblemN{\hypertarget{3F36}{36}}{
	\TextNL{Sup $U$ is a subsp of $V$. Define $i:U\rightarrow V$  by $i\Par{u}=u$. Thus $i\apostrophe\in\Lm{V\apostrophe,U\apostrophe}.$\vspace{2pt}}
	...
	(b) \TextNL{Provet $\range i\apostrophe=U\apostrophe$\hspace{1pt}$:${\tgnr\large\envFontDefault\;\;$\range i\apostrophe=\Par{\null i}{_U^0}={\zeroSubs}{_U^0}=U\apostrophe$.}\vspace{2pt}}
	(c) \TextNL{Provet $\tilde{i\apostrophe}$ is an iso from $V\apostrophe\XSlash U^0$ onto $U\apostrophe$\hspace{1pt}$:${\tgnr\large\envFontDefault\;\;By (a), (b) and [3.91](d).}\vspace{2pt}}
}\par\quad
...
(b) Sup $\psi\in U\apostrophe.$ By (3.A.11), $\exists\,\varphi\in V\apostrophe,\varphi\mmid_U=\psi.$ Then $i\apostrophe\Par{\varphi}=\psi.$\PfEnd
\SepLine

\ProblemB{
	\TextB{Sup $T\in\Lm{V,W}.$ Provet $\range T\apostrophe=\BigPar{\null T}{^0}.$\hfill\Sbra[3pt]{{\tgsc\large Another proof of \tgnr\tgbfx[3.109](b)}}}
}\par\quad
Sup $\varPhi\in\BigPar{\null T}{^0}.$ Beca by (3.B.12), $T\mmid_U:U\rightarrow\range T$ is an iso; $V=U\oplus\null T.$\par\quad
...
{
\ProblemB[]{
	\hypertarget{3F4e17}{}\TextB{Sup $T\in\Lm{V,W}.$ {\large Using [3.108], [3.110].}\vspace{4pt}}
...
\ProblemN{\hypertarget{3F15}{15}}{
	\TextNL{Sup $T\in\Lm{V,W}$. Provet $T\apostrophe=0\Longleftrightarrow T=0.$}
}\par\quad
Sup $T=0.$ Then $\forall\varphi\in W\apostrophe,T\apostrophe\Par{\varphi}=\varphi\circ T=0.$ Hence $T\apostrophe=0.$\par\quad
Sup $T\apostrophe=0.$ Then $\null T\apostrophe=W\apostrophe=\Par{\range T}{^0},$  by [3.107](a).\par\quad
\!\Sbra[3pt]{{\tgsl$W$ can be infindim}} \;By Exe (25),\par\qquad
...
Now we provet if $\forall\varphi\in W\apostrophe,\varphi\Par{w}=0,$ then $w=0.$ So that $\range T=\zeroSubs$ and we are done.\par\quad
Asm $w\neq 0.$ Then let $U$ be shat $W=U\oplus\Span{w}.$\par\quad
Define $\psi\in W\apostrophe$ by $\psi\Par{u+\lambda w}=\lambda.$ So that $\psi\Par{w}=1\neq 0.$\PfEnd\vspace{6pt}\quad
\Or \Sbra[3pt]{{\tgsl Only if $W$ is findim}} \;By [3.106], $\dim \range T=\dim W-\Dim\Par{\range T}{^0}=0.$\PfEnd
\SepLine
...
\ProblemN{\hypertarget{3F16}{16}}{
	\TextNL{Sup $V,W$ are findim. Define $\Gamma$ by $\Gamma\Par{T}=T\apostrophe$ for any $T\in\Lm{V,W}$.}
	\TextNL{Provet $\Gamma$ is an iso of $\Lm{V,W}$ onto $\Lm{W\apostrophe,V\apostrophe}$.}
}By [3.101], $\Gamma$ is linear.\par\quad
Sup $\Gamma\Par{T}=T\apostrophe=0$. By Exe (15), $T=0$. Thus $\Gamma$ is inje.\par\quad
Beca $V,W$ are findim. $\dim\Lm{V,W}=\dim\Lm{W\apostrophe,V\apostrophe}$. Now $\Gamma$ inje $\Rightarrow$ inv.\PfEnd\vspace{4pt}
%\Or [{\tgsl Compatible with the case that $W$ is infindim and $V$ is findim}]\par\quad
%Sup $\mathcal{T}\in\Lm{W\apostrophe,V\apostrophe}.$ Let $B_{\range\mathcal{T}}=\Par{\varphi_1,\dots,\varphi_p}.$ Let $\psi_k$ be shat $\mathcal{T}\Par{\psi_k}=\varphi_k$ for each $\varphi_k.$\par\quad
%Let $B_{V\apostrophe}=\Par{\varphi_1,,\dots,\varphi_p,\dots,\varphi_n}.$ By Exe (31), let the correspd $B_V=\Par{v_1,\dots,v_p,\dots,v_n}.$\par\quad
...
%\Or [{\tgsl Compatible with the case that $V$ is infindim and $W$ is findim}]\par\quad
%Sup $\mathcal{T}\in\Lm{W\apostrophe,V\apostrophe}.$ Let $B_{\null\mathcal{T}}=\Par{\varepsilon_1,\dots,\varepsilon_p}.$\par\quad
...
\Comment \,\,\,Let $X=\Bra{T\in\Lm{V,W}:\range T\text{ is findim}}.$\parCom
Let $Y=\Bra{\mathcal{T}\in\Lm{W\apostrophe,V\apostrophe}:\range\mathcal{T}\text{ is findim}}.$\parCom
Then $\Gamma\mmid_X$ is an iso of $X$ onto $Y,$ even if $V$ and $W$ are infindim.\par\quad
{\tgsl The inje of $\Gamma\mmid_X$ is equiv to the inje of $\Gamma,$ as shown before.}\par\quad
{\tgsl Now we showt $\Gamma\mmid_X$ is surj without the cond that $V$ or $W$ is findim.}\par\quad
Sup $\mathcal{T}\in Y.$ Let $B_{\range\mathcal{T}}=\Par{\varphi_1,\dots,\varphi_m},$ with the correspd $\Par{v_1,\dots,v_m}.$ Let $\varphi_k=\mathcal{T}\Par{\psi_k}.$\par\quad
Let $\mathcal{K}$ be shat $W\apostrophe=\mathcal{K}\oplus\null\mathcal{T}.$ Let $B_{\mathcal{K}}=\Par{\psi_1,\dots,\psi_m},$ with the correspd $\Par{w_1,\dots,w_m}.$\par\quad
Define $T\in\Lm{V,W}$ by $Tv_k=w_k,Tu=0;\;k\in\Bra{1,\dots,m},u\in U.$\par\quad
%Where $U$ is shat $V=\Span{v_1,\dots,v_m}\oplus U.$\par\quad
...
\ProblemN[]{\hypertarget{3F5}{5}}{
	\TextN{Provet $\Par{V_{\!1}\times\dots\times V_{\!m}}\apostrophe$ and $V\apostrophe_1 \times\dots\times V\apostrophe_m$ are iso.\hfill\Sbra[3pt]{{\large\tgnr Using notations in (3.E.2).}}}\vspace{4pt}
}
$\MathRightBrace{l}{$
...
\ProblemBnoor[]{\hypertarget{3F4e8}{4E 8}}{
	\TextB{Sup $B_V=\Par{v_1,\dots,v_{n}},\,B_{V\apostrophe}=\Par{\varphi_1,\dots,\varphi_{n}}$.}
...
\ProblemN{\hypertarget{3F4e24}{}\hypertarget{3F6}{6}}{
	...
	(a) \TextN{Showt $\Span{v_1,\dots,v_m}=V\,\Longleftrightarrow \Gamma$ is inje.}
	(b) \TextN{Showt $\Par{v_1,\dots,v_m}$ is linely inde $\Longleftrightarrow \Gamma$ is surj.}
}\par\quad
...
(b) Sup $\Gamma$ is surj. Then let $\Gamma\Par{\varphi_i}=e_i$ for each $i$, where $\Par{e_1,\dots,e_m}$ is the std basis of $\Fbb^m$.\par\quad\Hb
...
Sup $\Par{v_1,\dots,v_m}$ is linely inde. Let $U=\Span{\varphi_1,\dots,\varphi_m},B_{U\apostrophe}=\Par{\varphi_1,\dots,\varphi_m}.$\par\quad\Hb
Thus $\forall\Par{a_1,\dots,a_m}\in\Fbb^m,\exists\,!\,\varphi=a_1\varphi_1+\dots+a_m\varphi_m.$\par\quad\Hb
Let $W$ be shat $V=U\oplus W.$ Now $\forall v\in V,\exists\,!\,u_v\in U,w_v\in W,v=u_v+w_v.$\par\quad\Hb
...
\ProblemBnoor{\hypertarget{3F4e25}{4E 25}}{
	...
	(c) \TextB{Showt $\Span{\varphi_1,\dots,\varphi_m}=V\apostrophe\,\Longleftrightarrow \Gamma$ is inje.}
	(d) \TextB{Showt $\Par{\varphi_1,\dots,\varphi_m}$ is linely inde $\Longleftrightarrow \Gamma$ is surj.}
}\par\quad
...
(d) Sup $\Par{\varphi_1,\dots,\varphi_m}$ is linely inde. Then by Exe (31), $\Par{v_1,\dots,v_m}$ is linely inde.\par\quad\Hd
...
Sup $\Gamma$ is surj. Let $\Par{e_1,\dots,e_m}$ be the std basis of $\Fbb^m$.\par\quad\Hd
Sup $v_i\in V$ shat $\Gamma\Par{v_i}=\Par{\varphi_1\Par{v_i},\dots,\varphi_m\Par{v_i}}=e_i,$ for each $i.$\par\quad\Hd
...
\Or Similar to Exe (6), we get $\Par{e_1,\dots,e_m},\Par{\psi_1,\dots,\psi_m}$ and the iso $\Psi.$\par\quad
...
\ProblemN{\hypertarget{3F35}{35}}{
	\TextNL{Provet $\BigPar{\PoFi}\apostrophe$ is iso to $\Fbb^{\infty}.$}
}\par\quad
...
{Surj: $\forall \BigPar{a_k}{_{k=1}^\infty}\in\Fbb^\infty,$ let $\psi$ be shat $\forall k,\psi\BigPar{z^k}=a_k$ \Sbra{ by [3.5] } and thus $\theta\Par{\psi}=\BigPar{a_k}{_{k=1}^\infty}.$}\PfEnd\vspace{6pt}
...
\ProblemN{\hypertarget{3F7}{7}}{
	\TextN{Showt the dual basis of $\Par{1, x,\dots,x^m}$ of $\PoR{m}$ is $\Par{\varphi_0,\varphi_1,\dots,\varphi_m}$, where {\large\envFontDefault $\varphi_k\Par{p}=\Frac{p^{\SmallPar{k}}\Par{0}}{k!}$.}\vspace{2pt}}
...
\Or \;Beca $\forall j,k\in\Bra{1,\dots,m}$ shat $j\neq k,$ 
...
\Example \,\,\,\hypertarget{3F8}{}{\tgnr\large\envFontDefault Sup $m\in\Nbp$. By [2.C.10], $B$ $=\BigPar{1,x-5,\dots,\Par{x-5}{^m}}$ is a basis of $\PoR{m}$.}\par
...
\ProblemN{\hypertarget{3F34}{34}}{
	...
	(a) \TextNL{Showt $\Lambda$ is a linear map from $V$ to $V\apostrophe\apostrophe$.\vspace{2pt}}
	(b) \TextNL{Showt if $T\in\Lm{V}$, then $T\apostrophe\apostrophe\circ\Lambda=\Lambda\circ T$, where $T\apostrophe\apostrophe=\Par{T\apostrophe}\apostrophe$.\vspace{2pt}}
	(c) \TextNL{Showt if $V$ is findim, then {\tgsc $\Lambda$ is an iso from V onto V\hspace{1pt}$\apostrophe\apostrophe$}.\vspace{2pt}}
	\TextNL{\normalsize Sup $V$ is findim. Then $V$ and $V\apostrophe$ are iso, and finding an iso from $V$ onto $V\apostrophe$ generally requires choosing\vspace{-2pt}}
...
(c) Sup $\Lambda v=0$. Then $\forall\varphi\in V\apostrophe,\Par{\Lambda v}\Par{\varphi}=\varphi\Par{v}=0\Rightarrow v=0$. Thus $\Lambda$ is inje.\par\quad\Hc
又 Beca $V$ is findim. $\dim V=\dim V\apostrophe=\dim V\apostrophe\apostrophe$. Hence $\Lambda$ is an iso.$\,$\PfEnd
\SepLine
}
