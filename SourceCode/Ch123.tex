...
\BulletPointX\NoteForSmall{[3.69]}\;\;Suppose $V,W$ are finite-dim, $T\in\Lm{V,W}.$\TextB{}
...
\!\!\!$\MathRightMid{l}{Tv=0\Rightarrow v=R\Par{ST}v=RS\Par{Tv}=0.\\\forall v\in V,v=\Par{ST}Rv=S\Par{TRv}\in\range S.}\hMath{l}{\left.}{\right.}{$
$T$ is inje, $S$ is surj.$\\$
...
\Blind{\NoteForSmall{[3.65]}}\;\;Note that if $v=0,$ then $\Par{v}=\Par{\,}=B_{\Span[\SmallPar]{\,}}\,;\;$ similar for $Tv=0.$\vspace{-2pt}
\SepLine
...
\BulletPointX\Tips \,\,\,When using $\Mneg,$ you must first declare $\mathcal{M}:\Lm{V,W}\mapsto\Fbb^{m,n}$ or $\mathcal{M}:v\mapsto\Fbb^{n,1}.$ So that\TextB{}
\IndentTips{}$\Mneg\Par{AC}=\Mneg\Par{A}\Mneg\Par{C},\Mneg\Par{Ax}=\Mneg\Par{A}\Mneg\Par{x}.$\TextB{}
\IndentTips{}Where $\Mneg\Par{A},\Mneg\Par{x}$ are well-defined.
\SepLine
