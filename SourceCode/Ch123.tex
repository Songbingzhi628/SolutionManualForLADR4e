...
(a) $P\Rightarrow Q:$ \;Note that each $v_k\in V_{\!\Cbb}.$ \;\; $Q\Rightarrow P:$ \;If $\lambda_k\in\Rbb$ with $\lambda_1v_1+\dots+\lambda_nv_n=0,$ then each $\mathfrak{Re}\,\lambda_k=\lambda_k=0.$\par\quad
\Blind{(a)} ${}^\neg P\Rightarrow{}^\neg Q:$ \;$\exists\,v_j=a_{j-1}v_{j-1}+\dots+a_1v_1\in V_{\!\Cbb}.$\par\quad
\Blind{(a)} ${}^\neg Q\Rightarrow{}^\neg P:$ \;$\exists\,v_j=\lambda_{j-1}v_{j-1}+\dots+\lambda_1v_1\Rightarrow v_j=\Par{\mathfrak{Re}\,\lambda_{j-1}}v_{j-1}+\dots+\Par{\mathfrak{Re}\,\lambda_1}v_1\in V.$\par\vspace{3pt}\quad
...
\def\fT{\mathcal{T}}\def\fC{\mathcal{C}}\def\fR{\mathcal{R}}\def\fP{\mathcal{P}}
...
\ProblemBnoor{\hypertarget{3C4e17}{4E 17}, \OR 3.F.32}[\Sbra]{
	...
	\TextB{{\tgnr\large(a)} $T$ is inje$;$ \; {\tgnr\large(b)} $\BigPar{A_{\cdot,1},\cdots,A_{\cdot,n}}$ is a basis of \,$\Fbb^{n,1};$ \; {\tgnr\large(c)} $\BigPar{A_{1,\cdot},\cdots,A_{n,\cdot}}$ is a basis of \,$\Fbb^{1,n}.$\vspace{3pt}}
}$T$ is inje $\Longleftrightarrow\dim V=\dim\range T=n$\parSol{}
\Blind{$T$ is inje} $\Longleftrightarrow B_V=\Par{Tu_1,\dots,Tu_n}\overset{\Delta}{\Longleftrightarrow}B_{\Fbb^{n,1}}=\BigPar{A_{\cdot,1},\cdots,A_{\cdot,n}}\xLongleftrightarrow[\text{[2.42]}]{\rank} B_{\Fbb^{1,n}}=\BigPar{A_{1,\cdot},\cdots,A_{n,\cdot}}.$\parSol{\vspace{2pt}}
\Comment \,\,\,$T\apostrophe$ is inv $\Longleftrightarrow B_{V\apostrophe}=\BigPar{T\apostrophe\Par{\psi_1},\dots,T\apostrophe\Par{\psi_n}}\overset{\Delta}{\Longleftrightarrow}B_{\Fbb^{n,1}}=\envFontLarge\BigPar{\envFontDefault\Par{A^t}{_{\cdot,1}},\cdots,\Par{A^t}{_{\cdot,n}}\envFontLarge}.$\vspace{2pt}\parSol{\IndentComment}
\Or Because each $A_{j,\cdot}=\fT\fC_j{\fT\Par{A}}.$ By [2.39], $B_{\Fbb^{1,n}}=\BigPar{A_{1,\cdot},\cdots,A_{n,\cdot}}.$\PfEnd\vspace{8pt}\quad
Now we show $\Delta:$ [$P$] $B_V=\Par{Tu_1,\dots,Tu_n}\Longleftrightarrow B_{\Fbb^{n,1}}=\BigPar{A_{\cdot,1},\cdots,A_{\cdot,n}}.$ [$Q$]\par\quad
Note that $\mathcal{M}\!:\,Tu_k\rightarrow A_{\cdot,k}$ is an iso of $V$ onto $\Fbb^{1,n}.$\PfEnd\vspace{2pt}\quad
\Or Using {\TIPS} below. We show [$P$] $\Par{Tu_1,\dots,Tu_n}$ linely inde $\Longleftrightarrow\BigPar{A_{\cdot,1},\dots,A_{\cdot,n}}$ linely inde. [$Q$]\par\quad
...
\Blind{$Q\Rightarrow P:$\,\;}Which is equi to $b_1A_{\cdot,1}+\dots+b_nA_{\cdot,n}=0.$ \;Thus each $b_k=0\Rightarrow\null T=\zeroSubs.$\PfEnd\vspace{4pt}
\BulletPointX\Tips \,\,\,$b_1Tu_1+\dots+b_nTu_n=b_1\BigPar{A_{1,1}v_1+\dots+A_{n,1}v_n}+\dots+b_n\BigPar{A_{1,n}v_1+\dots+A_{n,n}v_n}$\TextB{\vspace{2pt}}
\IndentTips{}$\Blind{b_1Tu_1+\dots+b_nTu_n}=\BigPar{b_1A_{1,1}+\dots+b_nA_{1,n}}v_1+\dots+\BigPar{b_1A_{n,1}+\dots+b_nA_{n,n}}v_n.$\TextB{\vspace{4pt}}
\IndentTips{}And \;$b_1A_{\cdot,1}+\dots+b_nA_{\cdot,n}=b_1{\normalsize\begin{pmatrix}A_{1,1}\\[-4pt]\vdots\\[-4pt]A_{n,1}\end{pmatrix}}+\dots+b_n{\normalsize\begin{pmatrix}A_{1,n}\\[-4pt]\vdots\\[-4pt]A_{n,n}\end{pmatrix}}{}={}\normalsize\begin{pmatrix}
b_1 A_{1,1}+\dots+b_nA_{1,n}\\[-4pt]\vdots\\[-4pt]
b_1A_{n,1}+\dots+b_nA_{n,n}\end{pmatrix}$.\vspace{10pt}
\SepLine
