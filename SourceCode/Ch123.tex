...
\BulletPointX\NoteForSmall{[3.79, 3.83]}\TextB{}
...
Thus $S$ is well-defined.\PfEnd\vspace{2pt}
\hypertarget{3E20}{}\ACoro $\Gamma:$ {\FontSmall$\Lm{V,W}\rightarrow\Lm{V\XSlash U,W}$} with $S\mapsto S\circ\pi$ is inje, $\range\Gamma=\Bra{T\in\Lm{V,W}:U\subseteq\null T}.$\par\vspace{2pt}
...
Becs $\range S\mmid_{W\XSlash X\,\cap\,U_X\XSlash X}\subseteq W,$ and $U_X=\range S\Rightarrow U_X\subseteq\range S+W.$ \;Well-defined. Inje.\par\quad
For $u_x\in U_X,$ if $u_x+W=0,$ then $u_x\in U_X\cap W\Rightarrow u_x+X\in\Par{U_X\cap W}\XSlash X\supseteq\range\xi\mmid_{\null\LmQxx{S}[0][\!W]}.$ \;Surj.\PfEnd
\SepLine
...
$u\Sbra{2^{ml}+2^k}=0=\XPar{\sum_{r=1}^m a_re_r}\Sbra{2^k}=a_1+\dots+a_k.$ \,Thus each $a_k=0.$ Similr.\PfEnd
\SepLine
\ChEnd
