...
\uline{Define $E=\Par{S\mmid_U}{^{-1}}T\in\Lm{V,U}.$} Then write $E\in\Lm{V}.$\PfEnd\vspace{0pt}\quad
\Comment \,\,\,Let $U_1=U.$ Let $U_2\oplus\null T=V.$ Then $T\mmid_{U_2}:U_2\rightarrow\range T$ is an iso. And $E\mmid_{U_2}:U_2\rightarrow U_1.$\parCom\quad
Note that $T\mmid_{U_2}:U_2\rightarrow\range S$ is merely inje if $\range T\subsetneq\range S.$\parCom\quad
So is $E\mmid_{U_2}=\Par{S\mmid_{U_1}}{^{-1}}\Par{T\mmid_{U_2}}.$ Thus $E\mmid_{U_2}\in\Lm{U_2,V}$ cannot be extended to an iso.\par\vspace{4pt}\quad%TODO:Explain more.
\Corollary \,\,\,If $\range T=\range S.$ We show that $U_2,U_1$ are iso.\parCor\quad
$E\mmid_{U_2}\Par{v}=0\Rightarrow Tv=0\Rightarrow v=0.$ Hence $E\mmid_{U_2}$ is inje.\parCor\quad
$\forall u\in U_1,Su\in\range T\Rightarrow\exists\,v\in U_2,T\Par{v}=Su\Rightarrow\Par{S\mmid_{U_1}}{^{-1}}\Par{Tv}=u.$ Hence $E\mmid_{U_2}$ is surj.\parCor\quad
Thus $E\mmid_{U_2}:U_2\rightarrow U_1$ is an iso. Now by \Sbra{3.D \TIPS}, $\null T,\null S$ are iso.\parCor\quad
So that we can extend inje $E\mmid_{U_2}\in\Lm{U_2,V}$ to inv $E\in\Lm{V}.$\par\vspace{6pt}\quad
\Or \Sbra[3pt]{{\tgsl Req $\range T$ Finite-dim}} \;Let $B_{\range T}=\Par{Tv_1,\dots,Tv_n}.$ Then \uline{$V=\Span{v_1,\dots,v_n}\oplus\null T.$}\par\quad
\uline{Let $S\Par{u_i}=Tv_i$ for each $Tv_i.$} \;Define $E$ by $Ev_i=u_i,Ex=0$ for all $x\in\null T$ and each $v_i.$\par\vspace{2pt}\quad
...
Now $E\mmid_X$ is inje, but cannot be freely extend to inv $E\in\Lm{V}.$\par\vspace{4pt}\quad%TODO:Explain.
\Corollary \,\,\,\Sbra[3pt]{{\tgsl Req $V$ Finite-dim}} \;If $\range T=\range S,$ then $\dim\null T=\dim\null S=p.$\parCor\quad
Redefine $E$ by $Ev_i=u_i,\;Ex_j=y_j$ for each $v_i$ and $x_j.$ Then $E\in\Lm{V}$ is inv.\par\vspace{4pt}\quad
...
\BulletPointX\NoteFor{[3.88, 3.90, 3.91]}\;\;{Suppose $W\in\mathcal{S}_V U.$ Then $V\XSlash U$ and $W$ are iso.}\TextB{}
...
}\Sbra[3pt]{{\tgsl Req $V\XSlash U$ Finite-dim}} \;Let $B_{V\XSlash U}=\Par{v_1+U,\dots,v_n+U}.$\parSol{}
...
\Blind{Note that} $\Rightarrow \forall v\in V,\exists\,!\,a_i,b_j\in\Fbb,v=\sum_{i=1}^m a_i v_i+\sum_{j=1}^m b_j u_j.$\PfEnd\parSol{\vspace{4pt}}
\Or Because $\Par{v_1,\dots,v_m}$ is linely inde.\parSol{}
By Problem (12), $\Dim\BigPar{U\times\Par{V\XSlash U}}=m+n=\dim V=\dim\Span{v_1,\dots,v_m}\oplus\dim U.$\parSol{}
$v\in\Span{v_1,\dots,v_m}\cap U\Longleftrightarrow v+U=\sum_{i=1}^ma_i\Par{v_i+U}=0+U\Longleftrightarrow a_1=\dots=a_m=0\Longleftrightarrow v=0.$\parSol{}
Thus $\Span{v_1,\dots,v_m}\oplus U=V.$ By ({2.B.8}).\PfEnd
\SepLine
...
}Suppose $V_0\oplus U=V.$ Then $V_0$ and $V\XSlash U$ are iso. $\dim V_0=1.$\parSol{}
...
Then $V\XSlash U\subseteq\Span{w_1+U,\dots,w_n+U}.$ Hence $\dim V\XSlash U\leqslant\dim\Span{w_1+U,\dots,w_n+U}.$\par\vspace{4pt}\quad
(b) Reduce $\Par{w_1+U,\dots,w_n+U}$ to $B_{V\XSlash U}=\Par{w_1+U,\dots,w_m+U},$ and let $W=\Span{w_1,\dots,w_m}.$\PfEnd\vspace{2pt}\quad\Hb
\Or Let $B_{V\XSlash U}=\Par{v_1+U,\dots,v_m+U}$ and define $\tilde{T}\in\Lm{V\XSlash U,V}$ by $\tilde{T}\Par{v_k+U}=v_k.$\par\quad\Hb
...
\Or Define $S\in\Lm{V\XSlash U,W}$ by $S\Par{v+U}=Tv.$ Then $S\circ\pi=T.$ Now we show that $S$ is {\tgsl well-defined}.\par\vspace{0pt}\quad\Ha
$v_1+U=v_2+U\Longleftrightarrow\Par{v_1-v_2}\in U\Longleftrightarrow S\BigPar{\Par{v_1-v_2}+U}=T\Par{v_1-v_2}=0\Longleftrightarrow Tv_1=Tv_2.$\par\vspace{4pt}\quad
