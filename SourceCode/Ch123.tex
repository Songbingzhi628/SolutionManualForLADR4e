...
Asum $\forall v\in V,\Par{v,Tv}$ is linely dep, then by (2.A.2.(b)), $\exists\,\lambda_v\in\Fbb,Tv=\lambda_v v.$\par\quad
...
\IndentTipsN{2}Supp $\Omega$ is a subsp of $V\apostrophe.$ Denote $\Bra{v\in V:\varphi\Par{v}=0,\forall\varphi\in\Omega}$ by $C^0\,\Omega.$\TextB{\vspace{2pt}}
(1) If $\Omega$ is infinide. Then by def, $\bigcap_{\in\,\Omega}\null\varphi=C^0\,\Omega.$\TextB{\vspace{1pt}}
(2) If $\Omega=\Span{\varphi_1,\dots,\varphi_m}.$ Then $v\in\bigcap_1^m\null\varphi_I\Longleftrightarrow$ each $\varphi_k\Par{v}=0$\TextB{}
\Blind{(2) If $\Omega=\Span{\varphi_1,\dots,\varphi_m}.$ Then $v\in\bigcap_1^m\null\varphi_I$}${}\Longleftrightarrow\forall\varphi=\sum_{i=1}^na_i\varphi_i\in\Omega,\varphi\Par{v}=0\Longleftrightarrow v\in C^0\,\Omega.$
\SepLine
...
}Let $B_U=\Par{u_1,\dots,u_m},B_V=\Par{u_1,\dots,u_m,v_1,\dots,v_n},B_{V\apostrophe}=\Par{\psi_1,\dots,\psi_m,\varphi_1,\dots,\varphi_n}.$\parSol{}
...
$\range T=\Span{w_1,\dots,w_m}\Longleftrightarrow\range T={\null\psi_{m+1}}\cap\cdots\cap{\null\psi_n}\Longleftrightarrow\null T\apostrophe=\Span{\psi_{m+1},\dots,\psi_n}.$\par\quad
...
}Supp $T=0.$ Then $\forall\varphi\in W\apostrophe,T\apostrophe\Par{\varphi}=\varphi\circ T=0\Rightarrow T\apostrophe=0.$\parSol{}
...
%}By [3.101], $\Gamma$ is liney. By Exe (15), $\Gamma$ is inje. \,Now becs $\dim\Lm{V,W}=\dim\Lm{W\apostrophe,V\apostrophe}.$\PfEnd
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\ANote Let $X=\Bra{T\in\Lm{V,W}:\range T\text{ is finide}},\;Y=\Bra{\mathcal{T}\in\Lm{W\apostrophe,V\apostrophe}:\range\mathcal{T}\text{ is finide}}.$\parNot
%We show $\Gamma\mmid_X$ is iso of $X$ onto $Y$ \Sbra{mainly surj, by Exe (15)} without the hypo that $V$ or $W$ finide.\par\quad
...
\ProblemN{\hypertarget{3F9}{9}}{
	...
	\ANote {\tgsl For other $B_V'=\Par{u_1,\dots,u_n},B_{V\apostrophe}'=\Par{\rho_1,\dots,\rho_n},\forall\psi\in V\apostrophe,\psi=\psi\Par{u_1}\rho_1+\dots+\psi\Par{u_n}\rho_n.$}\TextB{}\vspace{2pt}
}$\psi\Par{v}=\psi\XPar{\sum_{i=1}^n a_i v_i}=\sum_{i=1}^n a_i\psi\Par{v_i}=\sum_{i=1}^n\psi\Par{v_{i}}\varphi_i\Par{v}=\Sbra{\psi\Par{v_1}\varphi_1+\dots+\psi\Par{v_n}\varphi_n}\Par{v}.$\PfEnd
\SepLine\pagebreak
...
\vfill\ChDecl{}{\Largebfx{Exes about Number Theory and Sequences in Chapter 3}}
...
\vfill\ChDecl{}{\Largebfx{Exes about Polys in Chapter 3}}

\vspace{4pt}

\ProblemN{\hypertarget{1C9}{1}}{
...
\ProblemN{\hypertarget{1C24}{2}}{
	\TextN{Let $V_{\!E}=\Bra{\,f\in\Rbb^{\Rbb}:\text{f is even}},V_{\!O}=\Bra{\,f\in\Rbb^{\Rbb}:\text{f is odd}}.$ Show $V_{\!E}\oplus V_{\!O}=\Rbb^{\Rbb}.$\vspace{4pt}}
...
\ProblemN{\hypertarget{2A16}{3}}{
	\TextN{Prove the vecsp of all continuous functions in $\Rbb^{[0,1]}$ is infinide.}
...
\ProblemN{\hypertarget{2A17}{4}}{
	\TextN{Supp $p_0,p_1,\dots,p_m\in\PoF{m}$ suth $p_k\Par{2}=0$ for each $k\in\Bra{0,\dots,m}$.}
	\TextN{Prove $\Par{p_0,p_1,\dots,p_m}$ is not liney indep in $\PoF{m}$.}
}\par\quad
...
\ProblemN{\hypertarget{2C7}{5}}{
	(a) \TextN{Let $U=\Bra{p\in\PoF{4}:p\Par{2}=p\Par{5}=p\Par{6}}$. Find a bss of $U$.}
	(b) \TextN{Extend the bss in {\tgnr(a)} to a bss of $\PoF{4}$, and find a $W$ suth $\PoF{4}=U\oplus W$.}
}Using (2.C.10).
...
\ProblemN{\hypertarget{2C10}{6}}{
	\TextN{Supp $m\in\Nbp,\;p_0,p_1,\dots,p_m\in\PoFi$ are suth each $\deg p_k=k.$}
	\TextN{Prove $\Par{p_0,p_1,\dots,p_m}$ is a bss of $\PoF{m}$.}
...
\ProblemN{\hypertarget{2C4e10}{7}}{
	\TextN{Supp $m$ is a positive integer. For $0\leqslant k\leqslant m$, let $p_k\Par{x}=x^k\Par{1-x}{^{m-k}}$.}
	\TextN{Show $\Par{p_0,\dots,p_m}$ is a bss of $\PoF{m}$.\vspace{0pt}}
...
\ProblemN{\hypertarget{3D4e20}{8}}{
	\TextN{Supp $q\in\PoRi.$ Prove $\exists\,p\in\PoRi,q\Par{x} = \Par{x^2 + x}p\apostrophe\apostrophe\Par{x} + 2xp\apostrophe\Par{x} + p\Par{3}.$\vspace{1pt}}
...
\ProblemN{\hypertarget{3D19}{9}}{
	\TextN{Supp $T\in\Lm[\BigPar]{\PoRi}$ is inje. And $\deg Tp\leqslant\deg p$ for every nonzero $p\in\PoRi$.}
	(a) \TextN{Prove $T$ is surj. \quad {\large\tgnr(b)} Prove for every nonzero $p$, $\deg Tp=\deg p$.}
...
\ProblemN{11}{
	\TextNL{Supp $V=\Rbb^\Rbb$ and $U=\Bra{\hspace{3pt}f\in\Rbb^\Rbb:f\Par{x_1}=\cdots=f\Par{x_m}=0}$ is a subsp of $V,$\vspace{2pt}}
	\TextNL{with each $x_k\in\Rbb.$ \;Prove $\forall W\in\mathcal{S}_V U,\dim W=m.$\hfill{\large\tgsc Hint$:$ }{\FontNorm Find an iso from $V\XSlash U$ onto $\Rbb^m.$}}
...
\ProblemN{\hypertarget{3F7}{12}}{
	\TextNL{Show the dual bss of $\Par{1, x,\dots,x^m}$ of $\PoR{m}$ is $\Par{\varphi_0,\varphi_1,\dots,\varphi_m}$, where {\FontNorm $\varphi_k\Par{p}=\Frac{p^{\SmallPar{k}}\Par{0}}{k!}$.}\vspace{-2pt}}
	\TextNL{{\FontNorm Here $p{^{\SmallPar{k}}}$ denotes the $k^{th}$ deri of $p$, with the understanding that the $0^{th}$ deri of $p$ is $p$.}\vspace{-2pt}}
