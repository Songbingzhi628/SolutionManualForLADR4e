...
The promotion of every subset of $V$ is the smallest subsp of $V$ containing $W.$
\SepLine\pagebreak
...
\Hc\Or $\range T\apostrophe=\Span[\BigPar]{T\apostrophe\Par{\varphi_1},\,T\apostrophe\Par{\varphi_2}}=\Span{4\psi_1+5\psi_2+6\psi_3,\:7\psi_1+8\psi_2+9\psi_3}.$\vspace{7.5pt}\TextNL{}
...
\Hc$\Rightarrow \range T=\Bra{Tx:x\in\Span{-2e_2,\:e_3}}=\Span[\BigPar]{T\Par{-2e_2},\,T\Par{e_3}}$\vspace{1.5pt}\TextNL{}
...
Define $\xi:U^0\rightarrow\Par{V\XSlash U}\apostrophe$ by $\xi\Par{\varphi}=\tilde{\varphi},$ where $\tilde{\varphi}\in\Par{V\XSlash U}\apostrophe$ is defined by $\tilde{\varphi}\Par{v+U}=\varphi\Par{v}.$\vspace{1pt}\par\quad
...
}(a) $\forall\varphi\in V\apostrophe,i\apostrophe\Par{\varphi}=\varphi\circ i=\varphi\mmid_U.$ Thus $i\apostrophe\Par{\varphi}=0\Longleftrightarrow\forall u\in U,\varphi\Par{u}=0\Longleftrightarrow\varphi\in U^0.$\parSol{}
...
\ProblemB{
	\TextB{Supp $T\in\Lm{V,W}.$ Prove $\range T\apostrophe=\BigPar{\null T}{^0}.$\hfill\Sbra[3pt]{{\tgsc\large Another proof of \tgnr\tgbfx[3.109](b)}}\vspace{2pt}}
}Supp $\varPhi\in\BigPar{\null T}{^0}.$ Becs by (3.B.12), $T\mmid_U:U\rightarrow\range T$ is iso; $V=U\oplus\null T.$\parSol{}
And $\forall v\in V,\exists\,!\,u_v\in U,w_v\in\null T,v=u_v+w_v.$ Define $\iota\in\Lm{V,U}$ by $\iota\Par{v}=u_v.$\vspace{4pt}\parSol{}
Let $\psi=\varPhi\circ\BigPar{T^{-1}\mmid_{\range T}}.$ Then $T\apostrophe\Par{\psi}=\psi\circ T=\varPhi\circ\BigPar{T^{-1}\mmid_{\range T}\circ T\mmid_V}.$\vspace{2pt}\parSol{}
Where $T^{-1}\mmid_{\range T}:\range T\rightarrow U;\;\;T:V\rightarrow\range T.$ Note that $T^{-1}\mmid_{\range T}\circ T\mmid_V=\iota.$\vspace{2pt}\parSol{}
...
\ProblemN[]{\hypertarget{3F12}{12}}{
	\TextNL{{\large\tgnr Note that $I_{V\apostrophe},I_{V}\upapostrophe:V\apostrophe\rightarrow V\apostrophe.$} \,For $\varphi\in V\apostrophe,\:I_{V\apostrophe}\Par{\varphi}=\varphi=\varphi\circ I_V=I_V\upapostrophe\Par{\varphi}.$ Thus $I_{V\apostrophe}=I_V\upapostrophe.$}
}\SepLine
...
\ProblemBnoor{\hypertarget{3F4e8}{4E 8}}{
	\TextB{Supp $B_V=\Par{v_1,\dots,v_{n}},$ with corres $B_{V\apostrophe}=\Par{\varphi_1,\dots,\varphi_{n}}.$}
	\TextB{Describe the relation between $v$'s and $\varphi$'s using inv liney maps.}
}Define $\Gamma:V\rightarrow\Fbb^n$ by $\Gamma\Par{v}=\Par{\varphi_1\Par{v},\dots,\varphi_{n}\Par{v}}.$\parSol{}
Define $\Lambda:\Fbb^n\rightarrow V$  by
$\Lambda\Par{a_1,\dots,a_n}=a_1v_1+\dots+a_nv_n.$ \;Now $\Lambda=\Gamma^{-1}.$\PfEnd
\SepLine
}
\pagebreak
%\def\OrbikenIX
