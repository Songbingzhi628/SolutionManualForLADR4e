...
\ProblemB{
	\Tips \,\,\,\TextB{Suppose $V_{\!1}\subseteq V_{\!2}$ in Exercise {\tgnr(23)}. Prove or give a counterexample\hspace{1pt}$:$ $V_{\!1}=V_{\!2}.$}
}\par\quad
...
\hspace{360pt}\includegraphics[width=2.4cm,height=1.2cm,scale=0.22]{diagram1C-1.png}\vspace{-4pt}\par\quad
Let $V=\Fbb^3,B_V=\Par{e_1,e_2,e_3},\;V_{\!1}=\Span{e_1},\;U_1=\Span{e_2,e_3},\;V_{\!2}=\Span{e_1,e_2},\;U_2=\Span{e_3}.$\par\quad
Now $V_{\!1}\subseteq V_{\!2},\;U_2\subseteq U_1$ and $V_{\!1}\oplus U_1=V_{\!2}\oplus U_2.$ But $V_{\!1}\neq V_{\!2},\;U_1\neq U_2.$\PfEnd
\SepLine
...
\ProblemN{\hypertarget{2A6}{6}}{
	...
	\TextN{\Blind{Prove that} $\Longleftrightarrow\BigPar{v_1-v_2,v_2-v_3,v_3-v_4,v_4}$ is linely inde. [Q]\vspace{-6pt}}
...
\Or Note that\; $\Frac{\;1\;}{1}>\Frac{\;1\;}{2}>\dots>\Frac{\;1\;}{m},\,\,\,\forall m\in\Nbp.$ Suppose\; $f_m=\left\{\begin{array}{l}
	...
	\hspace{-4pt}0,\hfill x\in\XSbra{\;0,\;\Frac{\;1\;}{m}\;}\end{array}\right.$\vspace{0pt}\par\quad
...
\Or 用比较系数法. {Denote the coeff of $x^k$ in $p\in\PoF{}$ by $\xi_k\Par{p}.$}\par\quad
...
\Blind{\Example \,\,\,}Let $B_M=\Par{v_1-v_2,v_3},B_N=\Par{v_2}.$ Then $\range T\mmid_M=\Span{w_1,w_2},\range T\mmid_N=\Span{w_1}$\par
...
$\hText{$\includegraphics[width=88pt]{diagram3B-2}$}\hText{$
	...
	while $E\mmid_{\def\envFontA{\normalsize}\range S\mmid_{U_\Delta}}\!\!\!:\range S\mmid_{U_\Delta}\rightarrow\range T$ is an iso.$}$\par\vspace{6pt}\quad
...
又 By (2.C.10), $\Span{Dx,Dx^2,Dx^3,\cdots}=\Span{1,x,x^2,\cdots}=\PoR{}.$\par\vspace{4pt}\quad
...
\BulletPointX\Comment \,\,\,If $U$ is merely a subset of $V,$ then [3.85, 3.86] do not hold, and $V\XSlash U$ is not a vecsp.\TextB{}
Because $\BigPar{\Par{v-w}+u}\in U$ or $u-u\apostrophe\in U$ needs that $U$ is closed under add.\TextB{}
And because $\Par{v-\hat{v}}+\Par{w-\hat{w}}\in U$ and $\lambda\Par{v-\hat{v}}\in U$ assume that $U$ is a subsp.\vspace{-2pt}
\SepLine
...
\ProblemBnoor{\hypertarget{3E4e8}{4E 8}}{
	\TextB{Suppose $T\in\Lm{V,W},w\in\range T.$ Prove that $\Bra{u\in V:Tu=w}=u+\null T.$}
}{\def\mK{\mathcal{K}_{\!u}}Let $\mK=\Bra{u\in V:Tu=w}.$ \Sbra{ Not a vecsp.\hspace{1pt}} \;Suppose $u\in\mK.$ Then $u+\null T\subseteq\mK.$\parSol{}
...
(b) Suppose $B=v+U,$ where $v\in V$ and $U$ is a subsp of $V.$ \uline{Let each $v_k=v+u_k\in B,\exists\,!\,u_k\in U.$}\par\quad\Hb
$\forall w\in A,\;w=\sum_{i=1}^m \lambda_i v_i=\sum_{i=1}^m \lambda_i\Par{v+u_i}=\uline{\sum_{i=1}^m \lambda_i v}+\sum_{i=1}^m \lambda_i u_i=v+\sum_{i=1}^m \lambda_i u_i\in v+U=B.$\PfEnd\vspace{8pt}\quad\Hb 
...
(ii) \vspace{6pt}$2\leqslant k<m.$ Assume that $v=\lambda_1 v_1+\dots+\lambda_k v_k\in A\subseteq B.\;\XPar{\forall\lambda_i$ such that $\sum_{i=1}^k\lambda_i=1}$\par\quad\Hb\Hii
\vspace{6pt}For $u=\mu_1 v_1+\dots+\mu_k v_k+\mu_{k+1} v_{k+1}\in A.$ \;Fix one $\mu_\iota\neq 1.$\par\quad\Hb\Hii
\vspace{6pt}Then \;$\sum_{i=1}^{k+1}\mu_i-\mu_\iota=1-\mu_\iota\Longrightarrow\XPar{\sum_{i=1}^{k+1}\Frac{\mu_i}{1-\mu_\iota}}-\Frac{\mu_\iota}{1-\mu_\iota}=1.$\par\quad\Hb\Hii
...
\vspace{6pt}Let \,$\lambda_i=\Frac{\mu_i}{1-\mu_\iota}$ for $i\in\Bra{1,\dots,\iota-1};$ \;$\lambda_j=\Frac{\mu_{j+1}}{1-\mu_\iota}$ for $j\in\Bra{\iota,\dots,k}.$ \;Then,\par\quad\Hb\Hii
...
\BulletPointX\Tips \,\,\,{Suppose $U$ is a subsp of $V.$ \uline{Define $S\in\Lm{V\XSlash U,V}$ by $S\Par{v+U}=v.$}}\TextB{}
