...
(b) $\left|\begin{array}{l}$
	...
	$\end{array}\right.$\par\vspace{3pt}\quad
...
}{\tgsl The set of all surj $T\in\Lm{V,W}$ is not a subsp either.}\par\quad
...
If $\dim W=1,$ then $W=\Span{w_0}.$ Thus $U=\Span{T_0},$ where each $T_0v_i=0\Rightarrow T_0=0.$\parCom
{\tgsl We have used the generalized version of [3.5] for this Exercise.}\vspace{-4pt}
\SepLine
...
$\forall v\in V,T\Par{v-a_1 v_1-\dots-a_n v_n}=0,$ letting $Tv=a_1 Tv_1+\dots+a_n Tv_n.$\parSol{}
...
Let $X\oplus\Par{U\oplus\null T}=V.$ Now $X$ is nonzero. And $\range T=\range T\mmid_X+\range T\mmid_U.$\parCor
Assume that $\range T=\range T\mmid_U.$ Then $\range T\mmid_X\subseteq\range T\mmid_U.$\parCor
Because $\forall x\in X,Tx\in\range T\mmid_U\Rightarrow\exists\,u\in U,Tu=Tx.$\parCor 又 $T\mmid_U$ is inje $\Rightarrow x=u\in U\cap X=\zeroSubs\Rightarrow X=\zeroSubs.$ Contradicts.\PfEnd
\SepLine
...
\BulletPointX\TipsN{2}\,\,\,{Suppose $T\in\Lm{V,W}$ and $U$ is a subsp such that $V=U\oplus\null T.$}\TextB{}
...
\ProblemB{
	\TipsN{4}\,\,\,\TextB{Suppose $T\in\Lm{V,W}$ and $T\mmid_U:U\rightarrow\range T$ is an iso. Let $U=X+Y.$}
	{\IndentTipsN{4}}(a) \TextB{Show that $\range T=\range T\mmid_X+\range T\mmid_Y.$\vspace{2pt}}
	{\IndentTipsN{4}}(b) \TextB{Show that if $X\cap Y=\zeroSubs,$ then $\range T\mmid_X\cap\range T\mmid_Y=\zeroSubs.$}
}\par\quad
...
\ProblemB[]{
	\TipsN{5}\,\,\,\TextB{Suppose $S_1,\dots,S_n\in\Lm{V}$ and $S_1S_2\cdots S_n$ makes sense. Then using induction\hspace{1pt}$:$}
	{\IndentTipsN{5}}(a) \TextB{$\range S_1\supseteq\range\Par{S_1S_2}\supseteq\cdots\supseteq\range\Par{S_1S_2\cdots S_n}.$}
	{\IndentTipsN{5}}(b) \TextB{$\null S_n\subseteq \null\Par{S_{n-1}S_n}\subseteq\cdots\subseteq\null\Par{S_1S_2\cdots S_n}.$\vspace{2pt}}
	{\IndentTipsN{5}}\Corollary \,\,\,If $S_1\cdots S_n$ is surj, then each $S_k$ is surj;\;\;If $S_1\cdots S_n$ is inje, then each $S_k$ is inje.\TextB{\vspace{-4pt}}
}\SepLine
...
%If we let $Y=Y_R\oplus W_0,$ where $Y_R=Y\cap\range T.$ Then $Tv\in Y\Longleftrightarrow Tv\in Y_R.$\parCom
%So that $\dim\mathcal{K}_Y=\dim\mathcal{K}_{Y_R}=\dim\null T+\dim Y_R.$\parCom
...
Suppose $\null S\subseteq\null T$. Let \uline{$W=\range S\oplus U.$}\par\quad
Define $E\in\Lm{W}$ by \uline{$E\Par{Sv+w}=Tv$} for all $w\in U$ and $Sv.$ Now we check that $E$ is linear.\par\quad
...
\Or Let \uline{$V=U\oplus\null S.$} Then $S\mmid_{U}:U\rightarrow\range S$ is an iso.\par\quad
Now \uline{extend $T\Par{S\mmid_{U}}{^{-1}}\in\Lm{\range S,W}$ to $E\in\Lm{W,W}.$}\vspace{6pt}\par\quad
\Or \Sbra[3pt]{{\tgsl Req $\range S$ Finite-dim} } \;Let $B_{\range S}=\Par{Sv_1,\dots,Sv_n}$. Then \uline{$V=\Span{v_1,\dots,v_n}\oplus\null S$.}\par\quad
Define $E\in\Lm{W}$ by $E\Par{Sv_i}=Tv_i,\;Eu=0$ for each $v_i$ and all $u\in\null S.$\par\quad
...
Suppose $\range S\subseteq\range T$. Let $B_V=\Par{v_1,\dots,v_m}.$\par\quad
\NOTICE that for each $Sv_i\in\range T,\exists\,u_i\in V,Tu_i=Sv_i.$ Define $E\in\Lm{V}$ by $Ev_i=u_i$ for each $v_i.$\PfEnd
\SepLine

%\ProblemB{
%	\TextB{Suppose $V$ is finite-dim. Let $U$ be such that $U\oplus X=V.$}
%	\TextB{Suppose $Y$ is a finite-dim subsp of $W$ such that $\dim Y=\dim V-\dim X.$}
%	\TextB{Define $T\in\Lm{V,W}$ by }
%	\TextB{Prove that $.$}
%}
%\SepLine
...
\Corollary \,\,\,(1) If $S$ is inje, then $\dim\range ST=\dim\range T$.\parCor
(2) If $T$ is surj, then $\dim\range ST=\dim\range S$.
\SepLine

\ProblemB{
	\hypertarget{3B4e24}(a) \TextB{Suppose $\dim V = 5,S,T\in\Lm{V}$ are such that $ST = 0$. Prove that $\dim\range TS\leqslant 2.$\vspace{2pt}}
	(b) \TextB{Let $\dim V=n$ in {\tgnr\large(a)}. Prove that $\dim\range TS\leqslant\left\lfloor\Frac{n}{2}\right\rfloor.$\vspace{4pt}}
...
}\NOTICE that the following proof is wrong:\parSol{}
...
{We will recursively define a sequence of polys $\Par{p_k}{_{k=0}^\infty}$ where $Dp_k=x^k.$}\par\vspace{2pt}\quad
(i) {Because $\deg Dx=\Par{\deg x}-1=0,$ we have $Dx=C\in\nonzeroFbb.$}\par\quad\Hi
...
{Now we get $\Par{p_k}{_{k=0}^\infty}$ by recursion. Hence $\forall p=\sum_{k=0}^{\deg p}a_k x^k\in\PoR{},\exists\,q=\BigPar{\sum_{k=0}^{\deg p}a_k p_k},Dq=p.$}\PfEnd\vspace{8pt}\quad
\Or Let $D\Par{C}=0,Dx^k=p_k$ of deg $\Par{k-1}$ for all $C\in\Rbb=\PoR{}$ and for each $k\in\Nbp.$\par\quad
Note that for any $m\in\Nbp,\Par{p_1,\dots,p_m,p_{m+1}}$ is a basis of $\PoR{m}.$\par\quad
For $q=0\in\PoR{},\deg q=-\infty,\exists\,p=C\in\PoR{0},Dp=q=0.$\par\quad
For all nonzero $q\in\PoR{},\exists\,!\,m\in\Nbb,\deg q=m,$ so that $q\in\PoR{m}.$\par\quad
Now $\exists\,!\,a_k\in\Rbb,q=a_{m+1}p_{m+1}+\dots+a_1p_1=D\Par{a_{m+1}x^{m+1}+\dots+a_1x}.$ Thus $\exists\,p\in\PoR{m+1},Dp=q.$\PfEnd
\SepLine
...
}Define $B\in\Lm[\BigPar]{\PoR{}}$ by $B=5D^2+3D\Rightarrow B\Par{q}=5q\apostrophe\apostrophe+3q\apostrophe$.\parSol{}
...
Let $B_V=\Par{v_1,\dots,v_m,u_1,\dots,u_n}$. Note that $\forall v\in V,v=\sum_{i=1}^m a_i v_i+\sum_{i=1}^n b_i u_i,\exists\,!\,a_i,b_i\in\Fbb.$\parSol{}
Define $\varphi_i:V\rightarrow\Fbb$ by $\varphi_i\Par{v}=a_i v_i$ for each $i$. We now check the linearity.\parSol{}
...
}Let $\Par{v_1,\dots,v_5}$ be a basis of $\Rbb^5$, $\Par{w_1,w_2}$ be a basis of $\Rbb^2$. Define $T,S\in\Lm{V,W}$ by\parSol{\vspace{2pt}}
...
Let $B_{\range T}=\Par{Tv_1,\dots,Tv_n},\;B_W=\Par{Tv_1,\dots,Tv_n,w_1,\dots,w_p}$.\par\quad
Let $U=\Span{v_1,\dots,v_n}.$ Let $B_{\null T}=\Par{u_1,\dots,u_m}$. Then $B_V=\Par{v_1,\dots,v_n,u_1,\dots,u_m}.$\PfEnd
\SepLine
...
And $Tv_1=T\XPar{\Frac{u_k}{A_{1,k}}}=\Frac{1}{A_{1,k}}\Par{A_{1,k}w_1+\dots+A_{n,k}w_n}=1w_1+\dots+\Frac{A_{n,k}}{A_{1,k}}w_n.$\par\quad
\vspace{15pt}\AlignEq{}{\forall j\in\underset{\qquad\quad\overset{\uwave{\quad\qquad\qquad\qquad}}{\quad\;\;i\,\in\,\envFontSmall\Bra{k+1,\dots,n}\quad}}{\Bra{2,\dots,k,{k+2,\dots,n+1}}},\;Tv_j=T\Par{u_{j-1}-A_{1,j-1}v_1}=Tu_{j-1}-T\XPar{\Frac{A_{1,j-1}u_k}{A_{1,k}}}\hspace{93pt}\\[-6pt]=A_{1,j-1}w_1+\dots+A_{n,j-1}w_n-A_{1,j-1}\XPar{1w_1+\dots+\Frac{A_{n,k}}{A_{1,k}}w_n}=0w_1+\dots+\XPar{A_{n,j-1}-\Frac{A_{1,j-1}A_{n,k}}{A_{1,k}}}w_n.}\PfEnd\vspace{4pt}\par
\SepLine
...
\BulletPointX\NoteForSmall{[3.49]} \vspace{-8pt}\AlignEq{}{&\textstyle\because\Sbra{\Par{AC}_{\cdot,k}}_{j,1}=\Par{AC}_{j,k}=\sum_{r=1}^n A_{j,r}C_{r,k}=\sum_{r=1}^n A_{j,r}\Par{C_{\cdot,k}}_{r,1}=\Par{AC_{\cdot,k}}_{j,1}\hspace{-50pt}\\&\therefore\Par{AC}_{\cdot,k}=A_{\cdot,\cdot}C_{\cdot,k}=AC_{\cdot,k}}\PfEnd[-25pt]
\SepLine

\BulletPointX\Exercise{\hypertarget{3C10}{10}} \vspace{-8pt}\AlignEq{}{&\textstyle\because\Sbra{\Par{AC}_{j,\cdot}}_{1,k}=\Par{AC}_{j,k}=\sum_{r=1}^n A_{j,r}C_{r,k}=\sum_{r=1}^n \Par{A_{j,\cdot}}_{1,r}C_{r,k}=\Par{A_{j,\cdot}C}_{1,k}\\
...
$\because\Par{Ac}_{j,1}=\sum_{r=1}^n A_{j,r}c_{r,1}=\Sbra{\sum_{r=1}^n\Par{A_{\cdot,r}c_{r,1}}}{_{j,1}}=\Par{c_1 A_{\cdot,1}+\dots+c_n A_{\cdot,n}}_{j,1}$\vspace{3pt}\par\quad
...
$\because\Par{aC}_{1,k}=\sum_{r=1}^n a_{1,r}C_{r,k}=\Sbra{\sum_{r=1}^n a_{1,r}\Par{C_{r,\cdot}}}{_{1,k}}=\Par{a_1 C_{1,\cdot}+\dots+a_n C_{n,\cdot}}_{1,k}$\vspace{3pt}\par\quad
...
Note that $\Sbra{A\Par{B+C}}_{j,k}=\sum_{r=1}^n A_{j,r}\Par{B+C}_{r,k}=\sum_{r=1}^n \Par{A_{j,r}B_{r,k}+A_{j,r}C_{r,k}}=\Par{AB+AC}_{j,k}.$\PfEnd\vspace{10pt}\quad
...
Suppose $A\in\Fbb^{m,n}$ and $B,C\in\Fbb^{n,p}.$ We will show that $LHS=\Sbra{\Par{AB}C}_{j,k}=\Sbra{A\Par{BC}}_{j,k}=RHS.$\par\quad
...
Let $B_{\range T}=B_W=\Par{w_1,\dots,w_m}\Rightarrow\forall w_i,\exists\,!\,v_i\in V,Tv_i=w_i.$ Let $B_{U}=\Par{v_1,\dots,v_m}$.\par\quad
Then $\dim U=\dim W.$ Thus $T\mmid_U$ is an iso of $U$ onto $W.$\PfEnd\vspace{6pt}\quad
...
Suppose $\varphi\in W^0.$ Because $\null\iota=W\subseteq\null\varphi.$ By \Sbra{3.B \TIPSN{2}}, $\varphi=\varphi\circ\iota=\iota\apostrophe\Par{\varphi}.$\PfEnd
\SepLine
...
By \Sbra{3.B \TIPSN{2}}, $\varPhi=\varPhi\circ \iota.$ Thus $T\apostrophe\Par{\psi}=\psi\circ T=\varPhi\circ{\iota}=\varPhi.$\PfEnd
\SepLine
