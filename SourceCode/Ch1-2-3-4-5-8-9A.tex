...
(b) \!\Sbra[3pt]{{\tgsl Only in Finide}} \;By Exe (22), $\Dim\BigPar{U^0+W^0}=\dim V-\Dim\Par{U\cap W}.$\PfEnd\vspace{4pt}\quad\Hb
...
}By \Sbra{3.A {\NOTEFOR} $\Fbb^S$}, immed. \,\Or Choose $m\in\Nbp.$ Let $p\Par{x}=a_0+a_1 x+\dots+a_m x^m=0\in\Rbb^{[0,1]}.$\parSol{}
Then $p$ has infily many roots and hence each $a_k=0,$ othws $\deg p\geqslant 0,$ ctradic [4.12].\parSol{}
Thus $\Par{1,x,\dots,x^m}$ is liney indep in $\Rbb^{[0,1]}.$ Simlr to [2.16], $U$ is infinide.\PfEnd
%\vspace{8pt}\parSol{}
...
\Or Define $S\in\Lm{\Fbb^{n,1}}$ by $Sx=A^tx.$ \;By (a), \Sbra{3.F \TIPSN{4}}, and Exe (15, 4E 15),\par\quad\Hb
...
\vfill\ChDecl{Ch5BI}{5.B}{\quad\footnotesize$\hText{$
...
}(a) $\forall\varphi\in V\apostrophe,\,p\Par{T\apostrophe}\Par{\varphi}=\varphi\circ p\Par{T}=0\Rightarrow \range p\Par{T}\subseteq\null\varphi.$ Thus $p\Par{T}=0.$ 又 $q\Par{T\apostrophe}=0.$\PfEnd\vspace{2pt}\parSol{}
...
%	\TextA{Supp $A,B\in\Fbb^{n,n}$ are up-trig with each $A_{p,p}=\alpha_p,B_{p,p}=\beta_p.$\hfill\tgnr\FontNorm $A_{j,k}=B_{j,k}=0$ for $j>k.$}
...
Becs by (L1), $\bigcap_{i=1}^j\Range\Par{T-\lambda_iI}\supseteq\Null\Par{T-\lambda_{j+1}I},$ and by \Sbra{1.C \TIPSN{3}},\par\quad
...
Define $p\Par{z}=\Par{z-\lambda_1}\cdots\Par{z-\lambda_m}.$ Then $\Sbra{p\Par{T}}{^{\dim V}}=0\Rightarrow p\Par{T}=0\Rightarrow p$ is the min.\PfEnd\parSol{}
\Or Using (8.A). Becs $V=\Null\Par{T-\lambda I}\oplus\Range\Par{T-\lambda I}$ for all $\lambda\in\Fbb.$\PfEnd
\SepLine
...
Note that $\Par{T-\lambda_jI}{^k}w=0=\sum_{i=1}^m\Par{\lambda_i-\lambda_j}{^k}v_i\Rightarrow w=v_j\in E\Par{\lambda_j,T}.$\PfEnd\vspace{3pt}\parSol{}\PfEnd
...
\Or Let the min of $N=\Par{T-\lambda I}\Big|{_{G\SmallPar{\:\!\lambda,\,T\:\!}}}$ be $z^m.$\parSol{}
...
}Each $\Par{T-\alpha I}{^k}\Big|{_{\:\!G\SmallPar{\:\!\lambda,\,T\:\!}}}$ are inv for $\alpha\neq\lambda.$ \;Becs $m_T\Par{T\mmid_{\:\!G\SmallPar{\:\!\lambda,\,T\:\!}}}=0\Longleftrightarrow\Par{T-\lambda I}{^k}\Big|{_{\:\!G\SmallPar{\:\!\lambda,\,T\:\!}}}=0.$\PfEnd\vspace{4pt}\parSol{}
...
}Let $w\in W$ and $\Par{T-\lambda I}{^j}w=0\neq\Par{T-\lambda I}{^{j-1}}w\in E\Par{\lambda,T}.$ 又 $W$ invar $\Rightarrow\Par{T-\lambda I}{^{j-1}}w\in W.$\PfEnd
\SepLine
...
}Each $\Par{T-\lambda_kI}{^{\alpha_k}}\Big|{_{\:\!G\SmallPar{\:\!\lambda_j,\,T\:\!}}}$ is inje for $k\neq j\Rightarrow\nullp q\Par{T}\Big|{_{\:\!G\SmallPar{\:\!\lambda_j,\,T\:\!}}}=F_{\!j}.$ \;\Or By \Sbra{B \TIPSN{1,4}}.\PfEnd
\SepLine
...
\Anchor{8BT4}\ProblemN{\BulletPointX B.\TipsN{4}}{
	\TextA{Supp $T\in\Lm{V},p,q\in\PoFi$ have no common zeros on $\Cbb,$ and $\Par{pq}\Par{T}=0.$}
	\TextA{Prove $V=\nullp p\Par{T}\oplus\nullp q\Par{T}=\rangep p\Par{T}\oplus\rangep q\Par{T}.$
%	\hfill\tgnr\FontNorm\ACoro $\nullp p\Par{T}=\rangep q\Par{T}.$
	}
}By Exe (4E 4.13), $\forall v\in V,v=u+w,$ where $u=r\Par{T}\:\!p\Par{T}\:\!v,\,w=s\Par{T}\:\!q\Par{T}\:\!v.$\parSol{}
Then $V=\rangep p\Par{T}+\rangep q\Par{T}$ and $\nullp p\Par{T}\cap\nullp q\Par{T}=\zeroSubs.$\parSol{}
Becs $\Par{pq}\Par{T}=0.$ Now $\rangep p\Par{T}\cap\rangep q\Par{T}=\zeroSubs$ and $V=\nullp p\Par{T}+\nullp q\Par{T}.$\PfEnd\vspace{2pt}\parSol{}
\Or Let $V_{\!\Cbb}=G\Par{\lambda_1,T_{\!\Cbb}}\oplus\cdots\oplus G\Par{\lambda_m,T_{\!\Cbb}},$ and $m_T\Par{z}=\Par{z-\lambda_1}{^{k_1}}\cdots\Par{z-\lambda_m}{^{k_m}}.$\parSol{}
Let $\mA=\Bra{\alpha_1,\dots,\alpha_{\!A}}$ be the intersec of the zeros of $p$ and the eigvals of $T.$ \,Simlr for $\mB.$\parSol{}
Then $\mA\cap\mB=\emptySet.$ Let $p=\omega_{\alpha_1}\!\!\cdots\omega_{\alpha_{\!A}}\,p_0,\;q=\omega_{\beta_1}\!\!\cdots\omega_{\beta_B}\,q_0,$ where $\omega_{\lambda_i}\Par{z}=\Par{z-\lambda_i}{^{k_i}}.$\parSol{}
By \Sbra{B \TIPSN{3}}, $\nullp p\Par{T_{\!\Cbb}}=G\Par{\alpha_1,T_{\!\Cbb}}\oplus\cdots\oplus G\Par{\alpha_{\!A},T_{\!\Cbb}}.$ \,Simlr for $q.$\vspace{1pt}\parSol{}
Note that if $\alpha_j\not\in\Rbb,$ then $\exists\,\alpha_i=\overline{\alpha_j}.$ Simlr for $\mB.$ \,Now $V_{\!\Cbb}=\nullp p\Par{T_{\!\Cbb}}\oplus\nullp q\Par{T_{\!\Cbb}}.$\parSol{}
Then becs $\rangep p\Par{T_{\!\Cbb}}\subseteq\nullp q\Par{T_{\!\Cbb}}$ and then $\rangep p\Par{T_{\!\Cbb}}\cap\nullp p\Par{T_{\!\Cbb}}\subseteq\zeroSubs.$\parSol{}
By \Sbra{1.C \TIPSN{2}}, $\rangep p\Par{T_{\!\Cbb}}=\nullp q\Par{T_{\!\Cbb}}.$ So that $V_{\!\Cbb}=\rangep p\Par{T_{\!\Cbb}}\oplus\rangep q\Par{T_{\!\Cbb}}.$\PfEnd\vspace{3pt}
\ACoro Supp $\Par{p_1\cdots p_k}\Par{T}=0,$ and $p_1,\dots,p_k$ have no common zeros on $\Cbb.$\parCor
(a) We show $V=\nullp p_1\Par{T}\oplus\cdots\oplus\nullp p_k\Par{T}$ by induc on $k$. (i) $k=1,2.$ Immed. (ii) $k>2.$\parCor{\Ha}
Asum it holds for $\Par{k-1}.$ Then $V=\nullp p_1\Par{T}\oplus\cdots\oplus\nullp p_{k-2}\Par{T}\oplus\Null\Par{p_{k-1}p}\Par{T}.$\parCor{\Ha}
Simlr to above, $\Null\Par{p_{k-1}p}\Par{T}=\nullp p_{k-1}\Par{T}\oplus\nullp p_k\Par{T}.$\hfill Simlr for $\rangep p\Par{T}.$\vspace{2pt}\parCor
(b) Becs $\rangep p_j\Par{T}\subseteq\bigoplus_{i\,\in\,\Gamma_{\!j}}\nullp p_i\Par{T},$ and then $\nullp p_j\Par{T}\cap\rangep p_j\Par{T}=\zeroSubs.$\parCor{\Hb}
Where $\Gamma_{\!j}=\Bra{1,\dots,j-1,j+1,\dots,k}.$ \,Now $\rangep p_j\Par{T}=\bigoplus_{i\,\in\,\Gamma_{\!j}}\nullp p_i\Par{T},$ by \Sbra{1.C \TIPSN{2}}.\vspace{2pt}\parCor
(c) $\nullp p_j\Par{T}=\bigcap_{i\,\in\,\Gamma_{\!j}}\rangep p_i\Par{T}.$ \;Simlr \OR by (b), immed.\par
...
%	(2) $\null N^{m-j}\oplus U_{m-j}=\null N^{m-j+1}.$\, (3) $\null I\oplus U_0=\null N.$\TextB{}
	Start by $B_{U_{m-1}}=\Par{v_{1,1},\dots,v_{n_1,1}}.$\, But $\Par{Nv_{1,1},\dots,Nv_{n_1,1}}$ might be liney dep. Invalid method.\par\vspace{2pt}
...
\Blind{$Q\Rightarrow P:$ \;}Then by Exe (D.3), the line directly above the diag of {\tgsc any} Jordan $\Mt{N}$ is all $1.$\parSol{}
...
\BulletPointX\NoteForSmall{[9.12]}\;\;Another proof: \;$\overline{T_{\!\Cbb}\Par{u+\i v}}=\overline{Tu+\i Tv}=Tu-\i Tv=T_{\!\Cbb}\Par{\overline{u+\i v}}.$\TextB{}
...
Asum it holds for $\Par{m-1}.$ Let $\Par{T_{\!\Cbb}-\lambda I}{^{m-1}}\Par{u+\i v}=x+\i y.$ Becs $\overline{\Par{T_{\!\Cbb}-\lambda I}{^{m-1}}\Par{u+\i v}}=x-\i y.$\TextB{}
Then $\overline{\Par{T_{\!\Cbb}-\lambda I}{^m}\Par{u+\i v}}=\overline{\Par{T_{\!\Cbb}-\lambda I}\Par{x+\i y}}=\Par{T_{\!\Cbb}-\overline{\lambda}I}\Par{x-\i y}.$\PfEnd\vspace{2pt}
...
}(a) Replace $V$ with $\nullp q\Par{T}{^j}$ and $T$ with $T$ restr to $V.$ Becs $m_T=q{^k}.$\parSol{\Ha}
By \Sbra{8.B \TIPSN{4}}, $V_{\!\Cbb}=\Null\Par{T_{\!\Cbb}-\lambda I}{^k}\oplus\Null\Par{T_{\!\Cbb}-\overline{\lambda}I}{^k}.$ By [9.17] and [9.4].\PfEnd\vspace{4pt}
