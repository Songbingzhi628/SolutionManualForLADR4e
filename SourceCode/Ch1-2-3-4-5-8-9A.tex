...
{$\Rightarrow A_{j,k}=d_k' A_{j,1}=c_j A_{1,k}=c_j d_k' A_{1,1}=c_j d_k,$ where $d_k=d_k' A_{1,1}.$} By CR Factoriz, immed.\PfEnd
\SepLine
...
\Anchor{3DNE4e22}\BulletPointX\NoteForSmall{Exe (4E 22)}\;\;$T\in\Lm{V,W}$ inv $\Longleftrightarrow\Mt{T}$ wrto some $B_V,B_W$ is inv $\Longleftrightarrow$ wrto any $B_V,B_W.$\TextB{}
Supp $\Mt{T}$ wrto some bses is inv. Let $S\in\Lm{W,V}$ be suth $\Mt{S}=\Mt{T}{^{-1}}.$\TextB{}
Then $\Mt{TS}=I=\Mt{ST}$ wrto same bses. Apply $\Mneg,$ now $TS=I,ST=I\Rightarrow S=T^{-1}.$
\SepLine
...
$\Par{\mX_{\!1,k},\dots,\mX_{\!r,k}}$ and  $\Par{\mX_{\!j,1},\dots,\mX_{\!j,r}}$ are liney indep lists.\PfEnd
\SepLine

\Anchor{3D7}\Anchor{3D4e10}\ProblemBnoor{4E 10}{
	\TextA{Supp $V,W$ finide, $U$ is a subsp of $V,$ $\mE=\Bra{T\in\Lm{V,W}:T\mmid_U=0}.$\vspace{1pt}}
	\TextA{Prove $\dim\mE=\dim\Lm{V,W}-\dim\Lm{U,W}.$\vspace{1pt}}
