...
Where each $p_j\Par{z}=z-\mu_j,\,q_k\Par{z}=z^2-2\Par{\Real\lambda_k}z+\aMid{\lambda_k}{^2}=z^2+bz+c.$\vspace{2pt}\parNot
Becs $\exists$ Jordan bss of $Q_k=Q\Par{\lambda_k,T}$ for $q_k\Par{T}.$ Fix one $k$ and replace $T$ with $T\mmid_{Q_k}.$\parNot
The Jordan $\Mt{T^2+bT+cI}=\Mt{T_{\!\Cbb}^2+bT_{\!\Cbb}+cI}=\Mt{T_{\!\Cbb}-\lambda I}\Mt{T_{\!\Cbb}-\overline{\lambda}I}.$\parNot
Note that $\overline{T_{\!\Cbb}-\lambda I}=T_{\!\Cbb}-\overline{\lambda}I.$
\SepLine
