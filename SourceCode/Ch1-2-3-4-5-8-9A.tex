...
\ChDecl{Ch8}{8}{\quad{\ANote {\FontSmall Supp $V$ is a non0 finide non0 vecsp over $\Fbb.$ Supp $T\in\Lm{V}.$ Let $m_T$ be the min of $T.$}}}\par\vspace{-6pt}
...
\Anchor{8A4e12}\ProblemBnoor{4E A.12}{
	\TextA{Supp every $v\in V$ is a gigvec of $T.$ Prove $V=G\Par{\lambda,T}.$}
...
\Anchor{8A4e15}\Anchor{8B5}\ProblemNnoor{B.5}{4E A.15}{
	\TextA{Prove non0 $T$ diag $\Rightarrow$ each $G\Par{\lambda,T}=E\Par{\lambda,T}.$}
...
\Or Let $m_T\Par{z}=\Par{z-\lambda}{^m}q\Par{z}$ with $q\Par{\lambda}\neq0.$ By (4E B.6) and (4E A.4), $d\geqslant m.$\PfEnd\vspace{3pt}\parSol{}
...
\Anchor{8BT4}\ProblemN{\BulletPointX B.\TipsN{4}}{
	\TextA{Supp $\Par{pq}\Par{T}=0$ and $p,q\in\PoFi$ have no common zeros on $\Cbb.$}
	\PrePa\TextA{Prove $V=\nullp p\Par{T}\oplus\nullp q\Par{T}=\rangep p\Par{T}\oplus\rangep q\Par{T}.$}
	\PrePb\TextA{Prove $\nullp p\Par{T}=\rangep q\Par{T}.$\vspace{2pt}}
}(a) By Exe (4E 4.13), $\forall v\in V,v=u+w,$ where $u=r\Par{T}\:\!p\Par{T}\:\!v,\,w=s\Par{T}\:\!q\Par{T}\:\!v.$\parSol{\Ha}
Then $V=\rangep p\Par{T}+\rangep q\Par{T}$ and $\nullp p\Par{T}\cap\nullp q\Par{T}=\zeroSubs.$\parSol{\Ha}
Becs $\Par{pq}\Par{T}=0.$ Now $\rangep p\Par{T}\cap\rangep q\Par{T}=\zeroSubs$ and $V=\nullp p\Par{T}+\nullp q\Par{T}.$\vspace{3pt}\parSol{}
(b) $\rangep q\Par{T}\subseteq\nullp p\Par{T}\Rightarrow\rangep q\Par{T}\cap\nullp q\Par{T}=\zeroSubs$\parSol{\Hb}
$\Rightarrow V=\rangep q\Par{T}\oplus\nullp q\Par{T}.$ Now By (a) and \Sbra{1.C \TIPSN{1}}.\PfEnd\vspace{4pt}\parSol{}
...
Simlr, $\rangep p\Par{T_{\!\Cbb}}=\nullp q\Par{T_{\!\Cbb}}.$ So that $V_{\!\Cbb}=\rangep p\Par{T_{\!\Cbb}}\oplus\rangep q\Par{T_{\!\Cbb}}.$\PfEnd\vspace{3pt}
\ACoro Supp $\Par{p_1\cdots p_k}\Par{T}=0,$ and $p_1,\dots,p_k\in\PoFi$ have no common zeros on $\Cbb.$\parCor
(a) We show $V=\nullp p_1\Par{T}\oplus\cdots\oplus\nullp p_k\Par{T}$ by induc on $k$. (i) $k=1,2.$ Immed. (ii) $k>2.$\parCor{\Ha}
Asum it holds for $\Par{k-1}.$ Simlr to above, $\Null\Par{p_{k-1}p}\Par{T}=\nullp p_{k-1}\Par{T}\oplus\nullp p_k\Par{T}.$
%\parCor{\Ha}
%We show $V=\rangep p_1\Par{T}\oplus\cdots\oplus\rangep p_k\Par{T}$ by induc simlr.\parCor{\Ha}
%$\Range\Par{p_{k-1}p}\Par{T}=\rangep p_{k-1}\Par{T}\oplus\rangep p_k\Par{T}.$
\vspace{2pt}\parCor
(b) Becs $\rangep p_j\Par{T}\subseteq\bigoplus_{i\,\in\,\Gamma_{\!j}}\nullp p_i\Par{T}\Rightarrow\nullp p_j\Par{T}\cap\rangep p_j\Par{T}=\zeroSubs.$\parCor{\Hb}
Where $\Gamma_{\!j}=\Bra{1,\dots,j-1,j+1,\dots,k}.$ \,Now $\rangep p_j\Par{T}=\bigoplus_{i\,\in\,\Gamma_{\!j}}\nullp p_i\Par{T}.$\parCor{\Hb}
Then $\nullp p_j\Par{T}=\bigcap_{i\,\in\,\Gamma_{\!j}}\rangep p_i\Par{T}.$ \;\NOTICE that $\bigcap_{i\,\in\,\Gamma_{\!j}}\rangep p_i\Par{T}\supsetneq\Range\Par{p_1\cdots p_k}\Par{T}.$
\SepLine
