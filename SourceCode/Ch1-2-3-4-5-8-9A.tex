...
%\ProblemB{
%	\TipsN{4}\,\,\,\TextB{Supp $U$ and $W$ are subsps of $V.$ Define $S_V X=\Pure V\:\!\!\XSlash X$ for any subsp $X$ of $V.$}
%	\IndentTipsN{4}\PrePa\TextB{Prove $S_V\Par{U+W}=S_V U\cap S_V W,$ \:{\tgnr(b)} $S_V\Par{U\cap W}=S_V U+S_V W.$}
%}$\forall\Par{a,b}\in\BigPar{S_V\Par{U+W}}\times\Par{U+W},\exists\,\Par{u,w}\in U+W,\,a+b=a+u+w.$\parSol{}
%$\forall\Par{a,b}\in\BigPar{S_V\Par{U\cap W}}\times\Par{U\cap W},\,a+b=2\times\frac{\:1\:}{2}\times\Par{a+b}.$
%\SepLine
...
\Or Let $B_{V\apostrophe}=\Par{\varphi_1,\varphi_2}$ be corres to the std bss. Let $u_1=u_2=e_1+e_2\Rightarrow\Span{u_1,u_2}\subsetneq V.$\PfEnd
%\vspace{2pt}
%\ANote Supp each $\Span{u_k}=\Pure V\:\!\!\XSlash\null\varphi_k$ and $\varphi_k\Par{u_k}=1$ if $\varphi_k\neq0.$ Then each non0 $\varphi_k$
\SepLine
\ChEnd\pagebreak
