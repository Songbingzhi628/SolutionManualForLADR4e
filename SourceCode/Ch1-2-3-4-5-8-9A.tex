...
\Anchor{3ANLC}\ProblemBX[]{\NoteForSmall{Complex of Liney Maps}}{
	...
	For $S,T\in\Lm{V,W},$ $\Par{S+\lambda T}{_\Cbb}=S_\Cbb+\lambda T_{\!\Cbb}.$ For $S\in\Lm{V,W},T\in\Lm{U,V},$ $\Par{ST}{_\Cbb}=S_\Cbb T_{\!\Cbb}.$\TextA{}
...
}Asum $V\neq\zeroSubs$ and $\forall v\in V,\Par{v,Tv}$ is linely dep, then $\exists\,\lambda_v\in\Fbb,Tv=\lambda_v v.$\parSol{}
To prove $\lambda_v$ is indep of $v$, we discuss in two cases:\vspace{2pt}\parSol{}
\!\!\!$\MathRightBrace{l}{$($-$) If $\Par{v,w}$ is liney indep, $\lambda_{v+w}\Par{v+w}=T\Par{v+w}=Tv+Tw=\lambda_v v+\lambda_w w\\$($=$) Othws, supp $w=cv$, $\lambda_w w=Tw=cTv=c\lambda_v v=\lambda_v w\Rightarrow\Par{\lambda_w-\lambda_v}w}\Rightarrow \lambda_w=\lambda_v.$\vspace{4pt}\parSol{}
Now we prove the asum. Asum $\exists\,v\in V,\Par{v,Tv}$ is liney indep. Let $B_V=\Par{v,Tv,u_1,\dots,u_n}$.\parSol{}
Define $S\in\Lm{V}$ by $S\Par{av+bTv+c_1 u_1+\dots+c_n u_n}=bv\Rightarrow S\Par{Tv}=v=T\Par{Sv}=0.$\PfEnd\vspace{6pt}\quad
...
\vfill\ChDecl{Ch3B}{3$\cdot$B}{\quad{\small\textbf{注意\,:}\;这里我将3.D可逆性、同构部分前置。}}
...
\Anchor{3CN4e3.51}\ProblemBnoor[]{4E 3.51}[\Sbra]{
	...
	\Blind{Supp} $R\in\Fbb^{c,p}.$\:\: (b) For $j=1,\dots,m,$\; $\BigPar{CR}{_{j,\cdot}}=C_{j,\cdot}R_{\cdot,\cdot}=\sum_{r=1}^c C_{j,r}R_{r,\cdot}=C_{j,1}R_{1,\cdot}+\dots+C_{j,c}R_{c,\cdot}$\TextA{\vspace{0pt}}
}\SepLine

\Anchor{3C9}\BulletPointX\NoteForSmall{[3.52]}\;\;$A\in\Fbb^{m,n},c\in\Fbb^{n,1}\Rightarrow Ac\in\Fbb^{m,1}.$\hfill By \Sbra{4E 3.51(a)}, $\BigPar{Ac}{_{\cdot,1}}=c_1A_{\cdot,1}+\dots+c_nA_{\cdot,n}.$\Blind{\quad}\PfEnd\vspace{4pt}\quad
...
\Anchor{3C11}\BulletPointX\Exercise{11}\;\;$a\in\Fbb^{1,n},C\in\Fbb^{n,p}\Rightarrow aC\in\Fbb^{1,p}.$\hfill By \Sbra{4E 3.51(b)}, $\BigPar{aC}{_{1,\cdot}}=a_1C_{1,\cdot}+\dots+a_nC_{n,\cdot}.$\Blind{\quad}\PfEnd\vspace{4pt}\quad
...
\AExa $A={}${\small$\begin{pmatrix} 10 &\hspace{-4pt} 7 &\hspace{-4pt} 4 &\hspace{-4pt} 1 \\[-2pt] 26 &\hspace{-4pt} 19 &\hspace{-4pt} 12 &\hspace{-4pt} 5\\[-2pt] 46 &\hspace{-4pt} 33 &\hspace{-4pt} 20 &\hspace{-4pt} 7\end{pmatrix}$
...
\Anchor{3CT1}\ProblemBX{\TipsN{1}}{
	\TextA{Supp $p$ is a poly of $\,n\,$ variables in $\Fbb.$ Prove $\Mt[\BigPar]{p\Par{T_{\!1},\dots,T_{\!n}}}=p\BigPar{\Mt{T_{\!1}},\dots,\Mt{T_{\!n}}}.$}
...
%	\TextA{Prove the distr holds for matrix add and matrix multi. \large We prove the left distr.}
...
\Anchor{3E6}\ProblemBnoor{{3.E.6}}{
	\TextB{Supp $m\in\Nbp.$ Prove $V^m$ and $\Lm{\Fbb^m,V}$ are iso.\tgnr\FontNorm\hfill By (3.D.18, 3.E.4), immed.}
...
\Anchor{3D9}\ProblemN{9}{
	\TextA{Supp $U,V,W$ are iso and finide, $S\in\Lm{V,W},T\in\Lm{U,V},$ and $ST$ inv.}
...
Then $S\BigPar{\Par{TS}w}=ST\Par{Sw}=Sw\Rightarrow\Par{TS}w=w.$ \;\Or {\FontSmall$S^{-1}=T$ 又 $S=S\Rightarrow TS=S^{-1}S=I$.}\PfEnd
\SepLine
...
%	\TextA{Prove if $T\in\Lm{\Fbb^{n,1},\Fbb^{m,1}}$, then $\exists\,A\in\Fbb^{m,n},Tx = Ax,\,\forall x\in\Fbb^{n,1}$.}
...
\Anchor{3DT3}\ProblemBX[]{\TipsN{3}}{
	Identify $\Fbb^{m,n}$ with $\Lm{\Fbb^{n,1},\Fbb^{m,1}},$ due to $Tx=Ax;$ or with $\Lm{\Fbb^{1,n},\Fbb^{1,m}},$ due to $Tx=xA^t.$\TextB{}
	Details about the latter: $x\Mt{T}{^t}=\BigPar{\Mt{T}\Mt{x}{^t}}{^t}=\Mt{Tx}{^t}=\Mt{xA^t}{^t}=xA^t\Rightarrow\Mt{T}=A.$\TextB{}
	\ANote If we define $\Mt{v}=\Par{c_1\:\:\cdots\:\:c_n}.$Then [3,64]: $\Mt{T}{_{k,\cdot}}=\Mt{v_k}\Mt{T}=\Mt{Tv_k}.$\TextB{}
	\IndentNote Note that $A=\Mt[\BigPar]{T,\Par{v_1,\dots,v_n},\Par{w_1,\dots,w_m}}\in\Fbb^{n,m},$ and each $Tv_k=A_{k,1}w_1+\dots+A_{k,m}w_m.$\TextB{}
	\IndentNote [3.65]: $\Mt{Tv}=c_1\Mt{Tv_1}+\dots+c_n\Mt{Tv_n}=c_1\Mt{T}{_{1,\cdot}}+\dots+c_n\Mt{T}{_{n,\cdot}}=\Mt{v}{\Mt{T}}.$\TextB{}
	\IndentNote Exactly in trspose with the original. Now Exe (15): $T\in\Lm{\Fbb^{1,n},\Fbb^{1,m}}:Tx=xA.$\TextB{}
	\IndentNote Becs $Tx=\Mt{Tx}=\Mt{x}\Mt{T}=x\Mt{T}$ wrto std bses.\TextB{\vspace{-4pt}}
}\SepLine

\Anchor{3DT4}\ProblemBX[]{\TipsN{4}}{
	You must first declare bses and the purpose when using $\Mneg\!:\Fbb^{n,1}\mapsto v,$ or $\Fbb^{m,n}\mapsto\Lm{V,W}.$\TextB{\vspace{-4pt}}
}\SepLine

\Anchor{3DNE34e22}\BulletPointX\NoteForSmall{Exe (3, 4E 22)}\;\;$T\in\Lm{V,W}$ inv $\Longleftrightarrow\Mt{T}$ inv, wrto any $B_V,B_W.$\TextB{}
Supp $\Mt{T}$ wrto some $B_V,B_W$ is inv. Let $S\in\Lm{W,V}$ be suth $\Mt{S,B_W,B_V}=\Mt{T}{^{-1}}.$\TextB{}
$\Mt{TS,B_W}=I=\Mt{ST,B_V}.$ Apply $\Mneg.$ Now $S=T^{-1}\Rightarrow\Mt{T,B_V,B_W}{^{-1}}=\Mt{T^{-1},B_W,B_V}.$
\SepLine
...
\Anchor{3D'4}\ProblemB{
	\TextB{Supp $A\in\Fbb^{n,n}.$ Define $T,S\in\Lm{\Fbb^{n,n}}$ by $T\Par{X}=AX,\:S\Par{Y}=YA^t.$ Find $\dim\range ST.$\vspace{2pt}}
...
\Or By \TIPSN{3}. Define $\varphi\in\Lm[\BigPar]{\Lm{\Fbb^{n,1}}}:X\mapsto AX;$ \;$\psi\in\Lm[\BigPar]{\Lm{\Fbb^{n,1},\range A}}:Y\mapsto YA^t.$\par\quad
...
\Anchor{3FT1}\TipsN{1}\,\,\,The nota $W_V\upapostrophe=\Bra{\varphi\in V\apostrophe:\varphi=\varphi\circ\iota}=U^0$ is not well-defined \Sbra{without a bss}.\par
\IndentTipsN{1}Simply becs $W_V\upapostrophe$ have no info about the given $U.$ Here is an informal explanation:\par
\IndentTipsN{1}Each liney map $T\in\Lm{V,W}$ that vanishes on a given nontrivial $U$ has its '$P$'\par
\IndentTipsN{1}\Par{ though not uniq } suth '$U\oplus P=V$' with $T:P\mapsto\range T$ being surj.\par
\IndentTipsN{1}Hence $\forall W\in\Scom{V}{U},\,U^0=W_{V}'.$ But given nontrivial '$P$', the corres '$U$' is not uniq.\par
\IndentTipsN{1}Fix one $W_V\upapostrophe,$ then $U^0$ is not uniq, with each $U_k$ not equal to each other while each $U_k^0=W_V\upapostrophe.$\par\vspace{2pt}
...
\ProblemN[]{\Anchor{3F26}{26}}{
	Supp $V$ is finide, $\Omega$ is a subsp of $V\apostrophe.$ Then get a $B_{\Omega}$ and by {\TIPSN{2}} and Exe (4E 23), $\Omega=\BigPar{C^0\,\Omega}{^0}.$\TextB{}
...
\Anchor{3FT5}\BulletPointX\TipsN{5}\,\,\,Define $\Phi\in\Lm{V\apostrophe,V}:\varphi_k\mapsto v_k;\;\Psi\in\Lm{W,W\apostrophe}:w_j\mapsto\psi_j.$\TextB{}
\IndentTipsN{5}Define $T\in\Lm{V,W}$ suth $\Mt{T,B_V,B_W}=A.$ Let $S=\Phi T\apostrophe\Psi\Rightarrow\Mt{S,B_W,B_V}=A^t.$\vspace{2pt}\par
\Anchor{3FT6}\BulletPointX\TipsN{6}\,\,\,Define each $E_{j,k}\in\Lm{V,W}:v_x\mapsto\delta_{j,x}w_k,$ and each $\reflectbox{\textit{E}}{_{k,j}}\in\Lm{W\apostrophe,V\apostrophe}:\psi_x\mapsto\delta_{k,x}\varphi_j.$\TextB{}
\IndentTipsN{6}Note that each $E_{j,k}\!\!\upapostrophe\,\,\Par{\psi_x}=\psi_x\circ E_{j,k}=\delta_{k,x}\varphi_j=\reflectbox{\textit{E}}{_{k,j}}\Par{\psi_x}\Rightarrow E_{j,k}\!\!\upapostrophe\,\,=\reflectbox{\textit{E}}{_{k,j}}.$\vspace{1pt}\TextB{}
...
\ProblemN{\Anchor{3F4e24}\Anchor{3F6}{6}}{
	\TextA{Define \,$\Gamma: V\apostrophe\rightarrow\Fbb^m\,$ by $\,\Gamma\Par{\varphi}=\BigPar{\varphi\Par{v_1},\dots,\varphi\Par{v_m}},$ where $v_1,\dots,v_m\in V$.\vspace{2pt}}
...
\Anchor{3F4e25}\ProblemBnoor{{4E 25}}{
	\TextB{Define \,$\Gamma: V\rightarrow\Fbb^m\,$ by $\,\Gamma\Par{v}=\BigPar{\varphi_1\Par{v},\dots,\varphi_m\Par{v}},$ where $\varphi_1,\dots,\varphi_m\in V\apostrophe.$\vspace{2pt}}
	\TextB{Show {\tgnr\large(a)} $\Span{\varphi_1,\dots,\varphi_m}=V\apostrophe\Longleftrightarrow \Gamma$ inje. \, {\tgnr\large(b)} $\Par{\varphi_1,\dots,\varphi_m}$ liney indep $\Longleftrightarrow \Gamma$ surj.}
}Let $\Par{e_1,\dots,e_m}$ be the std bss of $\Fbb^m.$\par\quad
...
would give a poly with some non0 coeffs but infily many zeros. By {\TIPSN{1}}.\PfEnd\vspace{-3pt}
\SepLine
...
Becs $Tq=0\Longleftrightarrow q\Par{z_1}=\dots=q\Par{z_m}=q\Par{z_{m+1}}=0\Longleftrightarrow q=0,$ by {\TIPSN{1}}. \;Now $T$ iso. Immed.\PfEnd\vspace{4pt}\quad
...
\ProblemN{\Anchor{4O11}{11}}{
	\TextA{Supp $p\in\PoFi$ with $\deg p=m\in\Nbb.$ Let $U=\Bra{pq:q\in\PoFi}$. Find a bss of $\PoFi\XSlash U.$\vspace{1pt}}
...
}By {\TIPSN{1}} and Exe (5), $\exists\,!\,q\in\PoR{m}$ suth $q\Par{x_k}=p\Par{x_k}.$ Hence $p=q.$\PfEnd\vspace{11pt}\quad
...
%	\TextA{Supp $V$ is finide, $R,T\in\Lm{V}$ has same $\dim V$ eigvals $\lambda_1,\dots,\lambda_n.$}
...
\ProblemN{\Anchor{5A24}{24}}{
	\TextA{Supp $A\in\Fbb^{n,n}.$ Define $T\in\Lm{\Fbb^{n,1}}$ by $Tx = Ax$. Prove $\lambda$ is eigval of $T$ if\hspace{2pt}$:$}
...
\Or Define $S\in\Lm{\Fbb^{n,1}}$ by $Sx=A^tx.$ \;By (a), \Sbra{3.F \TIPSN{5}}, and Exe (15, 4E 15),\par\quad\Hb
...
\Anchor{5A'1}\ProblemB{
	\TextB{Supp $A\in\Fbb^{n,n}.$ Define $T\in\Lm{\Fbb^{1,n}}$ by $Tx = xA.$ Prove $\lambda$ is eigval of $T$ if\hspace{2pt}$:$}
...
\Or Define $S\in\Lm{\Fbb^{1,n}}$ by $Sx=xA^t\Rightarrow S=\Phi{^{-1}}T\apostrophe\Phi.$ \;Simlr and by \Sbra{3.D \TIPSN{3}}.
...
\ChDecl{Ch5BI}{5.B}{\quad\footnotesize$\hText{$
	(I)覆盖4e的5.B节全部、3e前半部分与之相关的所有习题。(II)覆盖3e的5.B节后半部分「上三角矩阵」和4e的5.C节。$\\$
	{\small\textbf{注意\,:}\;4e的5.B节和3e的8.C节、9.A节许多结论和习题有交集。5.B(II)的题号使用4e的5.C节.}$}$
}
\vspace{4pt}
...
}\ANote Another proof of [4E 5.22] by Exe (I.17).
...
\Anchor{5BI4e16}\Anchor{8C18}\ProblemBnoor{8.C.18 \OR 4E I.16}{
...
\Anchor{5BI4e14}\ProblemBnoor{4E I.14}{
	...
	\TextA{Give a repres of $s,$ the min of $T^{-1}.$\vspace{2pt}}
...
\Anchor{5BII4e2}\ProblemN{II.2}{
	\TextA{Supp $A,B\in\Fbb^{n,n}$ are up-trig with each $A_{p,p}=\alpha_p,B_{p,p}=\beta_p.$}
...
\vfill\ChDecl{Ch5C}{5.C \& [4E] 5.D}{\qquad{\small\textbf{注意\,:}\;这一节的题号主要使用第四版5.D节.}}
...
$\exists\,\Bra{T_{\!1},\dots,T_{\!m}}\subseteq\mE$ with each elem of $\mE$ in $\Span{T_{\!1},\dots,T_{\!m}}$ and $m\leqslant n^2.$\par\quad
For each $T_{\!k},$ $V=\bigoplus_{\lambda_k\!\!}E\Par{\lambda_k,T_{\!k}}$ and $E\Par{\lambda_k,T_{\!k}}$ non0 for finily many $\lambda_k\in\Fbb.$\vspace{2pt}\par\quad
...
\ProblemBX[]{\NewNotation}{
	\TextA{$m_T$ denotes the min poly of $T.$\vspace{-4pt}}
}\SepLine
...
\Anchor{8A4e24}\Anchor{8N8.19}\ProblemBX[]{\NoteForSmall{[8.19] \OR [4E 8.18]}}{
	...
	If $m=\dim V.$ Then $B_V=\Par{T^{m-1}v,\dots,Tv,v}.$ Let each $w_k=T^{m-k}v.$ Then $Tw_1=0,T\Par{w_k}=w_{k-1}.$\TextB{\vspace{-2pt}}
}\SepLine
...
\Anchor{8A4e4}\ProblemBnoor{4E A.4}{
	\TextA{Supp $T\in\Lm{V},\lambda\in\Fbb,$ and $m_T$ is a multi of $\Par{z-\lambda}{^m}$ with $m\in\Nbp.$}
...
Where $B_V=\Par{v_1,\dots,v_n}$ are gigvecs and $v_j\in G_k.$ Let $D=\lambda_1P_{\!\!\!\:1}+\dots+\lambda_mP_{\!\!\!\:m}:v_j\mapsto\lambda_kv_j.$\parSol{}
...
Becs the char of $T$ \Sbra{See [9.21] for $\Fbb=\Rbb$} is a multi of $m_T,$ which is a multi of $s.$\PfEnd
\SepLine

\Anchor{8B4e6}\ProblemBnoor{4E B.6}{
	\TextA{Supp $T\in\Lm{V}$ and $\lambda$ is an eigval. Explain why the expo of $\Par{z-\lambda}$}
...
\Or Becs all $\Par{T-\alpha I}{^k}\Big|{_{\:\!G\SmallPar{\:\!\lambda,\,T\:\!}}}$ are inv. Note that $m_T\Par{T\mmid_{\:\!G\SmallPar{\:\!\lambda,\,T\:\!}}}=0\Longleftrightarrow\Par{T-\lambda I}{^k}\Big|{_{\:\!G\SmallPar{\:\!\lambda,\,T\:\!}}}=0.$\parSol{}
So that the expo $k$ of $\Par{z-\lambda}$ is no lesser, and cannot be more, than the smallest $m.$\PfEnd\vspace{4pt}\parSol{}
...
If $q\Par{T}v=0\Rightarrow s\Par{T}v=0.$ Othws, $q\Par{T}v\in\Null\Par{T-\lambda I}{^m}=\Null\Par{T-\lambda I}{^k}\Rightarrow s\Par{T}v=0.$\PfEnd\vspace{2pt}
\ANote The expo of an irreducible $q\Par{z}$ in the factoriz of $m_T$ is the smallest $m$ suth $q^m\Par{T}\Big|{_{\nullp q\SmallPar{T}}}=0.$
\SepLine

\Anchor{8C12}\ProblemN{C.12}{
	\TextA{Supp $\Fbb=\Cbb,$ $T\in\Lm{V},$ and $m_T$ has no repeated zeros. Prove $T$ diag. \hfill\tgnr\FontNorm By (4E B.6).}
}Supp $m_T\Par{z}=\Par{z-\lambda_1}\cdots\Par{z-\lambda_m}$ for disti $\lambda.$ Let $G_j=G\Par{\lambda_j,T}$ invard $T.$\parSol{}
Fix one $j.$ Supp $v\in G_j$ and $u=\Par{T-\lambda_j}v.$ We show $u=0,$ then $v\in E\Par{\lambda_j,T}.$\parSol{}
Let $q_j\Par{z}=\prod_{i\neq j}^m\Par{z-\lambda_i}.$ Then $q_j\Par{T}\Par{T-\lambda_jI}v=q_j\Par{T\mmid_{G_j}}u.$ \,又 Each $\Par{T\mmid_{G_j}-\lambda_kI}$ inv.\PfEnd
\SepLine
...
\Anchor{8B'1}\ProblemB{
	\TextA{Supp $T\in\Lm{V},$ and $W\oplus E\Par{\lambda,T}=G\Par{\lambda,T}.$ Prove $W$ is not invard $T.$}
...
}By Exe (4E 4.13), $\forall v\in V,v=u+w,$ where $u=s\Par{T}a\Par{T}v,\,w=q\Par{T}b\Par{T}v.$ Immed.\PfEnd\vspace{2pt}\parSol{}
...
%\ANote Supp $\Fbb=\Rbb.$ Let $m_T\Par{x}=\Par{x-\lambda_1}{^{\delta_1}}\cdots\Par{x-\lambda_m}{^{\delta_m}}\Par{z^2+b_1z+c_1}{^{\epsilon_1}}\cdots\Par{z^2+b_Mz+c_M}{^{\epsilon_M}},$ where each $b_j^2<4c_j.$\parNot
...
%}See Exa [5.43]: $V=\Cbb^2,B_U=\Par{e_1}.$ Define $Te_1=0,Te_2=e_1\Rightarrow G\Par{0,T}=V.$
...
\Anchor{8N8.55}\ProblemBX[]{\NoteForSmall{[8.55]}}{
	Another proof: Supp $N^m=0\neq N^{m-1}.$ Let each $\null N^{k+1}=\null N^k\oplus U_k.$\TextB{}
	(1) $\null N^{m-1}\oplus U_{m-1}=\null N^m=\Par{\null N^{m-2}\oplus U_{m-2}}\oplus U_{m-1}.$\TextB{}
	(2) $\null N^{m-j}\oplus U_{m-j}=\null N^{m-j+1}.$\, (3) $\null I\oplus U_0=\null N.$\TextB{}
	(1) Let $B_{U_{m-1}}=\Par{v_{1,1},\dots,v_{n_1,1}}.$\, Let $B_{U_{m-2}}=\Par{Nv_{1,1},\dots,Nv_{n_1,1},v_{1,2},\dots,v_{n_2,2}}.$\TextB{}
	(2) Let $B_{U_{m-j}}=\Par{N^{j-1}v_{1,1},\dots,N^{j-1}v_{n_1,1},\dots,Nv_{1,j-1},\dots,Nv_{n_{j-1},j-1},v_{1,j},\dots,v_{n_j,j}}.$\TextB{}
	(3) Let $B_{U_0}=\Par{N^{m-1}v_{1,1},\dots,N^{m-1}v_{n_1,1},\dots,Nv_{1,m-1},\dots,Nv_{n_{m-1},m-1},v_{1,m},\dots,v_{n_m,m}}.$ Sum up and done.\PfEnd\TextB{\vspace{-2pt}}
}\SepLine

%\Anchor{8C4e12}\ProblemBnoor{4E C.12}{
%	\TextA{Supp $T\in\Lm{V}$ diag. Show $\Mt{T}$ diag wrto any Jordan $B_V.$}
%}Each $v_k$ of a Jordan $B_V$ is a gigvec; so is an eigvec, by Exe (B.5).\PfEnd\vspace{2pt}\parSol{}
%\Or Let $A$ be a Jordan block diag matrix of $T.$ By Exe (D.3) and [4E 5.62].\PfEnd
%\SepLine

\Anchor{8D8}\ProblemN{D.8}{
	\TextA{Supp $\Fbb=\Cbb,$ and $T\in\Lm{V}.$}
	\TextA{Prove [P] $\nexists$ non0 invarsps $U,W$ suth $U\oplus W=V\Longleftrightarrow m_T\Par{z}=\Par{z-\lambda}{^{\dim V}}.$ [Q]}
...
\Blind{$Q\Rightarrow P:$ \;}Thus the only Jordan block of $\Mt{N}$ is $\Mt{N}$ itself.\vspace{3pt}\par\quad
${}{^\neg}P\Rightarrow{}{^\neg}Q:$ \;If $\exists$ two or more eigvals of $T\mmid_U$ or $T\mmid_W,$ then $m_T$ has two or more disti factors, done.\par\quad
\Blind{${}{^\neg}P\Rightarrow{}{^\neg}Q:$ \;}Now supp $\exists$ only one eigval $\lambda$ for $T\mmid_U,$ $T\mmid_W,$ and $T.$ Supp $m_T\Par{z}=\Par{z-\lambda}{^m}.$\par\quad
\Blind{${}{^\neg}P\Rightarrow{}{^\neg}Q:$ \;}Let $M=\max\!\Bra{\dim U,\dim W}.$ Let $S=\Par{T-\lambda I}{^M}\Rightarrow\null S\mmid_U\oplus\null S\mmid_W=\null S.$\par\quad
\Blind{${}{^\neg}P\Rightarrow{}{^\neg}Q:$ \;}Becs $G\Par{\lambda,T\mmid_U}=U,\,G\Par{\lambda,T\mmid_W}=W,\,G\Par{\lambda,T}=V,\,\Par{T-\lambda I}{^M}=0.$ Now by Exe (4E B.6).\vspace{2pt}\par\quad
\Blind{${}{^\neg}P\Rightarrow{}{^\neg}Q:$ \;}\Or Becs $\exists$ Jordan $\Mt{T\mmid_U},\Mt{T\mmid_W}\Rightarrow$ Jordan $\Mt{T}.$ Consider $z^{M+1}$ by Exe (D.3).\vspace{4pt}\par\quad
...
By \Sbra{8.B \TIPSN{3}}, $V_{\!\Cbb}=\Null\Par{T_{\!\Cbb}-\lambda I}{^k}\oplus\Null\Par{T_{\!\Cbb}-\overline{\lambda}I}.$ By [9.17] and [9.4].\PfEnd\vspace{4pt}
\ANote Let $Q\Par{\lambda,T}=\nullp q\Par{T}{^{\dim V}}.$ Then by (4E 8.B.6,7) for $T_{\!\Cbb},$ by [9.10,20], and by \Sbra{8.B \TIPSN{3}},\parNot
