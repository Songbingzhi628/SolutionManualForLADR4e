...
(b) Get a $B_U,$ apply $S,$ then extend to $B_V,B_W.$\PfEnd\vspace{3pt}
...
%	You must first declare bses and the purpose when using $\Mneg\!:\Fbb^{n,1}\mapsto v,$ or $\Fbb^{m,n}\mapsto\Lm{V,W}.$\TextB{\vspace{-3pt}}
%}\SepLine
...
Each $\mX_{\!j,k}=C_{1,k}\mC_{\!j,1}+\dots+C_{n,k}\mC_{\!j,n}=C_{1,j}\Par{\mC_{\!k,1}}{^t}+\dots+C_{n,j}\Par{\mC_{\!k,n}}{^t}.$\PfEnd
\SepLine
...
\hfill$\Rightarrow\Lm{V,W}=\spn R+\mE.$\Blind{\quad}\PfEnd\vspace{10pt}\par
\SepLine\pagebreak
