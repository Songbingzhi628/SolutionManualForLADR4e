...
\Anchor{5BIN4e5.33}\ProblemBX{\NoteForSmall{[4E 5.33]}}{
	...
	\TextB{Prove each $\dim\Null\BigPar{T^2+bT+cI}{^j}$ is even for $j\in\Nbp.$}
...
\Anchor{5E3}\ProblemN[]{3}{
	$S,T\in\Lm{V}$ commu, $p\in\PoFi\Rightarrow Tp\Par{S}=p\Par{S}T\Rightarrow\nullp p\Par{S},\rangep p\Par{S}$ invard $T,$ by (5.A.2,3).\PfEnd\TextA{\vspace{-4pt}}
}\SepLine
...
\ChDecl{Ch8A}{8.A}{\quad{\FontNorm\ANote $V$ denotes a finide non0 vecsp over $\Fbb.$}}

\vspace{4pt}

\Anchor{8A3}\ProblemN{3}{
	\TextA{Supp $T\in\Lm{V}$ inv. Prove $G\Par{\lambda,T}=G\Par{\lambda^{-1},T^{-1}}$ for any non0 $\lambda\in\Fbb.$}
}$\Par{T-\lambda I}{^j}v=0=\sum_{i=0}^j\mathC_j^i\Par{-\lambda}{^{j-i}}T^iv.$ Apply $\Par{-\lambda}{^{-j}}T^{-j}$ to both sides. $\Par{T^{-1}-\lambda^{-1}}{^j}v=0.$\parSol{}
\Or\PfEnd
\SepLine
...
\Anchor{8A4e24}\ProblemBX[]{\NoteForSmall{[8.19] \OR [4E 8.18]}}{
...
\Anchor{8A6}\ProblemN{6}{
	\TextA{Supp $T\in\Lm{V},n=\dim V,T^n=0\neq T^{n-1}.$ Prove $\nexists\,S\in\Lm{V}$ suth $S^k=T$ for all $k>1.$}
...
\Anchor{8A4e3}\ProblemBnoor{4E 3}{
	\TextA{Supp $T\in\Lm{V}.$ Prove $V=\null T\oplus\range T\Longleftrightarrow\null T^2=\null T.$}
}(a) $\null T^2=\null T=\null T^{\dim V}\Rightarrow\dim\range T^{\dim V}=\dim\range T.$\parSol{\Ha}
\Or $\forall v\in\null T\cap\range T,\exists\,u\in V,Tu=v,T^2u=Tv=0\Longleftrightarrow u\in\null T^2=\null T.$\parSol{\vspace{2pt}}
(b) $V=\null T\oplus U,U=\range T,$ 又 $\dim\null T^2=\dim\null T+\dim\null T\mmid_{\range T}.$\PfEnd
\SepLine

\Anchor{8A4e4}\ProblemBnoor{4E 4}{
	\TextA{Supp $T\in\Lm{V},\lambda\in\Fbb,$ the min of $T$ is a multi of $\Par{z-\lambda}{^m}$ for $m\in\Nbp.$}
	\TextA{Prove $\dim\Null\Par{T-\lambda I}{^m}\geqslant m.$}
...
$\dim\Null\Par{T-\lambda I}\Par{T-\lambda I}{^{m-1}}=\dim\Null\Par{T-\lambda I}\Big|{_{\Null\SmallPar{T-\lambda I}{^{m-1}}}}+\uline{\dim\Null\Par{T-\lambda I}{^{m-1}}}.$\PfEnd
\SepLine

\Anchor{8A17}\ProblemN{17}{
	\TextA{Supp $T\in\Lm{V},m\in\Nbb,\range T^m=\range T^{m+1}.$ Show $\range T^m=\range T^{m+1}=\cdots.$}
}By Exe (19), $\null T^m=\null T^{m+1}=\cdots\Rightarrow\dim\range T^m=\dim\range T^{m+1}=\cdots.$\PfEnd\parSol{}
\Or Supp $w=T^{m+k}v.$ Then becs $T^mv\in\range T^{m+1},\exists\,T^{m+1}u=T^mv.$ Thus $w=T^{m+k+1}u.$\PfEnd
\SepLine

\Anchor{8A18}\ProblemN{18}{
	\TextA{Supp $T\in\Lm{V},\dim V=n.$ Show $\range T^{n}=\range T^{n+1}=\cdots.$}
}By Exe (19), becs $\null T^{\dim V}=\null T^{\dim V+1}=\cdots.$ Simlr.\PfEnd\parSol{}
\Or Asum $\range T^{n}\supsetneq\range T^{n+1}.$ By Exe (17), $V=\range T^0\supsetneq\range T\supsetneq\cdots\supsetneq\range T^{n+1}.$\parSol{}
Now each $\dim\range T^{k+1}\leqslant\dim\range T^k-1\Rightarrow\dim\range T^{n+1}\leqslant\dim\range T^0-\Par{n+1}.$\PfEnd
\SepLine\pagebreak

\Anchor{8A4e16}\ProblemBnoor{4E 16}{
	\TextA{Supp $S,T\in\Lm{V}$ nilp and commu. Prove $S+T,ST$ are nilp}
}By [4E 5.80], $\exists\,B_V$ suth $S,T$ up-trig \Par{with only $0$'s on diags}. By (4E 5.C.2).\PfEnd
\ANote If $S,T$ not commu. Then generally $\Par{ST}{^k}\neq S^kT^k.$
\SepLine

\Anchor{8A4e11}\ProblemBnoor{4E 11}{
	\TextA{Supp $T\in\Lm{V}.$ Prove $\exists\,B_V$ of gigvecs of $T\Longleftrightarrow$ the min of $T$ is $\Par{z-\lambda_1}\cdots\Par{z-\lambda_m}.$}
}
\SepLine

\Anchor{8A4e15}\ProblemBnoor{4E 15}{
	\TextA{Supp $\Fbb=\Cbb,T\in\Lm{V}.$ Prove $T$ diag $\Longleftrightarrow$ each gigvec is an eigvec.}
}
\SepLine
...
\ChDecl{Ch9A}{9.A}{\quad{\FontNorm\ANote $V$ denotes a finide non0 vecsp over $\Fbb.$}}

\vspace{4pt}

\Anchor{9A13}\ProblemN{13}{
	\TextA{Supp $\Fbb=\Rbb,T\in\Lm{V},$ and $b^2<4c$ for $b,c\in\Fbb.$}
	\TextA{Prove each $\dim\Null\BigPar{T^2+bT+cI}{^j}$ is even for $j\in\Nbp.$}
}
\SepLine
...
}Supp $V\neq\zeroSubs.$ Let $2N=\dim V$ as real vecsp. We construct a real $B_V$ via a $\Par[0pt]{N-1}$\hspace{1pt}-\hspace{1pt}step process.\par\quad
...
\Anchor{9A18}\ProblemN{18}{
	\TextA{Supp $\Fbb=\Rbb,$ and $T\in\Lm{V}.$}
	\PrePa\TextA{Show all eigvals of $T_{\!\Cbb}$ are real $\Rightarrow\exists\,B_V$ suth $T$ up-trig.}
	\PrePb\TextA{Show all eigvals of $T_{\!\Cbb}$ are real $\Rightarrow\exists\,B_V$ of geigvecs of $T.$}
}(a) Using induc on $\dim V.$ (i) Immed. (ii) $\dim V>1.$ Asum it holds for smaller $V.$\parSol{\Ha}
Supp all eigvals of $T_{\!\Cbb}$ are real. Let $U=\Range\Par{T-\lambda I}.$\parSol{\Ha}
