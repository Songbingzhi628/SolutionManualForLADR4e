...
%	\TextA{Give a non-trivial $U\subseteq\RbbP{2},$ $U$ is}
%	\PrePa\TextA{closd taking add invs and add, but is not a subsp of $\RbbP{2}.$\hfill\tgnr\FontNorm Let $U=\Zbb^2$ or $\Qbb^2,$ with $0\in U.$}
%	\PrePb\TextA{scalar multi, but is not a subsp of $\RbbP{2}.$\hfill\tgnr\FontNorm Let $U=\Bra{\Par{x,y}\in\RbbP{2}:x=0\vee y=0}.$}
%}\SepLine
...
}Becs the subset $V_{\!1}$ of vecsp $V_{\!2}$ is closd add and scalar multi, $V_{\!1}$ is a subsp of $V_{\!2}.$\parSol{}
...
}Equiv to $\Par{v_1,\dots,v_m,w}$ liney dep $\Longleftrightarrow w\in\Span{v_1,\dots,v_m}.$ By [2.21], immed.\PfEnd
...
${}^\neg P\Rightarrow{}^\neg Q:\;$ Supp $V$ is finide and $V=\Span{w_1,\dots,w_m}$.\parSol{}
...
\ProblemN{\Anchor{2A17}{17}}{
	\TextA{Prove $\Par{p_0,p_1,\dots,p_m}$ cannot be liney indep in $\PoF{m}$ with each $p_k\Par{2}=0.$\vspace{-3pt}}
}\par\quad
Define $p\Par{z}=z.$ Becs if $z=2,$ then $\forall a_i\in\Fbb,\,p\Par{z}\neq \sum_{i=0}^ma_i\:\!p_i\Par{z}.$ Now $p\not\in\Span{p_0,p_1,\dots,p_m}$.\par\quad
Asum $\Par{p_0,p_1,\dots,p_m}$ liney indep. Then $\Par{p_0,p_1,\dots,p_m,p}$ of len $\Par{m+2}$ is liney indep,\par\quad
while $\Par{1,z,\dots,z^m}$ of len $\Par{m+1}$ spans $\PoF{m}.$ Ctradic by [2.23].\PfEnd\vspace{3pt}\par\quad
\Or Becs $\Par{1,z,\dots,z^m}$ of len $\Par{m+1}$ spans $\PoF{m}.$ Then $\Par{p_0,p_1,\dots,p_m,z}$ of len $\Par{m+2}$ is liney dep.\par\quad
As shown above,  $z\not\in\Span{p_0,p_1,\dots,p_m}.$ By the {\NOTE}\;\!(a) in Exe\;\!(11).\PfEnd
\SepLine
\ChEnd
...
%}A countexa\hspace{1pt}: Let $V=\RbbP{4}$ and $B_V=\Par{e_1,e_2,e_3,e_4}$ be std bss.\par\quad
%Let $v_1=e_1,v_2=e_2,v_3=e_3+e_4,v_4=e_4.$ Then $\Par{v_1,\dots,v_4}$ is a bss of $\RbbP{4}.$\par\quad
...
$\Par{v_1,\dots,v_m}$ liney indep $\Rightarrow$ $\BigPar{v_1,v_2-v_1,\dots,v_m-v_1}$ liney indep $\Rightarrow$ $\underbrace{\BigPar{v_2-v_1,\,\dots,\,v_m-v_1}}_{\text{of len }\SmallPar{m-1}}$ liney indep.\vspace{-15pt}\par\quad
...
\Anchor{2C4e16}\ProblemBnoor{{4E 16}}{
	...
	\TextA{Prove $\exists\,\Par{n-m}$ subsps  $U_1,\dots,U_{n-m}$, each of dim $\Par{n-1}$, suth $\bigcap_{i=1}^{n-m}U_i=U$.}
}Let $B_U=\Par{v_1,\dots,v_m},\;B_V=\BigPar{v_1,\dots,v_m,u_1,\dots,u_{n-m}}$.\parSol{}
Define each $U_i=\Span[\BigPar]{v_1,\dots,v_m,u_1,\dots,u_{i-1},u_{i+1},\dots,u_{n-m}}\Rightarrow U\subseteq U_i.$\parSol{}
$\forall v\in \bigcap_{i=1}^{n-m}U_i,v=v_0+b_1 u_1+\dots+b_{n-m} u_{n-m}\in U_i\Rightarrow$ each $b_i=0\Rightarrow v\in U.$\PfEnd
%\vspace{10pt}
...
\Anchor{3E1}\ProblemBnoor[]{3.E.1}{
	\TextB{A function $T:V\rightarrow W$ is liney $\Longleftrightarrow$ The graph of $T$ is a subsp of $V\times W.$\vspace{-6pt}}
}\SepLine
...
\Or \Sbra[3pt]{{\tgsl Req Finide}} \;Define by $T\BigBigPar{{\sum_{i=1}^m a_i u_i}}=\sum_{i=1}^n a_i Su_i.$ \,Let $B_V=\Par{\overbrace{u_1,\dots,u_n}^{B_U},\dots,u_m}.$\PfEnd
\SepLine
...
\Anchor{3AT2}\ProblemBX[]{\TipsN{2}}{
	...
	\Blind{$T\in\Lm{V,W}.$}\hspace{3.5pt}(b) If $U$ is a subsp of $V.$ Then $U\subseteq\null T\Longleftrightarrow T\mmid_U=0.$\TextA{\vspace{-2pt}}
}\SepLine
...
%	\TextA{Give a map $\varphi:\RbbP{2}\rightarrow\Rbb$ suth $\forall a\in\Rbb,v\in\RbbP{2},\varphi\Par{av} = a\varphi\Par{v}$ but $\varphi$ is not liney.\vspace{4pt}}
...
{Choose a non0 $w\in W.$ By asum, for {$\BigPar{\overline{a_1}w,\dots,\overline{a_m}w},\exists\,T\in\Lm{V,W},$ each $Tv_k=\overline{a_k}w.$}}\vspace{2pt}\par\quad
...
Define $S\in\Lm{V}$ by $S\Par{a\:\!v+b\:\!Tv+c_1 u_1+\dots+c_n u_n}=b\:\!v\Rightarrow S\Par{Tv}=v=T\Par{Sv}=0.$\parSol{}
...
\!\!\!$\MathRightBrace{l}{$($-$) If $\Par{v,w}$ is liney indep, $\lambda_{v+w}\Par{v+w}=T\Par{v+w}=Tv+Tw=\lambda_v v+\lambda_w w\\$($=$) Othws, let $w=cv$, \,$\lambda_w w=Tw=cTv=c\lambda_v v=\lambda_v w\Rightarrow\Par{\lambda_w-\lambda_v}\;\!w=0}\Rightarrow \lambda_w=\lambda_v.$\PfEnd\vspace{12pt}\quad
...
\Anchor{3A4e17}\ProblemBnoor{{4E 17}}{
	...
	\TextB{{\FontNorm A subsp $\mE$ of $\Lm{V}$ is called a two-sided ideal of $\Lm{V}$ if $TE\in\mE,ET\in\mE$ for all $E\in\mE$ and all $T\in\Lm{V}.$}}
...
\Anchor{3B4e32}\ProblemBnoor{{4E 3.B.32}}{
	\TextA{Supp $\dim V=n.$ and $\varphi:\Lm{V}\rightarrow\Fbb$ is liney.}
...
{Let $k\neq i,\:\!j\neq l\Rightarrow$ {\Large\vspace{4pt}$\varphi\Par{R_{i,\;\!j}\circ R_{l,\;\!k}}=\varphi\Par{R_{l,\;\!k}\circ R_{i,\;\!j}}=\varphi\Par{0}=0=\varphi\Par{R_{l,\;\!k}}\cdot\varphi\Par{R_{i,\;\!j}}$}}\parSol{}
...
%\ProblemN{\Anchor{3B31}{31}}{
	%	\TextA{Prove $\exists\,T_{\!1},T_{\!2}\in\Lm{\RbbP{5},\RbbP{2}},\null T_{\!1}=\null T_{\!2}$ and $T_{\!1}\neq cT_{\!2},\forall c\in\Fbb$.}
	%}Let $B_{\RbbP{5}}=\Par{v_1,\dots,v_5},B_{\RbbP{2}}=\Par{w_1,w_2}.$ Define $T,S\in\Lm{V,W}$ by\parSol{\vspace{2pt}}
...
(3) $\dim U=\dim V\Rightarrow\dim\null ST\geqslant\dim V-\min\!\Bra{{\dim\range S,\dim\range T}}$\parCor
\Blind{(3) $\dim U=\dim V\Rightarrow\dim\null ST$}${}=\max\!\Bra{{\dim V-\dim\range S,\dim V-\dim\range T}}.$\par\vspace{4pt}
\AExa Let $U=W=\Rbb,V=\RbbP{2}.$ Define $T\in\Lm{\Rbb,\RbbP{2}}:x\mapsto\Par{x,0},$ and $S\in\Lm{\RbbP{2},\Rbb}:\Par{x,y}\mapsto ax,a\neq0.$\parExa
...
\ProblemN{\Anchor{3B30}{30}}{
	\TextA{Supp non0 $\varphi,\beta\in\Lm{V,\Fbb}$ and $\null\varphi=\null\beta=\eta.$ Prove $\exists\,c\in\Fbb,\varphi=c\beta.$}
}Let $u$ be suth $\varphi\Par{u}\neq 0,$ equiv to $\beta\Par{u}\neq 0.$ Then $V=\null\varphi\oplus\Span{u}=\null\beta\oplus\Span{u}.$\parSol{}
\hspace{-5pt}$\hText{$
...
\Anchor{3C4e16}\ProblemBnoor{4E 16}{
	\TextB{Supp $A\in\FbbP{m,\:\!n}\nonzero.$ Prove $\rank A=1\Rightarrow\exists\,c_j,d_k\in\Fbb,$ each $A_{j,\;\!k}=c_j\;\!d_k.$\vspace{6pt}}
...
{$\Rightarrow A_{j,\;\!k}=d_k'\:\!A_{j,1}=c_j\:\!A_{1,\;\!k}=c_j\;\!d_k'\;\!A_{1,1}=c_j\;\!d_k,$ where $d_k=d_k'\:\!A_{1,1}.$}\quad\Or By CR Factoriz, immed.\PfEnd
\SepLine
...
Let $v_1=\Frac{u_k}{\:A_{1,\;\!k}\:}\Rightarrow Tv_1=1w_1+\Frac{\:A_{2,\;\!k}\:}{A_{1,\;\!k}}w_2+\dots+\Frac{\:A_{n,\;\!k}\:}{A_{1,\;\!k}}w_n.$\;$\MathLeftMid{l}{$
	\!\!Let $v_{j+1}=u_{j}-A_{1,\;\!j}\:\!v_1$ for $j\in\;\!\!\Bra{1,\dots,k-1}.\\$
...
Equiv to $\exists\,B_V$ \Sbra{by (3.F.31)} suth $\Mt{T,B_V,B_W}{_{1,\;\!\cdot}}={\small\begin{pmatrix}1&0&\cdots&0\end{pmatrix}}$ or ${\small\begin{pmatrix}0&\cdots&0\end{pmatrix}}.$\PfEnd
\SepLine\pagebreak
...
}(a) Becs $\mM\!:\,Tv_k\mapsto A_{\cdot,\;\!k}$ is an iso of $\spn L$ onto $\spn L_{\mM}.$ By (3.B.9).\parSol{}
...
(b) Each $\Mt{Tv_{\:\!\!\alpha_i}}=A_{\cdot,\,\alpha_i}$\par\quad\Hb
...
\Anchor{3D'2}\ProblemB{
	...
	\TextB{Prove $\exists\,X,Y\in\Lm{V}$ suth $AX+BY=C$ and $BX+AY=D.$}
...
}$\forall u\in U,\exists\,!\,\Par{w,x_1}\in W\times X,\,u=w+x_1.$ Thus $\pi:U\rightarrow W$ well defined by $\pi\Par{u}=w.$\parSol{}
While $\exists\,!\,\Par{u\apostrophe,x_2}\in U\times X,\,w=u\apostrophe+x_2.$ \,Now $x_1=-x_2,\:u=u\apostrophe\Longrightarrow\pi$ inje and surj.\parSol{\vspace{3pt}}
\Or 又 $\forall w\in W,\exists\,!\,\Par{u,x_1}\in U\times X,\,w=u+x_1.$ Thus $\xi:W\rightarrow U$ well defined by $\xi\Par{w}=u.$\parSol{}
While $\exists\,!\,\Par{w\apostrophe,x_2}\in W\times X,\,u=w\apostrophe+x_2.$ Now $x_1=-x_2,\:w=w\apostrophe\Longrightarrow\xi\circ\pi=I,$ and $\pi\circ\xi=I.$\PfEnd\vspace{4pt}
\AExa Let $V=X=\FbbP{\infty},Y=\Bra{\Par{0,x_1,x_2,\cdots}\in\FbbP{\infty}}\Rightarrow X,Y$ iso.\parExa
...
\ProblemB[]{
	\TextB{Supp $U,V,W$ are finide.\hfill\FontSmall\tgnr\ANote Not true if $U,V,W$ infinide. \;\AExa Forwd and backwd shift.}
}
\Anchor{3D9}\ProblemN{9}{
	\TextA{Supp $S\in\Lm{V,W},T\in\Lm{U,V},$ and $ST$ inv. Prove $S,T$ are inv.}
...
\ProblemN{\Anchor{3D10}{10}}{
	\TextA{Supp $\dim V=\dim W,$ and $T\in\Lm{V,W},S\in\Lm{W,V}.$ Prove $ST=I\Longleftrightarrow TS=I.$}
}$ST=I\Rightarrow S,T$ inv. Let $B_V=\Par{v_1,\dots,v_n}\Rightarrow$ each $S\Par{Tv_i}=v_i.$ Thus $TS=I.$ \,Rev the roles.\PfEnd\parSol{}
...
%\AExa $V=\RbbP{\infty},S\Par{a_1,a_2,\cdots}=\Par{a_2,\cdots};\;T\Par{a_1,a_2,\cdots}=\Par{0,a_1,a_2,\cdots};\;U=I\Rightarrow STU=I$ but $T$ is not inv.
%\SepLine
...
\Anchor{3DNE34e22}\BulletPointX\NoteForSmall{Exe (4E 3, 4E 22)}\;\;$T\in\Lm{V,W}$ inv $\Longleftrightarrow\Mt{T}$ inv, wrto any\Par{$\Leftrightarrow$ some} $B_V,B_W.$\TextB{}
Supp $\Mt{T}$ wrto some $B_V,B_W$ is inv. Let $S\in\Lm{W,V}$ be suth $\Mt{S,B_W,B_V}=\Mt{T,B_V,B_W}{^{-1}}.$\TextB{}
Then $\Mt{TS,B_W}=\Mt{T,B_V,B_W}\Mt{S,B_W,B_V}=I_W.$ \,Apply $\Mneg,$ we get $S=T^{-1}.$\vspace{2pt}\TextB{}
...
\Or Let $B_{\range S}=\Par{v_1,\dots,v_r}$ with each $v_i=Su_i.$ Let $B_V=\Par{v_1,\dots,v_m},B_{\null S}=\Par{u_{r+1},\dots,u_n}.$\par\quad
...
}Becs $A\mEnt{j,\;\!k}=\BigSbra{{\sum_{x=1}^nA_{x,\;\!j}\;\!\mEnt{x,\;\!j}}}\mEnt{j,\;\!k}=\sum_{x=1}^nA_{x,\;\!j}\;\!\mEnt{x,\;\!k}.$ \;Let $B_{\col A}=\Par{C_{\cdot,1},\dots,C_{\cdot,\;\!r}}.$\vspace{3pt}\parSol{}
Each $A_{\cdot,\;\!j}=R_{1,\;\!j}\:\!C_{\cdot,1}+\dots+R_{r\:\!\!,\;\!j}\:\!C_{\cdot,\;\!r}\Rightarrow B_{\range T}=\Bra{\mC_{\!j,k}=\sum_{x=1}^nC_{x,\;\!j}\;\!\mEnt{x,\;\!k}:1\leqslant j\leqslant r,\,1\leqslant k\leqslant n}.$\vspace{5pt}\parSol{}
...
Simlr, $B_{\range ST}=\Bra{\mX_{\!j,k}=\sum_{x=1}^n\sum_{y=1}^nC_{x,\;\!j}\;\!C_{y,\;\!k}\;\!\mEnt{x,\;\!y}:1\leqslant j,k\leqslant r}.\;\hMath{l}{\left|}{\right.}{\;\mX_{\!j,k}=\small\begin{pmatrix}C_{1,\;\!j}\:\!C_{1,\;\!k} &\hspace{-8pt} \cdots &\hspace{-8pt} C_{1,\;\!j}\:\!C_{n,\;\!k}\\[-4pt]
		...
		C_{n,\;\!j}\:\!C_{1,\;\!k} &\hspace{-8pt} \cdots &\hspace{-8pt} C_{n,\;\!j}\:\!C_{n,\;\!k}\end{pmatrix}.\\[20pt]\,}$\vspace{-19pt}\parSol{}
...
}Let $B_{\range S}=\Par{w_1,\dots,w_m}$ with each $w_i=Sv_i.$\parSol{}
Let $B_{\null T}=\Par{v_{m+1},\dots,v_n}.$ Let $B_1=\Par{v_1,\dots,v_n},\,B_2=\Par{w_1,\dots,w_n}.$\parSol{}
{\vspace{2pt}Let {$S=E_{1,1}+\dots+E_{m,\:\!m}\Rightarrow \Mt{S,B_1,B_2}=\Mt{P_{\!\!\;\!\range S},B_2,B_2}.$} Note that $R_{k,1}:w_x\mapsto\delta_{k,\;\!x}v_1.$}\parSol{}
...
\AExa Supp $U+W=V.$ Then $V\XSlash U=\Par{U+W}\XSlash U=W\XSlash U.$ Let $W\cap U=I,U_I\oplus I=U,W_{\!I}\oplus I=W.$\parExa{\IndentB}
Now $U_I\oplus W_{\!I}\oplus I=V.$ Thus $W\XSlash U=\Par{W_{\!I}\oplus I}\XSlash U=W_{\!I}\XSlash U.$\parExa{\IndentB}
$\forall w'_1,w'_2\in W_{\!I}$ suth $w'_1+U=w'_2+U\in W_{\!I}\XSlash U,$ we have $w'_1-w'_2\in U\cap W_{\!I}=\zeroSubs\Rightarrow w'_1=w'_2.$\par\vspace{4pt}
...
%\Anchor{3E4e7}\ProblemBnoor[]{{4E 7}}{
	%	Let $U=\Bra{\Par{x,y,z}\in\RbbP{3}:2x+3y+5z=0}.$ Supp $A\subseteq\RbbP{3}$.\TextB{}
	%	Then $A$ is a trslate of $U\Longleftrightarrow\exists\,c\in\Rbb,A=\Bra{\Par{x,y,z}\in\RbbP{3}:2x+3y+5z=c}.$\TextB{\vspace{-6pt}}
	%}\SepLine
...
$\Rightarrow 0\in A\apostrophe,$ and $\forall\Par{v-a},\Par{w-a}\in A\apostrophe,\:\!\forall\lambda\in\Fbb,$ \,(I) $\lambda\Par{v-a}=\Sbra{\lambda v+\Par{1-\lambda}a}-a\in A\apostrophe.$\parSol{\vspace{2pt}\Hb}
...
}$\forall\alpha+u,\beta+w\in A\cap B\neq\emptySet,\:\!\forall\lambda\in\Fbb,\,\,\lambda\Par{\alpha+u}+\Par{1-\lambda}\Par{\beta+w}\in A\cap B.$ By Exe (8).\PfEnd\parSol{}
...
$\forall x,y\in\bigcap_{\alpha\,\in\,\Gamma}A_{\alpha}\neq\emptySet,\:\!\forall\lambda\in\Fbb,\,\,\lambda x+\Par{1-\lambda}y\in A_{\alpha}$ for each $\alpha.$ By Exe (8).\PfEnd\parSol{\vspace{6pt}}
...
\Or Let $B_W=\Par{w_1,\dots,w_n}\Rightarrow V\XSlash U=\Span{w_1,\dots,w_n}\XSlash U=\Span{w_1+U,\dots,w_n+U}.$\PfEnd\vspace{3pt}
...
\Or Note that $B=\Par{v_1,\dots,v_m}$ is liney indep, and \:\!${\Span{v_1,\dots,v_m}+U}\subseteq V.$\parSol{}
...
\ProblemB{
	\TextB{Supp $V=U_1\oplus\cdots\oplus U_m,$ and each $V_{\!k}=\bigoplus_{i\,\in\,\Gamma_{\!k}}\!U_i,$ where $\Gamma_{\!k}=\Bra{1,\dots,n}\Backslash[\Big]\Bra{k}.$\vspace{2pt}}
	\TextB{Prove $V\apostrophe=V_{\!1}^0\oplus\cdots\oplus V_{\!m}^0$\FontSmall${}=\BigSbra{{\bigcap_{k=1}^n\!\Sbra{{\bigoplus_{i\,\in\,\Gamma_{\!k}}\!U_i}}}}{^0}=\bigoplus_{k=1}^n\!\Sbra{{\bigcap_{i\,\in\,\Gamma_{\!k}}\!U_i^0}}.$\PfEnd\vspace{2pt}}
}Supp $\varphi\in V\apostrophe.$ \,Define each $\varphi_k\in V_{\!k}^0$ by $\varphi_k\Par{u_k}=\varphi\Par{u_k}$ for all $u_k\in U_k.$ Then $\varphi=\varphi_1+\dots+\varphi_m.$\vspace{2pt}\parSol{}
We use induc by $k$ on $m$ to show the directness of sum. (i) $k=2.$ $\psi\in V_{\!1}^0\cap V_{\!2}^0\Rightarrow\psi=0.$\parSol{}
(ii) $k>2.$ Asum it holds for smaller $k.$ \,Then $\psi\in\BigPar{V_{\!1}^0\oplus\cdots\oplus V_{\!{k-1}}^0}\bigcap V_{\!k}^0\Rightarrow\psi=0.$\PfEnd
\SepLine
...
\Blind{\Or}Let $B_{V\apostrophe}=\Par{\psi_1,\dots,\psi_m,\varphi_1,\dots,\varphi_n}.$ Then each $\varphi\in\Span{\varphi_1,\dots,\varphi_n}$ will do.\PfEnd\vspace{7pt}\Anchor{3F19}
\ACoro \ProblemN[]{19}{
	\TextA{\tgnr $U^0=\zeroSubs=V^0\Longleftrightarrow U=V$.\vspace{0pt}}
}\par
\AComm\Anchor{3FN3.108}{\tgsc\large Another proof of \tgbfx[3.108]}: $T$ is surj $\Longleftrightarrow T\apostrophe$ is inje.\par
Supp $T\apostrophe$ inje. Becs $\psi\neq 0\Longleftrightarrow T\apostrophe\Par{\psi}\neq 0\Longleftrightarrow\psi\not\in\Par{\range T}{^0}.$ \;又 $T$ surj $\Rightarrow\Par{\range T}{^0}=\zeroSubs=\null T\apostrophe.$\PfEnd
\SepLine
...
\Anchor{3FNP}\AComm $X\subseteq W=\Bra{v\in V:\varphi\Par{v}=0,\forall\varphi\in X^0},$ the {\tgsc promotion} of the subset $X$ of $V.$\vspace{-4pt}
\SepLine[-2pt]
...
\Blind{(2)} Let $W=\Bra{\Par{1,0},\Par{0,1}}\not\supseteq U=\Bra{\Par{x,0}\in\RbbP{2}}.$ Then $Y=\RbbP{2}\supseteq X=U,\;Y^0=\zeroSubs\subseteq X^0.$
\SepLine

\ProblemN{\Anchor{3F22}{22}}{
	\TextA{Supp $U,W$ subsps of $V$. Prove $\BigPar{U+W}{^0}=U^0\cap W^0$.\;\;\FontSmall Then $\BigPar{U_1+\cdots+U_m}{^0}=U_1^0\cap\cdots\cap U_m^0.$}
}\vspace{-14pt}\parSol{}
\!\!\!$\hText{$
...
\ProblemN{\Anchor{3F23}{23}}{
	\TextA{Supp $U,W$ subsps of $V$. Prove $\BigPar{U\cap W}{^0}=U^0+W^0$.\;\;\FontSmall Then $\BigPar{U_1\cap\cdots\cap U_m}{^0}=U_1^0+\cdots+U_m^0.$\vspace{-2pt}}
}\vspace{3pt}\par\quad
\!\!\!$\hText{$
...
(b) \!\Sbra[3pt]{{\tgsl Req Finide}} \;By Exe (22), $\Dim\BigPar{U^0+W^0}=\dim V-\Dim\Par{U\cap W}.$\PfEnd\vspace{0pt}\quad\Hb
%\dim U^0+\dim W^0-\Dim\BigPar{U^0\cap W^0}$\par\vspace{0pt}\quad\Hb
%$=2\dim V-\dim U-\dim W-\BigPar{\!\dim V-\Dim\Par{U+W}}=
\Or {\small Thanks to \href{https://github.com/vecljox}{vecljox at GitHub} for issuing the corrected version of the solution}\par\vspace{-3pt}\quad\Hb
{\small in \textcolor{blue}{LADR4eSolutions经典最全（By Axler？）.pdf} for Exercise 3.F.22(b) \SmallPar{using exercise number in 4e}}.\par\quad\Hb
Let $\varphi\in\Par{U\cap W}{^0},U=\Par{U\cap W}\oplus X,W=\Par{U\cap W}\oplus Y,$\par\quad\Hb
and by \Sbra{1.C \TIPSN{3}}, let $V=\Par{U\cap W}\oplus X\oplus Y\oplus Z.$\par\quad\Hb
Becs $\forall v\in V,\exists\,!\,\Par{i,x,y,z}\in\Par{U\cap W}\times X\times Y\times Z.$\par\quad\Hb
Define $\alpha,\beta\in V\apostrophe$ by $\alpha\Par{v}=\varphi\Par{y}+\frac{\;1\;}{2}\varphi\Par{z},\:\beta\Par{v}=\varphi\Par{x}+\frac{\;1\;}{2}\varphi\Par{z}.$ Immed, $\varphi=\alpha+\beta.$\PfEnd\vspace{6pt}\quad\Hb
...
\BulletPointX\ACoro
$V=U\oplus W\Longleftrightarrow V\apostrophe=U^0\oplus W^0.$
\SepLine
...
}Becs $\null T\apostrophe=\zeroSubs=\Par{\range T}{^0}.$ By Exe (19) \Or (25).\PfEnd
...
\Anchor{3F'3}\ProblemB[]{
	\TextB{Supp $V=U\oplus W.$ Then $U^0=\Bra{\varphi\in V\apostrophe:\varphi=\varphi\circ\iota},$ \FontNorm where $\iota\in\Lm{V,W}:u_v+w_v\rightarrow w_v$.}
...
\AExa Let $B_V=\Par{e_1,e_2}.$ Let $B_U=\Par{e_1},B_X=\Par{e_2-e_1},B_Y=\Par{e_2}.$
%Then $\iota_X:ae_1+b\Par{e_2-e_1}\mapsto b\Par{e_2-e_1},\;\;\iota_Y:ae_1+be_2\mapsto be_2.$ 
Now by notat asum, $X_V\upapostrophe=Y_V\upapostrophe=U^0.$\parExa
...
(2) For $V=U\oplus Y,$ let $B_{U_V'}=\Par{\psi}$ with $\psi:e_1\mapsto 1,e_2\mapsto 0.$ \hfill Now $Y^0=U_V\upapostrophe.$\par\vspace{2pt}
...
Let $B_{V\apostrophe}=\Par{\beta_1,\dots,\beta_p,\gamma_1,\dots,\gamma_q}\Rightarrow B_V=\Par{v_1,\dots,v_p,u_1,\dots,u_q},$ by Exe (31).\parSol{}
...
Thus $\Par{\null\beta_1\cap\cdots\cap\null\beta_p}{^0}\subseteq\Span{\beta_1,\dots,\beta_p}=\Omega.$ 又 Each $\Span{\beta_k}\subseteq\Par{\null\beta_k}{^0}.$\PfEnd
\SepLine
...
$\forall v\in\bigcap_1^m\null\varphi_I,\:\beta_k\Par{v}=0.$ Thus $\bigcap_1^m\null\varphi_I\subseteq\bigcap_1^n\null\beta_I.$ \;Rev the roles and done.\PfEnd\vspace{4pt}
\AComm Supp $\Par{\varphi_1,\dots,\varphi_j}$ is a bss of $\Span{\varphi_1,\dots,\varphi_m}.$ Let $N_{k}\oplus\bigcap_1^{j}\null\varphi_I=\null\varphi_k.$\vspace{2pt}\parCom
Now $\bigcap_1^j\null\varphi_I\cap\BigPar{\null\varphi_k}=\bigcap_1^j\null\varphi_I.$ Thus $\bigcap_1^m\null\varphi_I=\bigcap_1^j\null\varphi_I.$\vspace{2pt}\parCom
...
\ProblemN{\Anchor{3F36}{36}}{
	...
	\PrePb\TextA{Prove $\rangep i\apostrophe=U\apostrophe$\hspace{1pt}$:${\tgnr\FontNorm\;\;$\rangep i\apostrophe=\Par{\nullp i}{_U^0}={\zeroSubs}{_U^0}=U\apostrophe$.\hfill\ACoro $\tilde{i\apostrophe}$ is iso from $V\apostrophe\XSlash U^0$ onto $U\apostrophe$}\vspace{2pt}}
...
(b) \Or Supp $\psi\in U\apostrophe.$ By (3.A.11), $\exists\,\varphi\in V\apostrophe,\varphi\mmid_U=\psi.$\PfEnd
\SepLine\pagebreak
...
By Exe\;\!(31),\;$B_V=\Par{v_1,\dots,v_n}$ and each $\varphi_k\Par{v_j}=\delta_{j,\:\!k}\Rightarrow$ each $e_k=\Gamma\Par{v_k}.$\par\quad\Hd
...
%	\TextA{Define $T:\RbbP{3}\!\rightarrow\!\RbbP{2}$ by $T\Par{x,y,z}=\Par{4x+5y+6z,7x+8y+9z}$.\vspace{3pt}}
%	\TextA{Let $\Par{\varphi_1,\varphi_2},\Par{\psi_1,\psi_2,\psi_3}$ denote the dual bss of std bss of $\RbbP{2}$ and $\RbbP{3}$.\vspace{4pt}}
%	\PrePa\TextA{Describe the liney functionals $T\apostrophe\Par{\varphi_1},T\apostrophe\Par{\varphi_2}.$}
%	\Blind{\PrePb}\TextA{{\FontNorm\tgnr For any $\Par{x,y,z}\in\RbbP{3}$, $\BigPar{T\apostrophe\Par{\varphi_1}}\Par{x,y,z}=4x+5y+6z, \BigPar{T\apostrophe\Par{\varphi_2}}\Par{x,y,z}=7x+8y+9z$.}\vspace{6pt}}
...
\ProblemN{\Anchor{3F34}{34}}{
	\TextA{Define $\Lambda:V\rightarrow \FbbP{V\apostrophe}$ by $\Lambda v=\overline{v},$\;and\;$\overline{v}\in V\apostrophe\apostrophe$ \:\!by \;\!$\overline{v}\Par{\varphi}=\varphi\Par{v}\Rightarrow\Lambda\in\Lm{V,V\apostrophe\apostrophe}.$\vspace{2pt}}
	\TextA{Show if $T\in\Lm{V}$, then $T\apostrophe\apostrophe\circ\Lambda=\Lambda\circ T$, where $T\apostrophe\apostrophe=\Par{T\apostrophe}\apostrophe$.}
	%	\PrePb\TextA{Show if $V$ is finide, then {\tgsc $\Lambda$ is iso from $V$ onto $V\apostrophe\apostrophe$}.}
}$\BigPar{T\apostrophe\apostrophe\overline{v}}\Par{\varphi}=\BigPar{\overline{v}\circ{T\apostrophe}}\Par{\varphi}=\overline{v}\BigPar{T\apostrophe\Par{\varphi}}=\Par{\varphi\circ T}\Par{v}=\overline{Tv}\Par{\varphi}\Longrightarrow T\apostrophe\apostrophe\overline{v}=\overline{Tv}.$\PfEnd
%(b) $\overline{v}=0\Rightarrow\forall\varphi\in V\apostrophe,\overline{v}\Par{\varphi}=\varphi\Par{v}=0\Rightarrow v=0.$ Inje. Now becs $V$ finide.\PfEnd
...
%	\TextB{Supp $\varphi_1,\dots,\varphi_m\in V\apostrophe$ liney indep and $\dim V=n.$ \,Show $V=\null\varphi_1+\dots+\null\varphi_m.$}
%}Extend to $B_{V\apostrophe}=\Par{\varphi_1,\dots,\varphi_n}\Rightarrow B_V=\Par{v_1,\dots,v_n}.$\parSol{}
%Each $\null\varphi_k=\Span{e_1,\dots,e_{k-1},e_{k+1},\dots,e_m}\oplus\Span{e_{m+1},\dots,e_n}.$\PfEnd
%\SepLine
\ChEnd
\hfill
\vfill\ChDecl{}{\Largebfx{I Will Never Forget{\,}——{\,}In 3.E, naive but fantastical:}}
...
\BulletPointX\NoteForSmall{Exe (13) and (4E 14)}\;\;Let $U\oplus W=V.$ Define $S\Par{w+U}=w$ \:\!as in \TIPSN{1}.\TextB{}
(a) Let $B_W=\Par{w_1,\dots,w_m}\Rightarrow B_{V\XSlash U}=\BigPar{w_1+U,\dots,w_m+U}.$ Then $S\Par{w_k+U}$ might not equal $w_k.$\TextB{}
(b) Let $B_{V\XSlash U}=\BigPar{w_1+U,\dots,w_m+U},$ then let $B_W=\Par{w_1,\dots,w_m}.$ Now each $S\Par{w_k+U}=w_k.$
\def\Pure{{\textup{\tgnr Pure}}\,}
\par\vspace{2pt} \BulletPointX\NewNotation\;$\Pure V\XSlash U=W\Longleftrightarrow V=U\oplus W,\;W=\range S.$ {\FontSmall\tgsl The uniqnes of $\Pure V\XSlash U$ follows from $\range S.$}
...
\ProblemB{
	\TipsN{2}\,\,\,\TextB{Supp $I$ is a subsp of $U.$ Supp $U$ is a subsp of $V.$}
	\IndentTipsN{2}\TextB{Let $V=S_V I\oplus I=S_V U\oplus U.$ \;Let $U=S_U I\oplus I.$ \;Then $V=S_V U\oplus S_U I\oplus I.$\vspace{2pt}}
	\IndentTipsN{2}\TextB{Supp $S_V I=\Pure V\!\!\:\XSlash I,$ simlr for $S_V U,S_U I.$ \;Prove $S_V I=S_V U\oplus S_U I.$}
...
\ProblemB{
	\TipsN{3}\,\,\,\TextB{Supp $T\in\Lm{V,W}$ is inv, $V=M\oplus N,\:\range T\mmid_M=X,\range T\mmid_N=Y.$}
	\IndentTipsN{3}\TextB{Then $W=\range T=X\oplus Y.$ Show $\Pure V\!\!\;\XSlash M=N\Longleftrightarrow\Pure W\XSlash X=Y.$}
...
}Let $I=U\cap W.$ \,Using \TIPSN{2}. Supp $\varphi\in I^0.$\vspace{-10pt}\parSol{}
...
\BulletPointX The def of $\Pure V\:\!\!\XSlash U$ is indeed ridic. To fix it, simply re-define $\Pure V\:\!\!\XSlash U=U^\perp,$\TextB{}
but may fail \TIPSN{3} and the weird solus of (3.F.23) above.
\SepLine

\BulletPointX\ANote Supp $U,W$ are subsps of finide $V.$ Define $X_V\upapostrophe=\Par{X^\perp}{^0}$ for all $X\subseteq V.$ Then by \Sbra{6.C \TIPSN{3,4}},\parNot{\IndentB}
$\Par{U+W}{_V\upapostrophe}=\BigPar{\Par{U+W}{^\perp}}{^0}=\BigPar{U^\perp\cap W^\perp}{^0}=U_V\upapostrophe+W_V\upapostrophe,$ and simlr, $\Par{U\cap W}{_V\upapostrophe}=U_V\upapostrophe\cap W_V\upapostrophe.$
\SepLine

\Anchor{3F'4}\ProblemB{
	\TextB{Supp $\Span{\varphi_1,\dots,\varphi_m}\subseteq V\apostrophe.$ Let each $\Span{u_k}\oplus\null\varphi_k=V.$}
	\TextB{Prove or give a countexa\hspace{1pt}$:$ $\Span{u_1,\dots,u_m}\oplus\BigPar{\null\varphi_1\cap\cdots\cap\null\varphi_m}=V.$}
}Let $V=\RbbP{2}.$ Define $\varphi_1=\varphi_2:\Par{x,y}\mapsto x.$ Let $u_1=e_1,\;\!u_2=e_1+e_2\Rightarrow\Span{u_1,u_2}=V.$\parSol{}
\Or Let $B_{V\apostrophe}=\Par{\varphi_1,\varphi_2}$ be corres to the std bss. Let $u_1=u_2=e_1+e_2\Rightarrow\Span{u_1,u_2}\subsetneq V.$\PfEnd\vspace{2pt}
\ANote Supp each $\Span{u_k}=\Par{\null\varphi_k}{^\perp}.$ Then $\Span{u_1,\dots,u_m}=\Par{\null\varphi_1\cap\cdots\cap\null\varphi_m}{^\perp}.$\SepLine
\ChEnd\pagebreak
...
\Anchor{2A16}\ProblemBnoor{2.A.16}{
	\TextA{Prove the vecsp $U$ of all continuous functions in $\RbbP{[0,1]}$ is infinide.}
...
Thus $\Par{1,x,\dots,x^m}$ is liney indep in $\RbbP{[0,1]}.$ Simlr to [2.16], $U$ is infinide.\PfEnd\vspace{8pt}\parSol{}
...
\Anchor{1C9}\ProblemBnoor{1.C.9}{
	...
	\TextA{Is the set of periodic functions $\Rbb\rightarrow\Rbb$ a subsp of $\RbbP{\Rbb}$ ? Explain.}
...
\Anchor{1C24}\ProblemBnoor{1.C.24}{
	\TextA{Let $V_{\!E}=\Bra{\,f\in\Fbb{^{\Rbb}}:\text{f is even}},V_{\!O}=\Bra{\,f\in\Fbb{^{\Rbb}}:\text{f is odd}}.$ Show $V_{\!E}\oplus V_{\!O}=\Fbb{^{\Rbb}}.$\vspace{4pt}}
}(a) {$V_{\!E}\cap V_{\!O}=\Bra{\,f\in\Fbb{^{\Rbb}}:f\Par{x}=f\Par{{-x}}=-f\Par{{-x}}}=\zeroSubs.$}\parSol{\vspace{8pt}}
...
}\Rightarrow\forall g\in\RbbP{\Rbb},\;g\Par{x}=f_e\Par{x}+f_o\Par{x}.$\PfEnd\vspace{4pt}
\SepLine
...
\Anchor{2C4e10}\ProblemBnoor{4E 2.C.10}{
	\TextA{Supp $m$ is a posi integer. For $0\leqslant k\leqslant m$, let $p_k\Par{x}=x^k\Par{1-x}{^{m-k}}$.}
...
\Anchor{3E'2}\ProblemB{
	\TextB{Supp $V=\RbbP{\Rbb}$ with a subsp $U=\Bra{\hspace{3pt}f\in\RbbP{\Rbb}:f\Par{x_1}=\cdots=f\Par{x_m}=0},$ where each $x_k\in\Rbb.$\vspace{2pt}}
	\TextB{Prove if $W\in\Scom{V}{U},$ then $\dim W=m.$\hfill{\large\tgsc Hint$:$ }{\FontNorm Find an iso from $V\XSlash U$ onto $\RbbP{m}.$}}
}Define $T\in\Lm{V\XSlash U,\RbbP{m}}$ by $T\Par{\,f+U}=\BigPar{\,f\Par{x_1},\dots,\,f\Par{x_m}}.$\parSol{}
...
\AExa By [2.C.10], $B_m=\BigBigPar{1,7x-5,\dots,\Par{7x-5}{^m}}$ is a bss of $\PoR{m}$. Let each $\varphi_k={}${\Large\envFontSmall[\footnotesize]$\frac{\;p^{\SmallPar{k}}\Par{5\text{\large/}\hspace{0.5pt}7}\;}{7^k\hspace{1pt}\cdot\,k!}$}.\par\vspace{2pt}
\SepLine
\ChEnd\pagebreak
...
\Anchor{4ON4.8}\BulletPointX\NoteFor{[4.8]} {\tgsl div algo for polys} \hfill[{\tgsc Another proof}]\TextB{\vspace{-9pt}}
...
%Becs $p\in\PoFi,\exists\,!\,a_i,b_j\in\Fbb,$\TextB{}
%$p\Par{z}=a_0+a_1 z+\dots+a_{\deg s-1}z^{\deg s-1}+ b_0\,s\Par{z}+b_1z\,s\Par{z}+\dots+ b_{\deg p-\deg s}\,z^{\deg p-\deg s}\:\!s\Par{z}$\TextB{}
...
%}$\aMidsq{w-z}=\Par{w-z}\Par{\overline{w}-\overline{z}}=\aMidsq{w}+\aMidsq{z}-2\REAL\Par{w\overline{z}}\geqslant\aMidsq{w}+\aMidsq{z}-2\aMid{w\overline{z}}=\aXMidsq{\aMid{w}-\aMid{z}}.$\vspace{2pt}\parSol{}
...
\hfill And $\prod_{i=1}^{k-1}\Par{z_j-z_i}=0 \Longleftrightarrow j\leqslant k-1,$ becs $z_1,\dots,z_{m+1}$ are disti.\Blind{\quad}\vspace{34pt}\par\quad
...
\ProblemN{\Anchor{4O6}{6}}{
	\TextA{Supp non0 $p\in\PoC{m}$ with $\deg p=m>1.$}
	\TextA{Prove [P] \:\!$p$ has $m$ disti zeros $\Longleftrightarrow p$ and $p\apostrophe$ have no common zeros. [Q]}
}(a) Supp [{\tgsl P}] holds. Write $p\Par{z}=c\Par{z-\lambda_1}\cdots\Par{z-\lambda_m}.$\vspace{1pt}\parSol{\Ha}
...
Now $p\apostrophe\Par{z}=\Par{z-\lambda_j}q_j\upapostrophe\Par{z}+q_j\Par{z}\Rightarrow p\apostrophe\Par{\lambda_j}=q_j\Par{\lambda_j}\neq 0.$\vspace{4pt}\parSol{\Ha}
\Or ${}^{\neg}Q\Rightarrow{}{^\neg}P:$ \;Supp $p\Par{z}=\Par{z-\lambda}q\Par{z},\;p\apostrophe\Par{z}=\Par{z-\lambda}r\Par{z}.$\parSol{\Ha}
Becs $p\apostrophe\Par{z}=\Par{z-\lambda}q\apostrophe\Par{z}+q\Par{z}\Rightarrow p\apostrophe\Par{\lambda}=q\Par{\lambda}=0\Rightarrow q\Par{z}=\Par{z-\lambda}s\Par{z}.$%Now $p\Par{z}=\Par{z-\lambda}{^2}s\Par{z}.$ Hence $p$ has strictly less than $m$ disti zeros.
\vspace{4pt}\parSol{}
...
\Anchor{4OT2}\ProblemBX{\TipsN{2}}{
	\TextB{Supp $\alpha,\beta\in\Nbp,\:\!\alpha\geqslant\beta.$ \,Show $p\Par{z}=\Par{z-\lambda}{^\alpha}\:\!q\Par{z}\Longrightarrow{p{^{\SmallPar{\beta}}}}\Par{z}=\Par{z-\lambda}{^{\alpha\:\!-\:\!\beta}}\:\!s\Par{z}.$\vspace{2pt}}
}By induc on $\beta.$ Immed. \;\Or By (L1), $\Sbra{\Par{z-\lambda}{^\alpha}q\Par{z}}{^{\SmallPar{\beta}}}=\sum_{j=0}^{\beta}\mathC_{\beta}^j\Sbra{\Par{z-\lambda}{^\alpha}}{^{\SmallPar{j}}}\:\!\Sbra{q\Par{z}}{^{\SmallPar{\beta\:\!-\:\!j}}}.$\vspace{3pt}\parSol{}
Note that $\Sbra{\Par{z-\lambda}{^\alpha}}{^{\SmallPar{j}}}=\Sbra{\alpha\Par{\alpha-1}\cdots\Par{\alpha-j+1}}\Par{z-\lambda}{^{\SmallPar{\alpha\:\!-\:\!j}}}$ for all $j\in\;\!\!\Bra{0,\dots,\alpha}.$\PfEnd
%\Or (i) $\beta=1.$ $p\apostrophe\Par{z}=\Par{z-\lambda}{^{\alpha\:\!-\:\!1}}\Sbra{\alpha\:\!q\Par{z}+\Par{z-\lambda}\:\!q\apostrophe\Par{z}}.$ \;(ii) $\beta>1.$ Asum true for $\Par{\beta-1}.$\par\quad
%\Blind{\Or }Let $p{^{\SmallPar{\beta\:\!-\:\!1}}}=\Par{z-\lambda}{^{\alpha\:\!-\:\!\beta\:\!+\:\!1}}\:\!s\Par{z}\Longrightarrow\Sbra{p{^{\SmallPar{\beta\:\!-\:\!1}}}}\apostrophe\Par{z}=\Par{z-\lambda}{^{\alpha\:\!-\:\!\beta}}\Sbra{\Par{\alpha-\beta+1}\:\!s\Par{z}+\Par{z-\lambda}\:\!s\apostrophe\Par{z}}.$\PfEnd
\SepLine

\Anchor{4O4e13}\ProblemBnoor{4E 13}{
	\TextA{Supp nonC $p,q\in\PoCi$ have no common zeros. Let $m = \deg p$, $n = \deg q$.\vspace{1pt}}
	\TextB{Define $T:\PoC{n-1}\times\PoC{m-1}\rightarrow\PoC{m+n-1}$ by $T\Par{r, s} = rp + sq.$ Prove $T$ is inje.\vspace{2pt}}
	\ACoro \TextB{$\exists\,!\,r\in\PoC{n-1},s\in\PoC{m-1}$ suth $rp + sq = 1.$\vspace{2pt}}
}Immed, $T$ is liney. \;Supp $T\Par{r,s}=rp+sq=0.$\par\quad
Then $rp=-sq.$ Becs $p,q$ are \uline{coprime} $\Rightarrow p\:\mmid\:s,$ while $\deg s\leqslant m-1\Rightarrow s=0\Rightarrow r=0.$\PfEnd\vspace{4pt}\quad
\Or Let $\lambda_1,\dots,\lambda_M$ and $\mu_1,\dots,\mu_N$ be the disti zeros of $p$ and $q$ respectly. \NOTICE that $M\leqslant m,N\leqslant n.$\par\quad
By the ctrapos of [4.13], $M=0\Longleftrightarrow m=0\Rightarrow s=0\Longleftrightarrow r=0 \Leftarrow n=0\Longleftrightarrow N=0.$\par\quad
Now supp $M,N\geqslant 1.$ We show $s=0.$ \,{\FontSmall Simlr for $r=0.$ \Or $s=0\Rightarrow r=0.$}\vspace{2pt}\par\quad
\Or By expr of $p\Par{z}$ below, define $\alpha_{\lambda}\Par{p}=\alpha_j$ if $\lambda=\lambda_j,$ and define $\alpha_{\mu}\Par{p}=0$ if $\mu$ is not zero of $p.$\par\quad
Now asum $r,s$ non0 $\Rightarrow\forall\lambda=\lambda_j,\,\alpha_{\lambda}\Par{rp}=\alpha_{\lambda}\Par{r}+\alpha_{\lambda}\Par{p}=\alpha_{\lambda}\Par{s}=\alpha_{\lambda}\Par{sq}.$ Sum up and ctradic.\PfEnd\vspace{3pt}\quad
\Or Write $p\Par{z}=a\Par{z-\lambda_1}{^{\alpha_1}}\cdots\Par{z-\lambda_M}{^{\alpha_M}}.$ \,Let $\max\!\Bra{\alpha_1,\dots,\alpha_M}=A=\alpha_L.$\par\quad
For each $D\in\;\!\!\Bra{0,1,\dots,A-1},$ let $I_{>D}=\Bra{{I_{D,\:\!1}},\dots,{I_{D,\,J_D}}}$ be suth each $\alpha\Sbra{I_{D,\:j}}=\alpha_{I_{D,\:j}}\geqslant D+1.$\vspace{1pt}\par\quad
Now $\Bra{L}=I_{>A-1}\subseteq \cdots\subseteq I_{>0}=\Bra{1,\dots,M}.$ Becs $rp+sq=0\Rightarrow\Par{rp+sq}{^{\SmallPar{k}}}=0$ for all $k\in\Nbp.$\vspace{1pt}\par\quad
We use induc on $D$ to show\;$s{^{\SmallPar{D}}}\BigPar{\lambda\Sbra{I_{D,\:j}}}=0$ for each $D\in\;\!\!\Bra{0,\dots,A-1}.$\par\quad
\NOTICE that by \TIPSN{2}, \,$p{^{\SmallPar{D}}}\BigPar{\lambda\Sbra{I_{D,\:j}}}=0$ for each $D\in\;\!\!\Bra{0,\dots,A-1}$ and each $I_{D,\:j}\in I_{>D}.$\vspace{2pt}\par\quad
...
\Blind{Each} $\Blind{\Sbra{rp+sq}{^{\SmallPar{D}}}\BigPar{\lambda\Sbra{I_{D,\:j}}}}=\Sbra{\mathC_{D}^D\:s{^{\SmallPar{D}}}q{^{\SmallPar{0}}}}\BigPar{\lambda\Sbra{I_{D,\:j}}}.$\; Where each $\lambda\Sbra{I_{D,\:j}}\in I_{>D}\subseteq I_{D-1,\;\!\alpha}.$\vspace{3pt}\par\quad\Hii
Hence \,$s{^{\SmallPar{D}}}\BigPar{\lambda\Sbra{I_{D,\:j}}}=0.$ The asum holds for all $D\in\;\!\!\Bra{0,\dots,A-1}.$\vspace{3pt}\par\quad
\NOTICE that $\forall k=\Bra{0,\dots,A-2},\,s{^{\SmallPar{k}}}$ and $s{^{\SmallPar{k+1}}}$ have common zeros $\BigBra{{\lambda\Sbra{I_{k+1,1}},\dots,\lambda\Sbra{I_{k+1,\,J_{k+1}}}}}.$\vspace{2pt}\par\quad
Now $\forall D\in\;\!\!\Bra{1,\dots,A-1},\,s=s{^{\SmallPar{0}}},\dots,s{^{\SmallPar{D}}}$ have common zeros $\BigBra{{\lambda\Sbra{I_{D,1}},\dots,\lambda\Sbra{I_{D,\,J_D}}}}.$\vspace{2pt}\par\quad
Thus \,$s\Par{z}$ is div by $\BigPar{z-\lambda\Sbra{I_{D,1}}}{^{\alpha\def\envFont{\scriptsize}\Sbra{I_{D,1}}}}\cdots\BigPar{z-\lambda\Sbra{I_{D,\,J_D}}}{^{\alpha\def\envFont{\scriptsize}\Sbra{I_{D,\,J_D}}}},$ for each $D\in\;\!\!\Bra{0,\dots,A-1}.$\vspace{2pt}\par\quad
...
\Anchor{4OL1}\ProblemN{L1}{
	\TextB{Prove $\forall p,q\in\PoFi,k\in\Nbp,\Par{pq}{^{\SmallPar{k}}}=\mathC_{k}^k\;\!p{^{\SmallPar{k}}}q{^{\SmallPar{0}}}+\dots+\mathC_{k}^j\;\!p{^{\SmallPar{j}}}q{^{\SmallPar{k-j}}}+\dots+\mathC_{k}^0\;\!p{^{\SmallPar{0}}}q{^{\SmallPar{k}}}.$}
}\par\quad
...
\Blind{Now} $\Blind{\Par{pq}{^{\SmallPar{k}}}=}+\dots+\XSbra{\mathC_{k-1}^{k-2}\XPar{ \uline{p{^{\SmallPar{k-1}}}q{^{\SmallPar{1}}}}+p{^{\SmallPar{k-2}}}q{^{\SmallPar{2}}}}}+\XSbra{\mathC_{k-1}^{k-1}\XPar{ \boxed{p{^{\SmallPar{k}}}q{^{\SmallPar{0}}}}+\uline{p{^{\SmallPar{k-1}}}q{^{\SmallPar{1}}}}}}.$\vspace{8pt}\par\quad
...
\Anchor{5AT4}\BulletPointX\TipsN{4}\,\,\,Supp $T\in\Lm{\RbbP{2}}$ is the countclockws rotat by $\theta\in\Rbb.$ Define $\mC\Par{a,b}=a+\i\:\!b.$\TextB{}
...
%	\PrePc\TextA{Supp inv $R,T$ have the same eigvals. Then $\exists$ inv $P\in\Lm{V},\,R=P^{-1}TP.$}
	\PrePc\TextA{Supp $R,T$ have the same eigvecs $B_V=\Par{v_1,\dots,v_n}.$ Then $RT=TR.$}
	\Anchor{5A23}\PrePd\TextA{Prove $RT$ and $TR$ have the same eigvals.\vspace{2pt}}
...
(d) \!\Sbra{{\tgsl False if infinide. See Exe (18, 20).}} Supp $v\neq 0$ and $RTv=\lambda v\Rightarrow T\Par{RTv}=\lambda Tv=TR\Par{Tv}.$\parSol{\Hd}
If $Tv=0$, then $T$ not inje, so are $TR,RT.$ Othws, $\lambda$ is eigval of $TR.$ \;Rev the roles.\PfEnd
\SepLine

%\ProblemN{\Anchor{5A17}{17}}{
%	\TextA{Give an exa of an optor on $\RbbP{4}$ that has no real eigvals.}
%}\par\quad
%Let $\Par{e_1,e_2,e_3,e_4}$ be the std bss of $\RbbP{4}.$\vspace{-6pt}\par\quad
%Define $T\in\Lm{\RbbP{4}}$ by $\Mt[\BigPar]{T,\Par{e_1,e_2,e_3,e_4}}={}${\normalsize$\begin{pmatrix}
%		1 & 1 & 1 & 1\\
%		-1 & 1 & -1 & -1\\
%		3 & 8 & 11 & 5\\
%		3 & -8 & -11 & 5
...
%\Or Define $T\in\Lm{\RbbP{4}}$ by $T\Par{x_1,x_2,x_3,x_4}=\Par{{-x_2,x_1,-x_4,x_3}}.$\par\quad
...
%\ProblemN{\Anchor{5A30}{30}}{
	%	\TextA{Supp $T\in\Lm{\RbbP{3}}$ and $-4,5,\sqrt{7}$ are eigvals. Prove $\exists\,x,Tx - 9x = \Par{{-4, 5, \sqrt{7}}}$.}
	%}$T$ has $\dim\RbbP{3}$ eigvals not including $9\Rightarrow\Par{T-9I}$ is inv. $x=\Par{T-9I}{^{-1}}\Par{{-4,5,\sqrt{7}}}.$\PfEnd
%\SepLine
...
%	(a)\ProblemN[]{32}{
%		\TextB{Prove $\Par{e^{\lambda_1x},\dots,e^{\lambda_nx}}$ is liney indep in $\Fbb{^{\Rbb}}$.}}
%	\TextB{\vspace{-2pt}}
%	(b)\ProblemNnoor[]{\hspace{-7pt}}{4E 36}{
%		\TextB{Show $\BigPar{\!\cos{\lambda_1 x},\dots,\cos{\lambda_n x}}$ is liney indep in $\Fbb{^{\Rbb}}.$\vspace{-3pt}}}
%	\TextB{}\vspace{2pt}
...
(a) Supp $A_{1,C}+\dots+A_{n,C}=\lambda.$ Let $x_1=\dots=x_n.$ Immed.\vspace{2pt}\par\quad
...
%	\TextB{Let $\Bra{F_n}$ be the Fibonacci Seq. Define $T\in\Lm{\RbbP{2}}:\Par{x,y}\mapsto\Par{y,x+y}.$}
...
%	\,and $\lambda_2={}${\large$\frac{\:1\,-\,\sqrt{5}\:}{2}$}, $v_2=\Par{1,\frac{\:1\,-\,\sqrt{5}\:}{2}}.$ Becs $\dim\RbbP{2}=2.$}\vspace{2pt}\parSol{}
...
%	\TextA{Give an exa of $T\in\Lm{\RbbP{2}}$ suth $T^4=-I$.}
%}Define $\i^n\in\Lm{\RbbP{2}}$ by $\i^n\Par{x,y}=\XPar{\REAL\BigPar{\i^n x+\i^{n+1} y},\,\IMAGINARY\BigPar{\i^n x+\i^{n+1} y}}.$\vspace{0pt}\par\quad
...
%%\Blind{\Or}We define $T\in\Lm{\RbbP{2}}$ suth $\Mt{T}={}${\small$\begin{pmatrix}
...
%	\TextA{Give a $T\in\Lm{\RbbP{2}}$ that shows the result above does not hold if $\Cbb$ is replaced with $\Rbb$.}
...
\Anchor{5BI4e8}\ProblemBnoor{4E I.8}{
	\TextB{Find the min $p$ of $T\in\Lm{\RbbP{2}},$ the countclockws rotat optor by $\theta\in\RbbP{+}$.\vspace{1pt}}
...
Othws, let $\Span{v,Tv}=\RbbP{2}.$ Let $L=x^2+y^2,$ where $v=\Par{x,y}.$\parSol{}
...
\Or Let $\deg p=m.$ Becs $T^{m}\in\Span{I,T,\dots,T^{m-1}}.$ For $\deg q<m,$ the expr of $q\Par{T}$ is uniq.\parSol{}
If $\deg q\geqslant m.$ Note that for each $k\in\Nbb,\exists\,!\,b_{j,\;\!k}\in\Fbb,\,T^{m+k}=b_{0,\;\!k}I+b_{1,\;\!k}T+\dots+b_{m-1,k}T^{m-1}.$\PfEnd
\SepLine[0pt][\Blind{\BulletPointX} ]
...
\Anchor{5BI4e14}\ProblemBnoor{4E I.14}{
	...
	\TextA{Give a representa of $s,$ the min of $T^{-1}.$\hfill\FontNorm\tgnr $s\Par{z}=z^m\,p\Par{0}{^{-1}}\,p\Par{z^{-1}},\:z\neq0.$\vspace{3pt}}
...
For if not, $\exists\,T^{-k}=b_0 I+b_1 T^{-1}+\dots+b_{k-1}T^{-\SmallPar{k-1}}.$ Apply $T^k.$ Now $a_kT^k+\dots+a_1T+a_0I=0.$\PfEnd
\SepLine[0pt][\Blind{\BulletPointX} ]
\pagebreak
...
%}Define $T\in\Lm{\RbbP{2}}:\Par{x,y}\mapsto\Par{y,x}.$
%\SepLine
%
...
%\AExa Convly not true. Define the inv $T\in\Lm{\RbbP{2}}:\Par{x,y}\mapsto\Par{{-y,x}}.$ No eigvals.
%\SepLine
...
\Anchor{5C4e13}\ProblemN{13}{
	\TextA{Supp $A,B\in\FbbP{n,\;\!n}$ and $A$ is diag with {\tgsc dist} ents on diag. Prove $AB=BA\Rightarrow B$ is diag.}
...
\AExa Not true if $\Fbb=\Rbb.$ Define $T\in\Lm{\RbbP{2}}:\Par{x,y}\mapsto\Par{{-y,x}}.$ No eigvals.
\SepLine
...
%\Anchor{5C'2}\ProblemB{
%	\TextB{Supp $S,T\in\Lm{V}$ diag and have the same eigvals. Prove $\exists$ inv $E\in\Lm{V},\:S=E^{-1}TE.$}
%}Let $\;\!\alpha_1,\dots,\alpha_n$ be eigvals of $S,T$ wrto $B_1=\Par{v_1,\dots,v_n},\,B_2=\Par{u_1,\dots,u_n}$ respectly.\parSol{}
%Define $E\Par{v_k}=u_k\Rightarrow Sv_k=\alpha_kv_k=E^{-1}\Par{\alpha_ku_k}=E^{-1}TEv_k,$ whether $\alpha_k=0$ or not.\PfEnd
%\AComm eigvals are disti $\Longleftrightarrow$ the exis of such $E$ is uniq.
%\SepLine
...
}Supp $T\apostrophe\Par{\psi}=\lambda\psi$ with $\psi=c_1\varphi_1+\dots+c_n\varphi_n\neq0\Rightarrow\lambda\psi=\sum_{j=1}^n\!\BigBigPar{{\sum_{k=1}^nA^t_{j,\;\!k}c_k}}\varphi_k=\sum_{k=1}^nc_k\lambda \varphi_k.$\vspace{3pt}\parSol{}
Let $\aMid{c_j}=\max\!\Bra{\aMid{c_1},\dots,\aMid{c_n}}.$ Now $\lambda c_j=\sum_{k=1}^nA^t_{j,\;\!k}c_k\Rightarrow\aXMid{\lambda-A^t_{j,\;\!j}}\leqslant\sum_{j\neq k=1}^n\aXMid{A_{k,\;\!j}}.$\PfEnd\vspace{5pt}\parSol{}
\Or Use [5.67] directly. \NOTICE that $\BigBra{z\in\Fbb:\aXMid{z-A_{j,\;\!j}}\leqslant\sum_{j\neq k=1}^n\aXMid{A^t_{j,\;\!k}}=\sum_{j\neq k=1}^n\aXMid{A_{k,\;\!j}}}.$\PfEnd\vspace{5pt}
\AComm In other words, $\aXMid{z-A_{k,\;\!k}}\leqslant\sum_{k\neq j=1}^n\aXMid{A_{j,\;\!k}}.$
\SepLine
...
%	\TextA{Find a bss of $\PoFx{m}{\RbbP{2}}$ suth $D_x,D_y$ up-trig in [5.72].}
%}Let $B=\Par{1,x,y,x^2,xy,y^2,\cdots,\cdots,x^m,x^{m-1}y,\cdots,xy^{m-1},y^m}$ in $\PoFx{m}{\RbbP{2}}.$\parSol{}
...
\ChDecl{Ch8}{8}{\quad{\ANote {\FontSmall Supp $V$ is a non0 finide vecsp over $\Fbb.$ \;Let $m_T$ be the min of $T\in\Lm{V}.$}}}\par\vspace{-6pt}
...
Let $u=\Par{T^{-1}-\lambda^{-1}I}{^{j-1}}v$ becs it's too long. Now $\Par{T-\lambda I}u=0\Longleftrightarrow  T^{-1}u=\lambda^{-1}u.$ Immed.\PfEnd
...
\Anchor{8A18}\ProblemN{A.18}{
	\TextA{Supp $\dim V=n.$ Show $\range T^{n}=\range T^{n+1}=\cdots.$\hfill\FontNorm\tgnr By [8.4] and Exe (A.19), immed.\Blind{\quad}\PfEnd}
...
\Or Let $S^p=T^q=0.$ Becs $S,T$ commu, $\Par{ST}{^{\max\!\Bra[\scriptsize]{p,\,q}}}=0=\Par{S+T}{^{p+q}}=\sum_{i=0}^{p+q}\mathC_{p+q}^i\;\!S^i\;\!T^{p+q-i}.$\PfEndB
\SepLine
...
Define $P_{\!\!\!\:1},P_{\!\!\!\:2}\in\Lm{V}$ by $P_{\!\!\!\:1}\Par{v_1+u}=v_1,\,P_{\!\!\!\:2}=I-P_{\!\!\!\:1}.$ \,Let $D=D_1P_{\!\!\!\:1}+D_2P_{\!\!\!\:2},\,N=N_1P_{\!\!\!\:1}+N_2P_{\!\!\!\:2}.$\parSol{}
$D+N=\Par{D_1+N_1}P_{\!\!\!\:1}+\Par{D_2+N_2}P_{\!\!\!\:2}=T,$ \, $DN=D_1N_1P_{\!\!\!\:1}+D_2N_2P_{\!\!\!\:2}=NP.$\PfEnd\vspace{4pt}\parSol{}
...
Now $T=T\mmid_{G_1}P_{\!\!\!\:1}+\dots+T\mmid_{G_m}P_{\!\!\!\:m}.$ \:Let $N_j=T\mmid_{G_j}-\lambda_jI\Rightarrow N_1P_{\!\!\!\:1}+\dots+N_mP_{\!\!\!\:m}:v_{j,\;\!k}\mapsto N_kv_{j,\;\!k}.$\parSol{}
Where each $B_{G_k}=\Par{v_{1,\;\!k},\dots,v_{n_k,\;\!k}}.$ \:Let $D=\lambda_1P_{\!\!\!\:1}+\dots+\lambda_mP_{\!\!\!\:m}:v_{j,\;\!k}\mapsto\lambda_kv_{j,\;\!k}.$\parSol{}
Hence $T=D+N,$ and $DN=ND:v_{j,\;\!k}\mapsto\lambda_kN_kv_{j,\;\!k}.$\PfEnd
\SepLine\pagebreak

%\Anchor{8BN1}\ProblemB{
%	\TextA{Supp $p\in\PoCi$ and $\exists\;\!$non0 $v\in\nullp p\Par{T\big|{_{\:\!G\SmallPar{\:\!\lambda,\,T\:\!}}}}.$ Prove $p\Par{\lambda}=0.$}
%}Let $\lambda_1=\lambda$ and $p\Par{z}=\Par{z-\lambda_1}{^{\alpha_1}}\cdots\Par{z-\lambda_m}{^{\alpha_m}},$ with disti $\lambda$'s and each $\alpha_k\in\Nbp$ except $\alpha_1.$\parSol{}
%\SepLine
...
}We show $G\Par{\lambda, T}=\Null\Par{T-\lambda I}{^m}$ where $m\leqslant d\Rightarrow\Null\Par{T-\lambda I}{^m}\subseteq\Null\Par{T-\lambda I}{^d}.$\vspace{1pt}\parSol{}
Let $N=T-\lambda I,$ and $\null N\subsetneq\cdots\subsetneq\null N^m=\null N^{m+1}.$ Choose $B_{\null N}.$\parSol{}
Extend to $B_{\null N^2}\Rightarrow\cdots\Rightarrow B_{\null N^m},$ with each step adding at least one vec. Thus $m\leqslant d.$\vspace{2pt}\parSol{}
\Or Using the $m$ in (4E B.6) below,  $G\Par{\lambda,T}\subseteq\Null\Par{T-\lambda I}{^m}.$ Now by (4E A.4), $d\geqslant m.$\PfEnd\vspace{4pt}\parSol{}
...
}(a) $\exists\,!\,r\in\PoF{\deg q\,-\,1},s\in\PoF{\deg p\,-\,1}$ suth $\forall v\in V,\,v=r\Par{T}\:\!p\Par{T}\:\!v+s\Par{T}\:\!q\Par{T}\:\!v.$\par\quad
...
Becs `$\supseteq$' holds $\Rightarrow\nullp q\Par{T}\cap\rangep q\Par{T}=\zeroSubs.$ By \Sbra{1.C \TIPSN{1}}. Simlr for (c) and (d) below.\parCor
...
Let $\mA=\Bra{\alpha_1,\dots,\alpha_{\!A}}$ be the intersec of the zeros of $p$ and $m_T.$ \,Simlr for $\mB$ and $q.$\parNot{\IndentB}
