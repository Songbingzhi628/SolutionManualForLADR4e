...
\Anchor{3CN3.49}\BulletPointX\NoteForSmall{[3.49]} ${\BigSbra{\BigPar{AC}_{\cdot,k}}}_{j,1}=\BigPar{AC}_{j,k}=\sum_{r=1}^n A_{j,r}C_{r,k}=\sum_{r=1}^n A_{j,r}\BigPar{C_{\cdot,k}}_{r,1}=\BigPar{AC_{\cdot,k}}_{j,1}$\PfEnd\vspace{6pt}
\Anchor{3C10}\BulletPointX\Exercise{10}\;\;${\BigSbra{\BigPar{AC}_{j,\cdot}}}_{1,k}=\BigPar{AC}_{j,k}=\sum_{r=1}^n A_{j,r}C_{r,k}=\sum_{r=1}^n \BigPar{A_{j,\cdot}}_{1,r}C_{r,k}=\BigPar{A_{j,\cdot}C}_{1,k}$\PfEnd\vspace{6pt}
\BulletPointX\ANote For [3.49], let $B_U=\Par{u_1,\dots,u_p},B_V=\Par{v_1,\dots,v_n},B_W=\Par{w_1,\dots,w_m}.$\vspace{2pt}\parNot{}\IndentB{}
And $C=\Mt{T,B_U,B_V}\in\Fbb^{n,p},A=\Mt{S,B_V,B_W}\in\Fbb^{m,n}.$\vspace{1pt}\parNot{}\IndentB{}
Then $\Mt{Tu_k,B_V}=C_{\cdot,k}\Rightarrow\Mt[\BigPar]{S\Par{Tu_k},B_W}=AC_{\cdot,k}\,,$ \;又 $\Mt[\BigBigPar]{\Par{ST}\Par{u_k},B_W}=\Par{AC}{_{\cdot,k}}$\PfEnd\vspace{4pt}\parNot{}\IndentB{}
By {\NOTEFOR} Trspose, $\BigPar{AC}{_{j,\cdot}}=\BigSbra{\BigBigPar{\BigPar{AC}{^t}}{_{\cdot,j}}}{^t}=\BigPar{C^t\Par{A^t}{_{\cdot,j}}}{^t}=\BigPar{\Par{A^t}{_{\cdot,j}}}{^t}C=A_{j,\cdot}C$\PfEnd
\SepLine
...
\Anchor{3DNE15}\BulletPointX\NoteForSmall{Exe (15)}\TextB{}
Supp $A\in\Fbb^{m,n}.$ Define $T\in\Lm{\Fbb^{n,1},\Fbb^{m,1}}:x\mapsto Ax\Rightarrow A=\Mt{T}$ wrto std bses $B_1,B_2$ of $\Fbb^{m,1},\Fbb^{n,1}.$\TextB{}
Becs $T$ have infily many defs of the form $T:x\mapsto C_2AC_1x,$ where $C_1=\Mt{I,B_1',B_1},C_2=\Mt{I,B_2,B_2'}.$\TextB{}
Now we can well define $\range A=\range T$ and $\null A=\range T,$ becs $A$ is indep of bses.
\SepLine

\Anchor{3DT3}\ProblemBX[]{\TipsN{3}}{
	Identify $\Fbb^{m,n}$ with $\Lm{\Fbb^{n,1},\Fbb^{m,1}},$ due to $Tx=Ax;$ or with $\Lm{\Fbb^{1,m},\Fbb^{1,n}},$ due to $Tx=xA^t.$\TextB{\vspace{-4pt}}
}\SepLine
...
\Anchor{3DNE34e22}\BulletPointX\NoteForSmall{Exe (3, 4E 22)}\;\;$T\in\Lm{V,W}$ inv $\Longleftrightarrow\Mt{T}$ inv, wrto some $B_V,B_W$ $\Longleftrightarrow$ wrto any $B_V,B_W.$\TextB{}
Supp $\Mt{T}$ wrto some bses is inv. Let $S\in\Lm{W,V}$ be suth $\Mt{S}=\Mt{T}{^{-1}}.$\TextB{}
Then $\Mt{TS}=I=\Mt{ST}$ wrto same bses. Apply $\Mneg,$ now $TS=I,ST=I\Rightarrow S=T^{-1}.$\PfEnd\TextB{}
\Or (a) $T\in\Lm{V,W}$ inje $\Longleftrightarrow$ the cols of $\Mt{T,B_V,B_W}$ are liney indep $\Longleftrightarrow\Mt{T}$ inje wrto any bses.\TextB{}
\Blind{\Or}(b) $T\in\Lm{V,W}$ surj $\Longleftrightarrow$ the cols of $\Mt{T,B_V,B_W}$ spans $\Fbb^{\dim\:\!\!W,1}\Longleftrightarrow\Mt{T}$ surj wrto any bses.
\SepLine
...
Let $\lambda=a_{i,i}.$ Hence each $w_j=\lambda v_j.$ Now fix one $j,$ we have $a_{1,1}v_j=\dots=a_{n,n}v_j,$ then all $a_{i,i}$ are equal.\PfEnd
\SepLine
...
}Using [4.17], $\deg p=2M+m\Rightarrow m$ is odd, done.\PfEnd\parSol{\vspace{3pt}}
...
\ProblemN{\Anchor{4O11}{11}}{
	\TextA{Supp $p\in\PoFi$ with $\deg p=m\in\Nbb.$ Let $U=\Bra{pq:q\in\PoFi}$.\vspace{1pt}}
...
Thus $\Range\Par{T-I}\subseteq\Bra{\Par{y_1,\dots,y_n}:y_1+\dots+y_n=0}.$ Now $\Par{T-I}$ is not inv.\PfEnd\vspace{3pt}\quad\Hb
...
Becs $Ae_k=A_{\cdot,k}=\sum_{j=1}^nA_{j,k}e_j\Rightarrow{\psi\Par{T-I}}{e_k}=\psi\BigBigPar{{\sum_{j=1}^n A_{j,k}e_j}-e_k}=\sum_{j=1}^n A_{j,k}-1=0.$\PfEnd\vspace{3pt}\quad\Hb
...
Thus $\Range\Par{T-I}\subseteq\Bra{\Par{y_1,\dots,y_n}:y_1+\dots+y_n=0}.$ Now $\Par{T-I}$ is not inv.\PfEnd\vspace{3pt}\quad\Hb
\Or Simlr in Exe (24). Becs $e_jA=A_{j,\cdot}=\sum_{k=1}^n A_{j,k}e_k\Rightarrow{\psi\Par{T-I}}{e_j}={\sum_{k=1}^n A_{j,k}}-1=0.$\PfEnd\vspace{3pt}\quad\Hb
