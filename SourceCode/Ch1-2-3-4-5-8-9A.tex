...
\Anchor{5BI4e16}\Anchor{8C18}\ProblemBnoor{8.C.18}{
	\TextA{Define $T\in\Lm{\Fbb^n}:\Par{x_1,\dots,x_n}\mapsto\BigPar{{-a_0\,x_n,\,x_1-a_1\hspace{1pt}x_n,\,\cdots,\,x_{n-1}-a_{n-1}\hspace{1pt}x_n}}.$}
	\TextA{Show the min $p$ of $T$ is $q\Par{z}=a_0+a_1z+\dots+a_{n-1}z^{n-1}+z^n.$}
}Becs $Te_0=e_1,\,T^2e_0=e_2,\cdots,\,T^{n-1}e_0=e_{n-1},\,T^ne_0=T^{n-k}e_k=Te_{n-1}=-\Par{a_0\hspace{1pt}e_0+\dots+a_{n-1}\hspace{1pt}e_{n-1}}.$\parSol{}
%=-a_0Ie_0-a_1Te_0-\dots-a_{n-1}T^{n-1}e_0.
Let $-T^ne_0=c_0e_1+c_1e_1+\dots+c_{n-1}e_{n-1}\Rightarrow c_k=a_k.$%TODO
\SepLine
...
\Anchor{5ET1}\ProblemBX{\Tips}{
	\TextA{Supp $S,T\in\Lm{V}$ commu. Prove $p\Par{S}q\Par{T}=q\Par{T}p\Par{S}$ for all $p,q\in\PoFi.$}
}Becs each $S^j$ and each $T^k$ commu. Now $LHS=\sum\sum a_jb_kS^jT^k=\sum\sum a_jb_kT^kS^j=RHS.$\PfEnd
\SepLine
...
\ChDecl{Ch8A}{8.A}{\quad{\ANote {\FontSmall $V$ denotes a finide non0 vecsp over $\Fbb.$ An Exe marked by $\blacksquare$ is true if infinide or partially finide.}}}
...
\Anchor{8A6}\ProblemN{6}{
	\TextA{Supp $T\in\Lm{V}$ nilp and $n=\dim V,T^{n-1}\neq0.$ Prove $\nexists\,S\in\Lm{V},\,S^k=T$ for all $k>1.$}
...
\Anchor{8A4e18}\ProblemBnoor{4E 18}{
...
\Anchor{8A4e12}\ProblemBnoor{4E 12}{
...
\Anchor{8A4e15}\ProblemBnoor{4E 15}{
	\TextA{Supp $\Fbb=\Cbb,T\in\Lm{V}.$ Prove non0 $T$ diag $\Rightarrow$ each gigvec is an eigvec.}
}Supp $V=E\Par{\lambda_1,T}\oplus\cdots\oplus E\Par{\lambda_m,T};$ $\lambda_1=0$ if possible, in this case $m>1.$\parSol{}
Supp $w\in G\Par{\lambda_j,T}.$ Then $w=v_1+\dots+v_m,$ where each $v_k\in E\Par{\lambda_k,T}.$\parSol{}
Becs $\Par{T-\lambda_jI}{^k}w=0=\sum_{i=1}^m\lambda_i\Par{\lambda_i-\lambda_k}{^m}v_i\Rightarrow w=v_k,$ othws ctradic.\PfEnd\vspace{3pt}\parSol{}
\Or Supp $w\in G\Par{\lambda_j,T}$ and $w$ not in any $E\Par{\lambda_i,T}.$\parSol{}
...
\Anchor{9A18}\ProblemN{18}{
	...
	\PrePb\TextA{Show all eigvals of $T_{\!\Cbb}$ are real $\Rightarrow\exists\,B_V$ of gigvecs of $T.$}
