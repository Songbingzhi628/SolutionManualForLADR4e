...
\NOTICE that $\forall k=\Bra{0,\dots,A-2},s{^{\SmallPar{k}}}$ and $s{^{\SmallPar{k+1}}}$ have zeros $\envFontLarge\Bra{{\envFontDefault\lambda\Sbra{I_{k+1,1}},\dots,\lambda\Sbra{I_{k+1,\,J_{k+1}}}}}$ in common.\vspace{2pt}\par\quad
Now $\forall D\in\Bra{1,\dots,A-1},s=s{^{\SmallPar{0}}},\dots,s{^{\SmallPar{D}}}$ have zeros $\envFontLarge\Bra{{\envFontDefault\lambda\Sbra{I_{D,1}},\dots,\lambda\Sbra{I_{D,\,J_D}}}}$ in common.\vspace{2pt}\par\quad
...
\Hc\AExa Forward and backward shift operators on $\Fbb^\infty.$
\SepLine

\ProblemB{
	\TextB{Supp $V$ is finide, $T\in\Lm{V},$ and $U$ is invarsp of $\,V$ under $T.$}
	\TextB{Prove or give a counterexa\hspace{1pt}$:$ there exis invarsp of dimension $\dim V-dim U.$}
}\parSol{}
\SepLine
...
\ProblemB{
	\TipsN{2}\,\,\TextB{Supp $\dim V=m\Rightarrow B_V=\Par{v_1,\dots,v_m}.$ Let each $E_k=E\Par{\lambda_k,T}=\Span{v_k}.$}
...
\ProblemN{\hypertarget{5A20}{20}}{
	\TextNL{Define $S\in\Lm{\Fbb^\infty}$ by $S\Par{z_1,z_2,z_3,\dots} = \Par{z_2,z_3,\dots}.$}
...
\ProblemN{\hypertarget{5A14}{14}}{
	\TextNL{Supp $V=U\oplus W$, where $U,W$ nonzero. Define $P\Par{u + w} = u.$}
...
(a) Supp $\lambda$ is eigval with $v.$ Let $U$ be invar with $U\oplus\Span{v}=V,$ by Exe (4E 39).\par\quad\Ha
Define $\psi\in V\apostrophe$ by $\psi\Par{cv+u}=c.$ Then $\Sbra{T\apostrophe\Par{\psi}}\Par{cv+u}=\psi\BigPar{c\lambda v+Tu}=\lambda c=\lambda\psi\Par{cv+u}.$\vspace{4pt}\par\quad
(b) Supp $\lambda$ is eigval $T\apostrophe$ with $\psi.$ Then $T\apostrophe\Par{\psi}=\psi\circ T=\lambda\psi.$\vspace{2pt}\par\quad\Hb
Note that $\psi\neq 0,\psi\Par{Tv}=\lambda\psi\Par{v}.$ Thus $\exists\,v,\;Tv={}${\Large\envFontSmall[\footnotesize]$\frac{\psi\Par{Tv}}{\psi\Par{v}}$}$\,v=\lambda v.$\PfEnd\vspace{6pt}\quad
\Or \Sbra[3pt]{{\tgsl Req Finide}} \;${T-\lambda I_V}$ not inv $\Longleftrightarrow\Par{T-\lambda I_V}\apostrophe=T\apostrophe-\lambda I_{V\apostrophe}$ not inv.\PfEnd
\SepLine
...
(b) Becs $0$ is not an eigval of $T$ or $T^{-1}.$ By (a), immed.\PfEnd
\SepLine
...
}Supp $\lambda$ is eigval of $ST$ with $v.$ Then $T\Par{STv}=\lambda Tv=TS\Par{Tv}.$\parSol{}
If $Tv=0$, then $T$ not inje $\Rightarrow TS,ST$ not inje. An eigval of $TS,ST$ with the same $v.$\parSol{}
...
$T\Par{v+w}=\lambda_{v+w}\Par{v+w}=Tv+Tw=\lambda_v v+\lambda_w w\Rightarrow\Par{\lambda_{v+w}-\lambda_v}v=\Par{\lambda_w-\lambda_{v+w}}w.$\PfEnd\vspace{4pt}\parSol{}
...
\ProblemBnoor{\hypertarget{5A4e39}{4E 39}}{
	\TextB{Supp $T\in\Lm{V},\dim V=n.$ Prove $T$ has an eigval $\Longleftrightarrow\exists\,U$ invard $T$ of dim $n-1$.}
}\par\quad
