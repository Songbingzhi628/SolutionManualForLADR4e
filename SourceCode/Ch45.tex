...
\Or Supp $y\neq 0$ with $\Par{\lambda x,\lambda y}=\Par{{-3y,x}}\Rightarrow-3y=\lambda^2 y\Rightarrow \lambda^2=-3.$ \Sbra{If $y=0,$ then $x=\lambda y=0.$}\PfEnd
\SepLine
...
}Let $p\Par{z}=\Par{z-\lambda}q\Par{z}\Rightarrow p\Par{T}v=\Par{T-\lambda I}q\Par{T}v\Rightarrow T\BigPar{q\Par{T}v}=\lambda q\Par{T}v.$\PfEnd
\SepLine
...
\Anchor{5BIT1}\ProblemBX{\TipsN{1}}{
...
\Or By (3.F.15), for any $s\in\PoFi,\:s\Par{T\apostrophe}=s\Par{T}\apostrophe=0\Longleftrightarrow s\Par{T}=0.$\PfEnd
\SepLine
...
%}(a) $\forall u+\i 0\in V_{\!\Cbb},\,p\Par{T_{\!\Cbb}}\Par{u}=p\Par{T}u=0\Rightarrow p\Par{T}=0.$\parSol{}
%(b) $q\Par{T}=0\Rightarrow \forall u+\i v\in V_{\!\Cbb},\,q\Par{T_{\!\Cbb}}\Par{u+\i v}=q\Par{T}u+\i q\Par{T}v=0.$\PfEnd
%\SepLine
...
%\Anchor{5BI4e12}\ProblemBnoor{4E 12}{
...
%\Anchor{5BII4e1}\ProblemN{1}{
%	\TextA{Supp $T^2$ has up-trig matrix. Give a countexa\hspace{1pt}$:$ $T$ has up-trig matrix.}
%}Let $\Fbb=\Rbb.$ Define $T\Par{x,y}=\Par{{-y,x}}.$ Then $T^2=-I.$ While $T$ has no eigval.\PfEnd
%\SepLine
...
Thus $T^{-1}v_k=\lambda_k^{-1}v_k-\lambda_k^{-1}T^{-1}u_k\in\Span{v_1,\dots,v_k},$ invard $T;$ and $\lambda_{k}^{-1}$ is the $k$-th ent on diag.\PfEnd
\SepLine

\Anchor{5BII9}\Anchor{5BII4e7}\ProblemNnoor{7}{3E 5.B.9}{
	\TextA{Supp $V$ is finide, and $v \in V$.}
	\PrePa\TextA{Prove $\exists\,!$ monic $p_v$ of smallest deg suth $p_v\Par{T}v = 0$.}
	\PrePb\TextA{Prove the min poly $p$ of $T$ is a poly multi of $p_v$.}
}(a) {\Existns} \;If $v=0,$ then let $p_v\Par{z}=1.$ Supp $v\neq0.$ Then $\Par{v,Tv,\dots,T^{\dim V}v}$ liney dep.\parSol{\Ha}
\Blind{\Existns} \;$\exists$ smallest $m$ suth $-T^mv=c_0v+c_1Tv+\dots+c_{m-1}T^{m-1}v.$ Thus define $p_v.$\parSol{\Ha}
\Blind{\Existns} \;\Or Let $U=\Span{v,Tv,\dots,T^{m-1}v}$ of dim $m$ invard $T.$ Let $p_v$ be the min of $T\mmid_U.$\vspace{2pt}\parSol{\Ha}
...
(b) \NOTICE that $p_v$ is the min of $T\mmid_{\nullp p_v\SmallPar{T}}.$ (?) Then by [4E 5.31].\PfEnd
\SepLine
...
}Define $p_v$ by Exe (7). Note that $\deg p_v\geqslant 1$ becs $v\neq 0.$ And $q\Par{T\mmid_{\nullp p_v\SmallPar{v}}}=0\Rightarrow q$ is multi of $p_v.$\parSol{}
(a) Note that $q$ has no liney factors $\Rightarrow \deg p_v\geqslant 2$. Hence by the min of $\deg p_v,$\parSol{\Hb}
and by (b) in Exe (7), the min of $T$ is multi of $p_v=q.$ By [4E 5.44], done.\parSol{}
...
\Anchor{5BII4e9}\ProblemN{9}{
	\TextA{Supp $\Fbb=\Cbb,$ $B\in\Fbb^{n,n}.$ Prove $\exists$ inv $A\in\Fbb^{n,n}$ suth $A^{-1} BA$ is up-trig.}
}Define $T\in\Fbb^n$ with $B=\Mt[\BigPar]{T,\Par{e_1,\dots,e_n}}$ Let $C=\Mt[\BigPar]{T,\Par{f_1,\dots,f_n}}$ be up-trig.\parSol{}
...
\Anchor{5BII4e11}\ProblemN{11}{
	\TextA{Supp $\Fbb=\Cbb,$ $V$ is finide. Prove $\exists\,B_V$ suth $\Mt{T}$ low-trig.}
}(i) $\dim V=0,1.$ Done. (ii) $\dim V>1.$ Asum the desired result is true for dim ${}<\dim V$.\parSol{}
Let $\lambda$ be eigval of $T$ by [5.21]. Let $U=\Range\Par{T-\lambda I},$ invard $T$ and of dim ${}<\dim V.$\parSol{}
Now $T\mmid_U\in\Lm{U},$ by asum, has a $B_U=\Par{u_1,\dots,u_m}$ suth $\Mt{T\mmid_U,B_U}$ low-trig.\parSol{}
Then each $Tu_j\in\Span{u_j,\dots,u_m}.$ \,Let $B_V=\Par{v_1,\dots,v_n,u_1,\dots,u_m}.$\parSol{}
Each $\Par{T-\lambda I}v_k\in\Span{u_1,\dots,u_m}\Rightarrow Tv_k=\Par{T-\lambda I}v_k+\lambda v_k\in\Span{v_k,\dots,v_n,u_1,\dots,u_m}.$\PfEnd\vspace{2pt}\parSol{}
\Or Simlr, using induc on $\dim V.$ Let $v_1$ be eigvec. Let $U=\Span{v_1},$ invard $T.$\parSol{}
Then $V\Slash U$ invard $T$ of dim $\dim V-1.$ Now $T\XSlash U\in\Lm{V\XSlash U}.$\parSol{}
By asum, $T\XSlash U$ has a $B_{V\XSlash U}=\Par{v_2+U,\dots,v_n+U}$ suth $\Mt{T\XSlash U}$ low-trig.\parSol{}
Now for each $j\in\Bra{1,\dots,n},\:\BigPar{T\XSlash U}\Par{v_j+U}=Tv_j+U\in\Span{v_j+U,\dots,v_n+U}.$\parSol{}
又 By (3.E.13), $B_V=\Par{v_1,\dots,v_n}.$ \,Thus $Tv_j\in\Span{v_j,\dots,v_n}.$\PfEnd
\SepLine
...
\Anchor{5BII4e14}\ProblemN[]{14}{
	\TextA{Supp $V$ is finide. Prove $T$ up-trig $\Longleftrightarrow$ $T\apostrophe$ up-trig.\hfill\FontNorm\tgnr By (4E 18) and [4E 5.44], immed.\vspace{-3pt}}
}\SepLine
\ChEnd\pagebreak

\ChDecl{Ch5C}{5.C \& [4E] 5.D}{}

\Anchor{5C4e1}\ProblemBnoor{4E 1}{
	\TextA{Supp $\Fbb=\Cbb,$ and $T\in\Lm{V}.$ Show if {\tgnr\large(a)} $T^4=I,$ or if {\tgnr\large(b)} $T^2=T,$ then $T$ diag.}
}(a) By Exe (4E 14). \Or Prove the ctrapos.\parSol{}
(b) Supp $T\Par{T-I}=0.$ Then $T$ inje $\Rightarrow\Par{T-I}=0,$ and $\Par{T-I}$ inje $\Rightarrow T=0.$\parSol{\Hb}
And $T$ not inje $\Rightarrow\Par{T-I}\big|{_{\range T}}=0,$ and $\Par{T-I}$ not inje $\Rightarrow T\mmid_{\Range\SmallPar{T-I}}=0.$\PfEnd
\AExa \BigPar{$T\in\Lm{\Cbb^2}$ not diag and $T^4=T^2.$}
\SepLine

%\Anchor{5CT1}\ProblemBX{\TipsN{1}}{
%	\TextA{For $T\in\Lm{V},\,E\Par{0,T}=\null T.$\vspace{-4pt}}
%}
...
\Anchor{5C5}\ProblemN{5}{
	...
	\TextA{Prove $T$ diag $\Longleftrightarrow V=\Null\Par{T-\lambda I}\oplus\Range\Par{T-\lambda I}$ for all $\lambda\in\Fbb.$}
}Supp $V=\Null\Par{T-\lambda I}\oplus\Range\Par{T-\lambda I}$ for all $\lambda\in\Fbb.$\parSol{}
\NOTICE that $S_\lambda=\Par{T-\lambda I}$ is surj for all non-eigval $\lambda.$ %Let $B_V=\Par{S_{\lambda}v_1,\dots,S_{\lambda}v_n}.$\parSol{}
\parSol{}
\parSol{}
Convly, becs $T,I\in\Lm{V}$ diag wrto same $B_V\Rightarrow\Par{T-\lambda I}$ diag.\PfEnd
\SepLine
...
\Anchor{5C4e13}\ProblemBnoor{4E 13}{
	\TextA{Supp $A,B\in\Fbb^{n,n}$ and $A$ is diag with dist ents on diag. Prove $AB=BA\Longleftrightarrow B$ is diag.}
...
\AComm Not true if $\Fbb=\Rbb.$ Exa: $T\in\Lm{\Rbb^2}:\Par{x,y}\mapsto\Par{{-y,x}}.$ No eigvals.
\SepLine
...
\Anchor{3C4e17}\ProblemBnoor{4E 17}{
	\TextA{Supp $V$ is finide. Prove $\Lm{V}$ has a bss consisting of diag optors.}
}
\SepLine

\Anchor{3C4e18}\ProblemBnoor{4E 18}{
	\TextA{Supp $T\in\Lm{V}$ is diag. Prove if $U$ invarspd $T,$ then $T\XSlash U\in\Lm{V\XSlash U}$ is diag.}
}\SepLine

\Anchor{3C4e19}\ProblemBnoor{4E 19}{
	\TextA{Give an exa of $T\in\Lm{V}$ not diag while $T\mmid_U,T\XSlash U$ both diag, where $U$ invarspd $T.$}
}
\SepLine

\Anchor{3C4e20}\ProblemBnoor{4E 20}{
	\TextA{Supp $V$ finide, $T\in\Lm{V}.$ Prove $T$ diag $\Longleftrightarrow T\apostrophe$ diag.}
}\SepLine

\Anchor{5C16}\ProblemN{16}{
	\TextA{Define $F_1=1,F_2=2,$ and $F_n=F_{n-1}+F_{n-2}$ for $n\geqslant 3.$}
	\TextA{Define $T\in\Lm{\Rbb^2}:\Par{x,y}\mapsto\Par{y,x+y}.$\vspace{2pt}}
	\PrePa\TextA{Show each $T^n\Par{0,1}=\Par{F_n,F_{n+1}}.$\vspace{3pt}}
	\PrePb\TextA{Find all eigvals of $T.$ \, {\tgnr\large(c)} Find a bss of $\Rbb^2$ consisting of eigvecs of $T.$\vspace{-4pt}}
	\PrePd\TextA{Show each $F_n={}${\LARGE$\frac{1}{\sqrt{5}\;}$}$\,\bigg[\bigg(\!${\LARGE$\frac{\:1\,+\,\sqrt{5}\:}{2}$}$\!\bigg)\begin{array}{l}\hspace{-5pt}{}_n\\\\\vspace{-18pt}\end{array}\hspace{-8pt}-\bigg(\!${\LARGE$\frac{\:1\,-\,\sqrt{5}\:}{2}$}$\!\bigg)\begin{array}{l}\hspace{-5pt}{}_n\\\\\vspace{-18pt}\end{array}\hspace{-5pt}\bigg].$\vspace{-21pt}}
	\PrePe\TextA{Show each $F_n$ is the integer that closest to {\LARGE$\frac{1}{\sqrt{5}\;}$}$\,\bigg(\!${\LARGE$\frac{\:1\,+\,\sqrt{5}\:}{2}$}$\!\bigg)\begin{array}{l}\hspace{-5pt}{}_n\\\\\vspace{-18pt}\end{array}\hspace{-8pt}.$}
}\SepLine
...
Thus $\Span{e_1}$ invard $T$ but not $S,$ and $\Span{e_3}$ invard $S$ but not $T.$
\SepLine

\Anchor{5E2}\ProblemN{2}{
	\TextA{Supp $\mathcal{E}\subseteq\Lm{V}$ and every elem of $\mathcal{E}$ diag.}
	\TextA{Prove $\exists\,B_V$ suth all elem of $\mathcal{E}$ diag $\Longleftrightarrow$ each pair of elems of $\mathcal{E}$ commu.}
}\par
\SepLine
...
\Anchor{5E7}\ProblemN{7}{
	\TextA{Supp $\Fbb=\Cbb,$ $S,T\in\Lm{V}$ commu, and $S$ diag. Prove $\exists\,B_V$ suth $S$ diag and $T$ up-trig.}
}\par
\SepLine
...
\Anchor{5E9}\ProblemN{9}{
	...
	\PrePa\TextA{Prove $\exists$ eigvec $v\in V$ for every elem of $\mathcal{E}$.}
	\PrePb\TextA{Prove $\exists$ a bss of $V$ wrto which every elem of $\mathcal{E}$ has up-trig matrix.}
}\par
\SepLine

\Anchor{5E10}\ProblemN{10}{
