...
\Or We shat if $p$ has at least $m$ disti zeros, then either $p=0$ or $\deg p\geqslant m.$\TextB{}
...
\BulletPointX\NoteFor{[4.8]} {\tgsl division algo for polys} \hfill[{\tgsc Another proof}]\TextB{\vspace{4pt}}
...
\ProblemBnoor{\hypertarget{4O4e2}{4E 4.2}}{
	\TextB{Provt if $w,z\in\Cbb,$ then $\aXMid{\aMid{w}-\aMid{z}}\leqslant\aMid{w-z}.$}
}\par\quad
\vspace{-6pt}\AlignEq{}{
...
\ProblemBnoor{\hypertarget{4O4e3}{4E 4.3}}{
	...
	\TextB{Shat $\varphi\Par{v}=\sigma\Par{v}-\i\sigma\Par{\i v}$ for all $v\in V.$\vspace{2pt}}
...
\ProblemN{\hypertarget{4O4}{4}}{
	...
	\TextN{Provt $\exists\,p\in\PoFi,\deg p = n,$ the zeros of $\,p\,$ are $\lambda_1,\dots,\lambda_m.$\vspace{3pt}}
...
\ProblemN{\hypertarget{4O5}{5}}{
	...
	\TextN{Provt $\exists\,!\,p\in\PoF{m},p\Par{z_k}=w_k$ for each $k\in\Bra{1,\dots, m+1}.$}
}\par\quad
...
We now shat $T$ is surj, so that such $p$ exists;\; and that $T$ is inje, so that such $p$ is unique.\vspace{1pt}\par\quad
...
\ProblemN{\hypertarget{4O6}{6}}{
	\TextN{Sup nonzero $p\in\PoF{m}$ has deg $m.$ Provt}
	\TextN{[P] $p$ has $m$ disti zeros $\Longleftrightarrow p$ and its deri $p\apostrophe$ have no zeros in common [Q].}
}\par\quad
...
Becs nonzero $p\in\PoF{m},$ we sup $\lambda_1,\dots,\lambda_M$ are all the disti zeros of $p,$ where $M<m.$\vspace{2pt}\par\quad\Hb
...
\ProblemN{\hypertarget{4O7}{7}}{
	\TextN{Provt every $p\in\PoRi$ of odd deg has a zero.}
}\par\quad
Using the nota and proof of [4.17]. $\deg p=2M+m$ is odd $\Rightarrow m$ is odd. Hence $\lambda_1$ exists.\PfEnd\vspace{6pt}\quad
...
\ProblemN{\hypertarget{4O9}{9}}{
	\TextN{Sup $p\in\PoCi$. Define $q:\Cbb\rightarrow\Cbb\,$ by $q\Par{z} = p\Par{z}\overline{p\Par{\overline z}}$. Provt $q\in\PoRi$.}
}\par\quad
...
\ProblemN{\hypertarget{4O8}{8}}{
	...
	\TextN{Shat {\tgnr(a)} $Tp\in\PoRi$ for all $p\in\PoRi$ and that {\tgnr(b)} $T:\PoRi\rightarrow\PoRi$ is linear.}
}\vspace{2pt}\par\quad
...
(b) Now we shat $T$ is linear: $\forall p,q\in\PoRi,\lambda\in\Rbb,$\vspace{6pt}\par\quad\Hb
...
\ProblemN{\hypertarget{4O11}{11}}{
	...
	(a) \TextNL{Shat $\dim\PoFi\XSlash U = \deg p$.}
...
\Or Define $R:\PoFi\rightarrow\PoF{\deg p-1}$ by $R\Par{s}=r$ for all $s\in\PoF.$ We shat $R$ is linear.\vspace{2pt}\par\quad\Ha
...
\ProblemN{\hypertarget{4O10}{10}}{
	\TextNL{Sup $m\in\Nbb,p\in\PoC{m}$ is suth $p\Par{x_k}\in\Rbb$ for each of disti $x_0,x_1,\dots,x_m\in\Rbb.$}
	\TextNL{Provt $p\in\PoRi$.}
}\par\quad
...
\ProblemBnoor{\hypertarget{4O4e13}{4E 4 13}}{
	...
	\TextB{Define $T:\PoC{n-1}\times\PoC{m-1}\rightarrow\PoC{m+n-1}$ by $T\Par{r, s} = rp + sq.$ Provt $T$ is iso.\vspace{3pt}}
	\Corollary \,\,\,\TextB{$\exists\,!\,r\in\PoC{n-1},s\in\PoC{m-1}$ suth $rp + sq = 1.$}
}\par\quad
$T$ is linear becs $\forall\,r_1,r_2\in\PoC{n-1},s_1,s_2\in\PoC{m-1},\lambda\in\Fbb,$\par\quad
...
Now sup $M,N\geqslant 1.$ We shat $s=0.$ Showing $r=0$ is almost the same.\vspace{2pt}\par\quad
...
We use induction by $D$ to shat $s{^{\SmallPar{D}}}\Par{\lambda_{\gamma_{D,j}}}=0$ for each $D\in\Bra{0,\dots,A-1}.$\vspace{2pt}\par\quad
...
\XPar{ Becs $\forall p,q\in\PoFi,k\in\Nbp,\Par{pq}{^{\SmallPar{k}}}=\mathC_{k}^k p{^{\SmallPar{k}}}q{^{\SmallPar{0}}}+\dots+\mathC_{k}^j p{^{\SmallPar{j}}}q{^{\SmallPar{k-j}}}+\dots+\mathC_{k}^0 p{^{\SmallPar{0}}}q{^{\SmallPar{k}}}.$ }\hfill{$\Par{\Delta}$}\vspace{4pt}\par\quad\Hii
...
\ProblemN{L1:}{
	\TextB{Provt $\forall p,q\in\PoFi,k\in\Nbp,\Par{pq}{^{\SmallPar{k}}}=\mathC_{k}^k p{^{\SmallPar{k}}}q{^{\SmallPar{0}}}+\dots+\mathC_{k}^j p{^{\SmallPar{j}}}q{^{\SmallPar{k-j}}}+\dots+\mathC_{k}^0 p{^{\SmallPar{0}}}q{^{\SmallPar{k}}}.$}
}\par\quad
...
\ProblemN{L2:}{
	\TextB{Sup $p\Par{z}=\Par{z-\lambda}{^\alpha}q\Par{z}$ and $\alpha\in\Nbp.$ Provt ${p{^{\SmallPar{\alpha-1}}}}\Par{\lambda}=0.$}
}\par\quad
Sup $p\in\PoFi.$ Write $p\Par{z}=\Par{z-\lambda}{^A}q\Par{z},$ where $A\in\Nbp,q\Par{\lambda}\neq 0.$\vspace{2pt}\par\quad
We use induction to shat for all $\alpha\in\Bra{1,\dots,A},{p{^{\SmallPar{\alpha-1}}}}\Par{\lambda}=0.$\vspace{2pt}\par\quad
...
More generally, sup we do not know whether $V$ is finid. We shat $(a)\Longleftrightarrow(b).$\TextB{}
...
%\TextB{Sup $T\in\Lm{V}.$ Provt every list of eigvecs of $T$ corres to disti eigvals is linely inde.}
%\Solution \Or Another Proof.\TextB{}
%Sup the desired result is false. Then ( $m\neq 1$ becs eigvecs are nonzero )\TextB{}
%$\,\exists$ smallest $m>1$ so that $\,\exists\,\Par{v_1,\dots,v_m}$ linely depe eigvecs of $T$ corres to disti eigvals $\lambda_1,\dots,\lambda_m$ of $T.$\TextB{}
...
%Thus $(v_1 ,\dots, v_{m-1})$ of len $(m-1)$ is linely depe eigvecs corres to disti eigvals.\TextB{}
...
\Hc\Example\,\, In infinid only. Let $V=\Fbb^\infty.$\TextE{}
...
\BulletPointX\NoteForSmall{[5.2]}\,\,\,Sup $T\in\Lm{V}.$ Then $U$ is invarsp of $V$ under $T\Longleftrightarrow\range T\mmid_U\subseteq U.$
\SepLine

\ProblemB{
	\TextB{Sup $V$ is finid, $T\in\Lm{V},$ and $U$ is invarsp of $\,V$ under $T.$}
	\TextB{Provt there exists invarsp $W$ of dimension $\dim V-dim U.$}
}\par\quad
...
\ProblemB[]{
	\TextB{Sup $T\in\Lm{V}$ and $U$ is invarsp of $V$ under $T.$}
	\TextB{Sup $\lambda_1,\dots,\lambda_m$ are the disti eigvals of $T$ corres eigvecs $v_1,\dots,v_m.$}
}
\ProblemB{
	\TipsN{1}\,\,\TextB{Provt $v_1+\dots+v_m\in U\Longleftrightarrow$ each $v_k\in U.$}
}\par\quad
Sup each $v_k\in U.$ Then becs $U$ is a subsp, $v_1+\dots+v_m\in U.$\par\quad
...
\ProblemB{
	\TipsN{2}\,\,\TextB{If $\dim V=m.$ Provt $U=\Par{U\cap E_1}\oplus\cdots\oplus\Par{U\cap E_m},$ where $E_k=\Span{v_k}.$}
}\par\quad
...
\ProblemB{
	...
	\Blind{\TipsN{3}\,\,}\TextB{Provt $W=\Span{v_{\alpha_1},\dots,v_{\alpha_A}}$ for some disti $\alpha_1,\dots,\alpha_A\in\Bra{1,\dots,m}.$}
}\par\quad
...
\ProblemB{
	\TextB{Sup $S,T\in\Lm{V}$ are suth $ST=TS.$ }
	(a) \TextB{Provt $\null\Par{T-\lambda I}$ is invard $S$ for any $\lambda\in\Fbb$.}
	(b) \TextB{Provt $\range\Par{T-\lambda I}$ is invard $S$ for any $\lambda\in\Fbb$.}
}\par\quad
...
\ProblemB[]{
	\TextB{Sup $S,T\in\Lm{V}$ are suth $ST=TS.$\vspace{4pt}}
}
\ProblemN[]{2}{
	\TextB{Shat $W=\null T$ is invard $S$.\quad\large$\forall u\in W,Tu=0\Rightarrow STu=0=TSu\Rightarrow Su\in W.$\PfEnd\vspace{3pt}}
}
\ProblemN[]{3}{
	\TextB{Shat $U=\range T$ is invard $S$.\,\,\,\large$\forall w\in U,\exists\,v\in V,Tv=w, TSv=STv=Sw\in U.$\PfEnd}
}
\SepLine
...
\ProblemN{\hypertarget{5A6}{6}}{
	\TextN{Sup $U$ is invarsp of $V$ under each $T\in\Lm{V}.$ Shat $U = \zeroSubs$ or $U = V$.\vspace{4pt}}
}If $V=\zeroSubs.$ Then we are done. Sup $V\neq\zeroSubs.$ We show the ctrapos:\parSol{}
{\tgsl Sup $U\neq\zeroSubs$ and $U\neq V.$ Provt $\exists\,T\in\Lm{V}$ suth $U$ is not invard $T$.}\parSol{}
...
\ProblemN{\hypertarget{5A18}{18}}{
	\TextNL{Define the forward shift optor $T\in\Lm{\Fbb^\infty}$ by $T\Par{z_1,z_2,\dots} = \Par{0, z_1,z_2,\dots}.$}
	\TextNL{Shat $T$ has no eigvals.}
...
\ProblemN{\hypertarget{5A20}{20}}{
	\TextNL{Define the backward shift optor $S\in\Lm{\Fbb^\infty}$ by $S\Par{z_1,z_2,z_3,\dots} = \Par{z_2,z_3,\dots}.$\vspace{4pt}}
	(a) \TextNL{Shat every ele of $\Fbb$ is an eigval of $S;\;\;$ {\tgnr\large(b)} Find all eigvecs of $S$.}
}\par\quad
...
\ProblemB[]{
	\TextB{Sup $V$ is finid, $T\in\Lm{V},\lambda\in\Fbb.$}
}
\ProblemN{\hypertarget{5A13}{13}}{
	 \TextB{Provt $\forall\lambda\in\Fbb,\exists\,\alpha\in\Fbb,\left|\alpha-\lambda\right|<\frac{1}{\;1000\;},\Par{T-\alpha I}$ is inv.}
}\par\quad
...
\ProblemBnoor{4E 5.A.11}{
	\TextB{Provt $\exists\,\delta > 0$ suth $\Par{T-\alpha I}$ is inv for all $\alpha\in\Fbb$ suth $0 < \aMid{\alpha-\lambda} < \delta$.}
}\par\quad
...
\ProblemBnoor{\hypertarget{5A4e8}{}{5.B.4} \,\OR\, 4E 3.B.27}{
	\TextB{Sup $\lambda$ is an eigval of $P\in\Lm{V},P^2 = P.$ Provt $\lambda = 0$ or $\lambda = 1$.}
...
\ProblemN{\hypertarget{5A15}{15}}{
	...
	(a) \TextNL{Provt $T$ and $S^{-1}TS$ have the same eigvals.\vspace{3pt}}
...
\ProblemBnoor{\hypertarget{5A4e16}{4E 5.A.16}}{
	...
	\TextB{Provt if $\lambda$ is an eigval of $T$, then $\aMid{\lambda}\leqslant n \max\!\Bra{\aMid{A_{j,k}}:1\leqslant j, k\leqslant n}.$}
}\par\quad
Sup $v$ is an eigval of $T$ corres to $\lambda.$ Let $v=c_1v_1+\dots+c_nv_n.$\vspace{2pt}\par\quad
...
\ProblemBnoor{\hypertarget{5A4e15}{4E 5.A.15}}{
	...
	\TextB{Shat $\lambda$ is an eigval of $T$ $\Longleftrightarrow$ $\lambda$ is an eigval of the dual optor $T\apostrophe\in\Lm{V\apostrophe}$.}
}\par\quad
...
\Or [{\tgsl Only in Finid }] Using [5.6], (4E 3.F.17), [3.101] and (3.F.12).\par\quad
...
\ProblemN{\hypertarget{5A24}{24}}{
	...
	(a) \TextNL{Sup the sum of the ent in each row of $A$ equals $1$. Provt $1$ is an eigval of $T$.}
	(b) \TextNL{Sup the sum of the ent in each col of $A$ equals $1$. Provt $1$ is an eigval of $T$.}
}\vspace{-8pt}\par\quad
...
\Or We shat $\Par{T-I}$ is not inv, so that $\lambda=1$ is an eigval.\vspace{2pt}\par\quad\Hb
...
\ProblemB{
	...
	(a) \TextB{Sup the sum of the ent in each col of $A$ equals $1$. Provt $1$ is an eigval of $T$.}
	(b) \TextB{Sup the sum of the ent in each row of $A$ equals $1$. Provt $1$ is an eigval of $T$.}
}\par\quad
...
\Or We shat $\Par{T-I}$ is not inv, so that $\lambda=1$ is an eigval.\par\quad\Hb
...
\ProblemB{
	...
	(a) \hypertarget{5A4e17}{}\Sbra{{\normalsize\OR (9.11)}} \TextB{$\lambda\in\Rbb.$ Provt $\lambda$ is an eigval of $T$ $\Longleftrightarrow$ $\lambda$ is an eigval of $T_{\!\Cbb}.$}
	(b) \hypertarget{5A16}{}\Sbra{{\normalsize\OR \Onumber{16} \;\OR [9.16]\,}} \TextB{$\lambda\in\Cbb.$ Provt $\lambda$ is an eigval of $T_{\!\Cbb}$ $\Longleftrightarrow$ $\overline \lambda$ is an eigval of $T_{\!\Cbb}.$}
}\par\quad
(a) Sup $\lambda$ is an eigval of $T$ with an eigvec $v.$\par\quad\Ha
...
\ProblemN{\hypertarget{5A21}{21}}{
	...
	(a) \TextNL{Sup $\lambda\in\Fbb$ with $\lambda\neq 0$. Provt $\lambda$ is an eigval of $T$ $\Longleftrightarrow\lambda^{-1}$ is an eigval of $T^{-1}$.}
	(b) \TextNL{Provt $T$ and $T^{-1}$ have the same eigvecs.}
...
\ProblemN{\hypertarget{5A22}{22}}{
	\TextNL{Sup $T\in\Lm{V}$ and $\exists$ nonzero vecs $u,w$ in $V$ suth $Tu = 3w,\;Tw = 3u$.}
	\TextNL{Provt $3$ or $-3$ is an eigval of $T$.}
...
\ProblemN{\hypertarget{5A23}{23}}{
	\TextNL{Sup $S, T\in\Lm{V}$. Provt $ST$ and $TS$\, have the same eigvals.\vspace{2pt}}
...
\ProblemBnoor{\hypertarget{5A2e20}{2E 20}}{
	...
	\TextB{\Par{ but might not with the same eigvals }. Provt $ST=TS.$}
...
\ProblemBnoor{\hypertarget{5A4e37}{4E 5.A.37}}{
	\TextB{Sup $V$ is finid and $T\in\Lm{V}$.}
	\TextB{Define $\mathcal{A}\in\Lm[\BigPar]{\Lm{V}}$ by $\mathcal{A}\Par{S} = TS$ for each $S\in\Lm{V}$.}
	\TextB{Provt the set of eigvals of \,$T$ equals the set of eigvals of $\mathcal{A}$.}
}\par\quad
...
\ProblemN{\hypertarget{5A25}{25}}{
	\TextNL{Sup $T\in\Lm{V}$ and $u, w$ are eigvecs of $T$ suth $u + w$ is also an eigvec of $T$.}
	\TextNL{Provt $u$ and $w$ corres to the same eigval.}
...
Othws, $\Par{u,w}$ is linely inde. Then $\lambda_0-\lambda_1=\lambda_2-\lambda_0=0\,\Rightarrow\lambda_1=\lambda_2=\lambda_0.$\PfEnd\parSol{}
...
\ProblemN{\hypertarget{5A26}{26}}{
	\TextNL{Sup $T\in\Lm{V}$ is suth every nonzero vec in $V$ is an eigvec of \,$T$.}
	\TextNL{Provt $T$ is a scalar multi of the id optor.}
...
\ProblemN{\hypertarget{5A27}{27}, \hypertarget{5A28}{28}}{
	\TextNL{Sup $V$ is finid and $k\in\Bra{1, \dots , \dim V - 1}$.}\qquad
	\TextNL{Sup $T\in\Lm{V}$ is suth every subsp of $V$ of dim $k$ is invard $T$.}\qquad
	\TextNL{Provt $T$ is a scalar multi of the id optor.}
...
\ProblemN{\hypertarget{5A29}{29}}{
	\TextNL{Sup $T\in\Lm{V}$ and $\range T$ is finid.}
	\TextNL{Provt $T$ has at most $1 + \dim\range T$ disti eigvals.}
}\par\quad
Let $\lambda_1,\dots,\lambda_m$ be the disti eigvals of $T$ with corres eigvecs $v_1,\dots,v_m.$\par\quad
\BigPar{ Becs $\range T$ is finid. The corres eigvals are finite. }\par\quad
Then $\Par{v_1,\dots,v_m}$ linely inde $\Longrightarrow\Par{\lambda_1 v_1,\dots,\lambda_m v_m}$ linely inde, if each $\lambda_k\neq 0.$\par\quad
Othws, $\exists\,!\,\lambda_k=0.$ Now $\Par{\lambda_1v_1,\dots,\lambda_{k-1}v_{k-1},\lambda_{k+1}v_{k+1},\dots,\lambda_mv_m}$ is linely inde.\par\quad
Hence, by [2.23], $m-1\leqslant\dim\range T.$\PfEnd
\SepLine

\ProblemN{\hypertarget{5A30}{30}}{
	\TextNL{Sup $T\in\Lm{\Rbb^3}$ and $-4,5,\sqrt{7}$ are eigvals. Provt $\exists\,x,Tx - 9x = \Par{-4, 5, \sqrt{7}}$.}
...
\ProblemN{\hypertarget{5A31}{31}}{
	\TextNL{Sup $V$ is finid, and $v_1,\dots,v_m \in V$. Provt}
	\TextNL{$\Par{v_1,\dots,v_m}$ is linely inde $\Longleftrightarrow v_1,\dots,v_m$ are eigvecs of some $T$ corres to disti eigvals.}
...
\ProblemB{
	...
	(a)\ProblemN[]{\hypertarget{5A32}{32}}{
		\TextB{Provt $\Par{e^{\lambda_1x},\dots,e^{\lambda_nx}}$ is linely inde in $\Rbb^{\Rbb}$.}
	...
	(b)\ProblemNnoor[]{\!\!}{4E 36}{
		\hypertarget{5A4e36}{}\TextB{Shat $\BigPar{\cos\!\Par{\lambda_1 x},\dots,\cos\!\Par{\lambda_n x}}$ is linely inde in $\Rbb^{\Rbb}.$\vspace{-4pt}}
	}
	\TextB{}
}\par\quad
...
\ProblemN{\hypertarget{5A33}{33}}{
	\TextNL{Sup $T\in\Lm{V}.$ Provt $T\XSlash \Par{\range T}=0.$\vspace{2pt}}
...
\ProblemB{
	\hypertarget{5A4e38}{}\TextB{Sup $V$ is finid, $T\in\Lm{V}$, and $U$ is invarsp of $V$ under $T$.}
	\TextB{Define $T\XSlash U:V\XSlash U\rightarrow V\XSlash U$ by $\Par{T\XSlash U}\Par{v + U} = Tv + U$ for each $v\in V$.}
	(a) \TextB{Shat $T\XSlash U$ is well-defined and is linear. {\large Requires that $U$ is invard $T.$}}
	(b) \Sbra{{\normalsize\OR \Onumber{\hypertarget{5A35}{35}}\,}} \TextB{Shat each eigval of $T\XSlash U$ is an eigval of $T$.}
}\par\quad
(a) $v+U=w+U\Longleftrightarrow v-w\in U\Longrightarrow T\Par{v-w}\in U\Longleftrightarrow Tv+U=Tw+U.$\par\quad\Ha
Hence $T\XSlash U$ is well-defined. Now we shat $T\XSlash U$ is linear.\par\quad\Ha
...
If $u=0\Rightarrow Tv=\lambda v,$ then we are done. Othws, we discuss in two cases.\par\quad\Hb
...
If $\Par{T-\lambda I}\mmid_U$ is not inv. Then becs $V$ is finid, $\Par{T-\lambda I}\mmid_U$ is not inje,\par\quad\Hb
...
\ProblemN{\hypertarget{5A36}{36}}{
	\TextNL{Prove or give a counterexa\hspace{1pt}$:$ The result in Exercise 35 is still true if $V$ is infinid.}
}A counterexa:\par\quad
Consider $V=\Bra{\,f\in\Rbb^{\Rbb}:\exists\,!\,m\in\Nbb,f\in\Span{1,e^x,\dots,e^{mx}}}.$ Note that $V$ is infinid.\par\quad
...
\ProblemBnoor{\hypertarget{5A4e39}{4E 5.A.39}}{
	\TextB{Sup $V$ is finid and $T\in\Lm{V}$.}
	\TextB{Provt $T$ has an eigval $\Longleftrightarrow\exists\,$ invarsp $U$ under $T$ of dimension $\dim V - 1$.}
}\par\quad
...
(b) Sup $U$ is invarsp under $T$ of dim $m=\dim V-1.$ \Par{ If $m=0,$ then we are done. }\par\quad\Hb
...
\ProblemBnoor{\hypertarget{5A4e33}{4E 5.A.33}}{
	...
	(a) \TextB{Provt $T$ is inje $\Longleftrightarrow$ $T^m$ is inje.}
	(b) \TextB{Provt $T$ is surj $\Longleftrightarrow$ $T^m$ is surj.}
}\par\quad
...
(b) Sup $u\in\range p\Par{T}$. Then $\exists\,v\in V$ suth $u = p\Par{T}v$.\par\quad\Hb
Thus $Tu = T\BigPar{p\Par{T}v} = p\Par{T}\Par{Tv}\in\range p\Par{T}.$\PfEnd
\SepLine

\BulletPointX\NoteFor{[5.21]} \TextB{\large Every optor on a finid nonzero complex vecsp has an eigval.}
Sup $V$ is a finid complex vecsp of dim $n > 0$ and $T\in\Lm{V}$.\TextB{}
...
\ProblemN{\hypertarget{5BI13}{13}}{
	...
	\TextNL{Provt every subsp of $V$ invard $T$ is either $\zeroSubs$ or infinid.}
}Sup $U$ is a finid nonzero invarsp on $\Cbb.$ Then by [5.21], $T\mmid_U$ has an eigval.\PfEnd
\SepLine
...
\TextB{Provt $\exists\,!\,p_k\in M_T^0,\deg p_k=\min\!${\large\envFontDefault$\Bra{\deg p_j}{_{j\in\Gamma}}$}$\leqslant\dim V$.}
...
{\tgsl Asm $\dim V > 0$ and that the desired result is true for all optors on all vecsps of smaller dim.}\par\quad\Hii
...
Becs $\Par{u, Tu, \dots , T^{m-1} u}$ is linely inde.\par\quad\Hii
...
\ProblemBnoor{4E 5.31, \hypertarget{5BI4e25}{4E 5.B.25} and \hypertarget{5BI4e26}{26}}{
	...
	(a) \TextB{Provt $\,p\,$ is a poly multi of the min poly of $T\mmid_U$.}
	(b) \TextB{Provt $\,p\,$ is a poly multi of the min poly of $T\XSlash U$.}
	(c) \TextB{Provt
 ( min poly of $T\mmid_U$ ) $\times$ ( min poly of $T\XSlash U$ ) is a poly multi of $\,p\,$.}
	(d) \TextB{Provt the set of eigvals of $T$ equals}
...
\ProblemBnoor{\hypertarget{5BI4e27}{4E 5.B.27}}{
	\TextB{Sup $\Fbb=\Rbb,$ $V$ is finid, and $T\in \Lm{V}$.}
	\TextB{Provt the min poly $\,p\,$ of $T_{\!\Cbb}$ equals the min poly $\,q\,$ of $T$.}
}\par\quad
...
\ProblemBnoor{\hypertarget{5BI4e28}{4E 5.B.28}}{
	\TextB{Sup $V$ is finid and $T\in \Lm{V}$.}
	\TextB{Provt the min poly $\,p\,$ of $T\apostrophe \in\Lm{V\apostrophe}$ equals the min poly $\,q\,$ of $T$.}
}\par\quad
...
\ProblemBnoor{4E 5.32}{
	...
	\TextB{Provt $T$ is not inje $\Longleftrightarrow$ the const term of $\,p\,$ is $0.$}
}\par\quad
...
\ProblemBnoor{\hypertarget{5BI4e22}{4E 5.B.22}}{
	\TextB{}
	\TextB{Sup $V$ is finid, $T\in \Lm{V}$. Provt $T$ is inv $\Longleftrightarrow I\in\Span{T, T^2, \dots , T^{\dim V}}.$}
}Denote the min poly by $p,$ where for all $z\in\Fbb,p\Par{z}=a_0 +a_1 z+\dots+z^m.$\par\quad
Notice that $V$ is finid. $T$ is inv $\Longleftrightarrow T$ is inje $\Longleftrightarrow$ $p\Par{0}\neq 0$.\par\quad
...
\ProblemN{\hypertarget{5BI6}{6}}{
	...
	\TextN{Provt $U$ is invard $p\Par{T}$ for every poly $p\in\PoFi$.}
}\par\quad
...
\ProblemBnoor{\hypertarget{5BI4e10}{4E 5.B.10}, \hypertarget{5BI4e23}{23}}{
	\TextB{Sup $V$ is finid, $T\in \Lm{V}$ and $\,p\,$ is the min poly with deg $\,m.$ Sup $v\in V.$}
	(a) \TextB{Provt $\,\,\Span{v,Tv,\dots,T^{m-1}v}=\Span{v,Tv,\dots,T^{j-1} v}$ for some $j\leqslant m.$}
	(b) \TextB{Provt $\,\,\Span{v,Tv,\dots,T^{m-1}v}=\Span{v, Tv, \dots , T^{m-1} v,\dots,T^n v}.$}
}\par\quad
...
(a) Sup $j\in\Nbp$ is the smallest suth $T^j v\in\Span{v,Tv,\dots,T^{j-1}v}=U_0.$ Then $j\leqslant m.$\par\quad\Ha
...
\ProblemBnoor{\hypertarget{5BI4e21}{4E 5.B.21}}{
	\TextB{Sup $V$ is finid and $T\in \Lm{V}$.}
	\TextB{Provt the min poly $\,p\,$ has deg at most $1 + \dim \range T$.}
...
\ProblemN{\hypertarget{5BI19}{19}}{
	\TextNL{Sup $V$ is finid, $\dim V>1,T\in\Lm{V}.$ Provt $\Bra{\,p\Par{T}:p\in\PoFi}\neq\Lm{V}.$\vspace{2pt}}
...
\ProblemB{
	\hypertarget{5BI4e19}{}\TextB{Sup $V$ is finid and $T\in \Lm{V}$. Let $\mathcal{E} =\Bra{q\Par{T}: q \in \PoFi}$.}
	\TextB{Provt $\dim \mathcal{E}$ equals the deg of the min poly of $T$.}
}\par\quad
Becs the list $\Par{I,T,\dots,T^{\Par{\dim V}^2}}$ of len $\dim\Lm{V}+1$ is linely depe in $\dim\Lm{V}.$\par\quad
Sup $m\in\Nbp$ is the smallest suth $T^m=a_0 I+\dots+a_{m-1}T^{m-1}.$\par\quad
...
\ProblemBnoor{\hypertarget{5BI4e13}{4E 5.B.13}}{
	...
	\TextB{Provt $\exists\,!\,r \in \PoFi$ suth $q\Par{T} = r\Par{T},\deg r<\deg p.$}
}\par\quad
...
Sup $f\in\PoFi$ suth $f\Par{z}=b_0 +b_1 z+\dots+b_n z^n+b_{n+1}z^{n+1}.$\par\quad\Hii
...
\TextB{Sup $V$ is a finid complex vecsp and $T\in\Lm{V}.$}
...
\ProblemN{\hypertarget{5BI10}{10}}{
	...
	\TextNL{Provt for any $p\in\PoFi,p\Par{T}v=p\Par{\lambda}v.$}
}\par\quad
Sup $p$ is defined by $p\Par{z}=a_0+a_1 z+\dots+a_m z^m$ for all $z\in\Fbb.$ Becs for any $n\in\Nbp,T^n v=\lambda^n v.$\par\quad
...
\ProblemN{\hypertarget{5BI1}{1}}{
	\TextN{Sup $T\in\Lm{V}$ and $\exists\,n\in\Nbp$ suth $T^n=0.$}
	\TextN{Provt $\Par{I-T}$ is inv and $\Par{I-T}{^{-1}}=I+T+\dots+T^{n-1}.$}
...
\ProblemN{\hypertarget{5BI2}{2}}{
	...
	\TextN{Sup $\lambda$ is an eigval of $T$. Provt $\lambda = 2$ or $\lambda = 3$ or $\lambda = 4$.}
}\par\quad
Sup $v$ is an eigvec corres to $\lambda.$ Then for any $p\in\PoFi,p\Par{T}v=p\Par{\lambda}v.$\par\quad
...
\ProblemNnoor{\hypertarget{5BI7}{7}}{See \hLk{5A22}{5.A.22}}{
	\TextN{Sup $T\in\Lm{V}$. Provt $9$ is an eigval of $T^2$ $\Longleftrightarrow$ $3$ or $-3$ is an eigval of $T$.}
}\par\quad
(a) Sup $\lambda$ is an eigval of $T$ with an eigvec $v.$\par\quad\Ha
...
\ProblemN{\hypertarget{5BI3}{3}}{
	\TextN{Sup $T\in\Lm{V},T^2=I$ and $-1$ is not an eigval of $T.$ Provt $T=I.$}
}\par\quad
...
\ProblemBnoor{\hypertarget{5A4e32}{4E 5.A.32}}{
	\TextB{Sup $T\in\Lm{V}$ has no eigvals and $T^4 = I$. Provt $T^2=-I$.}
}\par\quad
...
Othws $\Par{v,Tv}$ is linely inde. Then $\Span{v,Tv}=\Rbb^2.$ Note that $\nexists\,b\in\Fbb,T-bI=0.$\par\quad
...
\ProblemBnoor{\hypertarget{5BI4e11}{4E 5.B.11}}{
	...
	(a) \TextB{Shat $T^2 - \Par{a + d}T + \Par{ad - bc}I = 0.$}
	(b) \TextB{Shat the min poly of $T$ equals}
	\TextB{\large\envFontDefault\centerline{$\MathLeftBrace{l}{z-a\qquad\qquad\qquad\qquad\qquad$ if $b=c=0$ and $a=d$,$\\ z^2-\Par{a+d}z+\Par{ad-bc}\quad$ \,\,othws.$}$}}
}\par\quad
(a) Sup the basis is $\Par{v,w}$. Becs $\MathLeftBrace{l}{Tv=av+bw\Rightarrow\Par{T-aI}v=bw,$ then apply $\Par{T-dI}$ to both sides. $\\ Tw=cv+dw\Rightarrow \Par{T-dI}w=cv,$ then apply $\Par{T-aI}$ to both sides. $}$\par\vspace{6pt}\quad\Ha
...
\ProblemB{
	\TextB{Sup $S,T\in\Lm{V},S$ is inv, and $p\in\PoFi$. Provt $Sp\Par{TS}=p\Par{ST}S.$}
}\par\quad
...
\ProblemBnoor{\hypertarget{5BI4e7}{4E 5.B.7}}{
	...
	(a) \TextB{If $V=\Fbb^2.$ Give an exa suth $p\neq q;$ \;{\large\tgnr(b)} If $S$ or $T$ is inv. Provt $p=q.$}
}\par\quad
...
(b) Sup $S$ is inv. Becs $p,q$ are monic.\par\quad\Hb
$\MathRightBrace{l}{$
...
\ProblemN{\hypertarget{5BI11}{11}}{
	...
	\TextNL{Provt $\alpha$ is an eigval of $p\Par{T}$ $\Longleftrightarrow$ $\alpha = p\Par{\lambda}$ for some eigval $\lambda$ of $T$.}
}\par\quad
...
\ProblemBnoor{\hypertarget{5BI4e17}{4E 5.B.17}}{
	\TextB{Sup $V$ is finid, $T\in \Lm{V},\lambda\in\Fbb$, and $\,p\,$ is the min poly of $T$.}
	\TextB{Shat the min poly of $\Par{T - \lambda I}$ is the poly $q$ defined by $q\Par{z} = p\Par{z + \lambda}.$}
}\par\quad
...
\ProblemBnoor{\hypertarget{5BI4e18}{4E 5.B.18}}{
	\TextB{Sup $V$ is finid, $T\in \Lm{V},\lambda\in\nonzeroFbb$, and $\,p\,$ is the min poly of $T$.}
	\TextB{Shat the min poly of $\lambda T$ is the poly q defined by $q\Par{z} = \lambda^{\deg p} p\Par{\frac{z}{\lambda}}$.}
}\par\quad
...
\ProblemNor{\hypertarget{5BI18}{18}}{\hypertarget{5BI4e15}{4E 5.B.15}}{
	\TextNL{Sup $V$ is a finid complex vecsp with $\dim V > 0$ and $T\in\Lm{V}$.}
	\TextNL{Define $f:\Cbb\rightarrow\Rbb$ by $f\Par{\lambda} = \dim \range\Par{T-\lambda I}$.}
	\TextNL{Provt $f$ is not a continuous function.}
}Note that $V$ is finid.\par\quad
Let $\lambda_0$ be an eigval of $T.$ Then $\Par{T-\lambda_0 I}$ is not surj. Hence $\dim\range\Par{T-\lambda_0 I}<\dim V.$\par\quad
Becs $T$ has finily many eigvals. There exist a sequence of number $\Bra{\lambda_n}$ suth $\lim\limits_{n\rightarrow\infty}\lambda_n=\lambda_0$.\par\quad
...
\ProblemBnoor{\hypertarget{5BI4e9}{4E 5.B.9}}{
	\TextB{Sup $T\in\Lm{V}$ is suth with resp to some basis of $V$,}
...
Let $\Par{v_1,\dots,v_n}$ denote the basis suth $\Mt[\BigPar]{T,\Par{v_1,\dots,v_n}}_{j,k}=A_{j,k}\in\Qbb$ for all $j,k=1,\dots,n$.\par\quad
Denote $\Mt[\BigPar]{v_j,\Par{v_1,\dots,v_n}}$ by $x_j$ for each $v_j.$\par\quad
Sup $p$ is the min poly of $T$ and $p\Par{z}=z^m+\dots+c_1 z+c_0.$ Now we shat each $c_j\in\Qbb.$\par\quad
...
\ProblemBnoor{\OR (\hypertarget{5BI4e16}{4E 5.B.16}), \OR (8.C.18)}[\Sbra]{
	\TextB{Sup $a_0 ,\dots, a_{n-1}\in\Fbb.$ Let $T$ be the optor on $\Fbb^n$ suth\vspace{2pt}}
	\TextB{$\Mt{T}=\,${\normalsize$\begin{pmatrix}
	0 &   &        &  &   & -a_0     \\
	1 & 0 &        &  &   & -a_1     \\
	  & 1 & \ddots &  &   & \vdots   \\
...
\end{pmatrix} $}, with resp to the std basis $\Par{e_1,\dots,e_n}$.\vspace{4pt}}
	\TextB{Shat the min poly of $T$ is $\,p\,$ defined by $p\Par{z}=a_0 + a_1 z + \dots + a_{n-1} z^{n-1} + z^n$.}
	...
	\vspace{-2pt}\TextB{\small each poly  $[$ by 8.36(b) $]$. Thus there is no such formula or algo. However, efficient numeric methods exist for obtaining very good}
...
\ProblemB{
	...
	\TextB{Sup $\Fbb=\Rbb,$ $V$ is finid, $T\in\Lm{V}$ and $b, c\in\Rbb$ with $b^2 < 4c$.}
...
Asm $U\neq R.$ Then $\exists\,w\in R$ but $w\not\in U.$ Let $W$ be suth $\Par{w,T\mmid_R w}$ is a basis of $W.$\par\quad
...
\ProblemB{
	...
	(b) \TextB{Sup $\Fbb=\Rbb,$ $V$ is finid, and $\dim V=n$ is an odd number.}
	\Hb\TextB{Let $T\in\Lm{V}$ and the min poly is $\,p\,$. Provt $T$ has an eigval.}
}\par\quad
(i) If $n=1,$ then we are done.\par\quad\Endi
(ii) Sup $n\geqslant 3.$ Asm every optor, on odd-dim vecsps of dim less than $n,$ has an eigval.\par\quad\Hii
...
Now sup $b, c\in\Rbb$ suth $b^2 < 4c$ and $p$ is a poly multi of $x^2 + bx + c$ (see [4.17]).\par\quad\Hii
...
\ProblemBnoor{\hypertarget{5BI2e24}{2E Ch5.24}}{
	\TextB{Sup $\Fbb=\Rbb,T\in\Lm{V}$ has no eigvals.} \TextB{Provt every invarsp of $V$ under $T$ is even-dim.}
}\par\quad
...
\ProblemBnoor{\hypertarget{5BI4e29}{4E 5.B.29}}{
	\TextB{Shat every optor on a finid vecsp of dim $\geq 2$ has a $2$-dim invarsp.}
}\par\quad
...
\ProblemBnoor{\hypertarget{5BII4e2}{4E 5.C.2}}{
	...
	(a) \TextB{Shat $A + B$ is an upper-trig matrix with $\alpha_1 + \beta_1 , \dots , \alpha_n + \beta_n$ on the diag.}
	(b) \TextB{Shat $AB$ is an upper-trig matrix with $\alpha_1 \beta_1 , \dots , \alpha_n \beta_n$ on the diag.}
}\par
\SepLine

\ProblemBnoor{\hypertarget{5BII4e3}{4E 5.C.3}}{
	\TextB{}
	\TextB{Sup $T\in \Lm{V}$ is inv and $B=\Par{v_1 , \dots , v_n}$ is a basis of $V$ suth}
	\TextB{$\Mt{T,B}=A$ is upper trig, with $\lambda_1 , \dots , \lambda_n$ on the diag.}
	\TextB{Shat the matrix of $\Mt{T^{-1},B}=A^{-1}$ is also upper trig, with
{\normalsize$\Frac{1}{\lambda_1},\dots,\Frac{1}{\lambda_n}$} on the diag.}
}\par
\SepLine

\ProblemNnoor{\hypertarget{5BII9}{9}}{\hypertarget{5BII4e7}{4E 5.C.7}}{
	\TextN{Sup $V$ is finid, $T\in \Lm{V}$, and $v \in V$.}
	(a) \TextN{Provt $\exists\,!$ monic poly $p_v$ of smallest deg suth $p_v \Par{T}v = 0$.}
	(b) \TextN{Provt the min poly of $T$ is a poly multi of $p_v$.}
}
\SepLine


\ProblemNor{\hypertarget{5BII14}{14}}{\hypertarget{5BII4e4}{4E 5.C.4}}{
	\TextNL{ Give an optor $T$ suth with resp to some basis,}
...
\ProblemNor{\hypertarget{5BII20}{20}}{\normalsize \OR \hypertarget{5BII4e6}{4E 5.C.6}}{
	\TextNL{}
	\TextNL{Sup $\Fbb=\Cbb,$ $V$ is finid, and $T\in\Lm{V}$.}
	\TextNL{Provt if $k\in\Bra{1,\dots,\dim V},$ then $V$ has a $k$ dim subsp invard $T$.}
}\par
\SepLine

\ProblemBnoor{\hypertarget{5BII4e8}{4E 5.C.8}}{
	\TextB{Sup $V$ is finid, $T\in \Lm{V}$, and $\exists\,v \in V\nonzero$ suth $T^2 v + 2Tv = -2v$.}
	(a) \TextB{Provt if $\Fbb=\Rbb,$ then $\nexists$ a basis of $V$ with resp to which $T$ has an upper-trig matrix.}
	(b) \TextB{Provt if $\Fbb=\Cbb$ and $A$ is an upper-trig matrix that equals the matrix of $T$}
...
\ProblemBnoor{\hypertarget{5BII4e9}{4E 5.C.9}}{
	...
	\TextB{Provt $\exists$ inv $A\in\Fbb^{n,n}$ with complex ent suth $A^{-1} BA$ is an upper-trig matrix.}
}\par
...
\ProblemBnoor{\hypertarget{5BII4e10}{4E 5.C.10}}{
	...
	\TextB{Shat the following are equi.}
...
\ProblemBnoor{\hypertarget{5BII4e11}{4E 5.C.11}}{
	\TextB{Sup $\Fbb=\Cbb$ and $V$ is finid.}
	\TextB{Provt if $T\in \Lm{V}$, then $T$ has a lower-trig matrix with resp to some basis.}
}\par
\SepLine

\ProblemBnoor{\hypertarget{5BII4e12}{4E 5.C.12}}{
	\TextB{}
	\TextB{Sup $V$ is finid, $T\in \Lm{V}$ has an upper-trig matrix with resp to some basis,}
	\TextB{and $U$ is a subsp of $V$ that is invard $T$.}
	(a) \TextB{Provt $T\mmid_U$ has an upper-trig matrix with resp to some basis of $U$.}
	(b) \TextB{Provt $T\XSlash U$ has an upper-trig matrix with resp to some basis of $V\XSlash U$.}
}\par
...
\ProblemBnoor{\hypertarget{5BII4e13}{4E 5.C.13}}{
	\TextB{Sup $V$ is finid, $T\in \Lm{V}$. Sup $U$ is invarsp of $V$ under $T$}
	\TextB{suth $T\mmid_U,T\XSlash U$ have upper-trig matrix.}
	\TextB{Provt $T$ has upper-trig matrix.}
}\par
...
\ProblemBnoor{\hypertarget{5BII4e14}{4E 5.C.14}}{
	\TextB{Sup $V$ is finid and $T\in \Lm{V}$.}
	\TextB{Provt $T$ has upper-trig matrix $\Longleftrightarrow$ $T\apostrophe$ has upper-trig matrix.}
}\par
...
\ProblemN{2}{
	...
	\TextN{Provt $\exists$ a basis of $V$ with resp to which}
...
\ProblemN{3}{
	\TextN{Sup $S, T\in\Lm{V}$ are suth $ST = TS$. Sup $p\in\PoFi$.}
	(a) \TextN{Provt $\null p\Par{S}$ is invard $T$. \;\; {\tgnr\large(b)} Provt $\range p\Par{S}$ is invard $T$.}
...
\ProblemN{5}{
	\TextN{Provt a pair of optors on a finid vecsp commu $\Longleftrightarrow$ their dual optors commu.}
}\par
\SepLine

\ProblemN{6}{
	\TextN{Sup $V$ is a finid complex vecsp and $S, T\in\Lm{V}$ commu.}
	\TextN{Provt $\exists\,\alpha,\lambda\in\Cbb$ suth $\range\Par{S -\alpha I} + \range\Par{T -\lambda I}\neq V$.}
}\par
\SepLine

\ProblemN{7}{
	\TextN{Sup $V$ is a complex vecsp, $S\in\Lm{V}$ is diag, and $T$ commu with $S$.}
	\TextN{Provt $\exists$ basis $B$ of $V$ suth $S$ has a diag matrix with resp to $B$}
	\TextN{and T has upper-trig matrix with resp to $B$.}
}\par
\SepLine

\ProblemN{8}{
	\TextN{Sup $m = 3$ in [5.72] and $D_x , D_y$ are the commu partial diff optors on $\Po_3\Par{\Rbb^2}$ from [5.72].}
	\TextN{Find a basis of $\Po_3\Par{\Rbb^2}$ with resp to which $D_x$ and $D_y$ each have upper-trig matrix.}
}\par
\SepLine

\ProblemN{9}{
	\TextN{Sup $V$ is a finid nonzero complex vecsp.}
	\TextN{Sup that $\mathcal{E}\subseteq\Lm{V}$ is suth $S$ and $T$ commu for all $S, T\in\mathcal{E}$.}
	(a) \TextN{Provt $\exists$ eigvec $v\in V$ for every ele of $\mathcal{E}$.}
	(b) \TextN{Provt $\exists$ a basis of $V$ with resp to which every ele of $\mathcal{E}$ has upper-trig matrix.}
}\par
\SepLine

\ProblemN{10}{
	\TextNL{Give commu optors $S, T$ on a finid real vecsp suth}
