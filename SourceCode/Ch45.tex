...
\ProblemBnoor[]{\Tips \,\,\,}[]{
	\TextB{\tgsl\large\envFontDefault Sup $p\in\PoFi,\deg p\leqslant m$ and $p$ has at least $\Par{m+1}$ disti zeros.}
	\Blind{\Tips \,\,\,} \TextB{\tgsl\large\envFontDefault Then by the ctrapos of [4.12], 又 $\deg p=m,$ we conclude that $m<0.$ Hence $p=0.$}
}\TextB{}
\Or We showt if $p$ has at least $m$ disti zeros, then either $p=0$ or $\deg p\geqslant m.$\TextB{}
If $p=0$ then we are done. If not, then sup $p$ has exactly $n$ disti zeros $\lambda_1,\dots,\lambda_n.$\TextB{}
Beca $\exists\,!\,\alpha_i\geqslant 1,q\in\PoFi,$ and $q\neq 0,$ shat $p\Par{z}=\Sbra{\Par{z-\lambda_1}{^{\alpha_1}}\cdots\Par{z-\lambda_n}{^{\alpha_n}}}q\Par{z}.$\PfEnd\vspace{2pt}
...
would give a poly with some nonzero coeffs but infinily many zeros. By {\TIPS}.\PfEnd\vspace{-3pt}
\SepLine

\BulletPointX\NoteFor{[4.8]} {\tgsl division algorithm for polys} \hfill[{\tgsc Another proof}]\TextB{\vspace{4pt}}
Sup $\deg p\geqslant \deg s$. Then $\XPar{\underbrace{1, z,\dots, z^{\deg s-1}}_{\text{of\,len}\,\deg s},\underbrace{s,zs,\cdots,z^{\deg p-\deg s}s}_{\text{of\,len}\,\Par{\deg p-\deg s+1}}}$ is a basis of $\PoF{\deg p}.$\TextB{}
Beca $q\in\PoFi,\exists\,!\,a_i,b_j\in\Fbb,$\TextB{}
...
\BulletPointX\NoteFor{[4.11]}\;\;{\tgsl each zero of a poly corresponds to a deg-one factor;}\hfill[{\tgsc Another proof}]\TextB{\vspace{2pt}}
First sup $p\Par{\lambda}=0.$ Write $p\Par{z}=a_0+a_1 z+\dots+a_m z^m,\exists\,!\,a_0,a_1,\dots,a_m\in\Fbb$ for all $z\in\Fbb.$\vspace{2pt}\TextB{}
...
Sup a nonconst $p\in\PoCi$ with highest-order nonzero term $c_m z_m.$\vspace{3pt}\TextB{}
Then $\aMid{p\Par{z}}\rightarrow\infty$ as $\aMid{z}\rightarrow\infty$ \XPar{ beca $\Frac{\aMid{p\Par{z}}}{\aMid{z_m}}\rightarrow\aMid{c_m}$ as $\aMid{z}\rightarrow\infty$ }.\vspace{3pt}\TextB{}
\vspace{3pt}Thus the continuous function $z\rightarrow\aMid{p\Par{z}}$ has a global min at some point $\zeta\in\Cbb.$\TextB{}
\vspace{3pt}To showt $p\Par{\zeta} = 0$, asm $p\Par{\zeta}\neq 0$. Define $q\in\PoCi$ by $q\Par{z}=\Frac{p\Par{z+\zeta}}{p\Par{\zeta}}.$\TextB{}
\vspace{3pt}The function $z\rightarrow\aMid{q\Par{z}}$ has a global min value of $1$ at $z = 0$.\TextB{}
\vspace{3pt}Write $q\Par{z} = 1 + a_k z^k + \dots + a_m z^m$, where $k\in\Nbp$ is the smallest shat $a_k\neq 0$.\TextB{}
\vspace{3pt}Let $\beta\in\Cbb$ be shat $\beta^k=-\Frac{1}{a_k}$.\TextB{}
...
\ProblemBnoor{\hypertarget{4O4e2}{4E 4.2}}{
	\TextB{Provet if $w,z\in\Cbb,$ then $\aXMid{\aMid{w}-\aMid{z}}\leqslant\aMid{w-z}.$}
}\par\quad
\vspace{-6pt}\AlignEq{}{
...
\vspace{-130pt}\hspace{200pt}$\hMath{l}{\left|}{\right.}{$\;\Or \!\!$\MathRightBrace{l}{
	...
	\;\\\;\text{\tgsl\normalsize Geometric interpretation: The len of each side of a triangle}\\
	\;\text{\tgsl\normalsize is greater than or equal to the difference of the lens of the two other sides.}\\\;}$\PfEnd[7pt]\vspace{-2pt}
\SepLine

\ProblemBnoor{\hypertarget{4O4e3}{4E 4.3}}{
	\TextB{Sup $\Fbb=\Cbb,\varphi\in V\apostrophe.$ Define $σ:V\rightarrow\Rbb$ by $\sigma\Par{v} = \Real\varphi\Par{v}$ for each $v\in V.$\vspace{4pt}}
	\TextB{Showt $\varphi\Par{v}=\sigma\Par{v}-\i\sigma\Par{\i v}$ for all $v\in V.$\vspace{2pt}}
}Notice that \,$\varphi\Par{v}=\Real\varphi\Par{v}+\i\,\mathfrak{Im}\,\varphi\Par{v}=\sigma\Par{v}+\i\,\mathfrak{Im}\,\varphi\Par{v}.$\parSol{}
又 $\Real\varphi\Par{\i\, v}=\REAL\BigPar{\i\,\varphi\Par{v}}=-\mathfrak{Im}\,\varphi\Par{v}=\sigma\Par{\i\,v}.$ Hence $\varphi\Par{v}=\sigma\Par{v}-\i\,\sigma\Par{\i\,v}.$\PfEnd
\SepLine

\ProblemN{\hypertarget{4O4}{4}}{
	\TextN{Sup $m,n\in\Nbp$ with $m\leqslant n,\;\lambda_1,\dots,\lambda_m\in\Fbb.$}
	\TextN{Provet $\exists\,p\in\PoFi,\deg p = n,$ the zeros of $\,p\,$ are $\lambda_1,\dots,\lambda_m.$\vspace{3pt}}
...
\ProblemN{\hypertarget{4O5}{5}}{
	\TextN{Sup $m\in\Nbb,$ and $z_1,\dots,z_{m+1}$ are disti in $\Fbb,$ and $w_1,\dots,w_{m+1}\in\Fbb.$}
	\TextN{Provet $\exists\,!\,p\in\PoF{m},p\Par{z_k}=w_k$ for each $k\in\Bra{1,\dots, m+1}.$}
}\par\quad
...
We now showt $T$ is surj, so that such $p$ exists;\; and that $T$ is inje, so that such $p$ is unique.\vspace{1pt}\par\quad
...
And that $\prod_{i=1}^{k-1}\Par{z_j-z_i}=0\Longleftrightarrow j\leqslant k-1,$ beca $z_1,\dots,z_{m+1}$ are disti.\vspace{4pt}\par\quad
...
\ProblemN{\hypertarget{4O2}{2}}{
	\TextN{Sup $m\in\Nbp$. Is the set $U=\zeroSubs\cup\Bra{p\in\PoFi:\deg p = m}$ a subsp of $\PoFi$?}
}$x^m,x^m+x^{m-1}\in U$ \,\,but\,\, $\Deg\Sbra{\Par{x^m+x^{m-1}}-\Par{x^m}}\neq m\Rightarrow \Par{x^m+x^{m-1}}-\Par{x^m}\not\in U.$\PfEnd
\SepLine

\ProblemN{\hypertarget{4O3}{3}}{
	\TextN{Sup $m\in\Nbp$. Is the set $U=\zeroSubs\cup\Bra{p\in\PoFi:2\;\mmid\,\deg p}$ a subsp of $\PoFi$?}
}$x^2,x^2+x\in U$ \,\,but\,\, $\Deg\Sbra{\Par{x^2+x}-\Par{x^2}}$ is odd and hence $\Par{x^2+x}-\Par{x^2}\not\in U.$\PfEnd
\SepLine

\ProblemN{\hypertarget{4O6}{6}}{
	\TextN{Sup nonzero $p\in\PoF{m}$ has deg $m.$ Provet}
	\TextN{[P] $p$ has $m$ disti zeros $\Longleftrightarrow p$ and its derivative $p\apostrophe$ have no zeros in common [Q].}
}\par\quad
(a) Sup $p$ has $m$ disti zeros. And $\deg p=m.$ By [4.14], $\exists\,!\,c,\lambda_i\in\Rbb,p\Par{z}=c\Par{z-\lambda_1}\cdots\Par{z-\lambda_m}.$\par\vspace{2pt}\quad\Ha
...
For each $j\in\Bra{1,\dots,m}$, let $q_j\in\PoF{m-1}$ be shat ${p\Par{z}}={\Par{z-\lambda_j}}q_j\Rightarrow q_j\Par{\lambda_j}\neq 0.$\par\vspace{2pt}\quad\Ha
...
Sup $p\Par{z}=\Par{z-\lambda}q\Par{z},\;p\apostrophe\Par{z}=\Par{z-\lambda}r\Par{z}.$ 又 $p\apostrophe\Par{z}=\Par{z-\lambda}q\apostrophe\Par{z}+q\Par{z}.$\vspace{2pt}\par\quad\Ha
...
Beca nonzero $p\in\PoF{m},$ we sup $\lambda_1,\dots,\lambda_M$ are all the disti zeros of $p,$ where $M<m.$\vspace{2pt}\par\quad\Hb
By Pigeon Hole Principle, $\exists\,\lambda_k$ shat $p\Par{z}=\Par{z-\lambda_k}{^{2}}q\Par{z}$ for some $q\in\PoFi.$\vspace{3pt}\par\quad\Hb
...
\ProblemN{\hypertarget{4O9}{9}}{
	\TextN{Sup $p\in\PoCi$. Define $q:\Cbb\rightarrow\Cbb\,$ by $q\Par{z} = p\Par{z}\overline{p\Par{\overline z}}$. Provet $q\in\PoRi$.}
}\par\quad
\NOTICE that by [4.5], $\overline z{^n}=\overline{z^n}.$\par\quad
Sup $q\Par{z}=a_n z^n+\dots+a_1 z+a_0\Rightarrow q\Par{\overline z}=a_n \overline z{^n}+\dots+a_1 \overline z+a_0\Rightarrow \overline{q\Par{\overline z}}=\overline{a_n}z^n+\dots+\overline{a_1}z+\overline{a_0}.$\vspace{8pt}\par\quad
...
\Or Sup $p\Par{z}=a_m z^m+\dots+a_1z+a_0.$ Now $\overline{p\Par{\overline z}}=\overline{a_m} z^m+\dots+\overline{a_1}z+\overline{a_0}.$\vspace{4pt}\par\quad
...
\ProblemN{\hypertarget{4O8}{8}}{
	\TextN{For $p\in\PoRi$, define $Tp:\Rbb\rightarrow\Rbb\,$ by $\Par{Tp}\Par{x}=\,${\large\envFontDefault$\MathLeftBrace{l}{
		\Frac{p\Par{x}-p\Par{3}}{x-3}\;$ if $x\neq 3,\\[4pt]
		p\apostrophe\Par{3}$\hfill if $x=3.
	}$}\vspace{6pt}}
	\TextN{Showt {\tgnr(a)} $Tp\in\PoRi$ for all $p\in\PoRi$ and that {\tgnr(b)} $T:\PoRi\rightarrow\PoRi$ is linear.}
}\vspace{2pt}\par\quad
...
(b) Now we showt $T$ is linear: $\forall p,q\in\PoRi,\lambda\in\Rbb,$\vspace{6pt}\par\quad\Hb
...
\ProblemN{\hypertarget{4O11}{11}}{
	\TextNL{Sup $p\in\PoFi$ with $p\neq 0$. Let $U=\Bra{pq:q\in\PoFi}$.}
	(a) \TextNL{Showt $\dim\PoFi\XSlash U = \deg p$.}
	(b) \TextNL{Find a basis of $\PoFi\XSlash U$.}
}\NOTICE that $pq\neq p\circ q,$ see (4E 3.A.10).\par\quad
$U$ is a subsp of $\PoFi$ beca $\forall s_1,s_2\in\PoFi,\lambda\in\Fbb,ps_1+\lambda ps_2=p\Par{s_1+\lambda s_2}\in U.$\par\vspace{3pt}\quad
If $\deg p=0,$ then $U=\PoFi,\PoFi\XSlash U=\Bra{0},$ with the unique basis $\Par{}.$ Sup $\deg p\geqslant 1.$\par\vspace{6pt}\quad
...
\Or Define $R:\PoFi\rightarrow\PoF{\deg p-1}$ by $R\Par{s}=r$ for all $s\in\PoF.$ We showt $R$ is linear.\vspace{2pt}\par\quad\Ha
...
Beca $Rs=0\Longleftrightarrow s=pq,\exists\,!\,q\in\PoFi\Longleftrightarrow s\in U.$ And $\forall r\in\PoF{\deg p-1},Rr=r.$\vspace{2pt}\par\quad\Ha
Now $\null R=U,\;\range R=\PoF{\deg p-1}.$\vspace{3pt}\par\quad\Ha
Hence $\tilde{R}:\PoFi\XSlash U\rightarrow\PoF{\deg p-1}$ is defined by $\tilde{R}\Par{s+U}=Rs.$ By [3.91(d)], $\tilde{R}$ is an iso.\par\vspace{6pt}\quad
(b) For each $k\in\Bra{0,1,\dots,\deg p-1},\tilde{R}\Par{z^k+U}=R\Par{z^k}=z^k\Rightarrow \tilde{R}{^{-1}}\Par{z^k}=z^k+U.$\par\quad\Hb
Thus $\Par{1+U,z+U,\dots,z^{\deg p-1}+U}$ can be a basis of $\PoFi\XSlash U.$\PfEnd
\SepLine

\ProblemN{\hypertarget{4O10}{10}}{
	\TextNL{Sup $m\in\Nbb,p\in\PoC{m}$ is shat $p\Par{x_k}\in\Rbb$ for each of disti $x_0,x_1,\dots,x_m\in\Rbb.$}
	\TextNL{Provet $p\in\PoRi$.}
}\par\quad
By {\TIPS} and Exe (5), $\exists\,!\,q\in\PoR{m}$ shat $q\Par{x_k}=p\Par{x_k}.$ Hence $p=q.$\PfEnd\vspace{6pt}\quad
...
\ProblemBnoor{\hypertarget{4O4e13}{4E 4 13}}{
	\TextB{Sup nonconst $p,q\in\PoCi$ have no zeros in common. Let $m = \deg p$, $n = \deg q$.\vspace{3pt}}
	\TextB{Define $T:\PoC{n-1}\times\PoC{m-1}\rightarrow\PoC{m+n-1}$ by $T\Par{r, s} = rp + sq.$ Provet $T$ is an iso.\vspace{3pt}}
	\Corollary \,\,\,\TextB{$\exists\,!\,r\in\PoC{n-1},s\in\PoC{m-1}$ shat $rp + sq = 1.$}
}\par\quad
$T$ is linear beca $\forall\,r_1,r_2\in\PoC{n-1},s_1,s_2\in\PoC{m-1},\lambda\in\Fbb,$\par\quad
...
Now sup $M,N\geqslant 1.$ We showt $s=0.$ Showing $r=0$ is almost the same.\vspace{2pt}\par\quad
...
For each $D\in\Bra{0,1,\dots,A-1},$ let $I_{D,\alpha}=\Bra{{\gamma_{D,1}},\dots,{\gamma_{D,J}}}$ be shat each $\alpha_{\gamma_{D,j}}\geqslant D+1.$\vspace{2pt}\par\quad
Note that $I_{A-1,\alpha}\subseteq \cdots\subseteq I_{0,\alpha}=\Bra{1,\dots,M}.$ Beca $rp+sq=0\Rightarrow\Par{rp+sq}{^{\SmallPar{k}}}=0$ for all $k\in\Nbp.$\vspace{2pt}\par\quad
We use induction by $D$ to showt $s{^{\SmallPar{D}}}\Par{\lambda_{\gamma_{D,j}}}=0$ for each $D\in\Bra{0,\dots,A-1}.$\vspace{2pt}\par\quad
...
(ii) $2\leqslant D\leqslant A-1.$ Asm $\;s{^{\SmallPar{d}}}\Par{\lambda_{\gamma_{d,j}}}=0$ for each $d\in\Bra{1,\dots,D-1}$ and each $\lambda_{\gamma_{d,j}}\in I_{d,\alpha}.$\vspace{2pt}\par\quad\Hii
\XPar{ Beca $\forall p,q\in\PoFi,k\in\Nbp,\Par{pq}{^{\SmallPar{k}}}=\mathC_{k}^k p{^{\SmallPar{k}}}q{^{\SmallPar{0}}}+\dots+\mathC_{k}^j p{^{\SmallPar{j}}}q{^{\SmallPar{k-j}}}+\dots+\mathC_{k}^0 p{^{\SmallPar{0}}}q{^{\SmallPar{k}}}.$ }\hfill{$\Par{\Delta}$}\vspace{4pt}\par\quad\Hii
...
Hence $s{^{\SmallPar{D}}}\Par{\lambda_{\gamma_{D,j}}}=0.$ The asm holds for all $D\in\Bra{0,\dots,A-1}.$\vspace{6pt}\par\quad
...
\ProblemN{L1:}{
	\TextB{Provet $\forall p,q\in\PoFi,k\in\Nbp,\Par{pq}{^{\SmallPar{k}}}=\mathC_{k}^k p{^{\SmallPar{k}}}q{^{\SmallPar{0}}}+\dots+\mathC_{k}^j p{^{\SmallPar{j}}}q{^{\SmallPar{k-j}}}+\dots+\mathC_{k}^0 p{^{\SmallPar{0}}}q{^{\SmallPar{k}}}.$}
}\par\quad
...
(ii) $k\geqslant 2.$ Asm for $\Par{pq}{^{\SmallPar{k-1}}}=\mathC_{k-1}^{k-1} p{^{\SmallPar{k-1}}}q{^{\SmallPar{0}}}+\dots+\mathC_{k-1}^j p{^{\SmallPar{j}}}q{^{\SmallPar{k-1-j}}}+\dots+\mathC_{k-1}^0 p{^{\SmallPar{0}}}q{^{\SmallPar{k-1}}}.$\vspace{4pt}\par\quad\Hii
...
\ProblemN{L2:}{
	\TextB{Sup $p\Par{z}=\Par{z-\lambda}{^\alpha}q\Par{z}$ and $\alpha\in\Nbp.$ Provet ${p{^{\SmallPar{\alpha-1}}}}\Par{\lambda}=0.$}
}\par\quad
Sup $p\in\PoFi.$ Write $p\Par{z}=\Par{z-\lambda}{^A}q\Par{z},$ where $A\in\Nbp,q\Par{\lambda}\neq 0.$\vspace{2pt}\par\quad
We use induction to showt for all $\alpha\in\Bra{1,\dots,A},{p{^{\SmallPar{\alpha-1}}}}\Par{\lambda}=0.$\vspace{2pt}\par\quad
(i) $\alpha=1.$ $p{^{\SmallPar{0}}}\Par{\lambda}=0.$\par\quad\Endi
(ii) $2\leqslant\alpha\leqslant A.$ Asm $p{^{\SmallPar{a-2}}}\Par{\lambda}=0$ for all $a\in\Bra{1,\dots,\alpha}.$\par\quad\Hii
...
Beca $p{^{\SmallPar{\alpha-1}}}\Par{z}=\XSbra{\mathC_{\alpha-1}^{\alpha-1}\Par{z-\lambda}{^{0}}q_{\alpha-1}\Par{z}+\dots+\mathC_{\alpha-1}^{k}\Par{z-\lambda}{^{\alpha-1-k}}q_{\alpha-1-k}\Par{z}$\vspace{2pt}\par\quad\Hii
\Blind{Beca} $\Blind{p{^{\SmallPar{\alpha-1}}}\Par{z}=}+\dots+\mathC_{\alpha-1}^{0}\Par{z-\lambda}{^{\alpha-1}}q_{\alpha-1}{^{\SmallPar{\alpha-1}}}\Par{z}}.$ Now $p{^{\SmallPar{\alpha-1}}}\Par{\lambda}=\mathC_{\alpha-1}^{\alpha-1}q_{\alpha-1}\Par{\lambda}=0.$\PfEnd
\SepLine
\ChEnd
...
More generally, sup we do not know whether $V$ is findim. We showt $(a)\Longleftrightarrow(b).$\TextB{}
Sup (a) $\lambda$ is an eigval of $T$ with an eigvec $v.$ Then $\Par{T-\lambda I}v=0.$\TextB{}
Hence we get (b), $\Par{T-\lambda I}$ is not inje. And then (d), $\Par{T-\lambda I}$ is not inv.\TextB{}
But (d) $\Rightarrow$ (b) fails, beca $S$ is not inv $\Longleftrightarrow S$ is not inje \OR $S$ is not surj.\par
\SepLine

%\BulletPointX\NoteFor{[5.10]} {\normalsize linely inde eigvecs}\TextB{}
%\TextB{Sup $T\in\Lm{V}.$ Provet every list of eigvecs of $T$ correspd to disti eigvals is linely inde.}
%\Solution \Or Another Proof.\TextB{}
%Sup the desired result is false. Then ( $m\neq 1$ beca eigvecs are nonzero )\TextB{}
%$\,\exists$ smallest $m>1$ so that $\,\exists\,\Par{v_1,\dots,v_m}$ linely depe eigvecs of $T$ correspd to disti eigvals $\lambda_1,\dots,\lambda_m$ of $T.$\TextB{}
%Sup $a_1 v_1+\dots+a_{m-1}v_{m-1}+a_m v_m=0.$ Then $a_j\neq 0$ for each $j$. ( beca the min of $m$) \TextB{}
%Apply $T - \lambda_m I$ to both sides, now we get $a_1 (\lambda_1 - \lambda_m )v_1 + \dots + a_{m-1}(\lambda_{m-1} - \lambda_m )v_{m-1} = 0.$\TextB{}
%Thus $(v_1 ,\dots, v_{m-1})$ of len $(m-1)$ is linely depe eigvecs correspd to disti eigvals.\TextB{}
%Ctradic the min of $m.$\PfEnd
%\SepLine


\BulletPointX\Tips \,\,\,For $T_1,\dots,T_m\in\Lm{V}$:\TextB{\vspace{-2pt}}
(a) Sup $T_1,\dots,T_m$ are all inje. Then $\Par{T_1\circ\cdots\circ T_m}$ is inje.\TextB{}
(b) Sup $\Par{T_1\circ\cdots\circ T_m}$ is not inje. Then at least one of $T_1,\dots,T_m$ is not inje.\TextB{}
(c) At least one of $T_1,\dots,T_m$ is not inje $\nRightarrow$ $\Par{T_1\circ\cdots\circ T_m}$ is not inje.\TextB{}
\Hc\Example\,\, In infindim only. Let $V=\Fbb^\infty.$\TextE{}
...
\BulletPointX\NoteForSmall{[5.2]}\,\,\,Sup $T\in\Lm{V}.$ Then $U$ is an invarsp of $V$ under $T\Longleftrightarrow\range T\mmid_U\subseteq U.$
\SepLine

\ProblemB{
	\TextB{Sup $V$ is findim, $T\in\Lm{V},$ and $U$ is an invarsp of $\,V$ under $T.$}
	\TextB{Provet there exists an invarsp $W$ of dimension $\dim V-dim U.$}
}\par\quad
...
%{\tgbfx Step 1.} If $\exists\,w_1\in\Span{u_{m+1},\dots,u_n}$ shat $0\neq Tw_1\in U.$\par\quad
...
%{\tgbfx Step 2.} If $\exists\,w_2\in\Span{\alpha_{1,m+2},\dots,\alpha_{1,n}}$ shat $0\neq Tw_2\in U\oplus\Span{w_1}.$\par\quad
...
%{\tgbfx Step k.} If $\exists\,w_k\in\Span{a_{k-1,m+k},\dots,\alpha_{k-1,n}}$ shat $0\neq Tw_k\in U\oplus\Span{w_1,\dots,w_{k-1}}.$\par\quad
...
%If $U$ is shat $T\mmid_U\in\Lm{U}$ is inv. Then by [5.30], $T\mmid_X\in\Lm{X}$ is inv.\par\quad
...
\ProblemB[]{
	\TextB{Sup $T\in\Lm{V}$ and $U$ is an invarsp of $V$ under $T.$}
	\TextB{Sup $\lambda_1,\dots,\lambda_m$ are the disti eigvals of $T$ correspd eigvecs $v_1,\dots,v_m.$}
}
\ProblemB{
	\TipsN{1}\,\,\TextB{Provet $v_1+\dots+v_m\in U\Longleftrightarrow$ each $v_k\in U.$}
}\par\quad
Sup each $v_k\in U.$ Then beca $U$ is a subsp, $v_1+\dots+v_m\in U.$\par\quad
Define the stam $P\Par{k}:$ if $v_1+\dots+v_k\in U,$ then each $v_j\in U.$ We use induction on $m.$\par\quad
(i) For $k=1,$ $v_1\in U.$\par\quad\Endi
(ii) For $2\leqslant k\leqslant m.$ Asm $P\Par{k-1}$ holds. Sup $v=v_1+\dots+v_k\in U.$\par\quad\Hii
...
By asm, each $v_j'\in U.$ Thus $v_1,\dots,v_{k-1}\in U.$ So that $v_k=v-v_1-\dots-v_{k-1}\in U.$\PfEnd
\SepLine[0pt][\Blind{\BulletPointX} ]

\ProblemB{
	\TipsN{2}\,\,\TextB{If $\dim V=m.$ Provet $U=\Par{U\cap E_1}\oplus\cdots\oplus\Par{U\cap E_m},$ where $E_k=\Span{v_k}.$}
}\par\quad
Beca $V=E_1\oplus\cdots\oplus E_m.$ $\forall u\in U,\exists\,!\,e_j\in E_j,u=e_1+\dots+e_m.$\par\quad
If $e_j\neq 0,$ then $e_j$ is an eigvec correspd $\lambda_j.$ Otherwise $e_j=0\in U.$ By \TIPSN{1}, each nonzero $e_j\in U.$\par\quad
Thus $u\in\Par{U\cap E_1}+\dots+\Par{U\cap E_m}=U.$ Beca each $\Par{U\cap E_j}\subseteq E_j.$\par\quad
...
\ProblemB{
	\TipsN{3}\,\,\TextB{Sup $W$ is a nonzero invarsp of $V$ under $T.$ If $\dim V=m\geqslant 1.$}
	\Blind{\TipsN{3}\,\,}\TextB{Provet $W=\Span{v_{\alpha_1},\dots,v_{\alpha_A}}$ for some disti $\alpha_1,\dots,\alpha_A\in\Bra{1,\dots,m}.$}
}\par\quad
Each $\Span{v_{\alpha_1},\dots,v_{\alpha_A}}$ is invard $T.$\par\quad
By \TIPSN{2}, $U=\Par{U\cap E_1}\oplus\cdots\oplus\Par{U\cap E_m}.$ Beca each $\dim E_k=1,$ $U\cap E_k=\zeroSubs$ or $E_k.$\par\quad
There must be at least one $k$ shat $E_k=U\cap E_k,$ for if not, $U=\zeroSubs$ since $V=E_1\oplus\cdots\oplus E_m.$\par\quad
...
\ProblemN[]{\hypertarget{5A1}{1}}{
	\TextN{Sup $T\in\Lm{V}$ and $U$ is a subsp of $V$.}
	(a) \TextN{{If $U\subseteq\null T$, then $U$ is invard $T$.} {\large\envFontDefault$\forall u\in U\subseteq\null T,Tu=0\in U.$\PfEnd}\vspace{4pt}}
	(b) \TextN{{If $\range T\subseteq U$, then $U$ is invard $T$.} {\large\envFontDefault$\forall u\in U,Tu\in\range T\subseteq U.$\PfEnd}}
}\par
\SepLine

\ProblemB{
	\TextB{Sup $S,T\in\Lm{V}$ are shat $ST=TS.$ }
	(a) \TextB{Provet $\null\Par{T-\lambda I}$ is invard $S$ for any $\lambda\in\Fbb$.}
	(b) \TextB{Provet $\range\Par{T-\lambda I}$ is invard $S$ for any $\lambda\in\Fbb$.}
}\par\quad
...
\ProblemB[]{
	\TextB{Sup $S,T\in\Lm{V}$ are shat $ST=TS.$\vspace{4pt}}
}
\ProblemN[]{2}{
	\TextB{Showt $W=\null T$ is invard $S$.\quad\large$\forall u\in W,Tu=0\Rightarrow STu=0=TSu\Rightarrow Su\in W.$\PfEnd\vspace{3pt}}
}
\ProblemN[]{3}{
	\TextB{Showt $U=\range T$ is invard $S$.\,\,\,\large$\forall w\in U,\exists\,v\in V,Tv=w, TSv=STv=Sw\in U.$\PfEnd}
}
\SepLine

\ProblemB[]{
	\TextB{Sup $T\in\Lm{V}$ and $V_{\!1},\dots,V_{\!m}$ are invarsps of $V$ under $T$.}
}
...
\ProblemN{\hypertarget{5A6}{6}}{
	\TextN{Sup $U$ is an invarsp of $V$ under each $T\in\Lm{V}.$ Showt $U = \zeroSubs$ or $U = V$.\vspace{4pt}}
}If $V=\zeroSubs.$ Then we are done. Sup $V\neq\zeroSubs.$ We show the ctrapos:\parSol{}
{\tgsl Sup $U\neq\zeroSubs$ and $U\neq V.$ Provet $\exists\,T\in\Lm{V}$ shat $U$ is not invard $T$.}\parSol{}
Let $W$ be shat $V=U\oplus W.$ Define $T\in\Lm{V}$ by $T\Par{u+w}=w.$\PfEnd
\SepLine

\BulletPointX\Tips\,\,\, {Sup $T\in\Lm{\Rbb^2}$ is the counterclockwise rotation by the angle $\theta\in\Rbb.$}\par\quad
...
{Hence $T\Par{a,b}=\BigPar{a\cos\theta-b\sin\theta,a\sin\theta+b\cos\theta}.$ Now $\Mt{T}=\;${\normalsize$\begin{pmatrix}
		...
		\end{pmatrix}$}.}\vspace{6pt}\par\quad
\Example \,\,\,\OR\ProblemN[]{\hypertarget{5A7}{7}}{ Sup $T\in\Lm{\Rbb^2}$ is defined by $T\Par{x, y} = \Par{-3y, x}.$ Find all eigvals of $T$.\TextN{\vspace{4pt}}}\quad
\NOTICE that $\Mt{T}=\;${\normalsize$\begin{pmatrix}
...
\Or Sup $\lambda$ is an eigval with an eigvec $\Par{x,y}.$ Then $\Par{\lambda x,\lambda y}=\Par{-3y,x}\Rightarrow-3y=\lambda^2 y\Rightarrow \lambda^2=-3.$\par\quad
\Blind{\Or}\Sbra{ Ignoring the possibility of $y=0,$ beca $x=0\Longleftrightarrow y=0.$ }\PfEnd
\SepLine

\ProblemN{\hypertarget{5A8}{8}}{
	\TextN{Define $T\in\Lm{\Fbb^2}$ by $T\Par{w, z} = \Par{z, w}$. Find all eigvals and eigvecs.\vspace{4pt}}
}Sup $\lambda$ is an eigval with an eigvec $\Par{w,z}.$ Then $z=\lambda w$ \,{\small and}\, $ w=\lambda z.$\parSol{}
...
}Sup $\lambda$ is an eigval with an eigvec $\Par{z_1,z_2,z_3}.$\parSol{}
...
(a) Sup $x=\Par{x_1,x_2,x_3,\dots,x_n}$ is an eigvec with an eigval $\lambda.$\par\quad\Ha
...
(b) Let $\Par{e_1,\dots,e_n}$ be the std basis of $\Fbb^n.$ Let $V_{\!k}=\Span{e_k}.$ Then $V_{\!1},\dots,V_{\!n}$ are invard $T.$\par\quad\Hb
...
\ProblemN{\hypertarget{5A18}{18}}{
	...
	\TextNL{Showt $T$ has no eigvals.}
}Sup $\lambda$ is an eigval of $T$ with an eigvec $\Par{z_1,z_2,\dots}.$\parSol{}
...
\ProblemN{\hypertarget{5A19}{19}}{
	\TextNL{Sup $n\in\Nbp$. Define $T\in\Lm{\Fbb^n}$ by $T\Par{x_1,\dots,x_n}=\Par{x_1+\dots+x_n,\dots,x_1+\dots+x_n}.$}
	\TextNL{In other words, the ent of $\Mt{T}$ with resp to the std basis are all \,$1$'s.}
	\TextNL{Find all eigvals and eigvecs of $T.$}
}\par\quad
Sup $\lambda$ is an eigval of $T$ with an eigvec $\Par{x_1,\dots,x_n}.$\par\quad
...
\ProblemN{\hypertarget{5A20}{20}}{
	...
	(a) \TextNL{Showt every ele of $\Fbb$ is an eigval of $S;\;\;$ {\tgnr\large(b)} Find all eigvecs of $S$.}
}\par\quad
Sup $\lambda$ is an eigval of $S$ with an eigvec $\Par{z_1,z_2,\dots}.$\par\quad
...
Note that $\forall p\in\PoRi\nonzero,\deg p\apostrophe<\deg p.$ And $\deg 0=-\infty.$ Sup $\lambda$ is an eigval with an eigvec $p.$\par\quad
...
Sup $\lambda$ is an eigval of $T$ with an eigvec $p$, then $\Par{Tp}\Par{x}=xp\apostrophe\Par{x}=\lambda p\Par{x}.$\par\quad
...
\ProblemB[]{
	\TextB{Sup $V$ is findim, $T\in\Lm{V},\lambda\in\Fbb.$}
}
\ProblemN{\hypertarget{5A13}{13}}{
	 \TextB{Provet $\forall\lambda\in\Fbb,\exists\,\alpha\in\Fbb,\left|\alpha-\lambda\right|<\frac{1}{\;1000\;},\Par{T-\alpha I}$ is inv.}
}\par\quad
Let $\alpha_k\in\Fbb$ be shat $\aMid{\alpha_k-\lambda}=\frac{1}{\;1000+k\;}$ for each $k=1,\dots,\dim V+1.$\par\vspace{2pt}\quad
Note that each $T\in\Lm{V}$ has at most $\dim V$ disti eigvals.\par\quad
Hence $\exists\,k=1,\dots,\dim V+1$ shat $\alpha_k$ is not an eigval of $T$ and therefore $\Par{T-\alpha_k I}$ is inv.\PfEnd
\SepLine[0pt][\Blind{\BulletPointX} ]

\ProblemBnoor{4E 5.A.11}{
	\TextB{Provet $\exists\,\delta > 0$ shat $\Par{T-\alpha I}$ is inv for all $\alpha\in\Fbb$ shat $0 < \aMid{\alpha-\lambda} < \delta$.}
}\par\quad
If $T$ has no eigvals, then $\Par{T-\alpha I}$ is inje for all $\alpha\in\Fbb$ and we are done.\par\quad
Sup $\lambda_1,\dots,\lambda_m$ are all the disti eigvals of $T.$\par\quad
Let $\delta>0$ be shat, for each eigval $\lambda_k,$ $\lambda_k\not\in\Par{\lambda-\delta,\lambda}\cup \Par{\lambda,\lambda+\delta}.$\par\quad
So that for all $\alpha\in\Fbb$ shat $0<\aMid{\alpha-\lambda}<\delta,\Par{T-\alpha I}$ is not inje.\PfEnd\vspace{4pt}\quad
\Or Let $\delta=\min\!\Bra{\aMid{\lambda-\lambda_k}:k\in\envFontSmall\Bra{1,\dots,m}\envFontDefault,\lambda_k\neq\lambda}.$\par\quad
Then $\delta>0$ and each $\lambda_k\neq\alpha$ \Sbra{ $\Longleftrightarrow\Par{T-\alpha I}$ is inv } for all $\alpha\in\Fbb$ shat $0<\aMid{\alpha-\lambda}<\delta.$\PfEnd
\SepLine

\ProblemBnoor{\hypertarget{5A4e8}{}{5.B.4} \,\OR\, 4E 3.B.27}{
	\TextB{Sup $\lambda$ is an eigval of $P\in\Lm{V},P^2 = P.$ Provet $\lambda = 0$ or $\lambda = 1$.}
}Sup $\lambda$ is an eigval with an eigvec $v.$ Then $P\Par{Pv}=Pv\Rightarrow\lambda^2 v=\lambda v.$ Thus $\lambda=1$ or $0.$\PfEnd
\SepLine

\ProblemN{\hypertarget{5A14}{14}}{
	\TextNL{Sup $V = U\oplus W$, where $U$ and $W$ are nonzero subsps of $V$.}
	\TextNL{Define $P\in\Lm{V}$ by $P\Par{u + w} = u$ \,for each $u\in U$ and each $w\in W$.}
	\TextNL{Find all eigvals and eigvecs of $P$.}
}\par\quad
Sup $\lambda$ is an eigval of $P$ with an eigvec $\Par{u+w}.$\par\quad
...
\ProblemN{\hypertarget{5A15}{15}}{
	\TextNL{Sup $T\in\Lm{V}$. Sup $S\in\Lm{V}$ is inv.\vspace{4pt}}
	(a) \TextNL{Provet $T$ and $S^{-1}TS$ have the same eigvals.\vspace{3pt}}
...
(b) Beca $\lambda$ is an eigval of $T\Longleftrightarrow \lambda$ is an eigval of $S^{-1}TS.$\par\quad\Hb
...
\ProblemN{\hypertarget{5A17}{17}}{
	\TextNL{Give an exa of an operator on $\Rbb^4$ that has no real eigvals.}
}\par\quad
Let $\Par{e_1,e_2,e_3,e_4}$ be the std basis of $\Rbb^4.$\vspace{-6pt}\par\quad
...
Sup $\lambda$ is an eigval of $T$ with an eigvec $\Par{x,y,z,w}.$ Then we get \;{\normalsize$\MathLeftBrace{l}{
...
Sup $\lambda$ is an eigval of $T$ with an eigvec $\Par{x,y,z,w}.$\vspace{-3pt}\par\quad
...
\ProblemBnoor{\hypertarget{5A4e16}{4E 5.A.16}}{
	\TextB{Sup $B_V=\Par{v_1,\dots,v_n},T\in\Lm{V},\Mt[\BigPar]{T,\Par{v_1,\dots,v_n}}=A.$}
	\TextB{Provet if $\lambda$ is an eigval of $T$, then $\aMid{\lambda}\leqslant n \max\!\Bra{\aMid{A_{j,k}}:1\leqslant j, k\leqslant n}.$}
}\par\quad
Sup $v$ is an eigval of $T$ correspd to $\lambda.$ Let $v=c_1v_1+\dots+c_nv_n.$\vspace{2pt}\par\quad
Beca $\lambda c_1v_1+\dots+\lambda c_nv_n=c_1Tv_1+\dots+c_nTv_n=\sum_{k}^nc_k\BigPar{\sum_{j}^n A_{j,k}v_j}.$\vspace{3pt}\par\quad
...
\ProblemBnoor{\hypertarget{5A4e15}{4E 5.A.15}}{
	\TextB{Sup $T\in\Lm{V}$, and $\lambda\in\Fbb.$}
	\TextB{Showt $\lambda$ is an eigval of $T$ $\Longleftrightarrow$ $\lambda$ is an eigval of the dual operator $T\apostrophe\in\Lm{V\apostrophe}$.}
}\par\quad
(a) Sup $\lambda$ is an eigval of $T$ with an eigvec $v.$\par\quad\Ha
Let $U$ be invar shat $V=\Span{v}\oplus U$ \Sbra{ by (4E 5.A.39) }.\par\quad\Ha
...
(b) Sup $\lambda$ is an eigval $T\apostrophe$ with an eigvec $\psi.$ Then $T\apostrophe\Par{\psi}=\psi\circ T=\lambda\psi.$\par\quad\Hb
...
\ProblemN{\hypertarget{5A24}{24}}{
	\TextNL{Sup $A\in\Fbb^{n,n}.$ Define $T\in\Lm{\Fbb^{n,1}}$ by $Tx = Ax$.}
	(a) \TextNL{Sup the sum of the ent in each row of $A$ equals $1$. Provet $1$ is an eigval of $T$.}
	(b) \TextNL{Sup the sum of the ent in each col of $A$ equals $1$. Provet $1$ is an eigval of $T$.}
}\vspace{-8pt}\par\quad
Sup $\lambda$ is an eigval of $T$ with an eigvec $x.$ Then $Tx=Ax={}${\normalsize$\begin{pmatrix} \sum_{k=1}^n A_{1,k}x_k\\ \vdots\\ \sum_{k=1}^n A_{n,k}x_k\end{pmatrix}$}${}=\lambda ${\normalsize$\begin{pmatrix} x_1\\ \vdots\\ x_n\end{pmatrix}$}.\vspace{-6pt}\par\quad
(a) Sup $\sum_{r=1}^n A_{R,c}=1$ for each $R\in\Bra{1,\dots,n}.$\par\quad\Ha
Then if we let $x_1=\dots=x_n,$ then $\lambda=1,$ and hence is an eigval of $T.$\par\vspace{10pt}\quad
(b) Sup $\sum_{r=1}^n A_{r,C}=1$ for each $C\in\Bra{1,\dots,n}.$\vspace{4pt}\par\quad\Hb
...
Hence $\lambda=1$ for all $x\in\Fbb^{n,1}$ shat $\sum_{c=1}^n x_{c,1}\neq 0.$\PfEnd\vspace{10pt}\quad\Hb
\Or We showt $\Par{T-I}$ is not inv, so that $\lambda=1$ is an eigval.\vspace{2pt}\par\quad\Hb
Beca $\Par{T-I}x=\Par{A-I}x={}${\small$\begin{pmatrix} \sum_{r=1}^n A_{1,r}x_r-x_1\\ \vdots\\ \sum_{r=1}^n A_{n,r}x_r-x_n\end{pmatrix}=\begin{pmatrix} y_1\\ \vdots\\ y_n\end{pmatrix}$}.\vspace{4pt}\par\quad\Hb
...
\Or Let $\Par{e_1,\dots,e_n}$ be the std basis of $\Fbb^{n,1}.$ Define $\psi\in\BigPar{\Fbb^{n,1}}\apostrophe$ by $\psi\Par{e_k}=1.$\vspace{2pt}\par\quad\Hb
...
\Or Define $S\in\Lm{\Fbb^{n,1}}$ by $Sx=A^tx.$ Beca the rows of $A^t$ are the cols of $A.$\par\quad\Hb
...
\ProblemB{
	\TextB{Sup $A\in\Fbb^{n,n}.$ Define $T\in\Lm{\Fbb^{1,n}}$ by $Tx = xA.$}
	(a) \TextB{Sup the sum of the ent in each col of $A$ equals $1$. Provet $1$ is an eigval of $T$.}
	(b) \TextB{Sup the sum of the ent in each row of $A$ equals $1$. Provet $1$ is an eigval of $T$.}
}\par\quad
Sup $\lambda$ is an eigval with an eigvec $x.$ Then {\normalsize$\begin{pmatrix} \sum_{r=1}^n x_r A_{r,1} &\cdots & \sum_{r=1}^n x_r A_{r,n}\end{pmatrix}$}${}=\lambda${\normalsize$\begin{pmatrix} x_1 & \cdots & x_n\end{pmatrix}$}.\vspace{4pt}\par\quad
(a) Sup $\sum_{r=1}^n A_{r,C}=1$ for each $C\in\Bra{1,\dots,n}.$\par\quad\Ha
Thus if $x_1=\dots=x_n,$ then $\lambda=1,$ hence is an eigval of $T.$\vspace{4pt}\par\quad
(b) Sup $\sum_{c=1}^n A_{R,c}=1$ for each $R\in\Bra{1,\dots,n}.$\vspace{2pt}\par\quad\Hb
...
\Or Let $\Par{e_1,\dots,e_n}$ be the std basis of $\Fbb^{1,n}.$ Define $\psi\in\Par{\Fbb^{n,1}}\apostrophe$ by $\psi\Par{e_k}=1.$\vspace{2pt}\par\quad\Hb
Beca $Te_k=e_k A=\begin{pmatrix}A_{k,1}&\cdots&A_{k,n}\end{pmatrix}=\sum_{i=1}^n A_{k,i}e_i.$ \Corollary \,\,\,$\Mt{T}=A^t.$\vspace{3pt}\par\quad\Hb
...
\Or Define $S\in\Lm{\Fbb^{1,n}}$ by $Sx=xA^t.$ Beca the rows of $A$ are the cols of $A^t.$\par\quad\Hb
Now by (a), $1$ is an eigval of $S.$ Let $\Par{\varphi_1,\dots,\varphi_n}$ be the dual basis of $\Par{e_1,\dots,e_n}.$\vspace{2pt}\par\quad\Hb
Define $\Phi\in\Lm[\XPar]{\Fbb^{1,n},\BigPar{\Fbb^{1,n}}\apostrophe}$ by $\Phi\Par{e_k}=\varphi_k.$ Beca $\Sbra{T\apostrophe\Par{\varphi_k}}\Par{e_j}=\varphi_k\BigPar{\sum_{i=1}^n A_{j,i}e_i}=A_{j,k}.$\par\quad\Hb
...
\ProblemB{
	\TextB{Sup $\Fbb=\Rbb,$ $T\in\Lm{V}$.}
	(a) \hypertarget{5A4e17}{}\Sbra{{\normalsize\OR (9.11)}} \TextB{$\lambda\in\Rbb.$ Provet $\lambda$ is an eigval of $T$ $\Longleftrightarrow$ $\lambda$ is an eigval of $T_{\!\Cbb}.$}
	(b) \hypertarget{5A16}{}\Sbra{{\normalsize\OR \Onumber{16} \;\OR [9.16]\,}} \TextB{$\lambda\in\Cbb.$ Provet $\lambda$ is an eigval of $T_{\!\Cbb}$ $\Longleftrightarrow$ $\overline \lambda$ is an eigval of $T_{\!\Cbb}.$}
}\par\quad
(a) Sup $\lambda$ is an eigval of $T$ with an eigvec $v.$\par\quad\Ha
...
Sup $\lambda$ is an eigval of $T_{\!\Cbb}$ with an eigvec $v+\i u.$\par\quad\Ha
...
(b) Sup $\lambda$ is an eigval of $T_{\!\Cbb}$ with an eigvec $v+\i u.$ Then $T_{\!\Cbb}\Par{v+\i u}=Tv+\i Tu=\lambda\Par{v+\i u}.$\vspace{2pt}\par\quad\Hb
...
\Or Sup $\lambda=a+\i b$ is an eigval of $T_{\!\Cbb}$ with an eigvec $v+\i u.$\par\quad\Hb
Beca $T_{\!\Cbb}\Par{v+\i u}=\lambda\Par{v+\i u}=\Par{av-bu}+\i\Par{au+bv}=Tv+\i Tu\Longrightarrow Tv=av-bu,Tu=au+bv.$\par\quad\Hb
...
\ProblemN{\hypertarget{5A21}{21}}{
	\TextNL{Sup $T\in\Lm{V}$ is inv.}
	(a) \TextNL{Sup $\lambda\in\Fbb$ with $\lambda\neq 0$. Provet $\lambda$ is an eigval of $T$ $\Longleftrightarrow\lambda^{-1}$ is an eigval of $T^{-1}$.}
	(b) \TextNL{Provet $T$ and $T^{-1}$ have the same eigvecs.}
...
\ProblemN{\hypertarget{5A22}{22}}{
	\TextNL{Sup $T\in\Lm{V}$ and $\exists$ nonzero vecs $u,w$ in $V$ shat $Tu = 3w,\;Tw = 3u$.}
	\TextNL{Provet $3$ or $-3$ is an eigval of $T$.}
...
\ProblemN{\hypertarget{5A23}{23}}{
	\TextNL{Sup $S, T\in\Lm{V}$. Provet $ST$ and $TS$\, have the same eigvals.\vspace{2pt}}
}Sup $\lambda$ is an eigval of $ST$ with an eigvec $v.$ Then $T\Par{STv}=\lambda Tv=TS\Par{Tv}.$\parSol{}
...
\ProblemBnoor{\hypertarget{5A2e20}{2E 20}}{
	\TextB{Sup $T\in\Lm{V}$ has $\dim V$ disti eigvals and $S\in\Lm{V}$ has the same eigvecs}
	\TextB{\Par{ but might not with the same eigvals }. Provet $ST=TS.$}
}Let $n=\dim V.$ For each $j\in\Bra{1,\dots,n},$ let $v_j$ be an eigvec with eigval $\lambda_j$ of $T$ and $\alpha_j$ of $S$.\parSol{}
Then $B_V=\Par{v_1,\dots,v_n}.$ Beca $\Par{ST}v_j=\alpha_j\lambda_j v_j=\Par{TS}v_j$ for each $j.$ Hence $ST=TS.$\PfEnd
\SepLine

\ProblemBnoor{\hypertarget{5A4e37}{4E 5.A.37}}{
	\TextB{Sup $V$ is findim and $T\in\Lm{V}$.}
	\TextB{Define $\mathcal{A}\in\Lm[\BigPar]{\Lm{V}}$ by $\mathcal{A}\Par{S} = TS$ for each $S\in\Lm{V}$.}
	\TextB{Provet the set of eigvals of \,$T$ equals the set of eigvals of $\mathcal{A}$.}
}\par\quad
(a) Sup $\lambda$ is an eigval of $T$ with an eigvec $v=v_1.$ Let $B_V=\Par{v_1,\dots,v_m,\dots,v_n}.$\par\quad\Ha
...
(b) Sup $\lambda$ is an eigval of $\mathcal{A}$ with an eigvec $S.$\par\quad\Hb
...
\Or Beca $TS-\lambda S=\Par{T-\lambda I}S=0\Rightarrow\zeroSubs\subsetneq\range S\subseteq\null\Par{T-\lambda I}.$ $\Par{T-\lambda I}$ is not inje.\PfEnd\vspace{4pt}
...
\ProblemN{\hypertarget{5A25}{25}}{
	\TextNL{Sup $T\in\Lm{V}$ and $u, w$ are eigvecs of $T$ shat $u + w$ is also an eigvec of $T$.}
	\TextNL{Provet $u$ and $w$ correspd to the same eigval.}
}Sup $\lambda_1,\lambda_2,\lambda_0$ are eigvals of $T$ with eigvecs to $u,w,u+w$ respectively.\parSol{}
...
Otherwise, $\Par{u,w}$ is linely inde. Then $\lambda_0-\lambda_1=\lambda_2-\lambda_0=0\,\Rightarrow\lambda_1=\lambda_2=\lambda_0.$\PfEnd\parSol{}
\Or Asm $\lambda_1\neq \lambda_2.$ Then $\Par{u,w}$ is linely inde. Thus $\lambda_0-\lambda_1=\lambda_0-\lambda_2.$ Ctradic.\PfEnd
\SepLine

\ProblemN{\hypertarget{5A26}{26}}{
	\TextNL{Sup $T\in\Lm{V}$ is shat every nonzero vec in $V$ is an eigvec of \,$T$.}
	\TextNL{Provet $T$ is a scalar multi of the id operator.}
}If $\dim V=0,1$ then we are done. Sup $\dim V\geqslant 2.$\par\quad
Beca $\forall v\in V,\exists\,!\,\lambda_v\in\Fbb,Tv=\lambda_v v.$ For any two disti nonzero vecs $v,w\in V,$\par\quad
...
\ProblemN{\hypertarget{5A27}{27}, \hypertarget{5A28}{28}}{
	\TextNL{Sup $V$ is findim and $k\in\Bra{1, \dots , \dim V - 1}$.}\qquad
	\TextNL{Sup $T\in\Lm{V}$ is shat every subsp of $V$ of dim $k$ is invard $T$.}\qquad
	\TextNL{Provet $T$ is a scalar multi of the id operator.}
}If $\dim V\leqslant 1$ then we are done. Sup $\dim V\geqslant 2.$\par\quad
We prove the ctrapos: {\tgsl If $T$ is not a scalar multi of $I$. Then $\exists$ subsp $U$ of dim $k$ not invard $T.$}\par\quad
By Exe (26), $\exists\,v\in V$ and $v\neq 0$ shat $v$ is not an eigvec of $T.$\par\quad
Thus $\Par{v,Tv}$ is linely inde. Extend to $B_V=\Par{v,Tv,u_1,\dots,u_n}.$\par\quad
Let $U=\Span{v,u_1,\dots,u_{k-1}}\Rightarrow U$ is not an invarsp of $V$ under $T.$\PfEnd\vspace{6pt}\quad
\Or Sup $0\neq v=v_1\in V.$ Extend to $B_V=\Par{v_1,\dots,v_n}.$ Sup $Tv_1=c_1 v_1+\dots+c_n v_n,\exists\,!\,c_i\in\Fbb.$\par\quad
...
Beca every subsp such $U$ is invar. $Tv_1=c_1 v_1+\dots+c_n v_n\in U\Longrightarrow c_2=\dots=c_n=0.$\vspace{0pt}\par\quad
...
Hence $Tv_1=c_1 v_1.$ Beca $v_1=v\in V$ is arb. We conclude that $T=\lambda I$ for some $\lambda\in\Fbb.$\PfEnd\vspace{6pt}\quad
...
(ii) Asm $P\Par{k}$ holds for $k\in\Bra{1,\dots,\dim V-1}.$ And every subsp of dim $k+1$ is invar.\par\quad\Hii
Let $U$ be a subsp of dim $k.$ If $\dim U=\dim V-1$ then extend $B_U$ to $B_V$ and we are done.\par\quad\Hii
Sup $\dim U\in\Bra{1,\dots,\dim V-2}.$ Choose two linely inde vecs $v,w\not\in U.$\par\quad\Hii
Beca $U\oplus\Span{v}$ and $U\oplus\Span{w}$ of dim $k+1$ are invar.\par\quad\Hii
Sup $u\in U.$ Let $Tu=a_1u_1+bv=a_2u_2+cw,\exists\,!\,u_1,u_2\in U,a_1,a_2,b,c\in\Fbb.$\par\quad\Hii
Now $a_1u_1-a_2u_2=cw-bv\in U\cap\Span{v}=\zeroSubs\Rightarrow b=c=0.$ Thus $Tu\in U.$\par\quad\Hii
Beca $P\Par{k}$ holds, we conclude that $T=\lambda I.$ Thus $P\Par{k+1}$ holds.\PfEnd
\SepLine

\ProblemN{\hypertarget{5A29}{29}}{
	\TextNL{Sup $T\in\Lm{V}$ and $\range T$ is findim.}
	\TextNL{Provet $T$ has at most $1 + \dim\range T$ disti eigvals.}
}\par\quad
Let $\lambda_1,\dots,\lambda_m$ be the disti eigvals of $T$ with correspd eigvecs $v_1,\dots,v_m.$\par\quad
\BigPar{ Beca $\range T$ is findim. The correspd eigvals are finite. }\par\quad
...
\ProblemN{\hypertarget{5A30}{30}}{
	\TextNL{Sup $T\in\Lm{\Rbb^3}$ and $-4,5,\sqrt{7}$ are eigvals. Provet $\exists\,x,Tx - 9x = \Par{-4, 5, \sqrt{7}}$.}
...
\ProblemN{\hypertarget{5A31}{31}}{
	\TextNL{Sup $V$ is findim, and $v_1,\dots,v_m \in V$. Provet}
	\TextNL{$\Par{v_1,\dots,v_m}$ is linely inde $\Longleftrightarrow v_1,\dots,v_m$ are eigvecs of some $T$ correspd to disti eigvals.}
}Sup $\Par{v_1,\dots,v_m}$ is linely inde. Let $B_V=\Par{v_1,\dots,v_m,\dots,v_n}.$\parSol{}
Define $T\in\Lm{V}$ by $Tv_k=k\cdot v_k$ for each $k\in\Bra{1,\dots,m,\dots,n}.$ Conversely by [5.10].\PfEnd\vspace{-4pt}
\SepLine

\ProblemB{
	\TextB{Sup $\lambda_1,\dots,\lambda_n\in\Rbb$ are disti.}
	(a)\ProblemN[]{\hypertarget{5A32}{32}}{
		\TextB{Provet $\Par{e^{\lambda_1x},\dots,e^{\lambda_nx}}$ is linely inde in $\Rbb^{\Rbb}$.}
	...
	(b)\ProblemNnoor[]{\!\!}{4E 36}{
		\hypertarget{5A4e36}{}\TextB{Showt $\BigPar{\cos\!\Par{\lambda_1 x},\dots,\cos\!\Par{\lambda_n x}}$ is linely inde in $\Rbb^{\Rbb}.$\vspace{-4pt}}
	}
	\TextB{}
}\par\quad
(a) Define $V$ and $D\in\Lm{V}$ as in H{\small INT}. Then beca for each $k, D\Par{e^{\lambda_k x}}=\lambda_k e^{\lambda_k x}.$\par\quad\Ha
Thus $\lambda_1,\dots,\lambda_n$ are disti eigvals of $D.$ By [5.10], $\Par{e^{\lambda_1x},\dots,e^{\lambda_nx}}$ is linely inde in $\Rbb^{\Rbb}.$\PfEnd\vspace{2pt}\quad
(b) Let $V=\Span[\BigPar]{\cos\!\Par{\lambda_1 x},\dots,\cos\!\Par{\lambda_n x}}.$ Define $D\in\Lm{V}$ by $Df=f\apostrophe.$\par\quad\Hb
Then beca $D\BigPar{\cos\!\Par{\lambda_k x}}=-\lambda_k\sin\!\Par{\lambda_k x}.$ 又 $D\BigPar{\sin\!\Par{\lambda_k x}}=\lambda_k\cos\!\Par{\lambda_k x}.$\par\quad\Hb
...
\ProblemN{\hypertarget{5A33}{33}}{
	\TextNL{Sup $T\in\Lm{V}.$ Provet $T\XSlash \Par{\range T}=0.$\vspace{2pt}}
...
\ProblemB{
	\hypertarget{5A4e38}{}\TextB{Sup $V$ is findim, $T\in\Lm{V}$, and $U$ is an invarsp of $V$ under $T$.}
	\TextB{Define $T\XSlash U:V\XSlash U\rightarrow V\XSlash U$ by $\Par{T\XSlash U}\Par{v + U} = Tv + U$ for each $v\in V$.}
	(a) \TextB{Showt $T\XSlash U$ is well-defined and is linear. {\large Requires that $U$ is invard $T.$}}
	(b) \Sbra{{\normalsize\OR \Onumber{\hypertarget{5A35}{35}}\,}} \TextB{Showt each eigval of $T\XSlash U$ is an eigval of $T$.}
}\par\quad
(a) $v+U=w+U\Longleftrightarrow v-w\in U\Longrightarrow T\Par{v-w}\in U\Longleftrightarrow Tv+U=Tw+U.$\par\quad\Ha
Hence $T\XSlash U$ is well-defined. Now we showt $T\XSlash U$ is linear.\par\quad\Ha
...
(b) Sup $\lambda$ is an eigval of $T\XSlash U$ with an eigvec $v+U.$ Then $Tv+U=\lambda v+U\Rightarrow \Par{T-\lambda I}v=u\in U.$\par\quad\Hb
...
If $\Par{T-\lambda I}\mmid_U$ is not inv. Then beca $V$ is findim, $\Par{T-\lambda I}\mmid_U$ is not inje,\par\quad\Hb
...
\ProblemN{\hypertarget{5A36}{36}}{
	\TextNL{Prove or give a counterexa\hspace{1pt}$:$ The result in Exercise 35 is still true if $V$ is infindim.}
}A counterexa:\par\quad
Consider $V=\Bra{\,f\in\Rbb^{\Rbb}:\exists\,!\,m\in\Nbb,f\in\Span{1,e^x,\dots,e^{mx}}}.$ Note that $V$ is infindim.\par\quad
...
\ProblemBnoor{\hypertarget{5A4e39}{4E 5.A.39}}{
	\TextB{Sup $V$ is findim and $T\in\Lm{V}$.}
	\TextB{Provet $T$ has an eigval $\Longleftrightarrow\exists\,$ an invarsp $U$ under $T$ of dimension $\dim V - 1$.}
}\par\quad
(a) Sup $\lambda$ is an eigval of $T$ with an eigvec $v.$ \Par{ If $\dim V=1,$ then $U=\zeroSubs$ and we are done. }\par\quad\Ha
Extend $v_1=v$ to $B_V=\Par{v_1,v_2\dots,v_n}.$\par\quad\Ha
{\tgbfx Step 1.} If $\exists\,w_1\in\Span{v_2,\dots,v_n}$ shat $0\neq Tw_1\in\Span{v_1}.$\par\quad\Ha
...
{\tgbfx Step 2.} If $\exists\,w_2\in\Span{\alpha_{1,3},\dots,\alpha_{1,n}}$ shat $0\neq Tw_2\in\Span{v_1,w_1}.$\par\quad\Ha
...
{\tgbfx Step k.} If $\exists\,w_k\in\Span{\alpha_{k-1,k+1},\dots,\alpha_{k-1,n}}$ shat $0\neq Tw_k\in\Span{v_1,w_1,\dots,w_{k-1}},$\par\quad\Ha
...
{\tgsl If $T\mmid_U$ is inv, then by the similar algorithm, we can extend $U$ to an invarsp.}\par\vspace{6pt}\quad\Ha
...
If $m=\dim V-1.$ \Sbra{ $\Longleftrightarrow n=0.$ } Then $\range\Par{T-\lambda I}$ is an invarsp of dim $\dim V-1.$\par\quad\Ha
...
(b) Sup $U$ is an invarsp under $T$ of dim $m=\dim V-1.$ \Par{ If $m=0,$ then we are done. }\par\quad\Hb
...
\Or Consider $T\XSlash U\in\Lm{V\XSlash U}.$ Beca $\dim V\XSlash U=1.$ $\exists\,\lambda\in\Fbb,T\XSlash U=\lambda I.$ By Exe (35).\PfEnd
\SepLine
\ChEnd\pagebreak
...
\ProblemBnoor{\hypertarget{5A4e33}{4E 5.A.33}}{
	\TextB{Sup $T\in\Lm{V}$ and $\,m\,$ is a positive integer.}
	(a) \TextB{Provet $T$ is inje $\Longleftrightarrow$ $T^m$ is inje.}
	(b) \TextB{Provet $T$ is surj $\Longleftrightarrow$ $T^m$ is surj.}
}\par\quad
(a) Sup $T^m$ is inje. Then $Tv=0\Rightarrow T^{m-1} Tv=T^m v=0\Rightarrow v=0.$\par\quad\Ha
Sup $T$ is inje. Then $T^m v=T^{m-1}v=\cdots=T^2 v=Tv=v=0.$\par\vspace{4pt}\quad
(b) Sup $T^m$ is surj. $\forall u\in V,\exists\,v\in V,T^m v=u=Tw,$ let $w=T^{m-1}v.$\par\quad\Hb
Sup $T$ is surj. Then $\forall u\in V,\exists\,v_1,\dots,v_m\in V, T\Par{v_1}=T^2v_2=\cdots=T^m v_m=u.$\PfEnd
\SepLine

\BulletPointX\NoteFor{[5.17]}\TextB{}
\TextB{Sup $T\in\Lm{V},p\in\PoFi$. Provet $\null p\Par{T}$ and $\range p\Par{T}$ are invard $T$.}
...
(b) Sup $u\in\range p\Par{T}$. Then $\exists\,v\in V$ shat $u = p\Par{T}v$.\par\quad\Hb
Thus $Tu = T\BigPar{p\Par{T}v} = p\Par{T}\Par{Tv}\in\range p\Par{T}.$\PfEnd
\SepLine

\BulletPointX\NoteFor{[5.21]} \TextB{\large Every operator on a findim nonzero complex vecsp has an eigval.}
Sup $V$ is a findim complex vecsp of dim $n > 0$ and $T\in\Lm{V}$.\TextB{}
Choose a nonzero $v\in V$. $\Par{v,Tv,T^2 v,\dots,T^n v}$ of len $n+1$ is linely depe.\TextB{}
Sup $a_0 I+a_1 T+\dots+a_n T^n=0.$ Then $\exists\,a_j\neq 0.$\TextB{}
{\tgsl Thus $\exists$ nonconst $\,p\,$ of smallest deg ( $\deg p>0$ ) shat $p\Par{T}v = 0$.}\TextB{}
Beca $\exists\,\lambda \in \Cbb$ shat $p\Par{\lambda} = 0\Rightarrow\exists\,q \in\PoCi,p\Par{z} = \Par{z-\lambda}q\Par{z},\forall z \in\Cbb.$\TextB{}
...
Sup $p\in\PoCi$ is a nonzero poly. Then $\deg Tp=\deg p+1,$ and thus $Tp\neq\lambda p,\,\forall\lambda\in\Cbb.$\TextB{}
Hence $T$ has no eigvals.\par
\SepLine

\ProblemN{\hypertarget{5BI13}{13}}{
	\TextNL{Sup $V$ is a complex vecsp and $T\in\Lm{V}$ has no eigvals.}
	\TextNL{Provet every subsp of $V$ invard $T$ is either $\zeroSubs$ or infindim.}
}Sup $U$ is a findim nonzero invarsp on $\Cbb.$ Then by [5.21], $T\mmid_U$ has an eigval.\PfEnd
\SepLine

\ProblemN{\hypertarget{5BI16}{16}}{
	\TextNL{Sup $0\neq v\in V.$ Define $S\in\Lm[\BigPar]{\PoC{\dim V},V}$ by $S\Par{p}=p\Par{T}v.$ Prove $[$ 5.21 $].$}
}\par\quad
Beca $\dim\PoC{\dim V}=\dim V+1.$ Then $S$ is not inje. Hence $\exists\,0\neq p\in\PoC{\dim V},p\Par{T}v=0.$\par\quad
...
Thus at least one of $\Par{T-\lambda_j I}$ is not inje ( beca $p\Par{T}$ is not inje ).\PfEnd
\SepLine
...
Beca $\dim\PoC{\SmallPar{\dim V}{^2}}=\Par{\dim V}{^2}+1.$ Then $S$ is not inje.\par\quad
...
\TextB{Sup $V$ is findim and $T\in\Lm{V}$.}
\TextB{Sup $M_T^0=\Bra{p_j}{_{j\in\Gamma}}$ is the set of all monic poly that give 0 whenever $T$ is applied.}
\TextB{Provet $\exists\,!\,p_k\in M_T^0,\deg p_k=\min\!${\large\envFontDefault$\Bra{\deg p_j}{_{j\in\Gamma}}$}$\leqslant\dim V$.}
...
(ii) Sup $\dim V\geqslant 1.$\par\quad\Hii
{\tgsl Asm $\dim V > 0$ and that the desired result is true for all operators on all vecsps of smaller dim.}\par\quad\Hii
Let $u\in V,u\neq 0$. The list $\Par{u, Tu, \dots, T^{\dim V} u}$ of len $\Par{1 + \dim V}$ is linely depe.\par\quad\Hii
Then $\exists\,!\,T^m$ of smallest deg shat $T^m u\in\Span{u, Tu, \dots, T^{m-1} u}$.\par\quad\Hii
...
Beca $\Par{u, Tu, \dots , T^{m-1} u}$ is linely inde.\par\quad\Hii
...
By {\tgsl asm}, $\exists\,s\in M_T^0$ of smallest deg ( and $\deg s\leqslant\dim W,$ ) so that $s\Par{T\mmid_W } = 0$.\par\quad\Hii
...
Sup $p,q\in M_T^0$ are of the smallest deg. Then $\Par{p-q}\Par{T}=0.$ 又 $\Deg\Par{p-q}=m<${\envFontSmall\normalsize$\min\!\Bra{\deg p_j}{_{j\in\Gamma}}$}.\par\quad
...
\ProblemBnoor{4E 5.31, \hypertarget{5BI4e25}{4E 5.B.25} and \hypertarget{5BI4e26}{26}}{
	\TextB{\large\; min poly of restriction operator and min poly of quotient operator}
	\TextB{Sup $V$ is findim, $T\in \Lm{V}$, and $U$ is an invarsp of $V$ under $T$.}
	\TextB{Let $\,p\,$ be the min poly of $T.$}
	(a) \TextB{Provet $\,p\,$ is a poly multi of the min poly of $T\mmid_U$.}
	(b) \TextB{Provet $\,p\,$ is a poly multi of the min poly of $T\XSlash U$.}
	(c) \TextB{Provet
 ( min poly of $T\mmid_U$ ) $\times$ ( min poly of $T\XSlash U$ ) is a poly multi of $\,p\,$.}
	(d) \TextB{Provet the set of eigvals of $T$ equals}
...
(c) Sup $r$ is the min poly of $T\mmid_U,s$ is the min poly of $T\XSlash U.$\par\quad\Hc
Beca $\forall v\in V,s\Par{T\XSlash U}\Par{v+U}=s\Par{T}v+U=0.$
...
And $\forall u\in U,\Par{rs}\Par{T}u=r\BigPar{s\Par{T}u}=0$ ( beca $s\Par{T}u=s\Par{T\mmid_U}u\in U$ ).\par\quad\Hc
...
\ProblemBnoor{\hypertarget{5BI4e27}{4E 5.B.27}}{
	\TextB{Sup $\Fbb=\Rbb,$ $V$ is findim, and $T\in \Lm{V}$.}
	\TextB{Provet the min poly $\,p\,$ of $T_{\!\Cbb}$ equals the min poly $\,q\,$ of $T$.}
}\par\quad
...
\ProblemBnoor{\hypertarget{5BI4e28}{4E 5.B.28}}{
	\TextB{Sup $V$ is findim and $T\in \Lm{V}$.}
	\TextB{Provet the min poly $\,p\,$ of $T\apostrophe \in\Lm{V\apostrophe}$ equals the min poly $\,q\,$ of $T$.}
}\par\quad
...
\ProblemBnoor{4E 5.32}{
	\TextB{Sup $T\in\Lm{V}$ and $\,p\,$ is the min poly.}
	\TextB{Provet $T$ is not inje $\Longleftrightarrow$ the const term of $\,p\,$ is $0.$}
}\par\quad
...
\Or Beca $p\Par{0}=\Par{z-0}\Par{z-\lambda_1}\cdots\Par{z-\lambda_m}=0\Rightarrow T\Par{T-\lambda_1 I}\cdots\Par{T-\lambda_m I}=0$\par\quad
又 $p$ is the min poly $\Rightarrow q$ define by $q\Par{z}=\Par{z-\lambda_1}\cdots\Par{z-\lambda_m}$ is shat $q\Par{T}\neq 0.$\par\quad
Hence $0=p\Par{T}=Tq\Par{T}\Rightarrow T$ is not inje.\par\quad
Conversely, sup $\Par{T-0I}$ is not inje, then $0$ is a zero of $p$, so that the const term is $0$.\PfEnd
\SepLine

\ProblemBnoor{\hypertarget{5BI4e22}{4E 5.B.22}}{
	\TextB{}
	\TextB{Sup $V$ is findim, $T\in \Lm{V}$. Provet $T$ is inv $\Longleftrightarrow I\in\Span{T, T^2, \dots , T^{\dim V}}.$}
}Denote the min poly by $p,$ where for all $z\in\Fbb,p\Par{z}=a_0 +a_1 z+\dots+z^m.$\par\quad
Notice that $V$ is findim. $T$ is inv $\Longleftrightarrow T$ is inje $\Longleftrightarrow$ $p\Par{0}\neq 0$.\par\quad
Hence $p\Par{T}=0=a_0 I+a_1 T+\dots+T^m,$ where $a_0\neq 0$ and $m\leqslant\dim V.$\PfEnd
\SepLine

\ProblemN{\hypertarget{5BI6}{6}}{
	\TextN{Sup $T\in\Lm{V}$ and $U$ is a subsp of $V$ invard $T$.}
	\TextN{Provet $U$ is invard $p\Par{T}$ for every poly $p\in\PoFi$.}
}\par\quad
...
\ProblemBnoor{\hypertarget{5BI4e10}{4E 5.B.10}, \hypertarget{5BI4e23}{23}}{
	\TextB{Sup $V$ is findim, $T\in \Lm{V}$ and $\,p\,$ is the min poly with deg $\,m.$ Sup $v\in V.$}
	(a) \TextB{Provet $\,\,\Span{v,Tv,\dots,T^{m-1}v}=\Span{v,Tv,\dots,T^{j-1} v}$ for some $j\leqslant m.$}
	(b) \TextB{Provet $\,\,\Span{v,Tv,\dots,T^{m-1}v}=\Span{v, Tv, \dots , T^{m-1} v,\dots,T^n v}.$}
}\par\quad
\Comment\,\,\, By \NOTEFOR [8.40], $j$ has an upper bound $m-1,$ $m$ has an upper bound $\dim V.$\par\quad
Write $p\Par{z}=a_0 +a_1 z+\dots+z^m \,\,\Par{\,m\leqslant \dim V\,}.$ If $v=0,$ then we are done. Sup $v\neq 0.$\par\quad
(a) Sup $j\in\Nbp$ is the smallest shat $T^j v\in\Span{v,Tv,\dots,T^{j-1}v}=U_0.$ Then $j\leqslant m.$\par\quad\Ha
Write $T^j v=c_0 v+c_1 Tv+\dots+c_{j-1}T^{j-1}v.$ And beca $T\Par{T^k v}=T^{k+1}\in U_0.$ $U_0$ is invard $T.$\par\quad\Ha
...
\ProblemBnoor{\hypertarget{5BI4e21}{4E 5.B.21}}{
	\TextB{Sup $V$ is findim and $T\in \Lm{V}$.}
	\TextB{Provet the min poly $\,p\,$ has deg at most $1 + \dim \range T$.}
...
$1$ is the smallest positive integer shat $T^1 v\in\Span{v,\dots,T^0 v}.$ Define $q$ by $q\Par{z}=z\Rightarrow q\Par{T}v=0.$\par\quad
...
\ProblemN{\hypertarget{5BI19}{19}}{
	\TextNL{Sup $V$ is findim, $\dim V>1,T\in\Lm{V}.$ Provet $\Bra{\,p\Par{T}:p\in\PoFi}\neq\Lm{V}.$\vspace{2pt}}
...
\ProblemB{
	\hypertarget{5BI4e19}{}\TextB{Sup $V$ is findim and $T\in \Lm{V}$. Let $\mathcal{E} =\Bra{q\Par{T}: q \in \PoFi}$.}
	\TextB{Provet $\dim \mathcal{E}$ equals the deg of the min poly of $T$.}
}\par\quad
Beca the list $\Par{I,T,\dots,T^{\Par{\dim V}^2}}$ of len $\dim\Lm{V}+1$ is linely depe in $\dim\Lm{V}.$\par\quad
Sup $m\in\Nbp$ is the smallest shat $T^m=a_0 I+\dots+a_{m-1}T^{m-1}.$\par\quad
...
%(ii) $k\geqslant 1,$ asm $T^{m+k-1}\in U,$\par\qquad\qquad\qquad\qquad\qquad\qquad\,\,\,
...
Note that by the min of $m,$ $\Par{I,T,\dots,T^{m-1}}$ is linely inde.\par\quad
...
(a) Sup $p\Par{T}=0.$ 又 $\deg p\leqslant m-1$ $\Rightarrow p=0.$ Then $\varphi$ is inje.\par\quad
...
\ProblemBnoor{\hypertarget{5BI4e13}{4E 5.B.13}}{
	\TextB{Sup $T\in \Lm{V}$ and $q \in \PoFi$ is defined by}
	...
	\TextB{Provet $\exists\,!\,r \in \PoFi$ shat $q\Par{T} = r\Par{T},\deg r<\deg p.$}
}\par\quad
...
Now sup $\deg q\geqslant\deg p.$ We use induction on $\deg q.$\par\quad
(i) $\deg q=\deg p,$ then the desired result is true, as shown above.\par\quad\Endi
(ii) $\deg q>\deg p,$ asm the desired result is true for $\deg q=n.$\par\quad\Hii
Sup $f\in\PoFi$ shat $f\Par{z}=b_0 +b_1 z+\dots+b_n z^n+b_{n+1}z^{n+1}.$\par\quad\Hii
Apply the asm to $g$ defined by $g\Par{z}=b_0 +b_1 z+\dots+b_n z^n,$
...
Apply the asm to $t$ defined by $t\Par{z}=z^n,$\par\quad\Hii\qquad\qquad\qquad\quad getting $\delta$ defined by $\delta\Par{z}=c_0\apostrophe +c_1\apostrophe z+\dots+c_{m-1}\apostrophe z^{m-1}.$\par\quad\Hii
...
\TextB{Sup $V$ is a findim complex vecsp and $T\in\Lm{V}.$}
...
\ProblemB[]{
	...
	\TextB{\tgnr Sup $\alpha_1,\dots,\alpha_n$ are all the disti eigvals of $T,$\vspace{2pt}}
	...
	\TextB{\tgnr ( Beca $\dim V>n$ and $n-1>0,\,\,\,n\Sbra{\dim V-\Par{n-1}}>\dim V$. )\vspace{2pt}}
...
\ProblemN{\hypertarget{5BI10}{10}}{
	\TextNL{Sup $T\in\Lm{V},\lambda$ is an eigval of $T$ with an eigvec $v.$}
	\TextNL{Provet for any $p\in\PoFi,p\Par{T}v=p\Par{\lambda}v.$}
}\par\quad
Sup $p$ is defined by $p\Par{z}=a_0+a_1 z+\dots+a_m z^m$ for all $z\in\Fbb.$ Beca for any $n\in\Nbp,T^n v=\lambda^n v.$\par\quad
Thus $p\Par{T}v=a_0 v+a_1 Tv+\dots+a_m T^m v=a_0 v+a_1 \lambda v+\dots+a_m \lambda^m v=p\Par{\lambda}v.$\PfEnd
\Comment\,\,\, For any $p\in\PoFi$ shat $p\Par{z}=\Par{z-\lambda_1}^{\alpha_1}\cdots\Par{z-\lambda_m}^{\alpha_m},$ the result is true as well.\par\quad
...
Beca $\Par{a+b}^n=a^n+\mathC_n^1 a^{n-1}b+\dots+\mathC_n^k a^{n-k}b^k+\dots+\mathC_n^nb^n.$\par\quad
...
\ProblemN{\hypertarget{5BI1}{1}}{
	\TextN{Sup $T\in\Lm{V}$ and $\exists\,n\in\Nbp$ shat $T^n=0.$}
	\TextN{Provet $\Par{I-T}$ is inv and $\Par{I-T}{^{-1}}=I+T+\dots+T^{n-1}.$}
...
\ProblemN{\hypertarget{5BI2}{2}}{
	\TextN{Sup $T\in\Lm{V}$ and $\Par{T - 2I}\Par{T - 3I}\Par{T - 4I} = 0$.}
	\TextN{Sup $\lambda$ is an eigval of $T$. Provet $\lambda = 2$ or $\lambda = 3$ or $\lambda = 4$.}
}\par\quad
Sup $v$ is an eigvec correspd to $\lambda.$ Then for any $p\in\PoFi,p\Par{T}v=p\Par{\lambda}v.$\par\quad
...
\ProblemNnoor{\hypertarget{5BI7}{7}}{See \hLk{5A22}{5.A.22}}{
	\TextN{Sup $T\in\Lm{V}$. Provet $9$ is an eigval of $T^2$ $\Longleftrightarrow$ $3$ or $-3$ is an eigval of $T$.}
}\par\quad
(a) Sup $\lambda$ is an eigval of $T$ with an eigvec $v.$\par\quad\Ha
Then $\Par{T-3I}\Par{T+3I}v=\Par{\lambda-3}\Par{\lambda+3}v=0\Rightarrow\lambda=\pm 3.$\par\quad
(b) Sup $3$ or $-3$ is an eigval of $T$ with an eigvec $v.$ Then $Tv=\pm 3v\Rightarrow T^2 v=T\Par{Tv}=9v$\PfEnd\vspace{4pt}\quad
...
\ProblemN{\hypertarget{5BI3}{3}}{
	\TextN{Sup $T\in\Lm{V},T^2=I$ and $-1$ is not an eigval of $T.$ Provet $T=I.$}
}\par\quad
...
\ProblemBnoor{\hypertarget{5A4e32}{4E 5.A.32}}{
	\TextB{Sup $T\in\Lm{V}$ has no eigvals and $T^4 = I$. Provet $T^2=-I$.}
}\par\quad
Beca $T^4-I=\Par{T^2-I}\Par{T^2+I}=0$ is not inje $\Rightarrow \Par{T^2-I}$ or $\Par{T^2+I}$ is not inje.\par\quad
...
\ProblemNor{\hypertarget{5BI8}{8}}{4E 5.A.31}{
	\TextN{Give an exa of $T\in\Lm{\Rbb^2}$ shat $T^4=-I$.}
}\par\quad
...
Define $\i^n\in\Lm{\Rbb^2}$ by $\i\Par{x,y}=\XPar{\REAL\BigPar{\i^n x+\i^{n+1} y},\mathfrak{Im}\BigPar{\i^n x+\i^{n+1} y}}.$\vspace{2pt}\par\quad
...
\Or Beca $\Mt{T^4}={}$\small$\begin{pmatrix}
...
\Blind{\Or}We define $T\in\Lm{\Rbb^2}$ shat $\Mt{T}={}${\small$\begin{pmatrix}
...
Beca $n$ and $0$ are all the eigvals of $T,$ 又 $\forall k\in\Bra{1,\dots,n},T e_k=e_1+\dots+e_n;\;T^2 e_k=n \Par{e_1+\dots+e_n}.$\par\quad
...
\ProblemBnoor{\hypertarget{5BI4e8}{4E 5.B.8}}{
	\TextB{Find the min poly of $T.$ Where $T\in\Lm{\Rbb^2}$ is the operator}
...
Thus sup the min poly $p$ is defined by $p\Par{z}=z^2+bz+c$ for all $z\in\Rbb.$\par\vspace{20pt}\quad
Beca\vspace{-35pt}\par\quad
...
\Or Let $\Par{e_1,e_2}$ be the std basis of $\Rbb^2.$ We use the pattern shown in [8.44].\par\quad
Beca $Te_1=\cos\theta\;e_1+\sin\theta\;e_2,\;T^2e_1=\cos2\theta\;e_1+\sin2\theta\;e_2.$\par\vspace{2pt}\quad
...
\ProblemBnoor{\hypertarget{5BI4e11}{4E 5.B.11}}{
	\TextB{Sup $V$ is a two-dim vecsp, $T\in\Lm{V}$, and the matrix of $T$}
	\TextB{with resp to some $B_V$ is {\large$\begin{pmatrix} a & c\\ b & d\end{pmatrix}.$}}
	(a) \TextB{Showt $T^2 - \Par{a + d}T + \Par{ad - bc}I = 0.$}
	(b) \TextB{Showt the min poly of $T$ equals}
...
(a) Sup the basis is $\Par{v,w}$. Beca $\MathLeftBrace{l}{Tv=av+bw\Rightarrow\Par{T-aI}v=bw,$ then apply $\Par{T-dI}$ to both sides. $\\ Tw=cv+dw\Rightarrow \Par{T-dI}w=cv,$ then apply $\Par{T-aI}$ to both sides. $}$\par\vspace{6pt}\quad\Ha
...
Otherwise, by (a), $z^2-\Par{a+d}z+\Par{ad-bc}$ is a poly multi of the min poly.\par\quad\Hb
Now we prove that $T\not\in\Span{I},$ so that then the min poly of $T$ has exactly deg $2.$\par\quad\Hb
( At least one of the asm of (I),(II) below is true. )\par\quad\Hb
(I) Sup $a=d,$ then $Tv=av+bw\not\in\Span{v},Tw=cv+aw\not\in\Span{w}.$\par\qquad
(II) Sup at most one of $b,c$ is not $0.$ If $b=0,$ then $Tw\not\in\Span{w};$ If $c=0,$ then $Tv\not\in\Span{v}.$\PfEnd
\SepLine

\ProblemB{
	\TextB{Sup $S,T\in\Lm{V},S$ is inv, and $p\in\PoFi$. Provet $Sp\Par{TS}=p\Par{ST}S.$}
}\par\quad
...
(ii) If $m>1.$ Asm $S\Par{TS}{^m}=\Par{ST}{^m} S.$\par\quad\Hii
...
\Corollary \,\,\,\ProblemN[]{\hypertarget{5BI5}{5}}{
	Beca $S$ is inv, $T\in\Lm{V}$ is arb $\Longleftrightarrow R=ST$ is arb.\TextN{}
...
\ProblemBnoor{\hypertarget{5BI4e7}{4E 5.B.7}}{
	\TextB{Sup $S, T\in\Lm{V}.$ Let $p,q$ be the min polys of $ST,TS$ respectively.}
	(a) \TextB{If $V=\Fbb^2.$ Give an exa shat $p\neq q;$ \;{\large\tgnr(b)} If $S$ or $T$ is inv. Provet $p=q.$}
}\par\quad
...
(b) Sup $S$ is inv. Beca $p,q$ are monic.\par\quad\Hb
$\MathRightBrace{l}{$
...
\ProblemN{\hypertarget{5BI11}{11}}{
	\TextNL{Sup $\Fbb = \Cbb,$ $T\in\Lm{V}, p\in\PoCi$, and $\alpha\in\Cbb.$}
	\TextNL{Provet $\alpha$ is an eigval of $p\Par{T}$ $\Longleftrightarrow$ $\alpha = p\Par{\lambda}$ for some eigval $\lambda$ of $T$.}
}\par\quad
(a) Sup $\alpha$ is an eigval of $p\Par{T}\Leftrightarrow \BigPar{p\Par{T}-\alpha I}$ is not inje.\par\quad\Ha
...
(b) Sup $\alpha=p\Par{\lambda}$ and $\lambda$ is an eigval of $T$ with an eigvec $v.$ Then $p\Par{T}v=p\Par{\lambda}v=\alpha v.$\PfEnd\vspace{3pt}\quad\Hb
\Or Define $q$ by $q\Par{z}=p\Par{z}-\alpha.$ $\lambda$ is a zero of $q.$\par\quad\Hb
Beca $q\Par{T}v=\BigPar{p\Par{T}-\alpha I}v=q\Par{\lambda}v=\BigPar{p\Par{\lambda}-\alpha}v=0.$\par\quad\Hb
Hence $q\Par{T}$ is not inje $\Rightarrow \BigPar{p\Par{T}-\alpha I}$ is not inje.\PfEnd
\SepLine

\ProblemNor{\hypertarget{5BI12}{12}}{4E.5.B.6}{
	\TextNL{Give an exa of an operator on $\Rbb^2$}
...
While $\nexists\,\lambda\in\Rbb$ shat $-1=p\Par{\lambda}=\lambda^2.$\PfEnd
\SepLine

\ProblemBnoor{\hypertarget{5BI4e17}{4E 5.B.17}}{
	\TextB{Sup $V$ is findim, $T\in \Lm{V},\lambda\in\Fbb$, and $\,p\,$ is the min poly of $T$.}
	\TextB{Showt the min poly of $\Par{T - \lambda I}$ is the poly $q$ defined by $q\Par{z} = p\Par{z + \lambda}.$}
}\par\quad
$q\Par{T-\lambda I}=0\Rightarrow q$ is poly multi of the min poly of $\Par{T-\lambda I}.$\par\quad
Sup the deg of the min poly of $\Par{T-\lambda I}$ is $n,$ and the deg of the min poly of $T$ is $m.$\par\quad
%Write $q(z)=p(z+\lambda)=a_0+a_1(z+\lambda)+\dots+a_{m-1}(z+\lambda)^{m-1}+(z+\lambda)^m.$\par\quad
By definition of min poly,\par\quad
$n$ is the smallest shat $\Par{T-\lambda I}^n\in\Span{I,\Par{T-\lambda I},\dots,\Par{T-\lambda I}^{n-1}};$\par\quad
$m$ is the smallest shat $T^m\in\Span{I,T,\dots,T^{m-1}}.$\par\quad
...
\ProblemBnoor{\hypertarget{5BI4e18}{4E 5.B.18}}{
	\TextB{Sup $V$ is findim, $T\in \Lm{V},\lambda\in\nonzeroFbb$, and $\,p\,$ is the min poly of $T$.}
	\TextB{Showt the min poly of $\lambda T$ is the poly q defined by $q\Par{z} = \lambda^{\deg p} p\Par{\frac{z}{\lambda}}$.}
}\par\quad
$q\Par{\lambda T}=\lambda^{\deg p}p\Par{T}=0\Rightarrow q$ is a poly multi of the min poly of $\lambda T.$\par\quad
Sup the deg of the min poly of $\lambda T$ is $n,$ and the deg of the min poly of $T$ is $m.$\par\quad
By definition of min poly,\par\quad
$n$ is the smallest shat $\Par{\lambda T}^n\in\Span{\lambda I,\lambda T,\dots,\Par{\lambda T}^{n-1}};$\par\quad
$m$ is the smallest shat $T^m\in\Span{I,T,\dots,T^{m-1}}.$\par\quad
...
\ProblemNor{\hypertarget{5BI18}{18}}{\hypertarget{5BI4e15}{4E 5.B.15}}{
	\TextNL{Sup $V$ is a findim complex vecsp with $\dim V > 0$ and $T\in\Lm{V}$.}
	\TextNL{Define $f:\Cbb\rightarrow\Rbb$ by $f\Par{\lambda} = \dim \range\Par{T-\lambda I}$.}
	\TextNL{Provet $f$ is not a continuous function.}
}Note that $V$ is findim.\par\quad
Let $\lambda_0$ be an eigval of $T.$ Then $\Par{T-\lambda_0 I}$ is not surj. Hence $\dim\range\Par{T-\lambda_0 I}<\dim V.$\par\quad
Beca $T$ has finily many eigvals. There exist a sequence of number $\Bra{\lambda_n}$ shat $\lim\limits_{n\rightarrow\infty}\lambda_n=\lambda_0$.\par\quad
...
\ProblemBnoor{\hypertarget{5BI4e9}{4E 5.B.9}}{
	\TextB{Sup $T\in\Lm{V}$ is shat with resp to some basis of $V$,}
...
Let $\Par{v_1,\dots,v_n}$ denote the basis shat $\Mt[\BigPar]{T,\Par{v_1,\dots,v_n}}_{j,k}=A_{j,k}\in\Qbb$ for all $j,k=1,\dots,n$.\par\quad
Denote $\Mt[\BigPar]{v_j,\Par{v_1,\dots,v_n}}$ by $x_j$ for each $v_j.$\par\quad
Sup $p$ is the min poly of $T$ and $p\Par{z}=z^m+\dots+c_1 z+c_0.$ Now we showt each $c_j\in\Qbb.$\par\quad
...
\ProblemBnoor{\OR (\hypertarget{5BI4e16}{4E 5.B.16}), \OR (8.C.18)}[\Sbra]{
	\TextB{Sup $a_0 ,\dots, a_{n-1}\in\Fbb.$ Let $T$ be the operator on $\Fbb^n$ shat\vspace{2pt}}
	\TextB{$\Mt{T}=\,${\normalsize$\begin{pmatrix}
	0 &   &        &  &   & -a_0     \\
	1 & 0 &        &  &   & -a_1     \\
	  & 1 & \ddots &  &   & \vdots   \\
	  &   & \ddots &  & 0 & -a_{n-2} \\
	0 &   &        &  & 1 & -a_{n-1}
\end{pmatrix} $}, with resp to the std basis $\Par{e_1,\dots,e_n}$.\vspace{4pt}}
	\TextB{Showt the min poly of $T$ is $\,p\,$ defined by $p\Par{z}=a_0 + a_1 z + \dots + a_{n-1} z^{n-1} + z^n$.}
	\vspace{-2pt}\TextB{\small $\Mt{T}$ is called the {\tgsc companion matrix} of the poly above. This exercise shows that every monic poly is the min poly of some operator.}
...
}Note that $\Par{e_1,Te_1,\dots,T^{n-1}e_1}$ is linely inde. 又 The deg of min poly is at most $n.$\par\quad
...
\ProblemB{
	...
	\TextB{Sup $\Fbb=\Rbb,$ $V$ is findim, $T\in\Lm{V}$ and $b, c\in\Rbb$ with $b^2 < 4c$.}
...
Sup $\lambda$ is an eigval of $T_R$ with an eigvec $v\in R.$\par\quad
Then $0=\Par{T\mmid_R^2+bT\mmid_R+cI_R}\Par{v}=\Par{\lambda^2+\lambda b+c}v=\BigPar{\Par{\lambda+b}^2+c-\Frac{b^2}{4}}v.$\par\quad
Beca $c-\Frac{b^2}{4}>0$ and we have $v=0.$ Thus $T_R$ has no eigvals.\par\quad
Let $U$ be an invarsp of $R$ that has the largest, even dim among all invarsps.\par\quad
Asm $U\neq R.$ Then $\exists\,w\in R$ but $w\not\in U.$ Let $W$ be shat $\Par{w,T\mmid_R w}$ is a basis of $W.$\par\quad
Beca $T\mmid_R^2 w=-bT\mmid_R w-cw\in W.$ Hence $W$ is an invarsp of dim $2.$\par\quad
Thus $\dim \Par{U+W}=\dim U+2-\Dim\Par{U\cap W},$ where $U\cap W=\zeroSubs,$\par\qquad\qquad
for if not, beca $w\not\in U,T\mmid_R w\in U,$\par\qquad\qquad $U\cap W$ is invard $T\mmid_R$ of one dim ( impossible beca $T\mmid_R$ has no eigvecs ).\par\quad
Hence $U+W$ is even-dim invarsp under $T\mmid_R$, ctradic the max of $\dim U.$\par\quad
Thus the asm was incorrect. Hence $R=\null\Par{T^2+bT+cI}=U$ has even dim.\PfEnd
\SepLine

\ProblemB{
	\textsc{Operators On Odd-Dimensional Vector Spaces Have Eigenvalues}\TextB{}
	(a) \TextB{Sup $\Fbb=\Cbb.$ \tgnr\large Then by [5.21], we are done.}
	(b) \TextB{Sup $\Fbb=\Rbb,$ $V$ is findim, and $\dim V=n$ is an odd number.}
	\Hb\TextB{Let $T\in\Lm{V}$ and the min poly is $\,p\,$. Provet $T$ has an eigval.}
}\par\quad
(i) If $n=1,$ then we are done.\par\quad\Endi
(ii) Sup $n\geqslant 3.$ Asm every operator, on odd-dim vecsps of dim less than $n,$ has an eigval.\par\quad\Hii
...
Now sup $b, c\in\Rbb$ shat $b^2 < 4c$ and $p$ is a poly multi of $x^2 + bx + c$ (see [4.17]).\par\quad\Hii
Then $\exists\,q\in\PoRi$ shat $p\Par{x} = q\Par{x}\Par{x^2 + bx + c}$ for all $x\in\Rbb.$\par\quad\Hii
Now $0 = p\Par{T} = \BigPar{q\Par{T}}\Par{T^2 + bT + cI},$ which means that $q\Par{T}\mmid_{\range\Par{T^2+bT+cI}}=0.$\par\quad\Hii
Beca $\deg q < \deg p$ and $p$ is the min poly of $T$, hence $\range\Par{T^2 + bT + cI}\neq V$.\par\quad\Hii
...
By [5.18], $\range\Par{T^2 + bT + cI}$ is an invarsp of $V$ under $T$ that has odd dim less than $n.$\par\quad\Hii
...
\ProblemBnoor{\hypertarget{5BI2e24}{2E Ch5.24}}{
	\TextB{Sup $\Fbb=\Rbb,T\in\Lm{V}$ has no eigvals.} \TextB{Provet every invarsp of $V$ under $T$ is even-dim.}
}\par\quad
Sup $U$ is such a subsp. Then $T\mmid_U\in\Lm{U}.$
...
\ProblemBnoor{\hypertarget{5BI4e29}{4E 5.B.29}}{
	\TextB{Showt every operator on a findim vecsp of dim $\geq 2$ has a $2$-dim invarsp.}
}\par\quad
Using induction on $\dim V.$\par\quad
(i) $\dim V=2,$ we are done.\par\quad\Endi
(ii) $\dim V>2.$ Asm the desired result is true for vecsp of smaller dim.\par\quad\Hii
Sup $p$ is the min poly of deg $m$ and $p\Par{z}=\Par{z-\lambda_1}\cdots\Par{z-\lambda_m}.$\par\quad\Hii
...
By asm, $T\mmid_{\null q\Par{T}}$ has an invarsp of dim $2.$\PfEnd
\SepLine
...
\ProblemBnoor{\hypertarget{5BII4e2}{4E 5.C.2}}{
	\TextB{Sup $A$ and $B$ are upper-trig matrices of the same size,}
	\TextB{with $\alpha_1 , \dots , \alpha_n$ on the diag of $A$ and $\beta_1 , \dots , \beta_n$ on the diag of $B$.}
	(a) \TextB{Showt $A + B$ is an upper-trig matrix with $\alpha_1 + \beta_1 , \dots , \alpha_n + \beta_n$ on the diag.}
	(b) \TextB{Showt $AB$ is an upper-trig matrix with $\alpha_1 \beta_1 , \dots , \alpha_n \beta_n$ on the diag.}
}\par
\SepLine

\ProblemBnoor{\hypertarget{5BII4e3}{4E 5.C.3}}{
	\TextB{}
	\TextB{Sup $T\in \Lm{V}$ is inv and $B=\Par{v_1 , \dots , v_n}$ is a basis of $V$ shat}
	\TextB{$\Mt{T,B}=A$ is upper trig, with $\lambda_1 , \dots , \lambda_n$ on the diag.}
	\TextB{Showt the matrix of $\Mt{T^{-1},B}=A^{-1}$ is also upper trig, with
{\normalsize$\Frac{1}{\lambda_1},\dots,\Frac{1}{\lambda_n}$} on the diag.}
}\par
\SepLine

\ProblemNnoor{\hypertarget{5BII9}{9}}{\hypertarget{5BII4e7}{4E 5.C.7}}{
	\TextN{Sup $V$ is findim, $T\in \Lm{V}$, and $v \in V$.}
	(a) \TextN{Provet $\exists\,!$ monic poly $p_v$ of smallest deg shat $p_v \Par{T}v = 0$.}
	(b) \TextN{Provet the min poly of $T$ is a poly multi of $p_v$.}
}
\SepLine


\ProblemNor{\hypertarget{5BII14}{14}}{\hypertarget{5BII4e4}{4E 5.C.4}}{
	\TextNL{ Give an operator $T$ shat with resp to some basis,}
...
\ProblemNor{\hypertarget{5BII20}{20}}{\normalsize \OR \hypertarget{5BII4e6}{4E 5.C.6}}{
	\TextNL{}
	\TextNL{Sup $\Fbb=\Cbb,$ $V$ is findim, and $T\in\Lm{V}$.}
	\TextNL{Provet if $k\in\Bra{1,\dots,\dim V},$ then $V$ has a $k$ dim subsp invard $T$.}
}\par
\SepLine

\ProblemBnoor{\hypertarget{5BII4e8}{4E 5.C.8}}{
	\TextB{Sup $V$ is findim, $T\in \Lm{V}$, and $\exists\,v \in V\nonzero$ shat $T^2 v + 2Tv = -2v$.}
	(a) \TextB{Provet if $\Fbb=\Rbb,$ then $\nexists$ a basis of $V$ with resp to which $T$ has an upper-trig matrix.}
	(b) \TextB{Provet if $\Fbb=\Cbb$ and $A$ is an upper-trig matrix that equals the matrix of $T$}
	\Hb\TextB{with resp to some basis of $V$, then $-1 + \i$ or $-1 - \i$ appears on the diag of $A$.}
}\par
\SepLine

\ProblemBnoor{\hypertarget{5BII4e9}{4E 5.C.9}}{
	\TextB{Sup $B\in\Fbb^{n,n}$ with complex ent.}
	\TextB{Provet $\exists$ inv $A\in\Fbb^{n,n}$ with complex ent shat $A^{-1} BA$ is an upper-trig matrix.}
}\par
...
\ProblemBnoor{\hypertarget{5BII4e10}{4E 5.C.10}}{
	\TextB{Sup $T\in \Lm{V}$ and $\Par{v_1,\dots,v_n}$ is a basis of $V$.}
	\TextB{Showt the following are equi.}
...
\ProblemBnoor{\hypertarget{5BII4e11}{4E 5.C.11}}{
	\TextB{Sup $\Fbb=\Cbb$ and $V$ is findim.}
	\TextB{Provet if $T\in \Lm{V}$, then $T$ has a lower-trig matrix with resp to some basis.}
}\par
\SepLine

\ProblemBnoor{\hypertarget{5BII4e12}{4E 5.C.12}}{
	\TextB{}
	\TextB{Sup $V$ is findim, $T\in \Lm{V}$ has an upper-trig matrix with resp to some basis,}
	\TextB{and $U$ is a subsp of $V$ that is invard $T$.}
	(a) \TextB{Provet $T\mmid_U$ has an upper-trig matrix with resp to some basis of $U$.}
	(b) \TextB{Provet $T\XSlash U$ has an upper-trig matrix with resp to some basis of $V\XSlash U$.}
}\par
...
\ProblemBnoor{\hypertarget{5BII4e13}{4E 5.C.13}}{
	\TextB{Sup $V$ is findim, $T\in \Lm{V}$. Sup $U$ is an invarsp of $V$ under $T$}
	\TextB{shat $T\mmid_U$ has an upper-trig matrix and also $T\XSlash U$ has an upper-trig matrix.}
	\TextB{Provet $T$ has an upper-trig matrix.}
}\par
...
\ProblemBnoor{\hypertarget{5BII4e14}{4E 5.C.14}}{
	\TextB{Sup $V$ is findim and $T\in \Lm{V}$.}
	\TextB{Provet $T$ has an upper-trig matrix $\Longleftrightarrow$ $T\apostrophe$ has an upper-trig matrix.}
}\par
...
\ProblemN{1}{
	\TextN{Give an exa of two commuting operators $S, T\in\Fbb^4$ shat}
	\TextN{there is an invarsp of $\Fbb^4$ under $S$ but not under $T$}
	\TextN{and an invarsp of $\Fbb^4$ under $T$ but not under $S$.}
}
\par
\SepLine

\ProblemN{2}{
	\TextN{Sup $\mathcal{E}$ is a subset of $\Lm{V}$ and every ele of $\mathcal{E}$ is diag.}
	\TextN{Provet $\exists$ a basis of $V$ with resp to which}
	\TextN{every ele of $\mathcal{E}$ has a diag matrix $\Longleftrightarrow$ every pair of ele of $\mathcal{E}$ commu.}
	\TextN{{\normalsize This exercise extends [5.76], which considers the case in which $\mathcal{E}$ contains only two ele.}}\hspace{0.5pt}
	\TextN{{\normalsize For this exercise, $\mathcal{E}$ may contain any number of ele, and $\mathcal{E}$ may even be an infini set.}}
}\par
\SepLine

\ProblemN{3}{
	\TextN{Sup $S, T\in\Lm{V}$ are shat $ST = TS$. Sup $p\in\PoFi$.}
	(a) \TextN{Provet $\null p\Par{S}$ is invard $T$.}
	(b) \TextN{Provet $\range p\Par{S}$ is invard $T$.}
	\TextN{See \NOTEFOR [5.17] for the special case $S = T$.}
}\par
\SepLine

\ProblemN{4}{
	\TextN{Prove or give a counterexa\hspace{1pt}$:$}
	\TextN{A diag matrix $A$ and an upper-trig matrix $B$ of the same size commu.}
}\par
\SepLine

\ProblemN{5}{
	\TextN{Provet a pair of operators on a findim vecsp commu $\Longleftrightarrow$ their dual operators commu.}
}\par
\SepLine

\ProblemN{6}{
	\TextN{Sup $V$ is a findim complex vecsp and $S, T\in\Lm{V}$ commu.}
	\TextN{Provet $\exists\,\alpha,\lambda\in\Cbb$ shat $\range\Par{S -\alpha I} + \range\Par{T -\lambda I}\neq V$.}
}\par
\SepLine

\ProblemN{7}{
	\TextN{Sup $V$ is a complex vecsp, $S\in\Lm{V}$ is diag, and $T$ commu with $S$.}
	\TextN{Provet $\exists$ basis $B$ of $V$ shat $S$ has a diag matrix with resp to $B$}
	\TextN{and T has an upper-trig matrix with resp to $B$.}
}\par
\SepLine

\ProblemN{8}{
	\TextN{Sup $m = 3$ in Example [5.72]}
	\TextN{and $D_x , D_y$ are the commuting partial differentiation operators on $\Po_3\Par{\Rbb^2}$ from that exa.}
	\TextN{Find a basis of $\Po_3\Par{\Rbb^2}$ with resp to which $D_x$ and $D_y$ each have an upper-trig matrix.}
}\par
\SepLine

\ProblemN{9}{
	\TextN{Sup $V$ is a findim nonzero complex vecsp.}
	\TextN{Sup that $\mathcal{E}\subseteq\Lm{V}$ is shat $S$ and $T$ commu for all $S, T\in\mathcal{E}$.}
	(a) \TextN{Provet $\exists\,v\in V$ is an eigvec for every ele of $\mathcal{E}$.}
	(b) \TextN{Provet $\exists$ a basis of $V$ with resp to which every ele of $\mathcal{E}$ has an upper-trig matrix.}
}\par
\SepLine

\ProblemN{10}{
	\TextNL{Give an exa of two commuting operators $S, T$ on a findim real vecsp shat}
