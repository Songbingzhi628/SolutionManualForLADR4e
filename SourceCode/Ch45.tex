...
Becs by (4E 5.A.39), $\exists\,\alpha$ eigval of $T\Rightarrow\exists$ invarsp $U$ of dim $\dim V-1.$\parSol{}
又 $T\mmid_U\in\Lm{U},$ $V=\Null\Par{T-\lambda I}\oplus\Range\Par{T-\lambda I}.$ Note by Exe (3E 4),\parSol{}
$\Null\Par{T\mmid_U-\lambda I}\cap\Range\Par{T\mmid_U-\lambda I}=\zeroSubs\Rightarrow U=\Null\Par{T\mmid_U-\lambda I}\oplus\Range\Par{T\mmid_U-\lambda I}.$\parSol{}
By asum, $T\mmid_U$ is diag wrto $B_U=\Par{v_2,\dots,v_n}.$ Let $v_1$ be an eigvec for $\alpha.$ Forming $B_V$ of eigvecs.\PfEnd\vspace{2pt}\parSol{}
\Or $\exists$ eigval $\lambda_0\Rightarrow U=\Range\Par{T-\lambda_0I}$ invard $T\Rightarrow U=\Null\Par{T\mmid_U-\lambda I}\oplus\Range\Par{T\mmid_U-\lambda I}.$\parSol{}
While $V=E\Par{\lambda_0,T}\oplus U\Rightarrow\dim U<\dim V.$ By asum, $T\mmid_U$ is diag wrto $B_U$ of eigvecs. Simlr.\PfEnd\vspace{4pt}\par\quad
...
Thus all $\mu$'s are dist. Let $s\Par{z}=\Par{z^k-\lambda_1}\cdots\Par{z^k-\lambda_m}=\prod_{j=1}^m\prod_{i=1}^k\Par{z-\mu_{i,j}}\Rightarrow s\Par{T}=0.$\PfEnd
...
}Let each $U_k=U\cap E\Par{\lambda_k,T}\Rightarrow U_1\oplus\dots\oplus U_m\subseteq U.$\par\quad
$\forall u\in U,\exists\,!\,v_k\in E\Par{\lambda_j,T},\,u=v_1+\dots+v_m.$ By \Sbra{5.A \TIPSN{1}}, each $v_k\in U_j\Rightarrow u\in U_1\oplus\cdots\oplus U_m.$\PfEnd
\SepLine
