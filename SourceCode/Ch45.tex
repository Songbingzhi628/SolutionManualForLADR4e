...
\ProblemBnoor[]{\Tips \,\,\,}[]{
	\TextB{\tgsl\large\envFontDefault Supp $p\in\PoFi,\deg p\leqslant m$ and $p$ has at least $\Par{m+1}$ disti zeros.}
...
\ProblemBnoor{\hypertarget{4O4e3}{4E 4.3}}{
	\TextB{Supp $\Fbb=\Cbb,\varphi\in V\apostrophe.$ Define $σ:V\rightarrow\Rbb$ by $\sigma\Par{v} = \Real\varphi\Par{v}$ for each $v\in V.$\vspace{4pt}}
...
\ProblemN{\hypertarget{4O4}{4}}{
	\TextN{Supp $m,n\in\Nbp$ with $m\leqslant n,\;\lambda_1,\dots,\lambda_m\in\Fbb.$}
...
\ProblemN{\hypertarget{4O9}{9}}{
	\TextN{Supp $p\in\PoCi$. Define $q:\Cbb\rightarrow\Cbb\,$ by $q\Par{z} = p\Par{z}\overline{p\Par{\overline z}}$. Prove $q\in\PoRi$.}
}\par\quad
...
\ProblemN{\hypertarget{4O10}{10}}{
	\TextNL{Supp $m\in\Nbb,p\in\PoC{m}$ is suth $p\Par{x_k}\in\Rbb$ for each of disti $x_0,x_1,\dots,x_m\in\Rbb.$}
...
\ProblemBnoor{\hypertarget{4O4e13}{4E 4 13}}{
	\TextB{Supp nonconst $p,q\in\PoCi$ have no common zeros. Let $m = \deg p$, $n = \deg q$.\vspace{3pt}}
...
(i) $k=1.$ $\Par{pq}{^{\SmallPar{1}}}=\Par{pq}\apostrophe=C_{1}^1 p{^{\SmallPar{1}}}q{^{\SmallPar{0}}}+C_{1}^0 p{^{\SmallPar{0}}}q{^{\SmallPar{1}}}.$\par\quad\Endi
...
\ProblemN{L2}{
	\TextB{Supp $p\Par{z}=\Par{z-\lambda}{^\alpha}q\Par{z}$ and $\alpha\in\Nbp.$ Prove ${p{^{\SmallPar{\alpha-1}}}}\Par{\lambda}=0.$}
}\par\quad
...
(ii) $2\leqslant\alpha\leqslant A.$ Asum $p{^{\SmallPar{a-2}}}\Par{\lambda}=0$ for all $a\in\Bra{2,\dots,\alpha}.$\par\quad\Hii
\NOTICE that $p\Par{z}=\Par{z-\lambda}{^{\alpha-1}}q_{\alpha-1}\Par{z}=\Par{z-\lambda}{^{\alpha}}q_{\alpha}\Par{z},$ where $q_{\alpha-1}\Par{z}=\Par{z-\lambda}q_{\alpha}\Par{z}.$\vspace{2pt}\par\quad\Hii
