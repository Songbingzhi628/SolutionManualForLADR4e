...
\ProblemN{\Anchor{5A15}{15}}{
	...
	\PrePb\TextA{Describe the relationship between the eigvecs of $T$ and the eigvecs of $S^{-1}TS.$}
...
\Anchor{5BI4e25}\Anchor{5BI4e26}\ProblemBnoor{4E 5.31, 4E 25, 26}{
	...
	\PrePa\TextB{Prove $p$ is a multi of the min of $T\mmid_U$. \; {\tgnr\large(b)} of the min of $T\XSlash U$.}
	\PrePc\TextB{Prove \Sbra{min poly of $T\mmid_U$}${}\times{}$\Sbra{min poly of $T\XSlash U$} is a poly multi of $p$.}
	\PrePd\TextB{Prove the set of eigvals of $T$ is the set of eigvals of $T\mmid_U$ or of $T\XSlash U$.\hfill\FontSmall\tgnr By [8.49], immed.}
...
}(a) $\forall\varphi\in V\apostrophe,\,p\Par{T\apostrophe}\Par{\varphi}=\varphi\BigPar{p\Par{T}}=0\Rightarrow p\Par{T}\in\null\varphi.$ Thus $p\Par{T}=0.$\parSol{}
(b) $q\Par{T}=0\Rightarrow\forall \varphi\in V\apostrophe,\varphi\BigPar{q\Par{T}}=q\Par{T\apostrophe}\Par{\varphi}=0.$ Thus $q\Par{T\apostrophe}=0.$\PfEnd
\SepLine
...
}If $T$ inje, then $\range T=V$ and done. Supp non0 $v\in\null T.$ Then $Tv=0.$\parSol{}
Define $q\Par{z}=z\Rightarrow q\Par{T}v=0.$ \Sbra{Simlr to [4E 5.22].} Let $W=\rangep q\Par{T}=\range T.$\parSol{}
$\exists$ monic $s\in\PoF{\dim W},\,s\Par{T\mmid_W}=0.$ Thus $p=sq$ with $\deg p=\deg s+\deg q\leqslant\dim W+1.$\PfEnd
\SepLine
...
}Becs $T^4-I=\Par{T^2-I}\Par{T^2+I}=0$ not inje, so is $\Par{T^2-I}$ or $\Par{T^2+I}.$\parSol{}
又 $T$ has no eigvals $\Rightarrow\Par{T-I},\Par{I+T}$ inje, so is $\Par{T^2-I}.$\parSol{}
Thus $\forall v\in V,\,0=\Par{T^2-I}\Par{T^2+I}v\Longleftrightarrow 0=\Par{T^2+I}v.$\PfEnd\vspace{4pt}\parSol{}
...
Define $\i\in\Lm{\Rbb^2}$ by $\i\Par{x,y}=\Par{-y,x}.$ \;Simlr to $\i\in\Lm{\Cbb}$ defined by $\i\Par{x+\i y}=-y+\i x.$\vspace{2pt}\par\quad
Define $\i^n\in\Lm{\Rbb^2}$ by $\i\Par{x,y}=\XPar{\REAL\BigPar{\i^n x+\i^{n+1} y},\,\IMAGINARY\BigPar{\i^n x+\i^{n+1} y}}.$\vspace{0pt}\par\quad
$\displaystyle T^4+I=\BigPar{T^2+\i I}\BigPar{T^2-\i I}=\BigPar{T+\i^{1/2}I}\BigPar{T-\i^{1/2}I}\BigPar{T-\Par{{-\i}}{^{1/2}}I}\BigPar{T+\Par{{-\i}}{^{1/2}}I}.$\vspace{2pt}\par\quad
Note that $\i^{1/2}=\frac{\sqrt{2}}{2}+\i\frac{\sqrt{2}}{2},\;\Par[1pt]{{-\i}}{^{1/2}}=\frac{\sqrt{2}}{2}-\i\frac{\sqrt{2}}{2}.$ \;Hence $T=\pm\Par[1pt]{{\pm\i}}{^{1/2}}I.$\vspace{4pt}\par\quad
...
\end{pmatrix}$\large. \;Using {\small$\begin{pmatrix}\Blind{-}\cos\alpha & \sin\alpha\\-\sin\alpha & \cos\alpha\end{pmatrix}$}$\begin{array}{l}\hspace{-5pt}{}_n\\\\\vspace{-6pt}\end{array}\hspace{-8pt}={}${\small$\begin{pmatrix}\Blind{-}\cos n\alpha & \sin n\alpha\\-\sin n\alpha & \cos n\alpha\end{pmatrix}$}.\PfEnd%\par\vspace{-6pt}\quad
...
$Tw=cv+dw\Rightarrow \Par{T-dI}w=cv\Rightarrow\Par{T-aI}\Par{T-dI}w=cTv-acv=bcw.$\vspace{4pt}\parSol{}
(b) If $b=c=0$ and $a=d.$ Then $\Mt{T}=a\Mt{I}\Rightarrow T=aI.$\parSol{\Hb}
Othws, we show $T\not\in\Span{I},$ so that $\deg p=2=\deg q.$\parSol{\Hb}
Let (1) $a=d,$ (2) $b=0,$ (3) $c=0.$ Then (1), (2) and (3) cannot be all true.\parSol{\Hb}
(I) Asum (1) is true, with (2) or (3) not true.\parSol{\Hb\HI}
Then $Tv=av+bw\not\in\Span{v},$ or $Tw=cv+aw\not\in\Span{w}.$\parSol{\Hb}
(II) Asum (2) or (3) are true, with (1) not true.\parSol{\Hb\HII}
Then $Tv=av+bw\not\in\Span{w},$ or $Tw=cv+dw\not\in\Span{w}.$\PfEnd
\SepLine

\ProblemB{
	\TextB{Supp $S,T\in\Lm{V},S$ is inv, and $p\in\PoFi$. Prove $Sp\Par{TS}=p\Par{ST}S.$}
}We prove each $S\Par{TS}{^m}=\Par{ST}{^m} S$ by induc.\parSol{}
(i) $m=0,1.$ $S\Par{TS}{^0}=SI=IS=\Par{ST}{^0}S;\;\,S\Par{TS}{^1}=\Par{ST}{^1}S.$\parSol{\Endi}
...
\Anchor{5BI4e7}\ProblemBnoor{4E 7}{
	\TextB{Supp $S, T\in\Lm{V}$ with the mins $p,q$ respectly. Supp $S$ or $T$ is inv. Prove $p=q.$}
...
(a) Supp $\alpha$ is eigval of $p\Par{T}\Longleftrightarrow\Sbra{p\Par{T}-\alpha I}$ not inje.\par\quad\Ha
Let $q\Par{z}=p\Par{z}-\alpha=c\Par{z-\lambda_1}\cdots\Par{z-\lambda_m}\Rightarrow\exists\,\Par{T-\lambda_j}$ not inje. Thus $p\Par{\lambda_j}-\alpha=0.$\par\vspace{4pt}\quad
(b) Supp $\alpha=p\Par{\lambda}$ and $\lambda$ is eigval of $T$ with $v.$ Then $p\Par{T}v=p\Par{\lambda}v=\alpha v.$\PfEnd\vspace{2pt}\quad\Hb
\Or Define $q$ by $q\Par{z}=p\Par{z}-\alpha.$ Let $\lambda$ be a zero of $q.$\par\quad\Hb
...
}Becs $q\Par{T-\lambda I}=p\Par{T}=0\Rightarrow q$ is multi of $s\Rightarrow\deg q=\deg p\geqslant\deg s.$\parSol{}
Becs $\deg s=n$ is the smallest suth $\Par{T-\lambda I}{^n}\in\Span[\BigPar]{I,\Par{T-\lambda I},\dots,\Par{T-\lambda I}{^{n-1}}};$\parSol{}
And $\deg p=m$ is the smallest suth $T^m\in\Span{I,T,\dots,T^{m-1}}.$\parSol{}
又 $T^k\in\Span{I,T,\dots,T^{k-1}}\Longleftrightarrow\Par{T-\lambda}{^k}\in\Span[\BigPar]{I,\Par{T-\lambda I},\dots,\Par{T-\lambda I}{^{k-1}}}.$\parSol{}
Thus $\deg s=\deg p.$ By the uniqnes.\PfEnd
\SepLine
...
Becs $\deg s=n$ is the smallest suth $\Par{\lambda T}^n\in\Span[\BigPar]{\lambda I,\lambda T,\dots,\Par{\lambda T}{^{n-1}}};$\parSol{}
And $\deg p=m$ is the smallest suth $T^m\in\Span{I,T,\dots,T^{m-1}}.$\parSol{}
又 $\Par{\lambda T}{^k}\in\Span[\BigPar]{\lambda I,\lambda T,\dots,\Par{\lambda T}{^{k-1}}}\Longleftrightarrow T^k\in\Span{I,T\dots,T^{k-1}}.$\parSol{}
Thus $\deg s=\deg p.$ By the uniqnes.\PfEnd
\SepLine
...
\Anchor{5BII4e2}\ProblemBnoor{{4E 5.C.2}}{
	...
	\PrePa\TextB{Show $A + B$ up-trig with $\alpha_1 + \beta_1 , \dots , \alpha_n + \beta_n$ on the diag.}
	\PrePb\TextB{Show $AB$ up-trig with $\alpha_1 \beta_1 , \dots , \alpha_n \beta_n$ on the diag.}
}
\SepLine
...
\Anchor{5BII9}\Anchor{5BII4e7}\ProblemNnoor{9}{4E 5.C.7}{
	...
	\PrePb\TextA{Prove the min poly of $T$ is a poly multi of $p_v$.}
}
\SepLine
...
\Anchor{5BII20}\Anchor{5BII4e6}\ProblemNor{20}{4E 5.C.6}{
	...
	\TextA{Prove $V$ has a $k$\hspace{1pt}-\hspace{1pt}dim invarspd $T.$}
}\par
\SepLine

\Anchor{5BII4e8}\ProblemBnoor{4E 5.C.8}{
	\TextB{Supp $V$ is finide, and $\exists\,v\in V\nonzero$ suth $T^2 v + 2Tv = -2v$.}
	\PrePa\TextB{Supp $\Fbb=\Rbb.$ Prove $\nexists\,B_V$ suth $\Mt{T}$ up-trig.}
	\PrePb\TextB{Supp $\Fbb=\Cbb,$ and $\exists\,B_V$ suth $A=\Mt{T}$ up-trig. Prove $-1+\i$ or $-1-\i$ on the diag of $A$.}
}\par
\SepLine
...
\Anchor{5BII4e10}\ProblemBnoor{4E 5.C.10}{
	...
	\PrePa\TextB{$\Mt{T,B_V}$ lower trig. \, {\tgnr\large(b)} Each $\Span{v_k,\dots,v_n}$ invard $T.$ \, {\tgnr\large(c)} Each $Tv_k\in\Span{v_k,\dots,v_n}.$}
}\par
\SepLine
...
\Anchor{5BII4e12}\ProblemBnoor{4E 5.C.12}{
	...
	\PrePa\TextB{Prove $\Mt{T\mmid_U}$ up-trig for some $B_U$. \, {\tgnr\large(b)} Prove $\Mt{T\XSlash U}$ up-trig for some $B_{V\XSlash U}.$}
}
\SepLine
