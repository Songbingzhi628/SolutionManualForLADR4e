...
\ProblemB[]{
	\TextB{Supp non0 $v\in V.$ Prove [5.21] using $S$ below.}
}
...
\Anchor{5BI4e25}\Anchor{5BI4e26}\ProblemBnoor{4E 5.31, 4E 25, 26}{
	...
	(a) \TextB{Prove $p$ is a multi of the min of $T\mmid_U$.}
	(b) \TextB{Prove $p$ is a multi of the min of $T\XSlash U$.}
...
\ProblemBnoor{4E 5.32}{
	\TextB{Supp $T\in\Lm{V}$ and $p$ is the min. Prove $T$ is not inje $\Longleftrightarrow$ the const term of $p$ is $0.$}
...
\Anchor{5BI4e19}\ProblemB{
	\TextB{Supp $V$ is finide and $T\in \Lm{V}$. Let $\mathcal{E} =\Bra{q\Par{T}: q \in \PoFi}$.}
...
\BulletPointX\NoteForSmall{[8.49]}\;\;Supp $V$ is a finide complex vecsp, $T\in\Lm{V},$ and $p$ is the min.\TextB{}
Let $p\Par{z}=\Par{z-\lambda_1}\cdots\Par{z-\lambda_m},$ where $\lambda_1,\dots,\lambda_m$ are all the eigvals of $T$, possibly with repeti.\par
\AComm {\FontSmall\tgsl A non0 poly has at most as many disti zeros as its deg.}\vspace{-4pt}\parCom
{\FontSmall\tgsl Thus by the upper bound for the deg of min poly given in {\NOTEFOR} [8.40], and by [8.49],}\vspace{-4pt}\parCom
{\FontSmall\tgsl we can {\tgsc give an alternative proof of [5.13]}.}\par\Anchor{5BI4e20}\Anchor{5BI4e24}
\ACoro Supp $\alpha_1,\dots,\alpha_n$ are all the disti eigvals of $T.$\parCor
\ANote the min of $T$ is a multi of, but not equal to, $\Par{z-\alpha_1}\cdots\Par{z-\alpha_n}.$\parCor
Let $q\Par{z}=\Par{z-\alpha_1}{^{\dim V-\SmallPar{n-1}}}\cdots\Par{z-\alpha_n}{^{\dim V-\SmallPar{n-1}}}.$ Then $q$ is a multi of the ch.\parCor
Becs $\dim V>n$ and $n-1\geqslant 0,\,\,\,n\Sbra{\dim V-\Par{n-1}}>\dim V$.\parCor
Form for ch: $\Par{z-\alpha_1}{^{\gamma_1}}\cdots\Par{z-\alpha_n}{^{\gamma_n}},$ where $\gamma_1+\dots+\gamma_n=\dim V.$\parCor
...
\ProblemN{\Anchor{5BI10}{10}}{
	\TextA{Supp $T\in\Lm{V},\lambda$ is an eigval of $T$ with an eigvec $v.$}
	\TextA{Prove for any $p\in\PoFi,p\Par{T}v=p\Par{\lambda}v.$}
}\par\quad
...
\AComm For any $p\in\PoFi$ suth $p\Par{z}=\Par{z-\lambda_1}^{\alpha_1}\cdots\Par{z-\lambda_m}^{\alpha_m},$ the result is true as well.\par\quad
