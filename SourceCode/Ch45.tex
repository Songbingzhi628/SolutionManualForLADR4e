...
\Anchor{5BIT1}\ProblemBX{\TipsN{1}}{
	\TextB{Supp $T\in\Lm{V}$ with invarsp $U.$ Prove if $p\in\PoFi,$ then $\pi p\Par{T}=p\Par{T\XSlash U}\pi.$}
}For each $k\in\Nbb,\:\pi\Par{T^kv}=T^kv+U=\Par{T\XSlash U}{^k}\Par{v+U}.$\PfEnd
\SepLine

\Anchor{5BIT2}\ProblemBX{\TipsN{2}}{
	\TextB{Supp $T\in\Lm{V},p\in\PoFi{}.$ Prove $p\Par{T\apostrophe}\varphi=\varphi\circ p\Par{T}.$}
}Each $\Par{T\apostrophe}{^k}\Par{\varphi}=\Par{T\apostrophe}{^{k-1}}\Par{\varphi\circ T}=\dots=\varphi\circ T^k.$\PfEnd
\SepLine

\Anchor{5BIT3}\ProblemBX{\TipsN{3}}{
