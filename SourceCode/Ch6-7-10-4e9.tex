...
}By Exe (6), $\Dvertsq{u}\leqslant\Dvertsq{u}+\aMidsq{a}\:\!\Dvertsq{v}=\Dvertsq{u+a\:\!v}$ for all $a\in\Fbb.$\PfEnd\parSol{}
\Or Becs $\REAL\BigPar{x\overline{y}\,\Ang{u,v}}=0.$ Take $\Par{x,y}=\Par{1,1}$ and $\Par{\i,1}.$\PfEnd
\SepLine
...
}By (8.B.10), $T=D+N.$ \,Supp $N^p=0\neq N^{p-1}.$ Supp $T$ up-trig wrto orthon $B_V=\Par{e_1,\dots,e_n}.$\vspace{1pt}\parSol{}
Becs $\Par{D+N}{^{2p+q}}=D^{2p+q}+\dots+D^{2p+q-k}N^k+\dots+D^{p+q+1}N^{p-1}=D^{p+q+1}\Par{D+N}{^{p-1}},\;q\in\Nbb.$\vspace{1pt}\parSol{}
