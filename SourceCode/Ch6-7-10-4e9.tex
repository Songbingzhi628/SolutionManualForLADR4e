...
\ChDecl{Ch7E}{7.E [4E] \& 7.F [4E]}{\quad{\ANote {\FontSmall $V,W$ are finide non0 vecsps over $\Fbb.$}}}
...
\Or Let $\lambda_1,\dots,\lambda_m$ be disti eigvals of $T$ with $\lambda_1=0$ if any. Let $U=\Par{\null T}{^\perp}.$\par\quad
...
Supp $\Dvert{Tv}=\Dvert{T}\,\Dvert{v}.$ Then $\BigDvertsq{T^*Tv-\Dvertsq{T}v}=\Dvertsq{T^*Tv}+\Dvert{T}{^4}\Dvertsq{v}-2\Real\BigAng{T^*Tv,\:\!\Dvertsq{T}v}.$\par\quad
...
%Becs $s_n=\min\!\Bra{\Dvert{S-T}:T\in\Lm{V}\text{ and }\dim\range T=n-1}>\Dvert{S-T}$\PfEnd
%\SepLine
...
Let $v\in V$ suth $\BigDvertsq{\sqrt{S}\,v}=\BigDvertsq{\sqrt{S}\;\!}=\Dvert{S}\Rightarrow\Dvert{S+T}\geqslant\Dvert{S}.$ \;Simlr, $\Dvert{S+T}\geqslant\Dvert{T}.$\vspace{2pt}\parSol{}
We show $\Dvert{S-T}\leqslant\Dvert{S},$ so that apply rev, $\Dvert{S-T}\leqslant\Dvert{T}.$\parSol{}
Let $B_V=\Par{e_1,\dots,e_n}$ be orthon eigvecs of $S$ with corres eigvals $\lambda_1,\dots,\lambda_n.$\parSol{}
Let $A=\Mt{T,B_V}.$ Then $\forall v\in V$ with $\Dvert{v}=1,\;\Dvert{Sv-Tv}\leqslant\max\!\Bra{\Dvertsq{A_{\cdot,k}-\lambda_ku_k}:1\leqslant k\leqslant n}.$
\SepLine

\Anchor{7F14}\ProblemN{F.14}{
	\TextA{Supp $U,W$ subsps of $V$ suth $\Dvert{P_{\!U}-P_{\!W}}<1.$ Prove $\dim U=\dim W.$}
}Get orthon $B_U=\Par{u_1,\dots,u_m},B_{U^\perp}=\Par{x_1,\dots,x_n}.$\parSol{}
Then $1=s_m=\min\!\Bra{\Dvert{P_{\!U}-T}:T\in\Lm{V}\text{ and }\dim\range T=0,1,\dots,m-1}.$\parSol{}
Thus $\dim\range P_{\!W}\geqslant\dim\range P_{\!U}.$ \;Apply revly, done.\PfEnd
\SepLine
...
\Anchor{7F19}\ProblemN{F.19}{
	\TextA{Prove $\Dvert{T^*T}=\Dvertsq{T}$ for $T\in\Lm{V,W}.$}
}$\forall v\in V,\:\BigDvertsq{\sqrt{T^*T}v}=\Ang{T^*Tv,v}=\Dvertsq{Tv}\leqslant\Dvertsq{T}\:\Dvertsq{v}\Rightarrow\BigDvert{\sqrt{T^*T}\;\!}\leqslant\Dvertsq{T}.$\parSol{}
Let $T=S\sqrt{T^*T}\Rightarrow\Dvert{T}\leqslant\Dvert{S}\,\BigDvert{\sqrt{T^*T}\;\!}=\BigDvert{\sqrt{T^*T}\;\!}.$ \quad\Or By Exe (2), $\Dvert{T^*T}=s_1^2.$\PfEnd
\SepLine
...
Note that $\Par{T^*T}{_\Cbb}$ and $T^*T$ have the same eigvals
...
Then $T_{\!\Cbb}^kv=\Dvert{T}{^k}v\Rightarrow\Dvert{T}{^k}=\BigDvert{T_{\!\Cbb}^kv}\leqslant\Dvert{T_{\!\Cbb}^k}.$ \,By Exe (5).\PfEnd
\SepLine
...
Supp $U$ a $k$\hspace{1pt}-\hspace{1pt}dim subsp of $V.$ Becs $\Dvert{T\mmid_U}=\BigDvert{T\mmid_UP_{\!U}}=\BigDvert{T-T\mmid_{U^\perp}P_{\!U^\perp}}\geqslant s_{n-k+1}.$\PfEnd\vspace{3pt}\parSol{}
...
%\Or %Supp $s_1,\dots,s_m$ are the posi. 
%$\forall u\in U,\;s_q^2\,\Dvertsq{u}\leqslant\Dvertsq{Tu}=s_1^2\,\innerAsq{u,\:\!e_1}+\dots+s_n^2\,\innerAsq{u,\:\!e_n}\leqslant s_p^2\,\Dvertsq{u},$\parSol{}
%where $p$ is the smallest suth $\Ang{u,\:\!e_p}\neq0$ and $q$ is the largest suth $\Ang{u,\:\!e_q}\neq0.$\parSol{}
%Note that $\exists\,u\not\in\Span{e_n,\dots,e_{n-k}}\Rightarrow p\leqslant n-k+1.$ \;And $\exists\,u\not\in\Span{e_1,\dots,e_{k-1}}\Rightarrow q\geqslant k.$
\SepLine
...
\Anchor{7F28}\ProblemN{F.28}{
	\TextA{Supp $T\in\Lm{V}.$ Prove $\exists$ unit $S\in\Lm{V},\,T=\sqrt{TT^*}\,S.$}
}Let $T^*=S\sqrt{TT^*}\Rightarrow T=\sqrt{TT^*}\,S^*.$\PfEnd\vspace{2pt}\parSol{}
\Or Supp $s_1,\dots,s_m$ are the positive singvals of $T$ with SVD $\Par{e_1,\dots,e_m},\Par{\,f_1,\dots,f_m}.$\vspace{1pt}\parSol{}
...
\Anchor{7F31}\ProblemN{F.31}{
	\TextA{Supp $T\in\Lm{V}$ self-adj. Or supp $\Fbb=\Cbb$ and $T\in\Lm{V}$ normal.}
	\TextA{Prove $\exists$ unit $S$ with $T=S\sqrt{T^*T}$ suth $S,\sqrt{T^*T}$ diag wrto same orthon bss.}
}Let $B_V=\Par{e_1,\dots,e_n}$ be orthon eigvecs of $T$ with corres $\lambda_1,\dots,\lambda_n.$\parSol{}
Note that $T^*e_k=\overline{\lambda_k}e_k\Rightarrow T^*Te_k=\aMidsq{\lambda_k}e_k.$ \;Define $S\in\Lm{V}$ by $Se_k=\aMid{\lambda_k}{^{-1}}\lambda_k\:\!e_k.$\PfEnd\vspace{2pt}\parSol{}
\Or Becs $T=S\sqrt{T^*T}=\sqrt{TT^*}\,S=\sqrt{T^*T}\,S.$\PfEnd
\SepLine
