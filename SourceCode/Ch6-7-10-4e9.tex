...
\ChDecl{Ch10}{10}{\quad{\ANote {\FontSmall $V$ denotes a finide non0 vecsp over $\Fbb.$}}}%An Exe marked by $\blacksquare$ is true if infinide or partially finide.

\vspace{4pt}

\Anchor{10A17}\ProblemN{A.17}{
...
\Anchor{10A4e10}\ProblemB{
	\TextB{Supp $\tau:\Lm{V}\rightarrow\Fbb,\tau\Par{I}=\dim V,$ and $\forall S,T\in\Lm{V},\tau\Par{ST}=\tau\Par{TS}.$ Prove $\tau=\Tr.$}
...
\ChDecl{Ch4e9A}{[4E] 9.A}{\quad{\ANote {\FontSmall $V,W$ denote finide non0 vecsps over $\Fbb.$}}}%An Exe marked by $\blacksquare$ is true if infinide or partially finide.

\vspace{6pt}

\Anchor{4e9A2}\ProblemN{2}{
	\TextA{Let $n=\dim V.$ Supp $\beta$ is biliney on $V.$ Prove $\exists\,\varphi_k,\tau_k\in V\apostrophe,\,\beta\Par{u,v}=\sum_{k=1}^n\varphi_k\Par{u}\;\!\tau_k\Par{v}.$}
...
Now $\forall u,v\in V,\,\beta\Par{u,v}=\beta\BigPar{u,\sum a_iv_i}=\sum a_i\,\beta\Par{u,v_i}=\sum a_i\:\!\varphi_i\Par{u}.$ Get the dual bss of $B_V.$\PfEnd\vspace{2pt}\parSol{}
\Or $\forall u\in V,\,V\apostrophe\ni\varphi_u:\:\!v\mapsto\beta\Par{u,v}.$ \,Define such $\varphi_1,\dots,\varphi_n$ for a $B_V=\Par{u_1,\dots,u_n}.$\parSol{}
Now $\forall u,v\in V,\,\beta\Par{u,v}=\beta\BigPar{\sum a_iu_i,v}=\sum a_i\,\beta\Par{u_i,v}=\sum a_i\:\!\varphi_i\Par{v}.$
\SepLine

\Anchor{4e9A4}\ProblemN{4}{
...
\ChDecl{Ch4e9B}{[4E] 9.B}{\quad{\ANote {\FontSmall $V,W$ denote finide non0 vecsps over $\Fbb.$}}}%An Exe marked by $\blacksquare$ is true if infinide or partially finide.

\vspace{6pt}

\Anchor{4e9B}\ProblemN{}{
	\TextA{}
}
\SepLine
...
% 遗憾的是，由于微积分的知识门槛，我放弃了第10章B节最后一小部分和相应习题（这部分在4E中被删掉了，所以我可以为我放弃那部分内容借口说它是3E不完善的地方）。
% 少了关于无限维内积空间及其算子的内容，LADR的后半节学起来明显更快更平常，没有前半部分许多题目去掉有限维假设后的悬疑和激动时刻。
% 但到了4E第九章时，情况则大有不同。