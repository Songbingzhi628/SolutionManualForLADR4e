...
%	\TextA{Supp $1<n\in\Nbb.$ Show $\exists\,A\in\FbbP{n,n}$ with all ents positive numbers and $A=\overline{A}{^t},$}
...
\Anchor{4e9DNE6}\ProblemBX[]{\NoteForSmall{Exe (D.6)}}{
	Let $R_r=\Bra{v_1\otimes w_1+\dots+v_r\otimes w_r:\Par{v_1,\dots,v_r,w_1,\dots,w_r}\in V^r\times W^r}.$\vspace{1pt}\TextB{}
	Let $V=\Fbb^m,W=\Fbb^n.$ We show $R_r$ can be id with $M_r=\Bra{A\in\Fbb^{m,n}:\rank A\leqslant r}.$ Id $v\otimes w$ with $vw^t.$\TextB{}
	Then $\Par{v_1w_1^t+\dots+v_rw_r^t}{_{\cdot,k}}=w_1[k]\cdot v_1+\dots+w_r[k]\cdot v_r\Rightarrow\Col\Par{v_1w_1^t+\dots+v_rw_r^t}=\Span{v_1,\dots,v_r}.$\vspace{1pt}\TextB{}
	Thus $\Rank\Par{v_1w_1^t+\dots+v_rw_r^t}\leqslant r.$ Let $N=\min\!\Bra{m,n}\Rightarrow R_1,\dots,R_{N-1}\neq V\otimes W$ id with $\Fbb^{m,n}=M_N.$\par\vspace{3pt}
	\BulletPointX\AComm For $r\in\!\!\;\Bra{1,\dots,N-1},$ $R_r$ is not a subsp of $V\otimes W,$ as $M_r$ is not a subsp of $\Fbb^{m,n},$\parCom{\IndentB}
