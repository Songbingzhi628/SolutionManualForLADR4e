...
\Anchor{6B'1}\ProblemB{
	\TextA{Supp $\Par{e_1,\dots,e_m}$ is orthog, and $v\in V.$ \,Show $\sum_{k=1}^m\!\BigPar{2-\Dvertsq{e_k}}\,\innerAsq{v,e_k}\leqslant\Dvertsq{v}.$\vspace{2pt}}
...
\Anchor{6B14}\ProblemN{14}{
	\TextA{Supp $\Par{e_1,\dots,e_n}$ is a orthon bss of $V$ and each $v_j\in V$ suth $\Dvert{e_j-v_j}<{}${\Large$\frac{\;1}{\sqrt{n}\;}$}.}
	\TextA{Prove $B_V=\Par{v_1,\dots,v_n}.$}
}Let $a_1v_1+\dots+a_nv_n=0.$ Note that each {\Large$\frac{\;\aMid{a_j}}{\sqrt{n}\;}$}${}\geqslant\BigDvert{a_j\Par{e_j-v_j}}.$\parSol{}
Then
$\sum_{j=1}^n\aMidsq{a_j}\Big/n\geqslant\sum_{j=1}^n\BigDvertsq{a_j\Par{e_j-v_j}}$\vspace{3pt}\parSol{}
\Blind{Then $\sum_{j=1}^n\aMidsq{a_j}\Big/n$}${}\geqslant\BigDvertsq{\sum_{j=1}^na_j\Par{e_j-v_j}}=\sum_{j=1}^n\aMidsq{a_j}\Longrightarrow\sum_{j=1}^n\aMidsq{a_j}=0$ by ctradic.\PfEnd\vspace{6pt}
\AComm If each $\Dvert{e_j-v_j}=1\Big/\sqrt{n}$, then $\Par{v_1,\dots,v_n}$ might not be a bss even if $n\neq1.$\parCom
Let $v_j=e_j-\Par{e_1+\dots+e_n}\Big/n\Rightarrow\Dvert{e_j-v_j}{^2}=1\big/n.$ \;Note that $v_1+\dots+v_n=0.$\PfEnd
\SepLine
