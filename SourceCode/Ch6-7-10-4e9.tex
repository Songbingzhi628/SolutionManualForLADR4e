...
%}Supp $\Par{T+\i\:\!I}\Par{T-\i\:\!I}{^{-1}}v=v.$\parSol{}
%Note that $\forall$ inv $S\in\Lm{V},\exists\,q_1,q_2\in\PoFi,\,S^{-1}=q_1\Par{S+\i I}=q_2\Par{S-\i I}.$\parSol{}
%Then $\Par{T-\i\:\!I}{^{-1}}\Par{T+\i\:\!I}v=v\Rightarrow\Par{T+\i\:\!I}v=\Par{T-\i\:\!I}v\Rightarrow v=0.$\parSol{}
%Now $\Sbra{\Par{T+\i\:\!I}\Par{T-\i\:\!I}{^{-1}}}{^*}=\Par{T+\i\:\!I}{^{-1}}\Par{T-\i\:\!I}=\Sbra{\Par{T+\i\:\!I}\Par{T-\i\:\!I}{^{-1}}}{^{-1}}.$\PfEnd
%\SepLine
...
\Anchor{7F16}\ProblemN{F.16}{
	\TextA{Supp $S\in\Lm{V}$ posi inv. Prove $\exists\,\delta>0$ suth if  $T$ self-adj and $\Dvert{S-T}<\delta,$ then $T$ posi.}
...
Note that for each $J=\Par{j_1,\dots,j_m}\in\Gamma\:\!\!,\:\alpha_I\Par{e_{j_1},\dots,e_{j_m}}=\delta_{I,J}\Rightarrow 0\Par{e_{j_1},\dots,e_{j_m}}=c_J.$\PfEnd
\SepLine
...
Let $A={}${\scriptsize$\begin{pmatrix}B &\hspace{-6pt} D\\[-2pt] 0 &\hspace{-6pt} C\end{pmatrix}$}$,\:P={}${\scriptsize$\begin{pmatrix}B &\hspace{-6pt} 0\\[-2pt] 0 &\hspace{-6pt} C\end{pmatrix}$}$,\:Q={}${\scriptsize$\begin{pmatrix}0 &\hspace{-6pt} D\\[-2pt] 0 &\hspace{-6pt} 0\end{pmatrix}$}$.$ Supp $A\in\Fbb^{n,n},B\in\Fbb^{m,m},C\in\Fbb^{p,p}.$\vspace{-2pt}\parSol{}
...
Becs each col of $D$ is in $\col B=\Fbb^m\Rightarrow$ each $Q_{\cdot,m+k}\in\Span{P_{\!\cdot,1},\dots,P_{\!\cdot,m}}\Rightarrow\det A=\det P.$\PfEnd
%\vspace{4pt}\parSol{}
%\Or Let $\Par{e_1,\dots,e_n}$ be std bses of $\Fbb^n.$ Let $U=\Span{e_1,\dots,e_m},W=\Span{e_{m+1},\dots,e_n}.$\parSol{}
%Define $T\in\Lm{\Fbb^n}:e_k\mapsto A_{\cdot,k}\Rightarrow T\mmid_U\in\Lm{U},$ and $\Mt{T\mmid_U}=B.$ \,Define $S\in\Lm{W}:e_{m+j}\mapsto C_{\cdot,j}$\parSol{}
...
Then \,$\det\Mt{I+\delta T_{\!\Cbb},B_V}=\Par{1+\delta\lambda_1}\cdots\Par{1+\delta\lambda_n}=1+\delta\Par{\lambda_1+\dots+\lambda_n}+\cdots+\delta^n\:\!\lambda_1\!\cdots\lambda_n$
%\parSol{}
%$\Rightarrow g\apostrophe\Par{0}=\lim_{\delta\rightarrow 0}\delta^{-1}\Sbra{g\Par{\delta}-g\Par{0}}=\lambda_1+\dots+\lambda_n.$
...
\Anchor{4e9C19}\ProblemN{C.19}{
	\TextA{Supp $V$ is inner prodsp, $B_V=\Par{e_1,\dots,e_n}$ is orthon, and $T\in\Lm{V}$ is posi.}
	\TextA{Prove $\det T\leqslant\prod_{k=1}^n\Ang{Te_k,e_k},$ and if $T$ inv, then equa $\Longleftrightarrow B_V$ are eigvecs.\vspace{2pt}}
}$\det T=\BigPar{{\det\sqrt{T}}}\BigPar{{\det\sqrt{T}}}\leqslant\prod_{k=1}^n\Dvertsq{\sqrt{T}e_k}=\prod_{k=1}^n\Ang{Te_k,e_k}.$\vspace{2pt}\parSol{}
Equa $\Longleftrightarrow\det\sqrt{T}\leqslant\prod_{k=1}^n\Dvert{\sqrt{T}e_k}\Longleftrightarrow\BigPar{{\sqrt{T}e_1,\dots,\sqrt{T}e_n}}$ orthog. \,Becs $T$ inv, $\nexists\,\sqrt{T}e_k=0.$\parSol{}
\SepLine

\Anchor{4e9C21}\ProblemN{C.22}{
	\TextA{Supp $n\in\Nbp,$ and $\delta:\Cbb^{n,n}\rightarrow\Cbb$ suth $\delta\Par{AB}=\delta\Par{A}\cdot\delta\Par{B},$ and $\delta\Par{D}=D_{1,1}\!\cdots D_{n,n},$}
	\TextA{for all $A,B\in\Cbb^{n,n}$ and all diag $D\in\Cbb^{n,n}.$ \;Prove $\delta={\det}.$}
}Supp $A\in\Cbb^{n,n}.$ Define $T\in\Lm{\Cbb^n}:e_k=A_{\cdot,k}\Rightarrow\Mt{T}=A$ wrto std bses.\parSol{}
Supp $T$ up-trig wrto $\Par{u_1,\dots,u_n}\Rightarrow B=\Mt{T,u\rightarrow u}=\Mt{I,e\rightarrow u}\Mt{T,e\rightarrow e}\Mt{I,u\rightarrow e}.$\parSol{}
\SepLine

\Anchor{4e9D}\ProblemN{D.}{
	\TextA{}
}
\SepLine
