...
\Anchor{7A11}\Anchor{7A4e20}\ProblemB{
	\TextB{Supp $P^2=P\in\Lm{V}.$ Prove {\tgnr\large(a)} $P=P_U\Rightarrow P$ is self-adj, \;{\tgnr\large(b)} $P$ is normal $\Rightarrow P=P_U.$}
}(a) $\forall\Par{u,w},\Par{x,y}\in U\times U^\perp,\:\BigAng{P\Par{u+w},x+y}=\Ang{u+w,x}=\BigAng{u+w,P^*\Par{x+y}}.$\parSol{}
(b) Becs $\range P=\range P^*=\Par{\null P}{^\perp}.$\PfEnd
\SepLine
