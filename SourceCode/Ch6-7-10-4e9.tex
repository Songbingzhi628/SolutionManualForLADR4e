...
Becs \,$s_n=\min\!\Bra{\Dvert{S-T}:T\in\Lm{V}\text{ and }\dim\range T=n-1}>\min\!\Bra{\Dvert{S-T}:T\in\Lm{V}\text{ inv}}.$\parSol{}
\SepLine
...
\Anchor{7F16}\ProblemN{16}{
	\TextA{Supp $S\in\Lm{V}$ is posi inv.}
	\TextA{Prove $\exists\,\delta>0$ suth if  $T\in\Lm{V}$ self-adj and $\Dvert{S-T}<\delta,$ then $T$ posi.}
}
\SepLine

\Anchor{7F22}\ProblemN{22}{
	\TextA{Supp $T\in\Lm{V,W}.$ Let $n=\dim V$ and $s_1\geqslant\cdots\geqslant s_n$ are the singvals.}
	\TextA{Prove $s_{n-k+1}=\min\!\Bra{\Dvert{T\mmid_U}:U\text{ is subsp of }V,\,\dim U=k}$ for $1\leqslant k\leqslant n.$}
}Get the SVD bses $\Par{e_1,\dots,e_n},\Par{\,f_1,\dots,f_n}.$ Let $U=\Span{e_{n-k+1},\dots,e_n}\Rightarrow\Dvert{T\mmid_U}=s_{n-k+1}.$\parSol{}
Asum $\exists$ $k$\hspace{1pt}-\hspace{1pt}dim subsp $U$ suth $\Dvert{T\mmid_U}<s_{n-k+1}.$ \,Get orthon $B_U=\Par{g_1,\dots,g_k}.$\parSol{}
Then $\forall u\in U$ suth $\Dvert{u}\leqslant 1,$ \,$\Dvertsq{Tu}=s_1^2\,\innerAsq{u,\:\!e_1}+\dots+s_m^2\,\innerAsq{u,\:\!e_m}<s_{n-k+1}^2.$\parSol{}
\PfEnd
\SepLine
...
Becs $T^*Te_k=s_k^2e_k,\;f_k=Te_k\big/s_k\Rightarrow TT^*\,f_k=TT^*Te_k\big/s_k=s_kTe_k=s_k^2\:f_k\Rightarrow\sqrt{TT^*}\,f_k=s_k\:f_k.$\vspace{1pt}\parSol{}
Now $\dim E\Par{s_k,\sqrt{T^*T}}\leqslant\dim E\Par{s_k,\sqrt{TT^*}}.$ Apply revly. Thus $\dim E\Par{0,\sqrt{T^*T}}=\dim E\Par{0,\sqrt{TT^*}}.$\vspace{1pt}\parSol{}
Get orthon bses of $E\Par{0,\sqrt{T^*T}}$ and $E\Par{0,\sqrt{TT^*}}.$ Forming two orthon bses of $V.$\parSol{}
Define $S\in\Lm{V}$ by $Se_j=f_j.$ \quad\ANote The same $S$ in $T=S\sqrt{T^*T}.$\PfEnd\vspace{3pt}
...
\Anchor{7F30}\ProblemN{F.30}{
	\TextA{Supp $T\in\Lm{V},$ and $S\in\Lm{V}$ unit suth $ST$ posi. Prove $T=S^*\sqrt{T^*T}.$}
}Becs $T=S^*T^*S^*.$\parSol{}
Write each $Te_k=s_k\,f_k=S^*T^*S^*e_k\Rightarrow s_kSf_k=T^*S^*e_k$ while $T^*f_k=s_ke_k.$\parSol{} 
\SepLine
