...
\LX{2} $\Ang{u+v,v}=\Ang{u,v}+\Ang{v,v}.$\Endl{}
\Blind{\LX{2}} $4\cdot LHS=\BigPar{\Dvertsq{u+v+v}+\Dvertsq{{-u+v+v}}}-\BigPar{\Dvertsq{{-u+v+v}}-\Dvertsq{u+v-v}}$\Endl{}
\Blind{\LX{2}} $\Blind{4\cdot LHS}=2\BigPar{\Dvertsq{u}+\Dvertsq{2v}}-2\BigPar{\Dvertsq{{-u+v}}+\Dvertsq{v}}$\Endl{}
\Blind{\LX{2}} $\Blind{4\cdot LHS}=6\,\Dvertsq{v}+2\,\Dvertsq{u}-2\,\Dvertsq{u-v}=4\Ang{v,v}+2\BigPar{\Dvertsq{u}+\Dvertsq{v}}-2\Dvertsq{u-v}=4\cdot RHS.$\ENDL{}
\LX{3} $\Ang{u,2v}=2\Ang{u,v}.$ \;So then $\Ang{2v,u}=2\Ang{v,u}.$\Endl{}
\Blind{\LX{3}} $4\cdot LHS=\BigPar{\Dvertsq{u+v+v}+\Dvertsq{u+v-v}}-\BigPar{\Dvertsq{u+v-v}+\Dvertsq{u-v-v}}$\Endl{}
\Blind{\LX{3}} $\Blind{4\cdot LHS}=2\BigPar{\Dvertsq{u+v}+\Dvertsq{v}}+2\BigPar{\Dvertsq{u-v}+\Dvertsq{v}}=4\cdot RHS.$\ENDL{}
{\tgbf Add:} \,$\Ang{u+w,v}=\Ang{u,v}+\Ang{w,v}.$\Endl{}
We show \:$\Dvertsq{u+w+v}-\Dvertsq{u+w-v}=\Dvert{u+v}+\Dvertsq{w+v}-\Dvertsq{u-v}-\Dvertsq{w-v}.$\Endl{}
$RHS={}${\Large$\frac{\:1\:}{2}$}$\BigPar{\Dvertsq{u+w+2v}+\Dvertsq{u-w}}-{}${\Large$\frac{\:1\:}{2}$}$\BigPar{\Dvertsq{u+w-2v}+\Dvertsq{u-w}}$\Endl{}
$\Blind{RHS}=2\,\Ang{u+w+v,v}-2\,\Ang{u+w-v,-v}$\Endl{}
$\Blind{RHS}=2\BigPar{\Ang{u+w,v}+\Ang{v,v}}-2\BigPar{\Ang{u+w,-v}+\Ang{{-v,-v}}}$\Endl{}
$\Blind{RHS}=2\,\Ang{u+w,v}+2\,\Ang{u+w,v}=LHS.$\ENDL{}
{\tgbf Homo:} \,$\Ang{\lambda u,v}=\lambda\Ang{u,v}.$ \;True by add if $\lambda\in\Nbb,$ and then by (1) if $\lambda\in\Zbb.$\Endl{}
Note that by add, $\;n\cdot\Ang{n^{-1}u,v}=\Ang{u,v}$ for $n\in\Nbp.$ Thus the case for $\lambda\in\Qbb^+$ holds, so for $\lambda\in\Qbb$.\Endl{}
We show the case for $\lambda\in\Rbb.$ \,By def, $\exists\,!\,\Par{r_n}\in\Qbb^{\infty}$ suth $\lim_{n\rightarrow\infty}r_n=\lambda.$\Endl{}
Becs $\aXMid{\Dvert{}}$\vspace{8pt}\parSol{}
If $\Fbb=\Cbb.$ Define $\Ang{u,v}={}${\Large$\frac{\:1\:}{4}$}$\BigPar{\Dvertsq{u+v}-\Dvertsq{u-v}+\Dvertsq{u+\i\,v}\i-\Dvertsq{u-\i\,v}\i}.$\vspace{2pt}\parSol{}
{\tgbf Conjug Symm:} $\,4\,\Imaginary\Ang{u,v}={\Dvertsq{u+\i\,v}\i-\Dvertsq{u-\i\,v}\i}={\Dvertsq{v-\i\,u}\i-\Dvertsq{v+\i\,u}\i}=-4\,\Imaginary\overline{\Ang{v,u}}.$\Endl{}
We show $\,4\,\Ang{u,\i\,v}=\Dvertsq{u+\i\,v}-\Dvertsq{u-\i\,v}=\Dvertsq{v-\i\,u}-\Dvertsq{v+\i\,u}=-4\,\Ang{v,\i\,u}.$\Endl{}
...
{\IndentTips}(i) $k=1.$ $\Span{e_1}=\Span{u_1}\Rightarrow u_1=\Ang{u_1,\,e_1}\,e_1,$ 又 $\innerA{u_1,\,e_1}=1.$ Let $c\Par{1}=\Ang{u_1,\,e_1}.$\TextB{}
{\IndentTips}(ii) $k>1.$ Asum each $\aMid{c\Par{i}}=1$ and $c\Par{i}e_i=u_i$ for $i\in\!\Bra{1,\dots,k-1}.$\TextB{}
...
Then $e_k=f_k\Big/\;\!\!a_k\Rightarrow\Ang{\,f_k,\,f_k}\big/a_k^2=1\Rightarrow a_k=\pm\Dvert{\,f_k}.$ 又 $a_k>0.$ \:Hence each $e_k=f_k\Big/\Dvert{\,f_k}.$\PfEnd\vspace{3pt}\parSol{}
